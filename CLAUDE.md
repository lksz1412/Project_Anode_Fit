...
8. **코드 = 부록 전용** (사용자 반복 지시 · F-11): 코드 함수명 · 클래스명 · 변수 식별자 (`func_*`, `solve_U_oc`, `_regsol`, `\code{}`, `\texttt{}` 등)가 **본문 절 (§) 에 등장하지 않는가** — 코드 대응은 **부록 (코드 대응 지도 · 코드 절) 에만**, 본문은 물리량 · 수식 기호로만 서술. 검증 = 부록 아닌 `_sections/*.tex` grep 코드토큰 **= 0** (확인 가능 게이트). rubric A5 · `CODE_MENTION_AUDIT` 승계를 CLAUDE.md 게이트로 명문화 (v1.0.24 신규 절 위반 재발 방지)
