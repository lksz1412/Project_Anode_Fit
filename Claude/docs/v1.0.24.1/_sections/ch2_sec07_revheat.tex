...
분리하고(\S\ref{ssec:balance}), 그 부호와 ``방전'' 라벨의 층위를 규정한 뒤(\S\ref{ssec:signlabel}), 가역 발열을
...
본 절의 부호는 라벨이 아니라 전류 부호 $I$ 로 읽는다(충전은 $I<0$ 으로 자동 처리 --- 가역 항은 $I$ 에
선형이라 부호가 뒤집히고, 소산 항은 $U_\oc-V$ 가 함께 뒤집혀 $\ge0$ 이 유지된다). 이 라벨 충돌이 본 장에서 부호가 가장 크게 어긋날 수 있는 자리이며, 부록 C 의 함정표가 이
층위를 다시 정리한다.
\begin{srcbox}
...
(식~\eqref{eq:single_config})이자 가역 발열 $\dot Q_\rev$ 의 배치 몫이다. 곧 $\dd Q/\dd V$ 봉우리(fluctuation)와 가역
...
%   [C-101] 해석 가역발열=분포 재배열·SOC 흡발열 교대·2→1 발열(전위차법 standardised2024 — C-047 오귀속 정정 반영)
