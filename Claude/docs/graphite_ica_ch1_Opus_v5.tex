...
\label{eq:eyring}
\end{equation}
$k_0=k_BT/h$ 는 열적 시간척도 $h/k_BT$ 의 역수($25^\circ$C 에서 약 $6\times10^{12}\ \mathrm{s^{-1}}$). $\simeq$ 인 까닭은 $k_0$ 가 $O(1)$ 수 인자$\cdot$활동도$\cdot$유효 시도 자유도를 흡수하는 현상론적 prefactor 로 작동하기 때문이다 — 이 흡수가 ``절대값은 prefactor 에 묻고 의존성만 뽑는'' 피팅 전략(\S\ref{sec:tempbranch})의 근거다.
