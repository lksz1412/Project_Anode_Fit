...
\label{eq:eyring}
\end{equation}
$k_0=k_BT/h$ 는 열적 시간척도 $h/k_BT$ 의 역수($25^\circ$C 에서 약 $6\times10^{12}\ \mathrm{s^{-1}}$). $\simeq$ 인 까닭은 $k_0$ 가 $O(1)$ 수 인자$\cdot$활동도$\cdot$유효 시도 자유도를 흡수하는 현상론적 prefactor 로 작동하기 때문이며(``절대값은 prefactor 에 묻고 의존성만 뽑는'' 전략의 근거, \S\ref{sec:tempbranch}).
...
\label{eq:bv}
\end{equation}
정$\cdot$역에 독립 계수 $\chi,\chi'$ 를 허용해 비를 만들면, 모든 구동력 $\mathcal A_j$ 에서 Boltzmann 점유비와 같아야 하므로
\begin{equation}
...
(그림~\ref{fig:fluxcross}). 기본형 $V_\drive=V_n$($V_n$ 은 \S\ref{sec:charge} 에서 정의)에서 $\mathcal A_j=sF(V_n-U_j)$ 를 넣고 $\xi_{\sstat,j}$ 에 대해 풀면
\begin{equation}
\xi_{\sstat,j}
...
% ====================================================================
logistic 등온선~\eqref{eq:logistic} 은 $\Omega=0$ 결과였다. 식~\eqref{eq:mu} 의 $\Omega$ 항($\Omega\ne0$ 이 staging 의 본질)이 등온선을 비트는 끝에서 상분리에 도달한다 — plateau(ICA peak 의 정체)와 히스테리시스의 뿌리.
...
식~\eqref{eq:gbar} 를 $\xi=1-\theta$ 로 옮기되 곡선 모양을 바꾸지 않는 $\xi$ 의 1차 몫을 상수 $g_j^0$ 와 함께 떼면
\begin{equation}
...
\label{eq:cotangent}
\end{equation}
이 두 접점 $\xi_b^\pm$ 가 \textbf{binodal(공존 조성)}. $g_j$ 가 $\xi=\tfrac12$ 대칭이라 공통 접선이 수평($g_j'=0$)이고, 식~\eqref{eq:gprime} 로
\begin{equation}
...
\label{eq:unique}
\end{equation}
— 우변이 $V_n$ 의 단조 증가 함수이므로 해는 있다면 하나뿐이고, 잔여 전하가 배경 치역 안에 있는 한 존재도 보장된다($C_\bg>0$ 은 배경 단상 안정성의 귀결). 평형에서는 $\xi_j=\xi_{\eq,j}(V_n)$ 이라 우변 기울기에 $Q_j\,\dd\xi_{\eq,j}/\dd V_n$ 이 더해지는데, \S\ref{sec:regsol} 의 $\Omega_j>2RT$ 비단조 등온선은 이 기울기가 음이 되는 구간을 가져 한 $V_n$ 에 평형 해가 공존한다 — ``OCV $=$ 보존식의 평형 해''는 단상에서는 유일해, 상분리 구간에서는 이력이 고르는 분기에 대한 조건부 명제이며, 이 다가성이 \S\ref{sec:hys} 분기의 관측축 쪽 얼굴이다.
...
\label{eq:eqpeak}
\end{equation}
위치 $=$ 전이 중심 $U_j$, 순높이 $=Q_j/(4w_j)$, 배경 차감 면적 $=Q_j$($\int_0^1\dd\xi=1$ — 분리 peak 기준, 겹침 합산은 \S\ref{sec:overlap}). 너비는 별도 지표 없이 면적/높이 비로 읽으며, $w_j$ 는 이상 극한에서 $RT/F$($25.7$ mV, $25^\circ$C), 상호작용 시 식~\eqref{eq:weff} 의 $w^{\eff}$ 로 좁아진다.
...
\label{eq:tailrate}
\end{equation}
— 같은 지수형, 진폭 $r_{a,j}/L_{q,j}$. 전압축으로 옮기면 — 관측 꼬리 몫은 연쇄율(\S\ref{sec:charge}) $Q_j(\dd\xi_j/\dd q)/(\dd V_n/\dd q)$ 이고, post-peak 국소 기울기로 $q-q_a=(V_n-V_{a,j})/|\dd V_n/\dd q|_{q_a}$ 를 식~\eqref{eq:tailrate} 지수에 대입하면
\begin{equation}
...
\label{eq:dlnk}
\end{equation}
식~\eqref{eq:v5-potbranch-2} 의 직렬 합성 $\ln k_j^{\eff}=-\ln[1/k_j+1/k_{j,\lim}]$ 을 $V_\drive$ 로 미분하면 전위 의존이 있는 $1/k_j$ 항만 살아
\begin{equation}
...
\label{eq:v5-overlap-1}
\end{equation}
둘째가 저온$\cdot$고율 겹침의 주범이다($L_{V,j}\propto|I|\,e^{\Delta H_a/RT}$ — 저온일수록 낮은 전류에서 융합 시작; 그림~\ref{fig:fusion}). $L_{V,j}=|\dd V_n/\dd q|_{q_a}L_{q,j}$ 에 식~\eqref{eq:LqT} 의 $L_{q,j}$ 를 넣고 융합 문턱 식~\eqref{eq:v5-overlap-1} 을 등호로 두어 $|I|$ 에 대해 풀면
\begin{equation}
...
% ====================================================================
\S\ref{sec:hyspol} 의 식~\eqref{eq:hysobsgap} 까지로 조각이 준비됐다 — 합산~\eqref{eq:total} 의 각 항을 식~\eqref{eq:simplefit} 로 채운 방전 한 줄 식을 닫고, 분기 중심~\eqref{eq:hyscenter} 과 분극 부호(식~\eqref{eq:vapp})만 방향별로 바꿔 통합형으로 넓힌 뒤, 측정 파일에서 파라미터까지 가는 알고리즘을 이 절만 보고 코드를 짤 수 있는 사양 수준으로 적는다.
...
\label{eq:dHeff}
\end{equation}
($\chi_d$ 는 방전 $\chi$, 충전 $1-\chi$ — 충전 깊은 꼬리는 $\xi\to0$ 거울이라 $r^-$ 장벽의 상수 몫이 같은 $+\Omega$ 로 흡수.) 복원은 둘 — 히스 전이는 S5 가 $\Omega_j$ 를 닫은 뒤 식~\eqref{eq:dHeff} 를 역산, 분기 없는($0<\Omega_j\le2RT$) 전이는 단상 폭 절편 $-\Omega_j/2F$(식~\eqref{eq:weff})에서 읽는다.
