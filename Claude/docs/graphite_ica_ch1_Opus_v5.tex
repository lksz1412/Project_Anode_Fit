...
\end{equation*}
둘째가 저온$\cdot$고율 겹침의 주범이다($L_{V,j}\propto|I|\,e^{\Delta H_a/RT}$ — 저온일수록 낮은 전류에서 융합 시작; 그림~\ref{fig:fusion}). $L_{V,j}=|\dd V_n/\dd q|_{q_a}L_{q,j}$ 에 식~\eqref{eq:LqT} 의 $L_{q,j}$ 를 넣고 융합 문턱 $L_{V,j}=|U_{j+1}-U_j|$ 을 등호로 두어 $|I|$ 에 대해 풀면
\begin{equation}
