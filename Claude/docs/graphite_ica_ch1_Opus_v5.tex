...
\label{eq:eqpeak}
\end{equation}
위치 $=$ 전이 중심 $U_j$, 순높이 $=Q_j/(4w_j)$(방전 $s=+1$ 크기 — 충전 가지는 부호만 반전, 통합형 \S\ref{sec:master}), 배경 차감 면적 $=Q_j$($\int_0^1\dd\xi=1$ — 분리 peak 기준, 겹침 합산은 \S\ref{sec:overlap}). 너비는 별도 지표 없이 면적/높이 비로 읽으며, $w_j$ 는 이상 극한에서 $RT/F$($25.7$ mV, $25^\circ$C), 상호작용 시 식~\eqref{eq:weff} 의 $w^{\eff}$ 로 좁아진다.
