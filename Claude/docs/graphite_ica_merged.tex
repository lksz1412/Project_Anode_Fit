...
$V_\app=V_n+s_I|I|R_n$ 에서, 방전($s_I=+1$)은 음극 전위를 $|I|R_{n,j}$ 만큼 올리고 충전은 내리므로 관측 peak 중심은
\begin{equation}
V_{\mathrm{peak},j}^{\dis}=U_j^\dis+|I|R_{n,j},\qquad V_{\mathrm{peak},j}^{\chg}=U_j^\chg-|I|R_{n,j},
\label{eq:hys_obspeak}
\end{equation}
\begin{equation}
\boxed{\;\Delta V_{\mathrm{obs},j}(|I|)=\underbrace{\gamma_j\,\Delta U_j^\hys}_{\text{열역학}(|I|\to0\text{ 잔존})}
+\underbrace{2|I|R_{n,j}}_{\text{동역학}(|I|\to0\text{ 소멸})}.\;}
\label{eq:hys_obsgap}
\end{equation}
\textbf{이 분리가 본 장의 실용 핵심}이다: 여러 율에서 gap 을 재어 $|I|\to0$ 으로 외삽하면 \emph{절편}이 열역학
gap $\gamma_j\Delta U_j^\hys$(\S\ref{sec:regsol} 의 $\Omega_j$$\cdot$$\gamma_j$), \emph{기울기}가 $2R_{n,j}$ 다.
$R_{n,j}$ 는 ohmic$\cdot$전하이동$\cdot$확산을 한 양으로 묶은 \emph{lumped} 저항이며(전이별 1개), 본 장 피팅은 그
...
$d\in\{\dis,\chg\}$, 부호 $\sigma_d=+1\,(\dis)/-1\,(\chg)$ 로
\begin{equation}
...
\begin{itemize}[leftmargin=1.4em,itemsep=2pt]
\item $\Omega_j\le2RT$(단상, \S\ref{sec:regsol})면 $\Delta U_j^\hys=0$ 라 $U_j^\dis=U_j^\chg=U_j$ — 식~\eqref{eq:hys_master}
  가 양방향 모두 방전식~\eqref{eq:master} 로 \emph{되돌아간다}(히스 없음). 히스는 상분리의 산물임이 식에서 드러난다.
...
$\to\Omega_j$, 기울기$\to R_{n,j}$ 교차확인(식~\eqref{eq:hys_obsgap}). 각 단계가 앞 단계 기지값을 써 미지수를
