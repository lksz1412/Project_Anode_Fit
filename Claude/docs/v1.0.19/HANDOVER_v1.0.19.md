...
## ② 다음 세션 주의
1. **★버전 분기(의도적)**: v1.0.19 = **Ch1 문건만 재작성**. Ch2·appendix = 18.2 승계 복제(미변경, 버전태그만 1.0.19). **코드 = 18.2 유지(frozen, 이번 범위 밖)** — `docs/v1.0.19/Anode_Fit_v1.0.18.2.py` 그대로. 즉 **문서(1.0.19) vs 코드(1.0.18.2) 버전 일시 분기**. Ch1 물리는 18.2 코드 모델과 정합(코드에 없는 새 물리 X). **차기 = 코드·Ch2·appendix를 신본 Ch1에 맞춰 재-sync**(이월).
...
## ③ 미완료/이월
- **코드·Ch2·appendix 재-sync**: 이번 Ch1 재작성 확정 → 코드(현 18.2)·Ch2·appendix를 신본 Ch1 구조/서술에 맞춰 재정합(차기 사이클). 문서/코드 버전 재통일.
