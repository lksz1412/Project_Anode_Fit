...
절의 $\partial S_\vib/\partial x$ 가 아니다 --- 두 양의 혼동을 절 전체에서 피한다. 이렇게 얻은 온도 확장은 이후
...
점유가 $n=1/(e^{u_j}-1)$ 이다(비 $u_j$ 로 적어 본 장의 리튬 함량 $x$ 와 구분한다). 대표값의 선택 --- 예컨대
...
항을 남기기 때문이다. 대신 모드 자유에너지 편차에서 출발해 중심 이동을 닫는다. 모드 Helmholtz 자유에너지
편차를
\[
...
\Delta F_\vib/\partial T$ 로 자유에너지의 온도 미분으로 회복되므로, 이를 $T_\mathrm{ref}$ 를 기준으로 적분해
평형 중심 이동 $\Delta U_\vib(T)$ 를 얻는다. 적분 상수를 $\Delta U_\vib(T_\mathrm{ref})=0$ 으로 고정하면
\begin{equation}
\boxed{\;
...
(식~\eqref{eq:Se}, $\propto T$)은 $T$ 에 선형이고 강제 영점이 없어, 다온도 $dQ/dV$ 곡률 피팅이 두 함수형을
