...
본 문건은 이 곡선을 \emph{한 번 그리는 계산}을 처음부터 끝까지 그대로 따라간다. 계산은 외부 실험조건을
모델 입력으로 환산하는 첫 단계(노드 $\mathrm N0$)에서 출발해, 전이별 물리식을 차례로 평가하는 본론
($\mathrm N1$--$\mathrm N9$)으로 진행한다(그림~\ref{fig:spine}). 각 절은 그 한 단계의 물리식을, 그 단계를
