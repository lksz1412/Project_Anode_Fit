...
고온 ($\kB T\!\sim\!E_F$ 근접) & electronic $\propto T$ 우세화 & Fermi--Dirac--Sommerfeld~\eqref{eq:Se} 는
...
세 가지가 본 장의 골격을 닫는다: (i) 중심 $\xi=\tfrac12$ 에서 config$=0$ 이라 파생 B 의 분리가 자기일관적이다;
...
극한이 분포 식을 검산했으니, 이제 이 $\partial U_\oc/\partial T(x)$ 를 가역 발열로 환산할 차례다.
