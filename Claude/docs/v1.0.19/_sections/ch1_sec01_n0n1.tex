...
흑연 staging 네 전이의 초기값(문헌 기반, 표~\ref{tab:staging})이 이 네 전이에 대응한다. 이 그림이 서론에서
...
결과로만 받아 쓰는 평형식들($\theta$-점유$\cdot$logistic$\cdot$평균장 $\Omega$$\cdot$Nernst)의 통계역학 유도를
Part 0(\S\ref{sec:sm-found})이 처음부터 놓는다.
