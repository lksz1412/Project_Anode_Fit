% ====================================================================
% 부록 E — 자기일관 해법: ratio 닫힘과 전달함수 (v1.0.23 신설)
...
L_V\!\downarrow$)과 정합한다. 미지 $\xi=\xi_\eq-r$ 이 감쇠율 $\kappa$ 안에 들어가 \textbf{미지량이 핵 안팎에 동시
...
수치 검산(P1, \code{comp\_v23/p1\_ratio\_check.py}): $g=0$ 에서 $\lVert r_1-r_0\rVert$ 가 이산화 바닥($\sim10^{-8}$),
$g$ 축소 시 동결오차 $\mathrm{err}_0\sim\mathcal O(\varepsilon)$ 대비 보정오차 $\mathrm{err}_1\sim\mathcal O(\varepsilon^2)$
--- ratio 치환이 선행 차수 오차를 제거함(논문 ``비는 함수보다 근사에 둔감''의 수치 확증).
\end{derivbox}
...
\label{eq:sc-valid}
\end{equation}
이 성립할 때 1차 닫힘이 제어된 근사다. $\varepsilon$ 은 자기일관 반복(Picard)의 수축률과 일치한다 --- 수치(P1):
$\varepsilon=0.073$ 에서 스텝당 오차감소 $\approx0.10$, $\varepsilon=0.28$ 에서 $\approx0.43$, $\varepsilon\to0.84$ 에서
$\approx0.88$(수축 정지 근방). 논문 Sec.~III 의 세 타당성과 1:1 로 대응한다:
\begin{center}
...
실이득창은 \emph{중간 전류}($0.1\lesssim L_V/w\lesssim0.6$)로, 여기서 1차가 동결오차를 $3$--$10\times$ 감축한다.
\end{warnbox}
...
($|H(\omega_c)|=1/\sqrt2$). 수치(P1, SURV): FFT 대조 상대오차 $\sim10^{-9}$(기계정밀), $\omega_c$$\cdot$반출력점 재현,
본문 \code{fig:reversal} 자기정합. 한 문장으로: \emph{관측 $\dd Q/\dd V$ 는 평형 곡선을 (상태의존) 저역통과 필터에
통과시킨 것}이다.
...
본 부록의 수식과 구현(\code{Anode\_Fit\_v1.0.23.py})의 대응은 다음과 같다(부록~\ref{sec:appendix-code} 의 지도 확장).
...
감쇠율 $\kappa(\xi)$ (\eqref{eq:sc-true}) & (상태의존 옵션) & $L_V(\xi)=L_V^{(0)}e^{2\chi_d(\Omega/RT)(1-\xi)}$ \\
1차 ratio 보정 $r_1$ (\eqref{eq:sc-ratio-local}) & (선택적 분석 경로) & 기준 궤적 $\xi_0$ 위 변계수 선형 --- 한 전진 패스 \\
전달함수 $H(\omega)$ (\eqref{eq:sc-transfer}) & (FFT 경로) & 동결 기준해의 주파수형(기기응답) \\
\bottomrule
\end{longtable}
