...
% ====================================================================
\subsection{Si-host 정칙용액 커널 --- Frumkin 단일상 peak(\texorpdfstring{$\Omega\!<\!2RT$}{Omega<2RT} 고용체)}\label{ssec:si-frumkin}
...
로 옮겨가 제일원리 계산이 이를 재현한다\cite{chevrier_dahn2009,wang_asi2013,mcdowell_coreshell2013}. 곧 순환 a-Si
...
gallery(전이)로 이산화한 것이라, 각 전이의 $\Omega_j^\mathrm{Si}$ 는 $\Omega\!<\!2RT$ 이되 넓은 고용체를 향해
...
(단상 고용체가 miscibility gap 문턱에 닿는 임계점, 식~\eqref{eq:staging-mujudge} 판정자와 같은 $2RT$); $\Omega_j\!>\!2RT$
는 두-상 영역이라 이 커널의 유효 범위 밖(그 때는 \S\ref{sec:broadening} 두-상 폭이 담당). (iv) \emph{부호$\cdot$면적.}
