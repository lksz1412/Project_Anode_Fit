...
peak(단일상). $\Omega$ 가 $2RT$ 로 다가갈수록 peak 정점이 $Q F/(4RT-2\Omega)$ 로 \emph{솟고}(sharpen, 곧 폭이
...
다중도 $n_j$(아래 (c))다; $\Omega<2RT$ 라는 부등호는 폭을 넓히는 손잡이가 아니라 ``새 상 없이 한 종''임을 보장하는
...
(아래 정직한 한계 (i))이면 식~\eqref{eq:sifr-kernel} 분모의 $-2\Omega$ 가 중심 분모를 줄여 peak 를 높이고
...
그 넓은 종이 두-상으로 갈라지지 않게 하는 \emph{단일상 가드}일 뿐(폭을 넓히는 손잡이가 아니며, 가드 경계
...
하나뿐이다: 결정질 Si 의 \emph{1차} 심리튬화 끝 $\sim$50--60 mV 의 $c$-Li$_{15}$Si$_4$ 결정화 plateau 는 진짜 두-상
...
단일 두-상 결정화''로 갈리며, 본 커널은 앞쪽을 담고 뒤쪽은 first-cycle 전용 feature 로 분리한다(honest scoping).
...
직접 결과}다(그 유한 폭의 \emph{크기}는 다중도 $n_j$ 가 정하지 $\Omega$ 가 아니다 --- (a)$\cdot$(c)). 또한
제일원리$\cdot$기계학습 퍼텐셜 계산이 a-Li$_x$Si 의 연속 경사 $U(x)$ 를 독립 재현한다\cite{chevrier_dahn2009,artrith2018}.
...
로지스틱으로 되지 않나.''} $\Omega{=}0$ 로지스틱은 폭이 $n_jRT/F$ 로만 정해지는 \emph{대칭 종}(모양 고정)만 내지만,
a-Si 종은 그 위에 평균장 $\Omega$ 가 주는 \emph{날카로움(peakedness)} 자유도 --- sharpen($0<\Omega<2RT$)$\cdot$broaden
($\Omega<0$) --- 하나가 더 필요하다. 이 $\Omega$ 는 \emph{모양(날카로움)} 손잡이이지 넓은 폭의 출처가 아니다(넓은
...
\begin{warnbox}
\textbf{정직한 한계.} (i) \emph{$\Omega_\mathrm{Si}$ 는 범위 시드일 뿐 점-식별 안 됨.} 물리가 확정하는 것은
...
$[Q]{\cdot}\mathrm{C/mol}\div\mathrm{J/mol}={[Q]}/\mathrm V$ 로 차원 정합한다. $\Omega>2RT$ 는 단일상 가드
$\Omega<2RT$ 를 넘겨 본 커널의 \emph{유효범위 밖}(분모 비단조$\cdot$부호 반전 $=$ van der Waals 고리)이라, 그
영역의 두-상 peak 은 이 커널에 $\Omega>2RT$ 를 대입해 얻지 않고 흑연처럼 broadening 이 정하는 현상학적 종
(\S\ref{sec:width} 이중지위 둘째 지위)으로 다룬다.
\end{warnbox}
...
\emph{정직 한계}: $\Omega_\mathrm{Si}$ 는 범위 시드($0.2RT\lesssim\Omega<2RT$, 인력)일 뿐 점-식별되지 않고
