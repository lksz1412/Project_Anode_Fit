% ====================================================================
% 부록 C — 열특성 기호·부호 함정 검산표 (재수록 전용, 신규 자산 없음)
% ====================================================================
\section*{부록 C --- 열특성 기호$\cdot$부호 함정 검산표}
\addcontentsline{toc}{section}{부록 C --- 열특성 기호·부호 함정 검산표}
\phantomsection\label{sec:traps-thermal}
...
본 장 부호 사슬 검산표(\S\ref{sec:signcheck})와 같은 용도다.
...
\S\ref{ssec:dvdt} \\
$\theta$ vs $\theta_E$ &
$\theta=$ 점유율(무차원), $\theta_{E,j}=$ Einstein 온도[K] --- 아래첨자 $E$ 가 구분자 &
같은 글자라 점유율로 오독(차원부터 다른 별개 양) &
\S\ref{sec:einstein} \\
$\bar x$ vs $x$ &
...
$g_j(\xi)$(앞 파트 조성 자유에너지)$\cdot$$g(\theta)$(BW)$\cdot$$g(E_F)$(상태밀도)와 혼동 &
\S\ref{ssec:overlap} \\
$F_\vib$ vs $F$ &
...
$u_j$ vs 앞 파트 $u_j$ &
Einstein 절 $u_j\equiv\theta_{E,j}/T$(진동 비 --- 절-국소 기호) &
앞 파트 \S\ref{sec:hys} spinodal 근 $u_j=\sqrt{1-2RT/\Omega_j}$ 와 동명 별개(같은 문서 내 --- 정의 절이 다름) --- 혼용 금지 &
\S\ref{ssec:einstein-closed} \\
``방전($I>0$)'' &
Bernardi 셀-수지 관례 $=$ 흑연 하프셀 \emph{리튬화}; 부호는 라벨 아닌 전류 $I$ 로(충전 $I<0$) &
Part I ``방전 $=$ 탈리튬화'' 라벨과 \emph{같은 단어 반대 방향} 혼동 --- 최대 부호 위험 &
...
LCO 장(Chapter 2)의 동명 식 \code{eq:Se}(별개 장 객체)와 이름 혼동 &
\S\ref{ssec:elec} \\
\bottomrule
\end{longtable}
