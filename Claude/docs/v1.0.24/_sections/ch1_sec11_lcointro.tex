% ====================================================================
% §11 Chapter 2 도입 — 전극-중립 골격과 방향 규약 (N0') [구 "Part II 도입" — 2026-07-18 사용자 결정 정합화]
% ====================================================================
\section{Chapter 2 도입 --- 전극-중립 골격과 방향 규약}\label{sec:lco-intro}
Part I(흑연)이 세운 forward 골격은 처음부터 전극을 가리지 않도록 설계되었다. 이 절은 그 골격을 두 번째 전극
$\mathrm{LiCoO_2}$(LCO) 양극에 걸기 전에, Chapter 2 전체가 전제할 세 가지 --- 무엇이 전극 무관인가
(\S\ref{sec:lco-map}), 방향 부호를 어떻게 먹이는가(\S\ref{sec:lco-direction}), 종반의 MSMR 동형은 어떤
모양으로 닫히는가(\S\ref{sec:lco-preview}) --- 를 한 곳에 고정한다. 이하 Chapter 2 의 절들은 이 규약을
전제하고 각자의 자리에서 적용만 한다.

\subsection{전극-중립 골격 --- 전극 무관 요소}\label{sec:lco-map}
...
흑연에서 $\xi_j$ 가 탈리튬화 진행이었듯, LCO 양극에서 충전(탈리튬화)이 $\xi:0\!\to\!1$ 의 주 진행 방향이다 --- 본 장의
...
등급 표기(본서 전역): tier A $=$ 1차 문헌의 정량 확정값, tier B $=$ 대표/부분 스케일 anchor 또는
...
과제는 \S\ref{sec:lco-code}). ★실측 근거(L2): T1 조성창 0.94--0.75 와 electronic 기원은 tier-A
실측\cite{menetrier1999}(XRD$\cdot$NMR), O3 질서상 조성($x{=}\tfrac12\cdot\tfrac56$)$\cdot$엔트로피 삼분해는 tier-A
실측\cite{reynier2004}, order--disorder $\Delta S$ 서명은 entropymetry 실측\cite{kim_entropymetry2020} ---
config $\Delta S$ \emph{수치}(0.47/1.49)는 원전 재확인 중이라 tier C 유지(\S\ref{sec:lco-hys} 혼동 가드).}
...
T2 (order--disorder a)& $\sim$4.05        & $\approx$0.55 & hex$\to$monoclinic 정렬 & config 주도($\approx$0.47 J/(mol\,K); 값$\cdot$배정 tier C) & 미발현 \\
T3 (order--disorder b)& $\sim$4.17--4.20  & $\approx$0.48 & monoclinic$\to$hex      & config($\approx$1.49 J/(mol\,K); 값$\cdot$배정 tier C) & 미발현 \\
\midrule
...
의 새 절이다. N0--N9 전 사슬에서 흑연과 LCO 는 \emph{같은 식}을 공유하므로 --- Part I 은 흑연으로 식을 세웠고,
Chapter 2 의 절들은 LCO 파라미터를 끼워 넣는 순서로 간다.
...
분극$\cdot$분기$\cdot$꼬리 --- 다. 이 구분이 Chapter 2 전체의 부호 실수를 막는 가드다.
...
파라미터화라, Chapter 2 종반에서 Chapter 1 곡선 클래스와 1:1 대응표 --- $U\leftrightarrow V$,
