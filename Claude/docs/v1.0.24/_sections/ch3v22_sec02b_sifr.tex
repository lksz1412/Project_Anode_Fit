% ====================================================================
% ch3v22_sec02b_sifr.tex — [SI-FR] Si-host 정칙용액(Frumkin, Ω<2RT 고용체) 커널
%   삽입: §3.2 케이스별 활물질(ch3v22_sec02_cases, sec:si-cases) 뒤. v1.0.24 신규(@3 반영).
%   저자 = comp_R1 W9 초안(master 통합판). 신규 cite: artrith2018·verbrugge2017(ch3v22_bib 등재).
% ====================================================================
...
tier-B 진단값이지 문헌 anchor 가 아니다). 이는 흑연 두-상 전이의 평형 등온선 폭(near-delta plateau,
...
합으로 이산화해 근사한다. \textbf{폭의 출처(정밀)} --- 각 종의 \emph{넓은} 폭은 다중도 $n_j{>}1$(넓은 gallery,
$w_j{=}n_jRT/F$)에서 오지 $\Omega<2RT$ 에서 오지 않는다: 인력 $\Omega{>}0$ 는 오히려
$w_\mathrm{eff}{=}(RT/F)(1-\Omega/2RT)$ 로 폭을 \emph{좁힌다}. 곧 폭 판정~\eqref{eq:sifr-width} 의 $\gtrsim1$ 을
지는 것은 $\Omega$ 가 아니라 $n_j$ 이며, $\Omega<2RT$ 는 그 넓은 종이 두-상으로 갈라지지 않게 하는
\emph{단일상 가드}일 뿐이다. 이렇게 넓은 종들이 겹쳐 연속 경사를 재구성한다(케이스별 개형은
...
로지스틱으로 되지 않나.''} $\Omega{=}0$ 로지스틱은 폭 $RT/F$ 의 고정 종만 내지만, a-Si 종은 $\Omega\ne0$ 로
sharpen($0<\Omega<2RT$)$\cdot$broaden($\Omega<0$)하는 \emph{폭 조율}이 필요하다 --- 단일 Frumkin 종은
$\theta{=}\tfrac12$ 대칭이며, 케이스별 개형의 \emph{비대칭}은 폭이 다른 여러 종의 포갬(envelope)에서 온다
(그림~\ref{fig:si-cases-shape}). 커널 분기가 이 폭 자유도 하나를 열되, 미지정 시 로지스틱 폴백으로 bit-exact
회수돼 흑연 골격을 손상하지 않는다.
...
$\Omega_k^\mathrm{Si}$ 매핑은 피팅 단계에서 확정한다(구조 대응이지 값 1:1 아님). (v) \emph{커널 유효범위$\cdot$차원.}
분모 $RT/[\theta(1-\theta)]-2\Omega$ 는 단위가 [J/mol]이라 $QF/|\cdot|$ 가 $[Q]{\cdot}\mathrm{C/mol}\div\mathrm{J/mol}={[Q]}/\mathrm V$ 로
차원 정합한다. $\Omega>2RT$ 는 본 커널의 \emph{유효범위 밖}(분모 비단조$\cdot$부호 반전 = van der Waals 고리)이라,
그 영역의 두-상 peak 은 이 커널에 $\Omega>2RT$ 를 대입해 얻지 않고 흑연처럼 broadening 이 정하는 현상학적 종
(\S\ref{sec:width} 이중지위)으로 다룬다.
\end{warnbox}
