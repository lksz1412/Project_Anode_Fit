...
내지만, 이 커널의 구현(\code{\_regsol\_dqdv})은 그 고리를 \emph{Maxwell 공존(binodal)} 구성으로 해소한다 ---
공존 조성 $\theta_a$(\code{\_regsol\_binodal\_xa}; $\Omega\!\le\!2RT$ 면 $\theta_a{=}\tfrac12$ 로 공존역 없음,
...
a-Si 는 상성격이 단일상($\Omega<2RT$)이라 이 공존 분기를 타지 않을 뿐이며(그래서 위 (a)$\cdot$(c)는 단일상
