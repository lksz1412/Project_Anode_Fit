...
\textbf{(c) 중간식 --- 여집합 항등식과 방향 인자 대응.} Chapter 1 logistic 서식은 여집합에 대해 닫혀 있다 ---
같은 중심$\cdot$폭에서
\begin{equation}
...
$\theta_j^{\mathrm{MSMR}}=x_j/X_j$(리튬화 분율)는 Chapter 1 의 점유 $\theta_{\eq,j}=1-\xi_{\eq,j}$ 와, 그 여집합
$\xi_j^{\mathrm{MSMR}}$(탈리튬화 진행률)는 Chapter 1 의 진행률 $\xi_{\eq,j}$(식~\eqref{eq:xieq})와 같은 종류의
...
대응표~\eqref{eq:lco-msmrmap} 아래에서 MSMR 종별 미분용량과 Chapter 1 평형 peak 은 같은 식이다:
\begin{equation}
...
따라서 Chapter 1 의 곡선 골격(평형 진행률 logistic$\cdot$평형 중심$\cdot$합산식~\eqref{eq:sum})은 구조 변경 없이
