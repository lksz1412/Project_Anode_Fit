...
불변, 식~\eqref{eq:lcoomega-Tref}). 기본을 \textbf{off}(전자항 제외)로 두는 것은 사용자 사양(``커브 구할 땐
빼고''$\cdot$브리프 ``기본 OFF'')을 따른 것이며, 상온 커브가 어차피 전자항과 무관하므로 곡선 산출엔 값 선택이
무의미하다. 토글이 실제로 가르는 것은 오직 $\partial U/\partial T$(가역열$\cdot$다온도)다 --- 회사가 \textbf{on 옵션}
으로 전자항의 \emph{가역열 기여}를 켜서 정량화하고 최종 유지/제거를 결정하는 ``있고 없고 선택'' 장치다.
v1.0.19/v1.0.23 의 상시-ON 거동은 \textbf{on} 경로가 bit-exact 로 보존하며 G1 회귀가 그 ON 경로로 검증한다
