...
\textbf{True (옵션)} & \Rightarrow\ \partial U_j/\partial T\ \text{에 }\Delta S_{e,j}(T_\mathrm{ref})\ \text{상수 오프셋 활성}\ (U\text{-이동 }T\text{-선형)} = \text{v1.0.19/v1.0.23 상시-ON 거동}\\
              & \phantom{\Rightarrow\ }\ (\text{완전 }\Delta S_{e,j}(T)\propto T\to U\text{-이동}\propto T^2,\ \text{식~\eqref{eq:U1T2}}\ \text{는 다온도 round-trip 미구현 타깃; }G1\text{ 은 이 ON 경로 검증}).
\end{cases}\;}
...
v1.0.19/v1.0.23 의 상시-ON 거동은 \textbf{True} 경로가 bit-exact 로 보존하며 G1 회귀가 그 ON 경로로 검증한다
(기본 OFF 의 $U(T_\mathrm{ref})$ 불변 보장 $=\Delta H^\mathrm{eff}$ 재기준, R2 코드 게이트 소관).
...
식~\eqref{eq:br-vanderven1998-1} 원식 불변).
...
\code{include\_electronic\_entropy} 는 \emph{기본 False}(전자항 제외$\cdot$상온 커브 불변)$\cdot$\emph{True 옵션}
(가역열에 전자항 활성 = v1.0.19 거동, G1 회귀가 ON 경로로 검증)으로 둔다(식~\eqref{eq:lcoomega-toggle}).
