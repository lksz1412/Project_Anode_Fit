...
% §4 히스테리시스 분기 중심 (N3)
% ====================================================================
\section{히스테리시스 분기 중심}\label{sec:hys}
평형 중심 $U_j$ 는 상호작용 $\Omega_j$ 와 분기 인자 $\gamma_j$ 가 있으면 방향별 분기 중심 $U_j^{\,d}$ 로 이동한다
(그렇지 않으면 $U_j$ 그대로). 본 장의 주 전개는 방전이며, 히스테리시스는 그 위의 \emph{구조적 분기}로 선언된다 --- 방전과 충전이
...
\subsection{상호작용에 의한 평형 이동 --- $\mu(\theta)$ 의 상호작용 몫}\label{sec:hys-spinodal}
...
이중웰의 기원, 식~\eqref{eq:sm-thresh}). \textbf{(b) 연산 --- 두 번 미분.}
...
--- 두 경우를 하나의 명시 분기 $\Omega_j\le2RT$ 로 묶는다). 부호 --- 극대 $>$ 극소라 $\Delta U_j^\hys\ge0$ 이고, $\Omega_j\to2RT^+$ 에서
...
도핑에 의한 $\Omega_j$ 감소(식~\eqref{eq:lco-dope})가 같은 축 위에서 gap 을 닫는 두 조절 변수다.
...
두 인자의 지위를 명확히 하면 --- spinodal 상한은 결함$\cdot$fluctuation$\cdot$불균일이 전혀 없는 이상 단일 입자가
안정성 한계까지 버틴다는 극한이고, 실재 전극은 대개 그 전에(국소 fluctuation$\cdot$불균일 핵생성으로) 가지를
...
그림을 \S\ref{sec:hys} 과 잇는 반대 방향 서술을 이미 둔다). 곧 결함$\cdot$fluctuation$\cdot$불균일이 없는
이상 단일 입자의 상한 $\gamma_j=1$(spinodal)에 대해, 실재 전극이 국소 fluctuation$\cdot$불균일 핵생성으로
