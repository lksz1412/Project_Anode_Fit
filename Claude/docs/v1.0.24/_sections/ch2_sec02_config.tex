...
앞 절은 점유 분포 $\avg{n}=\theta$ 를 세우고 그 여집합 $\xi=1-\theta$ 가 Part I 의 평형 종임을 보였다. 이제
...
낳는다는 것을 이 절이 유도한다. 문헌 흑연 측정과의 정합으로 절을 닫는다(\S\ref{ssec:litverif}).
...
\subsection{온도 미분 $\partial V/\partial T|_\xi$ --- $w$ 에 담긴 config}\label{ssec:dvdt}
...
\begin{scope}
  ...
  \node[below] at (0,-0.55) {\small (a) occupation distribution = Part I logistic};
\end{scope}
...
\end{scope}
\end{tikzpicture}
\caption{(a) 단일 자리 점유 분포 $\langle n\rangle$~\eqref{eq:occ} --- Fermi--Dirac 와 동형이며 Part I
logistic 평형 점유(\S\ref{sec:width})의 기원. (b) configurational 엔트로피 $S_\config(\theta)$~\eqref{eq:Sconfig}; 중심
...
봉우리 내부의 이 분포 항이 전이 중심 표준값과 어떻게 나뉘어 세어지는지를 여기서 규정한다. 이것이 본 장에서
...
흡수)이고 MIT(insulator-to-metal transition) 같은 급변이 있을 때만 유지한다(\S\ref{ssec:elec}). 이 분해는
Chapter 2 \S\ref{sec:lco-decomp} 의 반응 엔트로피 삼분해$\cdot$슬롯 규칙과, \emph{중심 표준값과 봉우리 내부 config 분포를 별개
출처로 가르는} 골격을 공유한다(세 성분의 슬롯 배정 전부가 아니라 config 중심/분포 분리에 한정한 대응이다).
단 그 절의 $\Delta S_j^0$ 는 vib$\cdot$electronic 을 별개 슬롯으로 분리한 분해의 \emph{config 슬롯 중심값}이고,
본 장의 $\Delta S^0_j$ 는 세 중심을 합산 흡수한 \emph{전체} 중심값이다 --- 두 기호를 직접 등치하지 않는다.
\end{warnbox}

\subsection{문헌 검증 --- Part I staging 엔트로피와의 정합}\label{ssec:litverif}
...
\smallskip
{\footnotesize $\dagger$ Part I 의 4 주요 plateau 전이와 Allart 등 \cite{allart2018} 의 세분 region(dilute
