...
\begin{keybox}
\textbf{이식의 실이득.} 문건 지연의 자기일관은 Markov(지수핵)라 \emph{참 문제 자체가} $\mathcal O(N)$ 전진 ODE 다
--- 동결 0차$\cdot$1차 ratio$\cdot$참 비선형 Picard 수렴이 모두 $\mathcal O(N)$. 따라서 이 이식의 의의는 논문에서의
