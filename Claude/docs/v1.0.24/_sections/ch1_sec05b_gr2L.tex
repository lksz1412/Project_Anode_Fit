...
과 맞춘다). 실제로
단일상($\Omega<2RT$) peak 의 유효 폭은 $w_\mathrm{eff}=(RT/F)(1-\Omega/2RT)$ 라(판정자~\eqref{eq:gr2l-disc} 의
$\theta{=}\tfrac12$ 강성에 반비례), $\Omega\!\to\!2RT$ 에서 $w_\mathrm{eff}\!\to\!0$ 으로 좁아져 두-상 델타로
수렴한다 --- 폭식~\eqref{eq:wbase} 의 이중지위가 이 한 식으로 연속화된다.
...
(\code{GRAPHITE\_STAGING\_XRD\_v1024}, \S\ref{sec:sum-staging} 4-전이 기준과 병존)를 두되, 다음을 명시한다:
...
(iii) \emph{명칭 대응}(gate-7 --- 혼동 금지): 피팅 기준선은 어디까지나 표~\ref{tab:staging} 의 \emph{네 행}
$\{4\!\to\!3,\ 3\!\to\!2\mathrm L,\ 2\mathrm L\!\to\!2,\ 2\!\to\!1\}$(코드 \code{GRAPHITE\_STAGING\_LIT}, 무변경)이고, 별도
5-feature 시드(\code{GRAPHITE\_STAGING\_XRD\_v1024})는 그 \emph{네 전이 라벨을 계승하고} 고전위 끝에 dilute
$1'\!\to\!4$ 특징 \emph{하나만} 더 얹는다(5 $=$ 4 라벨 $+$ 추가 1). 다만 시드의 U-\emph{위치}는 표의 표시 위치를
그대로 쓰지 않고 \emph{XRD 해상도로 재유도}된다 --- 코드 사다리는 dilute$\approx$0.210, $4\!\to\!3\approx$0.170,
...
(\code{GRAPHITE\_STAGING\_LIT}, 무변경)과 혼동해선 안 되며, 시드 U-위치는 그 기준선이 아니라 XRD 사다리다.
...
posit 하지 않으므로 정당하며, 코드에 \emph{선택} 시드 \code{GRAPHITE\_STAGING\_MSMR6\_LIT}(6-gallery, $\omega_j\cdot U0_j$
문헌 anchor)로 병존한다 --- 곧 해상도 사다리는 \{4-전이 기준(기본)$\cdot$5-feature XRD$\cdot$6-gallery MSMR\}이고,
어느 것도 4-전이 피팅 기준선을 바꾸지 않는다(전부 additive opt-in).
...
6+ 는 XRD 미지원으로 폐기하나, gallery$\ne$상이라 6-gallery MSMR 곡선표현(코드 \code{GRAPHITE\_STAGING\_MSMR6\_LIT}, opt-in)은
