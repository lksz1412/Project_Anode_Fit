...
$\{4\!\to\!3,\ 3\!\to\!2\mathrm L,\ 2\mathrm L\!\to\!2,\ 2\!\to\!1\}$ 이고, 5-feature 시드는 그 \emph{네 행을 라벨
그대로 $1{:}1$ 로 물려받은 위에} 고전위 끝에 dilute $1'\!\to\!4$ 특징 \emph{하나만} 더 얹은 것이다(5 $=$ 표의
4 $+$ 추가 1). 이 더한 한 특징은 \S\ref{sec:broadening-class} 이 이미 \emph{고용체}로 분류$\cdot$명명한
dilute$\to$stage4 영역이고(표는 이를 별행으로 두지 않고 고전위 끝에 접어 둔다), 표의 네 행은 어느 것도
개명$\cdot$재배치되지 않는다 --- 곧 시드의 다섯 라벨(XRD 구조 해상도)을 표의 네 전이 기준선(피팅 기준)과
혼동해선 안 된다. \emph{5 개를 넘어 6+ 로 쪼개는 것은 XRD 미지원 curve-fitting} 이라 폐기한다(봉우리를
...
하나만 더 얹는(5 $=$ 4 $+$ 1) \emph{추가 후보}(선택 시드, 상성격은 \S\ref{sec:broadening-class} 소관)이며 6+ 는 XRD 미지원으로 폐기.
\end{keybox}
