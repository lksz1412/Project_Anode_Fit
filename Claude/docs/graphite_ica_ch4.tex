...
\textbf{(C4) 구동력$\cdot$detailed balance.} $\mathcal A_j=s\,n_j^{\eff}F(V_\drive-U_j)$(Ch3 일반형,
$n_j^{\eff}=RT/(Fw_j)$; Ch1 이상형 $sF$ 는 $n_j^{\eff}=1$ 특수형; 기본 $V_\drive=V_n$),
...
$\mathcal A_j^{\chem}$ 의 scale 인자로만 붙는다.) 이는 Chapter 1 의 이상형 $\mathcal A_j=sF(V_\drive-U_j)$($n_j^{\eff}=1$) 와
