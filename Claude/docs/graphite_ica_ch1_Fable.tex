...
닫힌다. 그러나 \emph{관측} $\dd Q/\dd V_n$ 은 한 순간이 아니라 \emph{궤적}을 따라 보는 양이므로, $\xi_j$$\cdot$$V_n$
...
plateau 라 더욱 날카로움, \S\ref{sec:hys_peak}). 따라서 S1 의 OCV peak 판독에서 \emph{상한 sanity-check}(읽은 FWHM 이
$91$ mV 를 넘으면 이상 폭보다 넓음=꼬리 오염 신호)로만 쓰고 — \emph{등호로 닫지 않는다}; 다음 절 융합 판정
...
\end{enumerate}
\textbf{분리 가능성의 한계.} peak 가 완전히 융합하면 $j$별 \emph{면적}($Q_j$) 분리는 고율 데이터만으로는 불가능하다(\S\ref{sec:synth} 공선형 논리의 겹침판). \emph{저율 OCV anchor 가 있을 때만} 융합 peak 의 합산 정보를
...
\S\ref{sec:regsol} 의 상분리다: 충$\cdot$방전이 서로 다른 준안정 분기(spinodal 과주행)를 따라가기 때문이다.
...
\end{itemize}
즉 충방전 $\dd Q/\dd V$ 를 겹쳐 그리면 \emph{면적$\cdot$폭이 같고 중심만 $\gamma_j\Delta U_j^\hys$ 어긋난} 거울 쌍이 전이마다 보인다 —
...
일반화한 것 — 방전 $s_I=\sigma_d=+1$, 충전 $\sigma_d=-1$)로 쓴다. \textbf{주의 — $\sigma_d$ 는 \emph{분극에만} 작용한다}:
