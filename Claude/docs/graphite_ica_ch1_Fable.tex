...
일어나는 전위에서 $\dd Q/\dd V$ 에 \textbf{peak} 가 선다. 왜 상전이가 peak 가 되는가 — 일차 상전이 동안 두 상이
공존하면 전위는 거의 \emph{평탄}하게 머무는데 전하는 계속 흘러 들어가므로, 좁은 $\dd V$ 구간에 큰 $\dd Q$ 가 몰려
미분 $\dd Q/\dd V$ 가 그 전위에서 \emph{치솟는다}. 즉 ``전위 평탄 구간 $=$ ICA peak''가 본 장 전체가 다루는 대상의
정의다. 관측되는 유효 전이는 $j=1,\dots,N_p$ 개이며
...
\textbf{도착점은 하나의 통합 식이다.} 본 장은 충방전 분기의 물리를 유도한 뒤(\S\ref{sec:hys_branch}), 마지막에
\emph{방전} $\dd Q/\dd V$ 한 줄 피팅식과 그것을 \emph{충전$\cdot$방전 양방향}으로 넓힌 통합식을 한 절에서 닫는다
(\S\ref{sec:master}) — 히스테리시스 분기까지 \emph{같은 파라미터}로. 어느 측정(저율 OCV$\cdot$전류 차단$\cdot$다율
$\cdot$다온도$\cdot$충방전 gap)이 어느 파라미터를 닫는지까지 매듭짓는다. 그에 이르는 논리의 뼈대는 다음 순서다:
\begin{quote}
전하 보존이 내부 전위 $V_n$ 을 정의(\S\ref{sec:stage} — 모든 식의 전위축) $\to$ 평형 상승부(\S\ref{sec:eqpeak},
이상 극한) $\to$ \textbf{정규용액 상호작용$\cdot$상분리}(\S\ref{sec:regsol};
...
합산(\S\ref{sec:overlap}) $\to$ \textbf{충방전 분기}(\S\ref{sec:hys_branch}) $\to$ 통합식(\S\ref{sec:master}).
\end{quote}
...
규약(방전 시 분극이 apparent 전위를 올리는 방향을 $+$ 로; 본 장 방전 기준 $s_I=+1$). 충방전 \emph{양방향}을 다룰 때의
방향 부호 $\sigma_d=\pm1$ 은 \S\ref{sec:master} 에서 도입한다 — $s$ 는 평형 곡선 정의에 $+1$ 로 고정되고(곡선은 방향
불변 상태함수), $\sigma_d$ 는 분극 부호와 분기 중심 선택에만 작용한다.
...
아니라 — 방전은 탈리튬화로 Li 를 \emph{빼낸다} — 전하 \emph{수지}다). 계상의 근거는 Faraday 법칙이다: 외부로 전자
하나가 나갈 때마다 음극에서 Li$^+$ 하나가 빠져나가므로, 흘린 전하 $=F\times$(빠진 Li 몰수)가 \emph{항등}이다. 빠진
Li 는 \emph{서로소인 두 종류의 자리}에서 나온다 — 그래서 합이 \emph{선형}으로 갈라진다(교차항 없음):
...
\label{eq:cbg}
\end{equation}
가 양이면 충분하다. $C_\bg>0$ 의 물리적 출처: 배경 몫은 staging 으로 묶이지 않는 자유도 — 희박 고용체 영역의 연속
삽입, 표면$\cdot$결함 자리, 전기이중층 — 인데, 이들은 \emph{단상 안정 조건} $\partial\mu/\partial(\text{조성})>0$ 을 만족하는
한 ``전위가 낮아질수록 Li 가 더 들어가는'' 단조 응답을 가지므로 미분용량이 양수다(동어반복이 아니라 열역학 안정성의
귀결). 정확히는 우변이 (i) $V_n$ 에 연속, (ii) 순증가($C_\bg>0$),
...
비평형의 미분 경로를 구분해야 self-consistent 고리($V_n\!\to\!\xi_{\eq}\!\to\!V_n$)가 모순 없이 닫힌다. 분모
$\dd V_n/\dd q\to0$ 인 \emph{이상적 plateau} 에서 연쇄율이 발산하는 것은 결함이 아니라 서론의 ``전위 평탄 구간 $=$
peak'' 그 자체다 — 무한히 날카로운 이상 peak 를 유한 폭으로 만드는 것은 뒤의 broadening(평형 폭 \S\ref{sec:eqpeak}
$\cdot$분포 \S\ref{sec:dist}$\cdot$동역학 \S\ref{sec:lag})이고, 연쇄율은 $V_n(q)$ 가 구간 단조인 곳에서 정의된다.
...
Q_j\,\dd\xi_{\eq,j}/\dd V_n$ 인데 — \textbf{단상 영역}($\Omega_j<2RT$, $g_j''>0$; 임계 $\Omega_j=2RT$ 는 $\xi=\tfrac12$ 한 점서 $g''=0$ — 기울기 발산이나
유일성은 유지)에서는 $\dd\xi_{\eq,j}/\dd V_n>0$
(다음 절)라 더 가파른 단조라 해가 유일하다. \textbf{그러나 상분리 영역}($\Omega_j>2RT$)에서는 \emph{spinodal 불안정 구간}($\xi_{s,j}^-<\xi<\xi_{s,j}^+$, 거기서만
$g_j''<0$)에서 $\dd\xi_{\eq,j}/\dd V_n=sF/g_j''<0$ 이라 $V_{\eq,j}(\xi)$ 가 \emph{비단조}(loop)가 되고(\S\ref{sec:regsol};
양 어깨 $g_j''>0$ 은 양의 기울기), 우변이 비단조라 \emph{한 $V_n$ 에 여러 해}가 가능하다 — 이 \emph{다가성}이 곧
충$\cdot$방전 분기(히스테리시스, \S\ref{sec:hys_branch})의 뿌리다.
...
\label{eq:eqcond}
\end{equation}
\textbf{부호의 물리적 의미.} 좌변 $\mu_\mathrm{Li}$ 는 점유율이 높을수록 크고(이상/단상 $\Omega_j<2RT$ 한정 —
$\Omega_j>2RT$ 의 spinodal 구간은 \S\ref{sec:regsol}), $V_n$ 이 오르면(전자 일 $-FV_n$) 평형
...
드러난다). 부호 $s$ 는 유도되는 양이 아니라 \emph{진행률 정의에 묶인 규약}이다 — 방전 기준 $\xi_j=1-\theta_j$ 와 함께
$s=+1$ 로 두면 $\dd\xi_{\eq,j}/\dd V_n>0$(식~\eqref{eq:dxidV})이 되어 ``방전서 $V_n\uparrow\Rightarrow$ 진행$\uparrow$''
관측과 \emph{일치}한다(관측은 규약을 고정하는 게 아니라 규약의 자연스러움을 확인한다). 충방전 양방향에서도 $s$ 는
$+1$ 로 고정되고 방향은 $\sigma_d$ 가 맡는다(\S\ref{sec:notation} 부호 규약$\cdot$\S\ref{sec:master}).
...
\emph{전개 순서}의 선택이다. $\Omega_j$ 항은 \S\ref{sec:regsol} 에서 \emph{임계까지 포함해} 도입한다(작은 $\Omega_j$ 는
폭 보정의 섭동으로, $\Omega_j>2RT$ 는 비섭동 상분리로). 폭을 $w_j\equiv RT/F$ 로
...
는 본 단일-phase logistic 로 기술되지 않는다(흑연 staging 의 핵심 물리 — 본 \emph{단일상 logistic} 의 범위 밖이고,
\S\ref{sec:regsol}$\cdot$\S\ref{sec:hys_peak} 가 다룬다). ``Gaussian
...
때문이다(엄밀히는 동역학 꼬리가 평형 wing 보다 긴 $L_{V,j}\gtrsim RT/F$ 의 꼬리-우세 조건 하 — 그 미만이면 원거리를
진폭 $e^{-\Omega_j/RT}$ 로 억제된 평형 wing 이 지배하나 그땐 꼬리 자체가 관측되지 않는다)(꼬리 \emph{시작 위치$\cdot$진폭}은 peak 근처 평형 형태에 일부 의존). 형태 판별은 \S\ref{sec:falsify}.
\end{flagbox}
...
(\S\ref{sec:synth} S1). \emph{단 높이 $Q_j/(4w_j)$$\cdot$FWHM $3.53w_j$ 의 logistic 공식은 \textbf{단상}($\Omega_j<2RT$)
한정}이고, 상분리 staging peak($\Omega_j>2RT$)에서는 $w_j$ 가 평형 폭이 아니라 broadening 현상학 폭(\S\ref{sec:hys_peak}
$\cdot$\S\ref{sec:master})이라 그 logistic 높이/면적 공식을 그대로 쓰지 않는다(중심 $U_j$$\cdot$용량 $Q_j$ 만 직접 운반).
...
\emph{동종 자리 선호}(이종 쌍 불리)라 분리를 부추긴다. 이 최근접쌍 분해는 평균장 어림이고, 실제 흑연 staging 의
상호작용은 층간 탄성 변형 같은 \emph{장거리} 성분(Daumas--H\'erold 도메인의 기원)을 포함하므로, $\Omega_j$ 는 그것까지
한 쌍-에너지로 뭉뚱그린 \emph{유효} 척도로 읽는다(부호$\cdot$규모의 안내이지 미시 정량 예측이 아니다). $g_j$ 의 점유율 미분이 곧 앞 절 식~\eqref{eq:mu} 의
$\mu_\mathrm{Li}(\theta)$ 다(정확히는 $\partial g_j/\partial\theta=\mu_\mathrm{Li}-\mu^0$ — 상수 $\mu^0$ 차) —
\emph{같은 $g_j$ 를 한 번은 peak(미분)로, 한 번은 상분리(곡률)로 본다}.
...
비대칭해질 뿐 ``오목 구간 하나 $\Rightarrow$ 극소 둘''은 유지된다). 경계는 $g_j''$ 이 가장 작은 $\xi=\tfrac12$ 서 $4RT-2\Omega_j=0$, 곧
...
\textbf{전위로 보면 van der Waals 등온선.} 이중 우물이면 평형 전위 $V_{\eq,j}(\xi)$(평형 조건~\eqref{eq:eqcond} 를
$V_n$ 에 대해 푼 것 — 닫힌 꼴은 식~\eqref{eq:hys_Veq})가 \emph{비단조} loop 를 그린다 — 압력--부피 van der Waals 등온선과 \emph{동형}이다($g_j\leftrightarrow$ Helmholtz
...
같은 물리를 변수만 바꿔 본 것). 음의 기울기 구간(spinodal 안쪽)이 vdW 의 \emph{압축률이 음}($\partial P/\partial V>0$, $\kappa_T<0$)인
불안정 가지에, 공통 접선이 Maxwell 등면적 작도(=평탄 전위)에 대응한다.
...
같은 중심 기울기라도 전역 모양이 다르다: \emph{반치 영역}은 matched logistic 보다 좁아져 $3.53\,w_j^\eff$ 가 FWHM 을
\emph{과대} 추정하고(편향은 $\Omega_j\to2RT$ 서 커짐), \emph{원거리 wing} 은 감쇠 길이가 $\Omega_j$ 무관 $RT/F$($>w_j^\eff$)
로 고정된 채 진폭만 $e^{-\Omega_j/RT}$ 로 억제돼 오히려 \emph{무겁다}(점근: $1-\xi\simeq e^{-\Omega_j/RT}e^{-F(V_n-U_j)/RT}$).
중심 곡률만 정확, 전역 모양은 근사다. 즉 상호작용이 클수록($\Omega_j\to2RT$) peak 가 \emph{좁아지고},
...
기준선이 이상($\Omega=0$) $\to$ 폭 좁아짐($0<\Omega<2RT$) $\to$ 임계$\cdot$상분리($\Omega\ge2RT$)의 \emph{한 모수축}
($\Omega_j/2RT$)으로 닫혔다} — 단 $\Omega_j=2RT$ 는 매끄러운 연장이 아니라 단상$\leftrightarrow$2상이 갈리는 \emph{질적
경계}(임계점)임에 유의한다. 이제 전류가 흐를 때의 \emph{이탈}(동역학)로 간다.
...
\label{eq:Lq}
\end{equation}
여기 $k_j=r_j^++r_j^-$ 이나, 꼬리 영역($\mathcal A_j\gtrsim RT$)에서는 \emph{forward 우세}라 $k_j\simeq r_j^+\equiv k_j^{\mathrm{fwd}}$
(식~\eqref{eq:keff})로 읽는다 — 즉 본 식과 아래 모든 꼬리 표현의 $k_j$ 는 이 forward 속도 $k_j^{\mathrm{fwd}}$ 다(이는
\S\ref{sec:barrier} 의 \emph{직렬 율속} $k_j^\eff$ 와 다른 양 — $k_j^\eff$ 는 $k_{j,\mathrm{act}}\!\simeq\!k_j^{\mathrm{fwd}}$ 를 공율속과
직렬 합성한 것). peak 중심의
...
몫만 추가 — \emph{혼합 엔트로피 항은 넣지 않는다}: 식~\eqref{eq:relax} 의 점유 prefactor $(1-\xi_j)\cdot\xi_j$ 가
이미 그 통계를 담당하므로, 전체 $\mu_\mathrm{Li}$ 차를 쓰면 logit 이 이중 계상된다) 일반화하면 같은 논법이
...
저온($RT\downarrow$)일수록 길이 산포가 커진다. 고정된
...
$e^{-(x/L_{\mathrm{KWW}})^\beta}$($0<\beta<1$; $L_{\mathrm{KWW}}$=KWW 척도 — \S\ref{sec:dist} rigorbox 의 $L_0$ 와 별개)이 되는 것은 $\rho_G$ 가 특정 두꺼운-꼬리 분포(KWW 의 역-Laplace 핵)일 때}이며 $\beta$ 는
...
$\int f\rho_G\,\dd G$(식~\eqref{eq:wavg})로 기술된다. 길이가 배리어의 지수이므로 배리어 표준편차는 로그-길이 표준편차
$\sigma_G/RT$ 로 전파되고(식~\eqref{eq:varprop}) 저온에서 꼬리가 더 늘어진다. 배리어 분포는 길이 분포로 분율
...
\item \textbf{배리어 분포.} 한 값 대신 \emph{분포} $\rho_G(G)$ 로 기술한다(\S\ref{sec:stattools} 의 밀도;
  정규화 식~\eqref{eq:norm} $\int_0^\infty\rho_G(G)\,\dd G=1$). $\rho_G$ 는 꼬리에서 역산하지 않고 입자 크기$\cdot$국소
  상태 분포에 용량-반경 매핑을 곱해 \emph{독립으로 모형}하는 용량-분율 분포다(\S\ref{sec:stattools} 식별 주의$\cdot$아래
  boundbox). (구동력 $\mathcal A_j$ 는 집단 공통 —
  동일 평균장 $V_n$ — 이라 분포는 배리어 $G$ 에만 둔다; 이는 이상형 $\mathcal A_j=sF(V_\drive-U_j)$ 기준이고,
  $\Omega\ne0$ 일반화의 $\xi_j$ 의존 몫은 꼬리 포화창서 $\Delta\xi$ 가 작아 1차 무시.)
\item \textbf{집단별 꼬리 길이.} 장벽 $G$ 인 집단은 식~\eqref{eq:Lq},~\eqref{eq:keff} 로 자신의 꼬리
  ...
  \emph{선형} 전파다(``지수적 증폭''은 변환 $L\!\sim\!e^{G/RT}$ 자체를 가리키지 로그축 분산 전파가 지수란 뜻이 아니다).
...
좁을 때의 \emph{1차 근사}다(그 상관은 꼬리를 더 무겁게 미는 방향이라 stretched 결론을 약화시키지 않는다).
...
% ====================================================================
위 여섯 \emph{물리} 절(전하 보존 $\to$ 평형 $\to$ 정규용액 $\to$ 지연 $\to$ 유효 배리어 $\to$ 분포; 통계 도구절
\S\ref{sec:stattools} 제외 — 정규용액은 본 절의 \emph{방전 단일 전이} 닫힌식엔 $w_j$ 의 물리적 근거(식~\eqref{eq:weff})로,
충방전 분기엔 \S\ref{sec:hys_branch} 로 들어간다)를 묶으면 $\dd Q/\dd V$ 가 $(V;\,T,|I|,V_\drive)$ 의 닫힌 함수가 된다. peak 의
...
상한}이다 — 실측 staging peak 는 더 좁다(단상 $\Omega_j<2RT$ 면 $w^\eff$ 로, 식~\eqref{eq:weff}; 상분리 $\Omega_j>2RT$ 면
plateau 라 더욱 날카로움, \S\ref{sec:hys_peak}). 따라서 $S_1$ 의 OCV peak 판독에서 \emph{상한 sanity-check}(읽은 FWHM 이
$91$ mV 를 넘으면 이상 폭보다 넓음=꼬리 오염 신호)으로만 쓰고 — \emph{등호로 닫지 않는다}; 다음 절 융합 판정
...
$\{U_j,w_j,Q_j\}$ 가 \emph{서로 직교한 peak 모멘트}(위치=중심(1차 모멘트), 너비=중심 곡률, 면적=0차 모멘트(적분))라 동시에 분리되고,
...
\S\ref{sec:dist} 비유일성과 정합). \emph{분기 영역 확장}: 본 절 합산은 단상$\cdot$임계($\Omega_j\le2RT$, 전이당 peak 하나)를
전제로 적었으나, $\Omega_j>2RT$ 전이는 충$\cdot$방전 \emph{두} 중심 $U_j^{\dis/\chg}$ 의 peak 쌍(\S\ref{sec:hys_peak})이라
$\sum_j$ 가 방향별 중심 각각으로 확장된다 — 통합식~\eqref{eq:hys_master} 의 $\sum_j$ 가 그 겹침까지 포함한다.
...
\item \textbf{용량의 직접성}: ICA peak \emph{면적}은 곧 $Q_j$(전이 용량)다 — DVA 의 vs-$V$ 좌표에선 valley 폭으로
  간접적이다(단 $\dd V/\dd Q$ 를 \emph{vs-$Q$} 로 그리면 경계 간 $\Delta Q$ 로 $Q_j$ 직독 — 좌표 선택의 문제).
...
이유다 \cite{dubarry2012,bloom2005} — 경계의 급변이 \emph{형상}으로 도드라져 간격 변화의 판독이 쉽기 때문(통계적
우월이 아니라 표현상 이점, 위 (i)).
...
\S\ref{sec:hys_partial}). $\Omega_j$ 와 $\gamma_j$ 한 양씩이면 충분하다. \textbf{식별 주의}: 관측 gap 절편은
$\gamma_j\Delta U_j^\hys$ 의 \emph{곱}이라 한 온도의 절편만으론 $\gamma_j$ 와 $\Omega_j$ 가 분리되지 않는다 — \emph{1차
경로는 절편의 온도 의존}이다(식~\eqref{eq:hys_dU} 의 $\Delta U_j^\hys(T)$ 형이 $\Omega_j$ 를 정하고, 크기가 $\gamma_j$
를 정한다 — \S\ref{sec:master} S5$\cdot$파라미터 표와 동일; $\gamma_j$ 는 측정창서 $T$-무관 전제). 단상 온도 구간이
있으면 평형 폭 $w^\eff(T)$(식~\eqref{eq:weff})가 \emph{검증} 경로가 되나, 히스가 있는 전이는 그 영역서 $\Omega_j>2RT$
라 폭-경로가 무효임에 유의(같은 $g_j''$ 사영이라 어차피 \emph{내적 일관성}용, \S\ref{sec:falsify}).
\end{boundbox}
...
\label{eq:hys_dQdV}
\end{equation}
여기서 $\dd\xi_{\eq,j}/\dd V_n$ 는 종 모양(폭 $w_j$)이고 중심만 $U_j\to U_j^{\dis/\chg}$ 로 옮겼다. \textbf{단 형상의
...
절편과 기울기가 섞여 분리 불가다. $\gamma_j$ 평탄 가정의 물리 조건은 \emph{핵생성$\cdot$전환 시간상수 $\ll$ 구동 시간 $1/$율}이다 — 그러면 과주행이
\emph{핵생성 장벽이 정하는 율-무관 수준}($\gamma_j<1$, spinodal 아래)에 고정된다(빠른 핵생성 $=$ \emph{작은} 과주행으로
포화이지 spinodal 도달이 아님 — 위 boundbox 와 정합). 위배(고율)는 과주행이 율 따라 더 밀려 gap–$|I|$ 곡선의 곡률로
검출된다.
...
고율서 gap–$|I|$ 곡선의 기울기가 줄며 \emph{아래로(오목)} 휘면 선형 근사가 기각된다($\ln$ 포화 곡률 $=$ ct 우세 신호;
\emph{위로} 휘는 것은 확산 한계$\cdot$$\gamma_j(|I|)$ 쪽 신호다). 율 의존은 \S\ref{sec:lag} 지연
꼬리와 같은 뿌리다.
\begin{flagbox}
...
$\{r_{a,j}^\chg,L_{V,j}^\chg\}$ 다(리튬화 동역학이 대칭이라는 보장 없음). 감쇠 \emph{방향}도 거울상이다 — 충전
꼬리는 $V_n<V_{a}^{\chg}$ 쪽으로 트레일하므로 지지 조건이 $\sigma_d(V_n-V_{a,j}^{d})\ge0$ 으로 반전된다(박스의
지수형은 방전 $\sigma_d=+1$ 표기). 본 식의 ``한 식''은 \emph{중심$\cdot$분극}에
...
\emph{전이당 $\{\Omega_j,\gamma_j\}$ 둘만 더}하면 충방전의 \emph{중심$\cdot$분극}이 닫힌다(분기 영역의 $w_j$ 는 위처럼
독립 피팅; 충전 \emph{꼬리}는 위 적용 범위 주대로 \emph{S3$\cdot$S4 를 충전 방향 데이터로 반복}해 $\{r^{\chg},L^{\chg}\}$ 로
별도 — 그것까지가 완전한 양방향이다).
...
식별은 \textbf{S0$\to$S5} 순서다(\S\ref{sec:synth} 의 S1–S4 에 \emph{선행 진단} S0 와 \emph{충방전} S5 를 더한 것) —
각 단계가 앞 단계 기지값을 써 미지수를 하나씩만 남겨 공선형을 끊는다:
\textbf{S0}(선행) 수송 형상 진단(GITT 완화가 $\sqrt t$ Cottrell 형 vs 지수형, \S\ref{sec:falsify}(i))으로 \emph{활성화
지배}($k_{j,\mathrm{act}}\ll k_{j,\lim}$)를 \emph{확정}하고 $k_{j,\lim}$ 을 추정 — S3 의 $\chi$ 분리(식~\eqref{eq:LqV} 감쇠인자)가 이 확정에
...
$\to\Omega_j$(단 깊은 2상측 $T_{c,j}\gg$창 이면 절편의 $T$-의존이 포화해 $\Omega_j$ \emph{하한}만 잡히고 \emph{$\gamma_j$ 도
곱 $\gamma_j\Omega_j$ 로만} 남는다 — \S\ref{sec:falsify}(ii) regime 분기), 기울기$\to R_n$ 교차확인(식~\eqref{eq:hys_obsgap}). 각 단계가 앞 단계 기지값을 써 미지수를
...
③ prefactor $T$-안정 ④ $q_a$ 컷 고정; \emph{히스}: ⑤ $\xi=\tfrac12$ 대칭 ⑥ $\gamma_j$ 전류$\cdot$온도(측정창) 무관(저$\sim$중율, 식~\eqref{eq:hys_obsgap}; S5 온도-분리의 전제)
...
$\downarrow$''는 \emph{단순 확산$\cdot$전하전달 과전위}도 공유한다(상수 \emph{접촉저항}은 지수 꼬리가 아니라 강체
IR 이동이라 S2 전류 차단의 $R_n$ 으로 이미 분리 — 식~\eqref{eq:vapp}). 따라서 그것만으로
...
\item \textbf{온도 소멸.} 그 절편 $\gamma_j\Delta U_j^\hys(T)$ 는 $T\to T_{c,j}=\Omega_j/2R$ 서
  ...
  절편의 \emph{상대(로그) 온도 기울기} $|\dd\ln(\text{절편})/\dd T|$ 로 사전 판별한다(근처면
  $\sim\tfrac{3}{2(T_{c,j}-T)}$ 로 발산적 가파름$\to$ $3/2$ 정량 검정, 멀면 완만$\to$ 약화 검정 — \emph{절대} 기울기
  $|\dd\Delta U^\hys/\dd T|=(4R/sF)\,\mathrm{artanh}\,u_j$ 는 오히려 원거리서 크고 미지 $\gamma_j$ 와 섞이므로 로그
  기울기만이 $\gamma$-무관 판별량이다) —
  즉 $T_c$ \emph{사전 추정이 어느 검정을 쓸지 정하는} 확인 가능 gate 다(평균장 임계지수의 정량값 자체는 본 장
  근사의 한계). 절편이 온도에 무관하면 정규용액 상분리 그림이 틀렸다는 신호다(예:
