...
꼬리 자체가 관측되지 않는다)(꼬리 \emph{시작 위치$\cdot$진폭}은 peak 근처 평형 형태에 일부 의존). 형태 자체의 판별은 저율 OCV(S1,
\S\ref{sec:synth})의 분리 peak 형상 직독으로 한다 — \S\ref{sec:falsify} 가 판별하는 것은 \emph{율속 기전}이다.
\end{flagbox}
...
$1-\xi\simeq e^{-\Omega_j/RT}e^{-sF(V_n-U_j)/RT}$($s{=}+1$ 방전 규약) — 감쇠 길이는 $RT/F$ 고정, 진폭만
$\Omega_j$ 가 억제한다).
...
$(1+e^{-\mathcal A_j/RT})$ 가 최대 $\sim37\%$($\mathcal A_j=RT$)까지 살아 있다. \textbf{이 보정은 $\mathcal A_j$ 를 통해 $V_n$ 만의 함수라
...
있으면 평형 폭 $w^\eff(T)$(식~\eqref{eq:weff})가 \emph{검증} 경로가 되나, 창 전체에서 히스가 보이는 전이는 그 창이 전부 2상측($\Omega_j>2RT$, $T<T_{c,j}$)이라
폭-경로가 무효임에 유의(같은 $g_j''$ 사영이라 어차피 \emph{내적 일관성}용, \S\ref{sec:falsify}(iii)).
\end{boundbox}
...
\item 각 율 \emph{안에서}(율별로) 동일 SOC$\cdot$tail window 의 $\ln L_{q,j}$ 를 $V_\drive$ 로 회귀해 $\chi$
  ...
  $L_{q,j}=L_{V,j}/|\dd V_n/\dd q|_{q_a}$ 환산해 쓴다, 상세 절차는 피팅 장) —
