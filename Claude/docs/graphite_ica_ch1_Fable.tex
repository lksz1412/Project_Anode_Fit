...
$w_j=RT/F$) 위의 것이다 — $\Omega_j\ne0$ 이면 친화도에 상호작용 몫 $\Omega_j(1-2\theta_j)$ 가 더해진 \emph{일반}
친화도(전체 $\mu_\mathrm{Li}$ 차)로 detailed balance 를 쓰면 같은 논법이 $\Omega_j\ne0$ 등온선을 재생하며, 본 장
꼬리 결론(forward 우세 감쇠)은 이 세부에 둔감하다. \textbf{장벽 이동은 평형 target 을 옮기지 않는다}: 평형은 두
...
지수의 온도의존으로 본다(\S\ref{sec:dist}). 둘은 \emph{배타 regime} 이 아니라 \emph{같은 중첩 적분의 두 점근}이다 —
...
\S\ref{sec:dist} 비유일성과 정합). \emph{분기 영역 확장}: 본 절 합산은 단상($\Omega_j\le2RT$, 전이당 peak 하나)을
...
\emph{핵생성 장벽이 정하는 율-무관 수준}($\gamma_j<1$, spinodal 아래)에 고정된다(빠른 핵생성 $=$ \emph{작은} 과주행로
...
$\{r_{a,j}^\chg,L_{V,j}^\chg\}$ 다(리튬화 동역학이 대칭이라는 보장 없음). 본 식의 ``한 식''은 \emph{중심$\cdot$분극}에
...
독립 피팅; 충전 \emph{꼬리}는 위 적용 범위 주대로 충전 데이터의 S3$'$$\cdot$S4$'$ 로 별도 — 그것까지가 완전한 양방향이다).
...
$\gamma_j$ & 분기 축소(다입자, $\le1$) & 충방전 gap $|I|\!\to\!0$ 절편 \emph{크기}($\Omega_j$ 고정 후 분리) & \eqref{eq:hys_center} \\
\bottomrule
\end{tabular}
\end{center}
...
\item \textbf{온도 소멸.} 그 절편 $\gamma_j\Delta U_j^\hys(T)$ 는 $T\to T_{c,j}=\Omega_j/2R$ 서
  ...
  \emph{충분히 멀면}($T_{c,j}\gg$ 창, 전부 단상측) $u_j\to1$ 로 포화해 $3/2$ 멱이 안 보이므로, 그땐 정량 지수가 아니라
  ...
  \emph{절편의 온도 기울기}로 사전 판별한다(근처면 가파른 감소$\to$ $3/2$ 정량 검정, 멀면 완만$\to$ 약화 검정) —
