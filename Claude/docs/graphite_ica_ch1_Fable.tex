...
띄운다(\S\ref{sec:synth} worked example).
...
\item 구동력 $\mathcal A_j=sF(V_n-U_j^d)$ 로 꼬리 길이
  ...
  $T\Delta S_a$ 는 단독이 아니라 Arrhenius 절편 조합으로 들어간다(\S\ref{sec:synth} S4).
\item 측정 $V(q)$ 의 국소 기울기로 $L_{V,j}=|\dd V_n/\dd q|_{q_a}L_{q,j}$ 로 환산하고, 꼬리 시작점
  $V_{a,j}=V_n(q_{a,j})$ 와 잔류 지연 $r_{a,j}$ 를 $q_a$ 컷(가정 ④)에서 평가한다.
\item 항을 합산한다: $\widehat y=C_\bg+\sum_jQ_j\big[\xi_{\eq,j}(1-\xi_{\eq,j})/w_j
  +(r_{a,j}/L_{V,j})\,e^{-(V_n-V_{a,j})/L_{V,j}}\,\mathbf 1\{\sigma_d(V_n-V_{a,j})\ge0\}\big]$
  (식~\eqref{eq:hys_master}; 지시함수 $\mathbf 1$ 은 꼬리의 단방향 지지).
...
공선형이 되살아난다(\S\ref{sec:overlap} S$_4'$ 의 같은 규율). 전 단계가 끝나면 피팅에 쓰지 않은 hold-out 조건
(예: $0.5$C, $5^\circ$C, 또는 다른 율의 충방전 gap)을 예측해 측정과 대조한다 — walkthrough 의 검증 시험
...
\S\ref{sec:falsify} 의 경쟁 원인 배제를 적용한다. 이로써 측정 파일에서 파라미터, 그리고 임의 조건의 예측 곡선까지
