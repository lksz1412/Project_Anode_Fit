...
\item $L_{V,j}=|\dd V_n/\dd q|_{q_a}L_{q,j}$ 로 환산하고, 꼬리 시작점 $V_{a,j}=V_n(q_{a,j})$ 를 $q_a$ 컷
  ...
  $V_{n,\OCV}(q)$ 의 기울기로 대체한다(또는 최근접 측정 율의 $V(q)$ 를 재사용한다). 잔류 지연의 기본값은 \S\ref{sec:lag} 일반해
...
S3$\cdot$S4 를 충전 방향 데이터로 반복하되, 역방향 속도가 율속이 되므로 유효 기울기가
$-(1-\chi^{\chg})sF/RT$ 형으로 바뀐다 — $|\mathcal A_j|$ 를 가로축으로 같은 회귀를 돈다.
