...
\node at (1.8,-2.7) {\footnotesize 좁은 $\rho_G$(단일 지수)};
...
$j=2$ (중간 stage) & $120$ & $10$ & $0.40$ & $45$ & $\Omega_2=4RT_0$, $\gamma_2=0.6$ \\
$j=3$ (stage 2$\to$1) & $85$ & $8$ & $0.35$ & $50$ & $\Omega_3=4RT_0$, $\gamma_3=0.6$ \\
\bottomrule
\end{tabular}
\end{center}
...
$\times$ 충방전)을 그대로 흉내 낸다. 잡음은 평활 후 $\dd Q/\dd V$ 기준 peak 높이의 $1\%$ 백색으로 더한다. $\Omega=4RT_0$ 의 선택에는 두 가지
이유가 있다. 분기 반갈림이 $\pm16.6$ mV 에 그쳐 세 전이의 충$\cdot$방전 peak 가 전부 분리된 채 남고
(가장 좁은 간격이 충전 쪽 $35$ mV 로 반치반폭 합 $31.8$ mV 를 갓 넘는다 — 검출기의 경계 거동까지 시험된다),
측정창에서 절편의 온도 의존이 $-16\%$ 라 S5 의 $(\gamma_j,\Omega_j)$ 분리 회귀가 실제로 시험된다. 깊은
2상측 퇴화 분기까지 시험하려면 $\Omega=8RT_0$ 의 별도 시나리오를 더하되, 분기 갈림($\pm66$ mV)이 peak 를
흡수하지 않도록 $U_j$ 간격을 그만큼 벌려 배치해야 한다.
