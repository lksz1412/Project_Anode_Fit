...
만든다; $g_j$ 가 $\xi\!\leftrightarrow\!1-\xi$ \emph{대칭}이라 공통접선 기울기 $\mu_c$ 에 대해 $g_j'(\xi)=\mu_c$ 가 $\xi=\tfrac12$
외 두 대칭근을 더 가져 극소가 \emph{정확히 둘}이다. 두 변곡점(spinodal, $g_j''=0$, $\xi_{s,j}^\pm$)은 극소가 \emph{아니라}
오목 구간의 경계이고, 두 극소는 그 \emph{바깥}에 놓인다 — 극소가 둘인 것은 이 대칭의 결과이며 비대칭계면 깨진다). 경계는 $g_j''$ 이 가장 작은 $\xi=\tfrac12$ 서 $4RT-2\Omega_j=0$, 곧
...
\item \textbf{온도 소멸.} 그 절편 $\gamma_j\Delta U_j^\hys(T)$ 는 $T\to T_{c,j}=\Omega_j/2R$ 서
  ...
  오히려 유효}하다. \emph{수치 가늠으로 둘을 가른다}: 흑연 staging 상분리가 상온 부근서 보이므로 $\Omega_j\sim2RT_{\mathrm{room}}
  \!\sim5$ kJ/mol 규모면 $T_{c,j}=\Omega_j/2R\sim300$ K — 측정창(15/23/45$^\circ$C$=$288–318 K) \emph{안/근처}라 \textbf{이
  경우 $3/2$ 정량 검정이 산다}(따라서 두 시나리오는 배타가 아니라 $T_c$ 추정이 \emph{어느 검정을 쓸지}를 정하는 확인
  가능 gate다 — 평균장 임계지수의 정량값 자체는 본 장 근사의 한계). 절편이 온도에 무관하면 정규용액 상분리 그림이 틀렸다는 신호다(예:
