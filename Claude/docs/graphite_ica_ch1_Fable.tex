...
일반화한 것 — 방전 $s_I=\sigma_d=+1$, 충전 $\sigma_d=-1$)로 쓴다. \textbf{주의 — $\sigma_d$ 의 작용처는 분극 부호와 분기 중심 선택 둘뿐이며, 평형 곡선의 $s$ 에는 작용하지
않는다}(\S\ref{sec:notation} 규약 그대로). 평형 곡선 $V_{\eq,j}(\xi)$(식~\eqref{eq:hys_Veq})는 상태함수라
...
\item 측정 $V(q)$ 의 국소 기울기로 $L_{V,j}=|\dd V_n/\dd q|_{q_a}L_{q,j}$ 로 환산하고, 꼬리 시작점
  $V_{a,j}=V_n(q_{a,j})$ 를 $q_a$ 컷(가정 ④)에서 읽는다. 잔류 지연의 기본값은 \S\ref{sec:lag} 일반해
  극한 (i)의 접속값 $r_{a,j}=L_{q,j}\,(\dd\xi_{\eq,j}/\dd q)\big|_{q_a}$ 로 두며 — 상승부 해와 꼬리 해가
  $q_a$ 에서 연속이 되는 값이다 — 분포 폭이나 컷 민감도가 클 때는 $r_{a,j}\in(0,\,\xi_{\eq,j}(q_a)]$ 범위의
  자유 파라미터로 풀어 준다(그 경우 $\bm\theta$ 에 추가).
