...
\textbf{이 장을 읽는 두 경로.} 이론 독자는 절 순서대로 읽으면 된다 — 각 절이 앞 절의 결과만 써서 닫히도록
배열되어 있다. 피팅이 급한 실무 독자는 \S\ref{sec:master} 로 바로 가서 통합식과 피팅 알고리즘$\cdot$한눈
참조표를 먼저 보고, 각 항의 유도가 궁금해질 때 해당 절로 거슬러 올라와도 된다 — 모든 항에 출처 절이 표시되어
있다. 다만 식의 \emph{적용 한계}(가정 울타리$\cdot$반증 조건)는 \S\ref{sec:master} keybox 와
\S\ref{sec:falsify} 에 있으므로, 어느 경로든 그 둘은 건너뛰지 않기를 권한다.
...
\emph{동종 자리 선호}(이종 쌍 불리)라 분리를 부추긴다. 이 최근접쌍 분해는 평균장 어림이다. 실제 흑연 staging 의 상호작용은 층간 탄성 변형 같은 \emph{장거리} 성분을 포함한다. 같은 stage 라도 리튬이 층
전체를 고르게 채우는 것이 아니라 국소 도메인 단위로 채워지고 도메인 경계가 층의 굽힘(탄성 변형)을 동반한다는
그림 — Daumas--H\'erold 도메인 — 이 그 장거리 성분의 기원이다. 따라서 $\Omega_j$ 는 그것까지 한 쌍-에너지로 뭉뚱그린 \emph{유효}
...
— 두 골, 곧 \emph{이중 우물}이다(그림~\ref{fig:doublewell}). 국소 $g''<0$ 한 점이 아니라 ``양끝 볼록 $+$ 가운데 오목''의 \emph{전역} 조합이 두 최소를
...
\draw[dashed] (0.0,-0.0583) -- (1.0,-0.0583) node[pos=0.5,below] {\footnotesize 공통 접선(Maxwell)};
...
\draw[<->,gray] (0.2113,0.012) -- (0.7887,0.012) node[pos=0.5,above] {\footnotesize $g''<0$(불안정)};
\end{tikzpicture}
\caption{정규용액 자유에너지의 이중 우물($\Omega_j=3RT$, 식~\eqref{eq:regsol_g}). 가운데 오목 구간($g''<0$,
spinodal $\xi_s^\pm$ 사이)이 곡선을 위로 밀어 양쪽에 극소가 하나씩 생기고, 공통 접선이 두 binodal 조성
$\xi_b^\pm$ 에서 곡선에 접한다 — 그 접선의 기울기가 평탄 전위(plateau)다. binodal--spinodal 사이가 준안정
영역이며 뒤 히스테리시스의 무대가 된다.}
...
($\Omega\ge2RT$)의 한 모수축 $\Omega_j/2RT$ 위에 닫혔다}(그림~\ref{fig:isofamily}).
\begin{figure}[t]
\centering
...
\node[right] at (2.3,0.80) {\footnotesize $\Omega=0$};
\node[right] at (1.0,0.90) {\footnotesize $1.5RT$};
\node[right] at (-1.25,0.93) {\footnotesize $3RT$};
\end{tikzpicture}
\caption{평형 등온선의 한 모수축. 이상($\Omega=0$, 실선)은 폭 $RT/F$ 의 logistic, $\Omega=1.5RT$(파선)는
같은 함수족이되 중심 기울기가 가팔라진(좁아진) 단조 곡선, $\Omega=3RT>2RT$(점선)는 비단조가 되어 한 전위에
세 $\xi$ 가 대응하는 다가 구간이 생긴다 — 그 다가성이 \S\ref{sec:hys_branch} 분기의 뿌리다.}
\label{fig:isofamily}
\end{figure} 단 $\Omega_j=2RT$ 는 매끄러운 연장이 아니라 단상과
...
의존(S5, \S\ref{sec:master})에서 읽는다. 같은 $\Omega_j$ 가 두 경로를 내적 일관성으로 묶는다(\S\ref{sec:falsify}(iii)).
\end{verifybox}
...
까지 뻗는다. 꼬리 융합(\S\ref{sec:overlap})의 정량이 이 한 수에서 나온다. 일반해의 거동은 그림~\ref{fig:kernel} 이
한눈에 보인다.
\begin{figure}[t]
\centering
...
일반해의 기억 적분으로 계산한 지연 $r_j(q)$ 다. 지연은 원천을 길이 $L_{q,j}$ 만큼 뒤따르며 누적하다가 —
정점이 원천보다 늦고 높게 끌린다 — 원천이 포화로 꺼진 뒤에는 극한 (ii)의 순수 지수 감쇠로 넘어간다. 원천이
남아 있는 동안의 감쇠는 순수 지수보다 완만하다.}
\label{fig:kernel}
\end{figure}
...
무거운(heavier-tailed) 꼬리 — 초기에 빠르고 후미에서 완만한 형 — 가 된다. 이 중첩의 구조를 한 단계 더 보면, 감쇠율 $u=1/L$ 에 대한 가중 적분 $\int\rho(u)\,e^{-xu}\,\dd u$ 는 분포
$\rho$ 의 Laplace 변환 꼴이다. 따라서 그 점근형이 \emph{정확히} stretched exponential
$e^{-(x/L_{\mathrm{KWW}})^\beta}$($0<\beta<1$) — 관용명 KWW(Kohlrausch--Williams--Watts) 함수 — 가 되는
것은 $\rho_G$ 가 바로 그 함수의 역 Laplace 변환(특정 두꺼운-꼬리 핵)일 때이며, 지수 $\beta$ 는 그 분포가
정한다 \cite{lindsey1980,johnston2006,svare2000}. 여기서 $L_{\mathrm{KWW}}$ 는 KWW 척도로,
\S\ref{sec:dist} rigorbox 의 $L_0$ 와는 별개의 양이다. 요컨대
...
(\S\ref{sec:lag})는 분포가 좁은($\rho_G\to\delta$) 극한일 뿐이다. 반로그 좌표가 이 차이를 가장 선명하게
드러낸다(그림~\ref{fig:superpose}) — 단일 지수는 직선, 중첩은 아래로 처지는 곡선이다.
\begin{figure}[t]
\centering
\begin{tikzpicture}[x=0.42cm,y=1.5cm]
\draw[->] (0,-3.2) -- (21,-3.2) node[below] {$(q-q_a)/L_{q}(\bar G)$};
\draw[->] (0,-3.2) -- (0,0.25) node[above] {\footnotesize $\ln[\,r_j/r_j(q_a)\,]$};
\foreach \y in {0,-1,-2,-3} \draw (0.15,\y) -- (-0.15,\y) node[left] {\footnotesize $\y$};
\draw[thick] (0,0) -- (9.6,-3.2);
\draw[thick,densely dashed] plot[smooth] coordinates {(0,0) (1,-0.4159) (2,-0.7272) (3,-0.9726) (4,-1.1786) (5,-1.3606) (6,-1.5275) (7,-1.6831) (8,-1.8307) (9,-1.9714) (10,-2.1072) (11,-2.2379) (12,-2.3645) (13,-2.4889) (14,-2.6105) (15,-2.7305) (16,-2.849) (17,-2.9641) (18,-3.0817) (19,-3.194)};
\node at (5.1,-2.6) {\footnotesize 좁은 $\rho_G$(단일 지수)};
\node at (14.5,-1.9) {\footnotesize 넓은 $\rho_G$(중첩 — 늘어짐)};
\end{tikzpicture}
\caption{지수 감쇠의 연속 중첩(반로그). 실선은 단일 배리어(좁은 $\rho_G$)의 순수 지수 — 반로그에서 직선이다.
파선은 감쇠 길이 $L/3,\,L,\,3L$ 세 집단을 같은 무게로 섞은 중첩 — 초기엔 빠른 집단이 끌어내려 가파르고,
후미에선 느린 집단만 남아 완만해진다. 이 ``직선에서 아래로 휘는'' 모양이 stretched 꼬리의 지문이며, 피팅
알고리즘의 잔차 진단(\S\ref{sec:master})에서 $\rho_G$ 적분을 켜는 신호다.}
\label{fig:superpose}
\end{figure} 어떤 $\rho_G$ 가 두꺼운 꼬리를 주는지는
...
\item \textbf{펄스/전류 차단}. 고정$\to R_n$. 분리 근거: 전류 차단 직후 전압의 즉각 도약(IR-drop) $\Delta V=
  s_I|I|R_n$ 에서 $R_n$ 을 읽는다 \cite{weppner1977}. 이 측정이 GITT(galvanostatic intermittent titration
  technique)다 — 정전류 펄스를 짧게 흘린 뒤 전류를 끊고, 전압이 평형으로 되돌아가는 완화 과정을 기록하는
  펄스--휴지 프로토콜로, 차단 순간의 즉각 도약이 $R_n$ 을, 이어지는 완화의 형상이 율속 기전(S0, \S\ref{sec:master})을 준다. $R_n$ 과 $\chi$ 가 둘 다 꼬리를 늘려 공선형이므로
  $R_n$ 을 \emph{먼저} 묶는다.
...
전이의 면적이 한 봉우리에 섞여, 저율 anchor 없이는 $Q_j$ 를 되돌릴 수 없다(아래). 그림~\ref{fig:fusion} 이
이 과정을 모식으로 보인다.
\end{verifybox}
\begin{figure}[t]
\centering
...
\end{tikzpicture}
\caption{두 인접 전이의 융합 모식(중심 간격 $3.9\,w$). 실선은 저율 — 두 peak 사이 골이 깊어 분리가 읽힌다.
파선은 고율 — 각 전이의 동역학 꼬리($L_V=4w$)가 다음 전이 쪽으로 뻗어 골을 메우고 뒤쪽 어깨를 들어 올리며,
면적이 섞이기 시작한다. 저온도 같은 방향으로 작용한다(꼬리 길이 $\propto|I|\,e^{\Delta H_a/RT}$). 융합
후의 식별은 \S\ref{sec:overlap} 의 forward 합산만 허용된다.}
\label{fig:fusion}
\end{figure}
...
$\sum_j$ 가 그 겹침까지 포함한다. \begin{keybox}
\textbf{겹침의 요지.} 합산은 보존의 귀결이라 무조건이고, 전이 간 독립은 별도 가정이다. 융합 판정은 평형 폭
조건과 꼬리 길이 조건 둘로 정량화되며, 저온일수록 더 낮은 전류에서 융합이 시작된다 — 저온과 고율이 모두 꼬리
$L_{V,j}$ 를 키우기 때문이다. 융합 곡선의
역산은 비유일이라 금지되고, 식별은 ``저율로 고정 $\to$ 고율로 forward 예측$\cdot$검증'' 한 방향만 허용된다.
\end{keybox}
이로써 여러 전이의 합산이 닫혔다. 방전 본론의 마지막 조각으로, 다음 절은 같은
...
\end{enumerate}
노화 진단에서 \emph{DVA peak 간격}의 변화가 전극별 용량 손실 — LLI(loss of lithium inventory, 순환 가능한
리튬 재고의 손실)와 LAM(loss of active material, 전극 활물질 자체의 손실) — 을 분리하는 표준 도구인 것도 같은
...
\draw[dashed,gray] (0.0,-1.0657) -- (0.8536,-1.0657) node[left,xshift=-2pt] {};
...
\end{tikzpicture}
\caption{비단조 평형 전위 $V_{\eq,j}(\xi)$($\Omega_j=4RT$, 식~\eqref{eq:hys_Veq}). 방전은 $\xi\approx0$
에서 출발해 상승 준안정 가지를 타고 극대 $\xi_s^-$ 까지, 충전은 반대쪽 $\xi_s^+$ 까지 과주행한다. 두 극값의
전위 차가 spinodal 상한 gap $\Delta U_j^\hys$(식~\eqref{eq:hys_dU})이고, 실측 분기는 축소 인자 $\gamma_j$
만큼 그 안쪽에 선다(식~\eqref{eq:hys_center}).}
\label{fig:vdwloop}
\end{figure}
\subsection{충방전 분기와 gap}
비단조 $V_{\eq,j}$ 는 $\xi_{s,j}^-$ 서 \emph{극대}, 불안정 구간서 하강, $\xi_{s,j}^+$ 서 \emph{극소}, 다시
상승한다(그림~\ref{fig:vdwloop}).
...
$\xi_{s,j}^\pm=\tfrac12\pm\tfrac12 u_j$($u_j\equiv\sqrt{1-2RT/\Omega_j}$)를 식~\eqref{eq:hys_Veq} 에 넣어
빼는 중간 대수를 펼쳐 두면 다음과 같다. 방전 끝점 $\xi_{s}^-=\tfrac12(1-u_j)$ 에서
$\tfrac{\xi}{1-\xi}=\tfrac{1-u_j}{1+u_j}$ 이고 $1-2\xi=+u_j$ 이며, 충전 끝점 $\xi_s^+$ 에서는 두 값이 각각
역수와 부호 반전이 된다. 따라서 두 끝점의 로그 항 차는
$RT\big[\ln\tfrac{1-u_j}{1+u_j}-\ln\tfrac{1+u_j}{1-u_j}\big]=2RT\ln\tfrac{1-u_j}{1+u_j}
=-4RT\,\mathrm{artanh}\,u_j$ 이고($\mathrm{artanh}\,u=\tfrac12\ln\tfrac{1+u}{1-u}$ 의 정의 그대로),
$\Omega_j$ 항 차는 $\Omega_j[u_j-(-u_j)]=2\Omega_j u_j$ 다. 둘을 $sF$ 로 나눠 합치면
\begin{equation}
...
$\mathrm{artanh}(0.707)\approx0.881$, $\Delta U_j^\hys\approx(2/sF)(4{\cdot}0.707-2{\cdot}0.881)RT\approx2.13\,RT/(sF)
\approx54.8$ mV($25^\circ$C). 차원: $\Omega,RT$ J/mol, $sF$ C/mol $\Rightarrow$ V.
\end{verifybox}
\begin{verifybox}
\textbf{Worked example — gap 절편의 온도 의존이 보이는 크기.} S5(\S\ref{sec:master})가 회귀하는 절편$(T)=\gamma_j\Delta U_j^\hys(T)$
...
\end{tabular}
\end{center}
창 안($15\to45^\circ$C) 상대 감소는 각각 $-28\%$, $-16\%$, $-6\%$ 다. 같은 내용을 곡선으로 보면
그림~\ref{fig:gapT} 다. $T_{c,j}$ 가 측정창에 가까울수록
...
\end{tikzpicture}
\caption{spinodal gap $\Delta U_j^\hys(T)$(식~\eqref{eq:hys_dU})의 온도 의존, $\Omega_j=3/4/8RT_0$
($T_0=298$ K). 음영이 측정창($15$--$45^\circ$C)이다. $T_{c,j}$ 가 창에 가까운 곡선($3RT_0$)은 창 안에서도
가파르게 줄어 $\Omega_j$ 가 절편의 온도 의존으로 잘 식별되는 반면, 깊은 2상측($8RT_0$)은 창 안에서 거의
평탄해 lumped $\Delta_j$ 퇴화 처리(피팅 알고리즘 (6$'$))가 필요하다. $T\to T_{c,j}$ 에서 $(T_c-T)^{3/2}$
멱으로 매끄럽게 소멸한다(\S\ref{sec:falsify}(ii)).}
\label{fig:gapT}
\end{figure}
\begin{boundbox}
...
\label{eq:hys_center}
\end{equation}
로 둔다. $\gamma_j$ 의 선형 스케일링은 1차 근사다. $\gamma_j=1$ 은 spinodal 상한이고, $\gamma_j\to0$ 에서는 두 분기 중심이 binodal 평탄 전위, 곧 가역 중심
$U_j$ 로 합쳐져 gap 이 $0$ 이 된다 — \S\ref{sec:regsol} 의 ``$\gamma_j\to0$ 극한이 binodal(가역)로
수렴한다''와 같은 말이며, 잔류 gap 은 남지 않는 규약적 하한이다. 다중 전환점의 엄밀한 취급은 이력을 전환점
...
\end{itemize}
\begin{keybox}
\textbf{peak 쌍의 요지.} $\Omega_j>2RT$ 전이는 ICA 에서 면적$\cdot$폭이 같고 중심만 $\gamma_j\Delta U_j^\hys$
어긋난 거울 쌍으로 나타난다. 그 종 모양의 유한 폭은 평형이 아니라 다입자 번짐과 동역학에서 오는 현상학적
피팅 폭이며, 단상 영역의 평형 폭 $w^\eff$ 와는 지위가 다르다.
\end{keybox}
...
공통 $R_n$ 의 채택은 검증 가능한 가정이다. 그 내용은 ohmic 성분과 SEI(solid electrolyte interphase — 음극 표면에 형성되는 고체 전해질 계면 피막) 저항
성분이 전이 간에 공유되고, 전하이동과 확산의
...
과전위는 $\eta_\mathrm{ct}\propto\ln|I|$ 로 비선형이다. 로그 의존의 출처는 \S\ref{sec:barrier} 의 지수
속도식이다 — 계면 전하전달 속도가 과전위에 지수(식~\eqref{eq:bv} 형)이므로, 주어진 전류를 내는 데 필요한
과전위는 그 역함수인 로그, 곧 Tafel 거동이 된다(\S\ref{sec:falsify}). 이 로그 항은 저$\sim$중율에서는 $R_n$ 에
...
\begin{flagbox}
\textbf{왜 분극과 히스를 구분하나.} 분극만 있는 계(상분리 없음, $\Omega_j\le2RT$)는 $|I|\to0$ 에서 gap 이
닫힌다. 흑연 staging 은 닫히지 않는 잔류 gap 을 보이며, 그 절편이 곧 상분리($\Omega_j>2RT$)의 증거다. gap 의
온도 의존($\Delta U_j^\hys(T)$ 식~\eqref{eq:hys_dU}, $T\to T_{c,j}$ 서 소멸)도 같은 결론을 교차 검증한다.
요컨대 ``율 외삽의 절편 $=$ 열역학, 기울기 $=$ 동역학''이 실측 gap 을 두 기원으로 가르는 본 절의 한 줄 요지이며,
이 분해가 다음 절(부분 cycle)과 통합식(\S\ref{sec:master})의 S5 절차로 그대로 이어진다.
\end{flagbox}
...
\end{tikzpicture}
\caption{방전 피팅식~\eqref{eq:master} 한 전이의 해부(모식). 점선은 평형 상승부 항(logistic 미분), 파선은
동역학 꼬리 항(지수 감쇠), 실선이 그 합이다. 꼬리는 $V_{a,j}$ 에서 진폭 $r_{a,j}/L_{V,j}$ 로 점화되어 단방향
지지 $s(V_n-V_{a,j})\ge0$ 위에서만 산다 — $V_{a,j}$ 의 작은 계단은 오류가 아니라 지시함수 점화의 모델 성질이며,
실측에서는 평활 미분이 이를 뭉갠다.}
\label{fig:anatomy}
\end{figure}
$V_{a,j}=V_n(q_{a,j})$ 는 전이 $j$ 의 꼬리 시작 전위다. 한 전이의 두 항이 어떻게 합쳐져 관측 peak 가 되는지는
그림~\ref{fig:anatomy} 가 모식으로 보인다. 분포가 좁으면($\rho_G\to\delta$) 이 단일지수 꼬리가 정확하고,
...
식별은 \textbf{S0$\to$S5} 의 순서를 따른다(그림~\ref{fig:chain}). \S\ref{sec:synth} 의 S1--S4 에 선행
진단 S0 와 충방전 S5 를 더한 것이며, 각 단계가 앞 단계의 기지값을 써서 미지수를 하나씩만 남겨 공선형을 끊는다.
\begin{enumerate}[label=\textbf{S\arabic*.},leftmargin=2.6em,itemsep=2pt,start=0]
\item \textbf{(선행) 수송 형상 진단.} GITT 완화가 $\sqrt t$ Cottrell 형인지 지수형인지 판정한다
  (\S\ref{sec:falsify}(i)). 판정의 근거는 율속 기전마다 완화의 함수형이 다르다는 데 있다 — 고체상 확산이
  율속이면 표면 농도의 회복이 반무한 확산 해를 따라 $\sqrt t$ 로 풀리고(Cottrell), 계면 반응이 율속이면 단일
  시간상수의 1차 완화라 지수형이 된다. 지수형이면 \emph{활성화 지배}($k_{j,\mathrm{act}}\ll k_{j,\lim}$)를 확정하고
...
\end{tikzpicture}
\caption{비순환 식별 사슬 S0$\to$S5. 각 단계의 출력(아래 라벨)은 다음 단계에서 동결된 기지값으로 쓰이므로,
단계마다 미지수가 하나씩만 남아 공선형이 끊긴다. 화살표를 거슬러 자유화하는 것은 금지된다.}
\label{fig:chain}
\end{figure}
\begin{keybox}
...
\emph{이 문건만 보고 방전 $\dd Q/\dd V$ 피팅 코드를 작성할 수 있어야 한다.} 아래의 단계 (1)--(7)과 한눈 참조표 (8)이 그 사양이다.
...
파이프라인 전체를 모듈로 나누면 다음 골격이 된다 — 각 함수가 본 소절의 어느 항목을 구현하는지 주석으로
적었다.
\begin{quote}\ttfamily\footnotesize
preprocess(raw) $\to$ (q, V$_{\mathrm{app}}$, dQdV, T, d)\quad\rmfamily — (2) 전처리\\
\ttfamily detect\_peaks(dQdV$_{\mathrm{OCV}}$) $\to$ (N$_p$, U$^{(0)}$, w$^{(0)}$, Q$^{(0)}$,
C$_{\mathrm{bg}}^{(0)}$)\quad\rmfamily — (3) 검출$\cdot$초기값\\
\ttfamily cut\_qa($\xi_{\mathrm{eq}}$, V$_n$(q)) $\to$ (q$_a$, V$_a$, windows)\quad\rmfamily — (3$'$) 컷$\cdot$창\\
\ttfamily model\_eval($\theta$; T, I, d, grid) $\to$ dQdV$_{\mathrm{hat}}$\quad\rmfamily — (4) M1--M6\\
\ttfamily stage\_S0(gitt) $\dots$ stage\_S5(gaps)\quad\rmfamily — (6) 단계 회귀(각 단계 출력 동결)\\
\ttfamily predict($\theta$; T, I, d) $\to$ curve\quad\rmfamily — (7) 예측($V_{n,\mathrm{OCV}}(q)$ 자체 생성)\\
\ttfamily roundtrip($\theta^\ast$, noise) $\to$ report\quad\rmfamily — (7) 코드 검증
\end{quote}
...
\S\ref{sec:falsify} 의 경쟁 원인 배제를 적용한다. 잔차 진단을 표로 모아 두면 다음과 같다 — 피팅이 틀어지는 전형 패턴마다 의심 원인과 처방(가는 곳)이 본 장
안에 이미 준비되어 있다.
\begin{center}
\renewcommand{\arraystretch}{1.3}
...
증상 & 의심 원인 & 처방 \\
\midrule
저율 OCV 의 FWHM $>91$ mV & 준평형 미달(꼬리 오염) & 더 낮은 율/GITT 로 S1 재측정 \\
꼬리가 단일지수에서 휨(stretched) & 배리어 분포 $\rho_G$ 넓음 & 분포 적분 켜기(\S\ref{sec:dist}) \\
gap--$|I|$ 가 아래로 오목 & $\eta_\mathrm{ct}\propto\ln|I|$ 우세 & 저율 구간 한정$\cdot$$R_\mathrm{ct}$ 분리(\S\ref{sec:hys_pol}) \\
gap--$|I|$ 가 위로 볼록 & 확산 한계 또는 $\gamma_j(|I|)$ & 율 하향$\cdot$수송 진단(\S\ref{sec:hys_pol}) \\
$y(T)$--$1/T$ 가 비선형 & 공율속$\cdot$경쟁 꼬리원 & S0 재진단$\cdot$경쟁원 배제(\S\ref{sec:falsify}) \\
충방전 peak 면적 불일치 & 데이터 품질(용량 미평형) & 전처리$\cdot$cycle 안정화 재확인(\S\ref{sec:hys_peak}) \\
절편의 $T$-의존이 측정 불확도 미만 & 깊은 2상측 퇴화 & lumped $\Delta_j$ 강등((6$'$)) \\
전이별 gap 기울기 상이 & $R_n$ lumped 가정 위배 & $R_{n,j}(\xi)$ 로 세분(\S\ref{sec:hys_pol}) \\
\bottomrule
\end{tabular}
\end{center}
마지막으로 코드 자체의 검증에는 round-trip 시험을
...
구체적인 시험 설계 한 벌을 적어 두면 다음과 같다. 합성 시나리오는 흑연 반쪽전지의 전형 구도를 본뜬 3전이로
한다:
\begin{center}
\renewcommand{\arraystretch}{1.25}
\begin{tabular}{lccccc}
\toprule
전이 & $U_j$ [mV vs Li] & $w_j$ [mV] & $Q_j/Q_\cell$ & $\Delta H_{a,j}$ [kJ/mol] & 히스 \\
\midrule
$j=1$ (희박) & $210$ & $18$ & $0.15$ & $40$ & 없음 \\
$j=2$ (중간 stage) & $120$ & $10$ & $0.40$ & $45$ & $\Omega_2=8RT_0$, $\gamma_2=0.6$ \\
$j=3$ (stage 2$\to$1) & $85$ & $8$ & $0.35$ & $50$ & $\Omega_3=8RT_0$, $\gamma_3=0.6$ \\
\bottomrule
\end{tabular}
\end{center}
공통값은 $\chi=0.5$, $C_\bg=$ peak 순높이의 $5\%$ 수준 spline, $R_n$ 은 $0.2$C 에서 분극 $8$ mV 가 되는
값으로 잡고, 데이터 격자는 keybox 의 측정 조합(준평형$\cdot$0.1C$\cdot$0.2C $\times$ 15/23/45$^\circ$C
$\times$ 충방전)을 그대로 흉내 낸다. 잡음은 평활 후 $\dd Q/\dd V$ 기준 peak 높이의 $1\%$ 백색으로 더한다.
합격 기준은 셋이다. (i) \emph{회복}: 각 단계의 출력이 참값에서 그 단계 보고 불확도의 $2$배 안에 들어와야
한다. (ii) \emph{누설 없음}: S1 출력을 보고 불확도만큼 흔들어 S3$\cdot$S4 를 재실행했을 때 $\chi$ 와
$\Delta H_a$ 의 변동이 자체 불확도 이내여야 한다 — 동결 사슬이 공선형을 실제로 끊는다는 확인이다.
(iii) \emph{무편향}: 평활 창을 규정 상한(가장 좁은 peak FWHM 의 $1/5$)으로 두었을 때 $w_3$ 회복값의 계통
편향이 통계 산포보다 작아야 한다. 셋 중 하나라도 깨지면 코드가 아니라 그 단계의 구현(창 선정$\cdot$컷$\cdot$
회귀 범위)을 먼저 의심한다.
단계별 동결 순서가 공선형을 실제로 끊는지, 평활 창이 $w_j$ 에 편향을 넣지 않는지가 이 시험에서 정량으로
드러난다.

\textbf{(8) 한눈 참조표.} 코드를 쓰는 동안 옆에 둘 전 파라미터의 요약이다 — 기호, 물리적 의미, 닫는
데이터(동결 단계), 정의식, 전형 규모($25^\circ$C 기준 어림)를 한 줄씩 모았다.
\begin{center}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{p{0.09\textwidth} p{0.27\textwidth} p{0.17\textwidth} p{0.12\textwidth} p{0.21\textwidth}}
\toprule
기호 & 물리적 의미 & 닫는 데이터(단계) & 정의식 & 전형 규모 \\
\midrule
$U_j$ & 전이 $j$ 평형 중심 전위 & 저율 OCV(S1, 각 $T$) & \eqref{eq:eqpeak} & $0.08$--$0.21$ V vs Li \\
$w_j$ & 평형/현상학 폭 & 저율 OCV(S1) & \eqref{eq:weff} & $\lesssim25.7$ mV \\
$Q_j$ & 전이 용량 & 저율 OCV(S1) & \eqref{eq:eqpeak} & $0.1$--$0.5\,Q_\cell$ \\
$C_\bg$ & 배경 미분용량(spline) & 저율 OCV(S1) & \eqref{eq:cbg} & peak 높이의 $\lesssim10\%$ \\
$R_n$ & lumped 분극 저항 & 전류 차단(S2) & \eqref{eq:vapp} & $10$--$100\ \Omega\,$cm$^2$(면적당 — 전극 면적으로 나눠 $\Omega$ 환산) \\
$\chi$ & 전달 계수 & 다율 꼬리(S3) & \eqref{eq:LqV} & $0.3$--$0.7$ \\
$\Delta H_{a,j}$ & 활성화 엔탈피 & 다온도(S4) & \eqref{eq:arrhenius} & $30$--$60$ kJ/mol \\
$r_{a,j}$ & 꼬리 시작 잔류 지연 & 꼬리 진폭(S3 부산물) & M4 & $O(0.1$--$1)$ \\
$\gamma_j$ & 분기 축소 인자 & gap 절편 크기(S5) & \eqref{eq:hys_center} & $0.3$--$0.8$ \\
$\Omega_j$ & 정규용액 상호작용 & gap 절편 $T$-의존(S5) & \eqref{eq:hys_dU} & 히스 전이서 $>5$ kJ/mol \\
($\rho_G$ 폭) & 배리어 분포(선택) & 꼬리 stretched 휨 & \eqref{eq:superpose} & $\sigma_G\sim$ 수 kJ/mol \\
($h_j$) & 부분 cycle 보간(선택) & minor loop & \eqref{eq:hys_partial} & $h_j(\eta)\approx\eta$ \\
\bottomrule
\end{tabular}
\end{center}
전형 규모는 절대 기준이 아니라 초기값$\cdot$sanity check 용 어림이다 — 흑연 반쪽전지의 문헌 일반 범위
\cite{dahn1991,reynier2004,weppner1977}를 본 장 식의 자리수로 환산한 것이며, 실제 값은 데이터가 정한다. 이로써 측정 파일에서 파라미터, 그리고 임의 조건의 예측 곡선까지
...
\emph{잔여} 전위 의존이 남으면 본 설명은 기각된다. 검정이 실제 데이터에서 어떻게 보이는지 한 장면으로
적어 두자. 서로 다른 세 tail 전위 창에서 각각 $y(T)$ 를 만들어 $1/T$ 에 대해 그리면 직선 세 개가 나온다.
본 장이 옳다면 — 구동항을 이미 빼냈으므로 — 세 직선이 한 직선으로 포개져야 한다. 반대로 보정이 불완전하거나
다른 기전이 섞여 있으면 직선들이 전위 순서대로 층이 지는데, 그 층의 기울기 차는 $\chi\,\Delta\mathcal A_j/R$
규모로 정량 예측되므로, 합격/불합격의 이분이 아니라 잔여 기울기의 크기를 그 예측 규모와 비교해 검정력까지
보고할 수 있다.
...
\item \textbf{전하전달 분리.} charge-transfer 과전위 $\eta_\mathrm{ct}$ 도 전위 지수의존이라 단일 전극서 $\chi$ 와
  공선형이다. GITT/전류 차단 또는 EIS(electrochemical impedance spectroscopy — 주파수별 임피던스를
  복소평면(Nyquist)에 그리면 계면 전하전달이 반원 호로 나타나고 그 지름이 $R_\mathrm{ct}$ 다)로
  $R_\mathrm{ct}$ 를 독립 분리하는 표준 전기화학 진단 뒤에만 $\chi$ 의
...
% ====================================================================
\section{연습문제}\label{sec:exercises}
% ====================================================================
각 문제는 본문 한두 절 안에서 닫힌다 — 괄호가 그 절이다. 배포 후 동료 교육용으로 쓸 때는 1$\cdot$2$\cdot$4
를 먼저 권한다.
\begin{enumerate}[label=\textbf{\arabic*.},leftmargin=2.0em,itemsep=3pt]
\item 점유율 평형 조건 식~\eqref{eq:eqcond} 에서 출발해 진행률 logistic 식~\eqref{eq:logistic} 을 재유도하고,
  부호가 명시적으로 한 번 뒤집히는 지점이 어디인지 짚어라(\S\ref{sec:eqpeak}).
\item $25^\circ$C 저율 OCV 에서 어느 단상 전이의 폭이 $w_j=15$ mV 로 읽혔다. $\Omega_j$ 와 $T_{c,j}$ 를
  구하고, $-15^\circ$C 에서 같은 전이의 폭을 예측하라(\S\ref{sec:regsol}, 식~\eqref{eq:weff}).
\item $\Delta H_{a,j}=50$ kJ/mol 인 전이의 꼬리 길이가 $0.2$C$\cdot$$25^\circ$C 에서 $L_{q,j}=0.02$ 였다.
  같은 율의 $5^\circ$C 에서 $L_{q,j}$ 를 추정하고, 그 꼬리가 $100$ mV 떨어진 다음 peak 와 융합할지를
  $|\dd V_n/\dd q|_{q_a}=0.5$ V 가정 아래 판정하라(\S\ref{sec:lag}$\cdot$\S\ref{sec:overlap}).
\item 어떤 측정 결과가 ``꼬리는 고체상 확산 율속의 산물''임을 시사하는가. 그 경우 본 장 모델을 기각하기 전에
  먼저 확인해야 할 두 가지는 무엇인가(\S\ref{sec:falsify}).
\item gap 공식~\eqref{eq:hys_dU} 의 임계 극한 $\Delta U_j^\hys\to(8RT/3sF)\,u_j^3$ 을 Taylor 전개로 재현하고,
  이것이 절편의 온도 소멸 멱 $(T_{c}-T)^{3/2}$ 과 같은 내용임을 보여라(\S\ref{sec:hys_branch}$\cdot$
  \S\ref{sec:falsify}(ii)).
\item 측정창이 $0$--$30^\circ$C 로 바뀌었다. \S\ref{sec:hys_branch} 의 gap 절편 표(또는
  그림~\ref{fig:gapT})를 근거로, $\Omega_j=3/4/8RT_0$ 세 경우 중 어느 식별이 좋아지고 어느 쪽이 여전히
  퇴화 처리 대상인지 논하라.
\item 피팅 알고리즘의 M-체인(\S\ref{sec:master})에서 $\Omega_j\le2RT$ 인 전이가 입력되면 M1 부터 M6 까지
  각 단계가 어떻게 처리되는지 추적하고, 그 결과가 방전식~\eqref{eq:master} 와 일치함을 확인하라.
\end{enumerate}
