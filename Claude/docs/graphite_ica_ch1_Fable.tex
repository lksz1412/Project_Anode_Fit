...
\textbf{한 시점의 풀이 vs 궤적(순환 고리의 정직한 처리).} 먼저 무엇이 순환인지부터 명시한다: $\xi_{\eq,j}$ 는
$V_n$ 의 함수인데(다음 절), 그 $V_n$ 자체가 $\xi_j$ 를 입력으로 받는 식~\eqref{eq:charge} 의 해다 — 이 고리를 모순
없이 닫는 길은 \emph{미분 경로의 구분}이다. 한 \emph{순간}(용량 좌표 $q$ 고정)에는 그때의 동역학 상태
...
절)이고 식~\eqref{eq:charge} 의 해가 \emph{평형 음극 전위} $V_{n,\OCV}(q,T)$ 다. 여기서는 \S\ref{sec:regsol} 의 결과
두 가지만 미리 빌려 쓴다 — 평형 기울기가 $\dd\xi_{\eq,j}/\dd V_n=sF/g_j''$ 라 자유에너지 \emph{곡률} $g_j''$ 의
부호가 기울기 부호를 정하고, 상호작용이 강하면($\Omega_j>2RT$) $g_j''<0$ 구간(spinodal)이 생긴다는 것이다.
이때 우변 기울기는 $C_\bg+\sum_j
...
진폭 $e^{-\Omega_j/RT}$ 로 억제된 평형 wing(점근 유도는 \S\ref{sec:regsol} 의 $w^\eff$ 단락)이 지배하나 그땐
꼬리 자체가 관측되지 않는다)(꼬리 \emph{시작 위치$\cdot$진폭}은 peak 근처 평형 형태에 일부 의존). 형태 판별은 \S\ref{sec:falsify}.
\end{flagbox}
...
로 고정된 채 진폭만 $e^{-\Omega_j/RT}$ 로 억제돼 오히려 \emph{무겁다}(점근의 출처: wing $\xi\to1$ 서 상호작용 항이 $\Omega_j(1-2\xi)\to-\Omega_j$ 로 포화하므로
평형 조건~\eqref{eq:eqcond} 이 $RT\ln[\xi/(1-\xi)]\simeq sF(V_n-U_j)+\Omega_j$ 로 닫혀
$1-\xi\simeq e^{-\Omega_j/RT}e^{-F(V_n-U_j)/RT}$ — 감쇠 길이는 $RT/F$ 고정, 진폭만 $\Omega_j$ 가 억제한다).
...
적분 하한이 될 \emph{꼬리 시작점} $q_a$ 를 먼저 정의한다: 평형 항이 포화해 $\dd\xi_{\eq,j}/\dd q$ 가 정점값의
일정 분율(예 $\sim5\%$) 이하로 떨어지는 \emph{단일} 좌표 — 상승부에서 꼬리로 넘어가는 곳이다(정확한 컷은 피팅에서
선택, \S\ref{sec:synth} S1). 이 1계 선형 ODE 를 $q_a$ 에서 적분해
\begin{equation}
...
\emph{밖}으로 빠져나가는 공통 인자다) — 즉 \emph{$\rho_G$ 가 흡수하지 못한다}. 정직한 처리는 식~\eqref{eq:Lq} 의 $k_j$ 를 forward
...
\S\ref{sec:stattools} 제외)을 묶으면 $\dd Q/\dd V$ 가 $(V;\,T,|I|,V_\drive)$ 의 닫힌 함수가 된다. 이 중 정규용액은
본 절의 \emph{방전 단일 전이} 닫힌식에는 $w_j$ 의 물리적 근거(식~\eqref{eq:weff})로 들어가고, 충방전 분기 몫은
\S\ref{sec:hys_branch} 에서 합류한다. peak 의
...
plateau 가 아니라 \emph{히스 소멸}을 가리키는 규약적 하한 — 다중 전환점의 엄밀한 취급은 이력을 \emph{전환점 분포의 중첩}으로 쓰는 Preisach 류라 본 장 범위 밖,
\S\ref{sec:hys_partial}). $\Omega_j$ 와 $\gamma_j$ 한 양씩이면 충분하다. \textbf{식별 주의}: 관측 gap \emph{절편}(여러 율의 충방전 gap $\Delta V_{\mathrm{obs},j}(|I|)$ 를 $|I|\to0$ 으로
외삽한 값 — \S\ref{sec:hys_pol})은 $\gamma_j\Delta U_j^\hys$ 의 \emph{곱}이라 한 온도의 절편만으론 $\gamma_j$ 와 $\Omega_j$ 가 분리되지 않는다 — \emph{1차
...
있으면 평형 폭 $w^\eff(T)$(식~\eqref{eq:weff})가 \emph{검증} 경로가 되나, 히스가 있는 전이는 측정 온도창에서 $\Omega_j>2RT$
...
\textbf{S0}(선행) 수송 형상 진단(GITT 완화가 $\sqrt t$ Cottrell 형(반무한 \emph{확산} 율속)인지 지수형(\emph{계면} 율속)인지,
\S\ref{sec:falsify}(i))으로 \emph{활성화
...
곱 $\gamma_j\Omega_j$ 로만} 남는다 — \S\ref{sec:falsify}(ii) regime 분기), 기울기$\to R_n$ 교차확인(식~\eqref{eq:hys_obsgap}). 여러 전이가 겹치면 식~\eqref{eq:total} 의 합산으로 저율-anchored \emph{forward} 예측한다(역산
