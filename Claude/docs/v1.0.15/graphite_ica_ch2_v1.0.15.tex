...
종합식이 실제로 한 수를 내는 과정을 Chapter 1 의 4-전이 staging 파라미터($\{U_j,\Delta S^0_j,Q_j,w_j\}$, 표~\ref{tab:ds}
의 $\Delta S^0_j$ 와 Chapter 1 중심$\cdot$폭)로 한 SOC 에서 끝까지 따라간다. 대표점은 탈리튬화 분율
$\bar x=0.25$($T=298.15$ K) — stage $2\!\to\!1$($\mathrm{LiC_6}$, 큰 음의 $\Delta S^0=-16$ J\,mol$^{-1}$K$^{-1}$)이
지배하는 저-$\bar x$ 영역이다.
...
\begin{equation*}
U_\oc(\bar x{=}0.25,\,298.15\,\mathrm K)=82.4\ \mathrm{mV}.
\end{equation*}
...
모은다. 가중 분모 $\sum_jQ_jg_j=11.57\,Q_\cell$/V 가 곧 그 점의 측정 $dQ/dV$ 이며, 비중이 ``$2\!\to\!1$ 봉우리가
$89\%$ 활성''임을 그대로 읽는다(\S\ref{ssec:overlap}).

\begin{table}[htbp]
\centering\small
\renewcommand{\arraystretch}{1.2}
\caption{계산 예제($\bar x{=}0.25$, $298.15$ K)의 전이별 중간값 — 진행률 $\xi_j$, 봉우리 가중 $g_j=\xi_j(1-\xi_j)/w_j$,
국소 $dQ/dV$ 비중, 중심 표준 엔트로피 계수 $\Delta S^0_j/F$.}
\label{tab:worked}
\begin{tabular}{lccccc}
\toprule
전이 & $\xi_j$ & $g_j$ [V$^{-1}$] & $Q_jg_j$ [$Q_\cell$/V] & 비중 $Q_jg_j/\!\sum$ & $\Delta S^0_j/F$ [mV/K] \\
\midrule
$4\!\to\!3$   & $0.002$ & $0.085$ & $0.0085$ & $0.0007$ & $+0.301$ \\
$3\!\to\!2\mathrm L$ & $0.027$ & $1.62$  & $0.194$  & $0.017$  & $0.000$ \\
$2\mathrm L\!\to\!2$ & $0.064$ & $4.27$  & $1.068$  & $0.092$  & $-0.052$ \\
$2\!\to\!1$   & $0.446$ & $20.6$  & $10.30$  & $0.890$  & $-0.166$ \\
\bottomrule
\end{tabular}
\end{table}
\textbf{(c) 겹침 가중 엔트로피 계수(단순식).} 식~\eqref{eq:weighted} 의 중심값 가중을 대입하면
\begin{equation*}
\begin{aligned}
\frac{\partial U_\oc}{\partial T}\Big|_\text{단순식}
&=\sum_j\frac{Q_jg_j}{\sum_kQ_kg_k}\frac{\Delta S^0_j}{F}\\
&=0.890(-0.166)+0.092(-0.052)+0.017(0)+0.0007(+0.301)
=-0.152\ \mathrm{mV/K}.
\end{aligned}
\end{equation*}
\textbf{(d) round-trip 실증.} 표의 폭 $w_j$ 를 고정하고 중심 $U_j(T)=U_j+(\Delta S^0_j/F)(T-T_\mathrm{ref})$ 만
움직여 $U_\oc(\bar x,T)$ 를 $T\pm3$ K 에서 두 번 푼 유한차분 $[U_\oc(T{+}3)-U_\oc(T{-}3)]/6\,\mathrm K$ 는
$-0.152$ mV/K 로, 해석 단순식~\eqref{eq:weighted}과 $<\!0.01\ \mu$V/K 로 일치한다 — 음함수 미분 사슬이 수치와
왕복 정합함을 이 한 점에서 확인한다(폭을 열적 서식 $w_j{=}n_jRT/F$ 로 두면 유한차분은 봉우리 내부 config 를
얹은 완전식으로 수렴하며, 그 전 범위 검증이 파생 A 다). \textbf{(e) 유효 반응 엔트로피와 가역 발열.} 정의 $\Delta S(\bar x)=F\,\partial U_\oc/\partial T$ 와
Bernardi 출구~\eqref{eq:qrev} $\dot Q_\rev=-IT\,\partial U_\oc/\partial T$ 로 닫으면
\begin{equation*}
\Delta S(\bar x{=}0.25)=-14.7\ \mathrm{J\,mol^{-1}K^{-1}}<0,
\qquad
\frac{\dot Q_\rev}{I}=-T\frac{\partial U_\oc}{\partial T}=+45.4\ \mathrm{mV}\;(=+45.4\ \mathrm{mW/A}).
\end{equation*}
...
봉우리 내부 config(파생 B)를 얹는 완전식은 폭의 열적 서식 $w_j{=}n_jRT/F$ 를 전제하므로, 지배 $2\!\to\!1$ 이
두-상(현상학적 폭)인 이 예제에서 그 config 기여는 다온도 round-trip 확정 대상이고(use-this 박스$\cdot$\S\ref{ssec:weff}),
위 단순식 값이 보수적 기준이다.

\begin{table}[t]
\centering\small
\renewcommand{\arraystretch}{1.25}
\caption{탈리튬화 진행에 따른 가역 발열의 부호 교대(단순식, 298.15 K; Chapter 1 4-전이 staging). $\dot Q_\rev/I$ 는
...
\midrule
$0.10$ & $66.8$  & $-0.157$ & $-15.2$ & $+46.8$ & 발열 \\
$0.25$ & $82.4$  & $-0.152$ & $-14.7$ & $+45.4$ & 발열 \\
$0.50$ & $103.8$ & $-0.116$ & $-11.2$ & $+34.7$ & 발열 \\
$0.75$ & $134.3$ & $-0.042$ & $-4.1$  & $+12.6$ & 발열(약) \\
$0.90$ & $178.3$ & $+0.125$ & $+12.1$ & $-37.2$ & 흡열 \\
\bottomrule
\end{tabular}
\end{table}
