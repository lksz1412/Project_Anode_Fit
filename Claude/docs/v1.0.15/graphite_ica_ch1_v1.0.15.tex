% ====================================================================
% ★ release 1.0.15 (2026-07-04) — graphite_ica_ch1_v1.0.15.tex (계보 1.0.13 승계)
...
%     코드 Anode_Fit 1.0.15 와 동일 버전·matched.
% --------------------------------------------------------------------
...
%   Anode_Fit_v1.0.15.py(계보 v11_final→use_w_eff 제거→1.0.10→1.0.12→1.0.13→1.0.14→1.0.15). 무관 일반론(lever/chord·Marcus·KWW·적층·
...
\hypersetup{colorlinks=true, linkcolor=blue, urlcolor=blue, citecolor=blue,
  pdftitle={흑연 음극 + LCO 양극 dQ/dV 이론: 코드 진행을 따라가는 수식-구동 (유도 확장) — Chapter 1 (v1.0.15)}}
\pagestyle{fancy}\fancyhf{}
\lhead{흑연 음극 + LCO 양극 $dQ/dV$ 이론 — Ch.1 (v1.0.15, 통계역학-first + 코드 진행)}\rhead{\thepage/\pageref{LastPage}}
...
\author{}
\date{\normalsize 버전 1.0.15 \quad(코드 \texttt{Anode\_Fit 1.0.15} 와 동일 버전 · matched)}
...
지수 꼬리($L_V=1.5w_j$, 식~\eqref{eq:lag} 의 인과 메모리 적분으로 계산)가 종을 오른쪽으로 밀며,
...
\subsection{인과 메모리 적분과 지연 진행률 $\xi_\mathrm{lag}$}
...
의 인과 메모리는 시작점 $q_0$ 를 어디에 두어도 성립하는 항등식이며, $q_0$ 를 밀어낼 자유가 다음
단계를 연다.
...
% [P6: coordinates retained; recompute lagged curve from memory integral eq:lag]
...
\end{tikzpicture}
\caption{지연의 완화. 파선 $=$ 평형 목표 $\xi_\eq(q)$, 실선 $=$ 인과 메모리 적분(식~\eqref{eq:lag})으로
...
\label{eq:reversal}
\end{equation}
인과 메모리가 진행 방향의 과거를 훑는다는 것이 이 두 식의 공통 내용이다 --- 방향을 어기면 비인과가
된다.
...
% [P6: coordinates retained; recompute pointwise from memory integral eq:lag/eq:reversal.]
...
$V\!\uparrow$ 라 꼬리가 높은 $V$ 쪽으로, 정점 $({+}1.00w,\,0.192)$(식~\eqref{eq:reversal} 상단 분기).
(b) 충전은 진행이 $V\!\downarrow$ 라 인과 적분이 향하는 쪽이 뒤집혀(식~\eqref{eq:reversal} 하단 분기)
꼬리가 낮은 $V$ 쪽으로, 정점 $({-}1.00w,\,0.192)$ --- 방전의 정확한 거울. 두 경우 모두 peak 는 양수.
(좌표는 기억 적분의 도식 수치평가다.)}
\label{fig:reversal}
\end{figure}
...
포함한다 --- 고정 $E_F$(grand-canonical)에서는 엔트로피 핵 $s(\zeta)$ 가 짝함수라 $g'(E_F)$ 항이 $\int_{-\infty}^{\infty}\!\zeta\,s(\zeta)\,\dd\zeta=0$(홀$\times$짝)으로
상쇄되고, 첫 보정은 $\tfrac12 k_B\,g''(E_F)(k_BT)^3\!\int\zeta^2 s(\zeta)\,\dd\zeta$ 곧 $\mathcal O(T^3)$ 이다(고정 조성 $x$ 에서는
...
% ====================================================================
문건과 구현의 연계는 단방향이다 --- 구현(\code{Anode\_Fit\_v1.0.15.py})의 주석$\cdot$docstring 이 본 문건의
