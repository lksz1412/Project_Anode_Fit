...
\node[branchstage,below left=8mm and 3mm of n45] (n6) {N6: equilibrium peak\quad 식~\eqref{eq:eqpeak}\\$Q_j\,\xi_\eq(1-\xi_\eq)/w_j$};
\node[branchstage,below right=8mm and 3mm of n45] (n78) {N7/N8: lag $L_V$ + causal tail\quad 식~\eqref{eq:LV}\eqref{eq:peakshape}\\$Q_j(\xi_\eq-\xi_\mathrm{lag})/L_{V,j}$};
\coordinate (mid) at ($(n6)!0.5!(n78)$);
\node[sel,below=8mm of mid] (bsel) {branch select\ \ 식~\eqref{eq:branch}\\$L_{V,j}\!<\!\nu\Delta_\mathrm{grid}$: N6\quad else: N7/N8};
% ---- post-loop: sum over transitions j, once ----
\node[core,below=6mm of bsel] (n9) {N9: sum $+$ interp\quad 식~\eqref{eq:sum}\\$C_\bg+\sum_jQ_j[\cdot]_j$};
...
갈리는 두 경로다 — 식~\eqref{eq:branch} 의 문턱 $L_{V,j}<\nu\Delta_\mathrm{grid}$ 이 평형 종(N6, 식~\eqref{eq:eqpeak})과
지연 꼬리(N7/N8, 식~\eqref{eq:LV}$\cdot$\eqref{eq:peakshape}) 중 하나를 고르는 분기 선택(굵은 테두리 상자)으로 합류한다.
모든 전이가 이 loop 를 마치면 N9(진한 상자, 식~\eqref{eq:sum})가 배경과 합산$\cdot$역보간한다.}
\label{fig:spine}
\end{figure}
...
지수 꼬리($L_V=1.5w_j$, 식~\eqref{eq:lowpass} 와 동일한 1차 저역통과 재귀로 계산)가 종을 오른쪽으로 밀며,
...
격자 문턱을 넘는 데는 $\Delta H_a\approx70$--$74$ kJ/mol(전이별 — 위 대표 조건과 $\chi=0.5$ 기본 기준),
peak 폭에 견줄
가시적 꼬리에는 $\approx80$ kJ/mol 급 또는 음의 $\Delta S_a$ 배정이 필요하므로, 인용 문헌
