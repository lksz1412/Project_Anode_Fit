% ====================================================================
% graphite_ica_ch1_Opus_v6.tex — Chapter 1 (v6 by Opus: v5 를 계산 흐름도 순서로 재조립)
...
\hypersetup{colorlinks=true, linkcolor=blue, urlcolor=blue, citecolor=blue,
  pdftitle={흑연 음극 dQ/dV 이론: 수식-구동 흐름도 순 — Chapter 1 (v6)}}
\pagestyle{fancy}\fancyhf{}
\lhead{흑연 음극 $dQ/dV$ 이론 — Ch.1 (v6, 흐름도 순)}\rhead{\thepage/\pageref{LastPage}}
...
\title{\textbf{\LARGE Chapter 1}\\[0.35em]\Large 흑연 음극 $dQ/dV$ 이론: 하나의 속도식에서 피팅까지 — 수식-구동판 (v6, 흐름도 순)}
\author{}
\date{}
...
(\S\ref{sec:fwdrev}). 본 장은 그 곡선을 \emph{계산하는 순서}(흐름도)로 전개한다 — 평형 점유에서 상분리$\cdot$분기 중심(\S\ref{sec:regsol}--\S\ref{sec:hys})으로, 관측축$\cdot$평형 peak(\S\ref{sec:charge}--\S\ref{sec:eqpeak})과 동역학 기억(\S\ref{sec:lag}--\S\ref{sec:tempbranch})을 거쳐 닫힌식$\cdot$합산(\S\ref{sec:synth}--\S\ref{sec:overlap})을 지나, 양방향 통합식$\cdot$알고리즘(\S\ref{sec:master})과 관측 gap 분해(\S\ref{sec:hyspol}), 반증(\S\ref{sec:falsify}), 구현(\S\ref{sec:code})에 닿는다. 주 전개는 방전(탈리튬화)이고, 충방전 히스테리시스는 분기 중심으로 그 흐름에 합류한다.
...
% ====================================================================
\subsection{실험 기반 시작값 — 모델의 입력}\label{sec:inputs}
본 장의 모든 곡선은 전이 $j$ 마다 주어지는 소수의 \emph{실험 기반 시작값}에서 출발한다(수치 자체는 측정$\cdot$문헌으로 충당하며, 본 전개에서는 주어졌다고 둔다). 각 시작값과 그것이 진입하는 식:
\begin{center}
...
시작값 & 의미 & 진입하는 식(전개 위치) \\
\midrule
$U_j$ \ (또는 $\Delta H_j,\Delta S_j$) & 전이 중심 전위 / 반응 엔탈피$\cdot$엔트로피 & $\Delta G_j=-sFU_j$, $\partial U_j/\partial T=\Delta S_j/(sF)$~\eqref{eq:dUdT}; 평형 중심~\eqref{eq:logistic} \\
$w_j$ & 전이 폭 & 이상 극한 $RT/F$, 상호작용 시 $w^{\eff}$~\eqref{eq:weff} \\
$\Omega_j,\ \gamma_j$ & 정규용액 상호작용$\cdot$분기 축소 & 상분리$\cdot$히스테리시스~\eqref{eq:hysdU}$\cdot$\eqref{eq:hyscenter} \\
$Q_j$ & 전이 용량 & peak 면적~\eqref{eq:eqpeak} \\
$\Delta H_{a,j},\ \Delta S_{a,j}$ & 활성화 엔탈피$\cdot$엔트로피 & Eyring 속도~\eqref{eq:eyring}; $b_j=-\Delta S_{a,j}/R$, $\Delta H_{a,j}^{\eff}=\Delta H_{a,j}-\chi_d\Omega_j$~\eqref{eq:dHeff} $\to$ 기억 길이~\eqref{eq:lnLq} \\
\bottomrule
\end{tabular}
\end{center}
이 시작값들은 \emph{계산 흐름도}의 순서 — 평형 점유 $\xi_{\eq}$ $\to$ 분기 중심 $U^d$ $\to$ 관측축 $V_n$$\cdot$평형 peak $\to$ 동역학 기억 길이 $L_q$ $\to$ 닫힌식$\cdot$합산 $\to$ 양방향 통합식 — 로 전개되어 dQ/dV 곡선이 된다. 본 장의 절 배열이 그 순서다.
...
\label{eq:meanfield}
\end{equation}
— 선형 몫은 기준 몫으로 빠지고, 남는 조성 의존이 ``찬$\cdot$빈'' 쌍 분율 $\theta(1-\theta)$ 다. 동종 이웃 인력쌍($c<0$)이면 $\Omega=-c>0$. $\Omega$ 는 층간 탄성 등 장거리 성분(Daumas--H\'erold 도메인 \cite{daumas1969,mckinnon1983})까지 뭉친 \emph{유효} 값이다.
...
% ====================================================================
logistic 등온선~\eqref{eq:logistic} 은 $\Omega=0$ 결과였다. 식~\eqref{eq:mu} 의 $\Omega$ 항($\Omega\ne0$ 이 staging 의 본질)이 등온선을 비트는 끝에서 상분리에 도달한다. 준안정 갈래가 바로 다음 \S\ref{sec:hys} 충방전 히스테리시스의 씨앗이 되고, 여기서 나오는 plateau 가 \S\ref{sec:charge} 에서 ICA peak 의 정체가 된다 — 이 두 도착점이 본 절의 동기다.
...
% ====================================================================
상분리(\S\ref{sec:regsol})가 등온선을 비틀어 준안정 갈래를 낳았다 — 이 절은 충전과 방전이 서로 다른 준안정 가지를 과주행하는 것이 어떻게 전위 갈림(gap)이 되는지를 spinodal~\eqref{eq:spinodal} 에서 닫힌 식으로 유도한다.
...
\label{eq:hysmaster}
\end{equation}
꼬리 항에는 indicator $\mathbf 1\{\sigma_d(V_n-V_{a,j}^d)\ge0\}$ 가 곱해진다(박스 밖). 평형 곡선 안 $s$ 는 방전 규약 $+1$ 고정, 방향이 바꾸는 것은 $\sigma_d$ 의 작용처(분극 부호$\cdot$분기 중심$\cdot$꼬리 방향, 그리고 동역학 $L_q$ 평가 시 꼬리 구동력 정렬 $\mathcal A_j^d=\sigma_d F(V_{a,j}-U_j^d)\ge0$)뿐이다. 충전 꼬리 파라미터 일체($\{r^{\chg},L^{\chg}\}$)는 방향별 독립이고 $\Omega\ne0$ 전이는 $\chi_d$ 를 통해 $\Delta H^{\eff}$ 도 방향별이다. 충전 회귀는 같은 직선이되 율속이 역방향이라 계수가 $\chi\to1-\chi$ 로 바뀐다(합-1 강제, 식~\eqref{eq:chisum}); 가로축을 $|V_n-U_j^{\chg}|$ 로 두면 충전 기울기가 $-(1-\chi)F/RT$ 와 합치하는지가 독립 검정이다.

환원 검산 둘 — $\Omega_j\le2RT$ 면 $\Delta U^\hys=0$, 분기 중심이 단일 $U_j$ 로 합쳐져 $|I|\to0$ 에서 두 방향 곡선이 완전히 겹친다(히스는 상분리의 산물). 상분리가 있어도($\Omega_j>2RT$) $|I|\to0$ 이면 gap 절편 $\gamma_j\Delta U_j^\hys$ 만 남아 식~\eqref{eq:hysobsgap}(\S\ref{sec:hyspol} 에서 유도) 과 정합한다.
...
def s1_residual(theta, V, y_meas, Np):
    """S1 잔차 — theta = [U_1, w_1, Q_1, ..., U_Np, w_Np, Q_Np, Cbg0]; (1.87) 종 항만.
...
\bibitem{daumas1969} N. Daumas, A. H\'erold, ``Sur les relations entre la notion de stade et les m\'ecanismes r\'eactionnels dans les compos\'es d'insertion du graphite,'' \emph{C. R. Acad. Sci. S\'er. C} \textbf{268}, 373 (1969).
