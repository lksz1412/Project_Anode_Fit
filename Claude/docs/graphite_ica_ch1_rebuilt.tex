...
\label{eq:mu_mix}
\end{equation}
($+1$ 과 $-1$ 이 상쇄)이 되어, 식~\eqref{eq:mu_latticegas} 의 첫 로그 항이 정확히 재현된다.
...
$\chi_j$ 이며 대칭 Marcus 에서는 $\chi_j=1/2$, 일반(비대칭 BEP/Butler--Volmer)에서는 $0\le\chi_j\le1$ 로
둔다. 이 1차 항이 장벽을 낮추므로($\mathcal A_j>0$ 일 때 $-\chi_j\mathcal A_j<0$)
\begin{equation}
...
$\dot\xi_j=k_j(\xi_{\eq,j}-\xi_j)$, $k_j=r_j^++r_j^-$ 가 \emph{정확한 인수분해}로 동시에 나온다(\AL{12};
$r_j^\pm$ 가 국소 상수이므로 Taylor 가 아니라 항등 재배열). 즉 $k_j$=완화 속도(목표에 접근하는 빠르기),
...
$L\equiv|I|/(Q_\cell k_j)$ 로 두면 $\dd r/\dd q=-(1/L)\,r$ 다. 변수분리 $\dd r/r=-\dd q/L$ 를 $q_a$ 에서 $q$
까지 적분하면 $\ln[r(q)/r(q_a)]=-(q-q_a)/L$ 이고(이 구간에서 $L$ 은 거의 일정 — 위 구동력 완만 가정), 양변에
지수를 취해 지수 감쇠가 따라온다:
\begin{equation}
...
barrier $G\equiv\Delta G_{a,j}$, $\Delta G_{\eff,j}=G-\chi_j\mathcal A_j$)를 대입한다. $L$ 은 $k_j$ 의
\emph{역수}에 비례하므로 $1/k_j=(1/k_0)\exp[+(G-\chi_j\mathcal A_j)/RT]$ 로 \emph{지수 부호가 반전}되어(속도가
작을수록 길이가 길다)
\begin{equation}
