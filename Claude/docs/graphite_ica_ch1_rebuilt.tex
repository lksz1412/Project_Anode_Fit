...
%     · 최대한 심플(사용자 5-30): 근사 닫힌식(frozen-target KWW, 식 kww)까지 Ch1 자급(사용자 6-1);
%       본격 수치 풀이·수렴 판정·피팅 실무는 별도 후속(잠정 Ch6, 제거 가능).
...
\cite{marcus1956} 은 곡률을 주므로 $|\mathcal A_j|$ 가 클 때(역전 영역 포함) 깨진다. 유효범위를 꼬리 내부에서
다시 나눈다: peak 근방 shoulder($|V_{n,\drive}-U_j|\lesssim w_j$, 소구동)에서는 $|\mathcal A_j|$ 가 작아
선형이 정당하나, far-tail 로 갈수록 구동 편차 $|V_{n,\drive}-U_j|$ 가 커져 $|\mathcal A_j|$ 가 증가하므로
선형 근사가 점차 열화한다(2차 Marcus 곡률 보정 필요) — 이 far-tail 영역은 BOUNDED 로 둔다. 즉
``꼬리$=$저구동''을 전역으로 단정하지 않으며, 본 선형식은 전역 정당화가 아니다.
\end{boundbox}
...
자르지 않고, 배리어 값 $g$(intrinsic barrier $G$ 의 한 실현값$\cdot$적분 더미이며 기호표의 $G$ 와 같은
물리량, 단위 J/mol)를 가진 집단의 진행률을 $\xi_j(g,t)$ 로 두어 분포로 평균한다($\int_0^\infty
\rho_G(g;T)\,\dd g=1$):
\begin{equation}
...
와야 한다. \textbf{구체형(\AL{13}).} mode $j$ 에 대해
\begin{equation}
A_{0,j}=\frac{Q_{j,\tot}}{Q_p}\,a_j,\qquad a_j\in[0,1],
\label{eq:A0_def}
\end{equation}
즉 전이 용량분율 $Q_{j,\tot}/Q_p$(전하 보존 식~\eqref{eq:charge_balance} 에서 도출, 무차원)와
접근성$\cdot$활성분율 $a_j$(입자 크기 분포$\cdot$활성 site 밀도에서 \emph{독립} 결정; 역산 금지)의 곱이며,
$L$ 에 무관한 전이별 상수다($A_0(L)\equiv A_{0,j}$). $A_0\not\equiv1$ 이면 $A_L^{\mathrm{amp}}=A_0\,A_L^{\mathrm{prob}}$ 는 더 이상 규격화된 확률밀도가
...
% ====================================================================
\textbf{먼저 진행률 변수를 정의한다($\Theta$ 일가; \AL{13}).} 전체 phase progress 를 전이 용량으로 가중한
무차원 진행률로 묶는다($Q_p=\sum_j Q_{j,\tot}$, \S\ref{sec:fiteq} 식~\eqref{eq:dVdq} 이후의 $\Theta$ 와
\emph{같은 양}):
\begin{equation}
\Theta\equiv\sum_{j=1}^{N_p}\frac{Q_{j,\tot}}{Q_p}\,\xi_j,\quad
\Theta_\eq\equiv\sum_{j=1}^{N_p}\frac{Q_{j,\tot}}{Q_p}\,\xi_{\eq,j},\quad
\Theta_\mathrm{init}\equiv\Theta(q_a)=\sum_{j=1}^{N_p}\frac{Q_{j,\tot}}{Q_p}\,\xi_j(q_a),
\label{eq:theta_family}
\end{equation}
$\Theta_\mathrm{tail}$ 은 peak 통과점 $q_a$ 이후(post-peak)의 $\Theta$ 성분으로, 그 도함수
$\dd\Theta_\mathrm{tail}/\dd q$ 가 아래 kernel 적분이다. 두 종류의 다중성을 구분한다: 바깥의 \emph{이산} 합
$\sum_{j=1}^{N_p}$(전이 $N_p$ 개)과, 각 전이 내부의 \emph{연속} 배리어 분포 $\rho_{G,j}$(아래
$\int\dd L$). 둘을 한 적분으로 섞지 않는다.

전체 진행률의 post-peak 꼬리는 (i) 각 전이 $j$ 내부에서 진행률 기여 $\dd\xi_j/\dd q$(식~\eqref{eq:single_dxidq})를
\emph{연속} 배리어 분포에서 온 확률밀도 $A_{L,j}^{\mathrm{prob}}$ 로 가중 적분하고, (ii) 다시 $N_p$ 개 전이를
정적 진폭 $A_{0,j}$(식~\eqref{eq:A0_def})로 \emph{이산} 합한 것이다. 동역학 잔차 $r_j(q_a)\equiv
\xi_{\eq,j}-\xi_j(q_a)$ 는 정적 물리량인 $A_{0,j}$ 에 \emph{흡수하지 않고} 별도 인자로 둔다(그래야
``$A_0$=독립 물리량, 역산 금지''와 자가당착하지 않는다):
\begin{equation}
\boxed{\;\frac{\dd\Theta_\mathrm{tail}}{\dd q}\simeq\sum_{j=1}^{N_p}A_{0,j}\,r_j(q_a)
\int_{L_{\min,j}}^\infty A_{L,j}^{\mathrm{prob}}(L)\,\frac1L\,
\exp\!\Big[-\frac{q-q_{a,j}}{L}\Big]\,\dd L\;.}
\label{eq:kernel_integral}
\end{equation}
여기서 $A_{L,j}^{\mathrm{prob}},L_{\min,j},q_{a,j}$ 는 식~\eqref{eq:spectrum} 를 전이 $j$ 의
$\rho_{G,j},\chi_j,\mathcal A_j$ 로 평가한 것이다. 두 다중성(바깥 $\sum_j$ vs 안쪽 $\int\dd L$)이 분리돼 있어
한 적분으로 섞이지 않는다.
...
등가성 \cite{lindsey1980}). single-mode 극한($N_p=1$, $a_1=1$, $\rho_{G,1}=\delta(G-G_0)\Rightarrow
A_{L,1}^{\mathrm{prob}}=\delta(L-L_0)$, $r_1(q_a)=1$)은 식~\eqref{eq:single_kernel} 의 특수해다.
\begin{boundbox}
...
특정 $\rho_G$ 형태가 \emph{정확히 어떤} stretched 지수 $\beta$ 를 주는지 — 정규 배리어의 근사 사상은
아래 식~\eqref{eq:kww_params}, 그 정밀화는 데이터/수치(별도)로 정하며, (ii) 위
문헌은 전도도/유전 완화 맥락이라 graphite ICA 꼬리로의 \emph{적용}이 본 작업의 기여다. 이 두 항목은
established 가 아니라 BOUNDED 로 둔다.
\end{boundbox}

\textbf{근사 닫힌식(Ch1 deliverable; frozen-target 가정).} 식~\eqref{eq:kernel_integral} 의 적분을 닫아
\emph{Ch1 안에서} 피팅 가능한 함수형까지 끌고 간다. 전이 $j$ 의 배리어 분포를 정규
$\rho_{G,j}=\mathcal N(G_{0,j},\sigma_{G,j}^2)$ 로 두면, 식~\eqref{eq:L_of_G} 의 $\ln L=\ln L_0+
(G-\chi_j\mathcal A_j)/RT$ 에 의해 $\ln L$ 이 정규 $\Rightarrow L$ 은 log-normal 이다. log-normal 완화시간 위의
단일 지수 중첩(식~\eqref{eq:kernel_integral} 적분)은 stretched-exponential(KWW)로 잘 근사된다(\AL{14},
\cite{lindsey1980,johnston2006}). 따라서 $\Theta_{\mathrm{tail},j}\simeq r_j(q_a)\{1-\exp[-((q-q_{a,j})/
\tau_j)^{\beta_j}]\}$ 이고, 그 도함수가 Ch1 의 \emph{닫힌 피팅 함수}다:
\begin{equation}
\boxed{\;\frac{\dd\Theta_\mathrm{tail}}{\dd q}\approx\sum_{j=1}^{N_p}A_{0,j}\,r_j(q_a)\,
\frac{\beta_j}{\tau_j}\Big(\frac{q-q_{a,j}}{\tau_j}\Big)^{\beta_j-1}
\exp\!\Big[-\Big(\frac{q-q_{a,j}}{\tau_j}\Big)^{\beta_j}\Big]\;,}
\label{eq:kww}
\end{equation}
\begin{equation}
\tau_j=L_0\,e^{(G_{0,j}-\chi_j\mathcal A_j)/RT}\ (\text{중앙 길이}),\qquad
\beta_j=\beta(\sigma_{G,j}/RT)\in(0,1].
\label{eq:kww_params}
\end{equation}
$\sigma_{G,j}\to0$ 이면 $\beta_j\to1$ 이라 식~\eqref{eq:kww} 가 단일 지수(식~\eqref{eq:single_kernel} 의 도함수)로
\emph{해석적으로 환원}된다(무결성 점검). 이 식을 식~\eqref{eq:fiteq} 의 $\dd\Theta/\dd Q=(1/Q_\cell)\,
\dd\Theta_\mathrm{tail}/\dd q$ 에 대입하면 ICA 꼬리가 \emph{닫힌형}으로 평가되어, 여러 $T\cdot|I|$ 동시 피팅이
Ch1 단독으로 가능하다.
\begin{boundbox}
\textbf{근사의 한계(\AL{14}, 정직 표기).} (i) $\beta_j=\beta(\sigma_{G,j}/RT)$ 의 \emph{정확한} 닫힌 사상은
분포 가정(정규 배리어)에 의존하며 log-normal$\to$KWW 등가성은 \emph{근사}다(다른 $\rho_G$ 면 $\beta$ 관계가
달라짐) — BOUNDED. (ii) 식~\eqref{eq:kww} 는 평형 목표 $\Theta_\eq$ 가 꼬리 구간에서 거의 일정하다는
\emph{frozen-target} 가정의 forward(명시) 꼬리다. 목표가 경로 따라 변하면 아래 자기참조(식~\eqref{eq:volterra})
가 정확형이고, 식~\eqref{eq:kww} 는 그 1차 근사다.
\end{boundbox}
...
\qquad \mathcal K(q,q')=\sum_{j=1}^{N_p}A_{0,j}\int_{L_{\min,j}}^\infty A_{L,j}^{\mathrm{prob}}(L)\,\tfrac1L\,
e^{-(q-q')/L}\,\dd L.
\label{eq:volterra}
\end{equation}
\begin{boundbox}
...
\end{boundbox}
\textbf{어느 식을 피팅에 쓰는가.} 기본은 frozen-target 근사 닫힌식(식~\eqref{eq:kww}) — Ch1 의 deliverable
이며 단독으로 ICA 꼬리를 평가한다. 목표 변동이 커 그 1차 근사가 부족할 때만 \emph{정확} 자기참조식
(식~\eqref{eq:volterra})으로 올라간다. 후자를 닫아 푸는 본격 방법(직접 수치 적분, 또는 사용자 PhD 의
ratio-substitution closed-form = 검증 후 사용)과 수렴 판정은 \emph{수치 구현(별도 후속; 잠정 Ch.6)} 으로
분리한다. 본 장은 물리 구조와 근사 닫힌식(식~\eqref{eq:kernel_integral}--\eqref{eq:kww})까지 확정한다.
...
\item 꼬리 중첩 $\dd\Theta_\mathrm{tail}/\dd q$: 근사 닫힌식(식~\eqref{eq:kww})을 \emph{기본}으로 평가
  ($\tau_j,\beta_j$ = 식~\eqref{eq:kww_params}); 목표 변동 큰 경우의 정확 자기참조(식~\eqref{eq:volterra})
  풀이는 수치 구현(별도).
...
(여러 $T$ 의 Arrhenius), $\chi_j$(여러 $|I|$/전위), $a_j$(접근성, 독립 입력), $\rho_G\to(\tau_j,\beta_j)$
(독립 입력 — 역산 금지; 근사 닫힌식 식~\eqref{eq:kww_params}). \textbf{유효범위}: $G-\chi_j\mathcal A_j\ge0$
(Marcus, \AL{15}), shoulder $V_{n,\drive}\approx U_j+O(w_j)$(far-tail 은 BOUNDED, \S\ref{sec:spectrum}).
solver 코드와 수렴 판정, 경험식(EMG 등) 초기값은 \emph{수치 구현(별도 후속; 잠정 Ch.6)} 으로 분리한다
(GS-4; 이론식과 피팅 실무의 분리).
