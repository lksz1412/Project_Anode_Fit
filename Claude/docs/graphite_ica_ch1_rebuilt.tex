...
합한 것}이다: 전체 진행률의 post-peak 꼬리는 각 mode 의 진행률 기여 $\dd\xi_j/\dd q$(식~\eqref{eq:single_dxidq})를
$A_L$ 로 가중 합한 것이다(mode 별 초기 잔차 $r(q_a)$ 는 post-peak 공통값으로 \emph{근사}해 $A_0$ 가중에 흡수 —
빠른 mode(작은 $L$)는 $q_a$ 에서 이미 더 진행해 $r(q_a)$ 가 작을 수 있어 원칙적으로 $L$-의존이며, 이 균일 근사는
BOUNDED 다):
\begin{equation}
...
\label{eq:volterra}
\end{equation}
여기서 $\Theta_\mathrm{init}(q)=\int_0^\infty A_L(L)\,\Theta_0(L)\,e^{-q/L}\,\dd L$ 는 peak 진입 시점의 초기 mode
진행 $\Theta_0(L)$ 이 외부 구동 없이 자유 감쇠하는 동차항이고, 미지 $\Theta$ 는 좌변과 목표
...
\end{boundbox}
\textbf{자기참조식을 닫는 두 layer.} 식~\eqref{eq:volterra} 에서 \emph{값}을 얻으려면 미지 $\Theta$ 를 닫아야
한다. 두 방법을 둔다 — 물리 결론(저온 긴 꼬리 등)은 어느 쪽으로 풀든 같다.

\textbf{Plan B — 직접 $g$-grid 수치 적분(항상 보존되는 core/validator).} 배리어를 격자 $\{g_m\}$ 로 이산화해
식~\eqref{eq:xi_dist} 를 시간 적분하며, 매 시점 전하 보존(식~\eqref{eq:charge_balance})으로 $V_n$ 을 풀어 결합한다
($\xi_j(t)=\sum_m w_m\,\rho_G(g_m)\,\xi_j(g_m,t)$, $w_m$=quadrature 가중). 주어진 $\rho_G$ 와 완화 closure
하에서 이산화 오차만 남는 수치 기준해이며, 아래 Plan A 의 \emph{validator} 를 겸한다(수치 스킴$\cdot$수렴 판정
상세는 부록 \S\ref{sec:ch1_numeric}). 저온$\to$큰 $L$$\to$긴 꼬리 같은 \emph{물리 결론은 닫힌형 없이도 성립}한다.

\textbf{Plan A — 자기참조 선형화 $\to$ 닫힌형(무비약 3단계).} 데이터 피팅에 직접 쓸 닫힌 식을 만든다.
\emph{단계 0(선형화).} 식~\eqref{eq:volterra} 의 비선형성은 오직 $\Theta_\eq(V_n(q',\Theta(q')))$ 한 곳이다.
single-mode baseline $\Theta^{(0)}$ 둘레에서 1차 전개하면 $\Theta_\eq(V_n(\Theta))\approx a(q')+b(q')\,\Theta(q')$,
여기서 자기참조 계수는
\begin{equation}
...
\end{equation}
$\partial V_n/\partial\Theta=-Q_p/C_\bg$ 는 전하 보존 $Q_\cell q=Q_\bg(V_n)+Q_p\Theta$ 를 $\Theta$ 로 미분해 얻고
($C_\bg>0$), $b\le0$ 의 \emph{의미}는 ``진행이 늘면 목표가 줄어 진행을 \emph{되끌어당긴다}''는 음의 되먹임이다.
...
따르고 크기만 미지로 둔다).
\emph{단계 2($\Theta(q)$ 인수분해).} 대입하면 적분 안의 $\Theta(q')$ 가 $\Theta(q)\cdot[\cdots]$ 로 빠져나와
$\Theta(q)$ 를 좌우에서 모아 풀 수 있다:
\begin{equation}
...
외부 전하 $Q=Q_\ext=Q_\cell q$ 이므로 $\dd\Theta/\dd Q=(1/Q_\cell)\,\dd\Theta/\dd q$, post-peak 에서
$\dd\Theta/\dd q\simeq\dd\Theta_\mathrm{tail}/\dd q$). 분모
...
\emph{먼저} 묶지 않으면 아래가 식별 불가). 측정 $V_{n,\app}$ 는 전하 보존(S1)으로 내부 $V_n$ 으로 환산한다.
...
$\propto e^{-(q-q_a)/L}$ 로 회귀해 \emph{한 숫자} $L$ 을 뽑는다(전압축이면 $L_\varphi\simeq|\dd V_n/\dd q|_{q_a}L$).
\textbf{(2) 유효 엔탈피.} 여러 온도의 $\ln(1/L)$ 대 $1/T$ 기울기는 $-(\Delta H_a-\chi_j\mathcal A_j)/R$ 다 — 이는
\emph{유효} 활성화 엔탈피이며($\chi_j\mathcal A_j/(RT)=(\chi_j\mathcal A_j/R)(1/T)$ 가 $1/T$ 에 선형으로 섞이므로),
\emph{순수} $\Delta H_a$ 는 구동력을 없앤 꼬리 깊숙한 영역($V_{n,\drive}\to U_j$, $\mathcal A_j\to0$)으로 외삽하거나
(3)의 $\chi_j$ 로 $\chi_j\mathcal A_j/R$ 를 빼서 보정한다(두 영역은 서로 다른 측정점이다).
...
$\chi_j$ 를 얻는다. \textbf{즉 $\{L,\Delta H_a,\chi_j\}$ 가 단일 지수 꼬리 피팅 + 식별성 순서로 나온다}(필요한
근거가 모두 본 장 안에 있다: 근사식~\eqref{eq:simplefit}, 식별성 \S\ref{sec:falsify}). 꼬리가 단일 지수에서
...
\textbf{격자 수렴.} $N_g$ 를 $\Theta(q)$ 가 수렴할 때까지 늘린다(자기 수렴 점검). solver 코드 구현 자체는 본
이론 범위 밖(P1; 이론식$\cdot$검증 원리만 제시).
...
\S\ref{sec:ch1_simplefit}(심플 근사식)$\cdot$\S\ref{sec:kernel}(Plan A/B closure)$\cdot$\S\ref{sec:ch1_numeric}
(수치 평가)로 흡수했다 — Ch1 자기완결.)}
...
\bibitem{lee2011} (Refs.~6) propagator / ratio-substitution method, \emph{J. Chem. Phys.} \textbf{134},
  124112 (2011). DOI: 10.1063/1.3565476.
\bibitem{son2013} (Refs.~7) ratio-substitution closed-form, \emph{J. Chem. Phys.} \textbf{138}, 164123
  (2013). DOI: 10.1063/1.4802584.
\end{thebibliography}
