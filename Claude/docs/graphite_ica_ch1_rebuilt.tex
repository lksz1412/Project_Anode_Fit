...
=\rho_G(G)\,\dd G$. 역변환 $G(L)=\chi_j\mathcal A_j+RT\ln(L/L_0)$ ($L_0\equiv|I|/(Q_\cell k_0)$, $G=0$ 일 때
$L=L_0 e^{-\chi_j\mathcal A_j/RT}$)에서 $\dd G/\dd L=RT/L>0$ 이라 일대일이므로
\begin{equation}
...
\emph{단계 3 — 물리적 진폭 가중.} 단계 1$\cdot$2 는 ``각 길이에 mode 가 몇이나''(개수 확률밀도)만 준다. 그러나 관측
꼬리는 진행률 $\xi_j$ 의 \emph{개수} 평균(식~\eqref{eq:xi_dist})이 아니라 \emph{용량 가중} 진행 $\Theta$($\sum_j
Q_{j,\tot}\xi_j$, 각 mode 의 용량 기여)로 나타난다. 이 개수$\to$용량 전환에서 mode 별 \emph{진폭}(용량$\cdot$입자
크기 분포$\cdot$활성 site 밀도$\cdot$접근성)이 실리며, 이를 $A_0(L)$ 로 둔다($A_0$ 는 식~\eqref{eq:xi_dist} 의 개수
밀도 $\rho_G$ 와 \emph{별개} 물리량 — 측도 불일치가 아니라 관측량이 다른 것이다). 관측 spectrum 은 그 곱이다:
\begin{equation}
...
분포 $A_L$ 로 적분, 즉 $(1/L)e^{-(q-q')/L}\to\mathcal K(q,q')=\int A_L(1/L)e^{-(q-q')/L}\dd L$, 초기항
$\Theta(0)e^{-q/L}\to\Theta_\mathrm{init}=\int A_L\Theta_0(L)e^{-q/L}\dd L$. 그러면 인과 상한 $q$ 의 Volterra 2종이
...
$\Theta^{(0)}$ 둘레에서 1차 전개하면 $\Theta_\eq(V_n(\Theta))\approx a(q')+b(q')\,\Theta(q')$, 여기서 자기참조 계수는
\begin{equation}
...
($C_\bg>0$), $\partial\Theta_\eq/\partial V_n>0$(평형 진행률은 전위에 증가, 식~\eqref{eq:dxidV} 의 $\dd\xi_\eq/\dd V_n>0$
...
$V_n$ 과 (0)단계 고정 $U_j$ 로 \emph{계산되는 기지값}(자유 파라미터 아님)이다. 따라서 순서는 비순환이다 — (3)에서
$\chi_j$ 를 먼저 얻고, (2)의 측정 기울기 $-(\Delta H_a-\chi_j\mathcal A_j)/R$ 에 \emph{기지} $\chi_j\mathcal A_j/R$ 를
더해 순수 $\Delta H_a$ 를 분리한다((2)↔(3) 닭-달걀 아님). \textbf{즉 $\{L,\Delta H_a,\chi_j\}$ 가 단일 지수 꼬리 피팅
...
\item \textbf{비퇴화 — transport$\cdot$charge-transfer 와의 분리(★ 본 이론의 가장 약한 지점).} 꼬리 길이 scaling
  ...
  그것만으로 barrier-spectrum 에 귀속하면 \emph{퇴화}(오귀속)다. 구별 signature: (a) \emph{전위 의존 Arrhenius
  slope} — 측정 기울기 $\Delta H_a^{\eff}=\Delta H_a-\chi_j\mathcal A_j$(식~\eqref{eq:simplefit} (2))가 구동
  전위에 따라 이동함(순수 transport 는 이 $\chi_j\mathcal A_j$ 결합이 없다); (b) charge-transfer 과전압
  $\eta_{\mathrm{ct}}$ 도 전위 지수의존이라 단일 전극서 $\chi_j$ 와 공선형이므로 — \textbf{GITT/전류 차단으로
  $\eta_{\mathrm{ct}}(R_{\mathrm{ct}})$ 를 독립 분리한 뒤에만} $\chi_j$ 의 ``배리어 낮춤'' 귀속이 성립한다(단순
  rate-broadening \cite{dubarry2022,fly2020}
  과의 구별이 핵심). 이 분리 없이 단일 데이터셋에서 꼬리를 barrier-spectrum 으로 귀속하는 것은 금지한다.
\end{enumerate}
