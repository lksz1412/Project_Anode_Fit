...
$\xi_{\eq,j}=r_j^+/(r_j^++r_j^-)$ 근처에서 1차 선형화하면 $\dot\xi_j=k_j(\xi_{\eq,j}-\xi_j)$,
$k_j=r_j^++r_j^-$(\AL{12}). 즉 $k_j$=완화 속도(목표에 접근하는 빠르기), $\xi_{\eq,j}$=목표(방향). 식
\eqref{eq:kact} 처럼 $\chi_j\mathcal A_j$ 가 \emph{$k_j$ 전체}에 들어가면 $r_j^+,r_j^-$ 가 같은 배율로
커져 비 $r_j^+/r_j^-=\xi_\eq/(1-\xi_\eq)$ 가 \emph{불변} — 방향은 열역학(\S\ref{sec:eq})이 전담하고
$\mathcal A_j$ 는 방향 없는 속도 scale 이다. \emph{방향을 가진} 배리어 낮춤(forward 만 $-\beta\mathcal A$,
backward 는 $+(1-\beta)\mathcal A$ 라 비가 바뀜)은 \textbf{Ch.3 의 Level B} 에서 도입하며, 그때 본 장의
$\chi_j$ 가 transfer coefficient $\beta_j$ 와 같아진다. 본 장 결론식에서는 ``배리어 낮춤''이라는 방향성
함의 용어를 피한다.
\end{keybox}
\begin{boundbox}
