...
정적 진폭 $A_{0,j}$(식~\eqref{eq:A0_def})로 \emph{이산} 합한 것이다. 동역학 잔차 $r_j(q_{a,j})\equiv
\xi_{\eq,j}-\xi_j(q_{a,j})$ 는 정적 물리량인 $A_{0,j}$ 에 \emph{흡수하지 않고} 별도 인자로 둔다(그래야
``$A_0$=독립 물리량, 역산 금지''와 자가당착하지 않는다):
\begin{equation}
\boxed{\;\frac{\dd\Theta_\mathrm{tail}}{\dd q}\simeq\sum_{j=1}^{N_p}A_{0,j}\,r_j(q_{a,j})
\int_{L_{\min,j}}^\infty A_{L,j}^{\mathrm{prob}}(L)\,\frac1L\,
\exp\!\Big[-\frac{q-q_{a,j}}{L}\Big]\,\dd L\;.}
\label{eq:kernel_integral}
\end{equation}
여기서 $\rho_{G,j}$ 는 전이 $j$ 의 배리어 분포(\S\ref{sec:spectrum} 의 $\rho_G$ 를 전이별로 둔 것), $q_{a,j}$ 는
전이 $j$ 의 꼬리 시작(post-peak onset) 좌표(\S\ref{sec:dyn} 의 $q_a$ 를 전이별로 둔 것)이며,
$A_{L,j}^{\mathrm{prob}},L_{\min,j}$ 는 식~\eqref{eq:spectrum} 를 $\rho_{G,j},\chi_j,\mathcal A_j$ 로 평가한 것이다.
두 다중성(바깥 $\sum_j$ vs 안쪽 $\int\dd L$)이 분리돼 있어 한 적분으로 섞이지 않는다.
...
A_{L,1}^{\mathrm{prob}}=\delta(L-L_0)$, $r_1(q_{a,1})=1$)은 식~\eqref{eq:single_kernel} 의 특수해다.
\begin{boundbox}
...
\cite{lindsey1980,johnston2006}). 따라서 $\Theta_{\mathrm{tail},j}\simeq r_j(q_{a,j})\{1-\exp[-((q-q_{a,j})/
\tau_j)^{\beta_j}]\}$ 이고, 그 도함수가 Ch1 의 \emph{닫힌 피팅 함수}다:
\begin{equation}
\boxed{\;\frac{\dd\Theta_\mathrm{tail}}{\dd q}\approx\sum_{j=1}^{N_p}A_{0,j}\,r_j(q_{a,j})\,
...
(Marcus, \AL{15}), $V_{n,\drive}\approx U_j+O(w_j)$.
