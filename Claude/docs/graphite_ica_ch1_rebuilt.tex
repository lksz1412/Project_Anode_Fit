...
\footnotetext{}% (각주: 부분 구간 피팅용 상대 기준형과 총용량 정합 조건은 Ch.6 피팅 실무에서 다룬다.)
...
평형은 이 화학퍼텐셜이 전극 전위와 균형을 이루는 조건 $\mu(\xi_\eq)=s_{\phi,j}F(V_n-U_j^0)$ 이다. 가장
단순한 ideal 극한($\Omega_j=0$)에서 균형식은 $RT\ln[\xi_\eq/(1-\xi_\eq)]=F(V_n-U_j)$ 이고, 이를 $\xi_\eq$
에 대해 풀면(양변에 지수 취하고 정리)
\begin{equation}
\boxed{\;\xi_{\eq,j}(V_n,T)=\frac{1}{1+\exp[-(V_n-U_j(T))/w_j(T)]}\;,\qquad w_j=\frac{RT}{F}\ (\text{ideal})\;.}
\label{eq:logistic}
\end{equation}
중간 단계가 모두 대수적으로 따라오므로 비약이 없다. 이는 매끄러운 logistic 곡선이며 $0\!\to\!1$ 급점프가
...
\label{eq:dxidV}
\end{equation}
로 결정되어 peak 높이 $\propto 1/w_j$, 폭 $\propto w_j$ 다. 저온이면 ideal $w_j=RT/F$ 가 \emph{감소}하므로
...
\label{eq:kact}
\end{equation}
prefactor 가 $k_BT/h$(전이상태 이론의 보편 빈도)로 \emph{고정}이며 임의 상수가 아니다. $\Delta G_{\eff,j}\le0$
...
만든다는 메커니즘은 무질서계$\cdot$빠른 이온 전도체 동역학에서 확립돼 있다(\AL{14}, \cite{svare2000}).
...
\label{eq:spectrum}
\end{equation}
$A_0(L)$ 는 mode 별 진폭$\cdot$population$\cdot$접근성 가중이며 $A_0\equiv1$ 이면 $\int A_L\,\dd L=\int\rho_G\,
\dd G=1$ 로 규격이 보존된다.
\begin{boundbox}
...
spectrum $A_L$ 로 가중 합한 것이다($r(q_a)$ 는 $A_0$ 에 흡수):
\begin{equation}
...
\end{equation}
$1/L$ 인자는 진행률 도함수(식~\eqref{eq:single_dxidq})에서 온 것이지 임의 가중이 아니다. 이 적분은
...
$[\Theta_\eq-\Theta]$ 에 곱하면 상수 목표 극한에서 정상상태가 $\xi^\ast/2$ 로 어긋난다(완화를 두 번 세는
오류). 상수 목표$\cdot\mathcal K=(1/L)e^{-(q-q')/L}\cdot\Theta_0=0$ 극한에서 식~\eqref{eq:volterra} 는
$\Theta=\xi^\ast(1-e^{-q/L})$ 로 single-mode ODE 와 정확히 일치한다(무결성 점검).
\end{boundbox}
...
\label{eq:dVdq}
\end{equation}
배경 저장 $\dd Q_\bg=C_\bg\,\dd V_n$($C_\bg\equiv\partial Q_\bg/\partial V_n$)과 phase progress
$Q_p\,\dd\Theta$(여기서 $Q_p\Theta\equiv\sum_j Q_{j,\tot}\xi_j$)의 합 $\dd Q=C_\bg\,\dd V_n+Q_p\,\dd\Theta$
에서 내부 전위 기준 ICA 는
\begin{equation}
\boxed{\;\frac{\dd Q}{\dd V_n}=\frac{C_\bg(V_n,T)}{\,1-Q_p\,(\dd\Theta/\dd Q)\,}\;.}
\label{eq:fiteq}
\end{equation}
식~\eqref{eq:kernel_integral} 의 꼬리 기여가 $\dd\Theta/\dd Q$ 에 들어가 ICA 꼬리로 나타난다. 측정 축으로는
