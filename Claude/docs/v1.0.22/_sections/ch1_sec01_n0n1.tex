...
% ====================================================================
\section{기호와 규약, 실험조건 매핑, 분극 (N0, N1)}\label{sec:notation}
...
\subsection{실험조건 매핑과 방향 부호 --- $\mathrm N0$}\label{sec:n0map}
...
과 Part II 전건에서 유지되며 \S\ref{sec:signcheck} 가 전수 검산한다. 유도 편의를 위해 \S\ref{sec:center}$\cdot$\S\ref{sec:hys}
...
이 골격의 두 번째 전극(LCO 양극) 일반화는 Part II(\S\ref{sec:lco-intro})가 연다. 그에 앞서, $\mathrm N0$ 의
남은 한 걸음인 분극 환산($\mathrm N1$)을 다음 소절이 마친다.

\subsection{분극 --- 측정 전위에서 내부 전위로 ($\mathrm N1$)}\label{sec:pol}
...
\begin{bgbox}
\textbf{측정 원리 배경 --- 본문 전제가 어느 측정의 산물인가.} 본문이 입력으로 전제하는 세 양은 각각
확립된 측정 원리의 산물이며, 그 대응만 여기 적는다(기법 상세는 범위 밖). (i) \emph{준평형 개방회로 전위}
$U_\mathrm{oc}(\bar x)$: 정전류 펄스로 조성을 한 걸음 옮기고 충분히 완화된 뒤의 전위를 읽는 점열 ---
GITT(galvanostatic intermittent titration technique)\cite{weppner_huggins1977} --- 이 준평형 OCV 곡선의
정통 측정이고, 저율 정전류 곡선의 분극 보정(식~\eqref{eq:vn})은 같은 목표의 연속 근사판이다. (ii)
\emph{엔트로피 계수} $\partial U_\mathrm{oc}/\partial T$: 고정 조성에서 온도를 계단으로 바꾸며 평형 전위의
기울기를 읽는 전위차 엔트로피법이 직접 측정이며\cite{reynier2003,swiderska2019}, 측정된 부호$\cdot$모양을
상전이 유형(균질 고용체$\cdot$이온 질서화$\cdot$두-상 공존)별로 읽는 해석 지도는 리뷰
\cite{baek_pilon2022} 가 정리한다 --- 본문의 단상 폭(\S\ref{sec:width})$\cdot$두-상
broadening(\S\ref{sec:broadening})$\cdot$LCO 질서화(\S\ref{sec:lco-hys-od}) 구분과 Part T(\S\ref{ssec:qrevtab}) 의 SOC 부호
교대가 그 지도 위의 대상들이다. (iii) \emph{가역 발열}: $-IT\,\partial U_\mathrm{oc}/\partial T$ 는 등온
열량 측정으로 독립 검증되는 양이고, 그 계산 절차와 실측 대응은 Part T 의 방법론 절(\S\ref{sec:method})이 담당한다.
모델 식 자체는 측정 방식과 무관하게 성립한다 --- 본 박스는 전제와 측정의 대응 색인일 뿐이다.
\end{bgbox}
...
\emph{(ii) 엔트로피계수 판독틀.} 위 박스 항 (ii) 의 유형 분류(균질 고용체$\cdot$이온 질서화$\cdot$두-상 공존)는
Baek--Pilon 의 전위차 엔트로피 해석 지도\cite{baek_pilon2022} 다 --- 변수 대응은 측정 기울기
$\partial U_\oc/\partial T=\Delta S(x)/F$ $\leftrightarrow$ entropic potential $\partial U/\partial T$
하나로 닫히고(세부 유형 대응은 위 박스 항 (ii) 서술 그대로), \emph{방법 요지} --- 측정 기울기의 부호$\cdot$모양을
상전이 유형별 열역학 서명으로 갈라 읽는 해석 지도를 세운다. \emph{가정 차} --- 측정 해석 리뷰(tier B)라 특정
전극의 미시 모형을 규정하지 않으므로, 본문은 유형 분류만 판독틀로 빌리고 실제 부호$\cdot$크기는 세 분포
합(config$+$vib$+$elec, Part T)과 1차 문헌(\cite{reynier2003,allart2018})으로 확정한다(tier B 1차 병기 규약).
\end{srcbox}
