...
\Delta S_j^\mathrm{vib}\ &\leftarrow\ \text{창 내 완만 몫의 중심 흡수치}\ (\theta_{E,j}\text{ 부여 시 }T\text{-의존 Einstein 보정 --- Chapter 1 Part T Einstein 절 \S\ref{sec:einstein}}),\\
...
$\approx4.2\,k_B$/atom, 전 구간 최대 $\approx9.0\,k_B$/atom 이다(★두 수치는 원문 정량 대조 확인 필요 --- 방법$\cdot$분해는 검증분). \emph{방법 요지} --- Reynier 등은 평형
...
$\Delta S_j^0$($\approx0.47/1.49$ J/(mol\,K), T2/T3 --- 값$\cdot$배정 양쪽 tier C[원전 재확인 중, \S\ref{sec:lco-hys} 혼동 가드],
