...
1. **②-수식 | §3.3 boxed 중심식·검산 3건**: eq:blend-balance 항등·유일근(요동 양성) 이월·$f_\mathrm{Si}\to0$ 회수(eq:blend-limit)·부호 읽기 — **전 항목 재유도 통과**(로그-②a — R5 마스터 통과분 재검증 지시 이행). eq:blend-factor(이중곱 인수분해)·eq:blend-occ(Fermi 형 점유합)·eq:blend-dqdv(계수 $\xi(1-\xi)/w$·차원) 무발견. "$U_\mathrm{oc}\to\mp\infty$ 에서 $0/Q$" 표기 정합.
2. **②-이월 인용 | Ch1 라벨 10종 원문 대조**: eq:sm-workbal·sm-eqcond($s{=}+1$)·sm-logistic·sm-gc·sm-mc-factor·sm-mc-occ·sm-mc-balance·sm-mc-fluc·eq:sum·eq:eqpeak — 전부 존재·형태 일치·"글자까지 같다"/"한 줄 넓힘" 주장 과대 없음(단 eq:sum 은 M-13 의 극한 한정 제외). $\bar x$ 정의·★좌표 배향도 ch1 과 동일.
3. **②-GS-2 분류 논증**: "물리 가정 충돌" 4분류 판정(논리 공백·수치해법·정의상 implicit 각각의 배제 논증) — P3-4 기준 타당, 무발견. GS-2 공백 자체는 존중 — 메우기 제안 0건(보강 제안 H-4·M-13 은 공백 외곽의 정밀화).
4. **GS-1 경계**: §3.2:50-52 히스 취급의 \S ref{sec:si-mech} 이관 — 침범 없음.
5. **P3-1 전위 위계**: §3.3 은 $U_\mathrm{oc}(\bar x,T)$ 로 일관(V_n·V_app 혼용 없음); §3.5 의 "같은 $V_\app$ 배열 = 공통 $V_n$" 등가는 평형·$R_n{=}0$ 초기 상태 전제에서 정합(read-only 확인) — 무발견.
6. **P3-7 명칭**: 두 대상 파일에 ver.N 표기 부재 — Chapter 구조 명칭 충돌 무발견(§3.2 의 N2·N6 판정 어휘는 M-10 별건).
7. **표기·매크로**: `\eq`·`\bg`·`\dd`·`\app` 프리앰블 정의 확인(common_preamble_v1022.tex:43-58)·$f_\mathrm{Si}$·host 첨자 표기표 등재 확인 — 컴파일 위험 무발견(범위 값 자체는 H-4).
8. **수치 전사 대조(R4 원표 ↔ §3.2)**: 58.5/82.1·49.6→82.9–83.3·82.9(gautam)·60:15:10:15·0.3–0.1 V 경사/0.1 V 평탄역·3801/3117·82%·1200 사이클·$-40$~$-95$/$-45$~$-105$ $\mu$V/K·x=3.4–3.5·150 nm 계열 — **전부 원표와 일치**(무발견; 발견은 전부 해석·귀속·표기 층위).
9. **§3.3 srcbox 3탄 내용 요지**: BLEND_UP 원표(#2·#5·#6·#1)의 host 전환·비가산·전위 중첩·MSMR 다리 요지 전사 — 정합(M-14 는 "실측" 계수 표현만, M-16 은 선행사만).
10. **케이스 소절 구조(개형→1차vs순환→용량·비가역→히스)**: 3계열 병렬 구조·tier 명기·"확인 필요" 공백 표기 방식 — R4 규율 충실 이행, 구조 무발견(M-3 은 Si–C 1칸의 마커 누락 별건).
...
**집계**: H 5 · M 16 · L 9 = 총 30건 (§3.2 = 3H+11M+4L 상당·§3.3 = 1H+5M+5L·§3.5 파급 1H).

**확정** (내부 축자 대조·산술 재계산·원장/서지 grep 으로 폐합 — 즉시 적용 가능):
- H-1(자기모순 — 외부는 제목 부분 확인)·H-2(제목 확인)·H-5(원장·bib grep 0건 + 파일 대조)
- M-1·M-3·M-5·M-7·M-10·M-11·M-12·M-13·M-14·M-15·M-16 (전부 프로젝트 내부 원문 대조 또는 산술)
- L-1·L-2·L-4·L-5·L-6·L-7·L-8·L-9
- 부수: arnot_calorimetry2021 = 110509(Crossref 확정 — bib 무결·R4 원표 오기)·wen_huggins 415°C(검증분)

**추정** (논거 강함·원문 전문 미접근분 잔존):
- H-3 (두-상=평탄역: 물리 논거 + 내부 대조 3건은 확정급 — Limthongkul 원문의 곡선 서술 표현만 미접근; "완만한 기울기로 관측" 병기 대안 제공)
- H-4 (환산식·범위 충돌 구조는 확정 — 개별 f_cap 수치는 조성 가정(바인더 몫·q_Si) 의존 추정치, 범위 재선언 여부는 **사용자 결정 경계**)
- M-2 (1710 산술 정합은 확정 — zhang 원문 귀속은 미확인, 제안에 "확인 필요" 마커 내장)
- M-4 (탈리튬화 방향: §3.1 내부 대조 확정 — 정확 피크 전위 수치는 미검증이라 제안에서 수치 배제)
- M-6 (고온 415°C 검증분으로 상향 — Li₂₂Si₅ 상 명칭·3579 병기는 미검증이라 제안에서 상 이름만)
- M-8 (산술 관계 확정 — 원 논문의 전액 귀속 명시 여부 미확인, 대안 경로 제시)

**미검증** (하이쿠 접근 실패 — 후속 검증 대상으로 이관):
- M-9 질량 기준(andersen 원문)·obrovac 탈리튬화 피크 전위·Li₁₅Si₄ 3579 수치·yom 원문 귀속 문구·zhang 화학량론 문구·L-3 "원소 Si 6" 구성

**판단 보류** (사용자 결정 경계 — 보고만):
- H-4 의 근본 해소 방향(Option A 환산 각주 vs Option B 범위 재선언 — 중심식 box·표기표·G2 게이트 3곳 연동)
- H-5 의 moyassari_blend2022 원장 등재 여부(등재 시 G2 지목 복원 가능 — V1 절차 필요)

**적용 시 주의**: H-2/H-3 은 같은 두 행(S2:40-41)을 공유 — 동시 적용본이 H-2 블록에 제시됨. M-3 은 표 행+srcbox 두 곳 연동("이 넷"→"이 다섯"). 모든 제안은 대체·보강만 — 기존 자산·수식 삭제 0건. GS-1·GS-2 정직 공백과 "확인 필요" 표기는 전부 보존(충전 제안 0건).
<!-- TIER-END -->
