...
통계 폭 $w_j{=}RT/F$, $n_j{=}1$)이다. 항등의 층은 갈라 읽는다 --- 결선 항등
$(\dd Q/\dd V)^\eq=e^2\,\partial\langle N\rangle/\partial\mu$ 와 요동 읽기
$\mathrm{var}(N)=\kB T\,\partial\langle N\rangle/\partial\mu$ 는 대정준 일반(식~\eqref{eq:sm-mc-fluc})이라
단상 평형이면 상호작용이 있어도 성립하나, 독립-자리 분해
