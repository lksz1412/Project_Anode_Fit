% ====================================================================
% s32_cases.tex — §3.2 케이스별 활물질(원소 Si·SiOₓ·Si–C 복합) (W2 초안 — 실측·데이터 강조축)
%   강조: comp_R4 검증 수치·tier 명기·문헌 간 상충 시 채택 근거 각주(캡션 내 첨자). 수치는 표 확인분만.
%   원천 표 = comp_R4/SI_CASES.md · upgraded/{SIOX,SIC,SI_ENTROPY}_CASES.md.
% ====================================================================
\section{케이스별 활물질 --- 원소 Si$\cdot$SiO$_x$$\cdot$Si--C 복합}\label{sec:si-cases}
생존 지도(\S\ref{sec:si-survival})가 ``Si 는 골격을 버리지 않고 성분을 갈아 끼운다'' 로 닫혔으니, 남은 일은
...
이동 경계로 진행한다\cite{limthongkul2003,li_dahn2007,mcdowell_coreshell2013}. 깊은 리튬화 끝(약 $50$
...
solid-solution 경사로 옮겨간다\cite{wang_asi2013} --- 곧 \emph{개형 자체가 사이클 이력에 의존}하며, 이것이
