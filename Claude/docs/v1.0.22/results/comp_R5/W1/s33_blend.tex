% ====================================================================
% s33_blend.tex — §3.3 혼합 음극: 공통-μ 대정준 + GS-2 비가산 공백 (W1 이론 우선 초안, comp_R5)
%   W1 강조축(이론 완결성): eq:sm-mc-balance 의 클래스곱 → host 곱 "한 줄 일반화",
%   요동 양성 유일근 논증 이월(재유도 X — xr 인용), f_Si→0 bit-exact 회수 검산.
%   물리 핵심(마스터 지정) 중심식 그대로. GS-2 = 물리 가정 충돌(비가산·host 전환) 정직 공백.
%   신규 라벨 = sec:si-blend / eq:blend-*. 조기 저장 절: §3.3 완성분.
% ====================================================================
...
§3.1 이 앵커로 지목한 전하 보존의 전극 중립성(\S\ref{ssec:si-anchor-body})의 혼합판이며, 본 절은 그
...
이월, $f_\mathrm{Si}\to0$ 흑연 회수는 식~\eqref{eq:blend-limit} 로 닫힌다. 관측 dQ/dV 는
$\dd\bar x/\dd U_\mathrm{oc}$ 로부터 두 host 전이 기여의 배경 위 선형 합(\S\ref{sec:sum}
식~\eqref{eq:sum} 의 host 판)으로 얻으며, 이것이 §3.5 코드 합성의 규칙이다.
\end{keybox}
...
실측)\cite{zhan_blend2026}. 이론 다리는 동일 저자 계보의 MSMR 블렌드 축소모형이
...
구속을 넣은 반응전류 배분 모형(예: \cite{ai_blend2022} 계열)이 필요하다 --- 이는 평형 대정준 반전의
...
공백). 전위 중첩 구간의 실재\cite{zhan_blend2026} 가 1차 근사의 \emph{부분 지지}를, host 전환$\cdot$비가산
\cite{ai_blend2022,chatzogiannakis_blend2025} 이 그 \emph{한계}를 동시에 준다.
\end{itemize}
...
% 자산: [V22-R5-12 공통-μ 대정준 host 곱 인수분해 eq:blend-factor(eq:sm-mc-factor 일반화)]
%   [V22-R5-13 host×클래스 점유합 eq:blend-occ] [V22-R5-14 공통-μ 전하 보존 반전 eq:blend-balance(eq:sm-mc-balance 한 줄 일반화·중심식 유지)]
%   [V22-R5-15 요동 양성 유일근 이월+f_Si→0 bit-exact 회수 eq:blend-limit(N_p=1 회수 확장·R6 게이트 이론짝)]
%   [V22-R5-16 GS-2 비가산·host 전환 정직 공백 ssec:si-blend-gs2(물리 가정 충돌·범위 밖 선언·평형/율속 층 분리)]
