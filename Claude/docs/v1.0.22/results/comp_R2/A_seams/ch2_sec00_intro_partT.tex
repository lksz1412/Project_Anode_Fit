% ====================================================================
% [R2 O-A 초안] ch2_sec00_intro 의 "장→파트" 격 개작 전문
%   지위: comp_R2/A_seams/ 신규 초안 (마스터 소스 미수정). 집행은 마스터가 diff 1:1 로.
%   개작 원칙: 산문 논지·수식·자산 무변경. (i) "본 장(=구 Ch2 열특성 챕터)"→"본 파트",
%     (ii) 구 "Chapter 1"(=흑연 곡선/Part 0) 지칭→같은 문서 Part 0/I + live \S\ref,
%     (iii) 구 "Chapter 1 §15"(=LCO, 신 Ch2 로 이동)→xr 장 간 참조 "Chapter 2 \S\ref{sec:lco-electronic}".
%   대조 원본 = _sections/ch2_sec00_intro.tex (T2: 7·8·33 / T3: 62 / 격전환 "본 장"→"본 파트" 다수).
% ====================================================================
...
Part I 는 흑연 음극의 한 $\dd Q/\dd V$ 곡선을, 전이별 평형 중심 전위
$U_j(T)=(-\Delta H_{\rxn,j}+T\,\Delta S_{\rxn,j})/F$(\S\ref{sec:center} 의 평형 중심 전위) 위에 세운 평형 종과 그 위에
...
본 파트의 일은 이 잠긴 열을 끄집어내어 정량화하는 것이다. 그 작업을 본 파트는 통계열역학 파트로 수행한다 ---
...
\S\ref{sec:width} 의 logistic 평형 점유의 통계역학적 기원임을 보인다. 점유 분포에서 Boltzmann--Shannon 엔트로피
...
configurational$\cdot$vibrational$\cdot$electronic 엔트로피$\to$닫힌식$\to$극한$\to$가역 발열)이 본 파트의 골격이다.
사슬을 한눈에 적으면
\begin{equation*}
\boxed{\;
...
\begin{warnbox}
\textbf{범위 --- 코인 하프셀 단독.} 본 파트는 단일 전극(흑연 vs.\ Li 금속)의 하프셀 $\partial U_\oc/\partial T$ 와
그 가역 발열만 다룬다. 전셀 합성 $\partial U_\cell/\partial T=\partial U_\mathrm{cat}/\partial T-\partial
U_\mathrm{an}/\partial T$ (양극$-$음극 차감)는 \emph{범위 밖}이다 --- 본 파트의 분포는 한 전극의 점유 분포이며,
전셀은 두 전극의 이 양을 부호까지 맞추어 합성해야 하기 때문이다. 주 사례는 흑연 음극 하프셀이고, LCO
($\mathrm{LiCoO_2}$)의 전자 엔트로피 항은 분포 언어의 한 사례로만 포함한다(그 상세 전개는 Chapter 2
\S\ref{sec:lco-electronic}).
\end{warnbox}
