...
### 5.1b P3-5 접목 계획 — 사용자 논문 수치 산출법 (2026-07-18 사용자 질의 응답·차기 세션 집행 지침)

**질문**: 사용자 논문 JCP **147**(14) 144111 (2017)의 수치 산출법(ref. 6·7 방법론)을 본 문건에 접목 가능한가, 가능하면 어디에.

**판단 = 구조적으로 가능(높은 확신)**. 본 문건의 self-consistent 문제는 전부 한 종류 — **음함수 고정점**(전하 보존식을 만족하는 U_oc 반전 + ξ_j·Q_bg·dQ/dV 순환 의존, P3-3/P3-4 의 "정의상 implicit formulation·수치해법 필요" 분류) — 라서, self-consistent 수치 산출법은 아래 슬롯에 1:1 로 앉는다. **단 P1 규약: ref. 6·7 을 실물로 확인하기 전에는 반영 금지**(기억 서지 금지 — 아래 절차 선행).

**삽입 지점 (우선순위순)**:

| # | 위치 | 무엇을 넣나 |
|---|---|---|
| 1 | `ch1_sec02b_part0.tex` §2.5(sec:sm-mc) 반전 소절 직후 | **"반전의 수치 해법" 소절 신설**(본체) — 다클래스 대정준→정준 반전의 반복/가속 풀이법. P3-5 의무 5항(원 서지·사용자 논문 내 ref 위치 page·para·수학 구조·변수 매핑·물리 가정 차이)을 별도 sub-section 으로 |
| 2 | `ch1_sec10_sum.tex` §1.10 합산 절 | 순환 의존(ξ_j↔Q_bg↔dQ/dV) dependency 표에 수치 해법 연결 1문단 — #1 을 참조만 |
| 3 | `ch3v22_sec03_blend.tex` §3.3 | 블렌드 이중합 반전은 #1 방법을 host 곱으로 상속 — 한 줄 포인터 |
| 4 | `Anode_Fit_v1.0.22.py` `solve_U_oc` 경로 | 방법 교체/가속 옵션(기존 root-solve 와 결과 동일성 게이트 필수 — bit-exact 아닌 수렴 공차 기준) |
| 5 | Part T `ch2_sec09_method.tex` (해당 시에만) | ref 방법론이 피팅 루프 쪽이면 round-trip 절차에 연결 |

**선행 절차(차기 세션 — 오푸스로 충분)**: ①사용자 원본 논문 PDF(`_local_only/` 또는 사용자 제공)를 열어 ref. 6·7 의 정확한 서지와 본문 사용 위치(page·paragraph)를 확정 ②ref. 6·7 원 방법론의 수학 구조를 요약 ③본 문건 변수로 매핑(μ↔V 결선·ξ_j·Q_bg) ④그대로 못 가져오는 물리 가정 차이를 명시 ⑤그 다음에만 #1 소절 저작. 물리 판정이 갈리는 지점만 페이블 표적 질의 권장. 예상 규모 = 소절 1+포인터 3+코드 옵션 1(중형 — 오푸스 1창).
