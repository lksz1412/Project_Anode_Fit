...
사슬을 닫은 뒤 Part II(\S\ref{sec:lco-intro})가 같은 골격을 두 번째 전극 LCO 양극(MSMR 동형 + 전자 엔트로피 항)
에 건다 — 문서는 Part 0 $\to$ 흑연(Part I) $\to$ Part II 세 층이다.
...
계산해($\mathrm N2$--$\mathrm N8$) 합산$\cdot$역보간한다($\mathrm N9$). 본 문건의 절 \emph{순서}가 이 노드
순서를 따른다(통계역학 기초 Part 0 은 N0 뒤에 삽입된 기초 절).
...
$Q_\cell{=}1$ 에서는 무차원 배율). 방향 부호의 작용처는 셋 — 분극 부호($\mathrm N1$), 분기 중심 선택($\mathrm N3$), 꼬리($\mathrm N7\cdot\mathrm N8$ —
방향별 전달계수 $\chi_d$ 가 꼬리 \emph{길이}를, 격자 역전이 인과 기억의 진행 \emph{방향}을 가른다) — 이며,
...
$s$ (코드 미대응) & --- & 유도 전용 고정 부호 — 항상 $+1$(\S\ref{sec:center}$\cdot$\S\ref{sec:hys} 의 $U_j$ 정의 관례); 방향에 따라 $\pm1$ 로 바뀌는 $\sigma_d$ 와 별개, 본론 결과-사슬 boxed 식에서 $s{=}1$ 대입돼 소멸(Part 0 의 박스는 $s$ 를 명시 유지) \\
...
곱$\cdot$나눗셈 상수). 식~\eqref{eq:sm-fund} 의 $\mu$ 를 $F$ 로 옮기면
...
\label{eq:partfn}
\end{equation}
--- Chapter 2 의 단일 자리 분배함수(그 문건 식 (eq:Z1))와 같은 식이다. 엄밀히는 자리 1개의 \emph{대정준} 합
($\mu$ 포함)이라 위의 canonical $Z$ 와 앙상블이 다르지만, Ch2 표기와 맞추어 같은 글자 $Z$($=$Ch2 의 $Z_1$)로 적는다.
...
\emph{정규용액}(regular solution)이고, 본론 표~\ref{tab:staging} 의 $\Omega_j$ 와 Part II 의
...
갈라지기 시작하고 $T_c=\Omega_j/2R$ 아래에서만 상분리가 산다. 이 $\mu(\theta)$ 를 측정 전위에 결선하는
것이 다음 소절이다.
...
$U_j^{\,d}$(\S\ref{sec:hys}), 폭에 다중도 $w_j=n_jRT/F$ 를 넣은 것이 코드의 평형 점유다 --- 단 지수의
...
\label{eq:vwork}
\end{equation}
패딩 $p_\mathrm{lo}=p_\mathrm{hi}=0.15$, 점 수 $n_\work=\max(2048,\,2\,\mathrm{len}(V_n))$ 이다($\mathrm{len}(\cdot)$ 는 배열 원소 개수 — 이 문건에서 $|\cdot|$ 는 물리량 크기 전용이라 격자 점 수엔 $\mathrm{len}(\cdot)$ 를 쓴다; 코드 \code{grid\_pad\_lo/hi},
\code{n\_work\_min}). 스팬 $\Delta v=v_\mathrm{hi}-v_\mathrm{lo}$ 는 하한 \code{v\_span\_floor}$=10^{-6}$ V 로
바닥진다(단일점$\cdot$스칼라 입력의 격자 퇴화 가드). 온도가 배열 $T(V)$ 이면 같은 격자 위로 보간해 $T_\work$ 를 얻고($T_\mathrm{rep}=\overline{T_\work}$ 는
...
갈라져 드나들므로 각 종의 값어치에 전기 일 $zF\phi$(1몰)가 더해진 전기화학 퍼텐셜 $\tilde\mu\equiv\mu+zF\phi$(식~\eqref{eq:sm-emu})가 균형을
...
수치 정확 좌표는 그림~\ref{fig:sm-gxi} 의 같은 곡선. 신규 그림). $\Omega>2RT$ 면 가운데가
...
\end{tikzpicture}
\caption{비단조 평형 전위 $V_\eq(\xi)$(식~\eqref{eq:Veq}, $\Omega_j=4RT$ — 좌표는 식 그대로의 수치 평가)와 과주행(v7-11 유래). 방전(굵은 경로,
...
평형 점유의 폭 척도는 이상 극한에서 $w_j=n_jRT/F$ 다($n_j$ 는 폭 다중도): 아래 소절 logistic 유도의
...
미분 종(\S\ref{sec:eqpeak} 에서 유도)이고, 그 폭이 곧 $n_jRT/F$ 라($n_j\!\approx\!1$ 이면 $RT/F\!\approx\!25.7$ mV), 측정 폭으로 직접 검증된다.
...
전이별로 갈린다. dilute$\to$stage4$\cdot$4L$\to$3L$\cdot$3L$\to$2L 은
...
\emph{$\Omega_j>2RT$ 를 유지해 둘째 지위(현상학적 폭)}로 남는다(전이별 구분은 \S\ref{sec:broadening}(a)). 곧 코드 전이 리스트의 폭 폴백
...
합산한 것($C_\bg+\sum_jQ_j\xi_\eq(1-\xi_\eq)/w$)이다 --- 단 \code{equilibrium} 은 분기 없는 \emph{가역 기준선}
($U_j$ 중심)이고, \code{dqdv} 의 $|I|\to0$ 극한은 $\gamma_j\ne0$ 이면 분기 중심 $U_j^{\,d}$ 에 남는다
(히스테리시스는 열역학적 — \S\ref{sec:hys}). 부호 — $\sigma_d$ 가 미분에서 한 번 들어오지만 peak 모양은
...
코드의 사용 조건은 ``지연 길이가 격자에 비해 작을 때''다 — 다음 절(\S\ref{sec:lag}; 그 전에 아래 소절이
...
액체상 stage 를 가리키며 --- 그림~\ref{fig:staging} 의 stage 2L 과 같은 용법 --- 표의 정수 행 라벨과 같은 전이다)는 plateau 가 없는 연속 등온선이라\cite{rsc2021}, 단일 입자 평형 dQ/dV 가 \S\ref{sec:width} 의 logistic 미분
...
켜는} 유한율속 비대칭 꼬리(\emph{한 입자 안}에서 진행 중 변하는 과전압 $\eta(t)$, \S\ref{sec:lag}--\S\ref{sec:tail} 의 $L_V$,
...
(앞 소절의 동결), 코드 안에서 $L_q$ 는 $V$ 의 함수로 다시 미분되지 않는다: \emph{운영상 실현 미분은}
...
$[1-\rho]/[1,-\rho]$ 로, 아니면 같은 점화식 루프로 평가한다. 초기값은 $\xi_{\mathrm{lag},0}=\xi_{\eq,0}$ —
패딩 격자 끝에서 $r(q_0)=0$, 곧 식~\eqref{eq:memory} 의 첫 항을 소거하는 선택이다. $L_V\le0$ 이거나 비유한이면 원신호를 그대로 돌린다.
...
(바로 아래 \S 의 식~\eqref{eq:peakshape})는 이 차를 길이로 나눈 이산 미분이다. 이 매끈한 차분이 지연 점유의 미분 $\dd\xi/\dd V$(평형 종$+$꼬리)로 수렴하는
...
반대로 $L_V$ 가 작아지는 평형 회복은 이 매끈한 차분의 극한이 아니라 코드의 분기 스위치(아래 식~\eqref{eq:branch})가
...
비평활해진다. v12 코드에서 기본 $\nu$ 상향($\gtrsim10$) 또는 문턱 근방 blend 를 검토한다(물리 불변, 수치 처방만).}), 꼬리식 자신의 $L_V\to0$ 소멸
...
은 $U(298)$ 정합으로 정해졌고($U=(-\Delta H_\rxn+298.15\Delta S_\rxn)/F$ 가 표의 $U$ 와 $\pm1$ mV 안에서 정합 — $4\!\to\!3$ 행만 $0.2109$ 로 표시 자리수 경계), $\Delta H_a$ 는 저SOC$\to$
...
\end{enumerate}
이 골격이 LCO 양극에 걸리는 근거 목록이 위 다섯이다. 흑연 서술은 한 줄도 바뀌지 않으며(모든 흑연
...
따른다. 평형 중심의 온도 의존 $\partial U_j/\partial T=\Delta S_{\rxn,j}/F$(식~\eqref{eq:Uj} 미분)는
...
\textbf{층위 구분(모순 아님).} \S\ref{sec:lco-map} 의 ``셀 방전은 LCO 엔 리튬화'' 서술은 셀 방향
...
아니며, \S\ref{sec:lco-map} 는 라벨 의미론까지만 말하고 슬롯 배정은 이 절로 미룬다.
...
\draw[-{Latex[length=1.6mm]}] (-0.04,0) -- (1.04,0) node[below,font=\scriptsize] {Li content $x$ (delithiation $\rightarrow$: $x$ 감소)};
...
\node[right,font=\scriptsize] at (0.30,0.30) {insulator ($x\!\to\!1$ 좌측 끝: $g\!\to\!0$)};
...
(\S\ref{sec:lco-decomp} — 사이의 \S\ref{sec:lco-peak} 을 지나 두 소절 뒤)에서 셋째 성분으로 plug-in 되며(흑연은 이 자리가 0),
...
\item \textbf{config.} O3 영역에서 LCO $\Delta S$ 를 지배($>\tfrac12$\cite{reynier2004}, tier A)하며, 기존 Ch1 logistic 이 식~\eqref{eq:lco-configsplit} 의 봉우리 내부 항을 \emph{자동 생성}한다(격자기체 점유 분포, \S\ref{sec:dist}). \textbf{★이중계산 금지} — 식~\eqref{eq:lco-decomp} 의 $\Delta S_j^\mathrm{config}$ 자리에는 \emph{중심 표준값} $\Delta S_j^0$ 만 넣는다(표~\ref{tab:lco-staging} 의 config 값 = 중심 표준값으로 읽음).
...
\subsection{forward 코드의 LCO 일반화 — 파라미터 교체 + 전자항 plug-in}\label{sec:lco-code}
...
조성도 $x{=}x_\mathrm{center}$ 로 동결해 $V$-무관 상수)
...
\multicolumn{4}{l}{\emph{— LCO 추가 인자(전이 dict 키$\cdot$서브클래스, \S\ref{sec:lco-code}) —}}\\
\code{electronic},\code{x\_center} & 전이 dict & \code{False}, — & 전자항 plug-in 스위치$\cdot$동결 조성 \\
\code{g\_max\_eV},\code{x\_MIT},\code{dx\_MIT} & 전이 dict & $13$,\,$0.85$,\,$0.05$ & MIT 게이트 초기값 (\eqref{eq:ggate}) \\
...
\subsection{facade 와 전체 진행 한눈에}\label{sec:facade}
...
\multicolumn{4}{l}{\emph{\quad(N0$'$ 보강: \code{\_delith\_is\_discharge} 셀 라벨$\to$탈리튬화 부호 환산 = \S\ref{sec:facade})}}\\
...
% ====================================================================
본 문건의 부호 체크리스트(아래 S1--S8·R1--R5)를 전건 확인한다 — 한 곳이라도 어긋나면 결함이다. 범위는 흑연
사슬이며, LCO 부호는 \S\ref{sec:lco-center} verifybox·\S\ref{sec:lco-direction} 이 검산한다. 기준 명제 = \emph{방전 시
