...
order--disorder 봉우리) — 를 남긴다. order--disorder 의 큰 $\Omega_j^\mathrm{cat}$($>2RT$ 로 피팅될 경우)는
(i) plateau(두-상) 성격을 강화해 평형 봉우리를 델타에 가깝게 하고(실측 폭은 $w_j$ 가 현상학적으로 담음 —
...
N9$'$ & $\Delta S^\mathrm{cat}=$config$+$vib$+$elec (MSMR \eqref{eq:msmr}) & \eqref{eq:lco-decomp} & \code{\_effective\_dS\_rxn} \\
\bottomrule
\end{tabular}
\end{table}
