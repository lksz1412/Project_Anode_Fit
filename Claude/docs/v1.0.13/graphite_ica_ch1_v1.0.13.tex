...
\label{eq:partfn}
\end{equation}
--- 엄밀히는 자리 1개의 \emph{대정준} 합($\mu$ 포함)이라 \S\ref{sec:sm-ensemble} 의 canonical $Z$ 와 앙상블이
다르지만, Chapter 2 의 단일 자리 분배함수 $Z_1$(그 문건 식 (eq:Z1))과 같은 식이므로 그 표기에 맞추어 같은 글자
$Z$ 로 적는다.
...
override 되어 \emph{피팅 후} 물리적으로 전이별로 갈린다. dilute$\to$stage4$\cdot$4L$\to$3L$\cdot$3L$\to$2L 은
...
곧 코드 전이 리스트의 폭 폴백 값(0.020/0.016/0.014/0.012 V)도 같은 지위의 초기값이다 — 단상의 $w$ 는 평형이
...
V\_work, center, n\_j, sigma\_d)} 에서 함수의 \code{V\_n} 인자 자리에 $V_\work$ 가 들어간다(\S\ref{sec:pol} 의
세 전위 규약, 식~\eqref{eq:vwork}). 호출 시 중심 $U_j^{\,d}$ 는
...
$\Delta\mu\equiv\varepsilon-\mu_\mathrm{Li}$)를 받는다. \textbf{(c) 같은 식 --- logistic 과의 일치.}
...
경로로 양립한다. 곧 ``평형은 델타여야 하는데 실측은 종''이라는 물음은 \emph{이 두 2상 전이에만} 해당하고,
...
출발값은 $n_j{=}1$ 고정}이라 실제 평가 폭은 $RT/F\!\approx\!25.7$ mV 이고 \code{'w'} 폴백은 \code{'n'} 을
제거해야 활성화된다 — $n_j\!<\!1$ 은 피팅이 내려갈 수 있는 여지이지 현재 출발 상태가 아니다.
...
이 세 출처가 plateau 내부의 델타를 유한 폭의 치우친 종으로 펼친다 — 두-상 $w_j$ 가 ②$\otimes$③ 을 흡수하는
현상학적 자유 피팅 폭이라는 \S\ref{sec:width} 이중지위의 \emph{두-상} 측을 떠받치는 물리가 바로 이것이다.
...
(바로 아래 \S 의 식~\eqref{eq:peakshape})는 이 차를 길이로 나눈 이산 미분이다. 이 매끈한 차분이 지연 점유의
...
\code{LCO\_MSMR\_LIT} 로 둔다(MSMR — multi-species, multi-reaction 파라미터화, 상세
\S\ref{sec:lco-preview}$\cdot$\S\ref{sec:lco-code}) — 표~\ref{tab:lco-staging} 가 그 세 전이다. 흑연이 4 staging 전이를 남기듯, LCO
...
동결한 상수 오프셋으로 넣어(단일-기준 근사) 봉우리를 목표 전위에 둔다(조성 매핑$\cdot$다온도 $T^2$ 곡률
...
$f=+\sigma_d$ 는 같은 물리량끼리 짝짓는 pairing 하의 유일해이며, 그 유도와 가드(``함수형 동형 $\ne$ 물리량
...
위치 이동은 $\propto T^2$, 식~\eqref{eq:U1T2}$\cdot$\S\ref{sec:lco-Se}), 도핑은 \S\ref{sec:lco-hys} 대로 봉우리를 smear$\cdot$shift 시킨다($\Omega$ 는
