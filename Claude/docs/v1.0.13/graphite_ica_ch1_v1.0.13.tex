...
방향별 전달계수 $\chi_d$ 가 꼬리 \emph{길이}를, 격자 역전이 지지 \emph{방향}을 가른다) — 이며,
...
전자는 미분 분모, 후자는 전하 환산 곱). 식~\eqref{eq:sm-fund} 의 $\mu$ 를 $F$ 로 옮기면
...
전이(표~\ref{tab:staging} 의 $4\!\to\!3\cdot3\!\to\!2\mathrm L$ 행에 해당; 문헌의 L 접미 명명은 면내 질서가 풀린
...
$1-(1/\nu)\big/\!\big(e^{1/\nu}-1\big)$ (중심 peak 높이$\cdot$전이 면적 둘 다)라, \emph{기본값 $\nu=2$ 에선 $\sim23\%$}(면적 $\approx0.77$)에 이르는 상당한 점프다
...
\node[right,font=\scriptsize] at (0.30,0.55) {insulator ($x\!\approx\!1$)};
\draw[-{Latex[length=1.2mm]},gray] (0.29,0.55) -- (0.06,0.28);
