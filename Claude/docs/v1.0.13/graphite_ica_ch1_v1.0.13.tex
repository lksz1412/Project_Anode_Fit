...
전이 $j$ 마다 평형 중심$\cdot$히스 분기$\cdot$폭$\cdot$평형 진행률$\cdot$평형 peak$\cdot$지연 길이$\cdot$인과 꼬리를
계산해($\mathrm N2$--$\mathrm N8$) 합산$\cdot$역보간한다($\mathrm N9$). 본 문건의 절 \emph{순서}가 이 노드
순서를 따른다(통계역학 기초 Part 0 은 N0 뒤에 삽입된 기초 절).
파선 상자가 코드의 전이 루프(\code{for tr in transitions}). 그림 출처 — v7-11 의 spine 그림을 그대로 채택했다.}
\label{fig:spine}
\end{figure}
...
(방전 $+1$/충전 $-1$)이 코드의 \code{s} 인자이며, 평형 진행률 $\xi_\eq=\mathrm{logistic}[\sigma_d(V-U)/w]$ 에서 방전
...
\toprule 기호(코드) & 단위 & 의미 \\ \midrule \endhead
...
$\Omega_j$ (\code{Omega}) & J/mol & 정규용액(regular solution) 상호작용 에너지($>2RT$ 면 상분리$\cdot$히스) \\
...
$|\dd V/\dd q|_{q_a}$ (\code{dVdq\_qa}) & V & 컷점 OCV(open circuit voltage) 기울기($L_q\to L_V$ 환산) \\
...
$U_j^{\,d}$(\S\ref{sec:hys}), 폭에 다중도 $w_j=n_jRT/F$ 를 넣은 것이 코드의 평형 진행률 $\xi_\eq$ 다 --- 단 지수의
...
\label{eq:center}
\end{equation}
이 \code{center} 가 다음 절 평형 진행률 $\xi_\eq$ 의 중심으로 들어간다. 평형 종의 \emph{모양} 자체는 방향 불변이고,
...
% ====================================================================
중심이 정해지면 코드는 폭 $w_j$ 와 평형 진행률 $\xi_{\eq,j}$ 를 평가한다. 이 둘이 평형 종(다음 절의 peak)의 모양을
...
\node[right,font=\scriptsize] at (0.72,0.55) {reverse $r^-\xi$};
...
\end{tikzpicture}
\caption{평형 진행률과 그 미분(v7-11 유래). 실선 $=\xi_\eq$ logistic(식~\eqref{eq:xieq}), 파선 $=$ 폭으로 규격화한
...
% ====================================================================
코드는 평형 진행률에서 곧바로 평형 peak 모양 $\xi_\eq(1-\xi_\eq)/w$ 를 만든다. 이것이 전류가 없을 때($|I|\to0$)의
...
\draw[{Latex[length=1.2mm]}-{Latex[length=1.2mm]},gray] (1.5,1.08) -- (1.5,1.62) node[pos=0.5,left,font=\scriptsize] {$r=\xi_\eq-\xi_\mathrm{lag}$};
...
\code{LCO\_MSMR\_LIT} 로 둔다(상세 \S\ref{sec:lco-preview}$\cdot$\S\ref{sec:lco-code}) — 표~\ref{tab:lco-staging} 가 그 세 전이다. 흑연이 4 staging 전이를 남기듯, LCO
...
T2 (order--disorder a)& $\sim$4.05        & $\approx$0.55 & hex$\to$monoclinic 정렬 & config 주도($\approx$0.47 J/(mol\,K)) & 미발현 \\
...
\node[right,font=\scriptsize] at (0.34,0.30) {insulator ($x\!\to\!1$ 좌측 끝: $g\!\to\!0$)};
\node[font=\scriptsize] at (0.9,2.35) {metal};
% Delta S_e ~ -dg/dx : a localized bump near x_MIT (xx=0.15)
\draw[densely dashed,thick] plot[smooth] coordinates {(0.0,0.244) (0.05,0.567) (0.08,0.857) (0.10,1.062) (0.12,1.235) (0.15,1.350) (0.18,1.235) (0.20,1.062) (0.22,0.857) (0.25,0.567) (0.30,0.244) (0.35,0.096) (0.45,0.013) (0.7,0.001) (1.0,0.001)};
...
\code{g\_max\_eV},\code{x\_MIT},\code{dx\_MIT} & 전이 dict & 문헌 $13/0.85/0.05$ & MIT 게이트 초기값 (\eqref{eq:ggate}; 필수 키) \\
...
(3) 전이마다 $U_j(T)$(\eqref{eq:Uj}) → 분기 중심 \code{center}(\eqref{eq:center}) → 폭 $w_j$(\eqref{eq:wbase}) → 평형 진행률 $\xi_\eq$(\eqref{eq:xieq}).
...
N5$+$ & $\Delta S_{e,j}^{\,\mathrm{mol}}=N_A\tfrac{\pi^2}{3}k_B^2T\,\partial g/\partial x<0$ (삽입, 몰당) & \eqref{eq:dSemolar},\eqref{eq:ggate} & \code{func\_dSe\_molar} \\
N9$'$ & $\Delta S^\mathrm{cat}=$config$+$vib$+$elec (MSMR 동형 \eqref{eq:msmr}) & \eqref{eq:lco-decomp} & \code{\_effective\_dS\_rxn} \\
\bottomrule
\end{tabular}
\end{table}
...
\item \textbf{꼬리 극한의 방향(구판 감사 결함 코드 D-PEAK — 꼬리식의 잘못된 평형 환원 — 의 회귀 여부).} 한 칸 감쇠 $\rho=e^{-\Delta_\mathrm{grid}/L_V}$ 의 두 극한이 반대다 —
$L_V\!\gg\!\Delta_\mathrm{grid}\Rightarrow\rho\to1$ 이라 식~\eqref{eq:peakshape} 의 $(\xi_\eq-\xi_\mathrm{lag})/L_V$ 가
매끈한 미분 $\dd\xi/\dd V$(지연 진행률 $=$ 평형 종 $+$ 꼬리; 평형 종 $\dd\xi_\eq/\dd V$ 만은 $L_V\!\ll\!w$ 별도)로 수렴하고, $L_V\!\to\!0\Rightarrow\rho\to0$ 이라 $\xi_\mathrm{lag}\to\xi_\eq$
