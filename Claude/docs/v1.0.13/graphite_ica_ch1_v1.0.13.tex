...
흑연에서 합산식~\eqref{eq:sum} 의 $\Delta S_{\rxn,j}$ 는 평형 중심 $U_j(T)$ 를 통해 한 덩이로 작용했으나, LCO 양극에서는 이 한 덩이가 세 물리 성분의 합임을 명시한다 — 식$\cdot$코드 경로는 그대로이고 $\Delta S$ 의 \emph{내부 분해}만 더하며, 그 근거부터 식으로 놓는다. \textbf{(a) 출발 — 분배함수의 인수분해.} 전이 $j$ 창의 한 미시상태는 (리튬 자리 배열)$\times$
