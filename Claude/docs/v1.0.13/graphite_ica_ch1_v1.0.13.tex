...
($\mu$ 포함)이라 위의 canonical $Z$ 와 앙상블이 다르지만, Ch2 표기와 맞추어 같은 글자 $Z$ 로 적는다.
...
\end{tikzpicture}
\caption{격자기체 자유에너지 $g_j(\xi)$ (식~\eqref{eq:gxi} 의 \emph{개형} — 정성 곡선; 수치 정확 좌표는
그림~\ref{fig:sm-gxi} 의 $\Omega=3RT$ 곡선. $\Omega_j=3RT$; 신규 그림). $\Omega>2RT$ 면 가운데가
...
\emph{소멸}한다($L_V\propto|I|$ — \S\ref{sec:tail}; Fly 등\cite{fly2020}).
...
구동력에 평균장 $\mu$ 의 상호작용 몫(식~\eqref{eq:mu} 의 $\Omega(1-2\xi)$)을 마저 실으면
...
연속 $\Delta S(x)$$\cdot$$g(E_F,x)$$\cdot$도핑 shift 의 정밀값은 1차 문헌에 없으므로(갭 G1 $=$ 연속
$\Delta S(x)$, G2 $=$ 연속 $g(E_F,x)$ — \S\ref{sec:lco-Se}, G3 $=$ 도핑 shift; G$n$ 일련번호의 선언은 G2 첫 정의 자리),
