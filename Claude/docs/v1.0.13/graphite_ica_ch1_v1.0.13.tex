...
불변이고, 선형·상수 몫은 공통 접선·곡률 판정에 불변이라 $g_j^0$(직선 몫 포함)으로 묶어 떼어낸다:
\begin{equation}
...
% ====================================================================
\section{폭과 평형 종 $\xi_\eq$ (N4, N5)}\label{sec:width}
% ====================================================================
...
\subsection{평형 종 $\xi_\eq$ — logistic 의 유도}
...
는 \S\ref{sec:lco-decomp}(\S\ref{sec:lco-decomp} 의 분해식)에서 config$\cdot$vib$\cdot$전자 세 성분으로 분해된다.
...
\S\ref{sec:lco-Se} 의 $T^2$ 곡률식($\tfrac12(T^2-T_\mathrm{ref}^2)$ 곡률)다. \textbf{(부호 좌표 고정)} — 아래 검산의 $\Delta S_{\rxn,j}^\mathrm{cat}$ 는 반쪽반응을
...
정밀값이 아니다 — 전이별 $\Delta S_{\rxn,j}^\mathrm{cat}$ 는 표~\ref{tab:lco-staging}$\cdot$\S\ref{sec:lco-decomp} 의 분해식
...
\label{eq:dSemolar}
\end{equation}
가 \S\ref{sec:lco-decomp} 의 분해식 의 $\Delta S_{e,j}$ 자리에 들어가는 실제 몰당 값이다($g$ 가 e/eV 단위면 eV$\to$J
...
슬롯과 같은 삽입-기준 부호이며, 박스$\cdot$프레임 그림$\cdot$분해식(\S\ref{sec:lco-decomp} 분해식)이 모두 같은
