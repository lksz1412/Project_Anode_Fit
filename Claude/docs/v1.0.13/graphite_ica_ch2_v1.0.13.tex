...
충/방전 방향 부호(Chapter 1 규약)다.\footnote{이 평형 관계의 $\sigma_d$ 는 Chapter 1 의 유도 전용 고정 부호
$s(=+1)$ 에 해당한다 — 방향 부호로서의 $\sigma_d$(분극$\cdot$분기$\cdot$꼬리 세 작용처 전용)와 구분되며, 값은
동일하다($\sigma_d{=}+1$ 평형 규약).} 전위가 오르면 ($V>U_j$) Li 의 전기화학 퍼텐셜이 낮아져 자리에서
...
\textbf{계단이 아니라 연속 블렌드}다(그림~\ref{fig:blend}). 가중 분모 $\sum Q_jg_j$ 가 정확히 측정 $dQ/dV$(=$Q\,\partial x/\partial U$; $x$ 무차원이라 $Q$ 배)
...
완전식은 175 점 전 범위에서 유한차분값과 \emph{부동소수점 정밀도}로 일치(절대오차 $\approx0$ mV/K), 단순식은 봉우리
...
\begin{tikzpicture}[x=1.0cm,y=1.0cm]
  % axes
  \draw[->,>=Stealth] (-0.2,0) -- (9.4,0) node[right]{\small $x$ (delithiation)};
...
\item \textbf{두-상($\Omega>2RT$, 상분리; 흑연 staging 전이가 여기 속함)}: 평형 \emph{단일 입자}의 등온선은
...
\emph{실측 방향과 반대}이므로 본 장은 그런 유효-폭 축소식을 쓰지 않는다. $\Omega$ 의 역할은 plateau(상분리)를
...
$\Omega\to2RT^-$ & 상분리 임계(plateau) & 평형 등온선 비단조화$\to$plateau; \emph{실측} 두-상 폭은 현상학적 피팅(\S\ref{ssec:weff}) \\
...
다 \cite{newman,msmr_partI}. 본 장은 평형 전위 $U_\oc$ 기준 $\Delta S=+F\,\partial U_\oc/\partial T$ 규약으로 통일한다(전셀 조립 시 음극 몫은 부호 반전 $+IT\,\partial U_\mathrm{an}/\partial T$ 로 들어가나 본 장 범위 밖 — 상단 범위 박스 참조).
\end{srcbox}
...
broadening 절). 주어진 SOC $x$ 에서 $\xi_j$ 를 평가하려면 먼저 전하 보존
음함수~\eqref{eq:implicit} $\sum_j Q_j\xi_{\eq,j}(U_\oc,T)=Qx$ 를 풀어 $U_\oc(x,T)$ 를 얻고, 그 $U_\oc$ 를
$\xi_j(V,T)$ 에 되먹인다. $\Delta S^0_j$ 는 다온도 $dQ/dV$ 동시 피팅으로 얻고, 나머지는 Ch1 모수다.
...
\bibitem{numverif2026} 본 연구 내부 수치 검증(Derivative A), \texttt{Anode\_Fit\_v1.0.13} 4-전이 흑연 staging 파라미터, 유한차분 vs.\ 가중식 비교 (2026). [내부 자료 — 부동소수점 일치 PASS, 본문 \S\ref{ssec:overlap} 요지.]
\end{thebibliography}
