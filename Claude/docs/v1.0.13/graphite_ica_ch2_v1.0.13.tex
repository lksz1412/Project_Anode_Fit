...
기계 대입하면 점유와 진행률이 뒤바뀌는 여집합 함정이 열린다(종은 불변이라 잠복).} 전위가 오르면 ($V>U_j$)
...
다룬다. ★표기: $\theta=$ Li 점유율(만충에서 1), $\xi=1-\theta=$ 탈리튬화 진행률(희박에서 1); 둘은 같은 점유
...
동역학 꼬리$\cdot$분극이 만드는 소산, entropy production)이고, 둘째 항이 가역열로 부호 양방향이다.
...
$\xi_j(V,T)$(본 장 식~\eqref{eq:logistic} — Chapter 1 eq:xieq 의 $s{=}+1$ 형)다. 여기서 $\Delta S^0_j\equiv\Delta S_{\rxn,j}$ 는 봉우리 중심 표준값(파생 B 의
