...
\quad(\text{자리당 }n_j{=}1\text{ 서식 — 전이별 일반형은 }n_jR/F,\ \text{식~\eqref{eq:dxidT}})
...
$\sim0.2$ SOC(state of charge) 부근에서 부호 반전한다고 보고된다\cite{msmr_partII}(★단위 주의 — 이
...
구간에 따라 $[-0.21,+0.14]$ mV/K 범위다(그리드 실측 최대 0.18 은 극값을 정확히 샘플하지 않은 결과 — 해석 상한은 0.21). 내부 전이 경계 3 곳 모두 \emph{계단 없이} 연속 블렌드(인접 $\Delta S^0/F$
...
\end{tikzpicture}
\caption{겹침 가중식~\eqref{eq:weighted} 이 만드는 \emph{연속 블렌드}(파란 실선): $x$ 가 전이 $j$ 에서 $j{+}1$ 로 넘어갈 때 $\partial U_\oc/\partial T$ 가 $\Delta S^0_j/F$ 에서 $\Delta S^0_{j+1}/F$ 로 국소 $dQ/dV$ 비중 $Q_jg_j/\sum Q_kg_k$ 에 따라 \emph{연속}으로 이동한다. 계단(빨간 점선)이 아니다 — 수치 검증에서 내부 전이 경계 3 곳 모두 연속임이 확인됐다 \cite{numverif2026}.}
\label{fig:blend}
\end{figure}
...
분기 중심 $U_j^{(d)}=U_j\pm\tfrac12\Delta U_j^\hys$ 가 서로 달라 같은 $x$ 에서 $g_j^{(\mathrm{ch})}\!\ne\!g_j^{(\mathrm{dis})}$ 이면
