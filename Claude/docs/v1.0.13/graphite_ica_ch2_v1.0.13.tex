% ====================================================================
% graphite_ica_ch2_v1.0.13.tex — Chapter 2 (release 1.0.13; Fable 전수 재검 반영 —
...
\hypersetup{colorlinks=true, linkcolor=blue, urlcolor=blue, citecolor=blue,
  pdftitle={흑연 음극 dQ/dV — Chapter 2 (v1.0.13): 코인 하프셀 엔트로피·가역 발열의 통계열역학}}
\pagestyle{fancy}\fancyhf{}
\lhead{흑연 음극 $dQ/dV$ — Ch.2 (v1.0.13, 엔트로피 통계열역학)}\rhead{\thepage/\pageref{LastPage}}
...
(\S\ref{sec:mixing}). 극한과 코너(corner case — 경계 조건)에서 검산하고(\S\ref{sec:limits}), 마지막으로 분포 재배열로서의 가역
...
있던} logistic 평형 점유의 기원임을 보인다. 이 유도의 통계역학 배경(ensemble$\cdot$Gibbs 인자$\cdot$화학퍼텐셜
...
세 작용처 전용, $\pm1$)와 혼동하지 말 것 — 평형 관계에는 항상 $s{=}+1$ 이 들어가며, $\sigma_d{=}-1$ 을 이 식에
기계 대입하면 점유와 진행률이 뒤바뀌는 여집합 함정이 열린다(종형은 불변이라 잠복).} 전위가 오르면 ($V>U_j$)
Li 의 전기화학 퍼텐셜이 낮아져 자리에서
빠져나가므로 \emph{점유} $\avg{n}=\theta$ 는 $V$ 에 따라 \emph{감소}한다. 식~\eqref{eq:muV} 를
\eqref{eq:occ} 에 대입하고 $\varepsilon_0-\mu^0\equiv0$ 을 전이 중심 기준으로 잡으면, 평형 점유와 그
...
몰 에너지로 쓰면 지수가 $(V-U_j)/(RT/F)=(V-U_j)/w$ — 곧 몰 척도 역온도 $1/RT$ 기준으로 $F/RT=1/w$;
자리당 $\beta=1/\kB T$ 그대로는 $\beta F=N_A/w$ 다). 곧 \emph{점유} $\theta$ 는 $V$ 에
...
물리 유도가 아니라 코드 \texttt{func\_w} 가 $\Omega_j$ 를 가리지 않고 (\texttt{'n'} 키 보유 시) 모든 전이에 공통으로 두는
...
나온다. (한편 그 두-상 영역에서 \emph{실측} 봉우리가 갖는 유한 폭은 평형 등온선이 정하는 양이 아니라 현상학적
피팅 폭이다 — \S\ref{sec:revheat} 종합식의 $w_j$ 지위와 Chapter 1 broadening 절을 보라.)
...
\end{equation}
임을 확인한다(둘째 항의 계수 $R/F$ 는 자리당 $n_j{=}1$ 서식 — 전이별 일반형은 $n_jR/F$ 다, 후술
\S\ref{ssec:overlap} 식~\eqref{eq:dxidT}; $\partial U_j/\partial T=\Delta S_{\rxn,j}/F$ 를 첫 항으로 쓰고, $w=RT/F$ 의 명시적 $T$ 의존이
...
이고 MIT(metal--insulator transition) 같은 급변이 있을 때만 살린다(\S\ref{ssec:elec}).
\end{warnbox}
...
$\sim0.2$ SOC 부근에서 부호 반전한다고 보고된다\cite{msmr_partII}(★단위 주의 — 이
$+3\sim4$ mV/K 는 본 문서 스케일($\Delta S{\approx}+29$ J/(mol\,K) ⇒ $+0.30$ mV/K)과 자릿수가 다르다;
보고 조건$\cdot$단위 상이 가능성이 있어 원문 재확인 대상[미검증]이며, 본 문서 입력값과 무관한 참고 인용이다). 표~\ref{tab:ds} 는 Chapter 1 의 전이별 중심 표준값 $\Delta S^0_j=+29\to0\to-5\to-16$
...
본 장은 그 결과를 분포 언어로 확장해 받는다). 식~\eqref{eq:Se} 의 $S_e$ 는 \emph{자리당}(원자 1개당) 엔트로피로
$\kB$ 차원이다 — 계수의 $\kB^2$ 중 한 몫은 $T\,g(E_F)$($g$ 가 1/에너지 차원)가 상쇄해 알짜로 $\kB$ 가 남는다.
따라서 몰당 $\Delta S(x)$ 분해에 넣을 때는 Avogadro 수를 곱해 환산하며($N_A\kB=R$, 곧 자리당 $\kB$ 가 몰당 $R$ 로
올라감 — Chapter 1 의 단위 다리와 같다). 부분몰 전자 엔트로피
...
음함수 미분으로 닫고 수치로 검증하며, (B) 이중계산을 분리하고, (D) 히스테리시스 분기별 $\partial U/\partial T$ 를
가역/비가역으로 어떻게 가르는지를 닫힌식으로 다룬다. (이상 격자기체의 폭 $w=RT/F$ 는 단상 평형 예측이지만,
두-상 봉우리의 \emph{실현 폭}은 평형이 정하는 양이 아니라 현상학적 자유 피팅 폭이다 — 그 \emph{지위}는
\S\ref{sec:revheat} 종합식에서 못 박고, 폭의 \emph{기원}은 Chapter 1 broadening 절이 다룬다.)
...
\begin{srcbox}
★\textbf{수치 검증(파생 A).} Chapter 1 코드(\texttt{Anode\_Fit\_v1.0.13})의 4-전이 흑연 staging 파라미터로,
...
구간에 따라 $[-0.21,+0.14]$ mV/K 범위다(그리드 실측 최대 0.18 은 극값을 정확히 샘플하지 않은 결과 — 해석 상한은 0.21;
config 항은 $z_j$ 의 로그 구조 탓에 그리드 \emph{양끝에서 계속 커지는 스팬-의존 양}이라, 이 수치들은
\cite{numverif2026} 검증 그리드의 $\bar x$ 스팬 기준이다). 내부 전이 경계 3 곳 모두 \emph{계단 없이} 연속 블렌드(인접 $\Delta S^0/F$
...
\texttt{'n'} 키 보유 전이(검증 데이터셋은 전 전이 해당; \texttt{'w'}-단독 전이는 $T$-동결 폭)에 강제하는 열적 서식 $w_j=n_jRT/F$
...
\subsection{파생 C — 폭 $w$ 의 지위: 단상 평형 예측 vs.\ 두-상 현상학적 피팅 폭}\label{ssec:weff}
앞 두 파생(A$\cdot$B)은 봉우리 폭 $w_j$ 를 \emph{주어진 입력}으로 받아 모양을 닫았다. 그렇다면 그 $w_j$ 자체는
무엇이 정하는가? 답은 전이 종류에 따라 갈리며, 이 \emph{이중지위}가 본 절의 전부다.
\begin{itemize}[leftmargin=1.4em,itemsep=2pt]
\item \textbf{단상($\Omega<2RT$, 균질 고용체)}: 평형 등온선~\eqref{eq:Veq_BW} 이 단조이고, 봉우리 폭은 평형이
...
가 평형을 앞서/뒤처져 과전압 $\eta$ 가 봉우리를 한쪽으로 민다), (ii) 평탄역 가장자리의 내재 단상 폭($\sim RT/F$,
전류 무관), (iii) 집합 다입자 통계역학 — 전극은 입자 \emph{앙상블}이고, 입자마다 apparent-$U=U_j+\eta$ 가
...
크기 효과 배제)는 Chapter 1 의 broadening 절에서 전개하며, 본 장은 그 결과만 받아 $w_j$ 의 \emph{지위}를 못
박는다.}
\end{itemize}
...
$U_j^{(d)}=U_j+\tfrac12\sigma_d\,\Delta U_j^\hys$ 로 두면 — $\sigma_d$ 는 탈리튬화 부호(Ch1 eq:Ubranch 의
...
분기 중심 $U_j^{(d)}=U_j+\tfrac12\sigma_d\,\Delta U_j^\hys$ 가 서로 달라 같은 $x$ 에서 $g_j^{(\mathrm{ch})}\!\ne\!g_j^{(\mathrm{dis})}$ 이면
겹침 가중식~\eqref{eq:hys_branch} 의 config 곡률 때문에 두 분기 평균이 $\Delta S_{\rxn,j}/F$ 로 정확히 떨어지는 것은
$\Delta U^\hys$ 의 1차 차수까지이고, 유한 $\Delta U^\hys$ 에선 고차 보정이 남는다.
히스 gap $\Delta U^\hys$ 자체는 \emph{비가역} 과정으로, $\propto I\,\Delta U^\hys$ 의 소산율(한 사이클당
$\propto Q_\mathrm{cycle}\,\Delta U^\hys$ 의 entropy production 소산열)을 만든다 — 이는 온도 미분 $\partial/\partial T$ 와 \emph{별개}이며 가역열에 섞이지 않는다.
\begin{keybox}
...
상전이 임계를 스스로 짚는다(그 plateau 위 실측 봉우리의 유한 폭은 평형이 아니라 broadening 이 정함, \S\ref{ssec:weff}).
...
는 삽입 $n=1$ 기준 Li 1몰당 J\,mol$^{-1}$K$^{-1}$). SOC 에 따라 $\Delta S(x)$ 의 부호가 바뀌므로 흡$\cdot$발열이 교대한다: 저-$x$ 에서
...
$\xi_j(V,T)$(eq~\eqref{eq:logistic}). 여기서 $\Delta S^0_j\equiv\Delta S_{\rxn,j}$ 는 봉우리 중심 표준값(파생 B 의
정의)이고, $w_j$ 는 전이별 폭이다 — \emph{단상} 전이($\Omega<2RT$)에서는 평형이 예측하는 양(이상 극한
$w_j=RT/F$)이지만, 흑연 staging 의 \emph{두-상} 전이($\Omega>2RT$)에서는 평형 예측 $nRT/F$ 가 아니라 다온도
$dQ/dV$ 로 \emph{피팅하는 현상학적 자유 폭}이다(파생 C \S\ref{ssec:weff}; 폭의 broadening 기원은 Chapter 1
broadening 절). 주어진 탈리튬화 분율 $\bar x$(식~\eqref{eq:implicit} 좌표)에서 $\xi_j$ 를 평가하려면 먼저 전하 보존
음함수~\eqref{eq:implicit} $\sum_j Q_j\xi_{\eq,j}(U_\oc,T)=Q\bar x$ 를 풀어 $U_\oc$ 를 얻고, 그 $U_\oc$ 를
$\xi_j(V,T)$ 에 되먹인다. $\Delta S^0_j$ 는 다온도 $dQ/dV$ 동시 피팅으로 얻고 나머지는 Ch1 모수이며, 단일 전이 지배 영역에서 이 식은
eq~\eqref{eq:single_config} 로 환원된다.
\end{keybox}

\textbf{한계$\cdot$갭(정직).} 본 장의 닫힌식에는 다음 공백이 남는다: (1) 봉우리 내부 config 항을 포함한
\emph{완전식}은 단일 전이 지배 구간뿐 아니라 전이 경계의 겹침 구간을 포함한 전 범위에서 유한차분과 표시
정밀도로 일치한다(파생 A 의 175 점 검증, 절대오차 $\approx0$); 봉우리 내부 $x$-의존을 무시한 \emph{단순식}
(중심 근사)만 그만큼 체계적으로 빗나간다. 남는 공백은
모형 자체의 차수 한계가 아니라, 그 완전식이 가정하는 logistic 전이형$\cdot$파라미터 집합 $\{\Delta S^0_j,Q_j,U_j,w_j\}$
가 실측 흑연을 얼마나 충실히 재현하는지를 \emph{실데이터 round-trip 피팅}으로 확정하는 일이다(시뮬레이션 검증과
실측 검증의 구분). (2) 히스테리시스 하 경로의존 $\partial U_\oc/\partial T$ 의 측정 불확실도
...
\bibitem{numverif2026} 본 연구 내부 수치 검증(Derivative A), \texttt{Anode\_Fit\_v1.0.13} 4-전이 흑연 staging 파라미터, 유한차분 vs.\ 가중식 비교 (2026). [내부 자료 — 표시 정밀도 일치 PASS, 본문 \S\ref{ssec:overlap} 요지.]
\end{thebibliography}
