...
후술 \S\ref{ssec:overlap}$\cdot$\S\ref{sec:revheat}$\cdot$코드 \texttt{func\_w}) \emph{단상 전이($\Omega_j\le2RT$, 균질 고용체)에 한해} 임의
...
두-상 현상학적 자유 피팅 폭 — 를 못 박고(\S\ref{ssec:weff}; \S\ref{sec:revheat} 종합식이 재확인, 폭의
...
$\xi_j(V,T)$(식~\eqref{eq:logistic}). 여기서 $\Delta S^0_j\equiv\Delta S_{\rxn,j}$ 는 봉우리 중심 표준값(파생 B 의
