# -*- coding: utf-8 -*-
"""v1.0.21 하드 게이트 하네스 — G1·G2·G3 전건 수치 증빙 (doc-leads: 문건 v1.0.21 부록 B 요구명세).
...
[G3 θ_E bit-exact] vib 가산 4곳(equilibrium·dqdv·entropy_coefficient·solve_U_oc)을 소스에서
  제거한 '보정 도입 이전' 변형 모듈을 즉석 생성, θ_E 미지정 v1.0.21 과 전 경로 np.array_equal
...
CODE_V20 = os.path.join(HERE, "Anode_Fit_v1.0.21.py")
...
# ---------------------------------------------------------------- 공통 출력셋
def output_set(m):
    """v1.0.19/v1.0.21/변형 모듈 공통 산출 사전 — 신기능 전부 미지정(θ_E·n_T1 無)."""
...
def gate_G1(m19, m20):
    print("=" * 74)
    print("G1: backward-compat  v1.0.21 vs v1.0.19  (new features all unspecified)")
...
def gate_G3(m20):
    ...
    ok &= all_bit
    print(f"  v1.0.21(no theta_E) vs pre-correction variant: arrays={len(out20)}  "
