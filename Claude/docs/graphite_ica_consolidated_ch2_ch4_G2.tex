...
\label{eq:ch3_ratioB}
\end{equation}
detailed balance(식~\eqref{eq:ch3_db}) 와 식~\eqref{eq:ch3_dbwidth} 를 만족하려면 intrinsic 부분이
$\Pi_j^0=\exp[-\mathcal A_j^{0}/RT]$ 형태로 $U_j$ 를 정의하도록 묶인다. \textbf{핵심}: $\mathcal A_j$
가 forward 지수는 키우고($-\beta_j\mathcal A_j$) backward 지수는 줄이므로($+(1-\beta_j)\mathcal A_j$),
\emph{비가 바뀐다} — 즉 정상 target 이 이동한다. 이것이 Level A 와 \emph{다른} 물리다.
...
\item \textbf{일치(극한):} forward/backward 가 detailed balance(식~\eqref{eq:ch3_db})를 만족하면
  $\xi_{j,\mathrm{ss}}=\xi_{\eq,j}$ 이고, $k_j=r_j^++r_j^-$ 로 두면 Level B 가 \emph{정확히} Level A
  로 환원된다. 이때 $\chi_j$(Ch1 mobility 가속 계수)와 $\beta_j$(Ch3 transfer coeff.)는 동일물.
  $\Rightarrow$ \textbf{P3-6 무충돌 성립.}
\item \textbf{차이(novel):} 전류/과전압이 forward/backward 비를 detailed-balance 값에서 벗어나게 하면
  $\xi_{j,\mathrm{ss}}(\mathcal A_j)\ne\xi_{\eq,j}$ — 정상 target 이 \emph{이동}한다. 이는 Level A 가
  주지 못하는 신호(peak 위치의 전류 의존 shift)다. Level A 는 그 특수 극한($\xi_{\mathrm{ss}}\to\xi_\eq$).
\end{itemize}
...
Ch3 의 flux $\mathcal J_j^+=r_j^+(1-\xi_j)$, $\mathcal J_j^-=r_j^-\xi_j$ 로부터(Markov/CRN 형):
\begin{equation}
