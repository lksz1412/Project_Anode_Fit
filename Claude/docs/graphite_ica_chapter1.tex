...
대한 kernel integral 이다. 이 self-consistent spectrum integral 은 \textbf{dual-track}
으로 닫는다: 1안 = \textbf{사용자 박사학위 연구 (Refs~\cite{lee2011,son2013}) 의 propagator
/ ratio-substitution} 으로 얻는 analytic closed-form, 2안 = 그것을 검증하고 필요 시 대체하는
direct numerical integration (정량 전환 기준 + single-mode floor). low temperature 는 spectrum 을 large-$L$ 로, electrode-
...
S10 & ★ closure dual-track: 1안 Refs 6/7 closed-form + 2안 direct (validator) + floor & BOUNDED+DQ \AL{7} \\
...
$k$ & 1/s & rate constant \\
$L$ & --- & local relaxation length. $Q$-좌표서 $v_Q/k$; 정규화 $q=Q_\ext/Q_\cell$ 좌표서 $L=|I|/(Q_\cell k)$ (무차원) \\
...
equation} 이 된다. 시간 적분형은
\begin{equation}
\Theta(q)=\Theta_0+\int_0^q \mathcal K(q,q')\,[\Theta_\eq(q')-\Theta(q')]\,dq',
\label{eq:volterra_self}
\end{equation}
$\mathcal K(q,q')$ 는 식~\eqref{eq:K_def} 의 spectrum kernel. 적분 상한이 $q$ 이므로 본질
적으로 \emph{Volterra} 2nd-kind 다. 본 Chapter 는 이를 \textbf{dual-track} 으로 닫는다:
\emph{1안}(Fredholm + 사용자 PhD 방법으로 closed-form) 을 우선 쓰되, 그 정확도가 떨어지면
\emph{2안}(direct numerical, 항상 유효) 으로 회귀한다. 2안은 동시에 1안의 \textbf{validator}
다.

\subsection{1안 (primary): Fredholm 재정식화 + Refs 6/7 closed-form}\label{sec:track1}
\textbf{Derivation D8 (ratio-substitution closure, \AL{7}).}

\emph{단계 0 — 명시적 Volterra→Fredholm 근사.} 식~\eqref{eq:volterra_self} 는 causal
(triangular) kernel 의 Volterra 다. 이를 고정 구간 Fredholm 으로 바꾸는 것은 \emph{무조건}
성립하지 않으며, 본 Chapter 가 쓰는 controlled approximation 은: post-peak tail window
$[q_a,q]$ 에서 (i) moving target 이 거의 정지 ($d\Theta_\eq/dq\simeq0$, \S\ref{sec:rate})
하여 source 가 frozen 이고, (ii) kernel 의 dominant decay length $\bar L$ 이 window 폭보다
충분히 짧아 ($\bar L \ll q-q_a$) memory 가 局所的이라, self-consistency 가 고정-kernel
형태로 환원된다는 것이다. 이 근사가 언제 깨지는지는 \S\ref{sec:switch} 에서 \emph{측정}한다
(주장하지 않음).

\emph{단계 1 — ratio ansatz.} 사용자 박사 연구 \cite{lee2011,son2013} 의 propagator /
ratio-substitution 을 적용한다. 미지 $\Theta(q')$ 의 두 번째 (자기참조) 적분에만 ratio
$\Theta(q')/\Theta(q)\approx\Theta^{\simplecase}(q')/\Theta^{\simplecase}(q)$
($\Theta^{\simplecase}$ = single reference mode 해, $L$ 고정) 를 대입:
\begin{equation}
\Theta(q)=\Theta_0+\int_0^q\!\mathcal K\,\Theta_\eq\,dq'
-\frac{\Theta(q)}{\Theta^{\simplecase}(q)}\int_0^q\!\mathcal K\,\Theta^{\simplecase}\,dq'.
\label{eq:closed_pre}
\end{equation}
\emph{단계 2 — $\Theta(q)$ 분리.} 우변 마지막 항이 $\Theta(q)$ 를 인자로 가지므로 좌변으로
모아 factor out 하면
\begin{equation}
\boxed{\;\Theta(q)=\frac{\Theta_0+\displaystyle\int_0^q \mathcal K\,\Theta_\eq\,dq'}
{1+\dfrac{1}{\Theta^{\simplecase}(q)}\displaystyle\int_0^q \mathcal K\,\Theta^{\simplecase}\,dq'}.\;}
\label{eq:closed}
\end{equation}
이것이 fitting 에 쓰는 \emph{analytic closed-form} (사용자 PhD 방법을 본 형식에 적용해 얻은
결과) 이며, 그 유효성은 \S\ref{sec:switch} 의 measured switch 에 의해 gating 된다 (exact
로 주장하지 않음). ratio ansatz 의 self-consistency 는 $\Theta_0=0$ reference 정의상 성립.

\subsection{2안 (fallback + validator): direct numerical integration}\label{sec:track2}
1안의 전제(Volterra$\to$Fredholm 근사, 단계 0)가 의심스러우면, 식~\eqref{eq:volterra_self}
(또는 동등하게 미분형 residual ODE 식~\eqref{eq:r_ode} + charge balance) 를 \emph{직접 수치
적분}한다. 이는 \textbf{Fredholm 근사 없이} 풀므로, 주어진 spectrum $A_L$ 과 relaxation
closure 하에서 \emph{numerically exact reference} (discretization 오차로만 제한) 다.
\textbf{단 주의}: 2안은 \emph{1안의 Volterra→Fredholm 근사}만 검증한다 — 두 track 모두
동일한 spectrum 가설(\AL{6})과 relaxation closure(\AL{5})를 공유하므로, 그 \emph{공유 모델}
자체의 오류는 $\varepsilon$ 로 잡히지 않는다 (그것은 \S\ref{sec:falsify} 의 falsification
영역). (P1: 본 Chapter 는 방법만 기술하고 solver 구현은 범위 외.)

\subsection{전환 기준 (1안 ``사용 못할 경우''의 정량 정의)}\label{sec:switch}
\begin{boundbox}
\textbf{Switch criterion (\AL{7}, DQ-v3-2).} 운용 영역 (관심 $T$·rate·tail window) 에서
1안 식~\eqref{eq:closed} 의 결과를 2안(direct) 과 대조해 상대 오차
$\varepsilon = \lVert\Theta_{1}-\Theta_{2}\rVert / \lVert\Theta_{2}\rVert$ 를 측정한다.
$\varepsilon \le \varepsilon_{\mathrm{tol}}$ 이면 1안(closed-form) 사용, 초과하면 2안
(direct) 으로 회귀. 즉 Fredholm 적용성을 \emph{주장}하지 않고 \emph{측정}하여 자동 강등
한다. $\varepsilon_{\mathrm{tol}}$ 의 값은 fitting/검증 단계에서 측정 정밀도에 맞추어
정하며 (구체값은 P1 범위 외), 본 Chapter 는 ``measured switch'' 라는 구조만 규정한다. 이
dual-track 자체가 Volterra-vs-Fredholm 미확정(DQ-v3-2)을 정직하게 처리하는 방식이다.
\end{boundbox}

\subsection{Degenerate floor (최후 안정선)}\label{sec:floor}
spectrum $A_L$ 자체가 식별 불가한 극단에서는, single-mode kernel (식~\eqref{eq:single_kernel},
$A_L=\delta(L-L_0)$ 특수해) 로 회귀한다 — 가장 단순하고 항상 정의되는 floor. 따라서 본
Chapter 의 closure 는 3-tier: \emph{1안 closed-form $\to$ 2안 direct integral $\to$
single-mode floor}. 이는 사용자 3-stage 개발 (알파=2안 reference $\to$ 구조=1안 closed-form
$\to$ 가속) 과 정합.
...
anode 기여를 분리해야 한다. voltage-axis tail scale 은 $L_\varphi\simeq|dV_n/dQ|_{q_a}L$.
...
$\chi_j$ 를 통해 short-$L$ 로 shift 한다. transport 와 \emph{단순} polarization 은 이런
...
\item ★ 관측 tail = spectrum kernel integral (S9); closure = \textbf{dual-track} (1안 Refs 6/7
  closed-form / 2안 direct numerical validator+fallback / single-mode floor) + 정량 전환 기준
  (S10, \AL{7}, DQ-v3-2).
...
사용자 PhD Refs 6/7 = kernel closure 1안 (기법 grounded; 적용성은 measured-switch 로 강등, exact 주장 X) & 통과 (\S\ref{sec:closure}) \\
\bottomrule
\end{longtable}
