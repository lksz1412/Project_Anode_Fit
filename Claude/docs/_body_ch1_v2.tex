...
% ====================================================================
\textbf{물리 — 왜 분포가 필요한가.} \S\ref{sec:rate} 까지는 effective transition $j$ 가 \emph{단일}
intrinsic barrier $G\equiv\Delta G_{a,j}$ 를 갖는다고 두었다. 그러나 실제 전극의 한 transition 안에도
particle size$\cdot$local staging environment$\cdot$domain boundary$\cdot$defect$\cdot$stress$\cdot$
surface 차이가 있어, \emph{같은 transition $j$ 안에서도} domain 마다 intrinsic barrier 값이 다르다. 즉
$G$ 는 하나의 수가 아니라 \emph{분포}를 이룬다. 이 분포가 \S\ref{sec:staging} 에서 예고한 local mode
분포의 실체이며, 본 절의 목표는 ``barrier 가 분포하면 관측 tail 이 어떻게 바뀌는가''를 무비약으로 잇는 것이다.

\textbf{표기 약속(\,$g$ 는 $G$ 의 적분 더미\,).} 혼동을 막기 위해 분포를 적분할 때의 \emph{더미 변수}를
$g$ 로 쓴다. $g$ 와 $G$ 는 \emph{같은 물리량}(transition $j$ 의 intrinsic activation barrier,
$\mathrm{J\,mol^{-1}}$)이며, $g$ 는 ``$G=g$ 인 domain 집단''을 가리키는 적분 변수일 뿐이다. transition
$j$ 안에서 $G=g$ 근방 $[g,g+dg)$ 에 속하는 domain 의 \emph{인구 비율}을 $\rho_G(g;T)\,dg$ 로 두고, 음수
아닌 장벽 영역에서 정규화한다(인구 보존):
\begin{equation}
\rho_G(g;T)\ge0,\qquad \int_0^\infty \rho_G(g;T)\,dg=1.
\label{eq:rho_norm}
\end{equation}
$\rho_G$ 는 \emph{kinetic} activation barrier 분포(어느 domain 이 얼마나 느린가)이며, \S\ref{sec:eq} 의
\emph{평형} 전이 중심·폭 불균일성($U_j,w_j$ 또는 별도 분포 $p_j(U)$, 어느 domain 이 어느 전위에서 변하는가)과
물리적으로 다른 대상이다 — 둘을 한 분포로 섞지 않는다.
...
\textbf{집단별 동역학 = \S\ref{sec:dyn} 의 식을 자기 장벽으로 반복.} $G=g$ 인 domain 집단은 \S\ref{sec:rate}
의 식~\eqref{eq:Geff},\eqref{eq:kact} 를 \emph{그대로} 따르되 자기 장벽 $g$ 를 넣는다: 유효 장벽
$\Delta G_{\eff,j}(g)=g-\chi_j\mathcal A_j$, 활성화 rate
$k_j^\mathrm{act}(g)=\nu_j(T)\exp[-(g-\chi_j\mathcal A_j)/RT]$, 병목 포함 시
$1/k_j^\mathrm{phys}(g)=1/k_j^\mathrm{act}(g)+1/k_{j,\mathrm{lim}}(g)$(식~\eqref{eq:kphys}). 따라서 각
집단의 진행률 $\xi_j(g,t)$ 은 \S\ref{sec:dyn} 의 single-mode 완화식(식~\eqref{eq:dxidt})을 \emph{집단마다}
반복한 것이다:
\begin{equation}
\frac{d\xi_j(g,t)}{dt}=k_j^\mathrm{phys}(g)\,[\xi_{\eq,j}(V_n,T)-\xi_j(g,t)].
\label{eq:xi_class_ode}
\end{equation}
평형 target $\xi_{\eq,j}(V_n,T)$ 은 모든 집단이 \emph{공통}으로 본다(평형은 전위가 정하지 장벽이 정하지
않음); 집단마다 다른 것은 그 target 으로 가는 \emph{속도} $k_j^\mathrm{phys}(g)$ 뿐이다. transition $j$ 의
\emph{관측} 진행률은 이 집단들을 인구분포 $\rho_G$ 로 가중평균한 것이다:
\begin{equation}
\boxed{\;\xi_j(t)=\int_0^\infty \rho_G(g;T)\,\xi_j(g,t)\,dg\;.}
\label{eq:xi_dist}
\end{equation}
즉 ``느린 집단이 평형을 못 따라가 남긴 잔차''가 합쳐져 관측 tail 이 된다 — 이것이 \S\ref{sec:kernel} 적분의
물리적 내용이며, 아래에서 그 합을 명시적으로 length 좌표로 옮긴다.

\textbf{1단계 — barrier$\to$length 지수 매핑(stretched tail 의 근원).} 관측 tail 의 shape 은 $\rho_G$ 와
\emph{같지 않다}. 이유: 장벽 $g$ 가 먼저 rate 로, 다시 relaxation length 로 \emph{지수 변환}되기 때문이다.
\S\ref{sec:singlemode} 의 $L=|I|/(Q_\cell k_j)$(식~\eqref{eq:single_kernel})에 활성화-지배 rate
$k_j^\mathrm{act}(g)=\nu_j(T)\exp[-(g-\chi_j\mathcal A_j)/RT]$(식~\eqref{eq:kact})를 넣으면, 장벽 $g$ 인
집단의 relaxation length 는
\begin{equation}
\boxed{\;L(g;T,V_{n,\drive})=\frac{|I|}{Q_\cell\,\nu_j(T)}\exp\!\Big[\frac{g-\chi_j\mathcal A_j}{RT}\Big]\;}
\qquad(k_0(T)\equiv\nu_j(T)\ \text{표기 통일}).
\label{eq:L_of_G}
\end{equation}
(병목 지배 영역은 식~\eqref{eq:kphys} 의 $k_j^\mathrm{phys}$ 로 $L=|I|/(Q_\cell k_j^\mathrm{phys})$ 를
쓴다; 활성화 지배에서 위 지수형으로 환원.) $g$ 의 \emph{작은} 분산이 $L$ 에서 \emph{지수적으로} 확대되므로,
긴 꼬리는 $\rho_G$ 자체가 넓어서가 아니라 매핑이 지수이기 때문일 수 있다(저온 $RT\downarrow$ 면 확대가 더 심함).

\textbf{2단계 — 인구밀도를 length 좌표로 옮김(질량보존 Jacobian; 무비약).} $\rho_G$ 는 $g$ 에 대한
\emph{확률밀도}이므로 $L$ 좌표로 옮길 때 단순 대입이 아니라 인구(확률질량) 보존을 써야 한다:
\begin{equation}
\phi_L(L)\,dL=\rho_G(g)\,dg
\quad\Longrightarrow\quad
\phi_L(L)=\rho_G\big(g(L)\big)\,\Big|\frac{dg}{dL}\Big|.
\label{eq:massconv}
\end{equation}
식~\eqref{eq:L_of_G} 의 역변환은 $g(L)=\chi_j\mathcal A_j+RT\ln\!\big[Q_\cell\nu_j L/|I|\big]$ 이고
$dg/dL=RT/L>0$(단조 증가)이므로, $g\!\to\!L$ 이 일대일이라 역변환이 유일하다(단조성은 $k$ 가 $g$ 의 단조함수
임에서 옴). 따라서 \emph{정규화된 인구 spectrum}은
\begin{equation}
\phi_L(L;T,V_{n,\drive})=\rho_G\big(g(L)\big)\,\frac{RT}{L},\qquad
\int_0^\infty \phi_L(L)\,dL=\int_0^\infty \rho_G(g)\,dg=1.
\label{eq:popspectrum}
\end{equation}
$\phi_L$ 은 ``length 가 $L$ 근방인 domain 이 인구의 몇 %인가''만 옮긴 것이라 규격이 보존된다($\int\phi_L\,dL=1$).
\textbf{그러나 관측 tail 의 진폭은 인구만으로 정해지지 않는다} — 각 집단이 \emph{용량을 얼마나 차지하며}, peak
탈출 시점에 \emph{얼마나 미완료(잔차)}로 남았는지가 가중으로 들어간다. 그 가중을 $A_0(L)$ 로 묶어
\begin{equation}
A_L(L;T,V_{n,\drive})\equiv \phi_L(L)\,A_0(L)=\rho_G\big(g(L)\big)\,\frac{RT}{L}\,A_0(L)
\label{eq:spectrum}
\end{equation}
로 정의한다. $A_0(L)$ 의 \emph{구성·단위·결정법}은 다음 \S\ref{sec:kernel} 에서 관측 tail 을 명시적으로
합산하며 \emph{유도}한다(임의 인자 아님). 균일 잔차$\cdot$균일 용량 가중이면 $A_0\equiv1$ 이고 그때
$A_L=\phi_L$ 로 규격 보존된다.
\begin{boundbox}
...
% ====================================================================
관측되는 것은 개별 집단이 아니라 전체 phase progress $\Theta$ 의 도함수다. 아래 \textbf{S1--S5} 다섯 단계로
$\Theta$ 의 tail 을 length spectrum 적분으로 \emph{유도}한다(가정해서 적는 것이 아니다).

\textbf{S1 — 출발: 전체 진행률의 정의.} \S\ref{sec:charge} 식 직후의 정의
$Q_p\Theta\equiv\sum_j Q_{j,\tot}\xi_j$ 와 인구평균 식~\eqref{eq:xi_dist} 을 결합하면
\begin{equation}
Q_p\,\Theta(t)=\sum_{j=1}^{N_p}Q_{j,\tot}\,\xi_j(t)
=\sum_{j=1}^{N_p}Q_{j,\tot}\!\int_0^\infty \rho_G(g;T)\,\xi_j(g,t)\,dg.
\label{eq:theta_from_classes}
\end{equation}

\textbf{S2 — $q$ 로 미분(정전류).} 정전류서 $d(\cdot)/dq=(Q_\cell/|I|)d(\cdot)/dt$ 이므로 양변을 $q$ 로
미분하면(용량 $Q_{j,\tot},Q_p$ 는 상수)
\begin{equation}
Q_p\,\frac{d\Theta}{dq}=\sum_{j=1}^{N_p}Q_{j,\tot}\!\int_0^\infty \rho_G(g;T)\,\frac{d\xi_j(g,t)}{dq}\,dg.
\label{eq:dtheta_step}
\end{equation}

\textbf{S3 — 집단별 post-peak 해 대입.} peak 통과 직후($q>q_a$, target 거의 정지 $d\xi_{\eq,j}/dq\simeq0$)
각 집단은 \S\ref{sec:singlemode} 의 결과(식~\eqref{eq:single_dxidq})를 자기 length $L=L(g)$ 로 따른다:
\begin{equation}
\frac{d\xi_j(g,t)}{dq}=\frac{r_j(q_a;g)}{L(g)}\,\exp\!\Big[-\frac{q-q_a}{L(g)}\Big],\qquad
r_j(q_a;g)\equiv\big[\xi_{\eq,j}-\xi_j(g,t)\big]_{q=q_a}
\label{eq:class_dxidq}
\end{equation}
($r_j(q_a;g)$ = 그 집단이 peak 탈출 시점 $q_a$ 에 남긴 \emph{미완료 잔차}, 무차원 $\in[0,1]$). 이 잔차가
클수록(느린 집단) tail 기여가 크다 — tail 의 물리적 근원이 여기서 명시된다.

\textbf{S4 — 적분변수를 $g$ 에서 $L$ 로 바꿈.} 식~\eqref{eq:class_dxidq} 를 식~\eqref{eq:dtheta_step} 에
넣고, 각 $j$ 의 $g$-적분을 식~\eqref{eq:L_of_G},\eqref{eq:massconv} 의 Jacobian
$\rho_G(g)\,dg=\phi_{L,j}(L)\,dL$($\phi_{L,j}=\rho_G(g(L))\,RT/L$)로 $L$-적분으로 바꾸면
\begin{equation}
\frac{d\Theta}{dq}
=\int_0^\infty\!\underbrace{\Big[\frac{1}{Q_p}\sum_{j=1}^{N_p}Q_{j,\tot}\,r_j(q_a;L)\,\phi_{L,j}(L)\Big]}_{\displaystyle \equiv\,A_L(L)}
\;\frac{1}{L}\,\exp\!\Big[-\frac{q-q_a}{L}\Big]\,dL.
\label{eq:tail_derived}
\end{equation}

\textbf{S5 — $A_0$ 의 정체 확정.} 식~\eqref{eq:tail_derived} 의 대괄호가 바로 식~\eqref{eq:spectrum} 의
$A_L=\phi_L A_0$ 이며, 따라서 \emph{앞에서 미뤄둔 $A_0$ 가 무엇인지 여기서 정해진다}:
\begin{equation}
\boxed{\;A_0(L)=\underbrace{\frac{Q_{j,\tot}}{Q_p}}_{\text{용량 가중}}\cdot
\underbrace{r_j(q_a;L)}_{\text{peak 탈출 잔차}}\cdot
\underbrace{a_j^\mathrm{acc}(L)}_{\text{accessibility}}\;}\quad(\text{무차원}),
\label{eq:A0_def}
\end{equation}
즉 $A_0$ 는 (i) 그 집단이 차지하는 \emph{용량 비율} $Q_{j,\tot}/Q_p$, (ii) peak 탈출 시 \emph{미완료 잔차}
$r_j(q_a)$, (iii) 실제로 반응에 참여하는 \emph{accessibility} $a_j^\mathrm{acc}\in[0,1]$ 의 곱이다 — 세 가지
모두 독립적으로 측정/모형되는 물리량이지 자유 knob 이 아니다($a_j^\mathrm{acc}\equiv1$, 균일 잔차이면 $A_0$
는 상수). 단일 transition($N_p=1$)이면 합이 사라지고 $A_L(L)=r(q_a)\,a^\mathrm{acc}\,\phi_L(L)$. 이로써
``$r(q_a)$ 가 $A_0$ 에 흡수된다''는 앞서의 축약 표현이 \emph{정확히 어느 곱항인지} 확정됐다. 결과:
\begin{equation}
\boxed{\;\frac{d\Theta_\mathrm{tail}}{dq}=\int_0^\infty A_L(L)\,\frac1L\,
\exp\!\Big[-\frac{q-q_a}{L}\Big]\,dL\;.}
\label{eq:kernel_integral}
\end{equation}
\textbf{해석.} $1/L$ 인자는 식~\eqref{eq:class_dxidq} 진행률 도함수에서 온 것이지 임의 가중이 아니다. 이는
Laplace-type mixture 라 $\rho_G$ 형태에 따라 단일지수$\sim$stretched$\sim$power-law 를 smooth 하게 잇는다.
single-mode(식~\eqref{eq:single_kernel})는 $A_L=\delta(L-L_0)$ 특수해이며 그때
$d\Theta_\mathrm{tail}/dq=(r(q_a)/L_0)e^{-(q-q_a)/L_0}$ 로 \S\ref{sec:singlemode} 와 정확히 일치한다(무비약 점검).
일반적인 두 시점 kernel
\begin{equation}
...
% ====================================================================
\textbf{왜 \S\ref{sec:kernel} 적분으로는 부족한가.} 식~\eqref{eq:kernel_integral} 은 ``peak 탈출 후 target
이 거의 정지''($d\xi_{\eq,j}/dq\simeq0$)라는 \emph{국소} 가정에서 나온 tail 이다. 그러나 일반적으로 target
$\Theta_\eq(V_n,T)$ 은 $q$ 에 따라 움직이고, 더구나 $V_n$ 은 charge balance(식~\eqref{eq:charge_balance})로
\emph{현재 $\Theta$ 자신}에 묶여 있다($Q_\cell q=Q_\bg(V_n)+Q_p\Theta$). 두 가지를 모두 살리려면 ``움직이는
target 으로의 완화''를 풀어야 한다.

\textbf{단계 1 — 움직이는 target: Duhamel(변수변환 적분).} length $L$ 인 집단의 1계 선형 완화
$d\xi(L)/dq=(1/L)[\Theta_\eq(q')-\xi(L)]$ 의 해는 자유감쇠 + 과거 target 이력의 convolution(Duhamel
원리)이다: $\xi(L,q)=\xi(L,0)e^{-q/L}+\int_0^q (1/L)e^{-(q-q')/L}\Theta_\eq(q')\,dq'$. 이를 spectrum
$A_L$ 로 가중합하면(\S\ref{sec:kernel} 와 같은 합산) 자유항 $\Theta_\mathrm{init}$ 와 memory kernel
$\mathcal K(q,q')$(식~\eqref{eq:Kdef})가 나온다. 식~\eqref{eq:kernel_integral} 은 이 일반형에서 target 을
상수로 두고 미분한 \emph{국소 극한}이다(무비약 포함관계).

\textbf{단계 2 — self-consistency: 왜 Volterra 2종인가.} target 안의 $V_n$ 이 식~\eqref{eq:charge_balance}
로 $\Theta(q')$ 에 의존하므로 $\Theta_\eq=\Theta_\eq(V_n(q',\Theta(q')),T)$ 가 되어, 미지 $\Theta$ 가 적분
안에도 나타난다. 적분 상한이 \emph{인과적}으로 현재 $q$ 인($q'\le q$, 과거만 영향) (비선형) \emph{Volterra}
2종 적분방정식이며, kernel 은 잔차가 아니라 \textbf{target 에 곱해진다}:
\begin{equation}
...
\subsection{실데이터에서 바로 쓰는 최소 근사식 (minimal fittable form)}\label{sec:minfit}
위 식~\eqref{eq:fiteq} 는 일반형이라 spectrum 적분을 품는다. \textbf{실측 $dQ/dV$ 곡선에 곧장 비선형
최소제곱으로 맞출 수 있는 가장 단순한 닫힌 형태}는 다음 두 가정에서 나온다: (i) 각 peak 가 \emph{지배}
relaxation-length $L_j$ 하나로 근사된다(narrow-spectrum; spectrum 폭은 보정항으로 후행), (ii) post-peak
에서 $V_n$ 이 완만 변화해 $L$ 을 국소 상수로 본다(\S\ref{sec:singlemode} 숨은 전제). 그러면 peak $j$ 의
\emph{전압축} tail 은 지수 감쇠이고, 전위 환산 감쇠길이는
\begin{equation}
\boxed{\;L_{\varphi,j}(V,T,|I|)=\Big|\frac{dV_n}{dQ}\Big|\,Q_\cell\,L_j,\qquad
L_j=\frac{|I|}{Q_\cell\,\nu_j(T)}\exp\!\Big[\frac{\Delta H_{a,j}-T\Delta S_{a,j}-\chi_j F\,(V-U_j)}{RT}\Big]\;}
\label{eq:Lphi_fit}
\end{equation}
(식~\eqref{eq:L_of_G} 에 $\Delta G_{a,j}=\Delta H_{a,j}-T\Delta S_{a,j}$, $V_{n,\drive}\!\approx\!V$ 대입).
이를 평형 logistic peak(식~\eqref{eq:dxidV})에 \emph{비대칭 꼬리}로 결합하면, 측정축에서 직접 피팅하는
최소 모델은
\begin{equation}
\boxed{\;\frac{dQ}{dV}(V;T,|I|)\;\approx\;\underbrace{C_\bg(V,T)}_{\text{배경}}
+\sum_{j=1}^{N_p} Q_{j,\tot}\,\Pi_j\!\Big(V-U_j-s_I|I|R_n;\,w_j,\,L_{\varphi,j}\Big)\;}
\label{eq:minfit}
\end{equation}
여기서 $\Pi_j$ 는 \emph{대칭 평형 peak}(폭 $w_j$, 식~\eqref{eq:dxidV} 의 $\xi_\eq(1-\xi_\eq)/w_j$)와
\emph{지수 tail}(감쇠길이 $L_{\varphi,j}$)의 합성 — 즉 ``평형이 빠른 영역은 logistic, 평형을 못 따라가는
post-peak 영역은 $\exp[-(V-U_j)/L_{\varphi,j}]$''로 이어지는 비대칭 peak 이다. EMG 와 형태는 닮았으나
\textbf{$\mu_j=U_j+s_I|I|R_n$, 폭$=w_j$, tail$=L_{\varphi,j}$ 가 모두 물리량}(식~\eqref{eq:Lphi_fit})이라
온도$\cdot$전류 의존이 \emph{고정}된다는 점이 결정적 차이다(경험식은 $T,I$ 마다 따로 fit, 본 식은 한 세트
파라미터로 전 $T,I$ 동시 fit).

\textbf{실데이터 추출 절차(요약; 상세 순서 \S\ref{sec:ident}).}
\begin{enumerate}[label=\textbf{F\arabic*.},leftmargin=2.4em]
\item \textbf{저율(0.1$\sim$0.2C) $dQ/dV$} 로 평형 골격 추출: 각 peak 위치 $U_j$, 폭 $w_j$, 면적
  $Q_{j,\tot}$, 배경 $C_\bg(V,T)$. (이 단계서 $L_{\varphi,j}\!\to\!0$ 근사 — 저율 tail 무시.)
\item \textbf{pulse/current-interrupt} 로 $R_n(q,T,|I|)$ 독립 제한 $\Rightarrow$ peak 이동
  $s_I|I|R_n$ 분리.
\item \textbf{다중 온도} tail 의 감쇠길이 $L_{\varphi,j}(T)$ 를 식~\eqref{eq:Lphi_fit} 의 Arrhenius
  $\ln L_{\varphi,j}$ vs $1/T$ 기울기로 $\Delta H_{a,j},\Delta S_{a,j},\nu_j$ 추출.
\item \textbf{다중 전류/전위} tail 의 $\partial\ln L_{\varphi,j}/\partial V=-\chi_j F/RT$ 기울기로 $\chi_j$
  추출(단, \S\ref{sec:falsify} 의 $\eta_\mathrm{ct}$ 분리 선행).
\item narrow-spectrum 잔차가 크면 (i) Gaussian $\rho_G(g)$ 폭 $\sigma_j$ 를 도입해 식~\eqref{eq:kernel_integral}
  의 log-normal length-mixture 로 tail 을 stretched 화, 또는 (ii) 병목 $k_{j,\lim}$ 추가(물리 근거 명시).
\end{enumerate}
이 절차를 거치면 \textbf{Chapter 1 만으로} 실측 $dQ/dV(V;T,|I|)$ 에 대한 물리 파라미터 근사식
(식~\eqref{eq:minfit})을 산출할 수 있다. 전 spectrum 적분(식~\eqref{eq:fiteq})·self-consistent
closure(\S\ref{sec:closure})는 이 최소식의 \emph{정밀화}이며, narrow-spectrum 가정의 타당성은 F5 잔차로 측정한다.
