...
gap $\gamma_j\Delta U_j^\hys$(\S\ref{sec:regsol} 의 $\Omega_j$$\cdot$$\gamma_j$), \emph{기울기}가 $2R_n$ 다.
$R_n$ 은 ohmic$\cdot$전하이동$\cdot$확산을 한 양으로 묶은 \emph{lumped} 저항이며(식~\eqref{eq:vapp} 의 공통
$R_n$; SOC 의존이 크면 전이별 $R_{n,j}$ 로 확장하나 본 장 기본은 공통 1개), 본 장 피팅은 그
합본만 쓴다 — 율 의존은 \S\ref{sec:lag} 의 지연 꼬리와 같은 뿌리(평형을 못 따라감)다.
\begin{flagbox}
...
\end{equation}
로 줄어든다($h_j$ 단조 증가). 완전 cycle 만 다루면 $h_j=1$ 이라 앞 절들이 그대로고, 부분 cycle 데이터가 있을 때
$h_j$ 가 major-loop 사이의 보간을 준다 — \S\ref{sec:master} 의 예측이 부분 SOC 창에서도 닫히게 하는 \emph{최소}
확장이다(Preisach 류 분포 모형은 본 장 범위 밖; return-point + $h_j$ 면 충분).
