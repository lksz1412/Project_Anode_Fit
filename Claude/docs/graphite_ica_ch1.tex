...
만든다). 경계는 $g_j''$ 이 가장 작은 $\xi=\tfrac12$ 서 $4RT-2\Omega_j=0$, 곧
...
기술한다: 전환율이 분기 폭의 일부 $\eta\in[0,1]$ 만 진행했으면 유효 gap 이
\begin{equation}
\Delta U_{j}^{\hys,\,\mathrm{part}}=h_j(\eta)\,\Delta U_j^\hys,\qquad h_j(0)=0,\;h_j(1)=1,
\label{eq:hys_partial}
\end{equation}
로 줄어든다($h_j$ 단조 증가; 데이터 없으면 \emph{선형 기본형} $h_j(\eta)=\eta$, 곡률은 부분 cycle 데이터로 보정).
