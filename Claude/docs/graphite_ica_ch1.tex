...
절편과 기울기가 섞여 분리 불가다. 따라서 외삽은 $\gamma_j$ 가 평탄한 \emph{저$\sim$중율} 구간에 한정하고, 분극이 peak
\emph{위치}만 옮기고 \emph{형상}은 \S\ref{sec:lag} 꼬리로 따로 처리됨을 전제한다.
$R_n$ 은 ohmic$\cdot$전하이동$\cdot$확산을 한 양으로 묶은 \emph{lumped} 저항이다(식~\eqref{eq:vapp} 의 공통 $R_n$).
공통 $R_n$ 채택은 ohmic$\cdot$SEI 성분이 전이 간 공유되고 전하이동$\cdot$확산의 전이별 차가 측정 창 내에서 2차라는
\emph{검증 가능} 가정이다 — \textbf{전이별 gap 기울기가 유의하게 갈리면 기각}되고 전이별 $R_{n,j}$ 로 간다(도피가 아니라
falsifiable 예측: 모든 전이의 $2|I|R_n$ 기울기가 같아야 함). \emph{또 $2|I|R_n$ 의 \textbf{선형}성은 ohmic 근사다} —
