...
관찰(저온 긴 꼬리)을 수치로 압축한다. \emph{이 수치는 뒤에서 그대로 쓰인다}: $S_1$ 의 OCV peak 판독에서 기대
FWHM($\approx91$ mV)의 sanity-check 기준값이고, 다음 절 융합 판정(\S\ref{sec:overlap})이 이 $91$ mV 를 전이 간격과
비교하는 입력으로 받는다.
\end{verifybox}
...
(아니면 \S\ref{sec:overlap} forward-only). 또 $\Delta S_{a,j}$ 는 prefactor 와 결합으로만 들어가 \emph{단독 비식별}
(온도 외삽엔 절편 흡수로 무해). 이 단서가 깨지면 해당 추가 절차가 켜진다 — \emph{식은 하나지만 적용역은 이 울타리 안}.
\end{keybox}
...
\item \textbf{온도 소멸.} 그 절편 $\gamma_j\Delta U_j^\hys(T)$ 는 $T\to T_{c,j}=\Omega_j/2R$ 서
  ...
  외삽 부호}라는 약화된 검정을 쓴다(평균장 임계지수의 정량값은 본 장 근사의 한계). 절편이 온도에 무관하면 정규용액
  상분리 그림이 틀렸다는 신호다(예: 표면 재구성$\cdot$기계적 응력 기원 히스).
