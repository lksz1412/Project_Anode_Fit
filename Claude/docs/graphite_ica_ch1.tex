...
$RT\lesssim\mathcal A_j\lesssim3RT$ 에서는 단일지수가 \emph{선도 근사}일 뿐 보정 인자 $(1+e^{-\mathcal A_j/RT})$ 가
최대 $\sim37\%$($\mathcal A_j=RT$)까지 살아 있다 — 이 편차는 \emph{버리지 않고} \S\ref{sec:dist} 의 배리어 분포 중첩이
흡수한다(좁은 분포 극한서만 순수 단일지수). \emph{따라서 ``꼬리식을 $\mathcal A_j\gtrsim RT$ 에 쓴다''는 선도형이
그렇다는 뜻이고, 정확성은 $\gtrsim3RT$ 한정}이다. (파라미터 식별: $\{\Delta G_{a,j},\chi,k_0\}$ 피팅$\cdot$
...
\item \textbf{추정 강건성(새 정보 아님)}: $\dd V/\dd Q$ 와 $\dd Q/\dd V$ 는 전단사 변환이라 \emph{정보량은 같다} —
  한쪽이 더 많은 정보를 줄 수 없다. 다른 것은 \emph{추정량의 잡음 전파}다: DVA peak(경계)는 전위가 급변하는 급경사
  구간이라 peak-finding 의 위치 \emph{분산}이 ICA peak 중심 추정보다 작다(같은 $U_j$ 를 \emph{더 낮은 분산}으로 추정).
...
$\dd V/\dd Q$ 의 valley 폭$\cdot$깊이도 \emph{새 물리가 아니라} 같은 모델에서 온다: valley 폭은 식~\eqref{eq:weff}
의 $w_j$(정규용액으로 좁아진 유효 폭), 비대칭은 식~\eqref{eq:Lq} 의 동역학 꼬리, 충$\cdot$방전 DVA 의 갈림은
\S\ref{sec:hys_branch} 의 분기(식~\eqref{eq:hys_center})에서 같은 $\{\Omega_j,\gamma_j,R_n\}$ 로 결정된다. 그러므로 본 장 피팅은
$\dd Q/\dd V$ 로 수행하고, $\dd V/\dd Q$ 는 \emph{전이 경계 위치를 더 낮은 분산으로 추정}(S1 강건화 — 새 정보가 아니라
추정 경로의 차)하는 데 병용한다 — 같은 모델식의 두 그림.
...
일반화한 것 — 방전 $s_I=\sigma_d=+1$, 충전 $\sigma_d=-1$)로
\begin{equation}
...
\end{itemize}
한 전이의 충방전 peak 쌍은 \emph{면적 $Q_j$$\cdot$폭 $w_j$ 동일, 중심만} $\gamma_j\Delta U_j^\hys+2|I|R_n$ 갈린다 —
방전 전용 식~\eqref{eq:master} 의 모든 파라미터에 \emph{전이당 $\{\Omega_j,\gamma_j\}$ 둘만 더}하면 충방전이 닫힌다.
...
데이터원은 분리돼 단계적. \textbf{단, ``하나로 끝난다''는 다음 가정 하에서다}: 활성화 지배($k_{j,\lim}$ 무한, 아니면
\S\ref{sec:falsify}(i) 수송 진단 선행)$\cdot$좁은 배리어 분포($\rho_G\to\delta$, 휘면 \S\ref{sec:dist} 적분 켜짐)$\cdot$
prefactor $T$-안정$\cdot$$q_a$ 컷 고정$\cdot$$\xi=\tfrac12$ 대칭. 또 $\Delta S_{a,j}$ 는 prefactor 와 결합으로만 들어가
\emph{단독 비식별}(온도 외삽엔 절편 흡수로 무해, 절대 prefactor 가 필요하면 별도 고정). 이 단서들이 깨지면 해당
추가 절차가 켜진다 — \emph{식은 하나지만 그 적용역은 위 가정이 친 울타리 안}이다.
\end{keybox}
