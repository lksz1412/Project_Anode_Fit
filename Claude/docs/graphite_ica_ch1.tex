...
$\Omega_j=2RT$($T=T_{c,j}$)서 폭이 $0$ 으로 — 그 지점이 곧 상분리(delta 형 plateau)의 시작이다. \textbf{이것이
...
(식~\eqref{eq:keff}; peak 중심의 $k_j\simeq2r_j^+$ 와의 인자 차는 그 영역이 평형 상승부로 지배되어 꼬리 절대값에
무영향)로 읽는다 — 즉 본 식과 아래 모든 꼬리 표현의 $k_j$ 는 \emph{유효 속도} $k_j^\eff$ 다.
...
exponential)를 준다. \S\ref{sec:dist} 는 이 다섯 기술로 입자 분포를 정량화한다 — 새 물리가 아니라 분율 가중 합산이다.
\end{keybox}
...
\label{eq:hys_dQdV}
\end{equation}
여기서 $\dd\xi_{\eq,j}/\dd V$ 는 식~\eqref{eq:dxidV} 의 종 모양(폭 $w_j$)이고 중심만 $U_j\to U_j^{\dis/\chg}$ 로 옮겼다.
그러므로 한 전이는 ICA 에서 \textbf{peak 쌍}으로 나타나며, 그 쌍의
\begin{itemize}[leftmargin=1.4em,itemsep=2pt]
...
\item \textbf{면적} $Q_j$ 는 양방향 동일(같은 전하량이 같은 전이를 지난다),
\item \textbf{모양$\cdot$폭}은 같은 $w_j$ (같은 평형 열역학).
\end{itemize}
...
falsifiable 예측: 모든 전이의 $2|I|R_n$ 기울기가 같아야 함). 율 의존은 \S\ref{sec:lag} 지연 꼬리와 같은 뿌리다.
\begin{flagbox}
...
$\to\chi$; \textbf{S4} 다온도 Arrhenius$\to\Delta H_{a,j}$; \textbf{S5}(충방전 시) 여러 율의 충방전 gap
...
\item \textbf{온도 소멸.} 그 절편 $\gamma_j\Delta U_j^\hys(T)$ 는 $T\to T_{c,j}=\Omega_j/2R$ 서
  ...
  $u_j=\sqrt{1-T/T_{c,j}}$, $\Delta U_j^\hys\!\propto\!u_j^3$). \emph{단 지수 $3/2$ 는 \textbf{평균장} 정규용액 값으로
