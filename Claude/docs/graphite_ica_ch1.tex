...
유일하다 \cite{bazant2013}. \emph{비평형}에서 staging 항 $\sum_j Q_j\xi_j$ 은 주어진 입력이라 $V_n$-상수이므로,
$V_n$ 은 배경 $Q_\bg(V_n,T)$ 의 단조성 하나로 닫힌다 — 전이항은 배경에 남길 잔여 전하를 통해 $V_n$ 을 \emph{간접}
으로 민다(평형 분기에서만 아래처럼 $\xi_{\eq,j}(V_n)$ 의 직접 의존이 더해진다).
...
충방전 gap(\S\ref{sec:hys_branch})에서 독립으로 얻는 $\Omega_j$ 와 \emph{같아야} 한다(\S\ref{sec:falsify} 교차검증).
...
\item \textbf{$\Omega_j$ 의 두 독립 경로 일치.} 정규용액 $\Omega_j$ 는 \emph{두 가지로 따로} 얻힌다 — (a) 평형 peak
  \emph{폭}에서 $w_j=(RT/F)(1-\Omega_j/2RT)$(식~\eqref{eq:weff}, S1), (b) 충방전 \emph{gap} 절편에서
  식~\eqref{eq:hys_dU}(S5). 같은 $\Omega_j$ 라면 \emph{둘이 일치}해야 한다 — 어긋나면 폭 좁아짐이나 gap 중 하나가
  정규용액 상분리 아닌 다른 기원(입자 분산$\cdot$표면)임을 뜻한다. 이 \emph{과결정(over-determination)}이 본 장의
  가장 강한 자기일관 시험이다.
\end{enumerate}
