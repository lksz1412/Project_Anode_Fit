...
\hypersetup{colorlinks=true, linkcolor=blue, urlcolor=blue, citecolor=blue,
  pdftitle={흑연 음극 dQ/dV 피크의 물리 — Chapter 1}}
\pagestyle{fancy}\fancyhf{}
\lhead{흑연 음극 dQ/dV 피크의 물리 — Ch.1}\rhead{\thepage/\pageref{LastPage}}
...
\title{\textbf{\LARGE Chapter 1}\\[0.35em]\Large 흑연 음극 $dQ/dV$ 피크의 물리}
\author{}
\date{}
...
% ====================================================================
\section{무대: 전하 보존이 내부 전위 $V_n$ 을 정한다}\label{sec:stage}
% ====================================================================
\textbf{질문.} 상전이를 끌고 가는 것은 음극 \emph{내부}의 전위 $V_n$ 이다. 그런데 우리가 회로에서 읽는 전압은
$V_n$ 이 아니다(분극이 섞여 있다). \emph{그렇다면 $V_n$ 은 어디서 오는가?} 답: 외부에서 주어지는 입력이 아니라
...
차이를 메우려 $V_n$ 이 OCV 에서 밀려난다 — 이 밀림이 꼬리의 무대다.

\textbf{측정 전위와 구동 전위.} 우리가 \emph{측정}하는 전위는 내부 전위에 분극(저항 강하)이 더해진 값이다:
\begin{equation}
...
\begin{keybox}
\textbf{무대 요지.} $V_n$ 은 전하 보존식~\eqref{eq:charge} 의 해다(외부값 아님). 평형이면 그 해가 OCV,
전류가 흐르면 $\xi_j$ 의 지연만큼 $V_n$ 이 OCV 에서 밀린다. 측정은 $V_\app=V_n+s_I|I|R_n$, 속도 구동은
$V_\drive=V_n$. 이 무대 위에서 다음 절들이 $\xi_{\eq,j}$(목표)$\to$지연$\to$전위 배리어 낮춤을 쌓는다.
\end{keybox}

% ====================================================================
\section{평형에서의 peak — 기준선}\label{sec:eqpeak}
% ====================================================================
\textbf{질문.} 전류가 거의 흐르지 않는 \emph{평형}에서, 전이 $j$ 의 peak 는 어디에 서고 얼마나 넓고 높은가? 그리고
...
솟구쳐 \textbf{peak} 를 만든다. \emph{즉 peak 는 $\xi_j$ 가 $V_n$ 에 대해 가장 급히 변하는 곳}이다. 그러므로
...
$w=RT/F=8.314\times298/96485=25.7$ mV, $-15^\circ$C 에서 $\times258/96485=22.2$ mV). 높이 $\propto1/w_j\propto
1/T$ — 저온서 높아진다.
...
는 C-rate 에 민감하다. \textbf{따라서 꼬리의 주역은 평형 열역학이 아니라, 전류가 흐를 때 평형을 못 따라가는
동역학이다.} 다음 절이 이 지연을 다룬다.
\end{keybox}

% ====================================================================
\section{실제 peak 의 이탈: 동역학 지연이 꼬리를 만든다}\label{sec:lag}
% ====================================================================
\textbf{질문.} 전류가 흐를 때 전환율 $\xi_j$ 는 평형 목표 $\xi_{\eq,j}$ 를 \emph{즉시} 따라가지 못한다. 이
...
\textbf{저온서 $k_j$ 가 작아진다}($e^{-\Delta H_{a,j}/RT}$ 가 급감) — 이것이 곧 ``저온 긴 꼬리''의 씨앗이다.
(prefactor $k_0=k_BT/h$ 는 엄밀한 전이상태 빈도라기보다 \emph{현상론적} 상수로 쓴다 — 교환전류$\cdot$활성 면적$\cdot$
활동도 인자를 흡수한다. 꼬리 길이 \emph{절대값}은 $L_{q,j}\propto1/k_0$ 로 $k_0$ 에 의존하나, 본 장 \emph{결론}(꼬리의
온도$\cdot$전위 \emph{의존성}과 부호, Arrhenius 기울기로 뽑는 $\Delta H_a$)은 $k_0$ 를 피팅 prefactor 로 흡수하므로
그 정확한 값에 무관하다 — 단 흡수 인자들이 피팅 온도창에서 $T$-무관이라는 전제 하이며, 강한 $T$ 의존이 있으면
Arrhenius 절편$\cdot$기울기에 계통오차로 들어간다.)
...
분율(예 $\sim5\%$) 이하로 떨어지는 \emph{단일} 좌표 = 상승부에서 꼬리로 넘어가는 곳; 정확한 창은 피팅 선택, 부록)에서 적분해
\begin{equation}
...
% ====================================================================
\section{전위가 배리어를 낮춘다: 유효 배리어}\label{sec:barrier}
% ====================================================================
\textbf{질문.} 관찰의 핵심은 ``온도에 의한 배리어 \emph{외에}, 극판 전위가 배리어를 추가로 낮춘다''는 것이었다.
전위는 식~\eqref{eq:eyring} 의 $\Delta G_{a,j}$ 에 어떻게 들어가는가?

\subsection{구동력}
무대(\S\ref{sec:stage})에서 구동 전위 $V_\drive$(기본 $=V_n$)를 두었다. 전이 $j$ 의 \textbf{구동력}(affinity)은
...
\emph{수 $RT$} 이상일 때 정확하다:
\begin{equation}
\boxed{\;\Delta G_{\eff,j}=\Delta G_{a,j}-\chi\,\mathcal A_j\;,\qquad
k_j\simeq k_0\exp\!\Big[-\frac{\Delta G_{a,j}-\chi\mathcal A_j}{RT}\Big]\quad(\mathcal A_j\gtrsim RT)\;.}
\label{eq:keff}
\end{equation}
...
전극 입자가 \emph{동일하지 않다}는 것.
\end{keybox}

% ====================================================================
\section{입자 분포를 다루는 통계적 기술}\label{sec:stattools}
% ====================================================================
\textbf{질문.} \S\ref{sec:barrier} 까지 한 전이를 활성화 배리어 \emph{한 값} $\Delta G_{a,j}$ 로 다뤘으나, 실제 전극은
...
$L_q\propto e^{G/RT}$(\S\ref{sec:barrier}) — 로그를 취하면 $\ln L_q=\text{const}+G/RT$ 로 선형이 되어, $a=1/RT$ 로
\begin{equation}
...
% ====================================================================
\section{입자가 동일하지 않다: 배리어 분포와 중첩}\label{sec:dist}
% ====================================================================
\textbf{질문.} 지금까지 한 전이의 배리어 $\Delta G_{a,j}$ 를 \emph{한 값}으로 다뤘다. 그러나 실제 전극의 입자는
...
\textbf{저온 꼬리 확장(분산 전파 — \S\ref{sec:stattools}).} 식~\eqref{eq:superpose} 의 $L_V(G)$ 는 $\ln(L_V/L_{V,0})=G/RT$(기준 길이 $L_{V,0}$ 무차원화;
$\because L_V\propto L_q\propto e^{G/RT}$)이므로, 배리어 분산 $\sigma_G^2$ 가 $\mathrm{Var}[\ln(L_V/L_{V,0})]=\sigma_G^2/(RT)^2$(식~\eqref{eq:varprop} 을 $L_V\propto L_q$ 로 적용), 즉
...
% ====================================================================
\textbf{질문.} 위 다섯 \emph{물리} 절(무대 $\to$ 평형 $\to$ 지연 $\to$ 유효 배리어 $\to$ 분포; 통계 도구절
...
관측 peak 항은 식~\eqref{eq:dQdV} 의 $Q_j\,\dd\xi_j/\dd V_n$ 이다. 실제 전환율은 평형에서 지연만큼 모자라므로
$\xi_j=\xi_{\eq,j}-\bar r_j$ 다. 여기서 가중 평균 지연은 각 집단의 지연(식~\eqref{eq:rsol},
$r_j(q;G)=r_j(q_a)e^{-(q-q_a)/L_q(G)}$)을 $\rho_G$ 로 평균한 것 $\bar r_j(q)\equiv\int_0^\infty\rho_G(G)\,
r_j(q;G)\,\dd G$ 이고, 그 $q$-(또는 $V_n$-)미분이 식~\eqref{eq:superpose} 의 꼬리다. 따라서
\begin{equation}
...
각 칸은 \emph{어느 식의 어느 항}이 지배하는지를 위 도출에서 직접 읽은 것이다($s=+1$ 방전 기준).
\begin{center}
...
\emph{우세 조건} $L_{V,j}\gtrsim w_j$ 를 도출한다 — 무근거 단정이 아니다.) C-rate$\cdot$전위 방향도 가정이 아니라
위 식들에서 부호로 도출됐다.
...
총합 $\simeq Q_j$ 로 보존된다. \emph{이 인자 $r_{a,j}$ 없이 $Q_j/L_{V,j}$ 로 두면 꼬리만으로 $Q_j$ 를 다 써 보존이 깨진다.}

\textbf{식별 순서(비순환).} 모든 파라미터를 한 데이터에서 동시 자유 피팅하면 공선형이라 분리 불가다. 다음
\emph{순차} 제약으로 식별한다 — 각 단계는 ``무엇을 고정$\to$무엇이 분리되나$\to$공선형 해소 근거''로 읽는다.
...
\item \textbf{다전류/전위} ($\chi$ 를 \emph{Arrhenius 앞에서} 먼저). 고정$\to\chi$. 분리 근거: 식~\eqref{eq:LqV} 의
  ...
  (실측: 고정 $T$, S1$\cdot$S2 로 환산한 동일 SOC$\cdot$tail window 에서 $\ln L_{q,j}$ 를 $V_\drive$ 로 회귀; 상세 절차는 부록.)
...
% ====================================================================
\textbf{질문.} 관측의 한 축은 ``C-rate 가 커지면 peak 가 \emph{겹친다}''였다. 지금까지 한 전이 $j$ 를 다뤘는데,
여러 전이가 겹칠 때 관측 ICA 는 어떻게 합쳐지며, 저율 정보로 그 합을 어떻게 \emph{분해$\cdot$예측}하는가?
...
% ====================================================================
\textbf{질문.} ``꼬리는 전위가 낮춘 유효 배리어의 동역학 산물''이라는 본 장의 주장은 \emph{어떻게 틀릴 수 있는가}?
그리고 같은 꼬리를 내는 \emph{경쟁 원인}과 어떻게 구별하는가?
