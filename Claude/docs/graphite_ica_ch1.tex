...
\label{eq:weff}
\end{equation}
즉 상호작용이 클수록($\Omega_j\to2RT$) peak 가 \emph{좁아지고}, $\Omega_j=2RT$($T=T_{c,j}$)서 폭이 $0$ 으로 — 그
지점이 곧 상분리(delta 형 plateau)의 시작이다. \textbf{이것이 staging peak 가 이상 $RT/F$($25^\circ$C 서 $26$ mV)
보다 \emph{날카로운} 까닭}이며, 동시에 \S\ref{sec:master} 의 폭 파라미터 $w_j$ 에 물리적 근거와 \emph{온도 의존}을
