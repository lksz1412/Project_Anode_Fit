...
정의상 점별로 $\dd V/\dd Q=(\dd Q/\dd V)^{-1}$ 다(식~\eqref{eq:master} 의 역수). 따라서
\begin{equation}
...
\item \textbf{경계 위치의 강건성}: DVA peak(경계)는 전위가 급변하는 곳이라 \emph{위치가 날카롭고} 잡음$\cdot$기저선에
  강건하다 — 전이 경계 SOC 를 ICA peak 중심보다 정밀하게 준다(S1 의 $U_j$ 위치 정밀화).
\item \textbf{용량의 직접성}: ICA peak \emph{면적}은 곧 $Q_j$(전이 용량)다 — DVA 에선 valley 폭으로 간접적이다.
\item \textbf{교차검증}: 두 표현의 특징점 수가 같아야 하고($N_p$), \emph{DVA valley 위치 $=$ ICA peak 위치 $=U_j$}
  라 한 양($U_j$)을 두 좌표에서 독립으로 읽어 일치를 확인한다.
\end{enumerate}
...
$\dd Q/\dd V$ 로 수행하고, $\dd V/\dd Q$ 는 \emph{전이 경계 위치의 독립 교차검증}(S1 정밀화)으로 병용한다 — 같은
모델식의 두 그림.
...
\item \textbf{온도 소멸.} 그 절편 $\gamma_j\Delta U_j^\hys(T)$ 는 $T\to T_{c,j}=\Omega_j/2R$ 서
  ...
  $u_j=\sqrt{1-T/T_{c,j}}$, $\Delta U_j^\hys\!\propto\!u_j^3$). 절편이 온도에 무관하거나 다른 멱법칙이면 정규용액
