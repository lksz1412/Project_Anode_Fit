% ====================================================================
% graphite_ica_consolidated_ch1_G1.tex — 합류 Chapter 1 (v2, 원본 밀도 전면 재작성)
%   (B) 전하보존 6장 Ch1 전문 밀도 + (A) relaxation-length spectrum 엔진
%   + keystone(Level A) + 피팅식(N-2) + 심층 물리검토 5건.
%   지배 표준: 계획서 §00 GS-1~4. (B)(A) 원본 보존, 본 파일은 합류 정식본.
% Date: 2026-05-29 (v2 재작성)  Author: Project_Anode_Fit (Claude 측)
% ====================================================================
...
\hypersetup{colorlinks=true, linkcolor=blue, urlcolor=blue, citecolor=blue,
  pdftitle={Graphite ICA tail — 합류 Chapter 1 (v2, 원본 밀도)}}
\pagestyle{fancy}\fancyhf{}
\lhead{흑연 음극 ICA tail — 합류 Chapter 1 (v2)}\rhead{\thepage/\pageref{LastPage}}
...
\large 합류 Chapter 1 (방전 기준): 전하 보존식 기반 내부 전위 $\cdot$ 유효 mobility $\cdot$\\
relaxation-length spectrum kernel-integral $\cdot$ 피팅 논리식}
...
$p_j(U)$)과 혼동하지 않는다. 각 $g$ 집단의 유효 장벽 $\Delta G_{\eff,j}(g)=g-\chi_j\mathcal A_j$, 활성화
...
% ====================================================================
$\xi_j$ 와 $V_n$ 이 charge balance(식~\eqref{eq:charge_balance})로 결합되어 $\Theta$ 가 적분 안팎에 동시에
나타나는 self-consistent integral equation 이 된다. 시간 적분형은 인과 상한 $q$ 의 \emph{Volterra} 2종:
\begin{equation}
...
\textbf{단계 0 — Volterra$\to$Fredholm 근사.} 식~\eqref{eq:volterra} 는 causal(triangular) Volterra 다.
고정 구간 Fredholm 으로의 환원은 무조건 성립하지 않으며, 본 장의 controlled approximation 은 post-peak
window $[q_a,q]$ 에서 (i) moving target 정지($d\Theta_\eq/dq\simeq0$), (ii) kernel decay length
$\bar L\ll q-q_a$ 로 memory 국소. 이 근사의 붕괴는 아래 R-규칙에서 \emph{측정}한다(주장 X).

\textbf{단계 1 — ratio ansatz(사용자 PhD 방법 적용).} \cite{lee2011,son2013} 의 propagator /
ratio-substitution 을 미지 $\Theta(q')$ 의 자기참조 적분에만 적용:
$\Theta(q')/\Theta(q)\approx\Theta^\simplecase(q')/\Theta^\simplecase(q)$ ($\Theta^\simplecase$ = single-mode
baseline). \textbf{단계 2 — $\Theta(q)$ factor out}:
\begin{equation}
...
\label{eq:closed}
\end{equation}
이것이 fitting 에 쓰는 instantiated candidate closed-form 이다.
\begin{boundbox}
...
graphite 의 식~\eqref{eq:volterra} 는 \emph{Volterra}(인과, 상한 $q$). (i) 따라서 Fredholm 환원(단계 0)이
필요하며 그 붕괴는 측정한다. (ii) \textbf{사용자 논문 자신이 ``reaction zone 이 넓으면 ratio 근사
...
\end{boundbox}
\textbf{승격 조건 M1--M5.} M1 fixed-domain Fredholm 재정식 가능; M2 적분 안팎 동일 function class;
...
한다(Ch5). voltage-axis tail scale 은 $L_\varphi\simeq|dV_n/dQ|_{q_a}\,L$. \textbf{$R_n$(전압축 분극)과
