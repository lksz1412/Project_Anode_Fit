% ====================================================================
% graphite_ica_ch4_rebuilt.tex — Chapter 4 (RB 재구성본)
%   흑연 음극 ICA(dQ/dV) 이론의 발열 일반화 계층: Chapter 3 의 forward/backward
%   transition rate r_j^± 로부터 transition-level entropy production 을 무비약
%   유도하고, 그것이 거시 비가역열 일반형 Σ_j n_j^eff I_j η_j 로 정확히 환원됨을
%   보인다(미시=거시 정합). Ch2 의 Σ_j I_j η_j (n^eff=1) 은 그 ideal-width 특수해.
%     · flux–force entropy production  T Ṡ_irr = Σ_α J_α X_α ≥ 0      (AL-40)
%     · transition-level entropy production = network/master-equation  (AL-41)
%       Q̇_irr,j^tr = (Q_{j,tot}/F) RT (𝒥^+ - 𝒥^-) ln(𝒥^+/𝒥^-) ≥ 0
%     · 미시=거시:  𝒜_j^chem = n_j^eff F η_j  →  Q̇_irr,j^tr = n_j^eff I_j η_j (AL-44,45)
%     · 가역열 = OCV 온도계수 / transition entropy basis (택일, 정합 제약)
%
%   ★ Phase A CRITICAL/HIGH 해소(CHARTER):
%     · step2 (CRITICAL): η 의 기준 전위를 단일화 — η_j ≡ V_{n,drive} - V_eq,j(ξ_j),
%       기본형 V_{n,drive}=V_n. consolidated_ch4 verifybox 의 V_n/V_drive 이중정의를
%       "기본형 V_drive=V_n 전제 명시" 로 단일화(미시=거시 대입에서 전제 박음).
%     · step1 (HIGH): verifybox 에 s_{φ,j} 복원 — 𝒜_j=s_{φ,j} n_j^eff F (V_drive-U_j),
%       η_j=s_{φ,j}(...) 부호 인자 명시. q_irr=|I|η≥0 형(부호 양정치, "Iη 단독" 금지).
%     · step3 (HIGH): n^eff 일반형 Σ_j n^eff I_j η_j 명시; Ch2 특수형(n^eff=1) forward-ref 받음.
%
%   재구성 원칙(RB plan, RB_CHARTER, RB_AL_MASTER 상속):
%     · 방향-1 학부 무비약: 모든 식 = 물리 의미 prose 선행 → 중간단계 전부 명시.
%       "자명/clearly/쉽게/obviously" 0건.
%     · 방향-2 흐름 보존: consolidated_ch4 식 순서·역할 유지, 각 식 재유도(차원·부호·2법칙·극한).
...
%       ★ DOI 정확 귀속: degrootmazur1962/prigogine1967=book(DOI없음),
%         onsager1931=10.1103/PhysRev.37.405, schnakenberg1976=10.1103/RevModPhys.48.571.
%   AL 번호 = 통합 master(RB_AL_MASTER.md). Ch4 = AL-40~49.
...
\hypersetup{colorlinks=true, linkcolor=blue, urlcolor=blue, citecolor=blue,
  pdftitle={흑연 음극 ICA tail — Chapter 4 (RB 재구성본)}}
...
\title{\textbf{리튬이온전지 흑연 음극 ICA($dQ/dV$) tail 이론 — Chapter 4}\\[0.4em]
\large entropy-production 기반 발열 일반화: transition-level entropy production $\to$\\
비가역열 일반형 $\sum_j n_j^{\eff}I_j\eta_j$ (Ch2 특수형 $n^{\eff}{=}1$ 의 일반화)\\[0.2em]
\normalsize Ch3 $r_j^\pm$ $\to$ $\dot{\mathcal Q}_{\irr,j}^{\tr}{=}\tfrac{Q_{j,\tot}}{F}RT(\mathcal J_j^+{-}\mathcal J_j^-)\ln\tfrac{\mathcal J_j^+}{\mathcal J_j^-}\ge0$ $\to$ 미시$=$거시 $n_j^{\eff}I_j\eta_j$}
...
$h_\bg$ & V/K & background reversible-heat potential coefficient $=(\partial V_{\bg,\eq}/\partial T)$ (전위형 V/K, entropy J/(mol K) 아님; Ch2 동일물) \\
...
% ====================================================================
\section{$\eta_j$ 의 기준 전위 단일화 (CHARTER step2 — 이중정의 해소)}\label{sec:ch4_eta_single}
% ====================================================================
...
% ====================================================================
\section{비가역열 일반형 $\sum_j n_j^{\eff}I_j\eta_j\ge0$ (CHARTER step3 일반/특수 위계)}\label{sec:ch4_general}
% ====================================================================
...
EIS/small-signal & $R_\ct$ 단독 분리, transport time constant, film (EIS anchor $\to$ pulse 잔차 순서) \\
...
AL-40 & entropy production $T\dot S_\irr=\sum_\alpha J_\alpha X_\alpha\ge0$ (flux--force, 2법칙; 합 비음 — Onsager reciprocal 은 본 부등식 근거 아님, AL-47 transport) & GROUNDED & degrootmazur1962(book), prigogine1967(book) & \eqref{eq:ch4_fluxforce} \\
...
AL-44 & ★ 미시$=$거시: DB(AL-30)$+n_j^\eff{=}RT/(Fw_j)$(AL-34) $\Rightarrow\mathcal A_j^{\chem}{=}n_j^\eff F\eta_j$, $\dot{\mathcal Q}_{\irr,j}^{\tr}{=}n_j^\eff I_j\eta_j$ (★ $\eta_j{=}V_{n,\drive}{-}V_{\eq,j}$, 기본형 $V_{n,\drive}{=}V_n$ 전제 — CHARTER step2 이중정의 해소; $s_{\phi,j}$ 복원 step1) & GROUNDED$^\dagger$ ($\dagger$ 닫힘은 $C_j{\equiv}0$ 대칭 split $+$ 기본형 $V_{n,\drive}{=}V_n$ 조건부; 강구동 BOUNDED) & schnakenberg1976, bazant2013, degrootmazur1962(book) (DOI 10.1103/\brk RevModPhys.48.571; 10.1021/\brk ar300145c) & \eqref{eq:ch4_affinity_eta},\allowbreak\eqref{eq:ch4_micro_macro} \\
AL-45 & ★ 비가역열 일반형 $\dot{\mathcal Q}_\irr{=}\sum_j n_j^\eff I_j\eta_j\ge0$ (2법칙); Ch2 $\sum_j I_j\eta_j$ 는 $n^\eff{=}1$ ideal-width 특수해 (CHARTER step3 일반/특수 위계) & GROUNDED & degrootmazur1962(book), schnakenberg1976 (DOI 10.1103/\brk RevModPhys.48.571) & \eqref{eq:ch4_qirr_general},\allowbreak\eqref{eq:ch4_hierarchy} \\
...
AL-47 & transport/병목 발열 $=$ flux--force $\sum_\ell J_\ell X_\ell$ (선형 near-eq 서 $I^2R_\transp$); transport coupling $L_{\alpha\beta}$ 대칭 $=$ Onsager reciprocal; $R_n\ne R_{n,\eff}\ne R_\ct$ 유지($R_{n,\eff}$ 는 독립 calorimetric 제약 시만) & BOUNDED & degrootmazur1962(book), onsager1931, thomasnewman2003, newman, bardfaulkner (DOI 10.1103/\brk PhysRev.37.405; 10.1149/\brk 1.1531194; ISBN 978-0-\brk 471-47756-3; 978-0-\brk 471-04372-0) & \eqref{eq:ch4_transport_ff},\allowbreak\eqref{eq:ch4_Rsep} \\
...
% ====================================================================
\section{Chapter 5 로 전달되는 항목 (수치$\cdot$피팅은 Ch1 부록 B)}\label{sec:ch4_transmit}
% ====================================================================
\begin{enumerate}[label=\arabic*),leftmargin=2.0em]
