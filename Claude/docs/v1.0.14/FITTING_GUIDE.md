...
> `Anode_Fit_v1.0.14.py`(흑연 음극 + LCO 양극 MSMR + 가역 발열)를 실측 dQ/dV·엔트로피 계수에 피팅하는 워크플로. 파라미터 tier·round-trip 절차·초기값/경계·수렴 판정 + **역방향 식별 사슬(S0–S5)·가정 울타리·잔차 진단표**(v5 §1.15 자산 선별 복원 — D3). 이론 근거 = Ch1(dQ/dV·LCO)·Ch2(발열). ★흑연 회귀 0-diff 는 피팅 전 과정 불변(LCO 편입이 흑연 경로 무섭동).
