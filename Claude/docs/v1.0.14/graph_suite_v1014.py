...
로직 무변경 원칙 — v1.0.14 에서 어긋나는 최소 지점만 수정:
  ...
  (2) V6 라벨 현행화: 현행 LCO_MSMR_LIT 는 loop B 재정렬로 x_MIT=0.85(물리 anchor)가 코드값 —
      0.85 = "physical anchor (v1.0.14 코드 현행)", 0.50 = "pre-v1.0.14 demo (참조)" 로 라벨 교체
...
OUT = r"D:\Projects\Project_Anode_Fit\Claude\docs\v1.0.14\figs\graph_suite_v1014.png"
...
# V6 — 전자항 골 ΔS_e(x): 물리 anchor 0.85(v1.0.14 코드 현행) vs pre-v1.0.14 demo 0.50(참조)
xx = np.linspace(0.2, 1.0, 400)
ax[1,2].plot(xx, _chk("V6 0.85", m.func_dSe_molar(xx, 298.15, 13.0, 0.85, 0.05)), 'C3',
             label="x_MIT=0.85 physical anchor (v1.0.14 코드)")
...
print("=== V1.0.14 GRAPH SUITE DONE ===")
