...
$q(T)$ 는 점유된 이온의 진동 등 국소 자유도의 단일 자리 분배함수이며, 자리 우물을 3차원 조화
퍼텐셜(진동수 $\omega_0$)로 근사하면 고전 극한에서 $q=(k_BT/\hbar\omega_0)^3$, 양자 조화
진동자로는 $q=\prod_{i=1}^{3}[2\sinh(\beta\hbar\omega_i/2)]^{-1}$ 로 닫힌다. 이 전개는 국소화
...
$\varepsilon_0$ 쪽에 포함시킨 기준이다 --- 두 규약은 온도 무관 인자 $e^{-\beta\hbar\omega/2}$ 만큼만
다르므로 $T\ln q$ 의 온도 미분, 곧 엔트로피는 동일하다. 따라서 전이 중심의 온도 기울기 $\dd U_j/\dd T=\Delta S_j/(sF)$(\S\ref{sec:center})에서 진동
...
\end{equation}
($\gamma$ = 계면 에너지[J/m$^2$], $V_m$ = 삽입상 몰부피)이고, 둘째는 동역학 채널 — 고체 확산 평형화 시간
$\tau(r)=r^2/D$ 의 분산이다. 이제 실조건 수치를 대입한다. \emph{열역학 채널}: 흑연 몰부피에서
$V_m(\mathrm{LiC_6})=6M_\mathrm{C}/\rho_\mathrm{graphite}=6\times12.011/2.26\approx31.9$
cm$^3$/mol $=3.19\times10^{-5}$ m$^3$/mol 이고, 계면 에너지의 문헌 스케일 $\gamma\sim0.1$--$1$
J/m$^2$(tier C — 아래는 상한 $\gamma=1$ 을 쓰는 상한 논증이라 $\gamma$ 의 정확값에 둔감하다)를 취하면,
전형 마이크론 흑연 $r=1$--$15\,\mu$m 에서
\[
\Delta U(1\,\mu\mathrm m)\;\le\;\frac{2\times1\times3.19\times10^{-5}}{96485\times10^{-6}}
\;\approx\;0.66\ \mathrm{mV},
\qquad
\delta U_\mathrm{PSD}\;=\;\frac{2\gamma V_m}{F}\Big(\frac{1}{r_{\min}}-\frac{1}{r_{\max}}\Big)
\;\approx\;0.18\ \mathrm{mV}
\]
...
이동 절대값 기준으로도 상한 $\gamma$$\cdot$최소 $r$ 에서 $3\%$ 를 넘지 못한다. \emph{동역학 채널}:
...
모두 정량적으로 작고, 일반형~\eqref{eq:psdconv} 는 비-크기 $\eta$ 산포의
식~\eqref{eq:ensavg} 로 환원된다 — (b)-③ 의 $\rho(U_\app)$ 를 \emph{비-크기} heterogeneity 전용으로
두는 것은 선언이 아니라 이 수치 유도의 따름 결과다. 크기 채널이 유의해지는 것은 $1/r$ 이 큰 나노
영역($r\sim$수십 nm 에서 $\Delta U\sim$수십 mV)뿐이며, 그 영역(과 양자가둠 밴드 shift 류)은 본 장의
