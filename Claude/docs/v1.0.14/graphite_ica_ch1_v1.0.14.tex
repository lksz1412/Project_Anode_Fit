...
$\mathcal A$ & J/mol & 꼬리 컷점 affinity $=\min(z_\mathrm{cut}n_jRT,\,A_\mathrm{cap}RT)$ \\
...
$10^{-6}$ V 로 바닥진다(단일점$\cdot$스칼라 입력의 격자 퇴화 가드). 온도가 배열 $T(V)$ 이면 같은 격자 위로 보간해 $T_\work$ 를 얻고($T_\mathrm{rep}=\overline{T_\work}$ 는
...
곧 네 전이의 폭 출발값(0.020/0.016/0.014/0.012 V)도 같은 지위의 초기값이다 — 단상의 $w$ 는 평형이
...
\label{eq:eqpeak}
\end{equation}
이 값이 전이별로 합산되어 배경과 함께 $|I|\to0$ 극한(가역 기준선)을 이룬다 --- 단 이 기준선은 분기 없는
가역 기준선($U_j$ 중심)이고, 실제 $|I|\to0$ 극한은 $\gamma_j\ne0$ 이면 분기 중심 $U_j^{\,d}$ 에 남는다
...
가법이고 — 합성곱의 분산 가법은 분포의 모양과 무관한 항등식이다 — ① 은 대칭 폭이 아니라 한쪽으로만
자라는 지수 꼬리라 길이 척도가 따로 선다:
\begin{equation}
...
$\pi^2w_j^2/3$, 반높이 전폭은 $\mathrm{FWHM}=2\ln(3+2\sqrt2)\,w_j\approx3.53\,w_j$ 다 — $n_j{=}1$,
298 K 에서 $\approx91$ mV). 실측 종에서 조작적으로 분리되는 것도 이 두 축이다: 대칭 폭은 피팅 폭 $w_j$ 가
...
\node[font=\scriptsize,align=left] at (-4.0,0.128) {②$\otimes$③ 분산 가법\\$\sigma_\mathrm{sym}=\sqrt{1+1.25^2}\,w_j$};
...
합성곱이 분산 가법으로 종을 넓히며(예시 $\sigma_\eta=1.25w_j$ — 유효 scale
$\sqrt{1+1.25^2}\,w_j\approx1.6w_j$ 의 같은 함수족으로 표시, 파선), ① 유한율속이 한쪽 지수
...
% ====================================================================
유한 전류에서는 평형 목표 $\xi_\eq$ 를 점유가 즉시 따라가지 못하고 뒤처진다 — 그 지연이 peak 의 꼬리다. 유한 전류에서는 전이당 하나의 지연 길이 $L_{V,j}$ 를 구한다 — 이 절은 그 사슬
...
그 값을 쓰고, $I\le0$ 이거나 활성화 엔탈피 입력이 없으면 $L_V=0$ 이다.

N7 의 진행을 정리하면 — 다중도의 크기 $|n_j|$(부호 제거)에서 컷 affinity $\mathcal A$(식~\eqref{eq:Acut})로,
...
내부 전위 격자 $V_n$ 으로 역보간해 되돌린다. 스칼라 입력이면 첫 성분만 반환한다.

\subsection{staging 전이 초기값 — 피팅 출발점}
네 전이의 초기값이 표~\ref{tab:staging} 에 있다(신뢰값이 아닌 \emph{초기값} — 사용자 피팅이 override 하는 것이
...
N9$'$ & $\Delta S^\mathrm{cat}=$config$+$vib$+$elec (MSMR \eqref{eq:msmr}) & \eqref{eq:lco-decomp} \\
\bottomrule
\end{tabular}
\end{table}
...
가역 기준선은 메서드 \code{equilibrium}(식~\eqref{eq:eqpeak} 만 합산)이다. 본문 표~\ref{tab:staging} 의
...
$\chi$ & \code{chi}(생성자 인자 \code{x}) & 전달 계수(분할 규칙 주입 \code{chi\_split}) \\
