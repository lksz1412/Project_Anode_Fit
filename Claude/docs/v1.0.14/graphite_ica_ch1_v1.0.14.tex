...
298 K 에서 $\approx91$ mV. 기호 규약 — 식~\eqref{eq:widthbudget} 의 $\sigma$ 는 모두
\emph{표준편차}(분산의 제곱근)이고, logistic scale $s$ 와는 $\sigma=\pi s/\sqrt3$ 로 환산된다; 성분비로
적으면 $\sigma_\mathrm{sym}/\sigma_\mathrm{int}=\sqrt{1+(\sigma_\eta/\sigma_\mathrm{int})^2}$ 이라 같은
