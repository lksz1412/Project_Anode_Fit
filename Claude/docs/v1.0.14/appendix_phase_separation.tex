...
%   기호는 본문 Part 0 과 동일(ξ = 조성/점유율, Ω = 상호작용 파라미터).
...
구분한다(\S\ref{app:kinetics}). 기호는 본문 Part 0 과 동일하게 맞춘다: $\xi$ 는 조성(삽입 전극에서는
자리 점유율 또는 전이 진행률), $\Omega$ 는 상호작용 파라미터[J/mol]이다.
...
\textbf{(d) 박스 --- 정규 용액 자유에너지.} 두 결과를 $g=u-Ts$ 로 묶으면 몰당 혼합 자유에너지는
\begin{equation}
...
식~\eqref{eq:app-fxi} 는 본문 Part 0 의 자유에너지와 동일하며, $f(\xi)=f(1-\xi)$ 의 우함수 대칭을
갖는다. 극한 검산으로, $\xi\to0^+$ 에서 $f\to0$ 이되 기울기 $f'\to-\infty$ 로 발산하므로 순수 상은
...
\end{equation}
이 binodal 을 결정한다(둘째 식은 첫째 식을 $T/T_c=2RT/\Omega$ 로 푼 것). 주의할 점으로,
...
\draw[thick] plot[smooth] coordinates {(0.01,0.4266) (0.02,0.4933) (0.05,0.6113) (0.0707,0.6667) (0.10,0.7282) (0.15,0.8071) (0.20,0.8656) (0.25,0.9102) (0.30,0.9442) (0.35,0.9693) (0.40,0.9864) (0.45,0.9966) (0.50,1.0) (0.55,0.9966) (0.60,0.9864) (0.65,0.9693) (0.70,0.9442) (0.75,0.9102) (0.80,0.8656) (0.85,0.8071) (0.90,0.7282) (0.9293,0.6667) (0.95,0.6113) (0.98,0.4933) (0.99,0.4266)};
...
되돌아간다. 분해가 시작되려면 새 상의 작은 영역(핵)이 유한 크기로 생겨야 하는데, 핵의 부피는
자유에너지를 낮추지만($\Delta g_v<0$, 단위 부피당 구동력) 모상과의 계면은 계면 에너지
...
\label{eq:app-ch-F}
\end{equation}
로 확장된다(Cahn--Hilliard). 확산 흐름은 화학퍼텐셜 $\mu=\delta F/\delta\xi=f'(\xi)-2\kappa\nabla^2\xi$
...
plateau 가 단일 평형 전위로 나타나는 근거이자, 실측 곡선이 그 전위에 델타로 모이지 않는 이유(준안정
가지 과주행)를 진술할 기준선이다. 넷째, 핵생성 대 spinodal 분해의 구분(\S\ref{app:kinetics})은 본문
