...
## 3. 미완료·사용자 검토 결과 (2026-07-04 검토 완료)
- **①spinodal 부록** — 사용자 결정: **별도 문건으로 유지**(편입 안 함 — 본문 연결 문장 삽입 불요).
- **②경연 편입 그림** — 사용자 "거의 대부분 마음에 든다". 지적 1건 = ch1 그림 15(fig:reversal) — 진단: 두 패널 세로 간격 부족(2.9 cm)으로 (b) 축·라벨이 (a) 눈금 띠와 겹침 → 3.9 cm 로 정정 완료(빌드 green). ch2 그림 2장 과소 관찰 → v1.0.15 계획서 P5.1(선택)로 제안.
- 코드 업데이트: 필요 판정 — 계획서 `../plans/2026-07-04-v1015-code-update-plan.md`(rev.1) 작성, **GO 대기**.
