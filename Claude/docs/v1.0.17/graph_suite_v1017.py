# -*- coding: utf-8 -*-
"""v1.0.17 그래프 suite — 검증·복원 (V1-V9 핵심 패널). 이론 근거 Ch1·Ch2 (v1.0.17).
...
로직 무변경 원칙 — v1.0.14 에서 어긋나는 최소 지점만 수정:
  (1) CODE/OUT 경로: v1.0.10 → v1.0.17 (import 대상 = Anode_Fit_v1.0.17.py, 출력 = v1.0.17\\figs).
  (2) V6 라벨 현행화: 현행 LCO_MSMR_LIT 는 loop B 재정렬로 x_MIT=0.85(물리 anchor)가 코드값 —
      0.85 = "physical anchor (v1.0.17 코드 현행)", 0.50 = "pre-v1.0.14 demo (참조)" 로 라벨 교체
...
OUT = r"D:\Projects\Project_Anode_Fit\Claude\docs\v1.0.17\figs\graph_suite_v1017.png"
...
# V6 — 전자항 골 ΔS_e(x): 물리 anchor 0.85(v1.0.17 코드 현행) vs pre-v1.0.14 demo 0.50(참조)
xx = np.linspace(0.2, 1.0, 400)
ax[1,2].plot(xx, _chk("V6 0.85", m.func_dSe_molar(xx, 298.15, 13.0, 0.85, 0.05)), 'C3',
             label="x_MIT=0.85 physical anchor (v1.0.17 코드)")
...
print("=== V1.0.17 GRAPH SUITE DONE ===")
