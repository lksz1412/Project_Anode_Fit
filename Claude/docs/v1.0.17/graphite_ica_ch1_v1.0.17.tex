...
%     코드 Anode_Fit 1.0.16 와 동일 버전·matched.
% --------------------------------------------------------------------
...
\hypersetup{colorlinks=true, linkcolor=blue, urlcolor=blue, citecolor=blue,
  pdftitle={흑연 음극 + LCO 양극 dQ/dV 이론: 계산 진행을 따라가는 수식-구동 (유도 확장) — Chapter 1 (v1.0.17)}}
\pagestyle{fancy}\fancyhf{}
\lhead{흑연 음극 + LCO 양극 $dQ/dV$ 이론 — Ch.1 (v1.0.17, 통계역학-first + 계산 진행)}\rhead{\thepage/\pageref{LastPage}}
...
동일). 여기서 평가 전위는 \S\ref{sec:pol} 의 내부 전위 $V_n$ 이다. 중심 $U_j^{\,d}$
...
의 인과 기억는 시작점 $q_0$ 를 어디에 두어도 성립하는 항등식이며, $q_0$ 를 밀어낼 자유가 다음
단계를 연다.
...
\label{eq:reversal}
\end{equation}
인과 기억가 진행 방향의 과거를 훑는다는 것이 이 두 식의 공통 내용이다 --- 방향을 어기면 비인과가
된다.
...
& 식~\eqref{eq:LV}$\cdot$ \eqref{eq:tail-limit}$\cdot$\eqref{eq:eqpeak} \\
...
& 식~\eqref{eq:tail-limit}$\cdot$ \eqref{eq:peakshape}$\cdot$ \eqref{eq:eqpeak} \\
...
% ====================================================================
문건과 구현의 연계는 단방향이다 --- 구현(\code{Anode\_Fit\_v1.0.16.py})의 주석$\cdot$docstring 이 본 문건의
