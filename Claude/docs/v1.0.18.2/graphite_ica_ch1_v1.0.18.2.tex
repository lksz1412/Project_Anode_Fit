% ====================================================================
% ★ release 1.0.16 (2026-07-06) — graphite_ica_ch1_v1.0.16.tex (계보 1.0.15 승계: 폭 n(T) 피팅 방향 반영)
...
%     코드 Anode_Fit 1.0.18.2 와 동일 버전·matched (계보 1.0.16→1.0.17→1.0.18.1→1.0.18.2; 헤더 아래 블록은 1.0.16 이력).
% --------------------------------------------------------------------
...
%   보강(v8-10 계열) = 부록 A R5 꼬리 극한 검산(v1.0.15 에서 점별 아키텍처로 재유도).
...
%   Anode_Fit_v1.0.16.py(계보 v11_final→use_w_eff 제거→1.0.10→1.0.12→1.0.13→1.0.14→1.0.15→1.0.16). 무관 일반론(lever/chord·Marcus·KWW·적층·
...
\hypersetup{colorlinks=true, linkcolor=blue, urlcolor=blue, citecolor=blue,
  pdftitle={흑연 음극 + LCO 양극 dQ/dV 이론: 계산 진행을 따라가는 수식-구동 (유도 확장) — Chapter 1 (v1.0.18.2)}}
\pagestyle{fancy}\fancyhf{}
\lhead{흑연 음극 + LCO 양극 $dQ/dV$ 이론 — Ch.1 (v1.0.18.2, 통계역학-first + 계산 진행)}\rhead{\thepage/\pageref{LastPage}}
...
\title{\textbf{\LARGE Chapter 1}\\[0.35em]\Large 흑연 음극 + LCO 양극 $dQ/dV$ 이론: 한 곡선을 그리는 계산 진행을 따라가는 수식-구동판 — 유도 확장}
\author{}
\date{\normalsize 버전 1.0.18.2}
...
$T_\rep$ & K & 비등온 시 전이당 대표(평균) 온도 $=\overline T$ — N1$\cdot$N3$\cdot$N7 대표조건 평가 \\
...
전이마다 다르게 자연히 정해진다. 온도가 배열 $T(V)$ 이면 각 $V_n$ 점의 값을 그대로 취해 전이당 상수
평가용 대표 온도 $T_\rep=\overline{T}$ 를 얻고, 스칼라면 $T$ 를 공통으로 쓴다. 배경도 같은 점에서
...
이 식이 쓰이는 조건은 지연 길이가 peak 폭에 비해 작을 때다 --- 다음 절(\S\ref{sec:lag}; 그 전에 아래 소절이
두-상 폭의 지위를 먼저 닫는다)이 그 지연 길이 $L_V$ 를 세우고, $L_V$ 가 폭 $w_j$
...
0.014 V($2\mathrm L\!\to\!2$)$\cdot$0.012 V($2\!\to\!1$)와 모순되지 않는다. 단 \emph{현재 모델의 staging
...
\subsection{인과 기억 적분과 지연 진행률 $\xi_\mathrm{lag}$}
...
\subsection{충전에서의 방향 반전 --- 진행 방향의 과거}
...
\begin{figure}[t]
\centering
% T7 fig:reversal  (opus2 — quantitative-density; GENUINE numeric, not schematic)
% peak shape = (xi_eq - xi_lag)/L_V, xi_lag from causal memory integral eq:lag / eq:reversal
% (T7_calc.py): w=1, U=0, L_V=1.5w.
%   discharge sigma_d=+1: peak-shape max 0.1955 at V=+1.01 w (tail -> higher V).
%   charge   sigma_d=-1: eq:reversal lower branch = exact mirror,
%                        max 0.1955 at V=-1.01 w (tail -> lower V).
%   equilibrium species (dotted) = xi_eq(1-xi_eq)/w, peak 0.25 at 0, direction-invariant.
% [pointwise recomputed from memory integral eq:lag/eq:reversal (Anode_Fit_v1.0.16).]
...
$L_{V,j}$ 는 $10^{-10}$--$10^{-8}$ V 규모로 폭 $w_j$(수십 mV)에 비해 무시할 만큼 작아 식~\eqref{eq:tail-limit} 의 $L_V\to0$ 극한이 사실상 이미 실현되므로, $\gamma$ 미배정($=0$)$\cdot$$R_n=0$ 과 함께 \emph{초기 상태의 곡선은 꼬리$\cdot$분기 없는 평형
...
별개라 round-trip 피팅으로 정합한다. 또한 현행 모델은 $\Delta S_e$ 를 기준온도 $T_\mathrm{ref}$ 에서
...
$T^2$ 곡률식($\tfrac12(T^2-T_\mathrm{ref}^2)$ 곡률)이다. 이 ``$T^2$ 곡률 $=$ 전자 신호'' 식별은 config 부분몰 엔트로피가
...
\S\ref{sec:lco-gate}), 창 밖에서 $\approx\!0$ 이다. 창 중심에서 이 $-46$ 이 config$+$vib 기저에 더해져도 양의 총합을
뒤집지 못해 \emph{총 부호는 불변}($\approx\!+80-46\approx\!+34>0$)이고, 창 밖에선 보정이 소멸해 $\approx\!+80$ 만 남는다. 표~\ref{tab:lco-staging}
...
$S_e\to0$).
\textbf{★Sommerfeld 동결의 유효 경계.} 위 동결 $g\approx g(E_F)$ 의 정확형은 상태밀도의 에너지 의존을 담은 보정항을
포함한다 --- 고정 $E_F$(grand-canonical)에서는 엔트로피 핵 $\hat s(\zeta)$ 가 짝함수라 $g'(E_F)$ 항이 $\int_{-\infty}^{\infty}\!\zeta\,\hat s(\zeta)\,\dd\zeta=0$(홀$\times$짝)으로
상쇄되고, 첫 보정은 $\tfrac12 k_B\,g''(E_F)(k_BT)^3\!\int\zeta^2 \hat s(\zeta)\,\dd\zeta$ 곧 $\mathcal O(T^3)$ 이다(고정 조성 $x$ 에서는
$\mu(T)$ 이동이 같은 차수의 $g'^2/g$ 항을 더하나 역시 $\mathcal O(T^3)$; $g'(E_F)(k_BT)^2$ 형 $\mathcal O(T^2)$ Mott 항은
thermopower$\cdot$전도도 소관이지 엔트로피의 선도 보정이 아니다). 상대 크기는 여전히 $\mathcal O[(k_BT/E_F)^2]$ 급이라,
...
흑연에서 합산식~\eqref{eq:sum} 의 $\Delta S_{\rxn,j}$ 는 평형 중심 $U_j(T)$ 를 통해 한 덩이로 작용했으나, LCO 양극에서는 이 한 덩이가 세 물리 성분(config=배치$\cdot$vib=격자진동$\cdot$electronic=전자)의 합임을 명시한다 — 식$\cdot$모델 경로는 그대로이고 $\Delta S$ 의 \emph{내부 분해}만 더하며, 그 근거부터 식으로 놓는다. \textbf{(a) 출발 — 분배함수의 인수분해.} 전이 $j$ 창의 한 미시상태는 (리튬 자리 배열)$\times$
...
\Delta S_j^\mathrm{vib}\ &\leftarrow\ \text{창 내 완만 몫의 중심 흡수치}\ (\theta_{E,j}\text{ 부여 시 }T\text{-의존 Einstein 보정 --- Ch2 vibrational 절}),\\
...
$x\!\leftrightarrow\!\xi_{\eq,1}(V)$ 매핑으로 좌표를 잇는다(닫힌식은 \S\ref{sec:lco-code} 식~\eqref{eq:lco-xmap}). \emph{(ii) round-trip 가드.} 식~\eqref{eq:lco-decomp} 의 config 중심 표준값(갭 G1 --- 연속 $\Delta S(x)$ 부재)
...
% ====================================================================
문건과 구현의 연계는 단방향이다 --- 구현(\code{Anode\_Fit\_v1.0.18.2.py})의 주석$\cdot$docstring 이 본 문건의
...
$\xi_{\mathrm{lag},j}$ & \code{occ\_lagged} & 인과 기억 적분 진행률(식~\eqref{eq:lag}) \\
\bottomrule
\end{longtable}
...
\code{theta\_E},\code{theta\_E\_Tref} & 전이 dict & --- $/\,298.15$ & Einstein 온도 $\theta_{E,j}$[K]$\cdot$기준온도 (Ch2 vibrational 절) \\
\bottomrule
\end{longtable}
\endgroup
...
N8 & 기억 적분 $\xi_\mathrm{lag}$(\eqref{eq:lag}) 의 점별 수치 적분(\code{occ\_lagged}); 충전 분기는 상한 반전(\eqref{eq:reversal}) \\
