# -*- coding: utf-8 -*-
"""Anode_Fit v1.0.18.2 -- sample test figure (P4.4; English/ASCII labels only, glyph-safe).

Uses only v1.0.18.2 functions/classes (no re-implementation):
...
Output: sample_test_v1018_2.png (same folder, dpi=150). Deterministic (no RNG).
"""
...
CODE = r"D:\Projects\Project_Anode_Fit\Claude\docs\v1.0.18.2\Anode_Fit_v1.0.18.2.py"
OUT = r"D:\Projects\Project_Anode_Fit\Claude\docs\v1.0.18.2\sample_test_v1018_2.png"
spec = importlib.util.spec_from_file_location("anodefit", CODE)
m = importlib.util.module_from_spec(spec); spec.loader.exec_module(m)

# ---- instantiate v1.0.18.2 classes (representative parameters) ----
...
fig.suptitle("Anode_Fit v1.0.18.2 - sample test (graphite anode + LCO cathode + reversible heat)",
...
ax[1, 0].set_title("(c) q_rev = -I*T*(dU/dT), I = +1 A (cell discharge; graphite: lithiation)")
...
x_mit_demo = float(m.LCO_MSMR_LIT[0]["x_MIT"])  # v1.0.14 loop B: T1 realigned to physical anchor 0.85
...
ax[1, 1].plot(xx, m.func_dSe_molar(xx, 298.15, 13.0, 0.50, 0.05), "C1", lw=1.4, ls="--",
              label="x_MIT = 0.50 (pre-v1.0.14 tier-C demo, reference)")
...
print("=== SAMPLE TEST v1.0.18.2 DONE ===")
