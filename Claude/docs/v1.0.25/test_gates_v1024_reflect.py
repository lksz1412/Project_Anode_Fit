# -*- coding: utf-8 -*-
"""v1.0.24 R2 반영 게이트 — 신규 기능(@5 5-feature·LCO 토글·#1 단위) 검증.
v1.0.25: G-R3 을 "@3 regsol 커널 삭제 확인"(DG-1)으로 대체 — 삭제가 하위호환을 깨지 않음을 증빙.
...
# ===== G-R3: @3 regsol 커널 삭제 확인 (v1.0.25 DG-1) =====
#   (a) 커널 함수·격자 심볼이 모듈에 없다  (b) 'kernel':'regsol' 키가 남은 legacy 전이 dict 도
#   로지스틱 결과와 array_equal — 즉 키는 무시되며 삭제가 하위호환을 깨지 않는다.
T=298.15; RT=R*T
gone=[nm for nm in ('_regsol_dqdv','_regsol_binodal_xa','_REGSOL_XG') if not hasattr(m,nm)]
absent=(len(gone)==3)
tr_key=dict(U=0.30, w=0.02, Q=1.0, n=1.0, Omega=1.0*RT, kernel='regsol', delta=0.002,
            dH_a=40000.0, dS_a=0.0, dVdq_qa=0.30)
tr_log=dict(U=0.30, w=0.02, Q=1.0, n=1.0, dH_a=40000.0, dS_a=0.0, dVdq_qa=0.30)
gr_key=m.GraphiteAnodeDischargeDQDV([tr_key], x=0.5, Rn=0.01, Cbg=0.0)
gr_log=m.GraphiteAnodeDischargeDQDV([tr_log], x=0.5, Rn=0.01, Cbg=0.0)
Vr=np.linspace(0.05,0.55,3000)
dk=np.asarray(gr_key.equilibrium(Vr,T=T)); dl=np.asarray(gr_log.equilibrium(Vr,T=T))
ignored=bool(np.array_equal(dk,dl)); area=float(_trapz(dl,Vr))
g_r3 = absent and ignored and abs(area-1.0)<1e-3
print(f"G-R3 @3 regsol 삭제: 심볼부재={absent}(3/3) | 'kernel' 키 무시(legacy==로지스틱 array_equal)={ignored} | 로지스틱 면적={area:.6f}(=Q=1) → {'PASS' if g_r3 else 'FAIL'}")
PASS = PASS and g_r3
