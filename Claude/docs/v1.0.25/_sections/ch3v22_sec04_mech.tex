% ====================================================================
% ch3v22_sec04_mech.tex — §3.4 기계 히스테리시스 Larché–Cahn+GS-1 (R5 채택 = W1 기반)
% ====================================================================
\section{기계 히스테리시스 --- Larché--Cahn 유도 시도와 미결 공백 GS-1}\label{sec:si-mech}
대응 지도(§3.1)에서 골격 밖 ``새 물리'' 로 분류된 유일한 노드는 N3(히스테리시스)였다. 크기와 기원이
...
발열 3성분 중 최소, §3.2 \cite{arnot_calorimetry2021}). 골격에 없는 것은 \emph{응력이 화학퍼텐셜에 들어가는
...
(\S\ref{ssec:si-lc-derive}), 닫히지 않는 지점을 정확히 짚어 GS-1 을 미결 공백으로 선언한다
...
Li 부분몰 부피를 $\bar v_\mathrm{Li}\equiv\partial v/\partial(\text{Li 함량})$ 라 두면, 선형
결합의 1차 항은
\begin{equation}
...
\subsection{닫히지 않는 데 --- 미결 공백 GS-1}\label{ssec:si-lc-gap}
...
리튬화 경로와 탈리튬화 경로에서 서로 다른 값을 취한다. 곧
\begin{equation}
\Delta V^\mathrm{mech}=\Lambda_\sigma\bigl(\sigma_\mathrm{h}^{\dis}-\sigma_\mathrm{h}^{\chg}\bigr)\ne0
...
접속이 닫히는 범위까지만 --- 못 닫으면 미결 공백 유지). 유사 유도로 소성 구성식을 지어내지 않는다.
\end{keybox}

\begin{warnbox}
기계 히스테리시스의 가역 화학역학 결합은 식~\eqref{eq:si-coupling}까지 닫힌다. 남은 공백(GS-1)은
``응력항의 존재''가 아니라 ``응력의 경로 의존 구성식'' --- 곧 \emph{범위 밖 소산 역학}이다.
Si 전용 완결 유도(소성 구성식 접속)는 Si 실측 응력--전위 곡선 확보 후 후속 장 소관이다.
\end{warnbox}

% 자산: [V22-R5-16 Larché–Cahn 응력-화학퍼텐셜 유도 시도 sec:si-mech]
%   [V22-R5-17 응력항 실은 화학퍼텐셜 eq:si-lcmu(신규 기호 σ_h·v̄_Li 도입)]
%   [V22-R5-18 응력에 의한 전위 이동 eq:si-vshift·결합 계수 eq:si-coupling(Λ_σ=v̄_Li/F, ~100 mV/GPa 정성 정합)]
%   [V22-R5-19 경로 의존 소성 → GS-1 공백 eq:si-plastic(물리 가정 충돌+유도 미완결 4분류·부록 대비 공백 위치 1단계 좁힘)]
