...
대응 지도(\S\ref{sec:si-map})가 ``Si 는 골격을 버리지 않고 성분을 교체한다'' 로 닫혔으니, 남은 일은
그 성분 --- 곡선 개형$\cdot$평균 전위$\cdot$1차 비가역$\cdot$히스테리시스$\cdot$용량 --- 을 활물질 계열별로
읽는 것이다. 실리콘계는 상용 관행상 셋으로 나뉜다: 원소 Si, 산화규소 SiO$_x$, 탄소 복합 Si--C. 본 절은 각
계열의 \emph{실측 초기값}을 정리하되, 모든 수치는 검증 문헌의 원문 확인분만 싣고(tier 명기), 계열 간 값이
어긋나는 곳은 채택 근거를 표 각주로 드러낸다. 표~\ref{tab:si-cases} 가 그 검토용 파라미터
셋이며, 이하 세 소절이 개형과 1차 vs 순환 차이를 문헌별로 잇는다.

\begin{table}[t]
\centering\footnotesize
\renewcommand{\arraystretch}{1.3}
\caption{케이스별 검토 초기값(3계열) --- 출발점, 피팅으로 override. 모든 수치는 원문 확인분(tier A $=$
1차 문헌 정량 확정$\cdot$B $=$ 리뷰/2차 경유 또는 초록 elided). 첨자 채택 근거:
...
대응 지도 N5(성분$=$곡선 기술)의 실측 근거다. \textbf{용량$\cdot$비가역.} 1차 가역 $\sim$1000 mAh/g,
...
\textbf{프로토콜 의존 feature 와 고해상 gallery 옵션(v1.0.25).} 위 결정화 특징은 \emph{측정 프로토콜에 따라
보이거나 보이지 않는다}. 공개 반쪽셀 pOCV 재검증에서 완화 구간을 포함한 프로토콜(p-ocvhold)의 탈리튬 dQ/dV 는
$0.43$--$0.46$ V 에 두 개의 뚜렷한 feature($\approx0.433\cdot0.456$ V --- $c$-Li$_{15}$Si$_4$ 탈리튬화 쌍에
대응)를 보이는데, 완화 없는 프로토콜(p-ocv)의 같은 물질에서는 이 구간이 \emph{매끈}해 feature 가 잡히지 않는다.
곧 개형은 사이클 이력에만 의존하지 않고 \emph{준평형 도달 정도}에도 의존한다 --- 데이터의 프로토콜을 명시하지
않은 채 feature 유무를 물질 성질로 읽어서는 안 된다. 이 최고 해상도 조건을 담기 위해 Si 쪽 \emph{7-gallery}
opt-in 시드(위 두 전위를 anchor 로 포함, 순수 로지스틱)를 병존 옵션으로 둔다. \emph{gallery $\ne$ 상}
이라는 규율은 여기서도 같다 --- 전이를 세분해도 상 수는 늘지 않으며(Chapter 1 의 해상도 사다리와 동일 지위),
세분된 중심들은 새 상이 아니라 한 물리 봉우리의 이중 표현이다. 기본 케이스 셋$\cdot$표~\ref{tab:si-cases} 는
무변경이다(additive opt-in).
...
$60{:}15{:}10{:}15$ 질량비)은 밀링 정도에 따라 개형이 달라진다: 저-밀링 시료는 $\sim$0.1 V 뚜렷한 평탄역, 고-밀링
...
피크가 전위축에서 분리되어 관측된다는 이 사실이 \S\ref{sec:si-blend} 공통-μ 합성의 실측 접점이다.
...
peak$\cdot$변동이 없음)\cite{bohm_entropy2024}. 이는 대응 지도 N2$\cdot$N6(peak 문법 재해석)의 열역학
...
전제)이며 \S\ref{ssec:si-elemental}--\ref{ssec:si-sic} 서술과 대응한다. \textbf{명시 표식} --- (i) SiO$_x$ 의
