...
관측 평형 전위 $U_\oc(x,T)$ 는 모든 전이의 점유가 합쳐 전하 보존을 만족하는 \emph{음함수}(implicit function)로 정의된다. 좌표 주의
...
\label{eq:implicit}
\end{equation}
여기서 $Q_j$ 는 전이 $j$ 의 용량, $Q=\sum_j Q_j$ 다. 이 음함수는 피팅 관례가 아니라 다클래스 lattice gas 의
대정준 평균 입자수 $\langle N\rangle=\sum_jM_j\theta_j$ 를 고정 함량에서 $U_\oc$ 에 대해 뒤집은 제약
반전이며, 그 유도와 \emph{유일한 근}(unique root)의 fluctuation 증명($\partial\langle N\rangle/\partial\mu=\beta\,\mathrm{var}(N)>0$)은
Part 0 의 다클래스 소절(\S\ref{sec:sm-mc} 식~\eqref{eq:sm-mc-balance})이 닫는다. 양변을 고정 $\bar x$ 에서 $T$ 로 음함수 미분하면
(고정 $\bar x$ 는 고정 $x$ 와 같아 좌표 선택에 무관), 우변이 상수라 좌변의 $T$-전미분이 $0$ 이 되어야
한다. 각 $\xi_j\bigl(U_\oc(T),T\bigr)$ 에 연쇄율을 적용해 그 전미분을 풀어 적으면
\[
\sum_j Q_j\Big[\frac{\partial\xi_j}{\partial U}\Big|_T\,\frac{\partial U_\oc}{\partial T}\Big|_x
+\frac{\partial\xi_j}{\partial T}\Big|_U\Big]=0
\]
...
$dQ/dV$ 봉우리(Part I 의 peak) 모양}이다:
\begin{equation}
\frac{\partial\xi_j}{\partial U}\Big|_T \;=\; \frac{\xi_j(1-\xi_j)}{w_j} \;\equiv\; g_j(x).
\label{eq:gj}
\end{equation}
기호 주의 --- 이 $g_j(x)$ 는 봉우리 가중[1/V]으로, Part 0 의 조성 자유에너지 $g_j(\xi)$(\S\ref{sec:sm-mf})$\cdot$본 파트의
BW 자유에너지 $g(\theta)$(식~\eqref{eq:BW})$\cdot$상태밀도 $g(E_F)$(식~\eqref{eq:Se-ch2})와 무관한 별개
기호다(g 4종 충돌). 온도 미분은 \emph{두 조각}을 갖는다 --- 봉우리 중심 $U_j(T)$ 의 온도 이동과,
...
데이터가 정한다(본 파트는 스코프를 정하지 않는다).

\subsection{파생 A(이어서) --- 단순식과 완전식: 계단이 아닌 연속 블렌드}\label{ssec:blend}
먼저 첫 조각만(중심 이동)을 \eqref{eq:gj}$\cdot$\eqref{eq:dxidT} 와 함께 \eqref{eq:implicit_diff} 에 넣는다.
분모의 $\partial\xi_j/\partial U=g_j$ 와 분자의 $-g_j\partial U_j/\partial T$ 가 만나 $g_j$ 가 남고, 겹침 가중의
...
\begin{srcbox}
\textbf{수치 검증(파생 A).} Part I 의 4-전이 흑연 staging 파라미터로, 유한차분(finite difference)
...
``단순식의 잔차''이기 때문이다. 단일 전이가 지배하는 영역에서 식~\eqref{eq:dVdT_config} 가 그대로 성립한다:
\begin{equation}
...
기울기를 조정해 폭을 바꾸지만, 그 값 역시 평형 등온선이 결정한다).
\item \textbf{두-상($\Omega>2RT$, 상분리; 흑연 staging 의 두-상 전이 $2\mathrm L\!\to\!2\cdot2\!\to\!1$ 가 여기
속함)}:\footnote{어느 전이를 두-상으로 특정하는지는 $\Omega>2RT$ 문턱 자체가 가르지 않는다 --- Part I 의
초기값 $\Omega$ 는 네 전이가 모두 임계 $2RT\approx4957$ J/mol 를 넘어서므로, $2\mathrm L\!\to\!2\cdot2\!\to\!1$
을 두-상으로 지목하는 근거는 문턱 부등식이 아니라 실측 plateau$\cdot$staging 문헌의 상평형이다
\cite{dahn1991,ohzuku1993}(Part I 의 staging 초기값$\cdot$post-fit 기준). 문턱은 ``상분리가 가능한가''를, 문헌 상평형은 ``어느 전이가 실제로 두-상
plateau 인가''를 답한다.} 평형 \emph{단일 입자}의 등온선은 plateau \emph{안에서} 거의 \emph{델타}(한 전위에 집중)다 --- 평형은
...
출처의 분해$\cdot$앙상블 forward 통계평균$\cdot$역산 금지$\cdot$입자 크기 효과 배제)는 \S\ref{sec:broadening} 의
broadening 절에서 전개하며, 본 파트는 그 결과만 받아 $w_j$ 의 \emph{지위}를 규정한다.}
\end{itemize}
따라서 본 파트 종합식(\S\ref{sec:synthesis})에 들어가는 $w_j$ 는, 흑연 두-상 전이에 대해 평형 예측
...
plateau 가 실측에서 \emph{유한 종}이 되는 이유(=폭의 기원)는 위 broadening 이 다룬다 --- 둘은 다른 질문이다.
\begin{warnbox}
...
\emph{실측 방향과 반대}이며 본 장은 \emph{두-상 봉우리의 폭}에 그런 유효-폭 축소식
($w_\eff(\Omega)=w(1-\Omega/2RT)$ 류)을 쓰지 않는다(\emph{두-상 폭 한정} 재도입 금지). $\Omega$ 의 역할은
plateau(상분리)를 \emph{만드는} 임계 조건이며(\S\ref{ssec:BW}), 그 plateau 위의 \emph{유한 폭}은 평형이 아니라
현상학적 피팅이 정한다. (상호작용 엔트로피 $\propto\partial\Omega/\partial T$ 같은 2차 항이 있을 수 있으나,
흑연에서 소수이고 본 장 범위 밖이다.)

\emph{조준 좁힘(v1.0.25)} --- 이 금지는 \textbf{두-상 폭}에 대한 것이며, \emph{단상}($\Omega<2RT$) 영역에서
같은 조합 $w_\eff=(RT/F)(1-\Omega/2RT)$ 가 등온선 \emph{중심 높이} $Q/(4w_\eff)$ 의 국소 지표로 쓰이는 것은
별개다 --- 그 높이 항등은 단상에서 \emph{정확}하다(Part I \S\ref{ssec:gr-2L}, Chapter 3 의 단상 해석도 같은
용법). 다만 그 경우에도 $w_\eff$ 를 \emph{폭}으로 읽는 것은 여전히 금지다: 단상 반높이 폭은
$\lambda\equiv1-\Omega/2RT$ 에 비례하지 않고 $\lambda^{3/2}$ 로 닫히므로($\lambda\to0$ 에서 $\lambda$ 읽기는
$\approx0.66/\sqrt\lambda$ 배 과대 --- Part I 식~\eqref{eq:gr2l-fwhm}), 높이 지표와 폭은 서로 다른 스케일을
탄다. 곧 금지 대상은 ``$w_\eff$ 를 \emph{폭}으로 쓰는 것''과 ``그것으로 두-상 델타까지 연속 보간하는 것''
두 가지이고, 단상 \emph{높이} 용법은 허용이다.
\end{warnbox}
...
% 자산: [C-2] ★v5 정정 구 w_eff(Ω) narrowing 식 완전 제거(재도입 금지 — v1.0.25: 두-상 '폭' 한정으로 조준 좁힘, 단상 '중심 높이' 지표 용법은 허용·폭 읽기는 여전히 금지) · [C-3] 대체=두-상 w 현상학적 자유피팅·기원은 Ch1 broadening ·
