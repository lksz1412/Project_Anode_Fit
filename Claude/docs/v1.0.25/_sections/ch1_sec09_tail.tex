...
% ====================================================================
\section{인과 기억 꼬리와 충전에서의 방향 반전}\label{sec:tail}
...
평균한 값이다 --- 임의의 평가점 $V$ 마다 그 점 하나의 적분으로 닫힌다.\footnote{\emph{구현
각주(v1.0.25).} ``점별''은 \emph{출력}이 점별이라는 뜻이고, 식~\eqref{eq:lag} 의 하한은 $-\infty$ 인
\emph{이력} 적분이다 --- 그러므로 평가 격자를 그대로 적분 구간으로 쓰면 격자 \emph{시작점} 이전의 과거가
누락된다. 구현은 진행방향 과거로 $5L_{V,j}$ 만큼 격자를 확장(간격 $\le L_{V,j}/20$ 의 coarse pad, 상한
4000점)해 적분한 뒤 원 격자만 절단해 반환한다. 이로써 평가창의 시작점이 봉우리 \emph{안}에 있어도 결과가
창에 의존하지 않는다 --- 같은 물리점을 세 개의 다른 $V_\app$ 시작점에서 평가한 상대 산포가 종전 구현에서
$74.3\%$ 였고 pad 후 $0.02\%$ 다(신규 게이트 G-창의 대조 측정; pad 길이는 커널 잔여 $e^{-5}\approx0.7\%$ 에
해당해 이 잔차의 하한을 준다). 평형 종 경로와 기본 데이터셋($L_{V,j}\sim10^{-8}$ V 로 pad 가 격자 간격보다
작아 항등)은 무변경이다.}
...
\node[right,font=\scriptsize] at (1.2,0.205) {$({+}1.01w,\,0.196)$ tail};
...
\fill (0,0.25) circle (1.4pt); \fill (-1.0,0.1955) circle (1.4pt);
\node[above,font=\scriptsize] at (0,0.253) {$(0,\,0.25)$ eq};
\node[left,font=\scriptsize] at (-1.2,0.205) {tail $({-}1.01w,\,0.196)$};
