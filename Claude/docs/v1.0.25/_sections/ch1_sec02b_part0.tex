...
\emph{정규용액}이고, 본론 표~\ref{tab:staging} 의 $\Omega_j$ 와 Chapter 2 의
...
\label{eq:mu}
\end{equation}
(그림~\ref{fig:sm-mu} 가 이 함수의 세 가지 개형이다.) 진행률 $\xi=1-\theta$ 로 옮기면 섞임 몫과
...
전위의 조작적 정의에 흡수되어 별도 항으로 남지 않는다(전기화학의 표준 처리). 그 결과 남는 것이 측정 전위 $V$(vs Li/Li$^+$)다 --- 같은 재질 단자 사이 전위차라는 조작적 정의가
...
여럿으로 한 줄 넓혀, 본론과 Part T 가 중심식으로 쓰는 전하 보존 \emph{음함수}(implicit function)가 이 대정준 구조의
\emph{제약 반전}(constrained inversion)임을 보인다. 도착점은 그 음함수의 통계역학적 기원과 반전 유일한 근(unique root)의
fluctuation 증명이고, 이를 받아 다음 소절(\S\ref{sec:sm-macro})이 사다리를 마감한다.
...
교환하므로 화학퍼텐셜은 $\mu$ 하나뿐이다 --- 전이가 여러 개라도 전극에 거는 전위가 하나라는 실험
구도의 통계역학 쪽 표현이다.
...
\end{equation}
--- 독립 부분계 분배함수의 표준 인수분해다 \cite{hill1960,mcquarrie1976}. 근사 경계는 두 겹이다: (i) 클래스 \emph{안}의 이웃 상호작용 $\Omega_j$(\S\ref{sec:sm-mf})는 각 자리가
...
상호작용 몫을 상수로 $U_j$ 에 흡수한) 독립 자리 서식이다; (ii) 클래스
...
\begin{verifybox}
검산 세 건 --- (i) \emph{유일한 근(fluctuation positivity).} 식~\eqref{eq:sm-mc-occ} 을 $\mu$ 로 미분하면 클래스마다
fluctuation--response 관계~\eqref{eq:sm-flucres} 가 반복되어
\begin{equation}
...
입자수 fluctuation 의 positivity 가 유일한 근을 보증한다는 이 한 줄이 Part T 의 계산(그 파트의 유일한 근 서술, \S\ref{ssec:overlap})의 통계역학
증명이고, 단일 자리 fluctuation~\eqref{eq:sm-flucres} 의 거시판이다. 가드 --- 평균장 이중웰($\Omega_j>2RT$,
...
Nernst 식이다($N_p\ge2$ 겹침에서는 닫힌 역이 없어 수치 유일한 근만 남는다). (iii) \emph{부호 읽기.}
...
\begin{bgbox}
\textbf{앙상블 동등성 --- 상대 fluctuation 의 $1/\sqrt{M}$ 소멸.} \S\ref{sec:sm-lattice} 는 대정준 경로(자리 하나의
...
같은 열역학을 주는 근거가 방금 계산한 fluctuation 에 있다. 식~\eqref{eq:sm-mc-fluc} 로 입자수 분산은 자리 수에
비례하고($\mathrm{var}(N)=\sum_jM_j\theta_j(1-\theta_j)=O(M)$, $M\equiv\sum_jM_j$) 평균도 자리 수에
비례하므로($\langle N\rangle=\sum_jM_j\theta_j=O(M)$, 식~\eqref{eq:sm-mc-occ}), \emph{상대} fluctuation 은
\[
\frac{\sqrt{\mathrm{var}(N)}}{\langle N\rangle}\ \sim\ \frac{1}{\sqrt{M}}
\ \xrightarrow[\ M\to\infty\ ]{}\ 0
\]
로 거시 자리 수 $M$(마이크론 흑연 입자에서 $M\gg10^{8}$)에서 소멸한다. 곧 $\mu$ 를 고정하면 --- 대정준에서
$\langle N\rangle$ 에 fluctuation 이 있더라도 --- 함량이 사실상 한 값에 고정되어, $N$ 을 고정한 정준 기술과 같은 강도적
...
식~\eqref{eq:sm-thresh})의 상분리 창은 강상관 영역이라 fluctuation 이 거시적으로 자라 상대 fluctuation 이 $1/\sqrt{M}$ 로 죽지
...
\S\ref{sec:eqpeak} 과 Part T 의 겹침 가중·계산용 종합식(\S\ref{ssec:overlap}$\cdot$\S\ref{sec:synthesis})이 이 계단을 결과 사슬에서 그대로 재현한다. 남은
...
\subsection{거시 열역학 유도 --- $G\equiv H-TS$, $\Delta G_j=-sFU_j$, Nernst 식}\label{sec:sm-macro}
...
$\theta$~\eqref{eq:fermifn}·fluctuation~\eqref{eq:sm-flucres} $\to$ lattice gas·섞임 엔트로피~\eqref{eq:sm-Smix}
$\to$ 평균장 $\Omega$~\eqref{eq:mu}·\eqref{eq:gxi}·문턱~\eqref{eq:sm-thresh} $\to$ 전위
결선~\eqref{eq:sm-eqcond} $\to$ logistic~\eqref{eq:sm-logistic} $\to$ 다클래스 전하 보존
반전~\eqref{eq:sm-mc-balance}($\langle N\rangle{=}\sum_jM_j\theta_j$ 의 유일한 근 $U_\mathrm{oc}$)
