...
- **토글 기본값**: (R1 당시 G1 bit-exact 우려로 문건을 임시 "기본 True"로 뒀으나) **★R5 에서 시드/브리프 사양 "기본 OFF"로 재정정** — W9 초안의 "기본 False"가 옳았다. G1 회귀는 게이트 하네스가 ON 을 명시 지정해 ON 경로 bit-exact 로 검증(v1.0.19 는 상시 ON). 상온 커브는 토글 무관 불변(사용자 "커브 구할땐 빼고" = 값선택 무관 성립), 토글은 ∂U/∂T(가역열)만 가름.
