...
% §8 동역학 지연 길이 L_V (N7)
% ====================================================================
\section{동역학 지연 길이 $L_V$}\label{sec:lag}
...
\begin{equation*}
\mathcal A_j^{(d)}=\sigma_d\big[sF(V-U_j)-\Omega_j(1-2\xi_j)\big]
...
방향별로 풀어 적으면 다음과 같다(식~\eqref{eq:chid} 의 $\chi_d$ 대입 --- 기본 $\chi=0.5$ 면 두 방향이
같은 $\Delta H_{a,j}-\Omega_j/2$ 로 합쳐진다):
\begin{center}
\begin{tabular}{@{}lll@{}}
\toprule
방향 & $\chi_d$ & $\Delta H_{a,j}^\eff$ \\
\midrule
...
\label{eq:Lqmid2}
\end{equation}
이 갈라 적기의 활성화 엔트로피 $\Delta S_{a,j}$ 의 미시 기원 --- 반응좌표를 뺀 분배함수 비의 로그
$R\ln(q^{\ddagger}/q_R)$ --- 은 \S\ref{sec:width} 의 배경 박스(식~\eqref{eq:tst-box})가 세웠다: 이 절은 그
갈래를 \emph{쓰고}, 그 박스가 \emph{기원}이다.
...
원천 정점의 일정 분율로 잡는다''는 컷 규칙의 정당성이다. 한편 $|I|$ 의존은 동결 뒤에도 남는다 --- $L_q\propto|I|$($T_*\propto|I|$)라 $|I|\to0$
...
부록~\ref{sec:appendix-code} 에 둔다. 이 컷 동결이 우회하는 참 자기일관($L_V=L_V(\xi)$)의 해석적 1차 닫힘과 그
타당성 조건$\cdot$수치 해법은 부록~\ref{sec:appendix-selfconsistent} 에 둔다.
여기서 세운 $L_{V,j}$ 가 다음 절의 인과 기억 적분(식~\eqref{eq:lag})에 들어갈 때 그 적분의 하한이 $-\infty$
라는 점이 구현상 pad 를 요구한다 --- 그 규약(진행방향 과거로 $5L_{V,j}$ 확장 후 원 격자 절단)은
\S\ref{sec:tail} 의 구현 각주에 둔다.
