...
\begin{warnbox}
\textbf{적합도 이전에 볼 것 --- 전하 보존과 미분 규약(v1.0.25.2).}
...
\emph{(나) 실측 $\dd Q/\dd V$ 를 만드는 미분 규약.} 상용 계측기의 전위는 유한한 눈금으로 양자화되어
있고, 평탄역에서는 인접 표본의 전위차가 눈금 이하로 내려가거나 \emph{정확히 0} 이 되는 구간이 생긴다.
이 자료에 단일 창 차분을 그대로 적용하면 평탄역에서 0 으로 나누는 꼴이 되어 $\dd Q/\dd V$ 가 물리적으로
불가능한 크기까지 발산한다 --- 그리고 그 발산점이 바로 우리가 가장 정밀하게 보려는 두-상 평탄이다.
규약은 두 층으로 둔다: (i) 서로 다른 폭의 창으로 구한 차분들의 \emph{중앙값}을 취해 0-나눗셈 표본을
흡수하고, (ii) 그 위에 평활을 얹되 \emph{평활은 미분을 매끄럽게 할 뿐 맞지 않는 모델을 맞게 만들지
않는다}. 이후 전위축을 균일 격자로 재빈닝해야 위 (가)의 적분과 구역별 잔차가 창 안에서 같은 가중으로
계산된다.
\end{warnbox}
