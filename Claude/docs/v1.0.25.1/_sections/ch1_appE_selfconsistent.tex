...
($Q_\cell q=Q_\bg(V_n)+\sum_jQ_j\xi_j$ 의 대수 순환)은 \emph{적분핵이 없는 대수 근찾기}이며, fluctuation positivity
...
\subsection{비선형 Volterra 자기일관과 동결 0차 기준}\label{sec:sc-volterra}
...
\textbf{(상호작용 항 복원 --- 완전 문제.)} 동결은 완화율 $k_j$ 안의 컷 affinity 를 상수로 고정한 결과다. 그러나
...
\begin{bgbox}
\textbf{사용자 논문과의 구조 차이.} 논문 \cite{lee2017jcp} 의 대상은 \emph{고정 구간}$\cdot$양측 결합의 제2종
...
형식의 이식은 성립하지 않는다 --- 이식되는 것은 Eq.~34 의 \emph{철학}(미지량을 비로 남기고 그 비를 가해 기준의
...
\emph{구현 각주(v1.0.25).} 식~\eqref{eq:sc-ratio} 의 하한도 동결해와 똑같이 $-\infty$ 이므로, 구현은
\emph{두 경로(동결$\cdot$ratio)에 같은 인과 pad} 를 적용한다 --- 진행방향 과거로 $5L_V^{(0)}$ 확장 후 원 격자만
절단(\S\ref{sec:tail} 구현 각주). 한쪽만 pad 하면 위 동결극한 정확 회수가 깨지므로 이 대칭이 필수다: 실제로
pad 를 동결 경로에만 넣었을 때 자기일관 게이트의 동결 회수 검사가 실패했고, ratio 경로에 같은 pad 를 넣어
$g=0$ 에서 $\lVert r_1-r_0\rVert=0$ 이 복구되었다(아래 derivbox 의 기계정확 항등이 그 검사다).
...
\begin{itemize}[leftmargin=1.7em,itemsep=0.4em,label={}]
...
\item \textbf{⑤\ 물리 가정 차(그대로 이식 금지).} (1) \emph{종이 다름} --- Fredholm(전역$\cdot$양측) vs Volterra(인과$\cdot$1측
  $=$Markov): 문건은 등가 국소 ODE 라 저렴한 전진해가 존재하므로 ratio 의 \emph{계산 동기}가 전이되지 않는다. (2)
...
\begin{keybox}
\textbf{이식의 실질 이득.} 문건 지연의 자기일관은 Markov(지수핵)라 \emph{완전 문제 자체가} $\mathcal O(N)$ 전진 ODE 다
--- 동결 0차$\cdot$1차 ratio$\cdot$완전 비선형 Picard 수렴이 모두 $\mathcal O(N)$. 따라서 이 이식의 의의는 논문에서의
``전역 역산 회피(계산 절감)''가 \emph{아니라}, (i) 동결 근사 위의 \emph{분석적 1차 닫힌형}(식~\eqref{eq:sc-ratio}),
(ii) 동결이 언제 옳은지를 정량하는 \emph{타당 조건}(식~\eqref{eq:sc-valid}), (iii) 사용자 방법의 문건 내 시연 ---
이 셋이다. 이는 ``수치 인과기억 루프를 계산 절감으로 대체''를 뜻하지 않는다.
\end{keybox}

\subsection{타당성 부등식 --- 동결 근사의 유효 조건}\label{sec:sc-valid-sub}
...
\midrule
(i) 비등방 섭동(perturbation) $K$ 과대 아님 & 기준 이탈이 소섭동 & 상태의존 $\delta\kappa$ 가 동결률 $\kappa_0$ 의 소섭동 ($2\chi_d(\Omega/RT)\Delta\xi_{\text{supp}}\ll1$) \\
...
\emph{1차의 주파수 해석.} 상태의존 $\kappa(\xi_0(V))$ 는 병진불변을 깨므로 단일 $H$ 가 없다 --- 극점 $1/L_V$ 가
