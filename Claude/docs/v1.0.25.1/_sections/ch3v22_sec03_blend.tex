...
흑연 다수에 실리콘 소수를 섞은 혼합(블렌드) 음극은 두 활물질이 \emph{같은 전위 변수를 공유}한다 ---
...
host 곱으로 한 줄 넓히는} 일이다. 재유도가 아니라 일반화다: 유일한 근을 보증한 fluctuation 논증은 그대로 이월되고
...
율속 블렌드 실측과는 정성적 편차}가 있으므로, \S\ref{ssec:si-blend-gs2} 가 그 편차를 미결 공백 GS-2 로
분리 진단한다.

\subsection{클래스곱의 host 곱 일반화}\label{ssec:si-blend-derive}
...
host 의 자리도 \emph{같은 하나의} 저장조와 교환한다(같은 전위 변수) --- 저장조 관점에서 흑연 클래스와
...
$N_p^\mathrm{gr}+N_p^\mathrm{Si}\ge2$ 겹침이므로 닫힌 역이 없어 \emph{수치 유일한 근}으로 푼다. $f_\mathrm{Si}$
는 그 반전에 들어가는 용량 배분 하나를 조절하는 인자이며(§3.5 코드의 토글), 두 host 의 서로 다른 전이
집합($U_j^\mathrm{host}\cdot w_j^\mathrm{host}$, §3.2)이 같은 $U_\mathrm{oc}$ 인자를 공유해 합산된다.
\begin{verifybox}
검산 세 건 --- (i) \emph{유일한 근(fluctuation positivity) 이월.} 식~\eqref{eq:blend-occ} 을 $\mu$ 로 미분하면
$(\mathrm{host},j)$ 마다 fluctuation--response가 반복되어
...
얹는} 것일 뿐이다. 곧 \S\ref{sec:sm-mc} 의 fluctuation positivity 유일한 근 증명(식~\eqref{eq:sm-mc-fluc})이 재유도
...
$\sum_j\to\sum_\mathrm{host}\sum_j$ 로 넓힌 \eqref{eq:blend-balance} 하나다 --- fluctuation positivity 유일한 근은
...
\subsection{미결 공백 GS-2 --- 공통-$\mu$ 완전 동시반응은 1차 근사다}\label{ssec:si-blend-gs2}
식~\eqref{eq:blend-balance} 은 \emph{평형}에서 정확하다 --- 평형이란 접촉한 모든 상이 하나의 $\mu$ 를
공유하는 상태이므로, 무한히 느린 준정적 OCV 곡선 $U_\mathrm{oc}(\bar x,T)$ 는 두 host 가 실제로 공통 전위에
...
--- 논리 공백(식의 결함)도, 수치해법 문제(유일한 근은 이미 보증됨)도, 정의상 implicit(그것은
...
유일한 근$\cdot$$f_\mathrm{Si}\to0$ 회수 이월) 대 \emph{유한 율속 층}(1차 근사: 완전 동시반응, 편차는 GS-2
...
\begin{warnbox}
GS-2 는 골격을 부정하지 않는다 --- 식~\eqref{eq:blend-balance} 의 평형 지위와 유일한 근$\cdot$회수 검산은
그대로 유효하다. GS-2 가 선언하는 것은 그 평형식을 유한 율속 블렌드 dQ/dV 에 \emph{완전 동시반응으로
직결}하는 읽기가 1차 근사라는 점, 그리고 그 편차의 정량 모델링이 범위 밖이라는 점이다. 이 명시적 분리가
