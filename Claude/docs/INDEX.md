...
> 사용자 지시 **페이블 → 10종 → 페이블 3단계**: ①Fable 한 세션 재작성(기존 틀 폐기·새 framework, 자료=계획서 1.0.10~1.0.18+이력 1.0.18+작업물 1.0.18) → ②10종 문제검수(이전본 vs 신본·regression·물리 적대검산) → ③Fable 최종수정. **★2대 무결: 물리·화학·수학 골격 오류 0(CENTRAL 12·비자명 11 독립 재유도 재현)·자산 회귀 0(336 전건 보존·강화)**. 결함 24 = 전부 구조 재배치·doc↔code·이전본 계승(물리 무관). 구조 재설계 = Part II 7분할·삼분해 전진·broadening 독립절·§5 유도선행·N0N1 통합. **Ch2·appendix·코드 = 18.2 승계(미변경, 문서 1.0.19 vs 코드 1.0.18.2 일시 분기)**. 계획 = `../plans/2026-07-08-v1019-ch1-fable-rewrite-plan.md` · 레저 = `../results/process/V1019_EXECUTION_LEDGER.md`.
...
| v1.0.19/ (Ch2·appendix·Anode_Fit_v1.0.18.2.py) | 18.2 승계 복제(미변경) — Ch2 17p·appendix 8p 버전태그만 1.0.19, **코드 frozen=1.0.18.2**(이번 범위 밖). Ch1 물리는 이 코드 모델과 정합 | 1.0.19, 승계, Ch2, 코드 frozen, 18.2 | 2026-07-08 |
