...
## ★기획 중 — v1.0.20 (이해 중간다리·서지 무결·컨벤션 통일 증판 — 마스터플랜 GO 대기, 2026-07-16)
> 사용자 피드백 반영 증판(F-1~F-10): 양자통계(페르미온/보손) 배경 다리 · **정통 유도 선행→확장 텀 후행 2단 제시**(지도교수 감수 q(T) 지적) · 히스테리시스 이후 중간다리 · LCO 수식사슬 대보강(말↓ 수식↑) · 통계역학 파트 기준 컨벤션 통일 · **서지 전수 온라인 실재 검증**(무인용 12곳 해소·원장 기반 인용). **물리 모델·코드 기능 불변(matched).** ★이 버전 한정 위치 규약(사용자 명시): 계획 = `v1.0.20/plans/` · 스텝 이력/result/ledger = `v1.0.20/results/`.

| 경로 | 1줄 요약 | 동의어 키워드 | 갱신일 |
|---|---|---|---|
| v1.0.20/plans/2026-07-16-v1020-master-plan.md | ★MASTER(GO 대기) — P0 골격/환경/rubric → P1 서지 원장 → P2 Part0 배경+q(T) 2단 → P3 흑연 다리 → P4 LCO 대보강 → P5 Ch2 정합 → P6 컨벤션 통일 → P7 10창 검수+서지 정합 → P8 마감. Decisions D-1~D-11(기본값 명시) | 1.0.20, 마스터플랜, 중간다리, 배경 다리, 페르미온 보손, q(T) 2단, 서지 검증, 레퍼런스 원장, 컨벤션 통일, GO 대기 | 2026-07-16 |
| v1.0.20/results/V1020_KICKOFF_SURVEY_structure_citation.md | 킥오프 조사① — v1.0.19 절 구성 인벤토리(행수·소절)·인용 실태(Ch1 41회/28건·Ch2 27회/14건·불일치 0·무인용 주장 12곳·영인용 절 11·키 함정 3)·q(T)/FD/보손 좌표 | 1.0.20, 킥오프, 인용 실태, 무인용, 서지, 좌표 | 2026-07-16 |
| v1.0.20/results/V1020_KICKOFF_SURVEY_history.md | 킥오프 조사② — v1.0.19 기록 세트·10창/R-P1 5창/2대 무결 체계·이월 3건(범위 밖 판정)·부록 별도 유지(v1.0.14 기존 결정)·양식 근거 | 1.0.20, 킥오프, 이력, 10창, 이월, 검수 체계 | 2026-07-16 |

## ★현행 릴리스 계열 — v1.0.19 (Ch1+Ch2 전면 재작성 + 코드 개정 + 3종 정합·완결 검수 완결 — Fable→10종→Fable·doc-leads, 2026-07-08~13)
