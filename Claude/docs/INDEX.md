...
> 사용자 지시 **페이블 → 10종 → 페이블 3단계**: ①Fable 한 세션 재작성(기존 틀 폐기·새 framework, 자료=계획서 1.0.10~1.0.18+이력 1.0.18+작업물 1.0.18) → ②10종 문제검수(이전본 vs 신본·regression·물리 적대검산) → ③Fable 최종수정. **★2대 무결: 물리·화학·수학 골격 오류 0(CENTRAL 12·비자명 11 독립 재유도 재현)·자산 회귀 0(336 전건 보존·강화)**. 결함 24 = 전부 구조 재배치·doc↔code·이전본 계승(물리 무관). 구조 재설계 = Part II 7분할·삼분해 전진·broadening 독립절·§5 유도선행·N0N1 통합. **★doc-leads: 문건이 authoritative — 코드(현 Anode_Fit_v1.0.18.2.py, 미변경)는 이후 이 v1.0.19 문건에 맞춰 개정된다(문서 1.0.19 vs 코드 1.0.18.2 버전차 = 의도된 정상, 코드가 문건 따라옴). **Ch1·Ch2 둘 다 같은 방식 재작성 완결(Ch1 61p·Ch2 24p·appendix 8p 전 GREEN).** 계획 = `../plans/2026-07-08-v1019-ch1-fable-rewrite-plan.md` · 레저 = `../results/process/V1019_EXECUTION_LEDGER.md`.

| 경로 | 1줄 요약 | 동의어 키워드 | 갱신일 |
|---|---|---|---|
| v1.0.19/graphite_ica_ch1_v1.0.19.tex | Ch1 v1.0.19(61p, 24절 \input 조립) — **Fable 전면 재작성**: 서론·§1(N0N1)·§2(Part0)·§3(U_j)·§4(히스)·§5(ξ_eq·폭 유도선행)·§6(평형peak)·§7(broadening 독립절)·§8(L_V)·§9(꼬리·반전)·§10(합산)·§11-17(Part II 7분할: 도입·중심·2상역·삼분해·전자항·peak·MSMR)·§18(입력)+부록A/B+서지. 물리 골격 오류0·자산336 보존·CRITICAL 12 앵커. 코드=18.2 정합 | 1.0.19, Ch1, 전면 재작성, Fable, 10종, Part II 7분할, broadening 독립절, 삼분해 전진, 자산 336, 재구조화 | 2026-07-08 |
| v1.0.19/graphite_ica_ch2_v1.0.19.tex | Ch2 v1.0.19(24p, 15절 \input) — **Fable 전면 재작성**(가역 발열·엔트로피): 서·§2.1(격자기체 분배함수)·§2.2(config)·§2.3(vib BE·electron FD)·**§2.4 vib Einstein 전용절 승격**·§2.5(겹침 파생 A-D)·§2.6(극한코너)·§2.7(가역발열 Bernardi)·§2.8(종합식·계산예제)·§2.9(방법론)+부록A(함정표)/B(doc-leads 코드 요구명세). 물리 골격 무결(CU-1 부호 정정)·자산 133/133·narrowing 금지 강화. eq:Se-ch2(Ch1 분리) | 1.0.19, Ch2, 전면 재작성, 가역 발열, 엔트로피, vib Einstein §2.4, 파생 A-D, 자산 133, doc-leads | 2026-07-08 |
| v1.0.19/HANDOVER_v1.0.19.md | v1.0.19 인계 — Chain(←18.2←18.1←17←…)·페이블→10종→페이블 3단계·Ch1(2대 무결·구조 5건)+Ch2(물리 무결·CU-1 부호)·doc-leads(코드 후속 개정)·appendix 승계 | 핸드오버, 1.0.19, Ch1+Ch2 재작성, 3단계, doc-leads, 코드 개정 이월 | 2026-07-08 |
| v1.0.19/Anode_Fit_v1.0.18.2.py (미변경) | 현 구현(코드 = 18.2, 이번 미변경) — **문건이 권위·코드는 이후 v1.0.19 문건에 맞춰 개정 예정(doc-leads, 문건→코드)**. appendix 8p 승계 | 1.0.19, 코드 미변경, doc-leads, 후속 개정 | 2026-07-08 |
