...
#      C2 인과기억 pad(B): _causal_pad 신설 — 문건 eq:lag 하한 −∞ 실현(진행방향 과거 5·L_V, coarse
#         ≤L_V/20, 상한 4000점). dqdv 의 동결·ratio 인과 경로에 적용(평가창 시작점 의존 100%→≤1%).
#         기본 데이터셋(L_V~1e-6)은 인과 경로 미해상(평형 종 가드)이라 골든 bit-exact 무변경.
#      C3 상수 SI(opt-in): R_SI/F_SI + use_si_constants() — 기본 R/F 는 레거시 유지(즉시 교체는
#         골든 bit-exact 게이트와 양립 불가). SI 발효 시 회귀 기준 표시 정밀도 유지 = v1025 G-SI 증빙.
#      C6 @3 정칙용액(Frumkin) 커널 삭제(DG-1): 커널 함수 3종(_REGSOL_XG·_regsol_binodal_xa·
#         _regsol_dqdv)·equilibrium 의 'kernel' 분기 전부 제거 → 커널 계통 = 로지스틱 단일계.
#         근거: 전이 세분(gallery) 과 이득이 중복이고 세분 적용 시 열등(전이 고정 +0.97%p →
#         세분 후 −0.53%p 역전). 해석적 유도 기록은 문건 Ch3 sifr 절에만 남는다(코드 미구현).
#         'kernel' 키가 남아 있어도 무시된다(=로지스틱) — 하위호환 무해·reflect G-R3 이 증빙.
#      C7 Si 7-gallery opt-in: SI_MSMR7_LIT(0.433/0.456 V feature 포함) 추가. 기본 셋 무변경.
#      게이트: test_gates_v1025.py(G-α1~4·G-창·G-극단·G-SI·G-si7) + 기존 4종 전건 GREEN.
#   -- 이하 v1.0.24 헤더 원문(계보 보존) --
# ★ Anode_Fit  release 버전 = 1.0.24  — 문건 Ch1/Ch2 v1.0.24 와 동일 버전·matched
#    (기본 흑연 dQ/dV 평형곡선 경로만 1.0.21 bit-exact 승계. v1.0.24 additive: @3 regsol 커널(opt-in — v1.0.25 삭제)·@5 XRD
...
#   [v1.0.24 추가 — 부록 E 자기일관 해법(선택적·기본 off, 동결 경로 bit-exact 보존)]
...
# [v1.0.25 C3] SI(CODATA-2018) 상수 — ★opt-in. 문건 worked value 25.693 mV(=R_SI·298.15/F_SI)와
#   정합하려면 use_si_constants(True) 호출(모듈 전역 R·F 재바인딩). 기본은 레거시 R=8.314·F=96485.0
#   유지 — 이유: 즉시 교체는 전 곡선을 ~5e-5 상대 이동시켜 골든 bit-exact 게이트(test_gates_v1024
#   G1 max|d|=0)·'부재 시 bit-exact' 원칙과 양립 불가. SI 발효 시에도 회귀 기준(74.4 mV·−0.204/
#   −0.134/−0.070 mV/K·+60.8 mV 등)이 표시 정밀도 내 유지됨은 test_gates_v1025 G-SI 가 수치 증빙.
#   ※주의: 토글 이전에 생성된 모델 인스턴스의 seed_L_V 는 재계산되지 않는다(파생 초기값 캐시).
R_SI = 8.314462618      # [J/(mol·K)] CODATA-2018
F_SI = 96485.33212      # [C/mol]     CODATA-2018
_R_LEGACY, _F_LEGACY = R, F


def use_si_constants(enable: bool = True) -> None:
    """[v1.0.25 C3] 기체상수 R·패러데이 F 를 SI(CODATA-2018) 값으로 전환(opt-in)/복귀.

    enable=True → R=R_SI·F=F_SI, False → 레거시(8.314·96485.0 = 골든 bit-exact 기준) 복귀.
    모듈 전역 재바인딩이므로 이후 호출되는 모든 경로(func_w·func_U_j·엔트로피·발열 등)에 일괄
    적용된다. 기본 상태(미호출)는 레거시 — '부재 시 bit-exact' 계약 유지."""
    global R, F
    if enable:
        R, F = R_SI, F_SI
    else:
...
def func_dxi_eq(T: ScalarOrArray, V_n: ScalarOrArray, U: ScalarOrArray,
                n: float = 1.0, s: int = 1,
                alpha: float = 1.0) -> Tuple[ScalarOrArray, ScalarOrArray]:
    """[v1.0.25 C1 @2] skew-logistic 평형 미분 종·진행좌표(문건 eq:skewpeak — opt-in).

    σ = logistic[s(V_n−U)/w] (=func_ksi_eq), w = n·R·T/F 에서
        진행좌표  ξ_eq = σ^α                     (단조 0→1 — lag 적분·솔버 브래킷 그대로 유효)
        미분 종   dξ_eq/dV = (α/w)·σ^α·(1−σ)     (∫dξ_eq = 1 정확 → 면적 보존·C^∞)
    α>1 = 진입측(저-σ) 가파름·완료측(고-σ) 완만, α<1 = 반대. α 는 전류·η 무관 '평형' 형상
    파라미터(현상학적·tier-C — 새 물리/새 상 아님)로, 유한율속 broadening(①L_V)과 별개 축.
    정적 단일 T 곡선에서 α·L_V·gallery 세분은 상호 축퇴 — 동시 자유화 지양(§18 가드, G-α4).

    ★bit-exact 계약: alpha=1.0 이면 σ(1−σ)/w 를 기존 연산·순서 그대로 반환하고 ξ_eq=σ
      (거듭제곱 미경유) — 'alpha' 부재 경로가 v1.0.24.1 과 부동소수점까지 동일한 근거(G-α1).
    반환: (dξ_eq/dV, ξ_eq).
    """
    sig = func_ksi_eq(T, V_n, U, n, s)
    w = func_w(T, n)
    if alpha == 1.0:
        return sig * (1.0 - sig) / w, sig
    ksi = sig ** alpha
    return (alpha / w) * ksi * (1.0 - sig), ksi
...
# [v1.0.25 C2] 인과기억 pad(B) — 문건 eq:lag 적분 하한 −∞(자연경계) 실현 파라미터.
#   pad 길이 = 5·L_V(커널 잔여 e^{−5}≈0.7%), pad 격자 간격 ≤ L_V/20(커널 해상), 상한 4000점.
_LAG_PAD_NLV = 5.0
_LAG_PAD_MAXPTS = 4000


def _causal_pad(V_prog: np.ndarray, lag_length: float):
    """[v1.0.25 C2] 진행방향 '과거'로 lag_length·_LAG_PAD_NLV 만큼 확장한 격자와 pad 점수.

    반환 (V_ext, npad). npad=0 이면 확장 불가/불필요(단일점·비유한·L_V≤0·간격 0 격자).
    pad 격자는 데이터 격자에 묶지 않는다 — 커널 해상(간격 ≤ L_V/20)만 지키는 coarse 격자로
    잡아 점수 상한(_LAG_PAD_MAXPTS)이 실질 제약이 되지 않게 한다. 호출측은 확장 격자에서
    ξ_eq 를 재평가해 인과 적분한 뒤 원 격자 부분([npad:])만 절단 반환한다 — 평가창 시작점
    의존(원본 최대 100%)을 제거한다(test_gates_v1025 G-창 ≤1%).
    """
    n = V_prog.size
    if n < 2 or not np.isfinite(lag_length) or lag_length <= 0.0:
        return V_prog, 0
    step = float(V_prog[1] - V_prog[0])          # 진행방향 부호 포함
    if step == 0.0:
        return V_prog, 0
    need = _LAG_PAD_NLV * lag_length
    pstep = min(abs(step), lag_length / 20.0)
    npad = int(np.ceil(need / pstep))
    if npad <= 0:
        return V_prog, 0
    if npad > _LAG_PAD_MAXPTS:
        npad = _LAG_PAD_MAXPTS
        pstep = need / npad
    sgn = 1.0 if step > 0 else -1.0
    pre = V_prog[0] - sgn * pstep * np.arange(npad, 0, -1)
    return np.concatenate([pre, V_prog]), npad
...
class GraphiteAnodeDischargeDQDV:
    ...
    def __init__(self, transitions: List[Dict[str, Any]], x: float = 0.5,
        ...
        # [v1.0.25 C1/G-α4] α↔L_V 식별 축퇴 가드(문건 §18): 정적 단일 T 곡선에서 봉우리
        #   비대칭을 α(평형 skew)와 L_V(유한율속 꼬리)가 나눠 가질 수 있어 동시 자유화는
        #   축퇴다 — 같은 전이에 'alpha'≠1 와 'L_V'>0 직접 지정이 공존하면 경고(차단 아님).
        #   분리는 율속 스윕(①L_V∝|I|·전이 소멸 vs α 전류 무관)으로만 가능하다.
        for _tr in self.transitions:
            _a = _tr.get('alpha')
            _L = _tr.get('L_V')
            if (_a is not None and float(_a) != 1.0
                    and _L is not None and float(_L) > 0.0):
                import warnings
                warnings.warn(
                    "[v1.0.25 G-α4] 'alpha'(≠1)와 'L_V'(>0) 동시 지정: 정적 단일 T 곡선에서 "
                    "두 파라미터는 비대칭을 놓고 축퇴한다(§18) — 동시 자유화 지양, "
                    "율속 스윕(L_V∝|I|)으로만 분리 가능.", stacklevel=2)
    ...
    # ---- [v1.0.25 C1] 비대칭 지수 α (opt-in; 부재=1.0=현행 bit-exact) --------
    def _alpha_factor(self, tr: Dict[str, Any]) -> float:
        """전이의 skew 지수 α(키 'alpha', eq:skewpeak). 부재 → 1.0(현행 로지스틱 bit-exact).
        유한·양수 가드. v1.0.25 에서 커널 계통은 로지스틱 단일계이므로 α 규약은 네 경로
        (equilibrium·dqdv·entropy_coefficient·solve_U_oc) 전부에 동일하게 적용된다."""
        a = tr.get('alpha')
        if a is None:
            return 1.0
        return _finite_pos("alpha", a)
    ...
    def equilibrium(self, V_n: ScalarOrArray, T: float = 298.15) -> ScalarOrArray:
        ...
        for tr in self.transitions:
            ...
            w = self._width(tr, T)
            # 커널 = 로지스틱 MSMR 단일계(v1.0.25 DG-1: @3 regsol 분기 삭제) — bit-exact 보존.
            #   'kernel' 키가 남은 legacy 전이 dict 도 이 경로를 탄다(키 무시 — reflect G-R3).
            alpha_j = self._alpha_factor(tr)
            if alpha_j == 1.0:
                # [v1.0.25 C1] alpha 부재(=1.0): 기존 식·연산 순서 그대로(골든 bit-exact).
                #   func_dxi_eq(α=1)와 수학·수치 동일 — 동일성은 G-α1(array_equal)이 증빙.
                ksi_eq = func_ksi_eq(T, V, U_j, n_j)
                dqdv = dqdv + tr['Q'] * ksi_eq * (1.0 - ksi_eq) / w
            else:
                # [v1.0.25 C1 @2] skew-logistic 미분 종(eq:skewpeak): Q·(α/w)σ^α(1−σ) — opt-in.
                dxi_eq, _ksi = func_dxi_eq(T, V, U_j, n_j, 1, alpha_j)
                dqdv = dqdv + tr['Q'] * np.asarray(dxi_eq, dtype=float)
        return dqdv
    ...
    def dqdv(self,
        ...
        for tr in self.transitions:
            ...
            # [v1.0.25 C1] 평형 미분 종·진행좌표 — func_dxi_eq 경유. alpha 부재(=1.0):
            #   dxi_eq=ξ_eq(1−ξ_eq)/w·ksi_eq=σ 로 기존과 연산까지 동일(bit-exact).
            #   alpha≠1(opt-in @2): dxi_eq=(α/w)σ^α(1−σ)·진행좌표 ξ_eq=σ^α — 단조 0→1 이라
            #   인과 lag 적분·(ξ_eq−ξ_lag)/L_V 꼬리 구성이 그대로 유효하다.
            alpha_j = self._alpha_factor(tr)
            dxi_eq, ksi_eq = func_dxi_eq(T_prog, V_prog, center, n_j, sigma_d, alpha_j)
            dxi_eq = np.asarray(dxi_eq, dtype=float)
            ksi_eq = np.asarray(ksi_eq, dtype=float)
            ...
            # [v1.0.25 C2] pad 확장 격자 위 진행좌표 재평가 헬퍼(동결·ratio 두 인과 경로 공용).
            #   pad 구간의 T·center 는 진행 시작값으로 동결(pad 는 5·L_V 의 국소 과거 — 근사 명시).
            def _ksi_eq_ext(Vx: np.ndarray, npad: int) -> np.ndarray:
                if npad == 0:
                    return ksi_eq
                Tx = np.concatenate([np.full(npad, float(T_prog[0])), T_prog])
                n_x = self._n_factor(tr, Tx)
                if np.ndim(center) == 0:
                    cx = center
                else:
                    cv = np.asarray(center, dtype=float)
                    cx = np.concatenate([np.full(npad, float(cv.reshape(-1)[0])), cv])
                return np.asarray(func_dxi_eq(Tx, Vx, cx, n_x, sigma_d, alpha_j)[1],
                                  dtype=float)
            ...
            if (is_scalar or n_pts < 2 or (not np.isfinite(lag_len_V))
                ...
                peak_shape = dxi_eq   # 평형 종(eq:tail-limit; alpha=1 → ξ_eq(1−ξ_eq)/w bit-exact)
                ...
                #   [v1.0.25 C2] 동결 경로와 같은 pad 적용(g_eff=0 두 경로 일치 보존 — G-E2).
                Vx, npad = _causal_pad(V_prog, lag_len_V)
                kx = _ksi_eq_ext(Vx, npad)
                ksi_lag0 = _causal_memory_pointwise(Vx, kx, lag_len_V)
                g_eff = self._lag_ratio_geff(tr, T_rep, sigma_d)
                ksi_lag, L_loc = _causal_memory_ratio(
                    Vx, kx, ksi_lag0, lag_len_V, g_eff)
                if npad:
                    ksi_lag, L_loc = ksi_lag[npad:], L_loc[npad:]
                peak_shape = (ksi_eq - ksi_lag) / L_loc
            else:
                # 인과 기억 적분 ξ_lag(진행 방향, eq:lag·eq:reversal) → peak(eq:peakshape).
                # [v1.0.25 C2] 진행방향 과거로 5·L_V pad 후 절단 — 적분 하한 −∞(자연경계) 실현.
                Vx, npad = _causal_pad(V_prog, lag_len_V)
                kx = _ksi_eq_ext(Vx, npad)
                ksi_lag = _causal_memory_pointwise(Vx, kx, lag_len_V)
                if npad:
                    ksi_lag = ksi_lag[npad:]
    ...
    def entropy_coefficient(self, V_n: ScalarOrArray,
        ...
        for tr in self.transitions:
            ...
            # [v1.0.25 C1] 가중 g_j = dξ_eq/dV — alpha 부재(=1.0)는 기존 식·연산 그대로
            #   (bit-exact), alpha≠1 은 func_dxi_eq 의 skew 미분 종 (α/w)σ^α(1−σ) 로 교체.
            #   config 항 좌표 ln[ξ/(1−ξ)] 는 로지스틱 σ 기준을 유지한다(∂w/∂T 열적 서식의
            #   유도 좌표 — α 는 가중 g_j 에만 반영, α≠1 정밀 config 는 다온도 피팅 후속).
            alpha_j = self._alpha_factor(tr)
            if alpha_j == 1.0:
                g = xi * (1.0 - xi) / w
            else:
                g = np.asarray(func_dxi_eq(T, V, U_j, n_j, 1, alpha_j)[0], dtype=float)
    ...
    def solve_U_oc(self, x_bar: ScalarOrArray, T: float = 298.15,
        ...
        for tr in self.transitions:
            ...
            n_j = float(np.asarray(self._n_factor(tr, T)).reshape(-1)[0])
            Q_j = _finite("Q", tr['Q'])
            a_j = self._alpha_factor(tr)   # [v1.0.25 C1] 부재=1.0(bit-exact)
            params.append((Q_j, U_j, n_j, a_j))
            Q_tot += Q_j
        if Q_tot <= 0.0:
            raise ValueError("sum of transition 'Q' must be > 0 for eq:implicit.")

        # 초기 괄호 — 미지정 시 전이 중심·폭에서 자동(전위대 무관: 흑연 ~0.1 V·LCO ~3.9 V 공통).
        w_max = max(abs(float(func_w(T, n_j))) for (_, _, n_j, _) in params)
        # [v1.0.25 C1] α<1 은 저-V 꼬리 감쇠율이 α/w 로 느려짐 — 괄호 마진에 1/min(α,1)
        #   반영(전 전이 α=1 이면 ÷1.0 = 기존 마진과 bit-exact·IEEE 정확).
        a_min = min(min(a_j, 1.0) for (_, _, _, a_j) in params)
        margin = max(1.0, 40.0 * w_max / a_min)
        U_lo = (min(U_j for (_, U_j, _, _) in params) - margin
                if U_lo is None else _finite("U_lo", U_lo))
        U_hi = (max(U_j for (_, U_j, _, _) in params) + margin
        ...
        def _charge(U_oc: float) -> float:
            ...
            total = 0.0
            for Q_j, U_j, n_j, a_j in params:
                if a_j == 1.0:   # [v1.0.25 C1] 부재/1.0 = 기존 경로 그대로(bit-exact)
                    total += Q_j * float(func_ksi_eq(T, U_oc, U_j, n_j))
                else:            # 누적 ξ_eq = σ^α (단조 — 이분 브래킷 유효)
                    total += Q_j * float(func_dxi_eq(T, U_oc, U_j, n_j, 1, a_j)[1])
            return total
...
#   참고(감사 E-2): Ω/RT=[4.06,2.02,3.55,4.07](v1.0.24 regsol2 진단 산출값 — 커널 자체는 v1.0.25 삭제,
#     이 Ω 시드값은 §7 상성격 판정 근거로 유효)은 위 5-feature 가 아니라 tab:staging **4전이**
...
# ============================================================================
# [v1.0.25 C7] Si 7-gallery MSMR (고분해능 opt-in — 계획서 D-B: 기본 셋 무변경·additive)
#   목적: p-ocvhold(완화 포함) 프로토콜의 Si dQ/dV 는 0.43–0.46 V 결정화 feature(c-Li₁₅Si₄
#     탈리튬화 쌍)를 포함해 6–7 feature 로 해상된다(v1.0.25 세션 실측: si_Dhold·si_Chold 에서
#     0.433·0.456 V 검출, p-ocv 에는 부재 — 프로토콜 의존 feature). 본 상수는 그 최고 해상도
#     opt-in gallery 셋이다(순수 로지스틱 — v1.0.25 는 커널 분기 자체가 없다).
#   ★gallery≠상(phase): 전이 세분은 곡선 표현 해상도이지 XRD 상 수를 바꾸지 않는다
#     (§5b 해상도 사다리·§7 위임 — GRAPHITE_STAGING_MSMR6_LIT 와 동일 지위·동일 관용).
#   ★tier: U 의 0.433/0.456 anchor = v1.0.25 세션 실측(hold 프로토콜). 나머지 U·w·Q = tier-C
#     seed(피팅 override 전제 — 신뢰값 아님). 기본 SI_*_LIT·GRAPHITE_STAGING_MSMR6_LIT 무변경.
#   사용: GraphiteAnodeDischargeDQDV(SI_MSMR7_LIT, ...) 또는 BlendedAnodeDQDV(si_transitions=SI_MSMR7_LIT).
SI_MSMR7_LIT: List[Dict[str, Any]] = [
    {'U': 0.170, 'w': 0.045, 'Q': 0.10},   # 저전위 진입 경사(비정질 Li_xSi 깊은 리튬화측)
    {'U': 0.270, 'w': 0.050, 'Q': 0.20},   # 상단 경사역 1(비정질 고용체 broad)
    {'U': 0.330, 'w': 0.040, 'Q': 0.20},   # 상단 경사역 2
    {'U': 0.390, 'w': 0.030, 'Q': 0.15},   # 0.4 V 어깨(탈리튬 주 peak 하부)
    {'U': 0.433, 'w': 0.012, 'Q': 0.15},   # ★0.433 V feature(c-Li₁₅Si₄ 결정화 쌍 低V — hold 검출)
    {'U': 0.456, 'w': 0.010, 'Q': 0.12},   # ★0.456 V feature(결정화 쌍 高V — hold 검출)
    {'U': 0.520, 'w': 0.045, 'Q': 0.08},   # 고전위 완만 꼬리(잔여 탈리튬)
]


# ============================================================================
# [v1.0.25.2] 흑연 7-gallery skew (고분해능 opt-in — 사용자 확정 2026-07-27, 실측 적합값)
...
#     (b) alpha 7 개 중 5 개가 탐색 상한(8.0)에 포화했다 — 곧 alpha 값 자체는 **식별된 값이 아니다**
#         (상한 위 어느 값이어도 거의 같은 곡선). alpha 는 여기서 분해 한계 아래의 급준 평탄을
...
GRAPHITE_MSMR7_LIT: List[Dict[str, Any]] = [
    # U [V], w [V], Q [mAh], alpha(@2 skew 지수 — 부재 시 1.0=대칭)
    {'U': 0.100375, 'w': 0.028473, 'Q': 0.678044, 'alpha': 4.057348},  # 2→1 군 broad
    {'U': 0.102465, 'w': 0.002296, 'Q': 0.238334, 'alpha': 7.329476},  # 2→1 군 근축퇴
    {'U': 0.103049, 'w': 0.000563, 'Q': 0.154685, 'alpha': 7.935276},  # 2→1 군 sharp
    {'U': 0.103143, 'w': 0.000158, 'Q': 0.080494, 'alpha': 7.996230},  # 2→1 군 최급준 (주의 c)
    {'U': 0.135802, 'w': 0.003191, 'Q': 0.252390, 'alpha': 7.452150},  # 3→2L 군
    {'U': 0.140376, 'w': 0.000719, 'Q': 0.177268, 'alpha': 3.131948},  # 3→2L 군 sharp
    {'U': 0.222805, 'w': 0.002071, 'Q': 0.105206, 'alpha': 7.771239},  # 4→3 / dilute 측 단일
]
# 위 셋과 짝이 되는 배경: C_bg ≈ 0.55 (전극 1회, 창 0.060–0.300 V). 전이 파라미터만 옮기고
#   배경을 빠뜨리면 벨리역이 통째로 어긋난다 — 반드시 함께 쓸 것.


# ============================================================================
# [v1.0.25.2] Si 7-gallery skew (고분해능 opt-in — 확정 구성의 Si 쪽 짝)
#   SI_MSMR7_LIT(대칭·문헌 anchor 셋)은 **무변경 보존**하고, 본 상수를 additive 로 둔다.
...
SI_MSMR7_SKEW_LIT: List[Dict[str, Any]] = [
    ...
    {'U': 0.285051, 'w': 0.027272, 'Q': 0.040562, 'alpha': 7.365510},
    {'U': 0.432811, 'w': 0.002459, 'Q': 0.019585, 'alpha': 0.989408},
    {'U': 0.434231, 'w': 0.008224, 'Q': 0.069415, 'alpha': 0.150000},
    {'U': 0.445008, 'w': 0.005660, 'Q': 0.005803, 'alpha': 8.000000},
    {'U': 0.461570, 'w': 0.058229, 'Q': 1.247144, 'alpha': 0.254744},
    {'U': 0.468890, 'w': 0.022044, 'Q': 0.124121, 'alpha': 1.457959},
]
...
#     전부 추가(additive)다. 기존 게이트(test_gates_v1024.py G1/G2/G3/n(T))는 GREEN 유지.
# ============================================================================
...
class BlendedAnodeDQDV:
    """흑연+Si 블렌드(혼합) 음극 dQ/dV — 공통-μ 대정준 반전의 코드판(문건 v1.0.24 §3.3·§3.5).
    ...
    **host_kwargs
    ...
    def __init__(self, f_Si: float, si_case: str = 'sic',
        ...
        #   (wt% 기준 [0,0.30] 은 f_Si≈0–0.8 에 대응[sic q_Si=3117 서 0.78], §3.3 각주), 상한 배제로 1/(1−f_Si) 발산도 차단.
    ...
    def from_wt(cls, m_Si: float, q_Si: Optional[float] = None,
        ...
        (0–30 wt% ≈ f_Si 0–0.8; sic q_Si=3117 서 30 wt%→0.78). wt% 실측과의 대조는 이 환산을 거친다.
...
if __name__ == "__main__":
    ...
    # ---- [v1.0.25 C1] alpha 발효 실증: alpha=2 실제 비대칭·면적 보존·부재 bit-exact ----
    print("=== v1.0.25 alpha (skew-logistic @2, opt-in) demo ===")
    Vs = np.linspace(-0.35, 0.55, 240001)
    m_sym = GraphiteAnodeDischargeDQDV([{'U': 0.10, 'w': 0.014, 'Q': 1.0}], Cbg=0.0)
    m_a1 = GraphiteAnodeDischargeDQDV([{'U': 0.10, 'w': 0.014, 'Q': 1.0, 'alpha': 1.0}], Cbg=0.0)
    m_skw = GraphiteAnodeDischargeDQDV([{'U': 0.10, 'w': 0.014, 'Q': 1.0, 'alpha': 2.0}], Cbg=0.0)
    ys = np.asarray(m_sym.equilibrium(Vs), dtype=float)
    y1 = np.asarray(m_a1.equilibrium(Vs), dtype=float)
    yk = np.asarray(m_skw.equilibrium(Vs), dtype=float)
    _tz = getattr(np, "trapezoid", None) or getattr(np, "trapz")
    area = float(_tz(yk, Vs))
    Vpk = float(Vs[np.argmax(yk)])
    half = 0.5 * float(np.max(yk))
    above = Vs[yk >= half]
    lw, rw = Vpk - float(above[0]), float(above[-1]) - Vpk
    bitex = np.array_equal(ys, y1)                     # alpha=1 명시 == 부재 (bit-exact)
    skewed = (abs(lw / rw - 1.0) > 0.05) and (float(np.max(np.abs(yk - ys))) > 1e-3)
    ok_alpha = bitex and skewed and (abs(area - 1.0) <= 1e-6) and _ok(yk)
    all_ok &= ok_alpha
    print(f"  alpha=1.0 explicit == absent (np.array_equal): {bitex}")
    print(f"  alpha=2.0: area={area:.8f} (=Q=1, |err|<=1e-6)  peak@V={Vpk:.4f}  "
          f"half-max width L/R={lw*1e3:.2f}/{rw*1e3:.2f} mV (asymmetric)  -> effective: {ok_alpha}")
