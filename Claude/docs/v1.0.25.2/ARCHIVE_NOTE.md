# v1.0.25.1 — v1.0.25 검증·정직성 touch-up 리비전 (현행 최신)

> ★본 폴더 = v1.0.25(국소 수정판) + 마스터 산문 touch-up 4건(F1 regsol warnbox 정직화·F3 inline·
> M-w §5 포인터·L-bg §6 α=1 표지). 상세·검증·미완 = `results/V1025_1_TOUCHUP_NOTE.md`(권위 기록).
> 표시 버전 = v1.0.25.1(3 마스터), 코드 release = 1.0.25(byte-identical). 파일명·xr 키 불변(DG-2).
> 아래 본문은 **계보 승계분**(v1.0.24.1 원본 노트 + v1.0.25 S1~S6 추기)이며 provenance 참고용 보존이다.

---

# (계보 승계) v1.0.24.1 — v1.0.24 피드백 리비전(FB0~FB9) 동결 아카이브

## 지위
**동결(frozen) · 아카이브 전용.** 본 폴더는 v1.0.24 가 사용자 1차 정독 피드백(F-01~F-11) + 어조 강화(FB8) +
★ 마커 제거(FB9)를 거친 **리비전 완료 상태**의 충실한 스냅샷이다. 이후 개발은 본 폴더가 아니라 `v1.0.24/`
(또는 후속 버전)에서 진행한다. 파일 내용은 수정하지 않는다.

## 출처(provenance)
- 원본 = `Claude/docs/v1.0.24/` (동일 커밋 시점).
- 리비전 캠페인 = `Claude/results/V1024_FEEDBACK_EXECUTION_LEDGER.md` (FB0~FB9 12-col 원장).
- phase 결과 = `Claude/results/PHASE_FB0_RESULT.md` ~ `PHASE_FB9_RESULT.md`.
- 버전 비교 감사 = `Claude/results/comp_v24/VERSION_COMPARISON_v19_v23_v24.md`.

## 리비전 요약 (v1.0.24 원판 → 본 동결본)
- **FB0~FB7 (F-01~F-11)**: §1.1.4 배경 압축·확률/압력 P 충돌 해소·식1.21 f 명료화·제목 N-태그 제거·register
  평서화·조판(여백/줄간/microtype)·E.3·식2.39 overflow 해소·LCO 서두 재균형·일본어투 용어 정리·**코드=부록 전용**.
- **FB8 (어조 강화)**: 은유·의인·구어 중립화(되밟다→재현·되살리다→복원·비틀다→조정 등) + 정의어 rename
  (생존 지도→대응 지도·정직 공백→미결 공백).
- **FB9 (★ 마커 제거)**: 본문 ★/$\bigstar$ 57개 전량 제거(주석 34행 동결), 앵커 3쌍 phrase 헤더 정합 유지.
- **불변**: 물리 골격·식 번호·`\label` 정의·`\eqref`/`\ref`/`\cite` 키·코드(`.py`) 무변경.

## 구성 (파일명 = v1.0.24 유지 · 동결 사본 방식)
- 마스터 3본: `ch1_graphite_v1.0.24.tex` · `ch2_lco_v1.0.24.tex` · `ch3_si_v1.0.24.tex`
- `_sections/` (56 .tex) · `appendix_phase_separation.tex`
- 코드: `Anode_Fit_v1.0.24.py` (sha256 `f230f59b…`) · `CODE_GUIDE_v24.{md,html}` · `FITTING_GUIDE.md` ·
  `test_gates_v1024*.py`
- 빌드본 PDF 3종(검토·게이트 통과본): `ch1_graphite_v1.0.24.pdf`(97p) · `ch2_lco_v1.0.24.pdf`(30p) ·
  `ch3_si_v1.0.24.pdf`(21p)
- `plans/` · `results/` (버전-로컬 마감 문서: MERGE_READINESS·HANDOVER·INDEX_v24 + FB addenda)
- **제외**: 빌드 중간산물(`*.aux`/`*.log`/`*.out`/`*.toc`/`__pycache__`) — 재빌드로 복원 가능.

## 무결성 검증 (동결 시점)
- `diff -rq v1.0.24 v1.0.24.1` (빌드산물 제외) = **IDENTICAL**.
- 코드 `sha256 = f230f59b…` (FB 캠페인 baseline 과 동일 — 문건 한정 리비전).
- 빌드 GREEN: 0-error · undefined ref/cite 0 · 97/30/21 페이지.
- 본문 ★ = 0 · 주석 ★ = 34(동결).

## 파일명 규약 주의
동결 사본 방식(사용자 결정)이라 **내부 파일명은 v1.0.24 를 유지**한다. 개정 식별자는 **폴더명 `v1.0.24.1`** 이
담당한다(정식 rename 스냅샷 아님). 서브버전 번호 규약은 `v1.0.18.1`/`v1.0.18.2` 선례를 따른다.

---

# ★ v1.0.25 추기 (2026-07-26) — 위 본문은 **v1.0.24.1 동결 아카이브** 기준임

> **읽는 법**: 위 본문(§지위~§파일명 규약 주의)은 `docs/v1.0.24.1/` 동결 사본을 기술한 원문이며 **삭제하지 않고
> 이력으로 보존**한다. 본 폴더 `docs/v1.0.25/` 는 그 사본에서 갈라져 **코드가 실제로 변경된 작업 폴더**이므로,
> 위 본문의 일부 주장은 이 폴더에 대해 **스테일(stale)** 이다. 아래가 그 정정이다. 충돌 시 **본 절이 우선**.

## S1. 스테일 판정 — 위 본문 중 v1.0.25 에 적용되지 않는 주장

| 위 본문 위치 | 주장 | v1.0.25 판정 |
|---|---|---|
| §리비전 요약 "불변" | "코드(`.py`) **무변경**" | **스테일** — v1.0.25 에서 코드가 변경됨(S2) |
| §구성 | "코드: `Anode_Fit_v1.0.24.py` (sha256 `f230f59b…`)" | **스테일** — 본 폴더 코드는 다른 해시(S2) |
| §무결성 검증 | "`diff -rq v1.0.24 v1.0.24.1` = **IDENTICAL**" | **원본 두 폴더에 대해서는 여전히 참**(S4). 단 `v1.0.25` 는 이 등식에 포함되지 않는다 |
| §무결성 검증 | "코드 `sha256 = f230f59b…` (FB 캠페인 baseline 과 동일 — **문건 한정 리비전**)" | **스테일** — v1.0.25 는 문건 한정 리비전이 **아니다**(코드+문건) |
| §무결성 검증 | "빌드 GREEN: 0-error · 97/30/21 페이지" | **v1.0.25 에서는 미검증** — 본 PC 에 TeX 배포판이 없다(S5) |
| §구성 | "빌드본 PDF 3종" | **본 폴더에 부재**(`v1.0.25/*.pdf` 없음). PDF 는 `v1.0.24.1/` 에만 있다 |
| §파일명 규약 주의 | "내부 파일명은 v1.0.24 를 유지" | **유효 — v1.0.25 에서 재확인**(DG-2 사용자 확정, S3) |

## S2. 코드 변경 (v1.0.25) — 파일명 유지 · 내부 release 버전 = 1.0.25

- 파일: `Anode_Fit_v1.0.24.py` (**파일명 불변** — DG-2(a) 규약). 코드 헤더 line 3 = "release 버전 = **1.0.25**".
- **추가**: `func_dxi_eq`(@2 skew, 전이 dict `'alpha'` 부재=1.0=bit-exact) · `_causal_pad`(eq:lag 하한 −∞ 실현) ·
  `R_SI`/`F_SI`/`use_si_constants()`(C3 상수 SI opt-in) · `SI_MSMR7_LIT`(C7 Si 7-gallery opt-in).
- **삭제**: regsol 커널 3종(`_REGSOL_XG`·`_regsol_binodal_xa`·`_regsol_dqdv`) + `equilibrium()` 의 `'kernel'` 분기
  → 커널 계통 = **로지스틱 단일계**. **Ω 파라미터·Ω 소비 함수는 전량 존치**(`func_dU_hys`·`func_dH_a_eff`·
  §7 상성격 판정) — 삭제된 것은 dQ/dV **커널**뿐이다.
- **줄 수**(정정 포함 — 아래 두 수를 혼동하지 말 것):
  - `docs/v1.0.24` · `docs/v1.0.24.1` 아카이브 코드 = **1734 줄**(LF 기준) → `docs/v1.0.25` 코드 = **1917 줄**.
  - regsol 삭제 **그 단계**의 변화는 **1957 → 1917 줄**(−40)이다. 이 1957 은 v1.0.24 아카이브가 아니라
    **v1.0.25 개발 중 C1·C2·C3·C7 추가가 끝난 시점**의 줄 수다(계획서 C6 항 "−40줄" 과 일치).
- 골든 bit-exact 유지: `test_gates_v1024.py` **G1 golden max|d| = 0.0**.

## S3. 문건 변경 (v1.0.25) — 표시 버전 문자열만 (DG-2)

- 마스터 3본 `ch1_graphite_v1.0.24.tex`·`ch2_lco_v1.0.24.tex`·`ch3_si_v1.0.24.tex` 에서 **사람이 읽는 표시 버전만**
  1.0.24 → 1.0.25 로 교체: `\hypersetup{pdftitle=…}` · `\lhead{…}` · `\date{\normalsize 버전 …}` 각 1회.
  각 파일 상단 주석 블록에 v1.0.25 이력 2줄 추가(기존 주석 줄 보존).
- **불변(개명 금지)**: 파일명 · `\input{_sections/…}` 경로 · `\externaldocument{…}` 키 ·
  `_sections/common_preamble_v1024.tex`(그 헤더의 v1024 는 자기 출처 버전을 적은 이력 주석) ·
  파일명·식별자에 박힌 `_v1.0.24`/`v1024`/`v22` 토큰.
- 절 본문(`_sections/*.tex` 56종)·`appendix_phase_separation.tex` 의 내용 편집은 **별건**(계획서 P2·P3 소관).

## S4. 원본 아카이브 무변경 확인

- `docs/v1.0.24.1/` · `docs/v1.0.24/` = **무변경**. 코드 LF-sha256 앞 16자리 = **`f230f59bb10bcc49`**, **두 폴더 동일**.
- 두 폴더의 추적 파일 261개 전부 git 상태 clean(작업 트리 변경 0).
- 빌드산물 제외 `diff -rq v1.0.24 v1.0.24.1` 의 유일한 차이 = `ARCHIVE_NOTE.md`(v1.0.24.1 에만 존재).

## S5. 게이트·빌드 상태 (증빙 출처 구분)

- **마스터 세션 실행분(인용)**: `test_gates_v1024` 전건 PASS · `test_gates_v1024_reflect` **4/4** ·
  `test_gates_v1024_selfconsistent` **5/5** · `test_gates_v1025` **8/8** · 모듈 `__main__` overall OK: **True**.
- **본 추기 작성 서브세션이 직접 확인한 것**(재실행 아님 — 정의 대조): reflect 게이트 정의 4종(G-R1·G-R2·
  **G-R3 = "@3 regsol 커널 삭제 확인"**·G-R4) · selfconsistent 5종(G-E1~G-E5) · v1025 8종(G-α1~α4·G-창·G-극단·
  G-SI·G-si7) 이 각 파일에 실제로 정의되어 있음, 그리고 regsol 심볼이 v1.0.25 코드에서 **주석 2행 외 부재**임.
- **LaTeX 빌드는 v1.0.25 에서 수행되지 않았다**: 본 PC 에 TeX 배포판 부재(xelatex·pdflatex·latexmk·lualatex·tex
  전부 PATH 에 없음, `C:\texlive` 부재). 대신 **구조 검사**로 대체 확인:
  `results/tools_check_structure.py check .` → **STRUCTURE_CHECK: PASS**
  (ch1: label 265·dup 0·unresolved ref 0 / ch2: 82·0·0 / ch3: 44·0·0 / appendix: 30·0·0 · env pairing error 0 ·
  cite-undef 0 · bib-uncited 0). **PDF 페이지 수·0-error 빌드는 미검증** — 재빌드 가능한 환경에서 3-pass 확인 필요.

## S6. 데이터 기록 정정

v1.0.24 데이터·피팅 증빙(`Claude/results/comp_v24/`)의 정정은 원본 수정이 아니라 addendum 으로 처리했다:
**`results/V1025_DATA_ADDENDUM.md`**(A1 프로토콜 혼용 · A2 p-ocvhold 권고 · A3 독립셀 재현성 ·
A4 @3 철회+코드 삭제 · A5 C_bg 비상수 · A6 @1~@5 조합 실측표 · A7 gallery≠상). comp_v24 원본은 무수정 보존.

---

# ★ v1.0.25.2 추기 (2026-07-27) — 확정 구성 반영 + v1.0.26 carryover 1차

> 충돌 시 **본 절이 우선**. 상세 명세·잔여 = `Claude/results/HANDOVER_v1025_2_CARRYOVER.md`.

## U1. 확정 구성 (사용자 2026-07-27)

**skew-logistic 흑연 7 · 실리콘 7 · 블렌드 14 + 자유 배경**, `L_V` 동결(§18 식별 가드).
regsol 은 **이론 층에만** 유지하고 피팅 커널 계통은 **로지스틱 단일계**(v1.0.25 판정 유지).
α 는 신규 물리가 아니라 v1.0.25 가 도입해 둔 `eq:skewpeak` opt-in 의 **발효**다.

## U2. 코드 변경 — additive opt-in 1건 (파일명 불변, DG-2)

- **추가**: `GRAPHITE_MSMR7_LIT`(흑연 7-gallery 시드). `GRAPHITE_STAGING_MSMR6_LIT`·`SI_MSMR7_LIT` 와
  동일 지위. 블렌드 14 = 흑연 7 + Si 7 조합으로 성립.
- **기본 경로 무변경** → 골든 bit-exact 계약(G1) 보존. `'alpha'` 키 미부여(부재=1.0=대칭).
- **tier-C seed 3중 단서**: 단일 셀·비평형 pOCV / 산출 당시 자유 배경 부재로 배경이 한 전이의 Q 를
  겸함(N10 — 그 전이 Q 특히 신뢰 금지) / w=0.1 mV 급은 계측 분해 한계 아래.

## U3. 문건 변경 (신규 식 1 · 신규 절 4 — 전부 additive, 삭제 0)

| 파일 | 내용 |
|---|---|
| `ch3v22_sec02b_sifr.tex` | **★`eq:sifr-twophase` 신설** — Ω>2RT 두-상 정칙용액 dQ/dV 닫힌형(혼화갭 닫힌항 + 고용체 밀도⊛broadening 핵). 종전 §(v) 는 산문 명세뿐이었다. 면적 보존·단일상 연속접속·합성곱이 항등인 이유·격자합 3 함정·이론층 지위 명기. 그리고 :43 "모수 절약(AIC/BIC) 미확정" → 단일 셀 확정으로 조준 축소(N10 단서 병기) |
| `ch1_sec05b_gr2L.tex` | 해상도 사다리에 **7-gallery** 추가(N=7 최소·N=8 포화·근축퇴 쌍 증가·**커널 무관**) / **Ω≷2RT 판정자의 실측 대응**(Ω/RT 1.9–2.5 = marginal 두-상, 독립 앵커 2.5RT·3.4RT 정합, 두 명제 분리) / **최저 전위 평탄의 계측 분해 한계**(FWHM ≲ 1 mV = RT/F 의 1/25) |
| `ch1_sec18_inputs.tex` | 식별 가드에 **다섯째 손잡이 Ω** 추가(경계 포화 = 미식별) / "gallery+α 동시 자유화 지양" → **조건부**로 조준 정정(대가는 적합도가 아니라 점-식별성, L_V 동결은 불변) / **신규 warnbox**: 면적 보존 Δ_Q 를 적합도와 독립된 채택 전 진단으로, 실측 dQ/dV 미분 규약(전위 양자화 0-나눗셈 → 다중창 중앙값 + 평활 + 균일 재빈닝) |
| 마스터 3본 | 표시 버전 1.0.25.1 → **1.0.25.2**(pdftitle·lhead·date). 파일명·`\input` 경로·xr 키 **전부 불변** |

## U4. 검증 (본 리비전)

- `results/tools_check_structure.py check .` → **STRUCTURE_CHECK: PASS**(unresolved ref 0 · cite-undef 0)
- `eq:sifr-twophase` 정의 1회 · 참조 무결 · 기존 라벨 무변경
- **P3-8 게이트**: 편집한 본문 3종 코드토큰 **0**
- 코드 `ast.parse` OK
- ⚠️ **XeLaTeX 빌드 미수행** — 본 환경에 TeX 배포판 부재. **재빌드 3-pass 0-error 확인이 잔여**다.
- ⚠️ **피팅 재실행 미수행** — numpy 부재. `C_skew` 판 산출과 N10 수정 효과 확인이 잔여다.

## U5. 잔여 (다음 예산)

A-3 수치 함정의 부록 정식 등재 · D 함정 기록(Ch2 부록 A) · C-4 소재별 문헌 서지 확장(M4 해소) ·
N7(skew-regsol N=7 스윕) · N8(평형 데이터 재검) · N9(bootstrap CI). 전부 `HANDOVER_v1025_2_CARRYOVER.md`.

## U6. ★실측 확정 (2026-07-27 실행 — 세 판 동일 데이터·동일 절차)

흑연 `gr.csv`(744점)·실리콘 `si.csv`(528점)·블렌드 `sigr.csv`(1280점), **자유 배경 포함**(N10 수정 반영).

| 판 | 커널 | 소재 | N | 모수 | R² | BIC | 피크역 RMSE | 벨리역 RMSE |
|---|---|---|---:|---:|---:|---:|---:|---:|
| A | regsol | 흑연 | 4 | 17p | 0.92178 | 2554.9 | 18.079 | 1.768 |
| B | logistic | 흑연 | 7 | 22p | 0.97389 | 1771.7 | 11.304 | 1.376 |
| **C** | **skew-logistic** | **흑연** | **7** | **29p** | **0.99433** | **682.3** | **4.623** | **0.871** |
| B | logistic | 실리콘 | 7 | 22p | 0.99975 | −3447.9 | 0.074 | 0.073 |
| **C** | **skew-logistic** | **실리콘** | **7** | **29p** | **0.99991** | **−3922.1** | **0.017** | **0.033** |
| B | logistic | 블렌드 | 14 | 43p | 0.99943 | −4243.0 | 0.438 | 0.294 |
| **C** | **skew-logistic** | **블렌드** | **14** | **57p** | **0.99965** | **−4760.7** | **0.459** | **0.074** |

**판정 — 확정 구성(C)이 세 소재 전부에서 우세하다.** 모수 보정 지표 기준 흑연 **ΔBIC = −1089.4**,
실리콘 −474.2, 블렌드 −517.7. 흑연 피크역 RMSE 가 11.304 → **4.623** 으로 절반 이하가 되고,
블렌드 벨리역은 0.294 → **0.074** 로 4 배 개선된다. 곧 **@2 비대칭(α) 발효가 전이 수 증가보다 큰 단일 효과**
라는 v1.0.25 A6 의 판정이 모수 보정 축에서도 그대로 재현된다.

**⚠️ α 경계 포화 --- 값을 물리량으로 읽지 말 것.** 흑연 7 전이의 α 중 **5 개가 탐색 상한(8.0)에 붙었다**.
곧 α 는 식별된 값이 아니며(상한 위 어느 값이어도 거의 같은 곡선), 여기서 하는 일은 **분해 한계 아래의 급준
평탄을 세우는 것**이다 --- 이는 \S 5b 가 적은 ``최저 전위 평탄의 반높이 폭이 $RT/F$ 의 1/25'' 와 같은 사실의
다른 표현이다. **적합 이득은 실재하나 α 는 물리량이 아니다**(§18 식별 가드의 `경계 포화 = 미식별` 과 동일 병리).

**Ω 실측 재확인.** 흑연 정칙용액 4 전이 Ω/RT = **[1.916, 2.027, 2.472, 2.604]** --- 전부 문턱 $2RT$ 근방의
marginal 두-상. §5b 서술과 정합(그 값에 맞춰 본문 구간을 1.9--2.6 으로 확정).

**★N10 진단 정정(정직 기록).** 자유 배경을 넣어도 **흑연 로지스틱 적합은 사실상 변하지 않았다**
(R² 0.97389 동일, 벨리역 1.375 → 1.376). 자유화된 배경이 0.770 으로 종전 aliasing 값 0.763 과 거의 같았기
때문이다. 곧 **`배경 결함이 벨리 열세의 원인`이라는 앞선 진단은 로지스틱에 대해서는 성립하지 않는다.**
실제 원인은 **α 부재**였고, α 를 켜자 피크역이 v1.0.25 수준(4.708)과 같은 4.623 으로 내려왔다.
배경 결함이 실제로 불리하게 작용한 것은 **정칙용액 쪽**이다(배경이 마지막 폭 값 ≈0 을 가리켰던 판):
수정 후 BIC 2609.6 → 2554.9, R² 0.91506 → 0.92178 로 개선됐다. 커널 비교가 교란됐다는 지적 자체는 유효하나
**그 크기는 당초 우려보다 작다** --- 수정 후에도 A 는 B·C 에 크게 뒤진다.

## U7. 폴더 지위 (본 절이 최우선)

- **본 폴더 `docs/v1.0.25.2/` = 현행 최신 작업본.** 표시 버전 = v1.0.25.2. 내부 파일명은 규약(DG-2)대로
  `_v1.0.24` 토큰을 유지한다 — 개정 식별자는 **폴더명**이 담당한다(`v1.0.24.1`·`v1.0.25.1` 선례).
- **`docs/v1.0.25.1/` = 동결 아카이브.** v1.0.25.2 착수 시점 그대로이며 **한 줄도 수정하지 않는다**
  (git 검증: 빌드 커밋 `bada485` 대비 차이 0). PDF 3종은 v1.0.25.1 에만 있다.
- 위 U1~U6 은 전부 **본 폴더(v1.0.25.2)** 를 기술한다.
- 결과 리포트(적합 곡선 9종 이미지 포함, 자체완결 HTML) =
  `Claude/results/comp_v26_data/KERNEL_COMPARISON_REPORT.html`

## U8. 배치 정정 — 본문/부록 경계 (사용자 지적 2026-07-27)

**지적**: "문건은 코드 이야기를 하지 않는다는 규칙이 있고 오로지 코드의 근간이 되는 이론 문서여야 한다."
→ **타당**. U3 의 신규 서술 중 **두 곳이 본문에서 수치 구현 절차를 서술**하고 있었다(코드 *식별자* 는 0 이었으나,
"어떻게 구현하는가" 자체가 본문에 있었다). 게다가 그중 하나는 인계문이 이미 **부록 등재**로 지시한 항목이었다.

| 옮긴 것 | 이전(위반) | 이후 |
|---|---|---|
| 두-상 종의 이산화 3 함정(격자합 빗살·갭 항 빈 배치·조성 표본 간격) | Ch3 §3.2b 본문 warnbox | **Ch3 코드 절 §3.5 `ssec:code-twophase`**(요구명세) |
| 실측 dQ/dV 생성 규약(단일 창 차분 금지·다중 창 중앙값·평활 한정·균일 재빈닝) | Ch1 §18 본문 warnbox | **Ch1 부록 B `ssec:code-dqdv-numeric`** |

**본문에 남긴 것 = 물리식과 그 전제뿐**:
- `eq:sifr-twophase`(두-상 정칙용액 dQ/dV 닫힌형)과 그 성질(면적 보존·단일상 연속 접속·병진불변에 의한
  분해의 항등성) — 정칙용액 열역학이 정하는 사실이므로 채택 여부와 무관하게 본문 자산이다.
- §18 면적 보존 진단 $\Delta_Q$ — 본 문건 중심식(전하 보존)의 직접 적용이다.
- §18 (나)는 "계측 전위 양자화 탓에 미분 발산이 하필 두-상 평탄에서 생긴다"는 **전제 사실 1문단**만 남기고,
  절차는 부록으로 위임했다.
- §5b 의 $\Omega$ marginal 두-상$\cdot$최저 전위 평탄의 분해 한계 — 판정자 `eq:gr2l-disc` 의 실측 대응(물리).

**검증**: 본문(부록·코드 절 제외) 전 `.tex` 에서 구현 어휘(격자합·재빈닝·평활 함수명·빈 배치·변환 기법) **0 건**,
코드 토큰 **0 건**, `STRUCTURE_CHECK: PASS`.

## U9. 확정 구성의 코드 반영 완결 + 검산

**추가**: `SI_MSMR7_SKEW_LIT`(Si 7-gallery skew, 실측). 종전 `SI_MSMR7_LIT`(대칭·문헌 anchor)은 **무변경 보존**.
→ 확정 구성 **블렌드 14 = `GRAPHITE_MSMR7_LIT`(7) + `SI_MSMR7_SKEW_LIT`(7)** 이 이제 코드로 성립한다.
(직전까지는 Si 쪽에 α 가 없어 블렌드가 **흑연만 skew** 인 반쪽 상태였다.)
짝 배경: 흑연 창 ≈ 0.550 · Si 창 ≈ 0.051. Si 의 α 중 1 개가 상한 포화 — 흑연과 같은 단서 적용.

**중요 — 기본 경로는 여전히 4 전이 대칭이다.** `GRAPHITE_STAGING_LIT`(4 전이, α 없음)이 default 이고
7-gallery skew 셋 둘은 **전부 opt-in** 이다. 이는 골든 bit-exact 계약(G1) 보존을 위한 의도된 설계이며,
확정 구성으로 계산하려면 두 상수를 **명시적으로 넘겨야** 한다.

**식~`eq:sifr-twophase` 수치 검산(본 리비전에서 실행)**
| 주장 | 결과 |
|---|---|
| 면적 $=Q$ ($\Omega$·$\alpha$ 무관) | Ω/RT $=0\!\sim\!8$ × α $=1,4,8$ 전 조합에서 $\int=1.000000\!\sim\!1.000165$ — 오차는 유한 창·표본 몫 |
| $\Omega\!\to\!2RT$ 연속 접속 | $\varepsilon=10^{-3}\!\to\!10^{-5}$ 로 줄일 때 $\max|\Delta|$ $5.8\!\times\!10^{-2}\!\to\!5.5\!\times\!10^{-4}$ — 연속 확인. 단 갭 무게가 $\sqrt{\varepsilon}$ 로 닫혀 **접근은 매끄럽지 않다**(도함수 발산) |
| $\Omega<2RT$ 에서 갭 항 항등 $0$ | $x_a=0.5$ 정확 → 갭 무게 $0$, 단일상 가지만 남음 |

**용어 충돌 주의(미해소)**: §5b 는 $w_\eff$ 가 두-상 극한으로 ``\emph{연속 보간이 아니다}'' 라고 적고,
본 식의 서술은 ``연속 접속'' 이라 적는다. 둘은 **다른 대상**이다 --- 전자는 broadening 이전 평형 등온선의
성격 변화, 후자는 broadening 을 얹은 $\dd Q/\dd V$ 의 $\Omega$ 의존성. 모순은 아니나 **표현이 부딪히므로
한 문장 각주로 구분해 두는 것이 남은 정리 항목**이다.

## U10. 기본/opt-in 역전 + 표현 충돌 각주 (사용자 확정 2026-07-27 · 최종)

### (1) 곡선 표현의 기준선을 7-gallery skew 로 이동

v1.0.25 까지 기본 = 흑연 4 전이 대칭, 7-gallery skew = opt-in 이었다. **역전한다.**

- 신설: `DEFAULT_GRAPHITE_TRANSITIONS`(= `GRAPHITE_MSMR7_LIT`) · `DEFAULT_SI_TRANSITIONS`(= `SI_MSMR7_SKEW_LIT`)
  · `DEFAULT_CBG_GRAPHITE`(0.550) · `DEFAULT_CBG_SI`(0.051). 전이 셋 해석 지점이 이 포인터를 탄다.
- 신설 스위치 `use_legacy_4transition(enable)` — `True` 면 v1.0.25 거동(4 전이 대칭 · Si 케이스셋)으로 복귀,
  `False` 면 확정 구성. **명시 인자는 스위치와 무관하게 항상 우선**한다.
- **골든 bit-exact 계약(G1) 처리**: 그 계약은 4 전이 기준이므로 `test_gates_v1024.py` 의 모듈 로더가
  로드 직후 `use_legacy_4transition(True)` 를 호출하도록 했다. 곧 **골든 기준 자체는 무변경**이고,
  바뀐 것은 기본 진입 경로뿐이다.
- **상 수는 불변.** 바뀐 것은 곡선 표현의 기본 해상도이며, XRD 상 수(흑연 staging 4)는 그대로다.
  표~`tab:staging` 의 4-전이는 **상 수와 1:1 대응하는 물리 기준**으로 계속 유효하다 --- 두 축이 갈린 것이지
  하나가 다른 하나를 대체한 것이 아니다(§5b·§18 keybox 반영).

**게이트 전건 GREEN(본 리비전에서 실행)**: `test_gates_v1024` R6-G1/G2/G3 + coverage PASS ·
`test_gates_v1025` 9/9 · `reflect` 4/4 · `selfconsistent` 5/5.
경로 검증: 기본 = 7 전이(α 보유) · 레거시 전환 = 4 전이 · 복귀 시 곡선 bit-exact 재현 · NaN/Inf 0.

### (2) 연속/불연속 표현 충돌 해소 (U9 잔여 항목)

`eq:sifr-twophase` 서술의 "연속 접속" 에 각주를 달아 §5b 의 "연속 보간이 아니다" 와 **대상이 다름**을 명시했다:
전자는 broadening **이후** dQ/dV 의 Ω 의존성(값이 이어짐), 후자는 broadening **이전** 평형 등온선의
성격 변화(불연속). 덧붙여 갭 무게가 문턱에서 $\sqrt{\Omega/2RT-1}$ 로 닫히므로
**값은 이어지되 기울기는 이어지지 않는다**(도함수 발산)는 점을 수치 확인과 함께 적었다.

### 잔여

**XeLaTeX 3-pass 만 남았다** --- 본 환경에 TeX 배포판이 없다. 신규 요소 = `eq:sifr-twophase`(식 1) ·
각주 1 · `\subsection` 2(`ssec:code-twophase` · `ssec:code-dqdv-numeric`) · `enumerate` 3.

## U11. Codex 과학감사(`codex-local-audit-20260720`) 대조 — 상호 검토

> 브랜치 `origin/codex-local-audit-20260720`(2026-07-26 푸시) · 대상 = v1.0.10–v1.0.23 · 문서 =
> `Codex/results/PHASE_010_STRENGTHENING_ROADMAP.md`. 그 P0 목록을 **v1.0.25.2 코드에 직접 실행해** 대조했다.

### (1) ★본 리비전의 오류 — 기본값 역전이 $\partial w/\partial T$ 를 소거했다

| 경로 | 폭 키 | $\partial w/\partial T$ |
|---|---|---|
| 구 기본 `GRAPHITE_STAGING_LIT` | `'n'` 보유 | $R/F = 8.617\times10^{-5}$ V/K |
| **신 기본 `GRAPHITE_MSMR7_LIT`** | **`'w'` 단독** | **0** |

U10 의 기본값 역전으로 **기본 경로의 폭이 T-동결**이 되었고, 그 결과 **Ch2 가역 발열의 config 항이
기본 경로에서 소거**된다. U10 은 이 파급을 적지 않았다 --- **누락이다.**
게이트가 통과한 것은 골든 하네스가 로드 직후 레거시를 복원하기 때문이며, 결함을 가린 것이 아니라
**검사 대상이 아니었던 것**이다.

**판단**: 두-상 near-delta 폭은 열 척도가 아니라 현상학이 정하므로(§5 이중지위 둘째 지위)
T-동결 자체는 **정당한 선택**이다. 문제는 그것이 **묵시적**이었다는 점이다.
→ 두 시드 상수 주석에 명시하고, 열 척도 거동 복원 경로 두 가지를 함께 적었다
(`use_legacy_4transition(True)` / 전이별 `'n'` 추가 --- $n_j=w_jF/(R\cdot298.15)$, 298.15 K 에서 폭 동일).
**Ch2 가역 발열을 기본 경로로 계산하려는 사용자는 이 선택을 의식적으로 해야 한다.**

### (2) Codex 지적이 맞고 v1.0.25.2 에도 남아 있는 것 (실행 확인)

- **P0-6(a)** `'n'`$\cdot$`'w'` **둘 다 부재** 경로: 폭은 $RT/F$($n{=}1$ 취급 $=25.691$ mV)인데
  $\partial w/\partial T$ 는 **0** 을 반환한다. $n{=}1$ 폭이면 $R/F$ 여야 한다 --- **불일치 확인. 미수정.**
  (현 프리셋은 전부 `n` 또는 `w` 를 가지므로 사용자 제공 dict 에서만 발현되는 잠재 결함이다.)
- **P0-6(c)** `n`$\cdot$`w` **동시 보유 프리셋**에서 `n` 이 조용히 이기고 표시된 `w` 는 **불활성**:
  `GRAPHITE_STAGING_LIT` **4/4** · `LCO_MSMR_LIT` **3/3** --- **확인. 미수정.**
  (본 리비전의 신규 시드 2종은 `w` 단독이라 해당 없음.)

### (3) 상충 없음으로 확인된 것

- **P0-7**(양의 $\Omega$ 단일조성 정칙용액을 *ordering* 모델로 서술하지 말 것): 본 리비전이 §5b 에 추가한
  서술은 $\Omega/RT\,1.9$–$2.6$ 을 **marginal 두-상(demixing)** 으로 적었으므로 **상충하지 않는다.**
  다만 앵커로 병기한 2-sublattice 면내 항 $\Omega_a\approx3.4RT$ 는 **모델 계열이 다른 양**이다 ---
  기존 `srcbox` 가 "단일 $\Omega$ 는 면내 응축 sharpness 만 포착" 이라 적어 대응 자체는 정당하나,
  **한 문장 단서를 붙이는 것이 남은 정리 항목**이다.
- **P0-1**(시간 기저 $h^{-1}$ vs $s^{-1}$, 3600 인자): 코드가 이미 **흡수 규약으로 명시**하고 우회로
  (`Q_cell` 을 쿨롱으로 지정)까지 적어 두었다 --- 침묵 오류가 아니다. 다만 Codex 가 요구한
  **불변성 테스트는 여전히 부재**이므로 권고는 유효하다.

### (4) 감사의 적용 범위 주의

본 감사는 **v1.0.10–v1.0.23** 대상이며 v1.0.24/25/25.2 이후 변경을 반영하지 않는다. 위 (2) 두 건은
**본 리비전에서 직접 실행해 잔존을 확인한 것**이고, 나머지 P0 항목(P0-2~P0-5)은 대조하지 않았다 ---
**미검토**로 남긴다.

## U12. ★자기 감사 — U10 기본값 역전 철회 (사용자 지적 2026-07-27)

**사용자 지적**: "너 너무 대충 일했어." → 재감사 결과 **U10 은 반려되어야 한다.**

### 무엇이 잘못됐나

7-gallery skew 시드는 `U`$\cdot$`w`$\cdot$`Q`$\cdot$`alpha` **네 키뿐**이고, 레거시가 가진
`dH_rxn`$\cdot$`dS_rxn`(중심 $U_j(T)$) · `n`(폭 $w(T)$) · `Omega`(히스테리시스) ·
`dH_a`$\cdot$`dS_a`$\cdot$`dVdq_qa`(동역학 지연)를 **전부 결여**한다. 그것을 기본으로 세웠으므로
**기본 경로의 온도 의존이 통째로 사라졌다**.

| 기본 셋 | 288→308 K, $\max|\Delta|$ |
|---|---|
| 4-전이(레거시) | **0.5252** |
| 7-gallery skew(U10) | **0.0000** |

기여 분해(레거시 기준): 중심 몫 **1.135** · 폭 몫 **0.3447**(부분 상쇄되어 합 0.5252) --- **두 몫 모두 0** 이 됐다.
**ICA/DVA 를 열역학$\cdot$동역학으로 설명한다는 본 프로젝트의 목적(P1)에 정면으로 반한다.**
U11 이 보고한 $\partial w/\partial T$ 소거는 **이 문제의 일부**였고, 더 큰 몫인 중심 $U_j(T)$ 소실을 놓쳤다.

### 왜 게이트가 잡지 못했나

골든 하네스가 로드 직후 레거시를 복원하므로 **검사 대상이 아니었다.** 게이트 GREEN 이 정당성의 증거가
아니었던 사례다 --- 기본 경로를 바꿨으면 **기본 경로를 재는 게이트**가 있어야 했다.

### 조치 (되돌림)

- 기본 셋을 **열역학 입력을 갖춘 레거시 셋으로 환원**했다(`DEFAULT_GRAPHITE_TRANSITIONS = GRAPHITE_STAGING_LIT`).
- 스위치 방향을 뒤집어 `use_skew7_default(enable)` 로 개명 --- **7-gallery skew 를 켜는** 스위치다.
  경고를 docstring 에 명시: 정온 곡선 적합 전용, 가역 발열$\cdot$다온도 해석에 쓰지 말 것.
- 문건 §5b$\cdot$§18 을 같은 취지로 정정: **전이 세분과 비대칭은 곡선 표현의 개선이지 열역학 입력의 대체가 아니다.**
- 게이트의 레거시 복원 호출은 방어적으로 유지(호출자가 스위치를 켠 상태여도 골든 계약 불변).

### 남은 결정 (사용자 몫 --- 이것이 풀리면 기본을 되뒤집을 수 있다)

7-gallery 시드에 `dH_rxn`$\cdot$`dS_rxn`$\cdot$`n` 을 채우면 경고가 해소된다. 산술은 확정적이다 ---
$n_j=w_jF/(RT_0)$ 이고, 적합된 $U_j$ 를 보존하려면 $\Delta H_{\rxn,j}=-FU_j+T_0\Delta S_{\rxn,j}$ 이다
(검산: $U_j(T_0)$ 재현 오차 $<5$ nV). **미결은 하나뿐이다 --- 각 gallery 군집이 어느 staging 전이에 속하는가**
(그 전이의 $\Delta S_\rxn$ 을 상속). 최근접-$U$ 자동 배정은 **같은 군집을 두 모(母)로 쪼개므로 쓸 수 없다**
(103 mV 군집 4 개가 $\Delta S=-16$ 과 $-5$ 로 갈렸다). 이는 물리 배정이므로 추측하지 않고 남긴다.

### 이 감사가 드러낸 작업 결함 (기록)

1. 기본값을 바꾸면서 **새 기본이 모델의 핵심 결합을 보존하는지 확인하지 않았다.**
2. `N10`(배경) 진단을 **검증 전에 결론으로 기술**했고, 재실측에서 부분 오류로 판명됐다(U6).
3. 인계문이 부록 등재로 지시한 항목을 **예산을 이유로 본문에 넣었다**(U8 에서 이관).
4. 신규 식의 수치 검산을 **문건 반영 후에야** 수행했다(U9).

## U13. ★문건 오류 정정 — 문턱 근방 스케일링 (Codex 교차검증 `11f9054`)

Codex 브랜치 `codex/v1025_2-physics-conformance`(tip `11f9054`)가 v1.0.25.2 최신(`3b5fd05`)을 머지하고
`phase054_regsol_crosscheck.py` 로 `eq:sifr-twophase` 를 독립 재계산했다. **본 문건의 각주 하나가 틀렸다.**

| 주장 | 교차검증 | 판정 |
|---|---|---|
| 창 내 면적 $=Q$ | 오차 $7.8\times10^{-16}$ | ✅ **확인**(본 세션 자체 검산 $10^{-4}$보다 훨씬 엄격) |
| $\Omega<2RT$ 에서 갭 무게 $0$ | 정확히 $0.0$ | ✅ 확인 |
| **문턱 접근이 매끄럽지 않다($\Omega$-도함수 발산)** | $\max|\Delta|/\varepsilon = 5.90$ 이 $\varepsilon=10^{-3}$--$10^{-5}$ 에서 **불변** $\Rightarrow$ 차이는 $\varepsilon$ 에 **선형** | ❌ **오류** |

**무엇을 틀렸나.** 공존 조성 $\theta_a$ 가 문턱에서 제곱근으로 열리는 것은 맞고, 따라서 갭 무게
$(1-2\theta_a)\sim\sqrt\varepsilon$ 도 맞다. 그러나 **같은 순간 고용체 두 가지가 같은 무게를 잃는다** ---
두 항의 선행 차수가 상쇄되어 곡선 차이는 $O(\varepsilon)$ 이다. 곧 $\dd Q/\dd V$ 는 $\Omega$ 에 대해
**값과 1차 도함수가 모두 연속**이다. **갭 무게 한 항만 보고 전체 스케일링을 읽은 것이 오류의 원인이다.**

**본 세션 자체 검산에도 이미 답이 있었다.** U9 표의 $\max|\Delta|$ = $5.764\times10^{-2}$ / $5.698\times10^{-3}$ /
$5.488\times10^{-4}$ 는 $\varepsilon$ 로 나누면 $57.6/57.0/54.9$ 로 **불변**이다 --- 선형 스케일링이 그대로
드러나 있었는데, 옆 열의 갭 무게($\sqrt\varepsilon$)를 곡선 차이에 잘못 전이해 읽었다.
**수치를 냈지만 읽지 않은 사례이며, §4 개선 목록의 "확인 가능한 것을 확인하지 않고 진행" 과 같은 뿌리다.**

**조치**: `ch3v22_sec02b_sifr.tex` 의 `eq:sifr-twophase` 각주를 정정 --- 상쇄 기전과 선형 스케일링을 명기하고,
교차검증 수치($5.90$ 불변 · 면적 오차 $8\times10^{-16}$ · 아임계 갭 무게 $0$)를 병기했다.
**Codex 지적을 전면 수용한다.**
