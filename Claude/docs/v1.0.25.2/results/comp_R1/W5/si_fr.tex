...
$\Omega$/$\omega$ 정규용액 형식\cite{msmr_origin2017}). 이 Si-host 커널은 혼합 음극 합성식~\eqref{eq:blend-dqdv} 의
Si 이중합 몫에 그대로 꽂힌다 --- 식~\eqref{eq:blend-dqdv} 의 Si host 종
$\xi_{\eq,j}^{\mathrm{Si}}(1-\xi_{\eq,j}^{\mathrm{Si}})/w_j^{\mathrm{Si}}$ 를 식~\eqref{eq:si-fr-kernel} 의 Frumkin
종으로 바꾸고 흑연 몫은 로지스틱 종을 유지한다(식 자체는 \eqref{eq:blend-dqdv} 재정의 없이 host 커널 선택만
Si 에서 \code{regsol} 로 분기 --- 미지정 시 로지스틱 폴백 bit-exact). 두 host 가 \emph{같은 전위 손잡이}(공통
$\mu$)를 공유하므로 관측 dQ/dV 는 흑연 종과 Si Frumkin 종의 \emph{같은 전위축 위 선형 합}이다. 블렌드 실측이
두 host 기여의 중첩으로 읽힌다는 것
...
(1차 리튬화 $\sim$50--60 mV, 순환 a-Si 부재). Si-host 는 소수 broad gallery 로 이산화되어 공통-$\mu$
합성~\eqref{eq:blend-dqdv} 의 Si 몫으로 가산 중첩된다\cite{tu_blend2024}.
\end{keybox}
...
%   [W5-SIFR-06 소수 broad gallery 이산화·공통-μ 가산중첩(eq:blend-dqdv Si몫 커널 교체·regsol 분기·폴백 bit-exact)·tu2024 superposition]
