...
$0<\Omega_j<2RT$ 에서는 평형 등온선의 중심 기울기가 $(1-\Omega_j/2RT)$ 배 좁아진 유효 폭을 주되(이 유효
폭은 중심 \emph{높이} 척도이지 반높이 폭이 아니다 --- FWHM 은 $\lambda^{3/2}$ 스케일로 갈린다, \S\ref{ssec:gr-2L}) 그
...
\textbf{폭과 형상 지수의 분업(v1.0.25 --- opt-in).} 위 이중지위는 폭 항이 식~\eqref{eq:wbase} \emph{하나}라는
전제 위에 서 있고, 그 전제는 v1.0.25 에서도 불변이다. 다만 \S\ref{sec:eqpeak} 가 선택 확장으로 두는 형상
지수 $\alpha_j$(식~\eqref{eq:skewpeak})는 폭식을 건드리지 않고 \emph{종의 좌우 비대칭}을 담당한다 --- 곧
비대칭은 폭 항의 변형이 아니라 진행률 좌표의 재모수화 $\xi_\eq\!\to\![\xi_\eq]^{\alpha_j}$ 에서 온다.
단 $\alpha_j$ 는 비대칭만 바꾸지 않고 정점 위치$\cdot$반높이 폭까지 함께 옮기므로(식~\eqref{eq:skewapex}),
$\alpha_j$ 와 $n_j$(곧 $w_j$)를 같은 곡선에서 동시에 자유화하면 축퇴한다 --- 식별 가드는
\S\ref{sec:inputs} 다. $\alpha_j=1$(기본)에서 이 소절의 모든 서술은 문자 그대로 성립한다.
