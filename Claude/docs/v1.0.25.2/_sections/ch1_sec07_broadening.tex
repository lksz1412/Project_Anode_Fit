...
% ====================================================================
\section{두-상 broadening --- 평형 델타의 실측 종 전환과 폭 $w_j$ 의 지위}\label{sec:broadening}
...
\subsection{둘째 부류 델타의 종 전환 --- broadening 의 세 출처}\label{sec:broadening-sources}
...
\emph{가정}이다 --- tier C 추정이며(신뢰 등급 범례\footnote{신뢰 등급(본서 전역): tier A $=$ 1차 문헌의 정량 확정값, tier B $=$ 대표/부분 스케일 anchor 또는 2차 출처 경유 확인값(1차 원문 직접 확정 아님), tier C $=$ 추정$\cdot$placeholder(round-trip 피팅으로 갱신 예정). Chapter 2(\S\ref{sec:lco-map} 각주)$\cdot$Chapter 3 의 같은 규약과 동일하다.}), LCO 단독 GITT 가 입자
...
gap(식~\eqref{eq:dUhys})은 같은 Dreyer 다입자 열역학\cite{dreyer2010,dreyer2011} 의 두 측면이다. 대응은
...
아님), 다입자 앙상블 평형의 표준 그림\cite{dreyer2010,dreyer2011} 은 (iii-a) 평탄역까지이고 --- 그 골격 밖에 얹히는 (iii-b) 입자간 $\eta$ 산포가 여기서 이
...
\textbf{(추가 축) 평형 비대칭 $\alpha_j$ --- ① 과 무엇이 다른가(v1.0.25, opt-in).} 위 세 출처 가운데 종을
\emph{비대칭}하게 만드는 것은 ① 뿐이다 --- ② 의 내재 $RT/F$ 폭과 ③ 의 $\eta$ 산포 합성은 좌우 대칭 커널이고,
폭 예산(식~\eqref{eq:widthbudget})의 피타고라스 합성도 그 대칭을 전제로 선다. v1.0.25 는 여기에 선택 확장으로
\emph{네 번째 축}을 둔다: 평형 종 자체의 비대칭 지수 $\alpha_j$(식~\eqref{eq:skewpeak}). 이것은 ①②③ 의
번호를 바꾸지 않는 별도 축이며, ① 과의 구분이 그 지위를 정한다.
\emph{성격} --- ① 의 비대칭 꼬리는 \emph{전류가 켜야} 생겨 $|I|\!\to\!0$ 에서 소멸한다($L_V\propto|I|$).
$\alpha_j$ 는 전류$\cdot$과전압과 무관한 \emph{평형 잔여} 비대칭이라 $|I|\!\to\!0$ 에서도 남는다 --- 곧 ② 와
같은 부류(평형 잔여)이면서 ② 와 달리 좌우를 가른다.
\emph{교란} --- 둘은 관측 지표까지 겹친다. 그림~\ref{fig:widthbudget} 의 ① 은 종을 오른쪽으로 밀어 정점을
중심에서 이동시키는데, $\alpha_j>1$ 도 같은 방향으로 $\sigma_d w_j\ln\alpha_j$ 만큼 정점을
민다(식~\eqref{eq:skewapex}). 그러므로 단일 온도$\cdot$단일 율속 곡선 하나에서 $L_{V,j}$ 와 $\alpha_j$ 를
동시에 자유화하면 같은 비대칭을 두 번 세는 축퇴가 된다 --- 식별 가드는 \S\ref{sec:inputs} 다.
\emph{분리} --- 두 몫을 가르는 것은 곡선 하나의 모양이 아니라 \emph{율속 스윕}이다: ① 만 $|I|$ 에 비례해
자라고 $|I|\!\to\!0$ 에서 사라지며, $\alpha_j$ 가 담은 비대칭은 율속을 낮춰도 그대로 남는다. 이 분리는
다율속 데이터가 있어야 성립하므로, 그것이 없는 상태에서 $\alpha_j$ 로 얻은 비대칭을 ``평형 비대칭의
측정''으로 읽어서는 안 된다(현상학적$\cdot$tier C --- \S\ref{sec:eqpeak}). $\alpha_j=1$(기본)에서 이 소절의
세 출처 서술과 폭 예산은 문자 그대로 성립한다.

\subsection{broadening 유도의 경계}\label{sec:broadening-scope}
...
고주파 성분을 지수적으로 눌러 놓아 역산이 그 눌린 성분을 복원하는 과정에서 데이터의 작은 잡음을 크게
...
\item \textbf{입자 \emph{크기 분포}(PSD) 위로 합성곱하는 다입자 처리는 모델에 넣지 않는다.} 모델은
...
$\eta$ 산포로만, 본문 ③). ②$\otimes$③ 을 한꺼번에 흡수하는 것이 \emph{현상학적 자유 피팅 폭} $w_j$ 이고, ① 은 별도 축 $L_{V,j}$ 가 담당한다(합치면 이중계산 --- 식~\eqref{eq:widthbudget}) ---
