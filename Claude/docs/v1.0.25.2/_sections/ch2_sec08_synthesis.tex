...
\begin{equation*}
U_\oc(\bar x{=}0.25,\,298.15\,\mathrm K)=74.4\ \mathrm{mV}\quad(\text{raw }74.35\ \mathrm{mV}).
\end{equation*}
\emph{표시 각주(v1.0.25 --- 상수 opt-in).} 이 근의 raw 값은 레거시 상수($R{=}8.314\cdot F{=}96485.0$, 구현
기본)에서 $74.3511$ mV 이고 CODATA-2018 상수(\code{use\_si\_constants()} --- opt-in)에서 $74.3497$ mV 다.
차이는 $-1.4$ $\mu$V, 곧 $2\times10^{-5}$ 상대차로 \emph{물리적으로 무의미}하나, 하필 $74.35$ 라는 반올림
절벽에 걸려 소수 첫째 자리 표시가 $74.4\to74.3$ 으로 뒤집힌다. 그래서 본 예제는 표시값과 raw 값을 함께 적는다
--- 같은 이유로 아래 표~\ref{tab:qrev} 의 $U_\oc$ 열도 표시 자리를 넘는 정합 판정에 쓰지 말 것. 나머지 회귀
기준값($\partial U_\oc/\partial T=-0.204/-0.134/-0.070$ mV/K, $\dot Q_\rev/I=+60.8$ mV)은 두 상수계에서
표시 자리까지 \emph{동일}하다(신규 게이트 G-SI 증빙).
...
이 결과가 실전 계산의 도착점이며, 남은 것은 그 입력을 실측에서 얻는 방법과 적용 한계다.
