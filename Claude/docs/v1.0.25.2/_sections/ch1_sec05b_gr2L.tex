...
문헌 anchor)로 병존한다 --- 곧 해상도 사다리는 \{4-전이$\cdot$5-feature XRD$\cdot$6-gallery MSMR
$\cdot$7-gallery skew\} 이고, \textbf{v1.0.25.2 에서 \emph{정온 곡선 적합}의 최량 표현은 사다리 맨 위 7-gallery skew 다}(아래).
다만 \emph{기본 셋은 4-전이로 유지}한다 --- 7-gallery 시드는 곡선 형상만 담고 반응 엔탈피$\cdot$엔트로피와
폭 다중도를 갖지 않아 \emph{중심$\cdot$폭의 온도 의존이 없기} 때문이다. 곧 전이 세분과 비대칭은
\emph{곡선 표현}의 개선이지 \emph{열역학 입력}의 대체가 아니다 --- 다온도 해석$\cdot$가역 발열은
열역학 입력을 갖춘 4-전이 기준을 쓴다.
\emph{바뀐 것은 곡선 표현의 기본 해상도뿐이며 상(phase) 수는 불변이다}(\S\ref{sec:broadening-class} 위임).

\textbf{사다리의 맨 위 --- 흑연 7-gallery skew 가 v1.0.25.2 의 기준선이다.} 전이 수를 3 부터 8 까지 올리며
...
pOCV(완화 없음) 위에서 얻었고, 시드 파라미터는 피팅 override 전제의 tier-C 값이다. 기준선을 옮긴 근거는
\emph{적합}이지 \emph{상 판정}이 아니므로, 표~\ref{tab:staging} 의 4-전이는 XRD 상 수와 1:1 대응하는
\emph{물리 기준}으로서 그대로 유효하다 --- 두 축이 갈린 것이지 하나가 다른 하나를 대체한 것이 아니다.

\textbf{판정자 $\Omega\lessgtr2RT$ 의 실측 대응 --- 흑연은 두-상이되 \emph{marginal} 이다(v1.0.25.2).}
같은 흑연 반쪽셀을 정칙용액 커널의 물리 4 전이로 적합하면 전이별 $\Omega/RT$ 가 $1.9$--$2.6$ 구간에
