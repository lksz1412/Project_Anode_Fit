...
갭 항이 사라져 단일상 형태로 연속 접속한다. $\Omega\!<\!2RT$ 에서는 갭 항이 항등적으로 0 이므로
...
\emph{구현 주의(v1.0.25.2) --- 명세를 곧이곧대로 격자합으로 옮기면 틀린다.} (1) 둘째 항을 조성격자에 대한
단순 합으로 계산하면, 폭 $w_j$ 가 이웃 격자점의 전위 간격보다 좁을 때 곡선이 매끄러운 종이 아니라 빗살로
깨진다 --- 흑연 최급준 전이의 관측 폭이 정확히 그 영역이다. 격자를 키우는 것이 아니라 위처럼 \emph{적분을
분해}하는 것이 해법이다. (2) 첫째 항(갭)은 반드시 닫힌 형태로 더한다 --- 이를 전위 빈에 넣으면 중심
$U^\circ_j$ 가 빈 폭의 절반까지 밀려 좁은 $w_j$ 에서 정점이 크게 틀어진다. (3) 희박 극한에서
$U\!\sim\!-(RT/F)\ln\theta$ 로 발산하므로 조성 표본을 등간격으로 잡으면 꼬리에서 전위 간격이 벌어져 같은
빗살이 생긴다 --- 기하 간격이어야 전위축에서 등간격이 된다. \emph{이 세 함정은 정칙용액 계열에서만 나타난다}
--- 로지스틱 계열은 해석함수 하나라 애초에 격자를 쓰지 않는다. 곧 과거 이 커널에서만 관측된 꼬리 위글은
물리가 아니라 이산화의 산물이었다.

\emph{지위.} 식~\eqref{eq:sifr-twophase} 은 \emph{이론 층}의 자산이다 --- v1.0.25 계열의 피팅 커널 계통은
로지스틱 단일계이며(위 지위 상자), 본 식에 대응하는 구현 경로는 두지 않는다. 재구현하려면 새 게이트가 필요하다.
\end{warnbox}
