# -*- coding: utf-8 -*-
# ============================================================================
# ★ Anode_Fit  release 버전 = 1.0.20  — 문건 Ch1/Ch2 1.0.20 와 동일 버전·matched
#   (구현 계보: v11_final → use_w_eff 제거 → 1.0.10 → 1.0.12 → 1.0.13 → 1.0.14 → 1.0.15 → 1.0.16 → 1.0.17 → 1.0.18.1 → 1.0.18.2 → 1.0.19 → 1.0.20. 코드·문건 버전 통일.
...
#    1.0.20 = 문건 v1.0.20 부록 B(doc-leads 요구명세) 정합 이월 — B.1 x̄ 진입점 3종·B.3 θ_E 하위호환·ch1 부록 B n_j(T)=n_j+n_T1(T−T_ref) 선형 잔여 전부 v1.0.19 승계, 물리·수치 경로 무변경. 회귀 기준 B.2(74.4 mV·−0.204/−0.134/−0.070 mV/K·round-trip·5점 표·파생 A 175점)·G1 하위호환·G3 θ_E bit-exact 는 test_gates_v1020.py 로 전건 수치 재증빙.)
# ----------------------------------------------------------------------------
# Anode_Fit 흑연 음극 dQ/dV 물리 구현 — 1.0.20 (v11_final 기반 계보, 2026-07-16)
...
#
#   [근거 문건] graphite_ica_ch1_v1.0.20.tex(+ch2) — 커브식 명세(계보 원천 Opus_v6). 본 구현이 따르는 식:
