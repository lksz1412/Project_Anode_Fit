% ====================================================================
% appendix_phase_separation.tex — Appendix (초안; 최초 작성 v1.0.14 계획 Step 7 — 이력 스탬프, 버전업 시 보존)
%   v1.0.20 승계 — 물리 내용 미개정(각주 버전 갱신 + P6 표기 통일 8건[정규용액·격자기체] + [A5] 장 귀속 정정 C-019 + §A.6 단수 자기서술 정정)
%   (Part 0=§2 · 히스 gap=§4.2 · spinodal 근=§4.1 · broadening=§7).
...
\title{\textbf{\LARGE Appendix (초안)}\\[0.35em]\Large 상분리의 열역학 --- 혼합 자유에너지, binodal 과 spinodal,\\ Maxwell construction, 핵생성과 spinodal 분해}
\author{}
\date{\normalsize 버전 1.0.18.2 초안\thanks{v1.0.20 승계 --- 본 부록은 v1.0.20 에서 내용 미개정으로 승계된다(절번호 참조는 v1.0.19 골격과 동일).} \quad(본문 편입 여부는 검토 후 결정)}

\begin{document}
\maketitle

본문 Part 0(\S2)은 정규용액(regular solution)의 자유에너지가 $\Omega_j>2RT$ 에서 볼록성을 잃는다는
문턱식을 제시하고, \S4.2 는 그 이중웰 위에서 히스테리시스 gap 의 spinodal 상한을 닫는다. 두 논의는
...
구분한다(\S\ref{app:kinetics}). 전개의 표준적 원천은 상전이 교과서 [A3]·[A4] 와 재료 동역학(kinetics) 교과서
...
\S4.2 가 그 부호 처리를 우함수 대칭 논거로 닫고 있으므로, 본 부록의 $\mu(\xi)$ 를 본문의 진행률 좌표에
...
Li 이 점유한 자리에 해당하므로, 이 두-성분 혼합 문제는 본문 Part 0 의 격자기체(lattice gas)와 같은
...
\textbf{(d) 박스 --- 정규용액 자유에너지.} 두 결과를 $f=u_\mathrm{mix}-Ts_\mathrm{mix}$ 로 묶으면
몰당 혼합 자유에너지는
\begin{equation}
...
\label{eq:app-gain}
\end{equation}
이다. 정규용액에서는 본문 Part 0 의 문턱식이 보이듯 $\Omega\le2RT$ 이면 $f''\ge0$ 이 전 구간에서
...
일치'' 와 같은 내용임을 확인한다. 전체 자유에너지 $G=N g(\xi)$, $\xi=N_B/N$, $N=N_A+N_B$\footnote{이하
$N_A,N_B$ 는 Avogadro 수와 무관한 A/B-종 자리 수이다.} 에서
\begin{equation}
...
곡선(coexistence curve), 곧 binodal 이다. 대칭 정규용액에서는 $f(\xi)=f(1-\xi)$ 이므로 접점도
...
\end{equation}
로 닫힌 꼴이 나오며, 이는 본문 \S4.1 의 spinodal 근과 동일하다. $T/T_c$ 로 적으면 spinodal 은
$T/T_c=4\xi(1-\xi)$ 의 포물선형 곡선이고, 역시 꼭대기 $(\tfrac12,\,T_c)$ 에서 binodal 과 만난다.

\textbf{(c) 분류표 --- 세 영역의 분류.} binodal 과 spinodal 은 임계점에서만 만나고 그 아래 온도에서는
binodal 이 바깥, spinodal 이 안쪽에 놓인다(그림~\ref{fig:app-phasediag} 의 상평형도가 이 포함 관계를
온도 축 전체에서 보인다). 이는 두 판정의 논리적 포함 관계의 기하적 표현이다:
...
\end{tikzpicture}
\caption{대칭 정규용액의 상평형 그림(신규 그림 --- 좌표는 식~\eqref{eq:app-binodal}·%
...
작도(equal-area construction)이다. 대칭 정규용액에서는 $\mu(\xi)$(섞임 몫)가 $\xi=\tfrac12$ 에 대해
기함수이므로 $\mu^\ast=\mu(\tfrac12)$ 로 등면적이 자동 성립하고, 이는 수평 공통접선과 같은 내용이다.

\textbf{(d) 박스 --- 삽입 전극에서의 의미.} 삽입 전극에서는 본문 Part 0(\S2)의 결선
...
델타형으로 집중된다. 본문 \S4.2 가 다루는 히스테리시스는 실제 소인(sweep)이 이 Maxwell 전위에
...
차이 때문이다. 이하 두 소절(\S\ref{app:nucleation}, \S\ref{app:spindecomp})은 다른 절의 (a)--(d) 4단
구성과 달리 내용의 성격상 2\textasciitilde3단으로 축약되어 전개된다.
...
부피는 자유에너지를 낮추지만 모상과의 계면은 계면 에너지 $\gamma$[J/m$^2$]만큼 비용을 문다. 부피당
구동력은 모상($\bar\xi$)의 접선에서 잰 신상 몰당 이득의 부피 환산
$\Delta g_v=\bigl[f(\xi_\beta)-f(\bar\xi)-f'(\bar\xi)(\xi_\beta-\bar\xi)\bigr]/v_m<0\;[\mathrm{J/m^3}]$($v_m$ =
몰부피 $[\mathrm{m^3/mol}]$)이다
--- \S\ref{app:chord} 의 계 전체 이득 $\Delta f_\mathrm{sep}$ 와는 핵의 몰분율 $\lambda$ 를 매개로
$\Delta f_\mathrm{sep}=\lambda\,v_m\,\Delta g_v$(소량 핵 극한)로 연결되며, 공통접선 조건에서 이 접선
구동력이 정확히 $0$ 이 된다. 반지름 $r$ 의 구형 핵에 대해
\begin{equation}
...
binodal(공통접선) 접점에서 위 접선 구동력이 정확히 $0$ 이라 그 근처에서
$|\Delta g_v|\to0$ 이 되고 $\Delta G^\ast\propto|\Delta g_v|^{-2}$ 가 발산하여
...
이동도). 여기서 $f$ 는 식~\eqref{eq:app-fxi} 의 몰당 양을 몰부피 $v_m$ 으로 나눈 부피 밀도 $[\mathrm{J\,m^{-3}}]$ 로 읽어 $F$ 가 총자유에너지 $[\mathrm J]$ 가 되게 하며, 그러면 차원 정합이 $[\kappa]=\mathrm{J\,m^{-1}}$, $[M]=\mathrm{m^{5}\,J^{-1}\,s^{-1}}$ 을 준다.
...
$\Omega=2RT$ 는 Part 0(\S2)의 자유에너지 및 문턱식과 동일하며, 본 부록은 그 문턱의 앞뒤 ---
왜 비볼록이면 분해가 이득인지(\S\ref{app:chord}), 분해의 종착 조성이 무엇인지(\S\ref{app:binodal})
--- 를 채운다. 둘째, spinodal 닫힌 근 식~\eqref{eq:app-spinodal} 은 \S4.2 가 히스테리시스 gap 의
...
\emph{폭}의 기원은 별개의 질문으로, 본문 \S7(broadening 절)의 소관이다. 넷째, 핵생성 대 spinodal 분해의 구분(\S\ref{app:kinetics})은 본문
범위 밖의 동역학이지만, ``spinodal 까지 과주행한다'' 는 \S4.2 의 서술이 어떤 동역학적 그림을
...
Ch.~18--19 (Cahn--Hilliard 선형화$\cdot$핵생성 이론).
\end{enumerate}
