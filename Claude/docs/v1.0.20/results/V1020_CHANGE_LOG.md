...
| B-008 | 지시정정(D1 자립성) | ch1 main:title·ch1_preamble:pdftitle·ch2_sec00:42·appendix:title/pdftitle/\date | 독자 대면 텍스트에 과거 이력 표지 잔존: 제목 꼬리 "— 유도 확장 (재작성)"·pdftitle "(전면 재작성)"·"v5 계보가 규정한 골격" 문장·appendix "(초안)"·"버전 1.0.18.2 초안" \date·승계 \thanks·"편입 여부 검토 후 결정" | **사용자 지시("이 문건은 과거 이력 무관, 이 문서만 보고 이해되는 교과서") 집행**: 이력 표지 전건 제거 — 제목 "수식-구동 전개"로 정리·v5 문장은 사슬 실체(자산 [C-1] 골격)를 보존한 재서술·appendix 표지 정리. 현재 버전 자기식별자(v1.0.20 헤더)는 유지 | 사용자 지시 2026-07-16 (G3·C2O1-02 결정 겸함) | 무 | P7+ |
| B-009 | 보강 | ch1_sec02a:bgbox(페르미온/보손)·ch1_sec15:§15.1 bgbox(MIT) | 두 배경 박스가 산문 위주 — 교환 대칭·BE/FD 분포·Mott 판별이 말로만 서술(수식-주도 헌법 대비 결손, 사용자 지적) | **박스 수식화**: ①교환 대칭 ψ(2,1)=±ψ(1,2) 신설(eq:sm-exch) ②FD·BE 분포를 본문 자체 대정준 도구(eq:sm-gc)로 1줄 유도하는 병렬식 신설(eq:sm-fdbe — Ξ_F 두 항 합·Ξ_B 기하급수) ③Mott 판별 U≳W≃2zt 신설(eq:lco-mottcrit, underbrace 주석형·z 는 기존 배위수 기호)·밴드 절연체에 g(E_F)=0 인라인 연결·"부분적으로만 차"→"반쯤 차(half-filling)" 정밀화. **전부 교과서 표준 결과 — 새 물리 주장 없음, 기존 산문 자산·가드·인용 보존** | 사용자 지적("수식만으로도 이해 가능한 수준") + mcquarrie1976·mott1968·imada1998(기존 인용 유지)·master 재유도 검산(⟨n⟩=k_BT∂lnΞ/∂μ 두 분포 재현) | 무 | P7+ |
