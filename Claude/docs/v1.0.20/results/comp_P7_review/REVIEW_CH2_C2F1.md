...
## 특별 축 총괄 — Ch2 "Chapter 1 §N" 리터럴 참조 전건 대조표

grep 전수 추출(26개소) 기준, 지시된 11개 절 번호 전건 + §9 포함 검증 완료:

| §N | Ch2 출현 위치 | Ch1 대조 결과 |
|---|---|---|
| §1 | sec01:59(σ_d 각주)·sec09:16(V_n 분극) | ch1_sec01 σ_d 정의(11–14)·eq:vn(179) 문자 일치 ✓ |
| §2 | sec01:13·42·47·113, sec03:9(bgbox), sec05:39(g_j(ξ)) | ch1_sec02a/b — Ξ₁ 통일 선언(215–217)·ε̃ 정의(232)·s_int(277–280)·bgbox(171–188)·μ(θ)(§2.4)·섞임 엔트로피(§2.3) 전건 일치 ✓ |
| §3 | sec00:8 | eq:Uj = (−ΔH+TΔS)/F (ch1_sec03:53) 문자 일치 ✓ |
| §4 | sec01:133(spinodal)·sec04:21(u_j)·sec05:196(분기 중심식)·appA:51 | Ω>2RT 문턱(35–38)·u_j 식(35)·eq:Ubranch γ_jh_η,j(130) 전건 일치 ✓ |
| §5 | sec01:77·sec08:24 | ξ_eq logistic(ch1_sec05)·σ_d→s=+1 관계(ch1_sec01:29) 일치 ✓ |
| §7 | sec05:177·sec10:13 | 두-상 broadening·세 출처(TOC 1.7.2) 일치 ✓ |
| §8·§9 | sec03:89 | Eyring·ΔS_a(ch1_sec08:89–91) 일치 ✓ |
| §13 | sec03:76 | LCO order–disorder/MIT 제목·내용 일치 ✓ |
| §14 | sec02:128 | 삼분해·슬롯 규칙·eq:lco-configsplit 일치 ✓ |
| §15 | sec00:61·sec03:64·76·appA:67 | LCO 전자 엔트로피·Sommerfeld 사슬(61–93)·eq:Se 라벨 별개 확인 ✓ |
| §17 | sec09:20 | MSMR 동형(ch1_sec17)·풀네임 원장 일치 ✓ |

부수 확인: appendix 의 본문 참조(§2·§4.1·§4.2·§7)도 전건 정합(appendix 절 참조). q(T)→ε̃ 정의 문자 일치·Ξ₁ 표기 통일·u_j 동명 별개 각주·eq:Se-ch2/eq:Se 별개 — 지시 4항목 전부 성립.

## 검수 방법 요약 (허위 방지 증빙)

- 대조: 담당 17개 파일 전부 v1.0.19 동명 파일과 `diff` — 변경 발생 파일은 5개뿐(sec02·sec03·sec05·bib·preamble + appendix), 변경 전건이 CHANGE_LOG/BASELINE 의 의도 항목(U9·U10·B-003·B-005·C-005·C-006·C-007·C-017·C-018·C-019·P6 표기 8건·버전 스탬프)과 1:1 — **미등재 물리 변경·자산 유실·문장 훼손 0**.
- 재유도: B-005 전 단계, Ch2 전 표시 수식(eq:Z1~eq:complete·appendix 전 식) 손 재유도.
- 수치: sec04 Einstein 4점·sec08 계산 예제 전체(U_oc·표 2종·가중 평균 −0.204 산술)·appendix 그림 좌표 30점 — Python 독립 재계산 전건 일치.
- % 자산 주석: 불변 보존 대상으로 취급(지적 없음) — diff 상 자산 주석 변경 0 확인.

## REVIEW COMPLETE — 발견 총 3건 (H 0 / M 0 / L 3)

| # | 위치 | 심각도 | 축 | 한줄 요약 |
|---|---|---|---|---|
| C2F1-01 | ch2_sec03_vibel.tex:16 | L | ① | 모드 자유에너지 f_k 의 영점 에너지 기준 규약 무언급(결과 불변·구본 상속) — 반구절 가드 제안 |
| C2F1-02 | appendix:397–398 | L | ① | §A.6 메타 서술 "3~4단"이 실측 2단/3단과 불일치(구본 상속) |
| C2F1-03 | CHANGE_LOG C-019 행 | L | ③ | "[A5] 2곳" 집계가 실측 3곳과 불일치(무변경 정당성 결론은 유지·문서 본문 무결) |

## 가장 약한 1곳

**ch2_sec05_mixing.tex:102–110 (srcbox 전제 명시) ↔ ch2_sec08 수치 쇼케이스의 의존 사슬.** 본 장의 대표 수치(−0.204 mV/K)에서 config 몫 −0.070(약 1/3)과 tab:qrev 경계행(x̄≈0.75, +0.044 mV/K)의 부호 자체가, 지배 두-상 전이(2L→2·2→1)의 폭이 열적 서식 w_j=n_jRT/F 로 스케일한다는 **아직 실측 미확정 전제**에 걸려 있다. 문서는 이 한정을 세 곳(sec05 srcbox·sec08 keybox 아래·tab:qrev 캡션)에서 정직하게 선언하고 단순식 −0.134 를 보수 하한으로 명시했으므로 결함은 아니다 — 그러나 물리 사슬 전체에서 유일하게 "다온도 실데이터 round-trip 이전에는 대표 수치의 1/3 과 한 행의 흡/발열 판정이 뒤집힐 수 있는" 미봉인 고리이며, G-usable 렌즈에서 실전 사용자가 가장 먼저 부딪힐 약점이다. 차기 버전에서 실데이터 round-trip(sec09 한계 (1))이 닫히기 전까지 이 자리가 본 장의 최약점으로 남는다.
