...
## Step 90a — 사용자 결정 5건 접수·v1.0.20 품질 정정 집행(마스터플랜 v5)
- 결정: ①확장 전건 v1.0.21 분리(원본 보존) ②bgbox 본문 유지(D-2 종결) ③문서 이력 자립성(D1 계열 제거 지시 — G3 결정) ④통계역학 확장 2건 GO(v1.0.21 소관) ⑤LCO 확장+Si 접목 요청(v1.0.21 기획 조사 기동).
- 집행(v1.0.20): B-008 스윕 6곳(제목 "— 유도 확장 (재작성)" 제거·pdftitle "(전면 재작성)" 제거·"v5 계보" 문장 재서술[사슬 실체 보존]·appendix "(초안)"×2·\date 버전/승계/결정 문구 제거) + B-009 수식화(페르미온/보손 박스: eq:sm-exch 교환 대칭·eq:sm-fdbe FD/BE 병렬 1줄 유도[eq:sm-gc 도구 재사용] / MIT 박스: eq:lco-mottcrit U≳W≃2zt·g(E_F)=0 인라인·half-filling 정밀화).
- 검증: 빌드 66/25/8p err0(Ch1 +1p — 수식 추가분)·구조 PASS·diff labels+3=eqblocks+3=B-009 1:1·자산 336/21.
