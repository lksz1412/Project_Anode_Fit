...
\end{scope}
\draw[->] (0,0) -- (6.55,0) node[below,font=\scriptsize] {$\Omega_j/RT$};
...
\node[font=\scriptsize,black!50,anchor=west] at (2.62,-0.47) {상수 대입 오답 $-\tfrac43u_j^3<0$};
% cross-check markers
...
\node[font=\scriptsize] at (3.3,-0.98) {(a) 문턱$\cdot$연성 개시$\cdot$부호 함정};
\end{scope}
...
\end{scope}
\draw[->] (240,0) -- (815,0) node[below,font=\scriptsize] {$T$ [K]};
...
\node[font=\scriptsize,black!55,anchor=south] at (300,130) {측정 창};
% 298.15 K guide
...
% Tc ticks on axis
\foreach \tc/\lab in {360.8/{},481.1/{},601.4/{},781.8/{}} {\draw[black!60] (\tc,0) -- (\tc,3.4);}
\node[font=\tiny,anchor=north] at (368,-8) {$T_{c}$};
\node[font=\tiny,anchor=north] at (481.1,-8) {$T_{c}$};
\node[font=\tiny,anchor=north] at (601.4,-8) {$T_{c}$};
\node[font=\tiny,anchor=north] at (781.8,-8) {$T_{c}$};
...
% curve labels
\node[font=\tiny,anchor=west] at (560,31.5) {$\Omega{=}13000$};
\node[font=\tiny,anchor=west] at (468,20.0) {$10000$};
\node[font=\tiny,anchor=west] at (392,11.5) {$8000$};
\node[font=\tiny,anchor=west] at (318,6.5) {$6000$ J/mol};
% closing annotation
