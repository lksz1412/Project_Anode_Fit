...
\node[font=\tiny,align=center] at (306,8.5) {electronic $\propto T$ (개형)\\직선$\cdot$강제영점 없음};
\draw[black!45,line width=0.3pt] (312,8.0) -- (322,6.55);
% vib 라벨·주석
\node[anchor=north west,font=\tiny,align=left] at (301,-0.9) {vib 편차 $\partial\Delta U_\vib/\partial T$\\(강제영점 @ $T_\mathrm{ref}$ $+$ 곡률)};
\node[anchor=south east,font=\tiny] at (296,8.6) {$\theta_E{=}700$ K};
% 온도점 값 주석
\node[anchor=north east,font=\tiny] at (277.8,-4.0) {$-3.74$};
\node[anchor=south east,font=\tiny] at (318.0,3.9) {$+3.70$};
\node[anchor=south east,font=\tiny] at (347.5,9.3) {$+9.14$};
% 2점 축퇴 주석
\node[anchor=north,font=\tiny,align=center,black!70] at (312,-5.3) {국소 기울기(2-온도)만으로는 곡률$+$선형이 축퇴 --- 3온도점 이상이 둘을 가른다};
\end{tikzpicture}
