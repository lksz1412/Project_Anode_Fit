...
% threshold 2RT
\draw[densely dotted] (4.958,0) -- (4.958,100);
\node[font=\tiny,anchor=west] at (5.15,95) {$2RT\,(298\,\mathrm K)$};
...
\node[font=\tiny,anchor=west,align=left] at (8.7,13)
  {표~\ref{tab:staging} 초기값\\$\Omega_j{=}6/8/10/13$ kJ/mol\\상한 $6.2/28.3/56.0/102.8$ mV};
\draw[-{Latex[length=1.2mm]},black!55] (10.1,24) -- (10.0,51);
% onset annotation
\node[font=\tiny,anchor=west,align=left] at (0.4,108)
  {문턱 직상방 $\propto(\Omega-2RT)^{3/2}$\\--- $0$ 에서 \emph{양수로} 연속 발아};
\draw[-{Latex[length=1.2mm]},black!55] (2.5,101) -- (5.3,8);
\node[font=\scriptsize,anchor=west] at (0.4,124) {(a) $T=298.15$ K 고정};
\end{scope}
% =============== (b) vs T for three Omega ===============
\begin{scope}[xshift=5.3cm,x=0.0106cm,y=0.030cm]
\draw[-{Latex[length=1.5mm]}] (240,0) -- (240,132)
  node[above,font=\scriptsize] {$\Delta U_j^\hys$ [mV]};
\draw[-{Latex[length=1.5mm]}] (240,0) -- (820,0)
  node[right,font=\scriptsize] {$T$ [K]};
\foreach \xx in {300,500,700}
  {\draw (\xx,2.5) -- (\xx,-2.5) node[below,font=\scriptsize] {\xx};}
\foreach \yy in {40,80,120} {\draw (234,\yy) -- (246,\yy) node[left,font=\scriptsize] {\yy};}
\draw[densely dotted] (298.15,0) -- (298.15,112);
\node[font=\tiny,anchor=south] at (290,113) {$298$ K};
% Omega = 6000 (gray)
...
\node[font=\tiny,anchor=north] at (360.8,-3.5) {$T_c$};
\node[font=\tiny,anchor=north] at (601.4,-3.5) {$T_c$};
\node[font=\tiny,anchor=north] at (781.8,-3.5) {$T_c$};
% curve labels
\node[font=\tiny,anchor=west] at (398,81) {$\Omega{=}13$ kJ/mol};
\node[font=\tiny,anchor=west] at (393,34) {$10$};
...
\node[font=\scriptsize,anchor=west] at (330,122) {(b) $\Omega_j$ 고정};
\end{scope}
\end{tikzpicture}
