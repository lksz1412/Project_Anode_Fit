...
plateau 로 지목하는 실제 근거는 실측 plateau$\cdot$staging 문헌의 상평형이다(\S\ref{sec:width-w} 의
지위 구분$\cdot$Chapter 2 파생 C 각주와 같은 한정).
...
무관 OCP(open circuit potential --- 본 문건의 OCV 와 같은 반쪽전지 준평형 전위, 문헌 표기 따름)를
확정하면 갱신한다.
