...
\cite{mckinnon1983} 이다. 식~\eqref{eq:sm-bare} 의 $\langle n\rangle^{0}$ 은 준위 하나의 평균 전자 점유를
...
principle), 평균 점유가 Fermi--Dirac 분포 $f(E)=1/(e^{\beta(E-\mu)}+1)$ 를 따른다 \cite{mcquarrie1976} (정수 스핀 $\leftrightarrow$ 대칭, 반정수 스핀 $\leftrightarrow$ 반대칭의 대응이 \emph{스핀--통계 정리}이고 --- 광자$\cdot$포논이 보손, 전자가 페르미온의 예다).
...
같은 저장조 화학퍼텐셜의 몰 표기) 식~\eqref{eq:fermifn} 는 원형 박스~\eqref{eq:sm-bare} 의 $\langle n\rangle^{0}$ 을 그대로 회수한다.
