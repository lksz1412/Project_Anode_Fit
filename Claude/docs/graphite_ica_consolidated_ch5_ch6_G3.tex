% ====================================================================
% graphite_ica_consolidated_ch5_ch6_G3.tex  —  합류 Chapter 5~6 (G3)
%   G1(Ch1) + G2(Ch2~4) 위에 적층. keystone: Ch5 = Level B 의 branch 확장.
%   N-4 검증 backbone: 보유 데이터(반쪽셀 GITT, STO-OCV, 0.1/0.2/0.5/1.0C).
%   Plan B(g-grid reference) + Plan A(Refs 6/7 Fredholm/Padé 가속, dossier caveat).
%   지배 표준: 계획서 §00 GS-1~4 + 10-pass 규율(R3-1 effective-width 정합 등).
%   기존 (A)(B) 원본 보존, 본 파일은 신규 G3.
...
\usepackage{enumitem}\usepackage{hyperref}\usepackage{mdframed}
\usepackage{fancyhdr}\usepackage{lastpage}\usepackage{setspace}
\setstretch{1.12}\setlength{\parskip}{0.4em}\setlength{\parindent}{0pt}\setlength{\headheight}{14pt}
\hypersetup{colorlinks=true, linkcolor=blue, urlcolor=blue, citecolor=blue,
  pdftitle={Graphite ICA — 합류 Ch5~6 (G3)}}
\pagestyle{fancy}\fancyhf{}
\lhead{흑연 음극 ICA — 합류 Ch5~6 (G3)}\rhead{\thepage/\pageref{LastPage}}
...
\title{\textbf{흑연 음극 ICA 이론 — 합류 Chapter 5--6 (G3)}\\[0.3em]
\large 충방전$\cdot$히스테리시스(Level B branch, Ch5) $\cdot$ 수치해법과 보유 데이터 검증(Ch6)}
...
Ch3(G2) & ★Level B($\chi=\beta$), detailed balance & ★R3-1: $\mathcal A^\mathrm{th}=RT(V-U)/w_j$; $\xi_\ss\to\xi_\eq$ 극한 일치 \\
