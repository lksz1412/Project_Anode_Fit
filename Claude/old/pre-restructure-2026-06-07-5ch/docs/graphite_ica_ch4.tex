...
명시했다. 그러나 Chapter 2 의 비가역열은 ideal width($w_j=RT/F$) 특수형 $\sum_j I_j\eta_j$ 에 머물렀고, 그 일반형은
다음 장으로 미뤄 두었다.
...
% ====================================================================
\section{기호와 규약}\label{sec:ch4_inherit}
% ====================================================================
본 장은 앞 장(Chapter 1--3)의 확정 결과를 \textbf{수정 없이} 입력으로 받아 그 위에 entropy-production 계층을
\emph{적층}한다. 받는 기준식은 식 번호로만 지칭한다(단일 문건이므로 재기술하지 않는다): 전하 보존식 (1.1)(중심
상태식이며 발열로 대체되지 않는다; $V_n$ 은 그 해, 배경 용량 단조성 (1.2)로 유일), 평형 진행률 logistic (1.11)
(도함수 (1.13)), 완화 동역학 (1.16)과 정/역 속도 $\dd\xi_j/\dd t=r_j^+(1-\xi_j)-r_j^-\xi_j$ (3.4)($k_j=r_j^++r_j^-$,
정전류 좌표 (1.18)), 구동력 일반형 $\mathcal A_j=s\,n_j^{\eff}F(V_\drive-U_j)$ (3.2)($n_j^{\eff}=RT/(Fw_j)$ (3.16),
Ch1 이상형 $sF$ 는 $n_j^{\eff}=1$ 특수형)과 그 detailed balance $\ln(r_j^+/r_j^-)=(V_n-U_j)/w_j$ (3.6), 유효
배리어 (1.27), 측정 전위 $V_\app=V_n+s_I|I|R_n$ (1.3)(내부 $V_n$$\cdot$구동 $V_\drive$$\cdot$평형 $V_{n,\OCV}$ 구분과
함께), 위치 온도계수 $\partial U_j/\partial T=\Delta S_j/(sF)$ (1.15), 그리고 Chapter 2 의 과전압 $\eta_j=V_\drive
-V_{\eq,j}(\xi_j)$ (2.29)(국소 평형 $V_{\eq,j}=U_j+w_j\ln[\xi_j/(1-\xi_j)]$ 는 식 (2.30))$\cdot$비가역열 flux$\times$force
$T\dot S_\irr=\sum_j J_jF\eta_j\ge0$ (2.29)$\cdot$축약 $q_\irr=|I|\eta\ge0$ (2.31). 이들의 형태$\cdot$기호$\cdot$단위
$\cdot$부호 규약은 직전 장들을 그대로 따르며, 방전(탈리튬화) $s=+1$ 을 기본으로 한다.

\textbf{본 장이 닫는 일반화.} Chapter 2 의 비가역열은 ideal width($n^{\eff}=1$) 특수형 $\sum_j I_j\eta_j$ 였다.
본 장은 그 일반형 $\sum_j n_j^{\eff}I_j\eta_j$ 를 entropy production 으로부터 닫는다. 한 가지 혼동을 미리 차단한다:
$\Delta S_j$(reaction entropy, 평형 products$-$reactants)와 Eyring rate 의 activation entropy $\Delta S_{a,j}$
(식~(1.17))는 \emph{별개}이며 서로 대입하지 않는다.

\textbf{신규 기호(Chapter 1--3 위에 추가).} 본 장에서만 새로 도입하는 기호다(앞 장 기호는 직전 장들과
동일물). $Q_j$=전이 $j$ 의 총 용량, $N_p$=동시 진행 전이 수, $s_I$=apparent 축 전류 부호
규약은 앞 장 그대로다.
...
$\mathcal A_j/(n_j^{\eff}F)=s(V_\drive-U_j)$(식~(3.2) 둘째 식) 였다. 발열을 닫으려면 \emph{어느 기준 전위가
$\eta_j$ 의 정의인지를 먼저 명시해야} 한다(아니면 강구동에서 같은 기호가 다른 값을 가리킨다). 본 절이 그 규약을
동결한다.
...
\textbf{$V_{\eq,j}(\xi_j)$ 의 유도(logistic 역함수).} 식~(1.11) 의 logistic $\xi_{\eq,j}=[1+e^{-(V_n-U_j)/w_j}]^{-1}$
...
\begin{boundbox}
\textbf{rate-affinity $\mathcal A_j$ 와 heat-force $\eta_j$ 의 구분.} rate-affinity $\mathcal A_j=s\,n_j^{\eff}F(V_\drive-U_j)$
(C4; Ch1 이상형 $sF$ 의 일반화)는 \emph{중심} $U_j$ 기준이라 평형에서도 0 이 아니다(rate 구동용). 본 절 $\eta_j$ 는
\emph{현재} $\xi_j$ 의 국소 평형 $V_{\eq,j}(\xi_j)$ 기준이라 평형($\xi_j=\xi_{\eq,j}$)에서 0 이 된다(dissipation 용).
둘의 차이는 식~\eqref{eq:ch4_eta_def} 를 $\mathcal A_j$ 의 전위 단위 형 $\mathcal A_j/(s\,n_j^{\eff}F)=V_\drive-U_j$ 와 비교하면 드러난다
...
\]
즉 $\mathcal A_j/(s\,n_j^{\eff}F)=\eta_j+w_j\ln[\xi_j/(1-\xi_j)]$ 이며, 둘째 항(configurational 가역 혼합 entropy)은
...
\textbf{forward/backward unidirectional flux.} 식~(3.4) 의 net flux $\dd\xi_j/\dd t=r_j^+(1-\xi_j)-r_j^-\xi_j$ 를
forward 와 backward 의 \emph{unidirectional} 성분으로 분해한다:
\begin{equation}
...
\emph{(2단계) detailed balance 대입(두 전제 명시).} 식~(3.6) 의 detailed balance ln 형
...
$\mathcal A_j^{\chem}$ 의 scale 인자로만 붙는다.) 이는 식~(1.23) 의 이상형 $\mathcal A_j=sF(V_\drive-U_j)$($n_j^{\eff}=1$) 와
...
\textbf{과전압의 물리 성분 분해.} 구동 과전압 $\eta$ 는 charge-transfer($\eta_\ct$, 식~(1.27) 유효 배리어 활성화),
ohmic($\eta_\ohm$, 식~(1.3) 의 $R_n$ 분극), transport/concentration($\eta_\conc$) 의 \emph{dissipative} 기여의 합으로
분해되며 각 dissipative 성분은 구성상 $\ge0$ 이다. \emph{단 전이별 $\eta_j=V_\drive-V_{\eq,j}(\xi_j)$ 자체는 부호
있는 양}이다(현재 진행률이 평형을 추월하면 $\xi_j>\xi_{\eq,j}$ 라 $\eta_j<0$ 가능) — 본 장 핵심 양정치
$\sum_j n_j^{\eff}I_j\eta_j\ge0$ 은 \emph{항별} $I_j\eta_j\ge0$(전류와 과전압이 같은 부호)에서 나오지 $\eta_j\ge0$
단독에서 나오는 것이 아니다. 또 이 성분들을 \emph{발열 저항}으로 쓸 때는 $R_n\ne R_{\ct,n}\ne R_{n,\eff}$
...
고정하는 bookkeeping factor 다. 피팅 대상 아님. \textbf{부호가변성.} $q_{\rev,j}=I_j\Pi_j$($\Pi_j=T\,\partial U_j/\partial T$)
...
\begin{verifybox}
\textbf{Bernardi 가역열 $=$ 미시 reaction entropy 합.} 식~(1.15) 의 $\Delta S_j=sF\,\partial U_j/\partial T$(Chapter 2 reaction entropy 관계 (2.12))
...
\end{verifybox}
\textbf{total derivative 와 progress 의 구분.} 전하 보존식 (1.1) 을 시간 미분하면 $Q_\bg$ 가 $(V_n,T)$ 의 함수
...
% ====================================================================
휴지 구간에서는 $I=0,\ \dd q/\dd t=0$ 이지만 내부 진행률은 식~(1.16)$\cdot$(3.4) 로 계속 완화된다: $\dd\xi_j/\dd t=
...
\item 현재 $q,T,\{\xi_j\}$ 에서 전하 보존식 (1.1) 으로 $V_n$ root-find(implicit algebraic — 수치 solver 는
