...
명시한다. 다섯째, loop 면적이 소산열이되 그 전체가 진짜 히스테리시스 열이 아님을 Chapter 2$\cdot$4 의 발열 분해와
정합시킨다.
...
% ====================================================================
\section{기호와 규약}\label{sec:ch5_inherit}
% ====================================================================
본 장은 Chapter 1--4 의 확정 결과를 \textbf{수정 없이} 입력으로 받아 그 위에 충방전 branch 계층을 \emph{적층}한다
(앞 장 본문 기본식을 충전으로 고쳐 쓰지 않는다). 받는 기준식은 식 번호로만 지칭한다(단일 문건이므로 재기술하지
않는다): 전하 보존식 (1.1)(배경 용량 단조성 (1.2)로 $V_n$ 유일, 평형 특수해가 $V_{n,\OCV}$), 평형 진행률 logistic
(1.11)(도함수 (1.13)), 완화 동역학 (1.16)과 정/역 속도 $r_j^\pm$ (3.4)($k_j=r_j^++r_j^-$, 정상점 $\xi_{\sstat,j}
=r_j^+/(r_j^++r_j^-)$ (3.9), 정전류 좌표 (1.18)), detailed balance 하 $\xi_{\sstat,j}=\xi_{\eq,j}$ (3.9)와 구동력
$\mathcal A_j=sF(V_\drive-U_j)$ (1.23)$\cdot$유효 배리어 (1.27), 세 전위 $V_\app=V_n+s_I|I|R_n$ (1.3)(내부 $V_n$
$\cdot$구동 $V_\drive$$\cdot$평형 $V_{n,\OCV}$ 구분), 위치 온도계수 $\partial U_j/\partial T=\Delta S_j/(sF)$ (1.15)
($\Delta S_j$ reaction entropy 는 activation entropy $\Delta S_{a,j}$ (1.17)와 별개), 그리고 Chapter 4 가 닫은
비가역 발열 일반형 $\dot{\mathcal Q}_\irr=\sum_j n_j^{\eff}I_j\eta_j\ge0$ (4.14)($n_j^{\eff}=RT/(Fw_j)$). 이들의
형태$\cdot$기호$\cdot$단위$\cdot$부호 규약은 직전 장들을 그대로 따른다.

\textbf{본 장이 여는 것.} 위 발열 분해(식 (4.14))와 앞 장 전개는 모두 방전($s=+1$)만 다루었다. 본 장은 충전 branch
부호 인자 $s_{\phi,j}^\chg=-1$ 을 \emph{유도}(\S\ref{sec:ch5_chargesign})하고, 그 위에서 branch-dependent target
$\cdot$동역학$\cdot$hysteresis 열을 닫는다 — 앞 장 기본식을 고쳐 쓰지 않고 부호 계층만 적층한다.

\textbf{신규 기호(Chapter 1--4 위에 추가).} 본 장에서만 새로 도입하는 기호다(앞 장 기호는 직전 장들과 동일물).
...
$n_j^{\eff,b}$ & --- & branch 유효 전자수 $\equiv RT/(Fw_j^b)$ (식 (4.14) 의 $n_j^\eff=RT/(Fw_j)$ 의 branch 폭 판) \\
...
극한이면 $q_\stt-q_{\stt,\mathrm{ref}}\to q$ 가 되어 식~\eqref{eq:ch5_relbalance} 가 전하 보존식 (1.1)
...
규약(식~(1.23) 의 구동력 정의 $\mathcal A_j=s_{\phi,j}F(V_\drive-U_j)$ 에 내장된 부호 약속)을 충전에 그대로
...
\textbf{구동력 $\mathcal A_j$.} 식~(3.2) 의 구동력 일반형을 branch 로 확장한 $\mathcal A_j^b=s_{\phi,j}^b n_j^{\eff}F(V_\drive-U_j^b)$ 에
...
충전 branch 중심의 \emph{자리}만 표시). \textbf{과전압 $\eta_j$.} 식~(4.2) 의 과전압을 branch 로 확장한 $\eta_j^b=s_{\phi,j}^b
...
비가역 발열을 $\dot{\mathcal Q}_{\irr,j}=n_j^{\eff}I_j\eta_j\ge0$ 형으로 적은 것은 식~(4.14) 에서 방전 기준으로
...
명시한다. branch 별 apparent 음극 전위는 식~(1.3) 의 apparent 전위 $V_\app=V_n+s_I|I|R_n$ 을 signed current 로
확장한 것이다:
\begin{equation}
...
평형 분지 차다 — 이 점이 둘을 구분하는 \emph{조작적} 기준이다(피팅 장 Ch6 의 저율/GITT 비교).
