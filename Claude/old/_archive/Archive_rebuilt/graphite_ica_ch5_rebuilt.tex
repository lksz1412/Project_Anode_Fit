...
$\xi_{\tar,j}^b$ & --- & branch target progress (metastable); branch-local Ch3 $\xi_{\sstat,j}$ 형식(본 장 \S\ref{sec:ch5_branch} 구성) \\
...
$\Delta U_j^\hys$ & V & 방전--충전 branch center 차(signed); magnitude 강제 시 부호 인자 $s_{h,j}\in\{+1,-1\}$ 로 $\Delta U_j^\hys=s_{h,j}|\Delta U_j^\hys|$ \\
$\lambda_j^b$ & 1/s & hysteresis state $h_j$ relaxation rate (domain rearrangement/depinning time scale); rest 구간 특수화 $\lambda_j^{b=\rst}$ (식~\eqref{eq:ch5_h_rest_relax}) \\
$k_j^\rst$ & 1/s & rest 구간 $\xi_j$ 완화율(식~\eqref{eq:ch5_rest_eq}); Ch1 mobility $k_j$ 와 별개의 휴지 relaxation 상수 \\
...
% ====================================================================
\section{실데이터 피팅(Ch1 부록 B)으로 전달되는 항목}\label{sec:ch5_transmit}
% ====================================================================
...
\section*{실데이터 피팅(Ch1 부록 B)으로의 전달}
