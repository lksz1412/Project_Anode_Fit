...
## §1bis. ★ 완성 상태 정의 (North Star) — 무엇이 "거리 없음"인가

"세션·업체마다 거리 있다" 의 **반대 상태**를 검증 가능한 5조건으로 못박는다. 합류 산출물이 다음을 모두 충족하면 "의도 정합(거리 없음)" 으로 판정한다. (진단 Phase 의 모든 판정은 궁극적으로 이 5조건 충족도로 환원된다.)

- **N-1 (단일 서사)**: §0.2 관찰(저온 긴 꼬리·피크 겹침)에서 출발해 Ch1→Ch6 이 **하나의 인과 사슬**로 읽힌다. 각 장 서두에 "이 장이 관찰의 어느 부분을 어떻게 잇는가" 1문단.
- **N-2 (피팅식 표면화)**: Ch1 끝에 **닫힌 형태의 피팅 가능한 논리식** `(dQ/dV)(V_n,T,|I|;\, θ_eq, ΔG_a, χ, ρ_G, R_n)` 이 **본문 결과로** 제시된다 (P1 = solver 코드만 제외, 논리식 자체는 명시). 입력→출력 평가 순서 명기.
- **N-3 (전위·전류 내장)**: OCV 평형과 전류 인가 시 전위 차(`V_n` vs `V_{n,app}`/`V_{n,drive}`)가 식에 들어가 **C-rate 피크 겹침**을 재현/예측한다.
- **N-4 (검증 가능)**: 사용자 핵심 주장(꼬리=유효배리어 효과)이 **falsify 가능한 예측**(tail Arrhenius slope 의 potential 의존; χ vs η_ct 분리 설계)으로 제시되고 경쟁 꼬리원 배제 절차가 있다.
- **N-5 (grounding·정직성)**: 모든 본문 가정이 AL 인용 + FLAGGED/BOUNDED 표기. keystone(χA Level)·평형형태(erf/logistic) 미결은 **미결로 정직하게** 표기 (SELF_REVIEW 의 "미검증을 검증으로 위장 금지").
...
**★ grep 으로 확인된 사실 (2026-05-29, 진단 강화)**: (B) Chapter 3 은 이미 **Level A 와 Level B 를 본문에서 명시 정의**한다 (`eq:ch3_phys_consistent_split` + Level A/B 구분 단락, 그리고 forward/backward 활성화식 `eq:ch3_phys_rplus_act`/`eq:ch3_phys_rminus_act`). 따라서 본 keystone 해결은 **새 구조의 발명이 아니라 reconciliation** 이다: (i) Ch1 의 `χ𝒜` 명명을 Ch3 의 **Level A(mobility)** 와 일치(=barrier lowering 명칭 제거), (ii) `χ` 를 Ch3 Level B 의 detailed balance(`eq:ch3_phys_detailed_balance`, `eq:ch3_phys_db_width`)에서 **transfer coefficient β** 로 묶어 `r_+/r_-=exp(𝒜/RT)` 와 정합, (iii) 이 한 결정을 Ch1→Ch6 전체에 일관 전파(§6.8). → keystone 은 "미결 충돌"이 아니라 **"명명·전파를 한 번 확정하면 닫히는" 잘 정의된 작업**임이 확인됐다. (리뷰 지적서가 본 모순을 "피팅 전 반드시 해소"로 격상한 것과 정합.)
...
### §6.8 ★ Keystone 전파 trace Ch1→Ch6 (P3-6 핵심 — 한 결정이 6장에 일관되는가)
D2(χ𝒜 = Ch1 Level A / Ch3 Level B) 채택 시 각 장 전달식에 일으키는 결과. (이 표 자체가 P3-6 gate 의 구체 산출물.)

| 장 | 사용 rate/전위 | keystone 결정의 귀결 | 무충돌 조건(P3-6) |
|---|---|---|---|
| Ch1 | `k=k_phys` (Level A mobility), `θ_eq` thermo | χ𝒜 → k 가속만, ratio 불변; "barrier lowering" 명칭 제거 | k 가 ratio 안 바꿈 = `θ_eq`(thermo)가 평형 전담 |
| Ch2 | `dξ/dt` (Ch1 그대로) → 가역/relaxation heat | Level A k 사용 → entropy basis 와 무모순 | 가역열=`θ_eq`(thermo)서, 비가역열=flux서 (이중계상 금지) |
| Ch3 | `r_+,r_-` (Level B), detailed balance | χ=β 로 forward/backward 분배; `r_+/r_-=exp(𝒜/RT)` | Level A(Ch1) = Level B(Ch3)의 `ξ_ss→ξ_eq` 극한 일치(`eq:ch3_phys_xiss`) |
| Ch4 | Ch3 flux–force `(J^+−J^-)ln(J^+/J^-)≥0` | Level B rate 로 entropy production ≥0 (리뷰 정합 확인) | Ch3 detailed balance 가 ln 항 부호 보장 |
| Ch5 | branch-dependent `r_+,r_-` | 충/방전 비대칭 = Level B 의 β·landscape 차이 | Level B 가 Ch3 서 확정돼야 branch 분기 정의 가능 |
| Ch6 | 수치/Prony/검증 | Level A(Plan B reference) ↔ Level B 수치 정합 | 두 level limiting-case 수치 일치 검증 |

→ **단일 결정(D2)이 6장 전부에 모순 없이 흐른다.** 유일 충돌점은 Ch1↔Ch3 경계(Level A↔B 극한 일치)이며 수식 검증 가능(Phase C).

### §6.9 ★ erf vs logistic — 합류의 유일한 진짜 모순(CONTRADICTORY)과 매듭
CROSSCHECK 가 식별한 **유일한 식별식 수준 모순**. grep 으로 (B) 의존성 확인:
- (B) Ch3 detailed balance `ln(r_+/r_-)=(V_n−U)/w` (`eq:ch3_phys_db_width`) 는 **logistic 평형(`eq:ch3_phys_logistic_eq`)일 때만 V 에 선형**. erf 면 inverse-erf 로 무너짐 → **평형형태 선택이 Ch3 kinetics 와 얽힘.**
- (A) relaxation `k(θ_eq−θ)` 는 평형형태 agnostic — 둘 다 수용.

**매듭 처리 (dual-track 구체화):**
1. 평형 isotherm = **lattice-gas(logistic 포함) grounded baseline**(AL-3a); erf = heterogeneous disorder **FLAGGED 옵션**(AL-3b).
2. Ch3 detailed balance 는 **logistic baseline 위에서 전개**(깔끔함 유지); erf 일반화는 별도 FLAGGED 노트.
3. 최종 판정 = **저속 OCV near-peak 감쇠 plot** (`log dQ/dV` vs `(V−U)`→지수=logistic / vs `(V−U)²`→가우시안=erf), DQ-v3-1. 데이터 전 baseline=logistic, erf 병기.
4. ★ 본 이론 **tail 결론은 평형형태에 무의존** (post-peak `dθ_eq/dq≈0` 만 사용). ∴ erf/logistic 미결이 합류·tail 핵심을 막지 않음 — **정지 사유 아님.**

### §6.10 ★ 사용자 PhD Refs 6/7 dossier (P3-5 의무 5항목 — 현 상태·빈칸)

| 항목 | 현 상태 | 빈칸(Phase C 에서 채움) |
|---|---|---|
| (a) 서지 | Lee 2011 (JCP 134, 121102; 10.1063/1.3565476); Son 2013 (JCP 138, 164123; 10.1063/1.4802584) | 확정 |
| (b) 사용자 논문 내 위치 | **미기록** (JCP 147(14) 144111 (2017) 의 ref.6,7 로 명시됐으나 page·para 미대조) | `_local_only` PDF 사용자 제공 시 위치 확정 |
| (c) 원 수학구조 | propagator / ratio-substitution = **Fredholm 2nd-kind** 적분방정식 해법 | 원식 1:1 전사 |
| (d) 변수 매핑 | kernel `𝒦(q,q')` ↔ Refs propagator; `Θ(q')/Θ(q)` ratio ansatz | 기호 대응표 완성 |
| (e) ★ 물리가정 차이 | **본 문제=Volterra(상한 q), Refs=Fredholm(고정구간)** — 적용성 = DQ-v3-2 | Volterra→Fredholm 근사조건 + 붕괴 측정(R-규칙) |

→ (e) 가 load-bearing(그대로 가져오면 안 되는 핵심 차이). (b) 는 사용자 원천 PDF 접근 필요(명시 시).
...
**★ 전체 arc (진단 너머 — 별도 GO).** 본 계획(Phase A~F)은 **진단·로드맵까지**(D4 기본값). 이후 합류 본문 작성은 별도 GO 의 후속 G-series 로, scope 가늠용 골격만 둔다:

| 후속 Phase | Block | Purpose | 선행 GO |
|---|---|---|---|
| G1 | 합류 Ch1 | (B) Ch1 + (A) tail 엔진 이식, keystone Level A 명명, 피팅식 표면화(N-2) | D1·D2 확정 |
| G2 | 합류 Ch2~Ch4 | 발열·반응속도론·entropy heat 를 keystone 전파(§6.8) 위에 재정합 | G1 검수 |
| G3 | 합류 Ch5~Ch6 | 충전 branch·hysteresis·수치검증 | G2 검수 |
| G4 | 빌드·검수 | xelatex 빌드 + 사용자 PDF 검수 | 각 장 검수 |

→ 진단(A~F)은 G-series 의 **설계도**다. 재작성은 "확장"이 아니라 **기존 (A)(B) salvage 재배치**이므로, 신규 물리 도입보다 **이식·정합·명명**이 주가 된다(scope 통제).
