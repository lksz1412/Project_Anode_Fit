...
- 2026-06-10 GO 시 박사님 보강 3건 반영:
  1. **품질 목표 격상**: 논리적 깊이 = 실질적 리뷰 논문급, 형식 = 교과서(친절한 단계 전개). 완전 무결 직전("턱밑")까지.
  2. **재검토 횟수**: 10회는 하한 — **완결성 수렴까지 루프 지속**(R.11+ 허용, 박사님 부재 ≥12h). Phase Range 의 R.1~R.10 은 최소 보장 횟수로 재해석, 수렴 판정(연속 2라운드 확정결함 0) 후에도 10회 미만이면 10회 채움.
  3. **원본 불가침 강화**: `.tex`/`.pdf` 원본도 덮어쓰기 금지(Opus 산출물 보존) — **`_Fable` 접미 사본**(`graphite_ica_ch1_Fable.tex` 등)을 생성해 그 위에서만 작업(모델 식별·양 모델 산출물 비교 가능). ch3/ch4 사본의 인계 식번호는 ch1_Fable 기준으로 관리. Implementation Changes 의 수정 대상은 `*_Fable.tex/pdf` 로 대체.
