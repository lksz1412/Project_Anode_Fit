...
## 확정 (사용자 2026-07-19)
- **음극 최종 선택 2건 반영**: **@3 = Si 고용체(Frumkin Ω<2RT)** · **@5 = stage-2L 전이(XRD 5-feature 완성)**.
- **Codex 지적 중 실제 문제만 정정**해 반영.
- **결과물 = v1.0.24 문건 + 코드**(둘 다 수정).
- **★LCO 재판정(진단 `LCO_DIAGNOSIS.md`)**: LCO 실측(O2)이 흑연과 **동일 방법으로 동등 피팅**(plain R²=0.944≈흑연
  0.940)·**@3/@5도 +1.25~2.13%p 개선**(음극보다 큼). → **@3(정칙용액 커널)·@5(staging 세분)는 화학 불문 일반이라
  LCO에도 반영 포함**(O2 실측 근거). **보류는 LCO 전자항 T-의존(#5·#6)만**(다온도 O3 실측 부재). 즉 이전 "LCO
  안 맞음=데이터문제"는 **모델/추가내용 문제 아님**을 실증.
- 무응답 대기 아님 — 아래 D1~D6 기본값 확정 시 R0부터 5-stage 루프 집행.
...
- **D1 — @5 구현 방식.** 기본값(권고) = **XRD 5-feature + Ω/ΔS 물리 시드**(자유 5번째 peak 아님). 4↔3은 고용체 shoulder(Ω<2RT). → 안정성 확보. (대안: 기존 4전이 유지 + 2L만 T-조건부 — 더 보수적.)
- **D2 — Si Ω_Si 시드.** 기본값 = **regsol_si δ캡 결과 + Artrith/Braga 범위 시드**(피팅 위임). 단일 fitted Ω_Si 문헌 없음(정직). (대안: Ω 고정 없이 broad 로지스틱만 — 더 단순하나 물리성↓.)
- **D3 — #1 단위 정정.** 기본값(권고) = **bit-exact 보존**(주석+환산상수 명시, 곡선 불변, dH_a 해석만 정정). (대안: dH_a 값 재기준화 — 곡선 변화·재검증 필요, 비권고.)
- **D4 — #5·#6(LCO 전자항).** 기본값 = **구현 보류·근사 라벨 강화만**(실 O3 부재). (대안: T²(#6)만 코드 반영 — 검증 불가라 비권고.)
- **D5 — 버전/구조.** 기본값 = **docs/v1.0.24/ 신 디렉토리 + Anode_Fit_v1.0.24.py**, additive·v1.0.23 보존. (대안: v1.0.23 in-place — 비권고, 이력 훼손.)
- **D6 — 착수 Phase.** 기본값 = **R0(골격복제·시드표)부터**. 시드표가 전 반영의 전제.

> **D1~D6 확정(또는 기본값 일괄 승인) + R0 GO** 주시면 5-stage 루프로 집행. 무응답 시 대기(기본값 제안 상태).
