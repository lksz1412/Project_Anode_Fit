...
>
> ⚠️ **본 INDEX 는 v1.0.22 이후 갱신이 밀려 있다** — 아래 "현재 활성 v1.0.22" 표기는 스테일이고, 실제 최신은
> 바로 아래 v1.0.25 다(v1.0.23·v1.0.24·v1.0.24.1 구간의 계획서는 파일명으로만 찾을 수 있다:
...
산출물 색인 = `../docs/v1.0.25/results/INDEX_v25.md` · 변경 원장 = `V1025_CHANGE_LEDGER.md` ·
문건 편집 보고 = `V1025_DOC_EDIT_REPORT.md` · 데이터 정정 = `V1025_DATA_ADDENDUM.md` ·
머지 판정 = `MERGE_READINESS_v25.md` · 인계 = `HANDOVER_v25.md`.

## 스테일 표기 — v1.0.22 (활물질별 챕터 재편 — GO 대기)
| 파일 | 역할 | 상태 |
|---|---|---|
| **`2026-07-17-v1022-master-plan.md`** | ★MASTER v3 — 3챕터 재편(흑연+열 / LCO+열[추가 텀] / Si·혼합 0~30%)+인용 다리 12곳+통계역학 증축+블렌드 토글+장별 기호표/레퍼런스. **운용 구성 명문화 = 3+1/3+1/1/1**(저작 O3+F1 / 검수 O3+F1 / 서지 저비용1+승급 / 코드 마스터1). **R0~R3 완료**(재편·계보 감사 ③=0건·흑연 장 80p·LCO 장 25p — 이음매 전건·다리 11·CLT/CNT·자산 357 불변), R4(Si·블렌드 조사+L2 등재·L5 재소싱)부터 잔여. 병합 빌드 금지(사용자 별도 세션) | GO·진행 중 |
