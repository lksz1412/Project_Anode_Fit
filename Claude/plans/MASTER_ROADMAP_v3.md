...
| **S3** | 불균질 broadening = 전이중심전위 분포 `p(U_j)` (가우시안). 관측 "가우시안 peak" = S2 ⊛ S3 → erf-유사 누적 *창발* | **FLAGGED ansatz** (Bässler GDM 1993 analogy + 경험 peak-fit; 직접 intercalation-thermo 근거 부재) |
...
1. **평형 isotherm: erf primary → lattice-gas primary.** erf/Gaussian 누적은 흑연
   ...
   primary 는 lattice-gas μ(ξ) (McKinnon-Haering/Bazant). **Gaussian peak 관측은 폐기
   안 함** — 불균질 site-disorder broadening (S3, FLAGGED ansatz)으로 *창발*시켜
   honor. erf 는 "균질 lattice-gas ⊛ 가우시안 disorder"의 극한으로 자연 등장.
   → 사용자 "가우시안" 관측 + grounding 표준 **양립**.
...
| AL-3b | erf/Gaussian 누적 isotherm (site-disorder) | 전이중심전위 불균질 분포 | Bässler GDM 1993 (organic semic., 10.1002/pssb.2221750102) analogy; 경험 dQ/dV Gaussian-fit (Dubarry) | **FLAGGED ansatz** | 흑연 intercalation 직접 열역학 근거 **부재**. "ansatz"로만 사용 |
...
- **DQ-v3-1**: 평형 broadening 이 균질(lattice-gas) vs 불균질(가우시안 disorder) 지배 →
  **저속 OCV peak near-peak 감쇠 형상** 실측 (`log dQ/dV` vs `(V−U)` 지수 / vs `(V−U)²`
  가우시안). 사용자 데이터 필요.
...
- DQ-v3-1(erf/lattice-gas)은 데이터 대기지만 **dual-track 으로 진행 가능** — 정지 사유 아님.
- **확인 요청**: (a) 본 grounded spine·pivot 5건 방향 승인? (b) Chapter 1 단독 우선
  (ch2~ 후속) 승인? (c) erf/lattice-gas dual-track 진행 승인 (또는 사용자가 즉시
  한쪽 지정)?
