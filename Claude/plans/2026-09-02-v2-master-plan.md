...
| 4.6 | 흑연 적용 | Part II 첫 장 — 현행 Ch1 곡선 사슬 회수(boxed 27/27)·gr2L·gallery≠상·해상도 사다리 · **TH-4.1 다중 부분격자 staging 일반식(설명층)·TH-4.2 Daumas–Hérold/Safran bgbox** · U_j 간격의 열역학 해석 · TH-3.2 dilute 공백 warnbox · tab:staging ΔS 초기값(R2:P-11) · N4 두-상 4 vs 2 확정 표기 | boxed 회수(Step 41 매핑표 기준 — 현행 분포 참고 27; 4.1~4.5 로 이관된 일반 유도 boxed 는 그쪽에서 계수) · T-19/T-20/T-21 · 빌드 |
...
- **유닛** = master + 작업 sub + 검수 sub, **직렬**. 동시 산 서브 ≤1. 병렬(fan-out)은 DR-5 sign-off 시에만 · 미승인 병렬 필요는 안 묻고 직렬 진행 + Decision Queue 기록. 판독 단계(2026-09-03)의 8 에이전트 병렬은 본 arc 착수 전 master 워크플로에서 있었던 사실이며, 그 승인 출처 = **사용자 2026-09-02 발화 "울트라 코드로 다시 시작해"**(헌법 §1-병렬 승인 3경로 ③ 사용자발 명시 지시 = fan-out 승인 · 해당 세션 대화 기록 — master 기재, DQ-I16 해소). 그 승인은 그 워크플로 1회에 한하며, 본 계획 실행의 병렬은 DR-5 sign-off 를 따른다.
...
| T-20 | 교재 형식 요소(기준 2 저작 게이트)(신규 v3) | 절별(section·subsection) O/X 표: 정의·정리(명제) 환경(4.0 `\newtheorem` 신설 후) ≥1 또는 "N/A(적용 절)" 사유 · (a)~(d) 표지(유도 절 100%) · 예제 ≥1(유도 절 — R4a DQ-10 (iv) "절당 1건 수준" 후보) · 절 요약 keybox · 기호 첫 출현 정의(기호표 대조) · 도입/마무리 다리 · 연습문제 = N/A(제외 — 사용자 기준 2 는 연습문제를 요구하지 않는다는 master 읽기[비평 #10]; 포함하려면 DR 로 올림) | 표 | 빈 셀 0 · 유도 절 (a)~(d) 100% · 예제·요약·정의 열의 X 는 사유 명기(빈 통과 금지); 하한값 조정은 4.x 세부 계획서 DQ | 2.5(Step 27 진단판)·4.1~4.9·6.2 |
...
> 위 DR 확정 + GO 사인이 오면 실행 직전 load-bearing 전제(§Assumptions)를 실물 대조(1g)한 뒤 Step 1 부터 5-stage 루프로 진행한다. 판독·통합·수정 에이전트의 Decision Queue 처리표·미해결·Read Coverage 는 `Claude/results/handoffs/2026-09-02-v2-master-plan/wf/plan_v3_appendix_ABC.md`(11-section 밖 handoff 노트)에 있다.
