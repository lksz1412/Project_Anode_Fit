...
> **모델 배정** = 전원 **Fable 5.1**(brief:45 사용자 verbatim "이 작업은 모델 배정을 예외적으로 모두 페이블 5.1로 진행한다.") — 헌법 배분표의 사용자 명시 예외 · **v5.6(2026-09-11 사용자 재결정)으로 챕터별 경계로 대체 — Implementation Interfaces 「모델」 참조**. **Codex 측** = 무접근(읽기 포함).
...
|---|---|---|
| A1 | `CLAUDE.md` | 90(R3 실측 89 · **1.1 Step 1 정본 88** = `(Get-Content).Count`; R3 89 는 Read 도구 표기, 90 은 오기 — v5.3) |
...
- **빌드 미포함 잔존 파일** **[R3·R4b 확정]**: `_sections/ch1_appD_si.tex`(91줄)는 세 마스터 어느 `\input` 목록에도 없는 orphan(`ch1_graphite_v1.0.24.tex`:25–60·`ch2_lco_v1.0.24.tex`:22–32·`ch3_si_v1.0.24.tex`:23–32; 내용은 v1.0.21 부록 D → Ch3 §3.1 승격 `ch3_si_v1.0.24.tex`:3). 자산 태그 [V21-Q7-01~05] 가 이 파일에만 남아 있다(R4b G27). brief §4.2 "_sections 56" 카운트에는 포함돼 있다. **1.1 Step 1 실측 정정(v5.3)**: `_sections/ch1_preamble.tex`(77)·`ch2_preamble.tex`(56)도 어느 마스터에서도 `\input` 되지 않는 v1.0.21 잔재 orphan(헤더 :2–3 이 `graphite_ica_ch{1,2}_v1.0.21.tex` 를 지목 · `common_preamble_v1024.tex`:3 이 두 파일을 흡수) → orphan 3 · 빌드 포함 = 마스터 3 + `\input` 53 = 56 · 미포함 4(orphan 3 + 독립 부록 1). 빌드 포함 지원 파일은 `common_preamble_v1024`(마스터 3본 `\input`)·`ch1v22_partT_divider`(ch1 L39) 2본뿐.
- **자산 카운트**(brief §4.2 = master 실측 · **[sub 재계수 ✓ 전건 일치]** · 모집단 = **파일 집합 60 tex = 빌드 포함 56(마스터 3 + `\input` 53) + 빌드 미포함 4(orphan `ch1_appD_si.tex`·`ch1_preamble.tex`·`ch2_preamble.tex` + 독립 `appendix_phase_separation.tex`) — v5.3 정정** — "빌드 세트" 가 아니다: 빌드 포함분만 세면 boxed 61(독립 부록 3 제외)·label 392(부록 30 + orphan `ch1_appD_si` 7 제외 — preamble 잔재 2본의 boxed·label 은 2.1 Step 14 재계수에서 확정)가 되어 brief 수치와 정의상 어긋난다(R4b §1.3·§1.4; T-4 정의 문서)):
...
| 박스 환경 | warnbox 14 · keybox 18 · bgbox 10 · verifybox 15 · srcbox 16 · derivbox 1 (+ signbox·codebox·procedurebox — R4a §1 합계 9종) | `common_preamble_v1024.tex`:33–41(빌드 실사용 9종; `ch1_preamble.tex`:31–36·`ch2_preamble.tex`:31–35 는 orphan 잔재 — v5.3)의 `\newtheorem*` 은 박스 이름용 — **정의·정리·보조정리·증명 환경은 정의돼 있지 않다**(R4a §1) |
...
> 괄호 안 줄수 = 판독 Read 도구 표기(+1) · 1.1 Step 1 정본(TSV `(Get-Content).Count`)은 OUT-INV §2 — v5.4 각주(검수 R3 AUD-R3-09).

```
D:\Projects\Project_Anode_Fit\
├─ CLAUDE.md                                   프로젝트 지침 P1~P5(88행 — 1.1 실측; R3 89 = Read 표기 · brief 90 = 오기)
├─ Claude\
│  ├─ docs\
│  │  ├─ INDEX.md                              문건 MOC(197줄)
│  │  ├─ v1.0.25.1\  ★현행 최신(동결 base·무수정) — ch1/ch2/ch3 *_v1.0.24.tex · appendix_phase_separation.tex(독립)
│  │  │   · _sections\(56 — orphan 3: ch1_appD_si.tex + v1.0.21 잔재 ch1_preamble.tex·ch2_preamble.tex) · Anode_Fit_v1.0.24.py(1917) · test_gates_*.py · results\(HANDOVER_v25·v24·TOUCHUP_NOTE·INDEX_v25·tools_*.py·MERGE_READINESS·CHANGE_LEDGER·DATA_ADDENDUM …) · PDF 3
...
│  ├─ plans\  INDEX.md(69 — v1.0.27 행 추가) + 날짜 계획서 90 + MASTER_ROADMAP_* 2 = 1.1 정본 93파일/10,384줄(OUT-INV §3.1)
│  ├─ results\
...
**신규 예정(저작 시 생성 — 생성 시점에 이 맵을 즉시 갱신)**: `Claude/plans/2026-09-02-v2-master-plan.md`(master 최종 저장) · `Claude/plans/2026-MM-DD-v1027-phase-<id>-plan.md`(별도 세부 계획서) · `Claude/results/Step <N> — <제목>.md` · `Claude/results/PHASE_<id>_V1027_<topic>_RESULT.md`(+`.json`) · `Claude/results/PHASE_1-7_V1027_EXECUTION_LEDGER.md` · 등록부·레지스터·블루프린트(§Implementation Changes) · `Claude/docs/v1.0.27/`(4.0) · `Claude/docs/v1.0.27/results/V1027_REFERENCE_LEDGER.md`(+`.json`) · `Claude/docs/v1.0.27/results/refcheck/`(Crossref 응답 보존, R7 규약 4(e)) · `Claude/docs/HANDOVER_v1.0.27.md`(진행 중 판 — 1.1 Step 1 신설 · 최종판 6.3).
...
|---|---|---|---|
| `Claude/plans/*.md` | **92 파일·9,567줄(master 실측 2026-09-03 = 날짜 계획서 89 + `INDEX.md` + `MASTER_ROADMAP_*` 2)** → **1.1 Step 1 정본 = 93 파일·10,384줄**(본 arc 계획서 812 + `INDEX.md` 69 갱신 포함 · `(Get-Content).Count` 정의, OUT-INV §3.1) — v1 sub 91/9,503·brief 90/9,567 은 집계 기준 차이 | 8본(A8·B6·B7·v1021·v1022·v1024 completeness·v1024 feedback + anodefit) | 83본 |
| `docs/**/PLAN_*.md` + `docs/v1.0.20/plans/2026-07-16-v1020-master-plan.md` | 15 + 1 | 0 | 16본 전부 |
| `HANDOVER*.md` | **25(old/ 제외)·1,612줄 / old/ 포함 28 — master 실측 ✓ 2026-09-03**(brief §3-C 의 "28본(old/ 제외)" 은 old/ 포함 건수를 잘못 붙인 것 — 정본 = 25/1,612, old/ 3본은 구트랙 별도 표시); `HANDOVER_v24.md` 사본 4곳 | R1 14본·R2 7본·R3 1본(중복 포함) → 고유본 약 20 | 구트랙 3 등 잔여 + hash 판정(v5.3 정정: `HANDOVER_v1.0.10`·v1.0.12 인계는 실물 부재 — `docs/v1.0.10/` 의 인계 = `HANDOVER_v1.0.11.md`, `docs/v1.0.12/*.md` = `FITTING_GUIDE.md` 뿐; 정본 25본 = OUT-INV (iii)) |
...
| 현행 문건 tex | 60 tex·9214줄 | R4a 29본(5558줄)·R4b 24본(3324줄)·R5·R6 보강 정독 → `_sections` 56 중 미정독 = `ch1v22_partT_divider`·`ch1_preamble`·`ch2_preamble`·`common_preamble_v1024`(grep 만 — v5.3: 이 중 `ch1_preamble`·`ch2_preamble` 는 orphan 잔재, 빌드 포함 지원 파일은 divider·common_preamble 2본) + 마스터 tex 3본(부분) | 위 지원·orphan 4본 + 마스터 3본 전문 — **배정 = 2.1 Step 14**(2.7 Read Coverage 에 계수); 나머지 53본 = 2.2 Step 17~19 |
...
| **2.1** | 자산 지도 | 14–15 | 카운트 정의 고정(모집단 = 파일 집합 60 = 빌드 56 + 미포함 4 — v5.3) 후 재계수 = brief §4.2 전건 일치 · 마스터 3본 + 지원·orphan 4본 전문 정독(행 범위) · boxed 64 ID 표(라벨 부재 1 표시) · 빌드 포함/미포함 열 · 자산 태그 계열 확정 · G-금지 게이트 실물 수 · diff 3+3+hash · touch-up 4건 판정 · **v1.0.26A/B 자산 지도(DR-1 A+B)** · Phase Result | — | 본 §2.2 표·R4a §1·R4b §1·R2 §2.10 | 대기 |
...
> **실행 순서 재배열(v5.6 · 사용자 2026-09-11 "흑연 먼저 완성해서 내놓는 방식으로 진행")** — Phase·Step 삭제 없이 **실행 순서만** 바꾼다. **Ch1(흑연) 우선 경로** = 1.1~1.5 → 2.1 · 2.2 Step 16~18 + **Step 20(Ch1 몫 = R4a N1~N8·Part 0↔Part T 4쌍·다중 수록 3계열 중 Ch1 항 — Ch2/Ch3 항은 이연분에서 Addendum)** · 2.3 Step 21 + **Step 23(Ch1 몫 = S1~S35 잎 boxed 41 · R5/R6 부재 단 대조 — A01~A30 은 이연 Addendum)** · 2.4/2.5(Ch1 몫) · 2.6 · 2.7 → 3.1~3.7(3.3 Step 43 제외) → 4.0~4.5 → 5.x·6.x(Ch1 한정) → **Ch1 PDF 산출·보고** → **이연분**(2.2 Step 19 + Step 20 Ch2/Ch3 항 · 2.3 Step 22 + Step 23 A01~A30 항 · 2.4/2.5 LCO·Si 몫 · 3.3 Step 43 · 4.6~4.8 · 5/6 잔여 · 챕터 7) — v5.9(AUD-R6-03: 혼합 Step 20·23 의 배치 명시). cumulative Step 번호는 그대로(이연 Step 은 번호 유지·실행 시점만 뒤 · Step 파일은 실행 순으로 생성). 챕터 말 Phase Result(2.7·3.7·4.9·5.3·6.3)는 Ch1 한정판으로 먼저 저장하고 이연분 완료 시 Addendum(결과 보호 규범).
---
...
| OUT-HANDOVER | 인계 | `Claude/docs/HANDOVER_v1.0.27.md` | 생성(진행 중 판 · Step 1 에서 신설 `baccb49`) · 갱신(매 Step/Phase 종료·중단) | 1.1~6.3(최종판 6.3) | — | 5항목 + chain |
...
| OUT-CLAUDEMD | CLAUDE.md 개정안(문서만) | `Claude/results/V1027_CLAUDE_MD_REVISION_PROPOSAL.md` | 생성 | 6.3 | R3 A-3 스테일 조항 + P3 #8 "코드 = 부록 전용" → "코드 = 문건 밖"(DR-14) + 용어 정책(DR-20)·tier 폐지(DR-21) + **P1 인용 경로 스테일**(1.1 실측 — `Claude/docs/graphite_ica_dynamic_ver5.tex`·`…charge_balance_ver1_rechecked2.tex` 실물 = `Claude/old/_archive/Archive_old/`, v5.4) | 사용자 결정 전 CLAUDE.md 무수정(DR-13) |
...
- **Step 14 — 카운트 정의 고정 + 재계수**: 카운트 스크립트(§Test Plan T-4)를 v1.0.25.1 **파일 집합 60 tex(빌드 포함 56 + 미포함 4 — v5.3 · 열별 부분합 병기)** 에 재실행. **정의를 먼저 문서화**(R7 DQ-15·R4a DQ-7·R2 DQ-12): display = `equation/align/gather/multline(*)`(230) 과 `\[…\]`(38) 분리 · boxed = 전 문건 64(범위 분포 27/10/4/3/16/4) · cite = 명령 265(본문 262 + bib 내부 3)·키 단위 315 · bibitem 95·distinct 93 · 주석 줄 `%` 제외 여부 명시. → brief §4.2 표와 전건 일치 확인. `\boxed` 64 각각 = ID·파일·행·`\label`(부재 1건 `ch1_sec06_eqpeak.tex`:91 표시)·절·문건 Chapter·spine 노드(N0~N9/Part T/부록)·현행 지위(채택/해석적 기록/opt-in/비유도 박스). `\label` 429·`\bibitem` 95·distinct cite 93 목록 첨부(2.4·챕터 5·6 무유실 게이트 기준 파일). **빌드 포함/미포함 열**(orphan `ch1_appD_si`·독립 부록·`ch1v22_partT_divider` 등) · **자산 태그 계열 기계 추출**(v1 `[A-xxx]`159/`[E-xxx]`8 vs R3 다계열 138 상충 해소 — 패턴별 건수·파일) · **G-금지 게이트 실물 수**(`test_gates_v1025.py` 함수 수, N11) · 박스 환경 9종 카운트 + `\newtheorem*` 정의 목록(정리·정의·증명 환경 0 확정) · **마스터 tex 3본(`ch1_graphite_v1.0.24.tex`·`ch2_lco_v1.0.24.tex`·`ch3_si_v1.0.24.tex`) + 지원·orphan 4본(`ch1v22_partT_divider`·`common_preamble_v1024` = 빌드 포함 지원 / `ch1_preamble`·`ch2_preamble` = v1.0.21 잔재 orphan — v5.3) 전문 정독**(행 범위 기록 — §2.9 미정독 7본의 배정처이며 2.7 Read Coverage 에 계수; 판독은 grep·부분만 봤다). **orphan 라벨·서브라벨 검사**(W2-2 orphan 라벨·N6a/N6b 서브라벨 — R1 L-18·비평 #11).
...
**게이트 2.1.** 카운트 표 = brief §4.2 전건 일치(정의 문서 첨부) · boxed 표 64행(지위 열 빈 셀 0) · 빌드 포함/미포함 60/60 · 자산 태그 계열 표(상충 해소 명시) · G-금지 실물 수 확정 · diff 파일 집합 = {3+3} + hash · 승계표 시드. **중단 조건.** 카운트 불일치 → 원인(정의·파일 집합) 규명 후 정본 확정(정지 아님). **다음 조건.** Step 14·15 이력 + 마스터 3본·지원·orphan 4본 정독 행 범위 + `PHASE_2.1_V1027_<topic>_RESULT.md`+`.json` + Ledger 행.
...
**청크(§2.6 정량 기준, 각 ≤~500줄 창·최대 ~700 — 실제 행 범위는 세부 계획서).** 총 9214줄 → 최소 19 청크(Step 17 = 7 · Step 18 = 5 · Step 19 = 7 — R4a §1·R4b §1.4 파일 줄수로 기계 산출; 나머지 361줄(TSV 정의: 마스터 3본 130 + 지원·orphan 4본 231 — v5.3; R4a 표기 합 332 는 Read 도구 +1 표기) = 마스터 3본 + 지원·orphan 4본은 2.1 Step 14 소관).
...
- **게이트 2.7.** GAP 전건 4-tier 라벨 · Read Coverage(**본 arc Read 만 계수** — 2.1 Step 14 마스터 3본+지원·orphan 4본(361줄) + 2.2 Step 17~19 53본(8,853줄) = v1.0.25.1 9,214줄 전 영역 cover(TSV 정의 — v5.3); 판독 R4a/R4b 커버리지는 참조 열) · **v1.0.25 커버리지 = v1.0.25.1 전 영역 + diff hunk 4건 정독**(`ch3v22_sec02b_sifr.tex`:40–44·:144–145 / `ch1_sec05_width.tex`:303–304 / `ch1_sec06_eqpeak.tex`:107–108 — brief:33 "1.0.25 두 가지 버전을 검토" 의 감사 가능 게이트) · md+json 쌍 · R4a DQ 10·R4b DQ 10·R7 DQ 15 각각 "해소/이관/미해소". **중단 조건.** 없음. **다음 조건.** Result 없이 챕터 3 진입 금지.
...
- **모델** = **본작업 모델 경계(v5.6 · 사용자 2026-09-11 재결정 — 종전 "전원 Fable" 예외 brief:45 를 대체)**: 챕터 1·2(등록부·진단 = 기존 문서 정독·판정) 작업 sub·검수 sub·master = **Fable 5.1**(단 기계 산출 — Step 2 배정표 정렬·청크 계산·2.1 카운트 재계수 류 — 는 **Sonnet 4.6 + 자체검수 1회**, 헌법 A3) · 챕터 3 설계 작업 sub = **Opus 5.0**(A1 · Step 단위 투입) + 검수 sub Fable · 챕터 4·5·6 저작 작업 sub = **Opus 4.8**(A2) + 검수 sub Fable(검수에서 확정결함이 2회 연속 나는 절만 Fable 저작으로 승격) · 챕터 7 = Opus 4.8 + Fable 검수 · master 통합·삼각검증 = Fable 5.1 · **동시 에이전트 ≤3**(v5.5) · **조사(문헌 검색·DOI 검증·공개데이터 확보·기계 추출) = Haiku(A4) / Sonnet 4.6(A3) 서브**(DR-5·DR-6 사용자 확정 — 토큰 절약 기본 골조; 조사 서브는 본작업 유닛과 별개로 운용 가능, 산출은 파일로 인계하고 본작업 sub 가 정독 후 사용). 지정 모델 런타임 장애 시 silent substitution 없이 정지·보고.
...
- **기록** = 스텝 이력 `Claude/results/Step <N> — <제목>.md`(step 하나 = 파일 하나 · 5항목 · 즉시 기록) · Result `Claude/results/PHASE_<id>_V1027_<topic>_RESULT.md`+`.json`(12항목) · Ledger `Claude/results/PHASE_1-7_V1027_EXECUTION_LEDGER.md`(12-col) · 세부 계획서 = 챕터 1 은 마스터 내 Phase 절 / 챕터 2 이후 = `Claude/plans/2026-MM-DD-v1027-phase-<id>-plan.md`(헤더의 3단 구분) · 핸드오프 `Claude/results/handoffs/<task>/` · 인계 `Claude/docs/HANDOVER_v1.0.27.md`(**진행 중 판 — Step/Phase 종료·중단·사용자 결정 편입마다 즉시 갱신**, v5.6 ④ 사용자 기록 의무; 6.3 에서 최종판).
...
- **검수 라운드 정독 규칙(v5.7 · 사용자 2026-09-11)** = **1라운드 = 전문 정독**(중간 생략·추론 채우기 차단 — 사용자가 정독을 요구한 본뜻) · **2라운드 이후 = 발췌독**(직전 iter 가 처분표에 열거한 변경 지점 + 직전 발견 반영 위치 + 그 문맥 이해에 필요한 앞뒤 절만) **+ 기계 검사 전건 재실행**(카운트·Test-Path·hash 표기 파싱·policy_check 등 — 발췌독이 못 보는 영역은 스크립트가 본다) · 반박 3인·refute mandate·최약점 1곳은 유지. **Step 1 한정 예외(사용자 2026-09-11 "6라운드 문제 없으면 그냥 다음으로" — 토큰·시간 제약)**: 연속 2R 대신 R6 단독 확정결함 0 으로 수렴 처리(Step 2 배정표가 OUT-INV 전 행을 기계 재생성하므로 표 수준 결함은 거기서 한 번 더 걸린다).
...
| T-4 | 자산 카운트(정의 고정) | PowerShell/Python — **카운트 집합 정의: 파일 집합 60 tex(마스터 3 + `_sections` 56 + 독립 부록 1) = 빌드 포함 56 + 빌드 미포함 4(orphan `ch1_appD_si`·`ch1_preamble`·`ch2_preamble`·독립 부록 — v5.3), 열별 부분합 병기(빌드 포함분만 세면 boxed 61·label 392); 주석 제외 여부 명시** — **정의 문서**: display = `equation/align/gather/multline(*)` 230 과 `\[…\]` 38 분리 · boxed 64 전 문건(27+10+4+3+16+4) · cite 명령 265(262+3)/키 단위 315 · bibitem 95/distinct 93 | 카운트 표 + 정의 문서 | v1.0.25.1 = brief §4.2 전건 일치(파일 집합 60 기준) · v1.0.27 = 자산 무유실(라벨 429 전건 존재 또는 alias 매핑표 · boxed 64 회수표 100% = 27+10+4+3+16+4 · 태그 승계표) | 2.1·4.0·6.2 |
...
| T-15 | 빌드 포함/orphan 검사(신규) | 마스터 tex `\input{…}` 집합 vs `_sections/*.tex` 집합 차집합 | 목록 | v1.0.25.1 = orphan 3(`ch1_appD_si` + v1.0.21 잔재 `ch1_preamble`·`ch2_preamble` — v5.3, 1.1 실측) + 독립 1 확인 · v1.0.27 = orphan 0(이동 기록 있는 파일 제외) | 1.1·2.1·4.0·6.2 |
...
9. **모델 배정 = 챕터별 경계(v5.6 · 사용자 2026-09-11 재결정 — Interfaces 「모델」이 정본; 종전 "본작업 전원 Fable 5.1" brief:45 + DR-5·DR-6 은 v5.5 까지의 전제)** · 조사 Haiku/Sonnet 는 그대로. 참고 사실(판단 X): v1.0.25 는 Opus 5.0 master 직접 편집 + Opus 4.8·Fable 5.0 서브로 저작됐다(`HANDOVER_v25.md`:48,65, R3 B-12).
...
11. 이력 문서군 규모(**1.1 Step 1 정본 = OUT-INV §2 — 지정은 v5.4 도입 · 값은 최신 iter 시점 · 시점별 이력 = Correction History**) = plans 93(10,384줄) + docs/**/PLAN_* 16(852) + HANDOVER 28(1,915 — old/ 3 포함) + Fable 8(885) + CLOSING 105 + INDEX 10(982) + ledger 81(3,698) + Result·조사·원장·v1.0.26·시드·tex·추가 발견군 — **등재 합계 740 파일·111,160줄**(iter_6 · TSV `(Get-Content).Count` 정의; 종전 "≈ 12,500줄" 은 plans·HANDOVER·Fable·CLOSING·INDEX 5군만의 합 — 정독 비용 근거는 Step 2 배정표의 ①군 전문/②군 토픽 한정 분할로 재산정) + 유실 자산 원문 5본(Assumptions 23)(DR-7 비용 근거). 판독 8본이 그중 인계·감사·계획서·조사 문서 약 60본을 1회 정독했다(§2.9). 정독 범위는 DR-7 사용자 확정(핵심 문건 전문 + 세부 계획서·ledger·result 토픽 한정).
12. brief §3-C "plans 90·9567줄·HANDOVER 28(old/ 제외)·1612줄" — **master 재실측 2026-09-03**: plans **92 파일·9,567줄**(날짜 계획서 89 + `INDEX.md` + `MASTER_ROADMAP_*` 2) → **1.1 Step 1 정본 93/10,384**(v5.3) / HANDOVER **25(old/ 제외)·1,612줄**, old/ 포함 28 / `docs/**/PLAN_*.md` 15. brief 의 "28(old/ 제외)" 은 old/ 포함 건수의 오기, v1 sub 의 91/9,503 은 집계 기준 차이. 1.1 게이트에서 파일명 단위로 정본 고정.
...
- **2026-09-03 v5.3(master — Phase 1.1 Step 1 실측에 따른 사실 정정 · 골격·Phase·Step·게이트 불변)**: OUT-INV(`Claude/results/V1027_HISTORY_INVENTORY.md`, 검수 라운드 1 AUD-13·04·08·07 반영) 실측으로 정정 — ① 현행 tex 60 의 빌드 포함/미포함 = 마스터 3 + `\input` 53 = **56 / 미포함 4**(orphan = `ch1_appD_si` + v1.0.21 잔재 `ch1_preamble`·`ch2_preamble`, 독립 부록 1; 빌드 포함 지원 파일 = `common_preamble_v1024`·`ch1v22_partT_divider` 2본) — §2.2·§2.6·§2.9·Phase Range 2.1·2.1 Step 14·2.2 청크·게이트 2.2·2.7·T-4·T-15 문면 정정, 박스 환경 정의 근거 = `common_preamble_v1024.tex`:33–41 ② 2.1/2.2 줄수 좌표를 TSV 정의로 통일: 마스터 3 + 지원·orphan 4 = 361줄, 53본 = 8,853줄(합 9,214; 종전 332/8,882 는 판독 Read 표기 +1 합산) ③ plans 정본 = 93 파일·10,384줄(1g 10,383 + 계획서 v5.2 행 +1 = `git show 8d9362f` 811 → HEAD 812) ④ `CLAUDE.md` = 88줄(90 은 오기, 89 는 Read 표기) ⑤ §2.9 HANDOVER 잔여의 `HANDOVER_v1.0.10`·v1.0.12 = 실물 부재(정본 25본). 정독·게이트 규칙 변경 없음(preamble 잔재 2본은 2.1 Step 14 정독 대상 유지 — 9,214줄 전 영역 cover).
- **2026-09-03 v5.4(master — 검수 라운드 2 AUD-R2-05·09 반영 · v5.3 잔존 정정 · 골격·Phase·Step·게이트 불변)**: §2.8 구조 맵 3곳(`CLAUDE.md` 88행 · `_sections` orphan 3 · plans 93/10,384·`INDEX.md` 69) · §2.9 tex 행 우측 열·Phase Range 2.1·게이트 2.1 의 "지원 4본" → "지원·orphan 4본" · Assumptions 11 규모를 OUT-INV §2 정본(당시 iter_3 722 파일·103,537줄 → 같은 commit 안 iter_3b 730/110,253; 검수 R3 추기 시 iter_4 738/111,107 — 시점별 실측, 라벨 정정 v5.5)으로 교체 · OUT-CLAUDEMD 행에 DR-13 후보 "P1 인용 경로 스테일" 추가(프로젝트 `CLAUDE.md`:14–15 의 `Claude/docs/graphite_ica_*.tex` 실물 = `Claude/old/_archive/Archive_old/`, OUT-INV (xvii-b)) · 검수 R3 반영 추기: §2.8 각주(줄수 정의 병기) · Assumptions 11 합계 738/111,107(FITTING_GUIDE 규약 기록 8본 등재, OUT-INV (vii-c)).
- **2026-09-05 v5.5(master — 검수 라운드 4 AUD-R4-07 반영 · 사실 정정 · 골격·Phase·Step·게이트 불변)**: Correction History v5.4 행의 수치 라벨 정정(iter_3 라벨에 뒤 시점 값이 덮어써진 것을 시점별 이력 병기로 복원 — 원문은 복원 지점 `731a94e`·`6bf32c9` 의 해당 행) · Assumptions 11 등재 합계 739 파일·111,130줄(iter_5: `docs/INDEX.md`:181 인용 `CODE_w_check.md` 등재, OUT-INV (ix-b) 행 6 — 정책 (b) 토크나이저 결함 정정) · 운용 기록: 검수 라운드 4 는 Workflow(렌즈 3 + 발견별 반박 3인 + 통합)로 실행했고 사용자 지적(2026-09-05 "병렬을 허가했지만 이정도로 많이 진행하면 안돼")에 따라 **동시 에이전트 ≤3** 을 이후 병렬 상한으로 둔다(Implementation Interfaces 「유닛」 병렬 조항의 실행 상한 — 판독 단계 8 에이전트 승인은 그 시점 한정).
- **2026-09-11 v5.6(master — 사용자 재결정 2건 편입 + 검수 라운드 5 반영 · Phase·Step·게이트 삭제 없음)**: ① **흑연 우선 실행 순서 재배열**(사용자 "흑연 먼저 완성해서 내놓는 방식" · Phase Range 표 아래 재배열 블록 — Ch1 PDF 산출 후 LCO·Si·코드 이연분) ② **모델 경계**(사용자 "저작을 오푸스나 소넷에 시키고 검수를 니가" + "기존 작업물 검토는 검수 과정과 크게 다르지 않으니 지금처럼" — 챕터 1·2 Fable 유지 · 챕터 3 Opus 5.0 설계 · 챕터 4~7 Opus 4.8 저작 · 검수 sub·master 는 Fable · 헌법 §1 독립 검수 sub 유지 — Interfaces 「모델」·헤더) ③ 검수 R5(AUD-R5-01 · P1) 반영: Assumptions 11 등재 합계 740 파일·111,160줄(iter_6 — `docs/INDEX.md`:187 stem 인용 `KNOWN_DEFECTS.md` 등재 · (b) 기계 검사 = 확장자 토큰 + stem 스윕으로 확장) · Assumptions 11 스탬프 의미 명시 ④ 기록 의무 재확인(사용자 "후임 마스터/서브세션들도 참고할 수 있게 기록") — Step/Phase 종료·중단마다 `Claude/docs/HANDOVER_v1.0.27.md`(진행 중 판) 갱신을 Interfaces 「기록」 관행에 추가.
- **2026-09-11 v5.7(master — 사용자 지시 2건 편입 · 골격·Phase·Step·게이트 삭제 없음)**: ① 검수 반복 라운드 정독 규칙 = 1R 전문 · 2R+ 발췌독 + 기계 검사(사용자 "문제점을 발견했으면 그 뒤에는 그 부분만 발췌독" — 정독 요구의 본뜻은 중간 생략·추론 채우기 차단이므로 첫 라운드에만 전문 정독을 건다) ② Step 1 한정 수렴 예외 = R6 단독 확정결함 0(사용자 "6라운드 문제 없으면 그냥 다음으로 넘어가자") — Interfaces 「검수 라운드 정독 규칙」. 이후 Step 의 검수 지침(audit_checklist)은 이 규칙을 그대로 싣는다.
- **2026-09-11 v5.8(master — 사용자 지시 편입 · 골격·Phase·Step·게이트 삭제 없음)**: 검수 라운드 읽기 범위 실행 규칙 ①~⑥(1R 전문 · 2R+ 발췌독 ±30행 · 반박자 ±20행·확정 후보만 3인 · 기계 검사 전건 · 동시 ≤3 · 템플릿 고정) — Interfaces 「검수 라운드 읽기 범위 실행 규칙」. 원인 기록: R2~R6 워크플로가 렌즈 프롬프트에 "전 영역 cover" 를 매 라운드 박았고 반박자 3인×발견 15~28건이 각자 전문을 다시 읽어 라운드당 40회 이상 전문 검토가 발생 — 사용자 지적으로 정정. Step 1 R6(진행 중)까지는 구 방식.
- **2026-09-11 v5.9(master — 검수 R6 반영 · Step 1 수렴 선언 · 골격·Phase·Step·게이트 삭제 없음)**: R6 = 확정결함 0 · 경미 5 · 제안 4(`iter_6/audit_log_r6.md`) → **Step 1 수렴 = R6 단독 확정결함 0(v5.7 사용자 예외 적용 · 브리핑 규칙 "연속 2R" 은 이 Step 에 한해 대체됨을 master 가 명시)**. R6 경미·제안 반영: Assumptions 9·§2.4 모델 배정 문안을 v5.6 경계로 동기화(AUD-R6-04) · 「기록」·OUT-HANDOVER·§2.8 에 인계판 진행 중 판·갱신 주기 명시(AUD-R6-05) · 재배열 블록에 혼합 Step 20·23 의 Ch1/이연 배치 명시(AUD-R6-03) · OUT-INV §10.8 = 자기 행 번호 인용 정정·`KNOWN_DEFECTS` 매핑 규칙·§10.1~10.3 행 기준·PC 포인터(AUD-R6-01·02·07·09) · (b-2) 정규식 형태 한정(AUD-R6-06)은 OUT-INV §10.8 에 한계 명기 + 광역 스윕 잔여 0 기록으로 처분 · PC 는 통제 문서 최종 편집 뒤 재실행(AUD-R6-08 — 스냅샷 = 본 v5.9 저장본).
