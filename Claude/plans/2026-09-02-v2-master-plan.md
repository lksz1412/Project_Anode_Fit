...
| 4.4 | **Ch1 후반부 부록 D — 통계역학(교과서급, 신규 요구)** | 현행 Part 0(sec02a·02b) + Part T 미시 유도부(분배함수·config·vib/electronic·Einstein·mixing·극한) + SM2-A/B/C + Sommerfeld + Einstein/Debye 전부 이전·**확장**: 앙상블·분배함수·Boltzmann/Gibbs 인자 → 격자기체 Hamiltonian(cluster expansion·Ising 동치, TH-1.1) → 평균장/Bragg–Williams·준화학/CVM(TH-2.2) → 요동-응답(감수율 항등·앙상블 동등성·켤레 쌍·일반 요동 항등 TH-1.2) → 엔트로피 분해(config·vib·electronic) → 포논 DOS → Einstein/Debye(TH-7.2) → FD/BE → Sommerfeld(TH-7.3) → Maxwell 관계 — 각 항목 = 전제 개념 선행 도입 → (a)~(d) 전 중간식 → 예제 ≥1 → 본문 어느 식으로 회수되는지 표. **비전공자·타전공 석박사 가독을 게이트로**(T-21 전항) — "대학원에서 통계역학을 오래 한 독자도 어려웠다" 는 사용자 판정을 기준 삼아 본문보다 느린 호흡으로 쓴다 | (a)~(d) 100% · 부록 D↔본문 회수 표 100%(Part 0 boxed 9 + Part T 미시 boxed 전건) · T-20 예제 ≥1/절 · T-21 7항목 전건 O · 학술 용어 영어 원어 · 빌드 |
...
| 7.3 | 검증·마감 | 공개데이터(기확보 SINTEF + 필요 시 조사 서브 확보분) 재현 · 문건 예제 수치 = 코드 출력 · `PHASE_7_V1027_CODE_RESULT.md`+`.json` · Ledger · HANDOVER 갱신 · commit·push |
