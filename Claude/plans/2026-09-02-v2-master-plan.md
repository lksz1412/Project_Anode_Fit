...
11. 이력 문서군 규모(**1.1 Step 1 정본 = OUT-INV §2, v5.4**) = plans 93(10,384줄) + docs/**/PLAN_* 16(852) + HANDOVER 28(1,915 — old/ 3 포함) + Fable 8(885) + CLOSING 105 + INDEX 10(982) + ledger 81(3,698) + Result·조사·원장·v1.0.26·시드·tex·추가 발견군 — **등재 합계 730 파일·110,253줄**(TSV `(Get-Content).Count` 정의; 종전 "≈ 12,500줄" 은 plans·HANDOVER·Fable·CLOSING·INDEX 5군만의 합 — 정독 비용 근거는 Step 2 배정표의 ①군 전문/②군 토픽 한정 분할로 재산정) + 유실 자산 원문 5본(Assumptions 23)(DR-7 비용 근거). 판독 8본이 그중 인계·감사·계획서·조사 문서 약 60본을 1회 정독했다(§2.9). 정독 범위는 DR-7 사용자 확정(핵심 문건 전문 + 세부 계획서·ledger·result 토픽 한정).
...
- **2026-09-03 v5.4(master — 검수 라운드 2 AUD-R2-05·09 반영 · v5.3 잔존 정정 · 골격·Phase·Step·게이트 불변)**: §2.8 구조 맵 3곳(`CLAUDE.md` 88행 · `_sections` orphan 3 · plans 93/10,384·`INDEX.md` 69) · §2.9 tex 행 우측 열·Phase Range 2.1·게이트 2.1 의 "지원 4본" → "지원·orphan 4본" · Assumptions 11 규모를 OUT-INV §2 정본(730 파일·110,253줄 — iter_3)으로 교체 · OUT-CLAUDEMD 행에 DR-13 후보 "P1 인용 경로 스테일" 추가(프로젝트 `CLAUDE.md`:14–15 의 `Claude/docs/graphite_ica_*.tex` 실물 = `Claude/old/_archive/Archive_old/`, OUT-INV (xvii-b)).
