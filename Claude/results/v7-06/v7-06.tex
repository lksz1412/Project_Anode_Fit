...
%   그림 = 전부 신규(v5/v6 그림 재사용 금지), TikZ, 내부 텍스트 영어 ASCII 만.
...
\hypersetup{colorlinks=true, linkcolor=blue, urlcolor=blue, citecolor=blue,
  pdftitle={흑연 음극 dQ/dV 이론: 코드 진행을 따라가는 수식-구동 — Chapter 1 (v7)}}
\pagestyle{fancy}\fancyhf{}
\lhead{흑연 음극 $dQ/dV$ 이론 — Ch.1 (v7, 코드 진행 수식-구동)}\rhead{\thepage/\pageref{LastPage}}
...
(방전 $+1$/충전 $-1$)이 코드의 \code{s} 인자이며, 평형 점유 $\xi_\eq=\mathrm{logistic}[\sigma_d(V_n-U)/w]$ 에서 방전
($\sigma_d=+1$)은 $V_n\!\uparrow\Rightarrow\xi_\eq\!\uparrow$(전위를 올리면 탈리튬화). 분기 중심 $\pm\tfrac12\sigma_d(\cdots)$
...
$\Delta G_j=\Delta H_{\rxn,j}-T\Delta S_{\rxn,j}$ 를 식~\eqref{eq:eqcond} 의 $\Delta G_j=-sFU_j$($s=+1$)에 넣어 $U_j$ 로 풀면
\begin{equation}
...
$\xi/(1-\xi)$ 가 두 끝점에서 서로 역수, $(1-2\xi)=\mp u_j$ 이고, 극대에서 극소를 빼면 $U_j$ 가 상쇄되어
\begin{equation}
\boxed{\;\Delta U_j^\hys
...
평형 점유의 폭 척도는 이상 극한에서 $w_j=n_jRT/F$ 다($n_j$ 는 폭 다중도). 코드 \code{func\_w(T, n)} 의 \code{n*R*T/F}
...
\label{eq:xieq}
\end{equation}
방향 부호가 logistic 인자의 부호로 들어간다 — 이것이 \code{func\_ksi\_eq(T, V\_n, U, n, s)} 의
$z=s(V_n-U)/w_j$(폭은 \code{func\_w}$(T,n)$)이며, 코드는 오버플로 방지를 위해 $z\ge0$ 과 $z<0$ 을 갈라 평가한다
(\code{np.where(z>=0, 1/(1+e\^{-z}), e\^{z}/(1+e\^{z}))} — 수학적으로 동일). 호출 시 중심 $U_j^{\,d}$ 는
...
들어온다. 완화 속도 $k_j$ 의 forward 극한 $k_j\simeq k_0\exp[-(\Delta G_{a,j}-\chi\mathcal A_j)/RT]$
...
$\xi_\eq$ 의 $5\%$ 컷에 대응). 상한 $A_\mathrm{cap}RT$ 와 함께
\begin{equation}
...
forward 극한과 $k_0=k_BT/h$ 를 식~\eqref{eq:Lq} 에 넣고 $\Delta G_a^\eff=\Delta H_a^\eff-T\Delta S_a$ 로 갈라 적으면,
코드가 평가하는 $L_q$ 가 된다:
\begin{equation}
...
$x=\chi_d$, $\dd G_a=\Delta H_a^\eff-T\Delta S_a$) 지수화한다. 전압축으로 옮기면, 컷점 OCV 기울기를 곱한다:
\begin{equation}
\boxed{\;L_{V,j}=\Big|\frac{\dd V}{\dd q}\Big|_{q_a}\,L_{q,j}\;=\;|\code{dVdq\_qa}|\cdot L_{q,j}\;.}
\label{eq:LV}
\end{equation}
코드의 \code{return abs(dVdq\_qa) * L\_q} 다. \emph{부호의 핵심} — $\partial\ln L_{q}/\partial V<0$ 이다($V\!\uparrow\Rightarrow
\mathcal A\!\uparrow\Rightarrow$ 유효 장벽$\downarrow\Rightarrow$ 꼬리 짧아짐). 또 $L_q\propto|I|$($T_*\propto|I|$)라 $|I|\to0$
...
N4 & $w_j=n_jRT/F$ ($w^\eff$) & \eqref{eq:weff} & \code{func\_w},\code{func\_w\_eff},\code{\_width} \\
...
\item \textbf{$\partial\ln L_q/\partial V<0$.} 식~\eqref{eq:Lqfull}$\cdot$\eqref{eq:LV} 논의. $V\!\uparrow\Rightarrow
\mathcal A\!\uparrow\Rightarrow$ 유효 장벽$\downarrow\Rightarrow L_q\downarrow$(꼬리 짧아짐). $\checkmark$
