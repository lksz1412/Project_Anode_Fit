...
\begin{equation*}
sF\big(V_{\eq,j}(\xi)-U_j\big)=g_j'(\xi)
...
\begin{equation*}
g_j''(\xi)=\frac{RT}{\xi(1-\xi)}-2\Omega_j,\qquad
g_j''\!\left(\tfrac12\right)=4RT-2\Omega_j.
\end{equation*}
중심에서 inverse-logistic 폭 $w_j^\eff$ 의 기울기는 $sF\,\dd V/\dd\xi|_{1/2}=4F w_j^\eff$ 이므로,
$4F w_j^\eff=4RT-2\Omega_j$ 를 풀어
\begin{equation}
...
\end{equation*}
이고 이를 전압축 길이로 나눈 것이 코드의 국소 peak 모양이다. 다만 이 꼬리식 자체의 평형 환원은 $L_V\to0$ 에서 일어나지 않는다.
$\rho=\exp(-\Delta_{\mathrm{grid}}/L_V)$ 이므로 $L_V\ll\Delta_{\mathrm{grid}}$ 에서는 $\rho\to0$, 곧
$\xi_{\mathrm{lag}}\to\xi_{\eq}$ 이고 $(\xi_\eq-\xi_{\mathrm{lag}})/L_V$ 는 수치적으로 $0$ 으로 무너진다. 반대로
$L_V\gg\Delta_{\mathrm{grid}}$ 에서는 $1-\rho\simeq\Delta_{\mathrm{grid}}/L_V$ 라서 매끈한 bell 환원이 이산 저역통과로 해상된다.
\end{derivebox}
...
의 출력). $L_V$ 가 격자보다 충분히 길면($L_V\gg\Delta_\mathrm{grid}$, $\rho\to1$) 식~\eqref{eq:peakshape} 는 매끈한
bell 환원 형태를 낸다. $L_V$ 가 격자보다 작으면($\rho\to0$) 꼬리식은 평형 종으로 자연 환원되지 않고
$\xi_{\mathrm{lag}}\to\xi_\eq$ 로 가서 peak 가 $0$ 에 가까워진다. 따라서 작은 $L_V$ 에서의 수치적 평형 복귀는
식~\eqref{eq:peakshape} 의 극한이 아니라, 아래 식~\eqref{eq:branch} 의 명시 스위치가 담당한다:
\begin{equation}
\text{peak\_shape}=
\begin{cases}
