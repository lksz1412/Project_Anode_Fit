...
### (b) two-phase 델타가 실측 유한 종이 되는 까닭 — 동역학·내재 [확정]
평형이 아니라 *측정 조건*이 폭을 만든다(전부 평형 U_j 분포 아님):
...
→ ★**그러므로 two-phase 전이의 폭 $w_j$ 는 평형 예측($nRT/F$)이 아니라 이 broadening 이 정하는 *현상학적 자유 피팅 폭*이다.** (이것이 v8 이 덜어낸 핵심 — apparent-U = U_j + η, 분포하는 건 η.)

### (c) 무엇을 *하지 않는가* — ★범위·경고 [확정·D3]
- **평형 U_j 분포·입자 반경 분포를 peak 에서 추출하지 않는다**: 반경→평형 U_j 채널(표면에너지 Gibbs-Thomson·양자가둠 밴드)은 마이크론 흑연서 ~0.01 mV 로 무시(radius Round1 확정; Cogswell-Bazant 2012 임계 22 nm). 평형 U_j 는 GITT 상 입자 무관 상수.
