...
## ③ 사용자 판단 → 해결 (문건을 코드에 맞춤)
- **A-1b XRD_v1024 U-값 재배치**: 코드 `GRAPHITE_STAGING_XRD_v1024` 5값(U=0.210/0.170/0.132/0.116/0.085)이 문건 §5b(d)(iii) 서술("표 4행 라벨 그대로 보존 + dilute 추가, 재배치 없음")과 불일치 — 코드는 dilute 를 0.210 에 두고 4↔3→0.170·3↔2L→0.132·2L↔2→0.116 으로 이동한 **재유도 XRD 사다리**.
  - **사용자 결정: "문건을 코드에 맞춤"** → §5b(d)(iii) 를 정정: "5-feature 시드는 네 전이 **라벨을 계승**하되 U-**위치는 XRD 해상도로 재유도**(dilute≈0.210·4→3≈0.170·3→2L≈0.132·2L→2≈0.116·2→1≈0.085), 표 표시 위치(0.210/0.140/0.120/0.085)와 다름. 피팅 기준선 `GRAPHITE_STAGING_LIT` 은 무변경." **ch1 재빌드 GREEN**(89p·0 err·0 undefined). → **전 감사항목 종결.**
