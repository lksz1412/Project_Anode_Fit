...
## 6. 즉시 검증 결과 (실행 완료 — `v24_regsol2.py`·`regsol2.png`·`regsol2_result.json`)
**M1/G3(near-delta 정칙용액 Ω>2RT+Maxwell⊗kinetic)** 을 SINTEF 흑연 실 dQ/dV(4전이 동시)로 로지스틱 MSMR 과 대조 피팅. 이전 단일상 실패(R²=0.79)의 Ω>2RT+Maxwell 정정판.

| 모델 | R² | 파라미터 |
|---|---|---|
| (A) 로지스틱 MSMR | 0.938 | w_j(mV)=[**37.0**, 2.08, 1.33, 3.19] |
| (B) 정칙용액+Maxwell | **0.943** | **Ω_j/RT=[4.06, 2.02, 3.55, 4.07]**(전부>2RT=두-상)·δ_j(mV)=[1.6,1.48,1.55,1.01] |

**해석(정직):**
1. **R² 개선은 미미(+0.5%)** — 자유폭 로지스틱은 이미 sharp 피크를 커널로 대부분 맞춤(로지스틱 피크 ξ(1−ξ)/w = sech² 커널 자체). near-delta 천장은 실재하나 full-curve 잔차서 작음.
2. **진짜 가치=물리 파라미터 의미·일관성**(=사용자 산업목표): 정칙용액은 **전 전이를 단일 해석가능 Ω 로** — Ω_j/RT=[4.06,2.02,3.55,4.07] 전부 >2RT(두-상 확증)·**Cordoba DFT 앵커 ~2.5RT 정합**·0.140V 전이가 Ω=2.02(임계 바로 위, x_α=0.414=최대 고용체성) = **Dahn 4↔3 고용체 예외 그림과 독립 일치**. 반면 로지스틱은 **w=37mV(비물리, 배경/중첩 흡수) vs 1.3mV** 로 뒤섞임 → 해석 불가.
3. **그림 증거**: 로지스틱 w=37mV 1전이가 0.227V 피크를 뭉개 **놓침**(청색 잔차 +10@0.227V); 정칙용액은 δ=1mV sharp 로 포착. 즉 정칙용액은 **폭이 다른 피크들을 하나의 물리량(Ω)으로 일관 처리**.

**→ 판정 정련: 정칙용액+Maxwell 는 R² 아닌 *파라미터 물리성·일관성*(사용자 핵심요구) 때문에 채용가치.** Ω>2RT 는 DFT 앵커가능·전이간 일관·해석가능; w 는 shape 파라미터일 뿐. **단 near-delta 천장 자체는 두-상이 원래 delta 라 kinetic 폭과의 합성이 정답이며, R² 극적개선은 아님**(정직 한계).
