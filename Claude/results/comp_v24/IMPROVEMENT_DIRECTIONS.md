...
### ★1순위 [저위험·최고효율] 전이 수 4→5(또는 6), canonical stage 전위에 고정 — **✅ 반영됨(opt-in, 709d9e9 후속)**
> **반영**: 문헌 6-gallery(ad2061 ω_j·U0_j)를 `GRAPHITE_STAGING_MSMR6_LIT` 로 코드에 **additive opt-in** 추가(기본 4전이 bit-exact 무변경·G1 0.0 보존). 문건 §5b(d)·keybox 에 해상도 사다리{4·5·6}+gallery≠상 구분 명시. 검증: 전이6·유한·비음·ω 검산 정확. (기본 승격은 bit-exact 파기라 미채택 — 사용자 결정.)
