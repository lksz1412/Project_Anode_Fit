# -*- coding: utf-8 -*-
# graphite_ica_model.py — Ch.1 (v3) 의 식만으로 구성한 dQ/dV 모델.
...
def U_branch(T, U, Omega, gamma, sigma_d, h_eta=1.0):
    """(1.82) 분기 중심 U^d. 부분 cycle 이면 gamma -> h(eta)*gamma (M1)."""
    return U + sigma_d * 0.5 * (h_eta * gamma) * dU_hys(T, Omega)


def xi_eq(Vn, Ud, w):
    """(1.26) 의 꼴 — M2. 중심은 분기 중심 U^d, 폭 w 는 S1 동결값
...
def ln_Lq(T, I_abs, Q_cell, dHa_eff, b, chi_d, A_d):
    ...
    chi_d: 방전이면 chi, 충전이면 1-chi — 같은 전이상태의 합-1 강제(1.19);
...
def dQdV_app(V_app, T, I_abs, Q_cell, sigma_d, par):
    """(1.87) 양방향 통합식의 평가 — M1~M6. 반환 단위 [Q_cell/V].
    ...
    파라미터 {Va, dVdq_qa, r_a, b}는 방향별 독립이다(1.87) — Omega!=0
...
def s1_residual(theta, V, y_meas, Np):
    ...
    저율(r_a -> 0)이라 (1.70) 의 종 항만 남는다. 배경을 상수 하나로
...
# S3: (V_drive, ln L_q) 점들의 직선 기울기 = -chi*F/RT        ... (1.60)
#     (방전 s=+1, S0-pass 활성화 지배 극한 — 직렬 감쇠 인자 1)
# S4: y(T) = ln(T*/(L_q T)) - chi*A/RT 를 1/T 로 회귀 —
#     기울기 = -dHa_eff/R, b = -절편                  ... (1.63)·(1.64)
# S5: 절편(T) 3점 -> (gamma, Omega) 2-파라미터 비선형 LSQ      ... (1.84)
