...
def U_branch(T, U, Omega, gamma, sigma_d, h_eta=1.0):
    """(1.91) 분기 중심 U^d. 부분 cycle 이면 gamma -> h(eta)*gamma (M1)."""
...
def dQdV_app(V_app, T, I_abs, Q_cell, sigma_d, par):
    """(1.96) 양방향 통합식의 평가 — M1~M6. 반환 단위 [Q_cell/V].
    ...
    파라미터 {Va, dVdq_qa, r_a, b}는 방향별 독립이다(1.96) — Omega!=0
    ...
    par 의 구성 (괄호는 그 값을 닫는 단계):
      ...
      transitions  전이 dict 리스트:
        U, w, Q        평형 3량 (1.51)/(1.32) (S1)
        dHa_eff, b     (1.71)+M3 (S4; Omega!=0 전이는 chi_d*Omega 흡수
                       유효값 — S5 후 dHa = dHa_eff + chi_d*Omega 복원)
        Omega, gamma   (1.88)/(1.91) (S5; 비분기 전이는 0)
    ...
    for tr in par['transitions']:
        ...
        tail = (ra / LV) * np.exp(
            np.where(sup, dv, -np.inf))                # (1.96) 꼬리
...
def s1_residual(theta, V, y_meas, Np):
    ...
    저율(r_a -> 0)이라 (1.79) 의 종 항만 남는다. 배경을 상수 하나로
