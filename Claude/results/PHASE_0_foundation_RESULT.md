...
## Summary
RB 재구성의 토대 3종 확정: cross-chapter 규약 동결(0.1), 참고문헌 grounding+DOI 검증(0.2), 통합 AL 체계+notation+가독gate 골격(0.3). 5-30 사용자 경고("무근거 무리한 가정")가 실제로 적중 — stretched-tail 핵심 물리 근거 문헌이 \emph{저자 오귀속}(폰 macdonald2000 → 실제 Svare/Martin/Borsa, DOI·물리는 유효)임을 적발·정정. funabiki title↔locator 혼합도 적발.
...
- 문제 집중 재검증: macdonald2000 → 워크플로 web 실측 결과 DOI 10.1103/PhysRevB.61.228 의 제목·권·페이지(228–233)·물리 \emph{전부 정확}, \textbf{저자만 Macdonald→Svare/Martin/Borsa 오귀속} → 키 svare2000 으로 정정·유효 사용(1차 기록의 "제거" 는 GS-5 로 철회). johnston2006(WebFetch 10.1103/PhysRevB.74.184430 verbatim 일치) 보강 추가. funabiki = title(JES 146,2443; 10.1149/1.1391953)↔locator(Electrochim. Acta 45,865; 10.1016/S0013-4686(99)00290-X) 혼합 → 둘 다 등재(D7). lee2011/son2013/lindsey1980 DOI 정정.
- jcp2017 DOI 10.1063/1.5000882 원문 직접 확인.

## Validation (gate 별)
- **PASS_CHARTER (0.1)**: 규약 5종 전부 "확인 가능한 조건" 1행씩 기술. G-cross 판정 기준 확정.
- **PASS_DOSSIER (0.2)**: 30종 DOI web 실측. CONFIRMED 다수 / CORRECTED(doyle·reynier·ohzuku·funabiki·dubarry·lee2011·son2013·lindsey1980 DOI·서지) / BOOK 5(newman·brenan·bardfaulkner·degrootmazur·prigogine·mckinnon) / 저자정정 1(macdonald→svare2000) / NOT_FOUND 1(plonka, FLAGGED). 환각 cite 0(전건 web 근거; 미확인은 정직 표기).
...
- macdonald2000 → svare2000 \emph{저자 정정}(문헌은 유효); 실제 tex 반영은 Ch1 재구성(Phase 1.3) 시.
- Codex 산출물 미read.
