% ====================================================================
% graphite_ica_ch1_v8-02b.tex — Chapter 1 (v8-02b: v8-02 증분 보완판)
%   베이스 = v8-02 (유도 과정 포함 교과서 확장판).
%   보완 4건:
%     D-PEAK : L_V→0 극한 서술 교정(eq:branch 스위치 역할 명시, L_V≫Δgrid 환원 전개).
%              기존 "한 칸 뒤처진 ξ_eq" 오기 삭제.
%     D-DHEFF: ΔH_a^eff = ΔH_a − χ_d Ω 로 가는 χ_d 계수 −χ_dΩ 경유 중간식 보강.
%     D-WEFF : w_eff 중심기울기 다리 sF·dξ/dV|_{1/2} = 4F w_eff^{-1} 중간식 추가.
%     D-VEQ  : eq:Veq 의 sF(V_eq−U)=g'(ξ) 다리를 inline 으로(forward-defer 제거).
%   부호   = 브리프 §7 8항 전건 PASS (R3 self-test D-PEAK 일관 업데이트).
...
\hypersetup{colorlinks=true, linkcolor=blue, urlcolor=blue, citecolor=blue,
  pdftitle={흑연 음극 dQ/dV 이론: 코드 진행을 따라가는 교과서 확장판 — Chapter 1 (v8-02b)}}
\pagestyle{fancy}\fancyhf{}
\lhead{흑연 음극 $dQ/dV$ 이론 — Ch.1 (v8-02b, 유도 확장판)}\rhead{\thepage/\pageref{LastPage}}
...
\title{\textbf{\LARGE Chapter 1}\\[0.35em]\Large 흑연 음극 $dQ/dV$ 이론: 코드 진행을 따라가는 유도 확장판 (v8-02b)}
\author{}
\date{}
...
평형 전위 곡선은 평형 조건 $sF(V_{\eq,j}-U_j)=g_j'(\xi)$ 에서 나온다.
식~\eqref{eq:gxi} 의 $g_j'(\xi)=RT\ln[\xi/(1-\xi)]+\Omega_j(1-2\xi)$ 를 대입하고 $s=+1$ 로 두면:
\begin{equation}
...
\label{eq:wbase}
\end{equation}
코드 \code{func\_w(T, n)} 의 \code{n*R*T/F}. 상호작용이 있을 때($\Omega_j\ne0$) 식~\eqref{eq:gxi} 의
$g_j''(\xi)=RT/[\xi(1-\xi)]-2\Omega_j$ 를 평형 중심 $\xi=\tfrac12$ 에서 평가하면
\begin{equation*}
g_j''\!\Big|_{\xi=1/2}=4RT-2\Omega_j,
\end{equation*}
logistic 중심 기울기는 $\dd\xi/\dd V|_{1/2}=sF/[g_j''|_{1/2}]$ (연쇄율) 이므로, 유효 기울기에 대응하는 폭은
$sF\cdot w_j^\eff=g_j''|_{1/2}$, 즉
\begin{equation}
w_j^\eff=\frac{RT}{F}\Big(1-\frac{\Omega_j}{2RT}\Big).
\label{eq:weff}
\end{equation}
$\Omega_j\to2RT$ 에서 $w_j^\eff\to0$ 으로 가므로 코드는 $\mathrm{floor\_frac}\cdot RT/F$ 하한으로 클립한다.
...
$r_j^+/r_j^-=e^{\mathcal A_j/RT}$ 가 항등식이 되려면 지수 지수부 차가 정확히 $\mathcal A_j/RT$ 이어야 하므로
$\chi+(1-\chi)=1$이 요구된다($\forall\mathcal A_j$).

\textbf{$\Delta H_{a,j}^\eff$ 경유.} 깊은 꼬리 $\xi\to1$(방전) 에서 정규용액 상호작용 에너지 $\Omega_j$ 의 상수 기여
$-\Omega_j(1-2\xi)\approx+\Omega_j$ 가 순방향 장벽에 상수 몫으로 흡수된다. 정방향 속도에서
$-\chi\mathcal A_j$ 경로를 통해 방향별 계수 $\chi_d$ 가 $\Omega_j$ 를 받으면:
\begin{equation*}
\Delta G_{a,j}^\eff \equiv \Delta G_{a,j} - \chi_d\,\Omega_j
\;\Longrightarrow\;
\Delta H_{a,j}^\eff = \Delta H_{a,j} - \chi_d\,\Omega_j.
\end{equation*}
충전은 $\xi\to0$ 거울, $\chi_d=1-\chi$ 가 같은 역할을 맡는다.
\end{derivebox}
\begin{equation}
...
두 극한이 이 식의 물리를 확정한다.
\emph{극한 (i): $L_V\to0$,} 즉 $\rho=e^{-\Delta_\mathrm{grid}/L_V}\to0$. 점화식~\eqref{eq:lowpass} 에서
$\xi_{\mathrm{lag},i}\to(1-0)\,\xi_{\eq,i}=\xi_{\eq,i}$, 즉 지연이 평형을 \emph{즉시} 추종하므로
$\xi_\eq-\xi_\mathrm{lag}\to0$. 그런데 분모 $L_V$ 도 $\to0$ 이고, 코드는 $L_V<\nu\,\Delta_\mathrm{grid}$ 이면
식~\eqref{eq:branch} 의 eq:branch 스위치로 평형 종(식~\eqref{eq:eqpeak})을 직접 반환해 $0/0$ 비결정을 우회한다.

\emph{극한 (ii): $L_V\gg\Delta_\mathrm{grid}$,} 즉 $\rho\to1$. 이산 점화식~\eqref{eq:lowpass} 를 연속 극한으로 전개하면
$(1-\rho)\approx\Delta_\mathrm{grid}/L_V\ll1$ 이므로
\begin{equation*}
\xi_{\mathrm{lag},i}\approx\xi_{\mathrm{lag},i-1}+\frac{\Delta_\mathrm{grid}}{L_V}(\xi_{\eq,i}-\xi_{\mathrm{lag},i-1}),
\end{equation*}
인접 격자점 차 $\xi_{\eq,i}-\xi_{\mathrm{lag},i-1}\approx\xi_{\eq,i}-\xi_{\eq,i-1}\approx(\dd\xi_\eq/\dd V)\Delta_\mathrm{grid}$,
따라서 $(\xi_\eq-\xi_\mathrm{lag})/L_V\to(\dd\xi_\eq/\dd V)(\Delta_\mathrm{grid})^2/L_V^2$ 가 아니라,
$\rho\to1$ 충분히 느린 상태에서 $\xi_\mathrm{lag}$ 가 한 격자 뒤의 $\xi_\eq$ 에 해당하는 연속 접근에서
$(\xi_\eq-\xi_\mathrm{lag})/L_V\approx\dd\xi_\eq/\dd V$ 로 수렴한다 — 매끈한 평형 종 환원이다. 이 극한이
식~\eqref{eq:eqpeak} 로의 매끄러운 환원 경로이며, 코드의 eq:branch 스위치는 반대 극한($L_V\to0$)에서의 평형 스위치다.
\begin{equation}
\text{peak\_shape}=
\begin{cases}
\xi_\eq(1-\xi_\eq)/w & L_{V,j}<\nu\,\Delta_\mathrm{grid}\quad[\text{eq:branch 스위치 — 평형, 식~\eqref{eq:eqpeak}}]\\[2pt]
(\xi_\eq-\xi_\mathrm{lag})/L_{V,j} & \text{그 외}\quad[\text{꼬리, 식~\eqref{eq:peakshape}}],
\end{cases}
\label{eq:branch}
\end{equation}
$\nu=\code{min\_lag\_grid\_steps}=2$ 다. 이 스위치는 $L_V\to0$ 극한의 $0/0$ 비결정을 평형 해석값으로 채우는 역할을 한다(위 극한 (i) 논의 참조). $L_V\gg\Delta_\mathrm{grid}$ 인 반대 극한은 꼬리 분기가 자연스럽게 매끄러운 종으로 수렴한다(위 극한 (ii)).
...
\item \textbf{$|I|\!\to\!0$ 환원.} $|I|=10^{-6}$ 에서 $L_V\propto|I|\to0$, $\rho=e^{-\Delta_\mathrm{grid}/L_V}\to0$ 이므로 점화식~\eqref{eq:lowpass} 에서 $\xi_\mathrm{lag}\to\xi_\eq$(즉시 추종). 코드는 $L_V<\nu\,\Delta_\mathrm{grid}$ 조건에 걸려 eq:branch 스위치로 평형 종 식~\eqref{eq:eqpeak} 를 반환, 방향 불변. $\checkmark$
