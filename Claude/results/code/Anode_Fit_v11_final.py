...
#     · 남은 매직넘버 전부 인자화(w_eff_floor_frac·grid_pad_lo/hi·n_work_min·min_lag_grid_steps)
#     · per-transition override(z_cut·A_cap_RT·use_w_eff·use_dH_eff) — "모든 인자 입력" 완성
...
#     · 폭          w = nRT/F  (옵션 w^eff = (RT/F)(1−Ω/2RT))       (func_w/eq:weff)
...
def func_w_eff(T: ScalarOrArray, Omega: float, floor_frac: float = 0.05) -> ScalarOrArray:
    """유효 폭 w^eff = (RT/F)(1−Ω/2RT) (eq:weff). 상호작용이 종을 좁힘.
    Ω→2RT 에서 0, Ω=0 에서 RT/F 회복. floor_frac·RT/F 하한 클립(인자화)."""
    w_eff = (R * T / F) * (1.0 - Omega / (2.0 * R * T))
    floor = floor_frac * R * T / F   # RT/F 의 floor_frac 하한(폭 0 → 발산 방지)
    return np.maximum(w_eff, floor)
...
class GraphiteAnodeDischargeDQDV:
    ...
    use_w_eff : bool           : w^eff(Ω) narrowing 적용(기본 False; eq:weff,
                                 폭은 기본 w|n 우선 — Ω-narrowing 은 명시 켤 때만)
    ...
    def __init__(self, transitions: List[Dict[str, Any]], x: float = 0.5,
                 ...
                 use_dH_eff: bool = True, use_w_eff: bool = False,
                 z_cut: float = 4.357, A_cap_RT: float = 4.0,
                 w_eff_floor_frac: float = 0.05,
        ...
        self.use_w_eff = bool(use_w_eff)
        self.z_cut = _finite_pos("z_cut", z_cut)
        self.A_cap_RT = _finite_pos("A_cap_RT", A_cap_RT)
        self.w_eff_floor_frac = _finite_pos("w_eff_floor_frac", w_eff_floor_frac)
    ...
    def _width(self, tr: Dict[str, Any], T: ScalarOrArray) -> ScalarOrArray:
        """전이 폭 w [V]. 기본 = nRT/F(=func_w). use_w_eff 시 Ω-narrowing(eq:weff)."""
        use_weff = bool(tr.get('use_w_eff', self.use_w_eff))
        if use_weff and (tr.get('Omega') is not None) and (tr.get('w') is None):
            return func_w_eff(T, float(tr['Omega']), self.w_eff_floor_frac)
...
if __name__ == "__main__":
    ...
    model = GraphiteAnodeDischargeDQDV(
        ...
        use_dH_eff=True, use_w_eff=False)
