...
## ★ D-PEAK2 (문턱 불연속 — v8-07b 자체 적대검수 심화, 최종 v8-11 정확 기술)
D-PEAK 정정의 *방향*은 옳으나(L_V→0⇒peak→0·L_V≫Δgrid⇒dξ_eq/dV), 다음을 정확히:
- **eq:branch 는 매끈한 극한이 아니라 이산 *모드 스위치***(L_V<νΔgrid, ν=2 → 평형 종 ξ_eq(1−ξ_eq)/w; else → 꼬리식). 코드 L466–479 그대로.
- ★문턱 ν=2 에서 두 분기 **진폭 불일치**: 꼬리분기 factor g(0.5)=0.77·s_max vs 평형 종(높이 ∝1/w) — *불연속*. "매끈한 handoff" 로 호도 금지 → "수치 안정 위한 실용 모드 전환, 문턱서 작은 진폭 점프 가능" 으로 정직 기술.
- 꼬리식 자체의 L→0 소멸(peak→0)은 *eq-분기 영역(L<Δ)* 이라 코드가 꼬리식으로 실행 안 함 — "꼬리식 극한"으로 적지 말고 "eq-분기가 인수" 로.
- 범위 충돌 주의: eq:peakshape(B) 를 무조건 dξ/dV≈r/L 로 적고 12줄 뒤 large-L 한정하면 모순 — 한 곳에서 조건 명시.
**적용**: v8-11 의 §peak_shape/§tail 을 위 정직 기술로(체리픽 베이스가 v8-09b/06b 라도 이 문턱 불연속 1–2문장 보강).
