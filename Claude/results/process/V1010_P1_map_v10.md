# Anode_Fit v1.0.10 — P1 통합본 (vN-10 체리픽 조립)
...
|---|---|---|---|
| **z_cut=4.357 의미** | docstring(L218) "ξ_eq 5%" | ξ_eq(z=4.357)=**98.73%** (1−ξ=1.27%); **정규화 미분** ξ(1−ξ)/0.25=**5.00%** | **★ docstring 부정확** — "5%"는 점유율이 아니라 **정규화 미분 ξ(1−ξ)/0.25의 5%**(→ §2 C1, ★정정 2) |
...
| **극한: γ=0** | hys_shift 미실행(L444 else) → center=U_j | 방·충 동일 | ✅ |
...
| ID | 심각도 | 위치 | 내용 | 근거 |
|---|---|---|---|---|
| **C1** | LOW(문서) | L218 docstring | **z_cut docstring 부정확** — "기본 4.357 = ξ_eq 5%"는 틀림. 실제 = **정규화 미분 ξ(1−ξ)/0.25의 5%**(ξ_eq 자체는 98.73%, 1−ξ=1.27%). tex(L1369-70)는 정확. **코드 docstring만 부정확** | §0 검산 (★정정 2) |
...
> [C3 추정·주의] 기본 동역학 seed L_V는 매우 작아(4.91e-08~4.75e-10 V) 일반 grid에서 sub-grid 판정(L462-465)에 걸려 평형 peak로 접힘 — rate tail 피팅하려면 L_V 직접 fit 또는 동역학 파라미터 재스케일 필요(보완 3의 해상도 의존성과 연동).
...
- **★정정 2**: z_cut docstring 부정확(L218 "ξ_eq 5%" 틀림 → 정규화 미분 ξ(1−ξ)/0.25의 5%; ξ_eq 자체 98.73%). (§0·§2-D C1)
...
*조립: 2026-07-01 | vN-10 체리픽 통합본 | 파일: V1010_P1_map_v10.md*
