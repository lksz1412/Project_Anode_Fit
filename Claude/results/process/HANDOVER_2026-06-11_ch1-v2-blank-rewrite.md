...
## Addendum (같은 날 추가 — V.E: 예시 구현 절 + E.R 10회)

### 추가 지시 (2026-06-11 /goal)
"저 문건만 보고 추가 지식 없이 파이썬으로 피팅 식을 세울 수 있나? 가능하면 제대로 된 예시 코드를 마지막 절로 추가(피팅 자체는 사용자 몫 — 모델 함수가 핵심). 동일 방법론·작업 순서. 검수·수정·내용 추가 10회. 완전성·물리 논리 검토 포함. 전문 생략 없이." + 작업 중 교정 2건: **검토를 grep 으로 하지 말 것(전문 정독만)**, **같은 전문을 렌즈별로 따로 N회 돌지 말 것(1회 정독에 렌즈 통합)** — `feedback_full_file_read_required` 메모리에 반영 완료.

### 수행 결과
- §1.17 "예시 구현 — 모델 함수에서 round-trip 까지" 신설(마지막 절): M1–M6 직역 파이썬(식 번호 주석 1:1)+단계 회귀 골격+실행 검증·대응표. **수록 코드=실행 파일(분할 diff 게이트)**, 실행 검증 PASS(ΔU^hys 54.8mV·방전/충전 면적 0.3998/0.4000·S1 회복 ±0.25mV). 코드·로그: `results/V2_E_codeexample/`.
- E.R 10회(ER1–ER10): 종단 물리 적대(Fable)+Codex 3·4차 병행 — **충전 가지 ln_Lq 파손을 두 검수가 독립 수렴 적발**(방향형 A_d·χ_d=1−χ 로 수정+충전 시험 추가), 울타리 ⑪–⑭ 확장, 공존-plateau 회수 별표, 분기 전이 유효값 변별 지문, Ω 복원 χ_d 방향별, fresh-eyes 전문 통독 27건 일괄 등. 최종 39p, 0/0/0, 커밋 723a85a 까지 전부 push.
- 챕터 전체 누적 검수 = V.R 14 + E.R 10 = **24회**.

### 미완·주의 (변동 없음 + 추가)
- 사용자 본인 검수 대기. ch3/ch4 (1.x) 재매핑은 v2 확정 후 별도 phase.
- §1.17 의 verbatim 은 `results/V2_E_codeexample/graphite_ica_model.py` 와 분할 diff 게이트로 동기 — **코드를 고치면 반드시 tex verbatim 재동기+diff 재실행**(스크립트 패턴은 Temp v2_er10_tex.py 참조).
- 검토 작업은 전문 정독 1회+렌즈 통합으로만(메모리 갱신분 — grep 은 빌드 카운트 게이트 한정).
