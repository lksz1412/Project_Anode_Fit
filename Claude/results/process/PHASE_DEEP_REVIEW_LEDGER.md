...
### Ch2 (최대 effort 재패스 — 완료, 11p clean)
- **§서론**: 과냉각 비유 clean. **§1**: 표 손검, 인력적→동종선호 시정(완료).
- **§2**: g″=RT/[ξ(1−ξ)]−2Ω 재유도·T_c=Ω/2R·공통접선=등μ·lever rule·vdW(전위↔−P)·CNT(ΔG*∝γ_s³/Δg²)·세 영역·과냉각 비유 **전수 손검 정확**. finding: 오타 "볶록"→"볼록" 시정(완료).
- **§3**: gap 분해(진짜/가짜) clean. **§4**: V_eq 부호반전·g″·spinodal ξ_s± 손검 정확. **§5**: ΔU_hys 두 분기차(−4artanh u·2u)·54mV 손검 정확.
- **§6~§11**: 분기 dQ/dV·과전압 분해·Preisach·param·식별성(§10 시정)·master 식(eq:hys_master(_center) 환원검산) 손검 정확.
- **Ch2 종합(최대 effort): clean, 11p. 신규 시정 = §2 볶록 오타.**

## 최대 effort 패스 종합 (양 챕터)
- Ch1 24p·Ch2 11p, overfull 0·undefined 0.
- 작업이력 누락·왜곡 0(전수 감사).
- 신규 시정 5건: Ch1 §7 잔여 C2-1 표기, Ch1 §8 r_{a,j} 정의역, Ch2 §1 인력적, Ch2 §10 Ω/γ 식별성, Ch2 §2 볶록 오타.
- 핵심 유도 전부 master 직접 손검(재유도) 통과. Codex는 본 세션 신규 내용 3회 검수 완료(이전 커밋).
