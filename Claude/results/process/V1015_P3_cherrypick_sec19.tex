...
다음 소절이 닫는다. (좌표는 기억 적분의 도식 평가이며, 점별 적분으로의 수치 재산출은 P6 소관.)}
\label{fig:relaxode}
\end{figure}
...
(식~\eqref{eq:belliden})가 $V,L_{V,j}$ 에 무관하게 성립하는 지배함수이므로, $L_{V,j}\to0$ 에서 각
$t$ 마다 평가점 $V-L_{V,j}t\to V$ 이고 $\dd\xi_{\eq,j}/\dd V$ 는 연속이라 극한과 적분을 바꿀 수 있다
(지배수렴정리):
\begin{equation}
...
=\sigma_d\,\frac{\xi_{\eq,j}(1-\xi_{\eq,j})}{w_j}\;.}
\label{eq:tail-limit}
\end{equation}
우변의 크기가 정확히 식~\eqref{eq:eqpeak} 의 평형 종이다 --- 꼬리식(식~\eqref{eq:peakshape})은 모든
$L_{V,j}>0$ 에서 하나의 표현이고, 평형 종은 그 $L_{V,j}\to0$ 해석적 극한이다. 지배함수
$1/(4w_j)$ 가 $L_{V,j}$ 에 무관한 상수라 이 수렴은 매끈하다 --- $\xi_{\eq,j}$ 자체도 매끈해 극한값이
어떤 문턱에서의 전환이 아니라 극한 그 자체로 닫힌다. $|I|\to0$ 이면 $L_{V,j}\propto|I|\to0$(식
~\eqref{eq:LV})이므로, 이 극한이 곧 저전류 기준선으로 가는 길이다.
...
(좌표는 이전 판의 수치를 유지하며, 점별 적분으로의 재산출은 P6 소관.)}
\label{fig:reversal}
\end{figure}
