% ====================================================================
% graphite_ica_ch1_v8-03b.tex — Chapter 1 (v8-03b: v8-03 증분 보완판)
%   v8-03 전체 보존 + 5개 보완:
%   (1) D-PEAK: L_V→0 혼동 정정 — 종 환원은 eq:branch 스위치 담당 명시.
%   (2) D-VEQ: eq:Veq 앞 sF(V_eq−U)=g'(ξ) 유도 다리 추가.
%   (3) D-DHEFF: χ_d 계수 중간식(BEP 상수 몫 흡수 과정) 보강.
%   (4) D-WEFF: eq:weff 앞 중심기울기 4Fw 다리 추가.
%   (5) orphan fig: fig:logistic·fig:reversal 본문 \ref 보강.
...
\textbf{(a) 출발: 비단조 평형 전위 곡선.} 자유에너지~\eqref{eq:gxi} 에서 $g_j'(\xi)=\partial g_j/\partial\xi$ 를 구하면
\begin{equation}
g_j'(\xi) = RT\ln\frac{\xi}{1-\xi} + \Omega_j(1-2\xi),
\label{eq:gder}
\end{equation}
이 식을 평형 조건 $sF(V_{\eq,j}-U_j)=g_j'(\xi)$ 에 대입해 $V_{\eq,j}$ 에 대해 풀면 곧바로
\begin{equation}
...
\label{eq:wbase}
\end{equation}
상호작용($\Omega_j\ne0$)이 있으면 중심기울기가 줄어 유효 폭이 넓어진다. 이상 logistic 의 중심기울기는
$sF/4$이고, 식~\eqref{eq:gpp} 에서 $g_j''(\xi)|_{\xi=1/2}=4RT-2\Omega_j$ 이므로 이상 기울기가
$(4RT-2\Omega_j)/RT$ 배로 줄어 등가 폭이
\begin{equation}
w_j^\eff=\frac{RT}{F}\Big(1-\frac{\Omega_j}{2RT}\Big)
\label{eq:weff}
\end{equation}
이 된다. $\Omega_j\to2RT$ 에서 $w_j^\eff\to0$ 이 됨에 주의한다 — 동시에 $u_j\to0$ 으로 spinodal 문턱과 정합한다.
...
그림~\ref{fig:logistic} 이 $\xi_\eq(z)$ logistic 곡선과 규격화된 미분 $\xi_\eq(1-\xi_\eq)$ — 다음 절에서
평형 peak 모양이 되는 종 — 을 나란히 보인다.
...
\label{eq:chid}
\end{equation}
BEP 비대칭 장벽의 꼬리 끝 거동을 자세히 보면: 방전의 깊은 꼬리($\xi\to1$)에서
$\mathcal A_j=sF(V-U_j)-\Omega_j(1-2\xi)\to sF(V-U_j)+\Omega_j$ 이고,
충전의 깊은 꼬리($\xi\to0$)에서 $\mathcal A_j\to sF(V-U_j)+\Omega_j$ 로 똑같다.
따라서 두 경우 모두 $\Omega_j$ 가 전진 장벽을 $\chi_d\Omega_j$ 만큼 낮추어
유효 활성화 엔탈피가
\begin{equation}
\boxed{\;\Delta H_{a,j}^\eff=\Delta H_{a,j}-\chi_d\,\Omega_j\;.}
\label{eq:dHeff}
\end{equation}
가 된다. 방전($\chi_d=\chi$)과 충전($\chi_d=1-\chi$)은 $\chi=0.5$ 에서 같은 장벽, $\chi\ne0.5$ 에서
서로 다른 유효 장벽을 가져 비대칭 꼬리 폭을 만든다.
...
\label{eq:peakshape}
\end{equation}
$L_V$ 가 격자 간격의 $\nu$ 배 미만이면 저역통과 한 칸 이동 근사
$\xi_\mathrm{lag}\approx\xi_\eq - L_V\,\dd\xi_\eq/\dd V$ 가 유효하여
$(\xi_\eq-\xi_\mathrm{lag})/L_V\approx\dd\xi_\eq/\dd V = \xi_\eq(1-\xi_\eq)/w$ 로 평형 종에 수렴한다.
\emph{주의}: ``$L_V\to0$ 이라 peak $\to0$''이 되는 것이 아니다 — $L_V\to0$ 이면 꼬리 폭이 $0$ 이 되어
peak 크기가 평형 종으로 \emph{매끄럽게 이어진다}. 격자 해상도 $\nu\,\Delta_\mathrm{grid}$ 보다
$L_V$ 가 작을 때의 수치 불안정(분모 $\approx0$)을 막는 것이 eq:branch 스위치의 실제 역할이다:
\begin{equation}
\text{peak\_shape}=
\begin{cases}
\xi_\eq(1-\xi_\eq)/w & L_{V,j}<\nu\,\Delta_\mathrm{grid}\ \text{(평형 종 — 수치 안전)}\\[2pt]
(\xi_\eq-\xi_\mathrm{lag})/L_{V,j} & \text{그 외 (꼬리 종)},
\end{cases}
\label{eq:branch}
\end{equation}
$\nu=\code{min\_lag\_grid\_steps}=2$ 다. 매끄러운 종 환원은 $L_V\gg\Delta_\mathrm{grid}$ 인 경우가 아니라
$L_V \ll \Delta_\mathrm{grid}$ 에서 코드 분기 없이 연속으로 일어난다.
...
그림~\ref{fig:reversal} 이 방전과 충전의 꼬리 방향을 나란히 보인다 — 두 경우 모두 peak 는 양수이며 충전이 방전의 거울이다.
...
이 때 $L_V<\nu\,\Delta_\mathrm{grid}$ 로 eq:branch 스위치가 평형 분기를 선택해
peak 가 $\xi_\eq(1-\xi_\eq)/w$ 로 떨어지며 $\sigma_d$ 불변.
(\emph{정정}: $L_V\to0$ 이 peak $\to0$ 을 뜻하지 않는다 — 스위치가 평형 종으로 \emph{대체}함.)
