...
# ---------- 합성 풀셀 측정 ----------
AN_true = build_electrode_curves(V_AN, AN_TRS, CBG_AN, +1.0, 1.0, OFF_AN)
...
for k in idx_w:       lb[k], ub[k] = 3.0, 90.0          # w
for k in idx_Q:       lb[k], ub[k] = 0.01, 1.2          # Q
...
# (dQ/dV 채널은 작은 dQ/dV 영역서 잡음 증폭 — §1.16 (5) 권고대로 형상 정밀화엔 V+dVdQ 사용.)
stages = [
    ('P1 coarse align (V)', idx_Q + [OFF_CT_IDX] + idx_U,         W_coarse),
    ('P2 +dVdQ fine',       idx_Q + [OFF_CT_IDX] + idx_U,         W_fine),
...
AN_f = build_electrode_curves(V_AN, an_fit, CBG_AN, +1.0, 1.0, oa)
