...
\hypersetup{colorlinks=true, linkcolor=blue, urlcolor=blue, citecolor=blue,
  pdftitle={흑연 음극 + LCO 양극 dQ/dV 이론: 코드 진행을 따라가는 수식-구동 (유도 확장, LCO 통합) — Chapter 1 (v9)}}
\pagestyle{fancy}\fancyhf{}
\lhead{흑연$\cdot$LCO $dQ/dV$ 이론 — Ch.1 (v9, 코드 진행 + 유도)}\rhead{\thepage/\pageref{LastPage}}
...
\title{\textbf{\LARGE Chapter 1}\\[0.35em]\Large 흑연 음극 $\cdot$ LCO 양극 $dQ/dV$ 이론: 한 곡선을 그리는 코드 진행을 따라가는 수식-구동판 — 유도 확장(LCO 통합)}
\author{}
\date{}
...
\subsection{두 번째 전극 — LCO 양극을 같은 프레임에}\label{sec:lco-intro}
앞 절까지의 기호$\cdot$매핑은 \emph{전극을 가리지 않는다} — 분극$\cdot$평형 중심$\cdot$logistic 폭$\cdot$히스 분기$\cdot$꼬리는
삽입형 전극의 보편 forward 모델이고, 흑연은 그 모델의 첫 사례다. 본 문건은 같은 프레임에 \emph{두 번째 전극} —
LiCoO$_2$(LCO) 양극을 코인 반쪽전지(vs Li/Li$^+$) 범위에서 — 짜 넣는다. 두 전극은 두 가지만 다르다: (i) 전이
파라미터의 \emph{값}(LCO 는 $U_j\sim3.9$--$4.2$ V 의 고전위 전이), (ii) LCO 에만 있는 \emph{전자(electronic) 엔트로피
항}(흑연엔 0; \S\ref{sec:electronic} 가 도입). 구조식은 흑연과 한 글자도 다르지 않다 — 식~\eqref{eq:Uj}$\cdot$
\eqref{eq:xieq}$\cdot$\eqref{eq:eqpeak} 가 그대로 LCO 에 쓰이고, 파라미터만 갈린다(\S\ref{sec:lco-sum} 의 코드 일반화 H 가
이를 닫는다).

\textbf{양극 부호 규약(흑연과 1:1).} LCO 반쪽전지에서 전위는 양극 전위(vs Li/Li$^+$)이고, 셀의 \emph{방전}은
LCO 의 \emph{리튬화}(Li$^+$ 가 LCO 로 들어옴), 충전은 \emph{탈리튬화}다 — 흑연 음극과 셀-방향의 의미가 정반대다.
forward 식의 부호 구조를 흑연과 글자 그대로 같게 유지하는 열쇠는 \emph{$\xi$ 와 $\sigma_d$ 의 정의를 전극이 아니라
\,$\xi$ 의 진행 방향에 붙잡아 두는} 것이다: 흑연과 동일하게 $\xi_j$ 를 \emph{탈리튬화 진행률}(전위를 올리면 느는 양)로
정의하면, 식~\eqref{eq:xieq} 의 logistic 규칙 ``$\sigma_d=+1$ 에서 $V\!\uparrow\Rightarrow\xi_\eq\!\uparrow$''
(\S\ref{sec:width} 의 signbox)이 양극에서도 \emph{문자 그대로} 성립한다. 다만 그렇게 두면 $\xi$-증가(탈리튬화)에
대응하는 코드 방향 부호는 양극에서 셀-충전이 $\sigma_d=+1$ 이 되어, 셀-방향 라벨(방전/충전)과 코드 $\sigma_d$ 의
대응이 음극과 \emph{뒤집힌다} — 곧 식을 바꾸는 대신 ``어느 셀-동작이 $\sigma_d=+1$ 인가''라는 라벨 매핑만 전극별로
정한다(N0 의 \code{\_direction\_to\_sigma} 가 담당). 이렇게 하면 평형 점유 식~\eqref{eq:xieq} 와 평형 중심
식~\eqref{eq:Uj} 의 $\partial U_j/\partial T=\Delta S_{\rxn,j}/F$ 가 \emph{부호 규칙 그대로} 양극에 적용된다
(\S\ref{sec:center-lco} 가 양극 부호를 식으로 회수). 차이는 부호식이 아니라 \emph{값의 자리}와 셀-라벨 매핑이다 —
LCO 의 $U_j$ 가 $\sim4$ V 로 높을 뿐이다. 흑연 staging(그림~\ref{fig:staging})에 대응하는 LCO 전이는 세 개다
(표~\ref{tab:lcostaging}): 저전압 끝의 절연체$\to$금속 천이(MIT) 한 개와, 중간 조성의 order--disorder 한 쌍.

\begin{table}[t]
\centering
\renewcommand{\arraystretch}{1.25}
\caption{LCO 전이 초기값 \code{LCO\_STAGING\_LIT}(3건, 코인 반쪽전지 vs Li) — 흑연 표~\ref{tab:staging} 와 같은 자리,
값만 양극. $U_j$ 는 tier A(Xia~\cite{xia2007}); $\Delta S_\rxn$ 초기값은 tier B 거친 출발점(단전극 $\dd\phi/\dd T\!\approx\!+0.83$ mV/K
$\approx\!+80$ J/(mol\,K)~\cite{swiderska2019} 전역 평균을 전이별로 배분한 추정 — 연속 $\Delta S(x)$ 표 부재 = G1, 데이터 피팅으로 식별);
절대 $\Delta H_\rxn$ 은 G5 갭(OCV plateau anchor)이라 $U_j$ 직접값을 \code{'U'} 폴백으로 쓴다(흑연과 동일 메커니즘).}
\label{tab:lcostaging}
\begin{tabular}{@{}l c c l c l@{}}
\toprule
전이 & $U_j$ [V] & $x$ 범위 & 성격 & $\Delta S_\rxn$ 초기 & 고유항 \\
 & vs Li & (Li$_x$CoO$_2$) & & [J/(mol\,K)] & \\
\midrule
T1 MIT            & 3.90        & $0.94$--$0.75$ & ins.$\to$metal & $\sim\!+12$ & ★전자 게이트 ON \\
T2 o--d $a$ & 4.05   & $\approx0.55$  & hex$\to$mono & $\sim\!+5$ & config 주도, 2상역 \\
T3 o--d $b$ & 4.17--4.20 & $\approx0.48$ & mono$\to$hex & $\sim\!+15$ & config, T2 와 쌍 \\
\bottomrule
\end{tabular}
\end{table}

표~\ref{tab:lcostaging} 의 셋이 흑연 4 전이와 \emph{대칭 구조}다 — 각 전이가 한 peak 을 남기고, 코드의 전이 리스트가
이 초기값을 담는다(고전압 O3$\to$H1-3 전이 $\sim4.55$ V 는 하프셀 상한 $\le4.2$--$4.5$ V 면 범위 밖이라 옵션이다).
``고유항''열의 $\Delta S_e$ 가 흑연엔 없던 \emph{전자 엔트로피 항}이며, T1 의 MIT 가 그 발현 자리다 —
\S\ref{sec:electronic} 가 그 닫힌식을 분포에서 유도한다. 도핑(Al$^{3+}$/Mg$^{2+}$, 비-redox)은 이 값들을 초기값으로
두고 봉우리를 smear/shift 하는 보정이며(흑연 \code{GRAPHITE\_STAGING\_LIT}=초기값 철학과 동일), \S\ref{sec:lco-sum}
에서 다룬다.
...
\subsection{양극(LCO)의 평형 중심 — 같은 식, 높은 전위}\label{sec:center-lco}
식~\eqref{eq:Uj} 은 전극을 가리지 않으므로 LCO 양극에도 그대로 쓴다 — 다른 것은 입력 $(\Delta H_\rxn,\Delta S_\rxn)$
의 값뿐이다. LCO 의 $U_j$ 가 $\sim3.9$--$4.2$ V 로 흑연($\sim0.1$ V)보다 높은 까닭은 분자 $-\Delta H_\rxn$ 이 더
크기 때문이다(Co$^{3+}$/Co$^{4+}$ redox 의 깊은 음의 반응 엔탈피). 코드는 흑연과 동일하게 $U_j$ 직접값(\code{'U'}
폴백)을 쓰거나 $(\Delta H_\rxn,\Delta S_\rxn)$ 정합으로 환산한다 — 표~\ref{tab:lcostaging} 의 절대 $\Delta H_\rxn$ 은
G5 갭(\S\ref{sec:sum})이라, 본 장은 $U_j$ 초기값을 OCV plateau anchor 로 쓰고 온도 의존만 $\Delta S_\rxn$ 으로 연다.

\textbf{양극 부호 — $\partial U_j/\partial T=\Delta S_{\rxn,j}/F$ 는 흑연과 1:1.} 식~\eqref{eq:Uj} 의 온도 미분
$\partial U_j/\partial T=\Delta S_{\rxn,j}/F$ 는 전극 불문 같은 형태다(\S\ref{sec:center} 의 Gibbs 항등식
$\partial\Delta G/\partial T=-\Delta S$ 와 $\Delta G=-FU$ 를 잇는 유도가 부호 그대로 양극에 옮겨진다). 부호의 자리만
짚으면 — LCO 단전극의 측정 엔트로피 계수는 $\dd\phi/\dd T\approx+0.83$ mV/K({\'S}widerska-Mocek 외, tier B~\cite{swiderska2019}), 곧 \emph{전체}
$\Delta S_{\rxn}\approx F\cdot0.83\,\mathrm{mV/K}\approx+80$ J/(mol\,K) 규모로 \emph{전이별로 부호$\cdot$크기가 갈린다}
(이 전체 계수는 config$+$vib 가 지배하며, MIT 발현 구간엔 전자 엔트로피 게이트가 \emph{작은} MIT-국소 성분을 더한다 —
그 항의 삽입 기준 부호는 $\Delta S_e=\partial S_e/\partial x<0$ 으로 \S\ref{sec:electronic} 가 닫는다; $+0.83$ 을 전자항 부호
sanity 로 오용하지 않는다). 흑연의 $\Delta S_\rxn$
($+29/0/-5/-16$ J/(mol\,K))와 같은 단위$\cdot$같은 식 자리이며, 부호만 양극 데이터를 따른다.

\begin{verifybox}
\textbf{양극 중심 부호 검산.} $T=298.15$ K, $\Delta S_\rxn=+80$ J/(mol\,K)(단전극 $+0.83$ mV/K 환산)이면
$\partial U_j/\partial T=80/96485=+0.83$ mV/K $>0$ — 온도를 올리면 양극 평형 전위가 \emph{오른다}(흑연 식과 같은 부호
규칙, 값만 양극). $U_j=3.90$ V 를 anchor 로 두면 $30$ K 창에서 $\Delta U_j\approx\pm25$ mV 규모로 중심이 이동한다 —
흑연 N2 의 ``온도마다 $U_j(T)$ 를 따로 읽어야'' 결론이 양극에도 그대로 적용된다.
\end{verifybox}
...
\subsection{양극(LCO)의 order--disorder 와 MIT — 같은 정규용액 틀}\label{sec:hys-lco}
LCO 양극의 상전이도 이 절의 격자기체$\cdot$정규용액 틀에 그대로 들어간다 — 식~\eqref{eq:mu}--\eqref{eq:spinodal} 의
$\Omega_j$ 가 전극을 가리지 않기 때문이다. LCO 의 $\Omega_j$ 는 흑연의 층간 인력 대신 Li 부격자의 \emph{order--disorder}
정렬(중간 조성 $x\approx0.5$ 에서 hex$\leftrightarrow$monoclinic 초격자)과, 저전압 끝의 절연체$\to$금속 2상역(MIT,
$x\approx0.75$--$0.94$)에서 온다. 두 현상 모두 ``동등한 자리의 점유가 균질 혼합을 마다하고 분리$\cdot$정렬하려는''
같은 물리라, 정규용액 $\Omega\theta(1-\theta)$(식~\eqref{eq:mu})로 같은 부호 규칙을 따른다 — $\Omega_j>2RT$ 면 2상역과
히스(식~\eqref{eq:spinodal} 의 $u_j$ 실수 조건), $\Omega_j\le2RT$ 면 매끈한 단일상. 표~\ref{tab:lcostaging} 의 T2/T3
order--disorder 쌍은 monoclinic 정렬이 강한 2상역이라 $\Omega_j>2RT$ 로 두고(히스 분기 식~\eqref{eq:dUhys} 적용),
T1 MIT 의 2상역도 같은 spinodal 틀로 폭을 받는다. 곧 식~\eqref{eq:dUhys}$\cdot$\eqref{eq:Ubranch} 의 분기 gap$\cdot$
방향별 중심이 LCO 전이에도 \emph{식 변경 없이} 쓰인다 — 다른 것은 $\Omega_j$ 의 값(우리 데이터 피팅 대상, G3/G4)뿐이다.
한 가지만 양극에서 추가된다: MIT 2상역은 전자 구조 변화(상태밀도 $g(E_F)$ 의 켜짐)를 동반하므로, 그 구간의
\emph{엔트로피}에 흑연엔 없던 전자 항이 더해진다 — 이는 평형 중심$\cdot$폭(이 절)이 아니라 \emph{엔트로피 분해}의
몫이라, \S\ref{sec:electronic} 가 별도 항으로 닫는다.
...
앞 두 소절은 $\xi_\eq$ 를 두 경로로 얻었다 — (i) kinetic: 속도식~\eqref{eq:bv} 의 detailed balance 정지점
(식~\eqref{eq:logisticsolve}), (ii) thermo: 자유에너지 최소(식~\eqref{eq:Veq} 의 $g_j'=sF(V_\eq-U)$). 둘은 같은
$\xi_\eq$ 를 낳는데, 그 까닭을 한 그림으로 묶는 것이 \emph{분포(분배함수)} 관점이다. 결과식$\cdot$부호$\cdot$코드는
불변이고, 유도만 추가한다 — 이 한 언어가 LCO 전자 엔트로피(\S\ref{sec:electronic} 의 Fermi--Dirac)와 Ch2(분포
$-R\sum p\ln p$ 의 엔트로피 전개)로 가는 다리다.

\textbf{(a) 출발 — 단일 자리 점유의 분배함수.} 한 등가 자리를 골라 ``비었다''($n=0$, 에너지 기준 $0$)와 ``Li 가 찼다''
($n=1$, 비배치 자유에너지 비용에서 전기화학 구동력을 뺀 $\Delta\mu\equiv\mu_\mathrm{Li}-(\tilde\mu_{\mathrm{Li}^+}+
\tilde\mu_{e^-})=-sF(V-U_j)$, 곧 식~\eqref{eq:eqcond} 의 한 자리 버전)의 두 상태만 갖는 grand-canonical 점유를 쓰면,
단일 자리 분배함수는
\begin{equation}
Z_1=1+e^{-\beta\Delta\mu}=1+e^{\,\beta sF(V-U_j)},\qquad \beta\equiv\frac{1}{RT}
\label{eq:Z1}
\end{equation}
이다(두 항 $=$ 빈 자리 $+$ 찬 자리의 Boltzmann 가중). \textbf{(b) 연산 — 평균 점유 $=$ Fermi 함수.} 찬 상태의 확률,
곧 \emph{평균 점유}는 찬 항을 $Z_1$ 로 나눈 것이다:
\begin{equation}
\langle n\rangle=\frac{e^{-\beta\Delta\mu}}{1+e^{-\beta\Delta\mu}}
=\frac{1}{1+e^{\beta\Delta\mu}}
=\frac{1}{1+e^{-\beta sF(V-U_j)}}.
\label{eq:fermiocc}
\end{equation}
\textbf{(c) 중간식 — logistic 과의 동일성.} 식~\eqref{eq:fermiocc} 은 $w_j=RT/F$($n_j=1$)에서 평형 logistic
식~\eqref{eq:logisticsolve}$\cdot$\eqref{eq:xieq} 와 \emph{글자 그대로 같다}: $\xi_\eq=\langle n\rangle$ — 진행률
$\xi_\eq$ 는 다름 아닌 \emph{격자기체 자리의 평형 점유 확률}이다. 곧 식~\eqref{eq:Z1} 형의 분배함수 한 줄이
kinetic(detailed balance)과 thermo(자유에너지) 두 유도를 \emph{하나의 점유 분포}로 통합한다 — detailed balance 비
$r^+/r^-=e^{\mathcal A/RT}$(식~\eqref{eq:db})는 바로 이 두 상태 Boltzmann 비이고, 자유에너지 최소도 같은 $Z_1$ 의
로그 미분이다. \textbf{(d) 두 극한 — $\Omega$ 의 자리.} 자리 간 상호작용이 없으면($\Omega=0$) 점유는
식~\eqref{eq:fermiocc} 의 \emph{Fermi-함수형} 그대로다($1/(1+e^{\beta(\varepsilon-\mu)})$ 와 동형 — 단일 준위의
Fermi--Dirac 점유와 같은 꼴). 상호작용이 켜지면($\Omega>0$) 평균장 몫 $\Omega(1-2\theta)$ 이 $\Delta\mu$ 에 더해져
(식~\eqref{eq:mu}) 분포가 자기일관(self-consistent)해지고, $\Omega>2RT$ 에서 다봉(2상역)으로 갈라진다 — 식~\eqref{eq:gpp}
의 spinodal 이 곧 이 분포의 안정성 한계다.

\begin{keybox}
\textbf{세 곳을 한 언어로.} $\xi_\eq$(이 절)$\cdot$LCO 전도 전자(\S\ref{sec:electronic}, Fermi--Dirac)$\cdot$Ch2(점유
분포의 엔트로피 $-R\sum p\ln p$)는 모두 \emph{평형 점유 확률 분포}다. $\Omega=0$ 한계의 식~\eqref{eq:fermiocc} 이
단일 준위 Fermi--Dirac 과 동형이라는 점이, 다음 절에서 LCO 전도 전자의 \emph{집합} Fermi--Dirac 분포로 자연 확장된다 —
같은 분배함수 언어로 전자 엔트로피 $S_e$ 를 유도한다.
\end{keybox}
...
% ====================================================================
\section{전자 엔트로피 항 — LCO 고유의 새 성분}\label{sec:electronic}
% ====================================================================
앞 절의 분포 관점(\S\ref{sec:dist})은 격자 \emph{이온} 점유를 분배함수로 닫았다. LCO 양극에는 흑연에 없던 두 번째
점유 분포가 있다 — 탈리튬화로 켜지는 \emph{전도 전자}의 Fermi--Dirac 분포다. 이 절은 그 전자 분포가 평형 엔트로피에
더하는 항 $\Delta S_e$ 를, 같은 ``점유 분포'' 언어로 분배함수에서 유도하고, 흑연엔 왜 없는지를 물리로 닫는다.
이 항은 $U_j(T)$ 의 온도 의존(식~\eqref{eq:Uj}, $\partial U_j/\partial T=\Delta S_{\rxn}/F$)에 새 성분으로 들어가며,
\S\ref{sec:sum} 의 ΔS 분해에서 합산된다.

\subsection{왜 흑연엔 없고 LCO엔 있나 — metal--insulator 천이}
흑연은 전 조성에서 반금속/전도성에 가까워 $g(E_F)$ 의 변화가 미미하다 — 탈리튬화로 전자 상태밀도가 거의 안 바뀌므로
전자 엔트로피 \emph{변화}가 무시할 만하다(흑연 N2 의 $\Delta S_\rxn$ 은 config$+$vib 로 닫혔다). LCO 는 다르다 —
완전 리튬화 LiCoO$_2$($x\!\to\!1$)는 띠 사이 간격을 가진 \emph{절연체}이고, 탈리튬화($x\!\downarrow$)로 Co $t_{2g}$
띠에 정공이 생겨 $x\approx0.75$--$0.94$ 에서 \emph{금속}으로 천이한다(metal--insulator transition, MIT;
M{\'e}n{\'e}trier 외~\cite{menetrier1999}, Reimers \& Dahn~\cite{reimers1992}). 곧 $g(E_F)$ 가 $0$ 에서
금속값($\sim13$ e/eV$\cdot$자리, CoO$_2$ anchor~\cite{motohashi2009})으로 켜진다 —
이 켜짐이 전자 엔트로피 항을 \emph{국소적으로} 발현시킨다. ``흑연은 config$+$vib 로 닫히나 LCO 는 MIT 로 전자 분포가
엔트로피에 추가''라는 이 대비가, 두 전극을 한 챕터에 묶는 교육적 핵심이다. 이 전자항이 \emph{독립적으로 필요}하다는
근거는 1차 계산에 있다 — 순수 configurational 통계역학만으로는 MIT 2상역($x\approx0.7$--$0.9$)의 전압 plateau 를
재현하지 못한다(Garikipati 그룹의 scale-bridging 계산이 이 누락을 명시)~\cite{ml2024}.

\subsection{전자 엔트로피 닫힌식 — Fermi--Dirac 에서 Sommerfeld 로}
\textbf{(a) 출발 — 전도 전자의 Fermi--Dirac 점유.} 축퇴 전자기체에서 에너지 $\varepsilon$ 준위의 점유 확률은
Fermi--Dirac 분포 $f(\varepsilon)=1/(1+e^{(\varepsilon-\mu_e)/k_BT})$ 다 — \S\ref{sec:dist} 의 단일 자리 점유
식~\eqref{eq:fermiocc} 과 같은 꼴의, 전자 준위 \emph{집합} 버전이다($\mu_e$ 는 전자 화학퍼텐셜$=E_F$). 분포의
엔트로피는 점유$\cdot$비점유의 정보 엔트로피를 상태밀도 $g(\varepsilon)$ 로 적분한 것이다:
\begin{equation}
S_e=-k_B\!\int\! g(\varepsilon)\big[f\ln f+(1-f)\ln(1-f)\big]\dd\varepsilon.
\label{eq:Sfd}
\end{equation}
\textbf{(b) 연산 — Sommerfeld 전개($k_BT\ll E_F$).} 적분의 무게 $-[f\ln f+(1-f)\ln(1-f)]$ 는 Fermi 준위 부근
폭 $\sim k_BT$ 의 좁은 창에만 산다(깊은 점유$\cdot$높은 빈 준위는 $f\!\approx\!1$ 또는 $0$ 이라 기여 $0$). 그 창에서
$g(\varepsilon)\approx g(E_F)$ 로 빼내고 무차원 변수 $u=(\varepsilon-E_F)/k_BT$ 로 바꾸면
\begin{equation}
S_e\simeq k_B^2T\,g(E_F)\!\int_{-\infty}^{\infty}\!\frac{u^2 e^{u}}{(1+e^{u})^2}\,\dd u,
\qquad \int_{-\infty}^{\infty}\!\frac{u^2 e^{u}}{(1+e^{u})^2}\,\dd u=\frac{\pi^2}{3}.
\label{eq:sommerfeld}
\end{equation}
(표준 Sommerfeld 적분; 피적분함수 $-[f\ln f+(1-f)\ln(1-f)]=u\,f'$ 부분적분 후 $f'(u)=e^u/(1+e^u)^2$ 의 2차
모멘트.) \textbf{(c) 중간식 — 닫힌식.} 적분값을 넣으면 전자 엔트로피가 $T$-선형으로 닫힌다:
\begin{equation}
\boxed{\;S_e(T,x)=\frac{\pi^2}{3}\,k_B^2\,T\,g(E_F,x)\;.}
\label{eq:Se}
\end{equation}
이 Sommerfeld 형은 Reynier 외가 Li$_x$CoO$_2$ 의 전자 엔트로피 분해에 쓴 식과 같다~\cite{reynier2004}.
\textbf{(d) 부분몰 항 — 삽입(리튬화) 기준 $\partial/\partial x$.} 곡선에 들어가는 것은 전이당 \emph{변화}이므로,
조성 $x$(Li$_x$CoO$_2$ 의 Li 함량) 미분이 전자 엔트로피의 부분몰 기여다. 이 미분을 \emph{흑연 $\Delta S_{\rxn,j}$
슬롯과 같은 삽입 방향}으로 잡는다 — 곧 $x$ 가 \emph{느는}(Li 가 들어오는, 리튬화$=$삽입) 방향의 $\partial S_e/\partial x$ 다:
\begin{equation}
\boxed{\;\Delta S_e(x)=\frac{\partial S_e}{\partial x}\Big|_T
=\frac{\pi^2}{3}\,k_B^2\,T\,\frac{\partial g(E_F,x)}{\partial x}\;<\;0\quad(\text{삽입 기준}).}
\label{eq:dSe}
\end{equation}
\textbf{★부호 규약(이 절의 결함 클래스 — box$\cdot$prose 통일).} 리튬화($x\!\uparrow$, 삽입)로 금속$\to$절연체라
$g(E_F)$ 가 $13\!\to\!0$ e/eV 로 \emph{줄고}, 따라서 $\partial g/\partial x<0$, 곧 \emph{삽입 기준 $\partial S_e/\partial x<0$}
이다 — 이것이 식~\eqref{eq:Uj}$\cdot$\eqref{eq:dScat} 의 $\Delta S_{\rxn,j}$ 슬롯(삽입 반응 엔트로피)과 \emph{같은 삽입
방향}이라, 흑연 $\Delta S_\rxn$($+29/0/-5/-16$, 삽입 기준)과 한 자리에 더해진다. \emph{물리 읽기는 방향만 뒤집으면 같은 수}다 —
탈리튬화(역방향, $x\!\downarrow$)로 절연체$\to$금속이라 $g(E_F)$ 가 켜지면 $S_e$ 가 \emph{늘어}, MIT 구간에서 $\sim\!+0.18$
$k_B$/atom 이 \emph{방출}된다(Reynier 외~\cite{reynier2004}; 같은 $|\Delta S_e|$ 의 탈리 방향 표현). 곧 box 의 $\partial S_e/\partial x<0$(삽입)과
이 ``탈리 시 $+0.18$ 방출''은 \emph{한 양의 두 방향 표기}이지 모순이 아니다. \emph{$+0.83$ mV/K sanity 의 주의} —
LCO 단전극 $\dd\phi/\dd T\!\approx\!+0.83$ mV/K(\,{\'S}widerska-Mocek 외, tier B~\cite{swiderska2019})는 config$+$vib 가 지배하는 \emph{전체} 엔트로피 계수라
전자항 $\Delta S_e$ 의 부호를 제약하지 않는다(전자항은 그 위에 얹히는 작은 MIT-국소 성분 — sanity 로 오용 금지).
\textbf{★온도 의존 — 전자항만 $T$-선형.} 식~\eqref{eq:Se}$\cdot$\eqref{eq:dSe} 가 명시적으로
$\propto T$ 다 — 다른 항(상수 $\Delta S_\rxn^\mathrm{config}$$\cdot$$\Delta S_\rxn^\mathrm{vib}$)과 달리, 전자 기여
$\partial U_j/\partial T|_e=\Delta S_e/F\propto T$ 는 \emph{온도에 선형으로 자란다}. 이 차이는 가역열$\cdot$다온도
피팅에서 전자항을 다른 항과 구분하는 표지이며, Ch2 의 가역 발열 전개에서 중요해진다.

\subsection{모델 가정 — $g(E_F,x)$ 의 MIT-logistic 게이트}
일차 문헌에 $g(E_F,x)$ 의 \emph{연속} 정량 곡선은 없다(G2; Motohashi 외~\cite{motohashi2009} 는 CoO$_2$ 의 $13$ e/eV 단일 anchor 와 정성
추세뿐). Ch1 의 ``문헌값$=$초기값, 데이터로 피팅'' 철학(흑연 \code{GRAPHITE\_STAGING\_LIT})을 그대로 적용해, MIT 를
\emph{logistic 게이트}로 근사한다 — 그 사유는 흑연 폭 logistic 과 같다(매끄러운 $\partial U/\partial T$, 결함농도에
따른 발현 $x$ 분산을 폭으로 흡수):
\begin{equation}
...
\label{eq:gate}
\end{equation}
($\sigma$ 는 \S\ref{sec:width} 와 같은 logistic; $x\!\downarrow$ 으로 $0\!\to\!g_{\max}$ 켜짐, 발현 2상역
$x\approx0.75$--$0.94$). 이를 식~\eqref{eq:dSe} 에 넣으면 $\partial g/\partial x=-\frac{g_{\max}}{\Delta x_\mathrm{MIT}}
\sigma(1-\sigma)$ — MIT 부근에 \emph{국소 양봉우리}(작지만 0 아님, 그림~\ref{fig:eentropy})다. \textbf{초기값(피팅 대상,
G2):} $g_{\max}$$\cdot$$x_\mathrm{MIT}$$\cdot$$\Delta x_\mathrm{MIT}$. step 대신 logistic 을 택한 이유는 위 폭 일관성과,
MSMR(\S\ref{sec:lco-sum}) 의 겹침 가중과 같은 매끄러운 형태를 유지하기 위함이다.

\textbf{★단위 다리 — 자리당 $k_B$ 에서 몰당 $R$ 로.} 식~\eqref{eq:Se}$\cdot$\eqref{eq:dSe} 의 $g(E_F)$ 는 \emph{자리당}
상태밀도(e/eV$\cdot$자리)라 $S_e$$\cdot$$\Delta S_e$ 는 \emph{자리당 $k_B$} 단위로 나온다(아래 검산도 그 단위다). 이를
식~\eqref{eq:Uj}$\cdot$\eqref{eq:dScat} 의 \emph{몰당} $\Delta S_{\rxn}$[J/(mol\,K)]에 합치려면 Avogadro 수
$N_A$ 를 곱한다($k_B N_A=R$):
\begin{equation}
\Delta S_e^{\,\mathrm{molar}}(x,T)=N_A\,\Delta S_e(x)=\frac{\pi^2}{3}\,R\,(k_BT)\,\frac{\partial g(E_F,x)}{\partial x}
\quad[\text{J/(mol\,K)}].
\label{eq:dSemolar}
\end{equation}
곧 ``$\Delta S_e$ 를 분해(식~\eqref{eq:dScat})에 plug-in'' 은 \emph{이 몰당 값}을 더하는 것이다 — 자리당 $k_B$ 값을
그대로 넣으면 $N_A$ 배만큼 어긋난다. 크기 감각: MIT 창을 가로지르는 $|\Delta(S_e/k_B)|\approx0.8\,k_B$/자리 $\Rightarrow$
$\approx0.8R\approx6.7$ J/(mol\,K) 규모(흑연 $\Delta S_\rxn$ 의 $+29\sim-16$ 과 같은 자릿수). 부호는 식~\eqref{eq:dSe} 의
삽입 규약 그대로 — 몰당 값도 \emph{삽입 기준 $\Delta S_e^{\,\mathrm{molar}}<0$}(MIT 게이트가 그 발현 $x$ 구간을 정함).

\begin{verifybox}
\textbf{전자 엔트로피 크기 검산($T=298.15$ K).} 완전 금속 극한($g=g_{\max}=13$ e/eV)에서 식~\eqref{eq:Se} 는
$S_e/k_B=\frac{\pi^2}{3}(k_BT)g(E_F)=3.29\times0.0257\,\mathrm{eV}\times13/\mathrm{eV}=1.10\ k_B$/자리. MIT 창을
가로지르는 부분몰 변화(식~\eqref{eq:gate}, $x\!:\!0.94\!\to\!0.75$)는 $\Delta(S_e/k_B)=\frac{\pi^2}{3}(k_BT)\,
[g(0.75)-g(0.94)]\approx0.81\ k_B$/자리이고, $\partial(S_e/k_B)/\partial x|_{x_\mathrm{MIT}}\approx-5.5$/단위 $x$ 다.
O3 영역 부분몰차 $\approx0.18\ k_B$/자리(Reynier 외, tier B~\cite{reynier2004})와 같은 자릿수 — \emph{작지만 0 아님}이 확인된다.
\textbf{필수성:} config 단독으로는 MIT 2상역의 엔트로피 구조를 재현하지 못하므로(전자 상태밀도 켜짐이 빠짐~\cite{ml2024}), 이
작은 항이 MIT 게이트로 \emph{필수}다. 흑연은 같은 검산에서 $\partial g/\partial x\approx0$ 이라 $\Delta S_e\approx0$ —
전자항이 정확히 사라진다(흑연 식 불변).
\end{verifybox}

\begin{figure}[t]
\centering
\begin{tikzpicture}[x=8.5cm,y=1.0cm]
% x axis from 1.0 (left, lithiated insulator) to 0.4 (right, delithiated metal) — but plot vs x increasing right for clarity
\draw[-{Latex[length=1.6mm]}] (0.40,0) -- (1.02,0) node[below,font=\scriptsize] {$x$ in Li$_x$CoO$_2$ (lithiation $\to$)};
\draw[-{Latex[length=1.6mm]}] (0.40,0) -- (0.40,3.3) node[above,font=\scriptsize] {$g(E_F)$ [e/eV] \&\ $|\partial g/\partial x|$ (scaled)};
\foreach \xx in {0.5,0.6,0.7,0.8,0.9,1.0} {\draw (\xx,0.04) -- (\xx,-0.04) node[below,font=\scriptsize] {\xx};}
\draw (0.405,3.0) -- (0.395,3.0) node[left,font=\scriptsize] {13};
\draw (0.405,1.5) -- (0.395,1.5) node[left,font=\scriptsize] {6.5};
% g(EF,x) = gmax[1 - sigma((x-xMIT)/dxMIT)], gmax=13 (scaled to 3.0), xMIT=0.85 dx=0.05
% scale: value/13*3.0 ; sample points x=0.5..1.0
\draw[thick] plot[smooth] coordinates {(0.50,2.997) (0.60,2.980) (0.70,2.858) (0.75,2.642) (0.80,2.193) (0.85,1.500) (0.90,0.807) (0.94,0.426) (1.00,0.142)};
\node[font=\scriptsize,align=center] at (0.56,2.55) {metal\\$g\approx13$};
\node[font=\scriptsize,align=center] at (0.97,0.62) {insulator\\$g\to0$};
% |dg/dx| gate peak (scaled), centered at xMIT=0.85
\draw[densely dashed,thick] plot[smooth] coordinates {(0.50,0.02) (0.65,0.08) (0.72,0.25) (0.78,0.85) (0.82,1.55) (0.85,1.90) (0.88,1.55) (0.92,0.70) (0.97,0.16) (1.00,0.04)};
\node[font=\scriptsize] at (0.85,2.15) {$|\partial g/\partial x|\propto\Delta S_e$};
% MIT 2-phase band
\fill[black!8] (0.75,0) rectangle (0.94,3.15);
\node[font=\scriptsize] at (0.845,2.95) {MIT 2-phase};
\draw[densely dotted] (0.85,0) -- (0.85,1.90);
\node[below,font=\scriptsize] at (0.85,-0.30) {$x_\mathrm{MIT}$};
\end{tikzpicture}
\caption{LCO 전자 상태밀도 게이트와 전자 엔트로피(신규, LCO). 실선 $=$ Fermi 준위 상태밀도 $g(E_F,x)$
(식~\eqref{eq:gate}, MIT-logistic 게이트): 리튬화 끝($x\!\to\!1$)은 절연체 $g\!\to\!0$, 탈리튬화로 금속
$g\!\to\!13$ e/eV. 파선 $=$ 그 기울기 크기 $|\partial g/\partial x|$, 곧 부분몰 전자 엔트로피 $|\Delta S_e|\propto T\,
|\partial g/\partial x|$(식~\eqref{eq:dSe}; 삽입 기준 $\Delta S_e=\partial S_e/\partial x<0$, 여기 그린 것은 그 \emph{크기})의
\emph{국소 봉우리} — MIT 2상역($x\approx0.75$--$0.94$, 음영)에만 산다.
흑연은 $g$ 가 거의 평탄해 이 봉우리가 없다(전자항 $\approx0$). 초기값 $g_{\max}\!=\!13$$\cdot$$x_\mathrm{MIT}\!=\!0.85$
$\cdot$$\Delta x_\mathrm{MIT}\!=\!0.05$ 는 데이터 피팅 대상(G2).}
\label{fig:eentropy}
\end{figure}

전자 엔트로피 항이 평형 중심의 온도 의존에 더해지는 새 성분으로 정해졌다 — 그 합산은 \S\ref{sec:sum} 의 ΔS 분해에서
config$\cdot$vib 와 함께 닫는다(이중계산을 피하는 분해 규칙 포함). 다음 절은 본 줄기로 돌아가, 이 평형 점유에서 곧바로
평형 peak 모양을 만든다 — 전자항을 켜든 끄든 peak 식~\eqref{eq:eqpeak} 자체는 불변이고, 전자항은 중심 위치와
온도 의존에만 들어간다.
...
\textbf{LCO 양극의 평형 peak.} 식~\eqref{eq:eqpeak} 은 전극 불문이라 LCO 에도 그대로다 — 다른 것은 중심 위치
($U_j^{\,d}\sim3.9$--$4.2$ V)와 전이 수(3개)뿐이다. 표~\ref{tab:lcostaging} 의 셋이 양극 dQ/dV 에 세 peak 을
남긴다: 저전압 끝 $\sim3.90$ V 의 MIT peak(전자항이 켜지는 자리)과, $\sim4.05$/$4.17$--$4.20$ V 의 order--disorder
쌍(전이 위치는 Xia 외의 박막 dQ/dV anchor~\cite{xia2007}; 그림~\ref{fig:lcopeak}). 흑연이 $\sim0.085$--$0.21$ V 에 네 peak 을 남기는 것과 \emph{같은 종 모양}이며, 전위
축의 자리만 $\sim4$ V 로 올라간 거울 사례다.

\begin{figure}[t]
\centering
\begin{tikzpicture}[x=2.55cm,y=0.95cm]
% LCO dQ/dV vs V (3.7 - 4.3 V). three bells: MIT 3.90, OD-a 4.05, OD-b 4.18
\draw[-{Latex[length=1.6mm]}] (3.68,0) -- (4.34,0) node[below,font=\scriptsize] {$V$ [V vs Li]};
\draw[-{Latex[length=1.6mm]}] (3.68,0) -- (3.68,3.1) node[above,font=\scriptsize] {$\dd Q/\dd V$ [arb.]};
\foreach \vv in {3.8,3.9,4.0,4.1,4.2,4.3} {\draw (\vv,0.04) -- (\vv,-0.04) node[below,font=\scriptsize] {\vv};}
% MIT peak at 3.90 (moderate height), width ~0.03
\draw[thick] plot[smooth] coordinates {(3.80,0.06) (3.84,0.20) (3.87,0.55) (3.89,1.35) (3.90,1.70) (3.91,1.35) (3.93,0.55) (3.96,0.18) (4.00,0.05)};
\node[font=\scriptsize,align=center] at (3.90,2.02) {T1 MIT\\$\sim$3.90 V};
\draw[densely dotted] (3.90,0)--(3.90,1.70);
% OD-a peak at 4.05 (tall, sharp)
\draw[thick] plot[smooth] coordinates {(3.98,0.05) (4.01,0.30) (4.03,1.10) (4.045,2.35) (4.05,2.75) (4.055,2.35) (4.07,1.10) (4.09,0.32) (4.12,0.08)};
\node[font=\scriptsize,align=center] at (4.05,2.98) {T2\\4.05 V};
% OD-b peak at 4.18 (tall)
\draw[thick] plot[smooth] coordinates {(4.12,0.07) (4.145,0.35) (4.165,1.20) (4.178,2.20) (4.185,2.55) (4.192,2.20) (4.21,1.10) (4.23,0.32) (4.26,0.08)};
\node[font=\scriptsize,align=center] at (4.185,2.80) {T3\\4.18 V};
% electronic term annotation under MIT
\draw[-{Latex[length=1.2mm]},gray] (3.78,1.1) -- (3.86,0.7);
\node[font=\scriptsize,align=center,gray] at (3.755,1.35) {$+\Delta S_e$\\gate ON};
\end{tikzpicture}
\caption{LCO 양극 평형 dQ/dV(신규, LCO; 정성). 식~\eqref{eq:eqpeak} 의 종 셋이 표~\ref{tab:lcostaging} 의 세
전이를 남긴다 — $\sim3.90$ V 의 MIT peak(\S\ref{sec:electronic} 의 전자 엔트로피 게이트가 켜지는 자리), $\sim4.05$
및 $\sim4.18$ V 의 order--disorder 쌍. 흑연 그림~\ref{fig:logistic} 의 종과 같은 모양이며, 전위 축만 $\sim4$ V 로
이동했다. peak 위치 $=$ 중심 $U_j^{\,d}$, 순높이 $=Q_j/(4w_j)$, 면적 $=Q_j$ — 흑연과 같은 세 양이 읽힌다.}
\label{fig:lcopeak}
\end{figure}
...
\subsection{LCO 반응 엔트로피 분해와 코드 일반화 (H)}\label{sec:lco-sum}
양극 합산도 같은 식~\eqref{eq:sum} 으로 닫힌다 — 다른 것은 전이당 파라미터와, $\Delta S_{\rxn,j}$ 의 \emph{분해}뿐이다.
LCO 의 전이별 반응 엔트로피는 세 성분의 합이다(흑연은 앞 둘로 닫혔다):
\begin{equation}
\boxed{\;\Delta S_{\rxn,j}^\mathrm{cat}(x,T)
=\underbrace{\Delta S^{\mathrm{config},0}_j}_{\substack{\text{중심 표준값만}\\ \text{(봉우리 내부는 logistic 이 담음)}}}
+\underbrace{\Delta S^\mathrm{vib}_j}_{\text{음의 baseline}}
+\underbrace{\Delta S_e^{\,\mathrm{molar}}(x,T)}_{\text{MIT 게이트, }\propto T}\;.}
\label{eq:dScat}
\end{equation}
\textbf{성분의 출처와 ★이중계산 금지.} (i) $\Delta S^{\mathrm{config},0}_j$ 는 O3 영역 ΔS 를 지배하며(Reynier 외~\cite{reynier2004}, $>\tfrac12$),
\emph{기존 Ch1 logistic 이 이미 자동 생성}한다 — 격자기체 자유에너지~\eqref{eq:gxi} 의 배치 몫
$R[\xi\ln\xi+(1-\xi)\ln(1-\xi)]$ 가 봉우리 내부에서 $\Delta S^\mathrm{config}$ 를 낳고, 그 \emph{중심 표준값}이 식의
$\Delta S^{\mathrm{config},0}_j$($=$ 표~\ref{tab:lcostaging} 의 $\Delta S_\rxn$ 초기값)다. 따라서 식~\eqref{eq:dScat} 의
config 자리는 \emph{중심 표준값만} 넣어야 한다(그래서 위 박스는 $\Delta S^{\mathrm{config},0}_j$ 로 적었다) — 봉우리 내부
배치 몫 $R\ln[(1-\xi)/\xi]$ 을 다시 더하면 logistic 이 이미 담은 것을 \emph{이중계산}한다(금지). (ii)
$\Delta S^\mathrm{vib}_j$ 는 phonon 의 음의 baseline 으로 흑연과 동형이다. (iii) $\Delta S_e^{\,\mathrm{molar}}(x,T)$ 는
\S\ref{sec:electronic} 식~\eqref{eq:dSemolar}(자리당 식~\eqref{eq:dSe} 에 $N_A$ 곱한 \emph{몰당} 값)의 신규 항으로,
MIT 게이트(식~\eqref{eq:gate})에서만 발현하고 \emph{유일하게 $\propto T$} 다 — 흑연엔 $0$. 이 셋이 식~\eqref{eq:Uj}
의 $\Delta S_{\rxn,j}$ 로 합쳐져 $U_j(T)$ 와 그 온도 의존에 들어간다(세 항 모두 J/(mol\,K) 몰당 단위).

\textbf{★코드 일반화 — MSMR 가 Ch1 logistic 과 동형(H).} 양극 분야 표준인 MSMR(Multi-Species Multi-Reaction) 모델~\cite{msmr2024} 의
한 종 점유 $x_j=X_j/(1+\exp[f(U-U_j^0)/\omega_j])$ 는 본 장의 transition-logistic 식~\eqref{eq:xieq}
($\xi_\eq=1/(1+\exp[-\sigma_d(V-U_j^d)/w_j])$)과 \emph{글자 그대로 같은 형태}다 — $X_j\leftrightarrow Q_j$,
$\omega_j\leftrightarrow w_j=n_jRT/F$, $U_j^0\leftrightarrow U_j^d$ 의 대응. 곧 Ch1 forward 코드(\code{func\_ksi\_eq}$\cdot$
\code{func\_U\_j})는 \emph{구조 변경 0}으로 LCO 에 적용된다 — 필요한 것은 (i) 전이 파라미터 교체
(\code{GRAPHITE\_STAGING\_LIT}$\to$\code{LCO\_STAGING\_LIT}, 표~\ref{tab:lcostaging}) + (ii) 전자 엔트로피 항
$\Delta S_e(x,T)$ 를 $\Delta S_{\rxn,j}$ 분해(식~\eqref{eq:dScat})에 plug-in, 둘뿐이다. logistic$\cdot$히스$\cdot$꼬리$\cdot$
역보간 전 사슬은 그대로 돌아간다(파라미터 $+$ 1항).

\textbf{도핑 보정(우리 시료).} Al$^{3+}$/Mg$^{2+}$ 첨가는 비-redox 라 Co redox$\cdot$전자 엔트로피 항을 \emph{보존}하고
(전자항 그대로), order--disorder$\cdot$MIT$\cdot$O3$\leftrightarrow$H1-3 상전이를 \emph{억제}한다 — logistic 폭 $w$ 를
넓히고 peak 를 smear, $U_j$ 를 미세 shift 한다(도핑 site 의 high-entropy 배치는 config 약간 추가). 곧
표~\ref{tab:lcostaging} 의 pure-LCO 값$=$초기값이고, 폭$\cdot$shift 는 우리 코인 하프셀 데이터로 피팅한다(G4) —
흑연 \code{GRAPHITE\_STAGING\_LIT}$=$초기값 철학과 같다.

\begin{keybox}
\textbf{갭 → 데이터 피팅(round-trip).} G1 연속 $\Delta S(x)$ 표$\cdot$G2 $g(E_F)(x)$ 게이트$\cdot$G3 MSMR LCO
파라미터$\cdot$G4 도핑 shift 는 초기값(표~\ref{tab:lcostaging}$\cdot$식~\eqref{eq:gate})$+$우리 데이터 round-trip
피팅으로 식별한다. G5 절대 $\Delta H_{f}$ 는 OCV plateau($3.90$ V 등) anchor 로 대체한다. 전부 Ch1 철학(문헌 초기값
$+$데이터 피팅) 안이며, 허위 정밀은 피한다(tier 표기). 범위는 코인 하프셀(LCO vs Li) 단독 — 전셀 합성
$\partial U_\cell=\partial U_\mathrm{cat}-\partial U_\mathrm{an}$ 은 후속이고, 단전극 $+0.83$ mV/K 와 전셀
부호전환을 혼동하지 않는다.
\end{keybox}
...
\textbf{(LCO 양극)} 같은 6단계를 돌되 (1) 을 표~\ref{tab:lcostaging} 의 3 전이로 구성하고, (3) 의 $\Delta S_{\rxn,j}$
자리에 분해 식~\eqref{eq:dScat}(config 중심값 $+$ vib $+$ 몰당 전자항 식~\eqref{eq:dSemolar})을 쓴다 — 나머지
$U_j(T)$$\cdot$$\xi_\eq$$\cdot$꼬리$\cdot$합산은 글자 그대로 같다(코드 일반화 H, \S\ref{sec:lco-sum}).
\end{keybox}
...
\bibitem{menetrier1999} M. M{\'e}n{\'e}trier, I. Saadoune, S. Levasseur, C. Delmas, ``The insulator-metal transition upon lithium deintercalation from LiCoO$_2$: electronic properties and $^7$Li NMR study,'' \emph{J. Mater. Chem.} \textbf{9}, 1135 (1999). DOI: 10.1039/a900016j.
...
\bibitem{motohashi2009} T. Motohashi, T. Ono, Y. Sugimoto, Y. Masubuchi, S. Kikkawa, R. Kanno, M. Karppinen, H. Yamauchi, ``Electronic phase diagram of the layered cobalt oxide system Li$_x$CoO$_2$ ($0.0\le x\le1.0$),'' \emph{Phys. Rev. B} \textbf{80}, 165114 (2009). DOI: 10.1103/PhysRevB.80.165114. arXiv:0909.3556.
\bibitem{reynier2004} Y. Reynier, J. Graetz, T. Swan-Wood, P. Rez, R. Yazami, B. Fultz, ``Entropy of Li intercalation in Li$_x$CoO$_2$,'' \emph{Phys. Rev. B} \textbf{70}, 174304 (2004). DOI: 10.1103/PhysRevB.70.174304. [동 그룹 \emph{J. Electrochem. Soc.} \textbf{151}, A422 (2004).]
\bibitem{swiderska2019} A. {\'S}widerska-Mocek, E. Rudnicka, A. Lewandowski, ``Temperature coefficients of Li-ion battery single electrode potentials and related entropy changes -- revisited,'' \emph{Phys. Chem. Chem. Phys.} \textbf{21}, 2115 (2019). DOI: 10.1039/c8cp06638h. [LiCoO$_2$ 단전극 $\dd\phi/\dd T\!\approx\!+0.83$ mV/K.]
\bibitem{ml2024} G. H. Teichert, S. Das, M. Faghih Shojaei, J. Holber, T. Mueller, L. Hung, V. Gavini, K. Garikipati, ``Bridging scales with machine learning: From first principles statistical mechanics to continuum phase field computations to study order--disorder transitions in Li$_x$CoO$_2$,'' \emph{J. Mech. Phys. Solids} \textbf{190}, 105727 (2024). DOI: 10.1016/j.jmps.2024.105727. arXiv:2302.08991.
\bibitem{msmr2024} A. Paul, K. Wolfe, M. W. Verbrugge, B. J. Koch, J. S. Lowe, J. Trembly, J. A. Staser, T. R. Garrick, ``Quantifying the Temperature Dependence of the Multi-Species, Multi-Reaction (MSMR) Model. Part 1: Parameterization for a Meso-Carbon Micro-Bead Graphite,'' \emph{ECS Advances} \textbf{3}, 042501 (2024). DOI: 10.1149/2754-2734/ad7d1c. [Part II: \emph{J. Electrochem. Soc.}, DOI: 10.1149/1945-7111/ad70d9. 원 MSMR: M. W. Verbrugge, D. R. Baker, B. J. Koch 외.]
\end{thebibliography}
