...
같은 전이를 서로 다른 준안정 가지로 과주행해 중심이 갈라진다. 이 절은 그 gap 의 닫힌 식과 부호를, 격자기체 자유에너지의
...
사이의 불안정 구간이다. 이 곡률 부호 전환이 \S\ref{sec:hys} 의 비단조 전위 곡선과 분기 gap 을 낳는다. 내부 텍스트는
ASCII.}
\label{fig:doublewell}
\end{figure}

\subsection{gap 의 닫힌 꼴 — 두 spinodal 의 전위 차}
\textbf{(a) 출발 — 비단조 평형 전위 곡선.} 자유에너지~\eqref{eq:gxi} 의 1계 미분이 곧 평형 전위다
...
같다고 둔 데서 온다 — $sF\,\dd V_\eq/\dd\xi|_{1/2}=4RT-2\Omega_j$ 와 $4Fw^\eff_j$ 를 같다고 풀면
\begin{equation}
...
$k\simeq k_0e^{-\Delta G_a/RT}$ 다(그림~\ref{fig:barrier}). 구동력 $\mathcal A_j=sF(V_n-U_j)$ 가 정$\cdot$역 장벽을
비대칭 이동시켜(분율 위치 $\chi$), 자리당 정$\cdot$역 속도가
\begin{equation}
...
(식~\eqref{eq:peakshape})는 이 차를 길이로 나눈 이산 미분이다. $L_V$ 가 작으면 $\xi_\mathrm{lag}$ 이 한 칸 뒤처진
$\xi_\eq$ 라 평형 종(식~\eqref{eq:eqpeak})으로 환원된다. 내부 텍스트 ASCII.}
\label{fig:relaxode}
\end{figure}
