...
방향별 한 자유도로 적는 것이 다음 ansatz(현상학적 매개변수화)다:
\begin{equation}
\boxed{\;U_j^{\,d}\;=\;U_j\;+\;\tfrac12\,\sigma_d\,h_{\eta,j}\,\gamma_j\,\Delta U_j^\hys\;.}
\label{eq:Ubranch}
\end{equation}
여기서 $\gamma_j\in[0,1]$ 이 \emph{현상학적 분기 축소 인자}다. 이상 spinodal 분기($\gamma_j=1$)와 분기 없음($\gamma_j=0$) 사이를 연속적으로 잇는 단일 자유도로, 값은 열역학 이론에서 결정되는 것이 아니라 실험 충방전 곡선에 피팅해 얻는 매개변수다. $\Delta U_j^\hys$ 는 spinodal 상한(식~\eqref{eq:dUhys})으로 스케일을 물리적으로 고정한다.
...
\begin{derivebox}
\textbf{출발.} Eyring 전이상태 이론에서 전진 속도 상수는 $k^+=k_0\exp[-(\Delta G_a-\chi\mathcal{A}_j)/RT]$ 이고,
$\mathcal{A}_j$ 는 국소 구동력이다. 깊은 꼬리 극한에서 $\mathcal{A}_j$ 의 상호작용 기여를 분리해 장벽에 흡수한다.

\textbf{중간식 — 상호작용 분리.}
평형 전위~\eqref{eq:Veq} 에서 $\Omega_j(1-2\xi)$ 항이 화학퍼텐셜에 들어온다.
\begin{itemize}
  \item \emph{방전 꼬리} ($\xi\to1$): $(1-2\xi)\to-1$, 상호작용 기여 $\to-\Omega_j$ — 역방향 장벽에 더해짐.
        정방향에 전달 계수 $\chi$ 만큼 반영되면, 장벽 보정 $=-\chi\cdot\Omega_j$.
  \item \emph{충전 꼬리} ($\xi\to0$): $(1-2\xi)\to+1$, 상호작용 기여 $\to+\Omega_j$ — 정방향 장벽을 낮춤.
        충전 전달 계수 $\chi_d=1-\chi$ 만큼 반영되면, 장벽 보정 $=-(1-\chi)\cdot\Omega_j$.
\end{itemize}
두 경우를 방향별 계수 $\chi_d$(식~\eqref{eq:chid})로 묶으면 동일한 형태가 된다:
\begin{equation*}
  \Delta H_a^{\eff,\,\text{방전}} = \Delta H_a - \chi\,\Omega_j,\qquad
  \Delta H_a^{\eff,\,\text{충전}} = \Delta H_a - (1-\chi)\,\Omega_j.
\end{equation*}
\textbf{통합.} 방향별 전달 계수 $\chi_d$ 를 쓰면 두 식이 하나로 합쳐진다.
\end{derivebox}
...
피팅 전위는 측정 단자전압 $V_\app$ 에서 ohm 강하를 뺀 음극 전위 $V_n$ 이다(N1 에서 유도한 식~\eqref{eq:vn} 참조):
\begin{equation}
...
% ====================================================================
\section*{회귀 자가검증 (Self-Test)}\label{sec:selftest}
\addcontentsline{toc}{section}{회귀 자가검증}
% ====================================================================
아래는 본 문건의 식들을 \code{GRAPHITE\_STAGING\_LIT}(표~\ref{tab:staging})의 stage $2\!\to\!1$ 전이 값으로
손계산해 코드 결과와 교차 검증하는 회귀 자가검증이다. 입력 고정 조건: $T=298.15\,\text{K}$,
$\Delta H_\rxn=-13000\,\text{J/mol}$, $\Delta S_\rxn=-16\,\text{J/(mol\,K)}$,
$\Omega=13000\,\text{J/mol}$, $\Delta H_a=40000\,\text{J/mol}$, $n=1$, $\chi=0.5$.

\begin{verifybox}
\textbf{R1. $U_j(T)$ (식~\eqref{eq:Uj}).}
\begin{equation*}
U_j = \frac{-(-13000)+298.15\times(-16)}{96485}
    = \frac{13000-4770.4}{96485}
    = \frac{8229.6}{96485}\approx0.08529\ \text{V}.
\end{equation*}
목표 $0.085\,\text{V}$ — 편차 $<0.1\,\text{mV}$. \checkmark

\textbf{R2. $w_j$ (식~\eqref{eq:wbase}).}
\begin{equation*}
w_j = \frac{1\times8.314\times298.15}{96485}\approx0.02569\ \text{V}.
\end{equation*}
코드 \code{func\_w(298.15, n=1)} 기대값. \checkmark

\textbf{R3. spinodal $u_j$ (식~\eqref{eq:spinodal}).}
\begin{equation*}
u_j=\sqrt{1-\frac{2\times8.314\times298.15}{13000}}=\sqrt{1-0.3814}=\sqrt{0.6186}\approx0.7865.
\end{equation*}
$\Omega=13000>2RT=4957\ \text{J/mol}$ — 상분리 조건 충족. \checkmark

\textbf{R4. $\Delta U_j^\hys$ (식~\eqref{eq:dUhys}).}
\begin{align*}
\mathrm{artanh}(0.7865)&=\tfrac12\ln\tfrac{1.7865}{0.2135}\approx\tfrac12\ln8.367\approx1.0387,\\
\Delta U_j^\hys &= \frac{2}{96485}\big[13000\times0.7865 - 2\times8.314\times298.15\times1.0387\big]\\
&=\frac{2}{96485}\big[10224.5-5148.8\big]=\frac{2\times5075.7}{96485}\approx0.1053\ \text{V}.
\end{align*}
\checkmark ($\Delta U^\hys>0$ 확인.)

\textbf{R5. $\Delta H_a^\eff$ (식~\eqref{eq:dHeff}), 방전 $\chi_d=0.5$.}
\begin{equation*}
\Delta H_a^\eff = 40000 - 0.5\times13000 = 33500\ \text{J/mol}.
\end{equation*}
충전 $\chi_d=1-0.5=0.5$: 동일값(대칭 $\chi=0.5$ 시). \checkmark

\textbf{R6. D-PEAK 이산 분기 확인.} $L_V$ 작은 극한($|I|\to0$): $\rho=e^{-|\Delta V|/L_V}\to0$,
$\xi_\lag\to\xi_\eq$, peak tail $\to0$. 종 환원으로 수렴하지 않고 이산 logistic 분기 중심은 $U_j^d=U_j+\tfrac12\sigma_d\gamma\Delta U^\hys$ 로 고정 — $|I|\to0$ 에서도 분기가 유지된다(D-PEAK 거짓 극한 회피). \checkmark
\end{verifybox}
...
\caption{전이 초기값 \code{GRAPHITE\_STAGING\_LIT}(4건) — 피팅 출발점; 피팅으로 override 가능.
$U_j$ 는 \code{'U'} 폴백이자 $(\Delta H_\rxn,\Delta S_\rxn)$ 정합 목표.\label{tab:staging}}
\small
\begin{tabular}{l c c c c c c c}
\toprule
stage & $U_j$ [V] & $\Delta H_\rxn$ [J/mol] & $\Delta S_\rxn$ [J/(mol\,K)]
      & $Q_j$ [$Q_\cell$] & $\Omega_j$ [J/mol] & $\Delta H_a$ [J/mol] & $|\dd V/\dd q|_{q_a}$ [V] \\
\midrule
