% ====================================================================
% graphite_ica_ch1_v8-04b.tex — Chapter 1 (v8: 유도 과정 포함 교과서 확장판)
%   v8-04.tex 의 증분 정정판(원본 보존). 정정 4건 + self-test:
%     (1) N8 D-PEAK(L905 부근): ρ→0(작은 L_V)이면 ξ_lag→같은 칸 ξ_eq ⇒ peak→0
%         (한 칸 뒤처짐 아님). 평형 환원은 반대 극한 ρ→1(L_V≫Δgrid)의 이산 미분과
%         eq:branch 스위치 담당 — 정정.
%     (2) fig:overshoot: 하드코딩 식번호(원본 "1.18=1.16" 오류)→\eqref{eq:Veq}=\eqref{eq:gpp}
%         (drift-free), 분기 라벨 상하 정정, base 곡선 Ω=3RT 로 차별화(fig:hysloop Ω=4RT
%         중복 해소·fig:doublewell Ω=3RT 와 정합).
%     (3) eq:Veq 다리 inline 보강(eq:isotherm 중간식 추가).
%     (4) orphan bib(bazant2013·dreyer2010) 본문 인용.
...
같은 전이를 서로 다른 준안정 가지로 과주행해 중심이 갈라진다. 이 준안정 과주행이 삽입전극 히스테리시스의 열역학적
기원이며 \cite{dreyer2010}, 본 절은 그 gap 의 닫힌 식과 부호를 자유에너지에서 spinodal 근, 비단조 평형 전위 곡선,
두 극값의 차까지 \emph{한 단계도 건너뛰지 않고} 유도한다.

\subsection{상호작용이 평형을 비트는 곳 — $g(\xi)$ 와 spinodal}
격자기체의 화학퍼텐셜에는 섞임 엔트로피 로그 몫 외에 정규용액 상호작용 몫 $\Omega_j(1-2\theta)$ 가 더해진다($\Omega>0$
이면 동종 이웃 인력). 이 상호작용 자유에너지가 평형(spinodal$\cdot$binodal)뿐 아니라 \S\ref{sec:width}$\cdot$\S\ref{sec:lag}
의 반응속도까지 일관되게 묶는 틀이 비평형 열역학 기반 반응속도론이다 \cite{bazant2013}. 자유에너지를 진행률 $\xi=1-\theta$ 로
적으면(상수$\cdot$$\xi$ 1차 몫은 공통 접선 판정에 불변이라 기준 $g_j^0$ 와 함께 떼어 둔다)
\begin{equation}
...
평형 전위 곡선은 $g_j$ 의 1차 미분이 곧 전위 환산이라는 데서 온다. 식~\eqref{eq:gxi} 의 $g_j(\xi)$ 를 한 번 미분하면
로그 몫에서 $RT[\ln\xi-\ln(1-\xi)]$, 상호작용 몫 $\Omega_j\xi(1-\xi)$ 에서 $\Omega_j(1-2\xi)$ 가 나와
$g_j'(\xi)=RT\ln[\xi/(1-\xi)]+\Omega_j(1-2\xi)$ 다. 정지점 조건(\S\ref{sec:width} 에서 유도할 detailed balance)이 이
$\mu$-기울기를 전위 구동력에 묶으므로(평형에서 $g_j'$ 가 곧 $sF(V_\eq-U_j)$), 평형 등온선이 음함수로 닫힌다:
\begin{equation}
sF\,(V_\eq-U_j)=g_j'(\xi)=RT\ln\frac{\xi}{1-\xi}+\Omega_j(1-2\xi).
\label{eq:isotherm}
\end{equation}
이를 $V_\eq$ 에 대해 풀려면 양변을 $sF$ 로 나누고($U_j$ 좌변으로 이항해 우변으로 옮김) 두 항을 각각 $sF$ 로 나누면 된다:
\begin{equation}
V_{\eq,j}(\xi)\;=\;U_j+\frac{1}{sF}g_j'(\xi)\;=\;U_j+\frac{RT}{sF}\ln\frac{\xi}{1-\xi}+\frac{\Omega_j}{sF}\,(1-2\xi).
\label{eq:Veq}
\end{equation}
...
\begin{figure}[t]
\centering
\begin{tikzpicture}[x=8.8cm,y=1.45cm]
% NEW figure: V_eq(xi) overshoot, marking that extrema sit at spinodal (g''=0).
% Base curve Omega=3RT (differs from fig:hysloop Omega=4RT; matches fig:doublewell 3RT).
\draw[-{Latex[length=1.6mm]}] (0,-1.25) -- (0,1.35) node[left,font=\scriptsize] {$(sF/RT)(V_\eq-U_j)$};
...
% non-monotone V_eq for Omega=3RT: extrema +-0.415 at xs=0.2113/0.7887
\draw[thick] plot[smooth] coordinates {(0.02,-1.012) (0.05,-0.244) (0.08,0.078) (0.12,0.288) (0.2113,0.415) (0.28,0.376) (0.36,0.265) (0.44,0.119) (0.5,0.0) (0.56,-0.119) (0.64,-0.265) (0.72,-0.376) (0.7887,-0.415) (0.88,-0.288) (0.92,-0.078) (0.95,0.244) (0.98,1.012)};
% mark extrema = spinodal points; tangent horizontal there (dV/dxi=0 = g''=0)
\fill (0.2113,0.415) circle (0.6pt) node[above right,font=\scriptsize] {$\xi_s^-$ (max, $g''=0$)};
\fill (0.7887,-0.415) circle (0.6pt) node[below left,font=\scriptsize] {$\xi_s^+$ (min, $g''=0$)};
% slope-zero markers (horizontal tangents at the extrema)
\draw[densely dotted] (0.12,0.415) -- (0.30,0.415);
\draw[densely dotted] (0.70,-0.415) -- (0.88,-0.415);
\node[align=center,font=\scriptsize] at (0.50,1.12) {extrema of $V_\eq(\xi)$ $\equiv$ spinodal ($g''=0$):\\[-1pt]Eq.~\eqref{eq:Veq}\,$\Rightarrow$\,Eq.~\eqref{eq:gpp}};
% branch labels: rising/delithiation sits with the upper (max) lobe, falling/lithiation with the lower (min) lobe
\node[font=\scriptsize] at (0.34,0.70) {rising branch (delithiation)};
\node[font=\scriptsize] at (0.66,-0.70) {falling branch (lithiation)};
\end{tikzpicture}
\caption{비단조 평형 전위 $V_\eq(\xi)$ 의 극값이 spinodal 에 서는 근거(신규, $\Omega_j=3RT$). 식~\eqref{eq:Veq} 를
$\xi$ 로 미분하면 $sF\,\dd V_\eq/\dd\xi=g_j''(\xi)$ 이므로(식~\eqref{eq:gpp}), 전위 곡선의 수평 접선($\dd V_\eq/\dd\xi=0$)
이 곡률 영점 $\xi_s^\pm$(식~\eqref{eq:spinodal})과 정확히 일치한다 — 여기 표시한 $\xi_s^\pm=0.211/0.789$ 가
$\Omega=3RT$ 의 spinodal 이고(그림~\ref{fig:doublewell} 의 띠 경계와 같은 점), 다음 그림~\ref{fig:hysloop}
($\Omega=4RT$, 과주행이 더 깊은 정성적 예)이 이 두 극값을 두 방향의 과주행 경로로 다시 보인다. $\Omega$ 가 크면 두
극값(따라서 gap)이 깊어진다.}
\label{fig:overshoot}
\end{figure}
...
의 출력). 두 극한을 정확히 갈라야 한다. \emph{큰 $L_V$}($L_V\gg\Delta_\mathrm{grid}$)에서는 식~\eqref{eq:lowpass} 의
$\rho=e^{-\Delta_\mathrm{grid}/L_V}\to1$ 이라 $\xi_\mathrm{lag}$ 가 $\xi_\eq$ 의 매끄러운 저역통과(인접 칸끼리 거의 같음)가
되고, $(\xi_\eq-\xi_\mathrm{lag})/L_V$ 는 천천히 변하는 두 신호의 작은 차를 길이로 나눈 이산 미분 $\to\dd\xi_\eq/\dd V$ 의
종에 접근한다(꼬리는 여전히 길지만 모양의 코어가 평형 종을 닮음). 반대로 \emph{작은 $L_V$}에서는
$\rho=e^{-\Delta_\mathrm{grid}/L_V}\to0$ 이라 점화식이 $\xi_{\mathrm{lag},i}\to\xi_{\eq,i}$(\emph{같은} 칸의 $\xi_\eq$,
한 칸 뒤처짐이 아님)로 떨어져 $(\xi_\eq-\xi_\mathrm{lag})/L_V\to0/L_V$ 의 퇴화 신호가 된다 — peak 가 사라진다. 따라서
작은 $L_V$ 에서 평형 peak 식~\eqref{eq:eqpeak} 로의 \emph{환원은 이 저역통과 극한이 아니라} 코드의 명시 분기 스위치가
담당한다 — $L_V$ 가 격자 몇 칸보다 짧으면 꼬리식을 버리고 평형 종 $\xi_\eq(1-\xi_\eq)/w$ 를 직접 평가한다(이 퇴화를
피하려는 것이 분기의 동기다):
\begin{equation}
\text{peak\_shape}=
\begin{cases}
