% ====================================================================
% v7-03b.tex — graphite_ica_ch1 v7 (sub-agent v7-03, 보완본)
...
\hypersetup{colorlinks=true, linkcolor=blue, urlcolor=blue, citecolor=blue,
  pdftitle={흑연 음극 dQ/dV 이론 — Chapter 1 (v7-03b)}}
\pagestyle{fancy}\fancyhf{}
\lhead{흑연 음극 $\mathrm{d}Q/\mathrm{d}V$ 이론 — Ch.1 (v7-03b, 수식-구동)}
...
\title{\textbf{\LARGE Chapter 1}\\[0.4em]
  ...
  코드 진행 N0--N9 를 따라가는 수식-구동판 (v7-03b)}
\author{}
\date{}
...
% 정규화: g/RT, spinodal xi_s^pm = 0.5 +- sqrt(1/3)/2 ~ 0.5 +- 0.2887
...
% spinodal: xi_s^pm = 0.5 +- sqrt(1/3)/2 = 0.2113 / 0.7887
\fill (0.2113,-0.0160) circle (0.5pt) node[above,font=\scriptsize] {$\xi_s^-$};
\fill (0.7887,-0.0160) circle (0.5pt) node[above,font=\scriptsize] {$\xi_s^+$};
...
\draw[->] (-1.15,0) -- (1.18,0) node[below] {$z=\sigma_d(V_n-U_j^d)/w_j$};
...
$w_j^{-1}\xi_{\eq}(1-\xi_{\eq})$ (파선, 식~\eqref{eq:dxidV}). 가로축은
$z=\sigma_d(V_n-U_j^d)/w_j$ — 본문 식~\eqref{eq:ksi} 의 logistic 인수. 미분이
$z=0$($V_n=U_j^d$)에서 $1/(4w_j)$ 의 최댓값을 가지는 종(bell) 모양이며,
...
\label{eq:Acut}
\end{equation}
($z_{\rm cut}=4.357$ 은 $\xi_{\eq}$ 정점의 약 $5\%$ 에 해당하는 컷.) $\mathcal{A}$ 는 전이당 상수다 — 전압 창을 이동해도 컷 지점에서 affinity 를 일정 기준으로 끊으므로, 같은 전이의 모든 $L_q$ 계산에서 $\mathcal{A}$ 값이 바뀌지 않는다(곡선 위에서 $L_q$ 크기만 달라질 뿐). Eyring 식과 방향별
유효 장벽을 결합하면:
\begin{equation}
