% ====================================================================
% v7-01b.tex — 흑연 음극 dQ/dV(ICA) 이론 Chapter 1 (v7, 독립 구현 01번, 보완b)
%   원본 v7-01.tex 의 비파괴 보완본. 변경 항목:
%   (1) N7 동역학 절: A 를 전이당 상수(컷 z_cut·n·RT)로 재서술,
%       L_q 의 전위의존 물리적 동기만으로 격하, 대표 컷 상수 L_V 충실 서술.
%   (2) 닫힌형 L_q 식에 −TΔS_a 항 추가(ΔG_a=ΔH_a−TΔS_a 정합).
%   (3) 로그형 b_j 절편과 닫힌형의 정합 복원.
%   (4) 브리프 §8 부호 체크리스트 전건 재확인 후 이상 없음.
...
\hypersetup{colorlinks=true, linkcolor=blue, urlcolor=blue, citecolor=blue,
  pdftitle={흑연 음극 dQ/dV 이론: Chapter 1 (v7-01b)}}
\pagestyle{fancy}\fancyhf{}
\lhead{흑연 음극 $dQ/dV$ 이론 — Ch.1 (v7-01b, 수식 주도)}\rhead{\thepage/\pageref{LastPage}}
...
\title{\textbf{\LARGE Chapter 1}\\[0.35em]
  \Large 흑연 음극 $dQ/dV$ 이론:\;코드 진행에서 피팅까지 — 수식 주도판 (v7-01b)}
\author{}
\date{}
...
$\Delta G_{a,j}^\eff$ & J/mol & 유효 활성화 자유에너지 $=\Delta H_{a,j}^\eff-T\Delta S_{a,j}$\\
...
\subsection{컷 affinity: 전이당 상수 $A_j$}

\textbf{코드는 전이 $j$ 마다 $A_j$ 를 전위와 무관한 상수로 정의한다.}
$A_j$ 는 정점 이후 $\xi_\eq$ 의 dq 변화율이 정점의 $\sim5\%$ 로 떨어지는 지점의
Logan affinity 추산치로, 전이 폭 $n_j RT$ 의 스케일로 상한 $A_\mathrm{cap} RT$ 까지:
\begin{equation}
\boxed{\;A_j = \min\!\bigl(z_\mathrm{cut}\,n_j RT,\;A_\mathrm{cap} RT\bigr).}
\label{eq:Aj}
\end{equation}
기본값 $z_\mathrm{cut}=4.357$, $A_\mathrm{cap}=4.0$. 코드: \texttt{\_resolve\_lag\_length} L335.
$A_j$ 는 전이당 한 번만 평가되며, 전위 격자 $V_n$ 에 의존하지 않는다.

\paragraph{물리적 동기.}
Butler--Volmer 관점에서 구동 affinity $A$ 가 클수록 정방향 장벽이 낮아져 $k$ 가 증가하고
꼬리가 짧아진다. 코드는 이 물리적 동기를 전이당 대표 컷 값으로 단순화해, 실험에서 측정되는
꼬리 길이와 1:1 대응하는 피팅 가능한 스케일 파라미터를 만든다.
...
활성화 엔트로피 $\Delta S_{a,j}$ 를 포함한 유효 활성화 자유에너지는:
\begin{equation}
\Delta G_{a,j}^\eff = \Delta H_{a,j}^\eff - T\Delta S_{a,j}.
\label{eq:dGeff}
\end{equation}
$\Delta S_{a,j}$ 의 기본값은 $0$ 이다(\texttt{dS\_a}, 코드 L338). 이 항은 로그형 절편에서
온도 불변 상수로 식별되며(\S\ref{sec:fitting} S4), 없으면 $\Delta G_{a,j}^\eff = \Delta H_{a,j}^\eff$.

\subsection{방향별 장벽 — Butler--Volmer 형}

컷 affinity $A_j$(식~\eqref{eq:Aj})에서 정방향 장벽이 $\chi_d A_j$ 만큼 낮아진다.
역방향 몫 $(1+e^{-A_j/RT})$ 까지 포함한 완화 속도 보편형:
\begin{equation}
k_j = k_0\,e^{-(\Delta G_{a,j}^\eff-\chi_dA_j)/RT}\bigl(1+e^{-A_j/RT}\bigr).
\label{eq:kuniv}
\end{equation}
...
\boxed{\;L_{q,j} = \frac{|I|\,h}{Q_\cell k_BT}\,
  \frac{e^{(\Delta G_{a,j}^\eff - \chi_d A_j)/RT}}{1 + e^{-A_j/RT}}\;.}
\label{eq:Lq}
\end{equation}
코드: \texttt{func\_L\_q(T, I, Q\_cell, dH\_a, dS\_a, x, A)} (L90). 코드 내부에서
\texttt{dG\_a = dH\_a - T * dS\_a} 로 $\Delta G_{a,j}^\eff$ 를 직접 계산하고, 이를
$RT$ 로 나누어 지수에 올린다.

로그 형태($-\ln(1+e^{-A/RT}) \approx -A/RT \cdot\mathbf{1}_{A\ll RT}$ 이지만, 일반형으로):
\begin{equation}
\ln L_{q,j} = \ln\frac{T_*}{T} + \frac{\Delta H_{a,j}^\eff - \chi_d A_j}{RT}
  - \frac{\Delta S_{a,j}}{R} - \ln\!\bigl(1+e^{-A_j/RT}\bigr),\qquad
T_* \equiv \frac{|I|\,h}{Q_\cell k_B}.
\label{eq:lnLq}
\end{equation}
$A_j$ 가 전이당 상수(식~\eqref{eq:Aj})이므로 등온에서 $\ln L_{q,j}$ 는 전위와 무관한
\emph{전이당 하나의 수}다. 다온도 실험에서는 온도를 따라 $\Delta H_{a,j}^\eff$ 를
기울기에서 읽고, 나머지를 절편 $b_j = -\Delta S_{a,j}/R - \ln(1+e^{-A_j/RT})$ 에서 읽는다.

\paragraph{$\partial\ln L_{q,j}/\partial V_n$ 의 물리적 해석.}
Butler--Volmer 틀에서 구동 affinity 가 클수록 장벽이 낮아진다는 직관이 있다. 코드에서는
이 의존성이 전이당 단일 컷 상수 $A_j$ 로 단순화되어 전위 연속 함수가 아닌 점을 주의한다.
브리프 §8 체크리스트 항목 6 의 $\partial\ln L_{q,j}/\partial V_n < 0$ 는 이 물리적 동기에
대한 진술이며, 코드 구현 내에서 $L_{q,j}$ 가 전위 격자별로 다르게 계산되는 것은 아니다.
...
\label{eq:LV}
\end{equation}
코드: \texttt{\_resolve\_lag\_length()} (L307) → L351. $|dV/dq|_{q_a}$ 는 컷점
근방의 OCV 기울기로, 전이 딕셔너리의 \texttt{'dVdq\_qa'} 키로 지정한다.
...
S3 & 등온 다전위 꼬리 & $\ln L_V$ vs 전이별 비교 $\to\chi$ & $\chi$\\
...
\subsection{S3: 전이별 꼬리에서 $\chi$ 식별}

$A_j$ 가 전이당 상수(식~\eqref{eq:Aj})이므로, 동일 온도에서 다른 전이의 $\ln L_{V,j}$
를 비교하면 $\chi_d A_j$ 의 상대적 기여에서 $\chi$ 를 읽는다. 구체적으로 식~\eqref{eq:lnLq}
에서 두 전이 $j, j'$ 의 차를 취하면:
\begin{equation}
\ln\frac{L_{V,j}}{L_{V,j'}} = \frac{\Delta H_{a,j}^\eff - \Delta H_{a,j'}^\eff}{RT}
  - \chi_d\,\frac{A_j - A_{j'}}{RT} + \text{const},
\label{eq:S3chi}
\end{equation}
$\Delta H_a$ 와 $A$ 의 전이별 차가 이미 알려진 경우 $\chi$ 를 기울기에서 식별한다.

\subsection{S4: Arrhenius 회귀에서 $\Delta H_a^\eff$ 식별}

$\chi_d A_j$ 와 $\ln(1+e^{-A_j/RT})$ 항을 제거한 보정량:
\begin{equation}
y(T) \equiv \ln\frac{T_*}{L_{q,j}\,T} - \frac{(-\chi_d A_j)}{RT} + \ln\!\bigl(1+e^{-A_j/RT}\bigr)
...
\item $A_j = \min(z_\mathrm{cut} n_j RT, A_\mathrm{cap}RT)$ — 전이당 상수; $L_{q,j}$ 는 전위 독립 스칼라.
\item 충전 격자역전 후 dQ/dV 양수 유지.
\item 분극: $V_n = V_\app - \sigma_d|I|R_n$.
\end{enumerate}

항목 6 을 v7-01b 에서 재정의: $\partial\ln L_{q,j}/\partial V_n<0$ 는 Butler--Volmer 의
물리적 동기이나 코드에서 $L_{q,j}$ 는 전이당 한 번 계산되는 상수다. 브리프의
``$V\uparrow\Rightarrow A\uparrow\Rightarrow$꼬리 짧아짐'' 서술은 $z_\mathrm{cut}$ 의
온도·전이 의존을 통한 간접 효과로 이해한다.
...
$\Delta S_{a,j}=0$: 식~\eqref{eq:Lq} 에서 $\Delta G_{a,j}^\eff = \Delta H_{a,j}^\eff$ — 코드 기본값 정합.
