% ====================================================================
% v7-02.tex  —  graphite_ica_ch1 v7 (경쟁 구현 02)
%   흑연 음극 dQ/dV(ICA) 이론 Chapter 1
%   절 순서 = N0→N9 (v11_flowchart.md 공유 척추)
%   코드 정본: Anode_Fit_v11_final.py (706줄)
%   형식 참조: graphite_ica_ch1_Opus_v5.tex
...
\hypersetup{colorlinks=true, linkcolor=blue, urlcolor=blue, citecolor=blue,
  pdftitle={흑연 음극 dQ/dV 이론: v11 코드 기반 수식-구동 --- Chapter 1 (v7-02)}}
\pagestyle{fancy}\fancyhf{}
\lhead{흑연 음극 $dQ/dV$ 이론 --- Ch.\,1 (v7-02)}\rhead{\thepage/\pageref{LastPage}}
...
$\mathcal{A}$ & J/mol & 구동력(affinity) $=\min(z_{\mathrm{cut}}\,n_j RT,\,A_{\mathrm{cap}}RT)$ \\
...
$\partial\ln L_{q,j}/\partial V < 0$ (전위 상승 $\Rightarrow$ 꼬리 짧아짐).
...
부호: 방전에서 $V_n < V_\app$이므로 평형보다 낮은 내부 전위 — 즉 전위 평탄 구간이
측정보다 낮게 놓인다. 이것이 방전 ICA peak가 OCV peak보다 낮은 전위에 나타나는 이유다.
\end{verifybox}
...
정·역 속도 $r_j^\pm$의 균형(detailed balance)에서 정지점이
\begin{equation}
  \xi_{\sstat,j} = \frac{r_j^+}{r_j^+ + r_j^-}
  = \frac{e^{\mathcal{A}_j/RT}}{1+e^{\mathcal{A}_j/RT}}
  \label{eq:stationary}
\end{equation}
으로 유도된다. 구동력 $\mathcal{A}_j = \sigma_d F(V_n - U_j^d)$를 폭 $w_j$로 묶으면,
정지점이 곧 평형 점유율이고
\begin{equation}
...
\subsection{구동력과 Butler--Volmer 속도}

전위 $V_n$과 전이 중심 $U_j^d$의 차를 전기화학 affinity로 정의한다.
꼬리 컷점에서의 affinity는 $z_{\mathrm{cut}}$과 상한 $A_{\mathrm{cap}}$로 제한된다:
\begin{equation}
  \mathcal{A} = \min\!\big(z_{\mathrm{cut}}\,n_j\,RT,\; A_{\mathrm{cap}}\,RT\big)
  \label{eq:affinity}
\end{equation}
[\texttt{\_resolve\_lag\_length}, L335]. 기본값 $z_{\mathrm{cut}}=4.357$은
$\xi_{\eq}$가 정점의 약 5\%가 되는 컷점이고, $A_{\mathrm{cap}}=4.0$이다.
...
전위 의존: $\partial\ln L_{q,j}/\partial V_n < 0$
(전위 상승 $\Rightarrow$ affinity 증가 $\Rightarrow$ 장벽 감소 $\Rightarrow$ 꼬리 짧아짐).
...
$L_{q,j}\to0$ → 꼬리 없음 (평형 회복).
$\partial\ln L_{q,j}/\partial V_n$: 식~\eqref{eq:lnLq}에서 $V_n$ 의존항은
$-\chi_d\mathcal{A}/RT = -\chi_d\sigma_d F(V_n-U_j^d)/RT$이므로
기울기 $= -\chi_d\sigma_d F/RT \le 0$ ($\sigma_d=+1$, $\chi_d>0$) \checkmark.
\end{verifybox}
...
peak\_shape $\to 0/0$이나 L'Hôpital로 식~\eqref{eq:dxidV}의 평형 peak가 회복된다.
...
\item $|I|\to0$: $L_{V,j}\to0$, $\xi_{\mathrm{lagged}}\to\xi_{\eq}$ 추종 → L'Hôpital에 의해
...
꼬리 길이 & 지수 증가($e^{\Delta H_a/RT}$) & 비례 증가($\propto|I|$) & 감소 ($\partial\ln L/\partial V<0$)\\
Peak 높이 & 꼬리 경쟁으로 감소 & 감소 (지연 증가) & 꼬리 단축으로 증가\\
\bottomrule
\end{tabular}
\end{center}
