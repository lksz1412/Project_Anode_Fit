...
\code{w} 또는 \code{n} & 전이 & --- / $1$ & 폭 $w_j=n_jRT/F$ (\eqref{eq:weff}) \\
\code{Q} & 전이 & --- & 용량 가중(면적) \\
...
(3) 전이마다 $U_j(T)$(\eqref{eq:Uj}) → 분기 중심 \code{center}(\eqref{eq:center}) → 폭 $w_j$(\eqref{eq:weff}) → 평형 점유 $\xi_\eq$(\eqref{eq:xieq}).
