% ====================================================================
% graphite_ica_ch1_v7-11.tex — Chapter 1 (v7-11: 9+9+1+1 최종본)
...
% graphite_ica_ch1_v7-10.tex — Chapter 1 (v7-10: 9+9 경쟁·검토 체리픽 통합본)
%   베이스 = v7-06b(검토2 30/30·부호 8/8·신규회귀 0). v6 합류로 물리(spinodal gap·χ_d/ΔH_eff·
%   분극)를 교차검증(일치), v11 의 컷-상수 𝒜 가 v6 의 전위의존 𝒜 보다 코드 정합임을 확인.
%   graft = 전체 입력 인자·기본값 표 + 6단계 재현 박스(사용자 목표: 전 인자 입력·Optuna 피팅 준비).
%   (이식 금지 회귀 — v7-01b arrhenius 부호·v7-03b fig:logistic 손상·v7-09b logitsolve n_j² — 미채택.)
%   ── 이하 베이스 v7-06b 원형 ──
...
$\xi/(1-\xi)$ 가 두 끝점에서 서로 역수, $(1-2\xi_{s,j}^-)=+u_j$, $(1-2\xi_{s,j}^+)=-u_j$ 이고, 극대에서 극소를 빼면 $U_j$ 가 상쇄되어
\begin{equation}
\boxed{\;\Delta U_j^\hys
...
의 출력). 매끈한 차분 $(\xi_\eq-\xi_\mathrm{lag})/L_V$ 가 미분 $\dd\xi_\eq/\dd V$ 의 종으로 수렴하는 극한은
$L_V\gg\Delta_\mathrm{grid}$($\rho\to1$)이다 — 반대로 $L_V\to0$ 이면 $\rho\to0$ 이라 $\xi_\mathrm{lag}\to\xi_\eq$
(같은 칸; 한 칸 뒤처짐이 아니다)이고 분자 $(\xi_\eq-\xi_\mathrm{lag})$ 와 분모 $L_V$ 가 함께 $0$ 인 $0/0$ 이라 종으로 매끈히
환원되지 \emph{않는다}. 작은 $L_V$ 의 평형 회복은 이 식의 극한이 아니라 코드의 \emph{이산 분기 스위치}가 담당한다 —
문턱 $L_V<\nu\Delta_\mathrm{grid}$ 에서 꼬리 분기와 평형 종의 진폭이 정확히 같지는 않아 작은 불연속이 있을 수 있는,
수치 안정용 모드 전환이다:
\begin{equation}
\text{peak\_shape}=
\begin{cases}
