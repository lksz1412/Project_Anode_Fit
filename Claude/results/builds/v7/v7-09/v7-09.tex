...
\node[box,right=of n1] (n2) {N2\\U\_j(T)};
\node[box,right=of n2] (n3) {N3\\hysteresis};
\node[box,below=of n3] (n4) {N4\\width};
\node[box,left=of n4] (n5) {N5\\ksi\_eq};
\node[box,left=of n5] (n6) {N6\\peak};
\node[box,left=of n6] (n7) {N7\\L\_V};
\node[box,below=of n7] (n8) {N8\\lowpass};
\node[box,right=of n8] (n9) {N9\\sum};
\draw[arr] (n0) -- (n1);
\draw[arr] (n1) -- (n2);
\draw[arr] (n2) -- (n3);
\draw[arr] (n3) -- (n4);
...
\draw[arr] (n8) -- (n9);
\end{tikzpicture}
\caption{v11 계산 흐름을 절 순서로 옮긴 그림이다. 노드 이름은 코드의 계산 순서와 맞추었고, 그림 내부 텍스트는 새로 작성한 English ASCII 라벨만 사용했다.}
\label{fig:spine}
\end{figure}
...
\label{eq:polar}
\end{equation}
방전에서는 $\sigma_d=+1$ 이므로 $V_n=V_{\app}-I_{\abs}R_n$ 이다. 충전에서는 $\sigma_d=-1$ 이므로 $V_n=V_{\app}+I_{\abs}R_n$ 이다. 이 부호 하나가 관측축과 열역학축을 분리하고, 이후 모든 전이 계산은 $V_{\work}$ 위의 내부 전위로 진행된다.
...
\toprule
transition & \texttt{U} & \texttt{w} & \texttt{Q} & \texttt{Omega} & \texttt{dH\_rxn} & \texttt{dS\_rxn}\\
\midrule
stage $4\to3$ & 0.210 & 0.020 & 0.10 & 6000 & -11700 & +29\\
stage $3\to2L$ & 0.140 & 0.016 & 0.12 & 8000 & -13500 & 0\\
stage $2L\to2$ & 0.120 & 0.014 & 0.25 & 10000 & -13100 & -5\\
stage $2\to1$ & 0.085 & 0.012 & 0.50 & 13000 & -13000 & -16\\
\bottomrule
\end{tabular}
\end{center}
...
% ====================================================================
N2 의 $U_j(T)$ 는 중심이고, N3 의 폭 $w$ 는 그 중심 주변에서 점유율이 얼마나 빠르게 바뀌는지를 정한다. 코드는 먼저 \texttt{n} 또는 \texttt{w} 에서 다중도 $n_j$ 를 얻는다.
...
먼저 \texttt{func\_dU\_hys} 는 spinodal 상한 gap 을 계산한다.
\begin{equation}
...
\label{eq:LV}
\end{equation}
식~\eqref{eq:lnLq} 에서 $V$ 가 커져 cutoff affinity 가 커지면 $-\chi_dA/(RT)$ 항이 작용해 $\ln L_q$ 가 감소한다. 따라서 전위 상승은 장벽을 낮추고 꼬리를 짧게 만든다.
