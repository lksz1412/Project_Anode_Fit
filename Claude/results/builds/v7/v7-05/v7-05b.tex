...
관측 전위 $V_\app$ 는 평형 전위가 아니다. 전류가 흐르면 직렬 저항 $R_n$ 에 분극이 걸려, 측정값이 내부(열역학) 전위에서 어긋난다. 어긋남의 \emph{부호}가 방향에 달렸다. 흑연 음극은 셀 방전에서 탈리튬화(산화)되는 전극이므로 양(anodic)의 과전압을 받는다 — 과전압의 정의 $\eta=V_\app-V_{\eq}$ 에서 $\eta>0$, 곧 방전($\sigma_d=+1$)에서는 분극이 측정 전위를 내부(평형) 전위보다 \emph{위로} 올려 읽히게 한다. 충전(리튬화, 환원)은 음의 과전압이라 측정 전위가 내부 전위보다 \emph{아래로} 내려 읽힌다. 따라서 내부 전위는 측정값에서 이 과전압을 \emph{벗겨} 복원하며, 그 한 줄이 N1 의 전부다:
\begin{equation}
\boxed{\;V_n\;=\;V_\app\;-\;\sigma_d\,|I|\,R_n\;.}
\label{eq:vn}
\end{equation}
부호를 검산하면 — 방전 시 $\sigma_d|I|R_n=+|I|R_n>0$ 이라 식~\eqref{eq:vn} 은 $V_n=V_\app-|I|R_n<V_\app$ 를 준다. 곧 측정 전위가 평형보다 \emph{높게} 읽힌 양의 과전압을 벗겨 내부 전위로 낮춘다(앞 문단 $\eta>0$ 과 합치). 충전 시 $\sigma_d=-1$ 이라 $V_n=V_\app+|I|R_n>V_\app$ 로, 음의 과전압을 도로 채워 올린다. $|I|\to0$ 또는 $R_n=0$ 이면 $V_n=V_\app$ 로 측정$=$내부가 합치한다. 분극은 $|I|$ 에 선형인 lumped 근사이며, 계면 과전압을 내부 전위에 흡수한 그림이다 — 율속이 고상 재배열$\cdot$핵생성에 지배될 때 타당하다.
...
\begin{boundbox}
\textbf{전위 의존의 부호 — 동기 부등식과 운영상 상수의 구분.} 두 지위를 갈라 둔다. \emph{운영상}으로 $\mathcal A$ 는 식~\eqref{eq:Acut} 의 \emph{컷} $\mathcal A=\min(z_\mathrm{cut}\,nRT,\,A_\mathrm{cap}RT)$ 라, 전이당 한 번 산출되는 \emph{상수}다($V_n$ 에 무의존). 따라서 코드가 실제로 평가하는 $L_V$ 는 전이당 상수이고, 격자 위 운영 미분은 $\partial_{V_n}\mathcal A=0$ 이라 $\partial\ln L_{q,j}/\partial V_n=0$ 이다 — 꼬리 길이가 $V_n$ 을 따라 변하지 않는다. \emph{동기}로서의 부등식은 다른 그림이다 — 만약 $\mathcal A$ 를 컷이 아니라 달리는 affinity~\eqref{eq:affinity} $\mathcal A_j=\sigma_dF(V_n-U_j)$ 로 보아 식~\eqref{eq:lnLq} 을 $V_n$ 으로 미분하면 $\partial\ln L_{q,j}/\partial V_n=-\chi_d\sigma_dF/RT\le0$($\sigma_d=+1$ 방전 기준)이 되어, 구동이 깊을수록 꼬리가 짧아진다(``전위$\uparrow\Rightarrow$꼬리$\downarrow$''). ★부호 체크리스트 6번은 이 \emph{모델 정합 방향}을 가리키는 동기 부등식이지, 운영 미분이 음수라는 주장이 아니다(운영상은 컷 상수라 $0$). 선형 근사 작동 창은 $3RT\lesssim\mathcal A_j\ll\lambda$(Marcus 재구성 에너지)이고, 삽입 상전이의 $\lambda$ 는 수십 kJ/mol 이라 넉넉하다 \cite{marcus1956}.
\end{boundbox}
...
\subsection{부호 회귀 self-test — 전건 한 표로}\label{sec:signregression}
닫힌식을 떠나기 전에, 본 장의 \emph{부호 사슬}을 코드 v11 과 1:1 로 대조하는 회귀 점검을 한 표에 모은다. 각 행은 방전 기준($\sigma_d=+1$)으로 ``입력 변화 $\Rightarrow$ 출력 부호''를 못박고, 충전($\sigma_d=-1$)이 그 거울임을 적는다. 한 행이라도 어긋나면 곡선 부호가 깨진 것이다. 이 표는 코드 \code{\_\_main\_\_} 의 self-test(평형 peak 양수$\cdot$$|I|\to0$ 방충 일치$\cdot$$\Delta U^\hys|_{\Omega=2RT}=0$)를 식 수준으로 확장한 \emph{부호 회귀}다.
...
\toprule
\# & 식(라벨) & 방전 부호 단언($\sigma_d=+1$) & 충전 거울 & 코드 \\
\midrule
1 & $U_j(T)$~\eqref{eq:Uj} & $\Delta H_\rxn<0\Rightarrow U_j>0$ 으로 중심 상승 & 방향 무관(상태함수) & \code{func\_U\_j} \\
2 & $\xi_\eq$~\eqref{eq:logistic} & $V_n\!\uparrow\Rightarrow z\!\uparrow\Rightarrow\xi_\eq\!\uparrow$(탈리튬화) & $V_n\!\downarrow\Rightarrow\xi_\eq\!\uparrow$ & \code{func\_ksi\_eq} \\
3 & $\dd\xi_\eq/\dd V_n$~\eqref{eq:dxidV} & $\sigma_d\xi(1{-}\xi)/w>0$(peak 양수) & $\sigma_d^2{=}1$ 로 양수 유지 & (인라인) \\
4 & $\Delta U^\hys$~\eqref{eq:hysdU} & $\Omega>2RT\Rightarrow\Delta U^\hys\!\ge\!0$; $\Omega\le2RT\Rightarrow0$ & gap 방향 무관 & \code{func\_dU\_hys} \\
5 & $U_j^d$~\eqref{eq:hyscenter} & $+\tfrac12\gamma\Delta U^\hys$(중심 위로) & $-\tfrac12\gamma\Delta U^\hys$(아래로) & \code{func\_U\_branch} \\
6 & $\chi_d,\Delta H_a^\eff$~\eqref{eq:chid}\eqref{eq:dHeff} & $\chi_d{=}\chi$, $\Delta H_a^\eff{=}\Delta H_a{-}\chi\Omega$ & $\chi_d{=}1{-}\chi$ & \code{func\_chi\_d} \\
7 & $\ln L_q$~\eqref{eq:lnLq} & 운영상 $\mathcal A{=}$컷 상수 $\Rightarrow\partial_{V_n}\!\ln L_q{=}0$; 동기 부등식 $<0$ & 거울(상수) & \code{func\_L\_q} \\
8 & 꼬리 역전~\eqref{eq:reversal} & 격자 그대로 저역통과 & 격자 역전 $[::\!-\!1]$ 후 되돌림 & \code{\_causal\_lowpass} \\
9 & 분극~\eqref{eq:vn} & $V_n{=}V_\app{-}|I|R_n<V_\app$($\eta>0$) & $V_n{=}V_\app{+}|I|R_n>V_\app$ & \code{V\_n=\dots} \\
\bottomrule
\end{tabular}
\caption{부호 회귀 self-test. 방전($\sigma_d=+1$)을 기준 전건으로 두고, 각 식의 부호 단언과 충전 거울$\cdot$코드 식별자를 1:1 로 못박는다. 핵심 사슬 — 방전 시 \emph{$V$ 상승 $\Rightarrow$ 탈리튬화 진행 $\xi$ 증가 $\Rightarrow$ 평형 peak 양수}, 충전 $\dd Q/\dd V$ 는 N1$\cdot$N3$\cdot$N8 의 세 $\sigma_d$ 작용을 거친 방전의 거울이며 양수를 유지. 7번은 운영상 컷 상수라 미분 $0$, 부등식은 모델 정합 동기임을 명시(§\ref{sec:tail} boundbox 와 정합). 9번은 흑연 음극이 방전에서 산화되어 양의 과전압을 받음과 합치(§\ref{sec:polar}).}
\label{tab:signregression}
\end{table}

\medskip
\emph{도착.} 한 줄 합산식~\eqref{eq:closed} 과 그 모든 기호의 코드 식별자$\cdot$초기값(표~\ref{tab:staging})$\cdot$산출 순서(그림~\ref{fig:flow})$\cdot$부호 회귀(표~\ref{tab:signregression})가 닫혔다. 곡선을 데이터에 맞추는 비순환 피팅 사슬 — 저율 OCV 로 $\{U_j,w_j,Q_j,C_\bg\}$, 전류 차단으로 $R_n$, 다전위 꼬리로 $\chi$, 다온도로 $\Delta H_a^\eff$, 다율 gap 으로 $\{\gamma_j,\Omega_j\}$ 를 단계마다 한 겹씩 동결 — 은 같은 닫힌식의 기호를 측정 파일에서 정하는 절차이며, 본 장의 식들이 그 각 단계의 모델 함수를 그대로 제공한다.
