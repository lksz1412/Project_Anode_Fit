...
$w_j$ (\code{func\_w}) & V & 전이 폭 척도: 단상은 $n_jRT/F$ 평형 예측, 두-상은 현상학적 피팅 폭 \\
...
상전이를 \emph{억제}한다 — 정규용액 틀에서 이는 $\Omega_j$ 를 $2RT$ 쪽으로 낮추거나, 관측 폭 $w_j$ 를 현상학적으로 크게
피팅해 peak 를 smear 하고 $U_j$ 를 미세 shift 시키는 것으로 들어온다. 흑연 \code{GRAPHITE\_STAGING\_LIT} 가 초기값이고 폭이
...
\subsection{폭 $w_j$ — 단상 평형 예측과 두-상 현상학적 폭}
평형 점유의 폭 척도는 단상(solid-solution, $\Omega_j<2RT$) 이상 극한에서 $w_j=n_jRT/F$ 다($n_j$ 는 폭 다중도):
아래 \S\ref{sec:width} logistic 유도의 중심 기울기 $\dd\xi_\eq/\dd V|_{1/2}=sF/(4RT)$ 가 폭 척도 $RT/F$ 를 정의하고,
폭 다중도 $n_j$ 로 일반화하면
\begin{equation}
...
여기서 $w_j$ 의 지위는 전이 성격에 따라 갈린다. dilute$\to$stage 4 와 $4\mathrm L\leftrightarrow3\mathrm L$ 처럼
plateau 가 없는 solid-solution 전이는 단일 입자 평형 등온선 자체가 이미 유한한 종을 만들므로, $w_j=n_jRT/F$ 는 검증
가능한 평형 예측이다. 반대로 $\Omega_j>2RT$ 의 staging 두-상 전이에서는 평형 단일 입자 응답이 plateau, 이상적으로는
델타에 가까운 선으로 가므로, 실측 유한 폭을 $nRT/F$ 평형 예측으로 해석하지 않는다. 이 경우 $w_j$ 는
\S\ref{sec:broadening} 의 broadening 을 흡수하는 \emph{현상학적 자유 피팅 폭}이다. 곧 같은 기호 $w_j$ 라도
단상에서는 평형 등온선의 폭, 두-상에서는 apparent-$U=U_j+\eta$ 의 유한율속$\cdot$내재 폭을 담는 피팅 파라미터다.
...
\subsection{평형 델타와 실측 종 — broadening 의 범위}\label{sec:broadening}
실측 흑연 ICA 의 모든 peak 를 ``원래 델타였는데 분포로 넓어진 것''으로 보지 않는다. 출발점은 전이별로 갈린다.
dilute$\to$stage 4 와 $4\mathrm L\leftrightarrow3\mathrm L$ 전이는 plateau 가 없는 solid-solution 성격이므로,
단일 입자 평형 $dQ/dV$ 가 Frumkin/정규용액 등온선의 기울기 자체로 이미 폭 $\sim nRT/F$ 의 종이다
\cite{levi1999,rsc2021}. 이 두 전이에는 별도의 델타-대-종 broadening 설명이 필요하지 않다. 반면
$2\mathrm L\to2(\mathrm{LiC}_{12})$ 와 $2\to1(\mathrm{LiC}_6)$ 전이는 두 상 공존 plateau 의 강한 1차 전이이므로,
평형 단일 입자 응답은 델타에 가까운 선이 기준이다. 따라서 ``왜 실측이 유한한 종인가''라는 질문은 이 두-상 전이에
해당한다.

두-상 전이가 유한한 종으로 보이는 주된 이유는 평형 $U_j$ 자체의 분포가 아니라 측정 조건이 만드는 apparent-$U$ 의
분포다. 유한 전류에서는 표면 SOC 가 평형을 앞서거나 뒤처져 과전압 $\eta$ 가 SOC 와 함께 변하고, peak 중심이 밀리며
진행 방향 뒤쪽으로 비대칭 꼬리가 생긴다. 본 모델에서 이 효과는 N7/N8 의 지연 길이 $L_V$ 와
$(\xi_\eq-\xi_\mathrm{lag})/L_V$ 꼬리로 들어가며, C-rate 가 커질수록 강해지고 $C\to0$ 에서 사라지는 kinetic 성분이다
\cite{fly2020}. 그 위에 평탄역 양끝의 단상 꼬리와 staging 정렬 폭이 만드는 내재 $RT/F$ 규모의 하한이 깔린다. 그러므로
두-상 전이의 $w_j$ 는 평형식이 예측한 폭이 아니라, $U_\mathrm{app}=U_j+\eta$ 에서 분포하는 $\eta$ 와 내재 폭을 한
전이별 숫자로 흡수한 \emph{현상학적 자유 피팅 폭}이다. 이 지위가 단상 전이의 $w_j=n_jRT/F$ 평형 예측과 다르다.

\begin{keybox}
\textbf{범위 경고.} 이 장은 peak 에서 평형 $U_j$ 분포나 입자 반경 분포를 역산하지 않는다. 마이크론 흑연에서 표면에너지
또는 밴드 효과가 반경을 통해 평형 $U_j$ 를 의미 있게 이동시키는 채널은 무시 가능한 크기이고, GITT 로 읽는 평형
$U_j$ 는 입자 크기별 분포가 아니라 상수에 가깝다\cite{cogswell2012,park2021}. 또한 $dQ/dV$ 하나에서 분포를 역산하는
문제는 비유일한 ill-posed 역문제이므로, 본 장은 forward-only 로만 쓴다. 입자 heterogeneity 는 별도 다입자 또는
PSD convolution 모델을 만들지 않고, 관측 폭 $w_j$ 와 동역학 꼬리 $L_V$ 에 현상학적으로 흡수되는 것으로만 언급한다.
\end{keybox}
...
폭의 해석은 흑연과 같다: 단상 전이는 $w_j=n_jRT/F$ 평형 예측이고, $\Omega_j>2RT$ 의 두-상 plateau 전이는
\S\ref{sec:broadening} 의 현상학적 $w_j$ 를 피팅한다. T1 의 위치는 \S\ref{sec:lco-electronic} 의 전자항이
...
\code{use\_dH\_eff} & 전이/공통 & True & 유효 장벽 옵션 토글(\eqref{eq:dHeff}) \\
...
N4 & $w_j$: 단상 평형 폭 / 두-상 현상학적 폭 & \eqref{eq:wbase}, \S\ref{sec:broadening} & \code{func\_w},\code{\_width} \\
...
\bibitem{levi1999} M. D. Levi and D. Aurbach, ``Frumkin intercalation isotherm---a tool for the description of lithium insertion into host materials,'' \emph{Electrochim. Acta} \textbf{45}, 167 (1999). DOI: 10.1016/S0013-4686(99)00202-9.
\bibitem{rsc2021} Graphite staging source used here for plateau character and the 4L--3L exception (DOI-only entry from project research note), DOI: 10.1039/D0TA12607A.
\bibitem{fly2020} A. Fly, E. Schaltz, and D.-I. Stroe, ``Prediction of differential voltage curves for lithium-ion batteries,'' \emph{J. Energy Storage} \textbf{29}, 101329 (2020). DOI: 10.1016/j.est.2019.101329.
\bibitem{cogswell2012} D. A. Cogswell and M. Z. Bazant, ``Coherency strain and the kinetics of phase separation in LiFePO$_4$ nanoparticles,'' \emph{ACS Nano} \textbf{6}, 2215 (2012). DOI: 10.1021/nn204177u.
\bibitem{park2021} J. H. Park, H. Yoon, Y. Cho, and C.-Y. Yoo, ``Investigation of Lithium Ion Diffusion of Graphite Anode by the Galvanostatic Intermittent Titration Technique,'' \emph{Materials} \textbf{14}, 4683 (2021). DOI: 10.3390/ma14164683.
