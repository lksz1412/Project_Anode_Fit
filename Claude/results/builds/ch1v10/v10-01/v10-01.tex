...
\subsection{폭 $w_j$ — 이상 극한, 이중지위, 옵션}
...
\textbf{★$w_j$ 이중지위.} 흑연의 네 전이는 지위가 둘로 갈린다.
\begin{itemize}[leftmargin=1.6em,itemsep=1pt]
\item \textbf{단상 전이 ($\Omega_j<2RT$, dilute$\to$stage\,4·4L$\leftrightarrow$3L)}: $w_j=n_jRT/F$ 는 등온선 폭의
  \emph{검증 가능한 평형 예측}이다 — 정규용액/Frumkin 틀에서 연속 전이의 폭이 직접 이 값으로 닫힌다.
\item \textbf{두-상 전이 ($\Omega_j>2RT$, 흑연 staging 4건 모두: 2L$\to$2·2$\to$1)}: 평형 단일 입자는 델타에 가까워
  $w_j=n_jRT/F$ 는 적용 불가이다. 실측 peak 의 유한 종 폭은 \S\ref{sec:broadening} 이 설명하는 broadening 기작이
  정하는 값이며, $w_j$ 는 그 결과를 흡수하는 \emph{현상학적 자유 피팅 폭}이다(평형 예측 아님).
\end{itemize}

유효 폭 옵션은, 비단조 등온선~\eqref{eq:Veq} 의 중심 기울기를 이상 logistic 의 중심 기울기와
같다고 둔 데서 오는 코드 토글이다 — $sF\,\dd V_\eq/\dd\xi|_{1/2}=4RT-2\Omega_j$ 와 $4Fw^\eff_j$ 를 같다고 풀면($s$ 부호는 양변 공통이라 상쇄)
\begin{equation}
...
기본 \code{w\_eff\_floor\_frac}$=0.05$). \textbf{★주의: 두-상 전이에서 이 식을 ``델타에서 종으로의 좁힘'' 으로 해석하는 것은
옳지 않다} — 실측 두-상 peak 는 델타가 아니라 유한 종이며, 그 폭을 결정하는 것은 broadening 기작이지 $w^\eff$ 가 아니다.
따라서 기본(\code{use\_w\_eff=False})이 올바른 운용 설정이고, 토글은 단상 등온선의 상호작용 효과를 탐색하는 도구로만 한정된다.
기본 설정은 $w_j=n_jRT/F$ 다(\code{\_width}).
...
폭은 흑연과 같은 $w_j=n_jRT/F$ 기본 식이며(\S\ref{sec:broadening} broadening 이 두-상 폭을 정하고 $w_j$ 가
현상학적으로 흡수함), order--disorder 의 큰 $\Omega$ 는 히스 gap $\Delta U^\hys$ 를 키워 T2$\cdot$T3 peak 를
흑연보다 날카롭게 분리시킨다. T1 의 위치는 \S\ref{sec:lco-electronic} 의 전자항이
...
% ====================================================================
\section{평형 peak 의 broadening — 왜 델타가 아니라 종인가}\label{sec:broadening}
% ====================================================================
앞 절의 평형 peak~\eqref{eq:eqpeak} 에서 이상 단일 입자의 두-상 전이는 델타 함수에 가깝다 — 그런데 실측 dQ/dV 는
유한 폭의 비대칭 종을 보인다. 이 절은 그 이유를 전이별로 정리하고, 두-상 전이의 $w_j$ 가 왜 현상학적 자유 피팅 폭인지를
확정한다. \textbf{모델은 확장하지 않는다} — 이 절은 설명(텍스트·물리 근거)만이며, 다입자/PSD convolution 모델은 만들지 않는다.

\subsection{(a) 전이별 출발 — 애초에 델타인 전이만 문제}\label{sec:broadening-type}
흑연의 네 staging 전이는 실험적 상 거동에 따라 두 부류로 나뉜다.

\textbf{연속/solid-solution 전이 (dilute$\to$stage\,4, 4L$\leftrightarrow$3L):}
이 두 전이는 plateau 가 없는 연속 고용체 전이라, 단일 입자 평형 dQ/dV 가 정규용액/Frumkin 등온선의
기울기 $\dd x/\dd E$ 로서 \emph{애초에 폭 $\sim n_jRT/F$ 의 종}이다 — broadening 추가 기작이 불요하다.
(Levi \& Aurbach 1999, DOI: 10.1016/S0013-4686(99)00202-9; RSC 2021, DOI:
10.1039/D0TA12607A: ``plateau\,\ldots except for 4L--3L''.)

\textbf{두-상/plateau 전이 (2L$\to$2 = LiC$_{12}$, 2$\to$1 = LiC$_6$):}
이 두 전이는 두 상이 공존하는 plateau 를 가져 평형 단일 입자는 델타에 가깝다.
\textbf{따라서 ``왜 델타가 아니라 종이냐?'' 는 이 두 전이(LiC$_{12}$·LiC$_6$)에만 해당한다.}

참고로 코드의 $\Omega_j$ 파라미터는 표~\ref{tab:staging} 에서 4건 모두 $>2RT$($\approx4958$ J/mol at $25^\circ$C)
이며, 이 때문에 \S\ref{sec:width} 에서 $w_j$ 이중지위는 staging 4건 전체에 적용된다 —
폭 $w_j$ 는 단상 연속(등온선 폭·평형 예측) 과 두-상 plateau(broadening 흡수·현상학적 피팅 폭)의 지위가 다르다.

\subsection{(b) 두-상 델타를 종으로 만드는 기작 — 동역학과 내재 폭}\label{sec:broadening-mech}
실측 유한 종 폭은 평형 입자 분포가 아니라 \emph{측정 조건}이 만든다. 두 기여가 합성된다. 셋 다 평형 $U_j$ 분포가 아닌 것에 주의한다.

\textbf{(b-i) 단일입자 유한율속 비대칭 꼬리 [확정]:}
단일 입자라도 유한 전류에서 표면 SOC 가 평형 $\xi_\eq$ 에 앞서거나 뒤처진다 — 과전압 $\eta(\xi)$ 가 SOC 에 따라
변하며 peak 를 한쪽으로 밀고 꼬리를 늘인다. 이것이 코드의 $(\xi_\eq-\xi_\mathrm{lag})/L_V$(N7/N8, \S\ref{sec:tail})다.
C-rate 증가로 심화하고, 평형($|I|\to0$)에서 소멸한다.
(Fly, Schaltz \& Stroe, \emph{J. Energy Storage} 2020, DOI: 10.1016/j.est.2019.101329.)

\textbf{(b-ii) 입자 크기 동역학 분산 [확정·강한 지지]:}
고정 셀 전류 하 확산 시상수 $\tau\propto r^2/D$ 가 입자 반경의 제곱으로 벌어져, 작은 입자는 먼저·큰 입자는 늦게
전이를 완료한다. 같은 인가 전위에서 입자군이 서로 다른 국소 SOC 를 가져 전이가 전압 구간에 퍼진다.
(Yang et al.\ 2023, \emph{Nat.\ Commun.}, DOI: 10.1038/s41467-023-40574-6: 흑연 operando, 전환 직경 $\sim7$ $\mu$m;
Kirk et al.\ 2022, DOI: 10.1137/20M1344305; R\"oder 2016, DOI: 10.1002/ente.201600232.)
이 분산이 사라지는 극한은 $|I|\to0$ (또는 단일 입자 측정) 이다.

\textbf{(b-iii) 내재 RT/F 폭 [확정]:}
평탄역 양끝 단상 꼬리·staging 정렬 폭이 $\sim n_jRT/F$($\approx26$ mV at $25^\circ$C)의 하한을 깐다.

\textbf{apparent-U = $U_j + \eta$:} 관측 peak 전위 $V_\mathrm{peak} = U_j + \eta$ 에서 \emph{분포하는 것은 과전압
$\eta$}이고, 평형 $U_j$ 는 GITT 상 입자 무관 열역학 상수다(Park 2021, DOI: 10.3390/ma14164683).

\subsection{(c) 범위 경고 — 무엇을 하지 않는가}\label{sec:broadening-scope}
\begin{itemize}[leftmargin=1.6em,itemsep=2pt]
\item \textbf{평형 $U_j$ 분포·반경 분포 추출 안 함:} 반경 $\to$ 평형 $U_j$ 채널(Gibbs-Thomson·양자가둠)은
마이크론 흑연에서 $\sim0.01$ mV 로 무시된다(Cogswell \& Bazant 2012 임계 22 nm 훨씬 위).
평형 $U_j$ 는 GITT 상 입자 무관 상수이므로 dQ/dV 에서 반경 분포를 역산하지 않는다.
\item \textbf{dQ/dV $\to$ 분포 역산 = ill-posed(forward-only):} 다입자/PSD convolution 전달함수 역산은
비유일(1종 Fredholm 적분)이라 ill-posed 다 — forward 방향으로 예측을 데이터와 대조하는 것만 한다.
\item \textbf{★다입자/PSD convolution 모델은 만들지 않는다:} 입자 heterogeneity 는 현상학적 $w_j$ 에
흡수되는 것으로만 언급하며, 별도 분포 모델·추출 경로는 없다.
\end{itemize}

\textbf{★두-상 전이의 $w_j$ 는 현상학적 자유 피팅 폭이다.}
위 기작(b-i)\,(b-ii)\,(b-iii)이 합성되어 두-상 전이에 유한 종 폭을 만들고, 코드의 $w_j$ 파라미터가 그 결과를 흡수한다.
$w_j$ 는 평형 예측이 아니라 \emph{피팅 대상 파라미터}이며, 이것이 단상 $w_j = n_jRT/F$(평형 예측·검증 가능)와의 지위 차이다.
