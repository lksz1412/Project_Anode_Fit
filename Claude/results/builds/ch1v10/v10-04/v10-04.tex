% ====================================================================
% graphite_ica_ch1_v10-04.tex — Chapter 1 (v10: v9 정정·업데이트 통합본; 경쟁 빌드 04)
%   v9 = 흑연 forward + LCO 전자 엔트로피 + ξ_eq 분포. v10 추가(불가침 v9 전부 보존):
%     (1) ★종모양 broadening 절 복원(평형 peak 절 부근) — 전이별 출발·현상학적 w·
%         범위 경고(다입자/PSD 모델 0·forward-only·반경=평형U 무효·apparent-U=U_j+η).
%     (2) w 이중지위 명시(단상 Ω<2RT=nRT/F 평형 예측 / 두-상 Ω>2RT=현상학적 자유 피팅 폭).
%     (3) w_eff "종 좁힘→델타" 자취 제거(two-phase 실측은 종이지 델타 아님; narrowing 반대).
%   설계 근거 = research/broadening_w_design.md·radius/{ORIGIN_VERDICT,DOCS_say_about_distribution}.md.
%   ── 이하 v9(= v8-11) 원형 보존 ──
...
\hypersetup{colorlinks=true, linkcolor=blue, urlcolor=blue, citecolor=blue,
  pdftitle={흑연 음극 + LCO 양극 dQ/dV 이론: 코드 진행을 따라가는 수식-구동 (유도 확장) — Chapter 1 (v10)}}
\pagestyle{fancy}\fancyhf{}
\lhead{흑연 음극 + LCO 양극 $dQ/dV$ 이론 — Ch.1 (v10, 코드 진행 + 유도)}\rhead{\thepage/\pageref{LastPage}}
...
$w_j$ (\code{func\_w}) & V & 전이 폭 척도 — 단상 $=n_jRT/F$(평형 예측), 두-상 $=$ 현상학적 피팅 폭(\S\ref{sec:width}) \\
...
상전이를 \emph{억제}한다 — 정규용액 틀에서 이는 $\Omega_j$ 를 $2RT$ 쪽으로 낮추고 현상학적 피팅 폭 $w_j$ 를 넓혀
(\S\ref{sec:width-dual} 의 두-상 지위) peak 를 smear 하고 $U_j$ 를 미세 shift 시키는 것으로 들어온다. 흑연 \code{GRAPHITE\_STAGING\_LIT} 가 초기값이고 폭이
...
\subsection{폭 $w_j$ — 이상 극한과 ★두 지위}\label{sec:width-dual}
...
\textbf{★폭의 두 지위 — 같은 기호 $w_j$ 가 전이 성격에 따라 의미가 갈린다.} 식~\eqref{eq:wbase} 의 $w_j$ 는 두 가지
\emph{서로 다른 지위}로 읽힌다(어느 쪽인지는 그 전이의 상호작용 $\Omega_j$ 가 $2RT$ 를 넘는지가 정한다 — 식~\eqref{eq:spinodal}
의 상분리 문턱과 같은 문턱이다).
\begin{itemize}[leftmargin=1.4em,itemsep=2pt]
\item \textbf{단상(고용체) 전이 $\Omega_j<2RT$ — $w_j=n_jRT/F$ 는 검증 가능한 평형 예측이다.} 이 경우 단일 입자 평형
등온선 자체가 폭 $\sim n_jRT/F$ 의 매끈한 종이며(\S\ref{sec:width} logistic 유도가 그 종이다), 폭이 온도 $T$ 에
비례한다는 것은 데이터로 직접 검증할 수 있는 평형 명제다. 흑연의 dilute$\to$stage4$\cdot$$4\mathrm L\!\leftrightarrow\!3\mathrm L$
연속(고용체) 구간이 여기에 든다.
\item \textbf{두-상(상분리) 전이 $\Omega_j>2RT$ — $w_j$ 는 평형 예측이 아니라 현상학적 자유 피팅 폭이다.} $\Omega_j>2RT$
면 평형 단일 입자는 2상 plateau(이상적으로 무한히 좁은 선)에 가까워(식~\eqref{eq:dUhys} 의 spinodal gap 을 낳는 바로 그
상분리), $w_j$ 를 \emph{평형} $n_jRT/F$ 로 강제할 근거가 사라진다. 실측 peak 의 유한 폭은 평형이 아니라
\emph{broadening}(\S\ref{sec:broadening})이 정하므로, 이 전이의 $w_j$ 는 ``평형이 예측하는 값''이 아니라 ``데이터에 맞춰
자유롭게 피팅하는 폭''이다 — 곧 \emph{현상학적 피팅 폭}이다. ★우리 흑연 staging 4 전이는 표~\ref{tab:staging} 의 $\Omega$
값($6000$--$13000$ J/mol)이 $T{=}298$ K 의 $2RT\approx4958$ J/mol 을 \emph{모두} 넘으므로, 넷 다 이 두-상 지위에 든다 —
폭은 전부 피팅 대상 파라미터(폭 폴백 $0.020$--$0.012$ V 는 그 \emph{초기값})다.
\end{itemize}
이 두 지위 구분이 v10 의 정정점이다 — 같은 식~\eqref{eq:wbase} 를 쓰되, $\Omega_j<2RT$ 면 그 값이 평형 예측이고
$\Omega_j>2RT$ 면 그 값이 자유 피팅 폭이라는 \emph{지위}만 다르다. 코드는 두 경우 모두 동일하게 $w_j=n_jRT/F$(또는 직접
\code{'w'})를 쓰며, 두-상에서 그 $n_j$(혹은 \code{'w'})가 피팅 자유도라는 점이 위 구분의 코드 측 표현이다(\code{\_width};
기본은 항상 $w_j=n_jRT/F$, 상호작용으로 폭을 \emph{좁히는} 별도 처방은 두지 않는다 — 그 까닭은 다음 문단).

\textbf{왜 ``상호작용이 폭을 좁힌다''는 처방을 두지 않는가.} 비단조 등온선~\eqref{eq:Veq} 의 중심 기울기가 $\Omega$ 와 함께
가팔라지므로($sF\,\dd V_\eq/\dd\xi|_{1/2}=4RT-2\Omega_j$), 형식적으로는 ``유효 폭이 $\Omega\!\to\!2RT$ 에서 $0$ 으로
좁아진다''고 적을 수 있다. 그러나 이는 \emph{관측과 방향이 반대}다 — 두-상 전이의 실측 peak 은 평형 델타에서 폭이
\emph{넓어진}(broadened) 유한 종이지, 더 \emph{좁아진}(narrowed) 델타가 아니다(\S\ref{sec:broadening}). 즉 ``상호작용이
종을 좁혀 델타로 보낸다''는 좁힘 그림은 두-상 실측의 폭 origin 을 거꾸로 세운 것이라, v10 은 이를 채택하지 않는다. 두-상
전이의 유한 폭은 \S\ref{sec:broadening} 의 broadening(동역학 $\eta$ 분산 $+$ 내재 $RT/F$ 폭)이 설명하고, 그 폭은 위 두
지위의 \emph{현상학적 피팅 폭} $w_j$ 로 흡수된다.
...
폭은 흑연과 같은 $w_j=n_jRT/F$ 이며($\Omega_j>2RT$ 인 LCO 세 전이는 모두 \S\ref{sec:width-dual} 의 두-상 지위라 $w_j$ 가
현상학적 피팅 폭이다), order--disorder 의 큰 $\Omega$ 가 T2$\cdot$T3 를 흑연보다 날카롭게 만든다(피팅 출발 폭이 좁다는 뜻). T1 의 위치는 \S\ref{sec:lco-electronic} 의 전자항이
...
% ====================================================================
\section{★종모양 broadening — 평형 델타가 실측 유한 종이 되는 까닭}\label{sec:broadening}
% ====================================================================
앞 절(\S\ref{sec:eqpeak})은 평형 peak 의 \emph{모양}을 닫았다 — 단상이면 폭 $\sim n_jRT/F$ 의 종(식~\eqref{eq:eqpeak}),
두-상이면 평형 단일 입자는 이상적으로 \emph{무한히 좁은 선}(델타)에 가깝다. 그런데 실측 $\dd Q/\dd V$ 의 두-상 전이
peak 는 델타가 아니라 \emph{유한 폭의 치우친 종}으로 보인다. 이 절은 그 간극 — ``평형은 델타인데 실측은 왜 종인가'' —
을 \emph{설명}으로만 닫는다(모델 기계장치를 더하지 않는다; 그 까닭은 \S\ref{sec:broaden-scope}). 이 절은 평형 peak(N6)과
다음의 동역학 꼬리(N7$\cdot$N8) 사이의 다리이며, broadening 의 동역학 몫이 바로 그 꼬리(\S\ref{sec:lag}$\cdot$\S\ref{sec:tail}
의 $L_V$)임을 미리 가리킨다.

\subsection{전이별 출발 — 애초에 종인 전이와 델타인 전이}\label{sec:broaden-bytrans}
``평형 델타가 왜 종인가''라는 물음은 \emph{모든} 전이에 해당하지 않는다 — 흑연 staging 은 전이 성격에 따라 갈린다
(\S\ref{sec:width-dual} 의 두 지위와 같은 갈림이다).
\begin{itemize}[leftmargin=1.4em,itemsep=2pt]
\item \textbf{단상(고용체) 전이 — 이미 종이라 broadening 이 불필요하다.} dilute$\to$stage4 와 $4\mathrm L\!\leftrightarrow\!3\mathrm L$
구간은 plateau 가 없는 연속(고용체) 전이라\cite{rsc2021}, 단일 입자 평형 등온선 $\dd x/\dd E$ 자체가 폭 $\sim n_jRT/F$
의 매끈한 종이다(\S\ref{sec:width} 의 logistic 미분; Frumkin/정규용액 등온선의 내재 폭\cite{levi1999}). 이 전이는 평형이
이미 종을 주므로, ``델타가 왜 종?''이라는 물음 자체가 성립하지 않는다.
\item \textbf{두-상(상분리) 전이 — 평형이 델타라 broadening 이 필요하다.} $2\mathrm L\!\to\!2$(LiC$_{12}$ 형성)과
$2\!\to\!1$(LiC$_6$)은 $\Omega_j>2RT$ 의 2상 plateau(강한 1차 전이)라, 평형 단일 입자는 델타에 가깝다(식~\eqref{eq:spinodal}
의 상분리 문턱을 넘는 바로 그 전이들). \textbf{곧 ``왜 델타가 아니고 종인가''는 이 두 전이에만 해당한다} — 이 절의 나머지는
이 두-상 전이의 broadening 을 푼다.
\end{itemize}

\subsection{두-상 델타가 유한 종이 되는 세 출처 — 동역학$\cdot$입자분산$\cdot$내재 폭}\label{sec:broaden-origin}
두-상 전이의 유한 폭을 만드는 것은 \emph{평형 $U_j$ 의 입자간 분포가 아니라} \emph{측정 조건}이다. 세 출처가 합성된다 —
앞 둘(i$\cdot$ii)은 동역학(과전압 $\eta$)이고, 셋째(iii)는 평형에 남는 내재 폭이다.
\begin{enumerate}[label=(\roman*),leftmargin=2.2em,itemsep=3pt]
\item \textbf{유한율속 비대칭 꼬리 [확정] — ``치우침(skew)''의 주범.} 입자 \emph{하나}라도 유한 전류면 표면 SOC 가 평형을
앞서거나 뒤처져 과전압 $\eta(\mathrm{SOC})$ 가 조성에 따라 변하며, peak 을 한쪽으로 밀고 한쪽 꼬리를 늘인다. 이것이 바로
다음 절들이 닫는 모델의 동역학 지연 $(\xi_\eq-\xi_\mathrm{lag})/L_V$(식~\eqref{eq:peakshape}, N7$\cdot$N8)이다 — 곧
broadening 의 비대칭 몫은 \emph{이미 모델 안에 있다}. C-rate 가 오르면 심화하고, 평형 극한($|I|\to0$, $L_V\to0$)에서
소멸한다(식~\eqref{eq:LV} 의 $L_V\propto|I|$; resistance correction 으로 저율 peak 위치가 회복된다는 관측\cite{fly2020}).
\item \textbf{입자 크기 kinetic 분산 [확정·강한 지지] — ``broadening(퍼짐)''의 주범.} 고정 셀 전류 하에서 확산 timescale
$\tau=r^2/D$ 가 반경 제곱으로 벌어져, 작은 입자는 먼저(낮은 $\eta$)$\cdot$큰 입자는 늦게(높은 $\eta$) 차오른다 — 같은 인가
전위에서 입자군의 국소 SOC 가 달라 거시 전이가 전압 구간에 퍼진다(흑연 operando 직접 입증\cite{yang2023}; $\tau\propto r^2$,
전류 증가 시 SOC 이질 뚜렷). 지배 무차원수 $\tau/t_\dis=(r^2/D)\cdot$C-rate 라 \emph{C-rate$\to0$ 에서 분산$\to0$}(저율
소멸)이고 PSD 폭이 배율이다. \textbf{★단, 이 입자 분산 몫은 \emph{원인으로 인정}하되 \emph{모델로 분리하지 않는다}} — 그
까닭(역산 ill-posed$\cdot$다입자 모델 무의미)은 \S\ref{sec:broaden-scope}(X2$\cdot$X3)이고, 본 모델에서는 이 몫이 위 (i)
단일입자 동역학 $\eta(\mathrm{SOC})$ 와 \emph{같은 apparent}-$U=U_j+\eta$ 채널을 통해 현상학적 폭 $w_j$ 에 흡수된다(별도
$g(r)$ 합성곱 없음).
\item \textbf{내재 $RT/F$ 폭 [확정] — floor.} 평탄역 양끝의 단상 꼬리와 staging 정렬의 내재 폭이 $\sim n_jRT/F$($\approx26$ mV
@$25^\circ$C) 규모의 하한을 깐다 — 이것이 $|I|\to0$ 으로 가도 남는 폭의 바닥이며, \S\ref{sec:width} logistic 의 폭 척도
$RT/F$ 와 같은 양이다.
\end{enumerate}

\textbf{★그러므로 두-상 폭 $w_j$ 는 평형 예측이 아니라 현상학적 자유 피팅 폭이다.} 위 두 출처가 정하는 폭은
``평형이 예측하는 $n_jRT/F$''가 아니므로, 두-상 전이의 $w_j$ 를 평형값으로 강제할 근거가 없다(\S\ref{sec:width-dual} 의
두-상 지위가 이것이다). ★결정적 재해석 — 관측 peak 전위는 \emph{apparent}-$U_j=U_j+\eta$ 이고, 입자간에 \emph{분포하는
것은 평형 $U_j$ 가 아니라 과전압 $\eta$}다. 평형 $U_j$ 는 GITT 준평형 측정에서 입자 무관한 thermodynamic 상수이고,
broadening 은 그 위에 얹힌 $\eta$(동역학)의 분산이다. 곧 ``peak 가 분포한다''는 관측은 \emph{평형 $U_j$ 의 분포가 아니라
$\eta$ 의 분포}이며, 그 분산이 현상학적 폭 $w_j$ 와 동역학 꼬리 $L_V$ 로 흡수된다.

\begin{figure}[t]
\centering
\begin{tikzpicture}[x=1.0cm,y=0.92cm]
% left: two-phase equilibrium (near-delta) ; right: measured finite skewed bell
\draw[-{Latex[length=1.6mm]}] (-3.5,0) -- (3.6,0) node[below,font=\scriptsize] {$V$};
\draw[-{Latex[length=1.6mm]}] (-3.5,0) -- (-3.5,3.2) node[above,font=\scriptsize] {$dQ/dV$};
% equilibrium near-delta spike at center (tall, very narrow)
\draw[thick] (0,0.0) -- (0,3.0);
\draw[thick] (-0.06,0.0) .. controls (-0.02,2.4) .. (0,3.0);
\draw[thick] (0,3.0) .. controls (0.02,2.4) .. (0.06,0.0);
\node[font=\scriptsize,align=center] at (0,3.25) {equilibrium\\(two-phase): near-$\delta$};
\draw[-{Latex[length=1.4mm]},gray] (0.3,2.4) .. controls (1.0,2.6) and (1.4,2.2) .. (1.7,1.7);
\node[font=\scriptsize,align=center] at (1.55,2.55) {broadening};
% measured finite skewed bell, shifted right (apparent-U = U_j + eta), with longer right tail
\draw[densely dashed,thick] plot[smooth] coordinates {(-2.2,0.05) (-1.4,0.22) (-0.8,0.6) (-0.3,1.25) (0.2,1.75) (0.6,1.9) (0.95,1.78) (1.4,1.42) (1.9,0.98) (2.5,0.55) (3.1,0.28) (3.5,0.16)};
\node[font=\scriptsize,align=center] at (1.7,1.15) {measured: finite\\skewed bell};
\draw[densely dotted,gray] (0,0) -- (0,3.0);
\draw[densely dotted,gray] (0.6,0) -- (0.6,1.9);
\draw[{Latex[length=1.2mm]}-{Latex[length=1.2mm]},gray] (0,-0.28) -- (0.6,-0.28) node[pos=0.5,below,font=\scriptsize] {$\eta$};
\node[below,font=\scriptsize] at (0,-0.02) {$U_j$};
\node[below,font=\scriptsize] at (0.6,1.9) {};
\end{tikzpicture}
\caption{두-상 전이의 평형 델타 vs 실측 종(신규). 솔리드 스파이크 $=$ 두-상 전이의 평형 단일 입자 응답(이상적으로
무한히 좁은 선, near-$\delta$, 위치 $=$ 평형 $U_j$). 파선 $=$ 실측 유한 폭의 치우친 종 — apparent-$U_j=U_j+\eta$ 로
이동하고($\eta=$ 과전압), 유한율속 비대칭 꼬리(\S\ref{sec:broaden-origin}(i)$\cdot$입자 크기 분산(ii), 모델 $L_V$$\cdot$현상학적
$w_j$)가 한쪽 꼬리를 늘이며, 내재 $RT/F$ 폭(iii)이 바닥을 깐다. 분포하는 것은 평형 $U_j$ 가 아니라 $\eta$ 이고, 그 폭은 현상학적 피팅 폭 $w_j$ 와
동역학 길이 $L_V$ 로 흡수된다 — 단상(고용체) 전이는 평형이 이미 종이라 이 그림이 필요 없다(\S\ref{sec:broaden-bytrans}).}
\label{fig:broadening}
\end{figure}

\subsection{★무엇을 하지 않는가 — 범위와 경고}\label{sec:broaden-scope}
broadening 을 \emph{설명}으로만 두는 것은 의도적이다 — 다음 세 가지를 \emph{하지 않는다}(이것이 v10 의 범위 선언이다).
\begin{enumerate}[label=\textbf{X\arabic*.},leftmargin=2.6em,itemsep=3pt]
\item \textbf{평형 $U_j$ 분포$\cdot$입자 반경 분포를 peak 에서 추출하지 않는다.} 반경이 평형 $U_j$ 를 바꾸는 채널(표면에너지
Gibbs--Thomson$\cdot$양자가둠 밴드 이동)은 마이크론 흑연에서 $\sim0.01$ mV 규모로 무시 가능하다(임계 크기 $\sim22$ nm
이하에서만 유의\cite{cogswell2012}; 우리 시료 5--20 $\mu$m 는 그보다 두세 자릿수 크다). 곧 평형 $U_j$ 는 GITT 상 \emph{입자
무관 상수}이고, peak 에서 ``반경 분포''나 ``평형 전위 분포''를 역산하는 것은 물리적 근거가 없다 — broadening 은 $\eta$(동역학)
이지 평형 $U_j$ 의 분산이 아니기 때문이다(\S\ref{sec:broaden-origin}).
\item \textbf{$\dd Q/\dd V\to$ 분포 역산은 ill-posed 라, forward-only 로만 둔다.} 측정 종에서 입자 분포(혹은 그와 얽힌
동역학 분산 분포)로 가는 역문제는 제1종 Fredholm 적분방정식이라 악조건(ill-posed)이다 — 같은 종을 무수히 많은 분포가
설명할 수 있어 유일하게 복원되지 않는다. 따라서 본 문건은 분포를 \emph{forward 로 모형해 넣고 예측을 데이터와 대조}하는
방향만 취하고, 역산은 하지 않는다(흑연 ``문헌 초기값 + 데이터 피팅'' 철학과 같은 forward-only 규율).
\item \textbf{★다입자/PSD convolution 모델을 만들지 않는다.} 입자 크기 분포(PSD) 위로 단일 입자 응답을 합성곱하는
다입자 모델 $\dd Q/\dd V=\int g(r)\,f(V;r)\,\dd r$ 은 구조적으로 가능하나, X1$\cdot$X2 의 결론(반경$\to$평형 $U$ 무효,
역산 ill-posed)에 비추어 \emph{추출되는 양이 의미를 갖지 못한다} — PSD$\cdot$확산계수$\cdot$형태학이 얽혀 단독 $\dd Q/\dd V$
로는 비유일하기 때문이다. 그러므로 v10 의 모델은 \emph{단일 유효입자 $+$ 동역학 지연 $L_V$}(N7$\cdot$N8) 구조를
유지하고, 입자 heterogeneity 는 별도 모델$\cdot$추출 없이 \emph{현상학적 폭 $w_j$ 에 흡수되는 것으로만 언급}한다. 곧
broadening 의 입자 분산 몫은 ``설명''으로 인정하되, 그것을 분리해 피팅하는 \emph{기계장치는 더하지 않는다}.
\end{enumerate}
요컨대 v10 은 두-상 종의 \emph{물리적 origin}(단일입자 유한율속 $\eta$ 꼬리 $+$ 입자 크기 kinetic 분산 $+$ 내재 $RT/F$ 폭)
을 올바로 \emph{서술}하되, 그 origin 을 분포로 역산하거나 다입자 모델로 확장하지 않는다 — 모델은 단일 유효입자에 머무르고,
종의 폭$\cdot$비대칭은 현상학적 $w_j$(\S\ref{sec:width-dual})와 동역학 $L_V$(\S\ref{sec:lag}$\cdot$\S\ref{sec:tail})가
흡수한다(세 origin 모두 apparent-$U=U_j+\eta$ 의 $\eta$ 분산이지 평형 $U_j$ 분산이 아니므로 같은 채널로 흡수된다). 다음
절이 그 동역학 몫 $L_V$ 를 세운다.
...
N4 & $w_j=n_jRT/F$ (두-상=피팅 폭) & \eqref{eq:wbase} & \code{func\_w},\code{\_width} \\
...
% --- broadening 절(v10 신규) 참고문헌 ---
\bibitem{levi1999} M. D. Levi and D. Aurbach, ``The mechanism of lithium intercalation in graphite film electrodes in aprotic media,'' \emph{Electrochim. Acta} \textbf{45} (1999). DOI: 10.1016/S0013-4686(99)00202-9. [단상(고용체) 등온선의 내재 $\sim nRT/F$ 폭, tier A.]
\bibitem{rsc2021} ``Graphite staging by operando methods: continuous (solid-solution) vs two-phase transitions,'' \emph{J. Mater. Chem. A} (RSC, 2021). DOI: 10.1039/D0TA12607A. [흑연 전이별 1차/연속 구분(``plateau \dots except for 4L--3L''), tier A — 저자명 미확인, DOI 로 식별.]
\bibitem{fly2020} A. Fly, M. Schaltz, D.-I. Stroe \emph{et al.}, ``C-rate dependence of incremental capacity peaks and resistance-corrected peak position,'' \emph{J. Energy Storage} \textbf{29}, 101329 (2020). DOI: 10.1016/j.est.2019.101329. [C-rate↑→peak 고전위 이동·둔화, 저항 보정으로 peak 위치 회복, tier B.]
\bibitem{yang2023} Yang \emph{et al.}, ``Operando observation of size-dependent lithiation kinetics in graphite particles,'' \emph{Nat. Commun.} (2023). DOI: 10.1038/s41467-023-40574-6. [고상확산 시간상수 $\tau\propto r^2$, 전류 증가 시 입자간 SOC 이질, tier A(본문 정량 검증); 권$\cdot$호 미확인, DOI 로 식별.]
\bibitem{cogswell2012} D. A. Cogswell and M. Z. Bazant, ``Coherency Strain and the Kinetics of Phase Separation in LiFePO$_4$ Nanoparticles,'' \emph{ACS Nano} \textbf{6}, 2215 (2012). DOI: 10.1021/nn204177u. [반경→평형 전위 채널은 임계 $\sim22$ nm 이하에서만 유의(마이크론 입자서 무시), tier A.]
