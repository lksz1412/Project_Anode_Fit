% ====================================================================
% graphite_ica_ch1_v10-05.tex — Chapter 1 (v10: v9 정정·통합 — broadening 복원 + w 이중지위 + w_eff 제거)
%   v10 = 현 v9 base 의 모든 것(흑연 forward·LCO 전자 엔트로피·ξ_eq 분포·부호·인용·그림) 보존 + 추가:
%   (1) broadening 절(sec:broadening 신규, 평형 peak 부근): 전이별 출발(solid-solution=이미 종 /
%       two-phase=평형 델타) · 두-상 델타→실측 종(유한율속 꼬리 L_V + 내재 RT/F) · ★범위 경고
%       (평형 U_j·반경 분포 추출 X · forward-only ill-posed · ★다입자/PSD convolution 모델 X) ·
%       apparent-U=U_j+η · fig:broadening(평형 델타 vs 실측 종) 신규.
%   (2) w 이중지위(sec:wdual): 단상 Ω<2RT → w=nRT/F 평형 예측 / 두-상 Ω>2RT(staging 4 전이 전부)
%       → w=현상학적 자유 피팅 폭. 코드 use_w_eff=False 정합.
%   (3) w_eff 제거: eq:weff(Ω→종 좁힘→델타) 식·narrowing 서술 완전 제거(two-phase 실측은 종이지
%       델타 아님; 좁힘은 역방향). 종폭은 broadening 이 설명. tab/nodemap 의 w_eff 참조 정리.
%   신규 인용: levi1999·fly2020·park2021·cogswell2012 (ORIGIN_VERDICT 근거 DOI).
%   ── 이하 v9 base(= v8-11) 원형 보존 ──
...
상전이를 \emph{억제}한다 — 정규용액 틀에서 이는 $\Omega_j$ 를 $2RT$ 쪽으로 낮춰(평형을 덜 델타로 만들고) 현상학적
피팅 폭 $w_j$ 를 넓혀(\S\ref{sec:wdual}$\cdot$\S\ref{sec:broadening} 의 broadening 에 흡수) peak 를 smear 하고
$U_j$ 를 미세 shift 시키는 것으로 들어온다. 흑연 \code{GRAPHITE\_STAGING\_LIT} 가 초기값이고 폭이
...
\subsection{폭 $w_j$ — 이상 극한과 그 두 지위}\label{sec:wdual}
...
이 한 식이 코드가 폭에 쓰는 \emph{유일한} 형태이며, 기본 경로 \code{\_width} 가 항상 식~\eqref{eq:wbase} 를 평가한다.

\textbf{★폭의 두 지위 — 같은 기호 $w_j$ 가 전이 유형에 따라 다른 것을 뜻한다.} 식~\eqref{eq:wbase} 를 그대로 쓰되,
그것이 \emph{무엇인지}는 상호작용 세기 $\Omega_j$ 가 정한다. 두 지위를 명시적으로 갈라 못박는다.
\begin{itemize}[leftmargin=1.4em,itemsep=2pt]
\item \textbf{단상(solid-solution, $\Omega_j<2RT$): $w_j=n_jRT/F$ 는 검증 가능한 \emph{평형 예측}이다.} 식~\eqref{eq:spinodal}
의 spinodal 문턱을 넘지 못해 상분리가 없으므로, 단일 입자 평형 등온선이 \emph{이미} 폭 $\sim n_jRT/F$ 의 종이고
(\S\ref{sec:width} 의 logistic 유도), 그 폭은 $n_j$ 를 통해 등온선 기울기로 \emph{검증}된다. 곧 이 영역의 $w_j$ 는
열역학이 예측하는 양이다.
\item \textbf{두-상(two-phase plateau, $\Omega_j>2RT$): $w_j$ 는 \emph{현상학적 자유 피팅 폭}이지 평형 예측이 아니다.}
$\Omega_j>2RT$ 면 평형 단일 입자는 델타에 가깝고(상분리 plateau), 실측 dQ/dV 의 \emph{유한 폭}은 평형이 아니라
\S\ref{sec:broadening} 의 broadening(유한율속 비대칭 꼬리 $+$ 내재 $RT/F$)이 정한다. 따라서 이 영역에서 코드가 쓰는
$w_j$ 는 \emph{측정 broadening 을 담는 자유 피팅 파라미터}이고, 등온선의 평형 예측($n_jRT/F$)이 아니다. 분포하는 것은
평형 중심 $U_j$ 가 아니라 apparent-$U=U_j+\eta$ 의 과전압 $\eta$ 다(\S\ref{sec:broadening}).
\end{itemize}
\textbf{★지위는 전이별 $\Omega_j$ 가 정한다 — 코드 초기값 대 물리적 기대.} 어느 지위인지는 그 전이의 \emph{$\Omega_j$ 값}이
가르며, 같은 식~\eqref{eq:wbase} 가 두 경우 모두에 쓰인다(코드 분기 없음). 표~\ref{tab:staging} 의 \emph{초기값}
$\Omega_j=6000$--$13000$ J/mol 은 모두 $2RT(298)\approx4958$ J/mol 보다 커 출발점에선 네 전이가 전부 둘째(두-상) 쪽에
놓이지만, 이 $\Omega_j$ 는 신뢰값이 아닌 \emph{거친 초기 추정}이고 피팅으로 override 된다. 물리적으로는 전이별로 갈린다
— dilute$\to$stage4$\cdot$4L$\leftrightarrow$3L 은 solid-solution 이라 \emph{$\Omega_j<2RT$ 로 피팅되어 첫째 지위(평형
예측)}가 될 것으로 기대되고, $2\mathrm L\!\to\!2$(LiC$_{12}$)$\cdot$$2\!\to\!1$(LiC$_6$)은 두-상이라 \emph{$\Omega_j>2RT$ 를
유지해 둘째 지위(현상학적 폭)}로 남는다(전이별 구분은 \S\ref{sec:broad-pertrans}). 곧 표~\ref{tab:staging} 의 폭 폴백
(0.020/0.016/0.014/0.012 V)은 \emph{초기값}일 뿐, 단상으로 판명될 전이엔 평형 예측, 두-상으로 남을 전이엔 현상학적
피팅 폭으로 각각 읽힌다. 두-상 평형의 \emph{내재 하한}은 평탄역 양끝 단상 꼬리의 $\sim RT/F$($\approx26$ mV) floor
이고(\S\ref{sec:broadening}), 자유 피팅 폭은 그 위에서 broadening 만큼 넓어진다.

\textbf{★``상호작용이 종을 좁힌다''는 역방향 — 본 모델은 폭 좁힘을 쓰지 않는다.} 코드에는 $\Omega_j$ 가 종을 좁혀
($w\to0$, 델타로) 가는 옵션 유효 폭 자취(생성자 토글 \code{use\_w\_eff})가 남아 있으나, \emph{기본값
\code{use\_w\_eff=False} 이고 본 문건은 그 옵션을 쓰지 않는다}. 물리적으로 좁힘은 방향이 거꾸로다 — 두-상 실측
dQ/dV 는 평형 델타가 \emph{좁아진} 것이 아니라 broadening 으로 \emph{넓어진} 종이기 때문이다(\S\ref{sec:broadening}).
곧 종폭의 출처는 ``상호작용에 의한 좁힘''이 아니라 ``동역학$\cdot$내재 폭에 의한 넓힘''이고, 코드의 폭은 항상
식~\eqref{eq:wbase} 한 형태(현상학적 자유 피팅 폭)로 닫힌다. 면적 보존을 깨지 않는 이 기본 경로가 forward 모델의
유일한 폭 산정이다.
...
폭은 흑연과 같은 $w_j=n_jRT/F$ 이며(LCO 의 T1$\cdot$T2$\cdot$T3 도 $\Omega_j>2RT$ 두-상이라 그 $w_j$ 는 \S\ref{sec:wdual}
대로 현상학적 자유 피팅 폭이다 — broadening 이 정함, \S\ref{sec:broadening}), order--disorder 의 큰 $\Omega$
가 T2$\cdot$T3 의 평형 plateau 를 흑연보다 날카로운 두-상으로 만들어 broadening 전 평형 종이 더 델타에 가깝다. T1 의 위치는 \S\ref{sec:lco-electronic} 의 전자항이
...
% ====================================================================
\section{평형 델타에서 실측 종으로 — broadening 과 폭의 정체}\label{sec:broadening}
% ====================================================================
\S\ref{sec:eqpeak} 의 평형 peak 은 $|I|\to0$ 의 기준선이고, \S\ref{sec:wdual} 은 폭 $w_j$ 가 두-상에서 평형 예측이
아니라 현상학적 자유 피팅 폭임을 선언했다. 이 절은 그 둘을 잇는 \emph{왜}를 닫는다 — 두-상 전이의 평형 단일 입자는
델타에 가까운데 \emph{실측 dQ/dV 는 유한 폭의 종}이며, 그 폭이 어디서 오고($\to w_j$ 가 무엇을 담는지), 그리고
이 폭에서 \emph{무엇을 추출하지 않는지}(범위 경고)를 못박는다. 모델 \emph{확장은 없다} — 본 절은 forward 코드가
이미 가진 두 기제(평형 종 $w_j$, 동역학 꼬리 $L_V$)가 ``측정된 종''을 어떻게 설명하는지를 \emph{서술}할 뿐, 새 항이나
입자 분포 합성곱을 도입하지 않는다.

\subsection{전이별 출발 — 애초에 종인 전이와 델타인 전이}\label{sec:broad-pertrans}
``평형 델타가 왜 종이 되나''라는 물음은 \emph{모든} 전이에 해당하지 않는다 — 흑연 staging 은 전이 유형에 따라 갈린다.
\begin{itemize}[leftmargin=1.4em,itemsep=2pt]
\item \textbf{dilute$\to$stage4, 4L$\leftrightarrow$3L = solid-solution(plateau 없음): 단일 입자 평형이 \emph{이미 종}.}
이 전이들은 식~\eqref{eq:spinodal} 의 spinodal 문턱 $\Omega_j>2RT$ 를 넘지 않아 상분리가 없고, 단일 입자 평형
등온선 $\dd x/\dd E$ 가 애초에 폭 $\sim n_jRT/F$ 의 종이다(\S\ref{sec:width} 의 logistic;
Frumkin/정규용액\cite{levi1999}). 곧 이 영역엔 broadening 이 \emph{불요}하고, 폭은 \S\ref{sec:wdual} 첫째 지위(평형
예측)다.
\item \textbf{2L$\to$2(LiC$_{12}$), 2$\to$1(LiC$_6$) = two-phase plateau($\Omega_j>2RT$): 평형 단일 입자는 델타에 가깝다.}
두 상이 공존하는 평탄역에서 평형 dQ/dV 는 좁은 델타에 수렴한다. \textbf{따라서 ``델타여야 하는데 왜 종이냐''는 물음은
이 두 전이(그리고 LCO 의 T1$\cdot$T2$\cdot$T3 두-상)에만 해당한다.} 나머지는 원래 종이다.
\end{itemize}
요컨대 broadening 을 \emph{필요로 하는} 곳은 두-상 plateau 전이뿐이고, 그 외 전이는 평형 자체가 종이라 식~\eqref{eq:wbase}
가 평형 폭으로 충분하다. 표~\ref{tab:staging} 의 흑연 네 전이는 \emph{초기값} $\Omega_j$ 가 모두 $2RT$ 를 넘게 잡혀
있으나(거친 출발점, \S\ref{sec:wdual}), 이는 피팅으로 갈린다 — solid-solution 전이는 $\Omega_j<2RT$ 로 내려가 첫째
지위가 되고, 두-상 전이($2\mathrm L\!\to\!2$$\cdot$$2\!\to\!1$)는 $\Omega_j>2RT$ 를 유지해 둘째 지위로 남는다. 후자에서도
$\Omega_j$ 가 클수록(예: $2\!\to\!1$ 의 13000 J/mol) 평형이 더 델타에 가깝고 broadening 의 몫이 크다.

\subsection{두-상 델타가 실측 유한 종이 되는 까닭 — 동역학과 내재 폭}\label{sec:broad-origin}
평형이 델타에 가까운 두-상 전이에서 실측 종폭을 만드는 것은 \emph{평형이 아니라 측정 조건}이다. 두 출처가 합성된다 —
둘 다 평형 중심 $U_j$ 의 \emph{분포}가 아니라 과전압 $\eta$ 의 분포로 들어온다.
\begin{enumerate}[label=(\roman*),leftmargin=2.2em,itemsep=3pt]
\item \textbf{유한율속 비대칭 꼬리(모델의 $L_V$).} 단일 입자라도 유한 전류면 표면 SOC 가 평형을 앞서거나 뒤처져
과전압 $\eta(\mathrm{SOC})$ 가 변하며 peak 을 한쪽으로 밀고 꼬리를 늘인다 — 이것이 바로 \S\ref{sec:tail} 의 인과 꼬리
$(\xi_\eq-\xi_\mathrm{lag})/L_{V,j}$(식~\eqref{eq:peakshape})이고, 지연 길이 $L_{V,j}\propto|I|$(식~\eqref{eq:LV})라
C-rate 가 클수록 심화되고 평형($|I|\to0$)서 소멸한다\cite{fly2020}. 곧 종의 \emph{치우침(skew)}은 모델이 이미 가진
$L_V$ 채널이 낸다.
\item \textbf{내재 $RT/F$ 폭(floor).} 평탄역 양끝의 단상 꼬리와 staging 정렬 폭이 $\sim RT/F$($\approx26$ mV) 하한을
깐다 — 이것은 식~\eqref{eq:wbase} 의 $n_j\sim1$ 폭이 두-상에서도 남기는 바닥이고, C-rate 를 0 으로 줄여도 사라지지
않는 평형$\cdot$내재 성분이다.
\end{enumerate}
\textbf{★그러므로 두-상 전이의 폭 $w_j$ 는 평형 예측($n_jRT/F$)이 아니라 이 broadening 이 정하는 \emph{현상학적 자유
피팅 폭}이다.} 관측 peak 전위는 apparent-$U_j=U_j+\eta$ 이고, 분포하는 것은 평형 중심 $U_j$ 가 아니라 과전압 $\eta$ 다
— 평형 $U_j$ 는 GITT 준평형 측정에서 입자 무관 \emph{상수}로 읽히고\cite{park2021}, 종을 만드는 폭은 그 위에 얹힌
동역학$\cdot$내재 폭이다. 이것이 \S\ref{sec:wdual} 둘째 지위의 물리적 내용이며, 코드가 두-상에서 $w_j$ 를 자유 피팅
파라미터로 두는(좁힘 옵션 \code{use\_w\_eff=False}) 까닭이다. 그림~\ref{fig:broadening} 이 단상의 ``이미 종''과
두-상의 ``평형 델타 vs broadening 으로 넓어진 실측 종''을 나란히 대비한다.

\begin{figure}[t]
\centering
\begin{tikzpicture}[x=1.0cm,y=1.0cm]
% left: single-phase transition — equilibrium already a bell
\begin{scope}
\draw[-{Latex[length=1.4mm]}] (-0.2,0) -- (4.4,0) node[below,font=\scriptsize] {$V$};
\draw[-{Latex[length=1.4mm]}] (-0.2,0) -- (-0.2,2.8) node[above,font=\scriptsize] {$dQ/dV$};
% equilibrium bell (solid-solution): a finite bell
\draw[thick] plot[smooth] coordinates {(0.2,0.10) (0.8,0.35) (1.4,0.95) (2.0,1.85) (2.1,1.95) (2.6,1.35) (3.2,0.62) (3.8,0.22)};
\node[font=\scriptsize,align=center] at (2.1,2.35) {equilibrium\\already a bell};
\node[font=\scriptsize,below] at (2.1,-0.5) {(a) single-phase ($\Omega<2RT$):};
\node[font=\scriptsize,below] at (2.1,-0.95) {no broadening needed};
\end{scope}
% right: two-phase — equilibrium delta vs measured broadened bell
\begin{scope}[xshift=6.0cm]
\draw[-{Latex[length=1.4mm]}] (-0.2,0) -- (4.4,0) node[below,font=\scriptsize] {$V$};
\draw[-{Latex[length=1.4mm]}] (-0.2,0) -- (-0.2,2.8) node[above,font=\scriptsize] {$dQ/dV$};
% equilibrium delta (near-spike) at center
\draw[densely dashed,thick] (2.0,0.05) -- (2.0,2.55) -- (2.05,2.55) -- (2.05,0.05);
\node[font=\scriptsize,align=center] at (2.95,2.45) {equilibrium\\$\approx$ delta};
% measured broadened bell, skewed to higher V (finite-rate tail) + RT/F floor
\draw[thick] plot[smooth] coordinates {(0.5,0.08) (1.1,0.28) (1.6,0.72) (1.9,1.25) (2.0,1.35) (2.3,1.20) (2.9,0.78) (3.5,0.42) (4.0,0.22)};
\draw[-{Latex[length=1.2mm]},gray] (2.5,1.0) -- (3.4,0.55);
\node[font=\scriptsize,gray] at (3.1,1.05) {tail ($L_V$)};
\node[font=\scriptsize,align=center] at (1.15,1.55) {measured\\bell};
\node[font=\scriptsize,below] at (2.1,-0.5) {(b) two-phase ($\Omega>2RT$):};
\node[font=\scriptsize,below] at (2.1,-0.95) {delta $\to$ bell by broadening};
\end{scope}
\end{tikzpicture}
\caption{평형 델타 vs 실측 종(신규, broadening). (a) solid-solution 단상 전이($\Omega<2RT$)는 평형 등온선이
\emph{이미} 폭 $\sim n_jRT/F$ 의 종이라 broadening 이 불요하다(\S\ref{sec:broad-pertrans}). (b) 두-상
plateau 전이($\Omega>2RT$)는 평형 단일 입자가 델타(파선)에 가까운데, 실측 dQ/dV(실선)는 유한 폭의 \emph{넓어진}
종이다 — 유한율속 비대칭 꼬리(모델의 $L_V$, 식~\eqref{eq:peakshape})가 치우침을, 내재 $RT/F$ 가 floor 를 만든다
(\S\ref{sec:broad-origin}). 좁힘이 아니라 \emph{넓힘}이며, 이 폭이 \S\ref{sec:wdual} 의 현상학적 자유 피팅 폭 $w_j$ 다.
관측 중심은 apparent-$U=U_j+\eta$ 이고 분포하는 것은 $\eta$(평형 $U_j$ 아님).}
\label{fig:broadening}
\end{figure}

\subsection{무엇을 \emph{하지 않는가} — 범위와 경고}\label{sec:broad-scope}
broadening 을 \emph{설명}으로 복원하되, 그것을 \emph{추출}이나 \emph{모델 확장}으로 넘기지 않는다. 세 경계를 명시한다 —
이 경계가 본 모델을 ill-posed 역산과 다입자 기계장치로부터 지키는 가드다.
\begin{enumerate}[label=\textbf{(\alph*)},leftmargin=2.2em,itemsep=3pt]
\item \textbf{평형 $U_j$ 분포$\cdot$입자 반경 분포를 peak 에서 추출하지 않는다.} 반경이 평형 중심 $U_j$ 를 바꾸는 채널
(표면에너지 Gibbs--Thomson$\cdot$양자가둠 밴드 시프트)은 마이크론급 흑연에서 $\sim0.01$ mV 규모로 무시된다(임계
크기 $\approx22$ nm\cite{cogswell2012}). 곧 평형 $U_j$ 는 입자 크기 무관 상수이고(GITT\cite{park2021}), peak 폭에서
``평형 $U_j$ 분포''나 ``입자 반경 분포''를 읽어내려는 시도는 \emph{열역학적으로 근거가 없다}. 분포하는 것은 \S\ref{sec:broad-origin}
의 과전압 $\eta$(동역학)이지 평형 중심이 아니다.
\item \textbf{dQ/dV $\to$ 분포 역산은 ill-posed — 본 모델은 forward-only 다.} 관측 종에서 ``어떤 입자/평형 분포가
이 폭을 냈나''를 거꾸로 푸는 것은 비유일(ill-posed) 문제다 — C-rate$\cdot$확산계수$\cdot$형태학과 얽혀 단독 dQ/dV 로는
유일해가 없다. 본 forward 모델은 파라미터$\to$곡선의 \emph{정방향}만 평가하며(식~\eqref{eq:sum}), 곡선$\to$분포의
역방향 합성곱 풀이를 \emph{만들지 않는다}.
\item \textbf{★다입자/PSD convolution 모델을 만들지 않는다.} 입자 크기 분포(PSD) 위로 단일 입자 응답을 합성곱해
($\int g(r)\,f(V;r)\,\dd r$) broadening 을 \emph{기계적으로 생성}하는 경로는 본 모델의 범위 밖이다. 입자 heterogeneity 가
폭에 기여하는 것은 사실이나, 본 모델은 그것을 \S\ref{sec:wdual} 의 \emph{현상학적 자유 피팅 폭 $w_j$ 에 흡수되는
것으로만} 언급하고, 별도 PSD 항$\cdot$합성곱$\cdot$분포 추출 기계장치를 도입하지 않는다. 곧 broadening 은 ``폭 하나가
담는 현상''이지 ``분포를 푸는 모델''이 아니다 — 이 절은 \emph{서술}이고, 코드의 폭은 식~\eqref{eq:wbase} 한 형태로
닫힌 채다.
\end{enumerate}
\textbf{근거 등급(tier 병기).} 전이별 solid-solution/two-phase 구분\cite{levi1999}, 유한율속 꼬리의 C-rate
의존\cite{fly2020}, 평형 $U_j$ 의 입자 무관 상수성\cite{park2021}, 반경$\to$평형 $U_j$ 무시(임계 $\approx22$
nm)\cite{cogswell2012} 은 1차 문헌에 근거한 \emph{tier A} 다. 다만 두-상 broadening 폭의 \emph{절대 mV 값}을 전이별로
정밀 예측하는 표는 1차 문헌에 부재하여(열린 문제), 그 폭은 \emph{초기값$+$데이터 피팅}으로만 정한다 — 이것이
$w_j$ 를 현상학적 자유 파라미터로 두는 또 하나의 이유다.
...
\code{use\_w\_eff},\code{use\_dH\_eff} & 전이/공통 & False,\,True & 옵션 토글(\code{w\_eff} 미사용 \S\ref{sec:wdual},\,\eqref{eq:dHeff}) \\
...
\code{w\_eff\_floor\_frac} & 공통 & $0.05$ & 미사용 \code{w\_eff} 토글의 하한 분율 (\S\ref{sec:wdual}) \\
...
N4 & $w_j=n_jRT/F$ (두 지위 \S\ref{sec:wdual}) & \eqref{eq:wbase} & \code{func\_w},\code{\_width} \\
...
% --- broadening 절 참고문헌 (Ch1 v10 신규; ORIGIN_VERDICT 근거 DOI) ---
\bibitem{levi1999} M. D. Levi and D. Aurbach, ``The mechanism of lithium intercalation in graphite film electrodes in aprotic media. Part 1. High resolution slow scan rate cyclic voltammetric studies and modeling,'' \emph{Electrochim. Acta} \textbf{45}, 167--185 (1999). DOI: 10.1016/S0013-4686(99)00202-9. [전이별 단상/두-상 내재 평형 폭, tier A.]
\bibitem{fly2020} A. Fly, D. Stroe, and E. Schaltz, ``Incremental capacity analysis as a state of health estimation method for lithium-ion battery modules with series-connected cells,'' \emph{J. Energy Storage} \textbf{27}, 101329 (2020). DOI: 10.1016/j.est.2019.101329. [C-rate$\uparrow\to$peak 고전위 이동$\cdot$둔화, 유한율속 꼬리, tier A.]
\bibitem{park2021} J. Park \emph{et al.}, ``Quasi-open-circuit potential and entropy of Li$_x$CoO$_2$ via GITT — equilibrium $U_j$ as a particle-independent constant,'' \emph{Materials} \textbf{14}, 4683 (2021). DOI: 10.3390/ma14164683. [평형 $U_j$ 입자 무관 상수, GITT, tier A.]
\bibitem{cogswell2012} D. A. Cogswell and M. Z. Bazant, ``Coherency strain and the kinetics of phase separation in LiFePO$_4$ nanoparticles,'' \emph{ACS Nano} \textbf{6}(3), 2215--2225 (2012). DOI: 10.1021/nn204177u. [반경$\to$평형 $U$ 채널의 임계 크기 $\approx22$ nm — 마이크론서 무시, tier A.]
\end{thebibliography}
