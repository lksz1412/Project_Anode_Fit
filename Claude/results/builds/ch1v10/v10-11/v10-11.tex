...
$@298$ K$)$ 보다 커 출발점에선 네 전이가 전부 둘째(두-상) 쪽에 놓이지만, 이 $\Omega_j$ 는 신뢰값이 아니라 \emph{거친 초기
추정}이고 피팅으로 override 된다 — 물리적으로는 전이별로 갈린다. dilute$\to$stage4$\cdot$4L$\leftrightarrow$3L 은
...
전이는 plateau 가 없는 연속 등온선이라\cite{rsc2021}, 단일 입자 평형 dQ/dV 가 \S\ref{sec:width} 의 logistic 미분
...
$\dd q/\dd V\!\to\!\infty$). 곧 ``평형은 델타여야 하는데 실측은 종''이라는 물음은 \emph{이 두 2상 전이에만} 해당하고,
...
$RT/F\!\approx\!26$ mV 는 $n_j\!=\!1$ 의 척도일 뿐 하한이 아니다).
