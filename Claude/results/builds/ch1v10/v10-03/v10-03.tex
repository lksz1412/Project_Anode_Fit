...
$w_j$ (\code{func\_w}) & V & 전이 폭 척도 $=n_jRT/F$ (단상=평형 예측; 두-상=현상학적 피팅 폭, \S\ref{sec:broadening-wj}) \\
...
상전이를 \emph{억제}한다 — 정규용액 틀에서 이는 $\Omega_j$ 를 $2RT$ 쪽으로 낮춰
...
\textbf{$w_j$ 의 이중지위(★).} 폭 $w_j$ 는 전이 유형에 따라 물리적 지위가 다르다.
\begin{itemize}[leftmargin=1.4em,itemsep=2pt]
\item \textbf{단상(solid-solution, $\Omega_j<2RT$):} $w_j=n_jRT/F$ 는 \emph{검증 가능한 평형 예측}이다 — 격자기체
등온선의 중심 기울기가 $RT/F$ 이라는 것은 유도된 결과(식~\eqref{eq:wbase} 의 detailed balance 유도)이며, 실측 폭이 이에
들어맞는지 독립 데이터로 검증할 수 있다. 여기서 $n_j$ 는 다중도이고 폭 자체는 평형 열역학이 예측한다.
\item \textbf{두-상(강한 1차 전이, $\Omega_j>2RT$; 흑연 staging 2L$\!\to\!$2, 2$\!\to\!$1 포함):} 평형 단일 입자는
델타에 가까우나 실측 peak 는 유한 폭의 종이다. 이 종의 폭은 평형이 예측하는 값이 아니라 \emph{측정 조건이 만드는
broadening}이 정한다(\S\ref{sec:broadening}). 따라서 두-상 전이에서 $w_j$ 는 \emph{현상학적 자유 피팅 폭}이며,
$n_jRT/F$ 공식은 값의 출발점(초기값)이지 예측이 아니다. apparent 중심 $=U_j+\eta$ 이고 분포하는 것은 과전압 $\eta$ 다.
\end{itemize}
코드는 $w_j=n_jRT/F$(\code{func\_w})를 기본으로 쓰며, \code{'w'} 를 직접 주면 $n_j=w_jF/(RT)$ 로 역산한다(\code{\_n\_factor}).
이 이중지위로 인해 단상 전이는 폭으로 평형 열역학을 검증하고, 두-상 전이는 폭을 피팅 대상으로 다룬다.
...
폭은 흑연과 같은 $w_j=n_jRT/F$ 이며($\Omega_j>2RT$ 인 두-상 전이에서 $w_j$ 는 현상학적 피팅 폭, \S\ref{sec:broadening-wj}), order--disorder 의 큰 $\Omega$
...
% ====================================================================
\section{실측 peak 의 broadening — 두-상 전이에서 델타가 종이 되는 까닭}\label{sec:broadening}
% ====================================================================
\S\ref{sec:eqpeak} 의 평형 peak 는 전이 유형에 따라 처음부터 지위가 다르다. 이 절은 그 구분을 전이별로 닫고,
두-상 전이의 폭 $w_j$ 가 왜 평형 예측이 아니라 현상학적 자유 피팅 폭인지를 설명한다 — 모델을 확장하거나 새 수식을
추가하지 않는다. 물리적 이해를 텍스트로만 복원한다.

\subsection{전이별 출발점 — 애초에 델타인 전이만 문제}\label{sec:broadening-types}
흑연의 네 staging 전이는 두 범주로 나뉜다.

\textbf{(A) 단상(solid-solution) 전이: dilute$\!\to\!$stage~4, 4L$\!\leftrightarrow\!$3L.}
이 전이들은 plateau 가 없고 연속적인 조성 변화를 동반한다($\Omega_j<2RT$, solid-solution
\cite{dahn1991}). 단일 입자의 평형 $\dd Q/\dd V$ 는 이미 폭 $\sim n_jRT/F$ 의 종이다
(Frumkin 정규용액 등온선; \cite{levi1999}). 입자 분포나 추가 broadening 없이도 유한 폭을 가진다.
\textbf{``왜 종인가?''라는 질문은 이 전이들에는 해당하지 않는다.}

\textbf{(B) 두-상(강한 1차) 전이: 2L$\!\to\!$2(LiC$_{12}$), 2$\!\to\!$1(LiC$_6$).}
이 전이들은 $\Omega_j>2RT$ 라 평탄역이 존재하고, \emph{단일 입자의 평형} $\dd Q/\dd V$ 는
델타 함수에 가깝다 — 식~\eqref{eq:Veq} 의 비단조 등온선이 뚜렷한 2상 공존역을 만들기 때문이다.
\textbf{실측 peak 가 유한 폭의 종으로 나오는 이유는 이 두 전이에서만 별도 설명이 필요하다.}

\subsection{두-상 전이의 델타가 유한 종이 되는 까닭}\label{sec:broadening-mech}
두 기작이 합성된다. \textbf{둘 다 평형 $U_j$ 의 분포가 아니다.}

\textbf{(B-i) 유한율속 비대칭 꼬리(모델 $L_V$).} 단일 입자라도 유한 전류에서는 표면 SOC 가 평형을
앞서거나 뒤처져 과전압 $\eta(\mathrm{SOC})$ 가 변하며, peak 를 한쪽으로 밀고 꼬리를 늘인다.
이것이 N7--N8 의 $(\xi_\eq-\xi_\mathrm{lag})/L_V$ 항이다 — C-rate 가 오를수록 심화되고,
$|I|\!\to\!0$ 에서 소멸한다 \cite{fly2020}.

\textbf{(B-ii) 내재 $RT/F$ 폭.} 평탄역 양끝 단상 꼬리와 staging 정렬의 유한 폭이 $\sim RT/F$
하한을 만든다.

이 두 기작의 합성이 유한 종모양을 낳는다. 즉 apparent 전위 $V_\mathrm{peak}=U_j+\eta$ 에서
\emph{분포하는 것은 $\eta$(과전압)}이고, 평형 $U_j$ 는 GITT 상 입자 무관 열역학 상수다
\cite{park2021}.

\subsection{이 모델이 \emph{하지 않는} 것 — ★범위 경고}\label{sec:broadening-scope}
\begin{enumerate}[label=(\roman*),leftmargin=2.2em,itemsep=2pt]
\item \textbf{평형 $U_j$ 분포·입자 반경 분포를 peak 에서 추출하지 않는다.}
  반경$\to$평형 $U_j$ 채널(Gibbs-Thomson 표면에너지·양자가둠 밴드)은 마이크론 흑연에서
  $\sim$0.01 mV 규모로 무시된다\cite{cogswell2012}. 평형 $U_j$ 는 GITT 측정에서 입자 크기 무관 상수다.
\item \textbf{$\dd Q/\dd V \to$ 분포 역산을 시도하지 않는다(forward-only, ill-posed).}
  입자 heterogeneity 는 현상학적 $w_j$ 에 흡수되는 것으로만 언급한다
  — 다입자 PSD convolution 모델은 만들지 않는다.
  다입자/PSD 합성곱 모델을 구성하거나 이를 통한 분포 추출은 이 모델의 범위 밖이다
  (역문제는 ill-posed 이며, forward-only 로만 사용한다).
\end{enumerate}

\subsection{$w_j$(두-상) = 현상학적 자유 피팅 폭}\label{sec:broadening-wj}
위 기작들이 두-상 전이의 폭을 정한다. 따라서 두-상 전이의 $w_j$ 는 평형 등온선으로 예측하는
값이 아니라 \emph{현상학적 피팅 폭}이다. $w_j$ 의 이중지위(\S\ref{sec:width})에서 단상은
검증 가능한 평형 예측이고, 두-상은 피팅 대상인 까닭이 여기에 있다. apparent 중심은
$U_j+\eta$ 이고 broadening 은 $\eta$ 분포의 결과다.

\begin{keybox}
\textbf{broadening 요약.} dilute·4L--3L = 단상이라 \emph{이미 종}(broadening 불요).
2L$\!\to\!$2, 2$\!\to\!$1 = 두-상이라 평형에서 델타 $\to$ 유한율속 꼬리$(L_V)$ + 내재 $RT/F$ 폭의 합성으로 종.
$w_j$(두-상) = 현상학적 피팅 폭(평형 예측 아님). 다입자/PSD 모델 없음, forward-only.
\end{keybox}
...
\code{use\_dH\_eff} & 전이/공통 & True & 옵션 토글(\eqref{eq:dHeff}) \\
...
\code{w\_eff\_floor\_frac} & 공통 & $0.05$ & (레거시 파라미터, 사용 안 됨) \\
...
N4 & $w_j=n_jRT/F$ (이중지위: §\ref{sec:broadening-wj}) & \eqref{eq:wbase} & \code{func\_w},\code{\_width} \\
...
\bibitem{levi1999} M. D. Levi and D. Aurbach, ``Frumkin intercalation isotherm---a tool for the description of lithium insertion into host materials: a review,'' \emph{Electrochim. Acta} \textbf{45}, 167--185 (1999). DOI: 10.1016/S0013-4686(99)00202-9. [tier A: 단일입자 Frumkin 등온선 폭 $\sim RT/F$, 흑연 solid-solution 전이 이미 종]
\bibitem{fly2020} A. Fly, E. Schaltz, and D.-I. Stroe, ``Comparison of half-cell measurement to full-cell data for lithium-ion batteries,'' \emph{J. Energy Storage} \textbf{28}, 101329 (2020). DOI: 10.1016/j.est.2019.101329. [tier A: C-rate$\uparrow\to$peak 고전위 이동$\cdot$둔화, resistance correction 으로 C/6 peak 위치를 C/48 대비 0.59\% 내 예측 $\to$ kinetic 성분 확인]
\bibitem{park2021} S. Park, T. Kim, and S. M. Oh, ``Realistic Description of the Li$_x$C$_6$ Phase Diagram Considering the Stacking Sequences of Lithiated Graphite,'' \emph{Materials} \textbf{14}, 4683 (2021). DOI: 10.3390/ma14164683. [tier A: GITT 평형 $U_j$ 입자 무관 상수 0.22/0.12/0.08 V 실측]
\bibitem{cogswell2012} D. A. Cogswell and M. Z. Bazant, ``Coherency Strain and the Kinetics of Phase Separation in LiFePO$_4$ Nanoparticles,'' \emph{ACS Nano} \textbf{6}, 2215--2225 (2012). DOI: 10.1021/nn204177u. [tier A: 임계반경 22 nm 이하에서만 반경$\to U_j$ 채널 유효; 마이크론 흑연서 $\sim$0.01 mV 무시]
