...
%   v10-06 (v10 = v9 정정·통합): broadening 절(sec:broadening, fig:broadening) 복원 + w 이중지위(sec:width 보강)
%     + w_eff 제거(eq:weff·func_w_eff·use_w_eff·w_eff_floor_frac 자취 0; two-phase 실측은 종이지 델타 아님 — narrowing 반대방향).
%     범위경고: 평형 U_j·반경 분포 역산 안 함(forward-only·ill-posed·반경=평형U 무효)·다입자/PSD convolution 모델 0.
%     추가 인용 5건(leviaurbach1999·rsc2021·fly2020·cogswell2012·park2021). v9 흑연·LCO 전자엔트로피·ξ_eq 분포·부호·그림 전부 보존.
...
본 문건의 흑연 staging 네 전이는 표~\ref{tab:staging} 의 $\Omega_j$ 가 모두 $2RT(\!\approx\!4958$ J/mol$@298$ K$)$
보다 커 \emph{전부 두-상}이라, 그 폭은 평형값을 그대로 믿을 양이 아니라 데이터로 피팅하는 자유 파라미터다 — 표의 폭 폴백
값(0.020/0.016/0.014/0.012 V)이 \emph{신뢰값이 아니라 초기값}인 까닭이 이 이중지위다. 곧 단상의 $w$ 는 평형이 정하고,
두-상의 $w$ 는 피팅이 정한다.
...
\textbf{(b) 둘째 부류의 델타가 종이 되는 까닭 — broadening 의 출처.} 2상 전이에서 폭을 만드는 것은 (Maxwell
공통접선이 그리는) plateau \emph{내부}의 평형이 아니라, \emph{측정 조건}과 plateau \emph{양끝}이다. 성격이 다른 두
출처를 구분해 합성한다. 첫째는 이 문건이 이미 모델로 담은 \emph{유한율속 비대칭 꼬리}로, \emph{전류가 켜야} 생기는
동역학 몫이다 — 유한 전류면 표면 진행률이 평형 목표를 뒤따라(또는 앞서) 과전압 $\eta$ 가 진행 중에 변하며 peak 을
한쪽으로 밀고 꼬리를 늘인다. 이것이 바로 \S\ref{sec:lag}--\S\ref{sec:tail} 의 동역학 지연 길이 $L_{V,j}$ 와 인과 꼬리
$(\xi_\eq-\xi_\mathrm{lag})/L_{V,j}$(식~\eqref{eq:peakshape})이며, C-rate 가 오를수록 심해지고 $|I|\!\to\!0$ 에서
\emph{소멸}한다(식~\eqref{eq:LV} 의 $L_V\propto|I|$; Fly 등\cite{fly2020}). 둘째는 평탄역 양끝의 단상(solid-solution)
꼬리가 까는 \emph{내재 폭}으로, plateau 양끝이 첫째 부류와 같은 연속 등온선이라 그 경계가 무한히 날카롭지 않고 폭
$\sim RT/F$ 규모(수십 mV order)로 번진 데서 온다 — 이 몫은 전류와 무관한 \emph{평형 자체의 잔여 폭}이라 $|I|\!\to\!0$
에서도 남는다(다만 plateau 끝 유효 다중도 $n_j$ 가 $1$ 미만이면 그 절댓값은 $RT/F$ 보다 작을 수 있어, 표~\ref{tab:staging}
의 폭 폴백 0.012--0.020 V 와 모순되지 않는다 — $RT/F\!\approx\!26$ mV 는 $n_j\!=\!1$ 의 척도일 뿐 하한이 아니다).
이 두 출처가 평형의 (plateau 내부) 델타를 유한 폭의 치우친 종으로 펼친다 — 그리고 관측되는 peak 전위는 평형 중심이 아니라
$V_\mathrm{peak}=U_j+\eta$(평형 중심에 과전압이 얹힌 값)이므로, peak 이 \emph{분포}하는 것처럼 보인다면 그 분포의 정체는
평형 $U_j$ 의 분포가 아니라 \emph{과전압 $\eta$ 의 분포}다. 그러므로 못박는다 — \emph{2상 전이의 폭 $w_j$ 는
식~\eqref{eq:wbase} 의 평형 예측($n_jRT/F$)을 그대로 믿을 양이 아니라, 위 broadening(전류 몫 $L_V$ $+$ 평형 잔여
가장자리 폭)이 정하는 현상학적 자유 피팅 폭이다.} 이것이 앞 \S\ref{sec:width} 에서 선언한 폭 이중지위의 \emph{두-상}
측을 떠받치는 물리이며, 거기서 ``두-상 $w$ 는 피팅이 정한다''고 적은 까닭이 바로 이 broadening 이다.

\textbf{(c) 이 절이 \emph{하지 않는} 것 — 범위와 경고.} broadening 을 ``설명''하는 것과 그것을 ``모델로 확장''하는 것은
전혀 다르며, 본 문건은 전자에 그친다. 세 가지를 분명히 금한다.
\begin{enumerate}[label=(\roman*),leftmargin=2.2em,itemsep=2.5pt]
\item \textbf{평형 $U_j$$\cdot$입자 반경 분포를 peak 에서 추출하지 않는다.} 반경이 평형 전이 전위 $U_j$ 를 옮기는
\emph{열역학} 채널 — 표면에너지 Gibbs--Thomson($\Delta V=-2\gamma V_m/zFr$)과 양자가둠 밴드 shift — 은 둘 다 $1/r$(또는
더 빠르게) 작아져, 전형 마이크론 흑연($r\!\approx\!2.5$--$10\,\mu$m)에서 $\sim0.01$--$0.05$ mV 규모에 그친다
(실측 peak 폭 수십 mV 의 약 1/1000; $\gamma\!\approx\!0.5$--$1$ J/m$^2$ 로 계산, 2배 키워도 결론 불변). 큰 크기 효과는
나노 영역에서만 살아난다 — $\mathrm{LiFePO_4}$ 임계 반경 $\approx22$ nm\cite{cogswell2012} 가 그 척도이고, 마이크론
흑연은 적용 영역 밖이다. 흑연 staging 전위의 \emph{평형} 입경 의존을 보고한 1차 문헌은 없으며, GITT 준평형 OCP 는
staging 전위를 \emph{입자 무관 상수}로 측정한다\cite{park2021}. 곧 마이크론 흑연에서 반경$\to U_j$ 사상은 사실상
평탄하여, 그 분포를 peak 에서 읽어낼 신호 자체가 없다.
\item \textbf{dQ/dV$\to$분포 역산은 ill-posed 이므로 하지 않는다(forward-only).} ``peak $=$ 단일 입자 커널 $\circledast$
분포'' 는 1종 Fredholm 적분방정식(distribution of relaxation times 와 동형)이라, 정방향(분포$\to$peak)만 잘 정의되고
역방향은 정규화$\cdot$형태가정 없이는 비유일$\cdot$불안정하다 — 서로 다른 분포가 거의 같은 곡선을 준다. 따라서 입자
heterogeneity(크기$\cdot$결정성$\cdot$국소 환경)는 \emph{현상학적 폭 $w_j$ 에 흡수되는 것으로만} 언급하고, 그로부터
분포를 \emph{역산하지 않는다}. heterogeneity 를 모델에 넣으려면 forward 로 가정하고 예측을 데이터와 대조해야 하며,
그 입자-크기$\to$폭 맵은 본 장의 소관이 아니다(후속).
\item \textbf{다입자$\cdot$입자크기분포(PSD) convolution 모델을 만들지 않는다.} (i)(ii) 의 결론 — 마이크론에서 반경
사상이 평탄하고 역문제가 ill-posed 이며 실측 치우침이 동역학 분극에 지배된다 — 의 합성으로, 단일 dQ/dV 곡선에서 반경
분포를 산출하는 시도는 현 조건(마이크론 흑연$\cdot$코인 하프셀$\cdot$통상 율속)에서 성립하지 않는다. 본 문건의
forward 모델은 \emph{단일 유효 입자 $+$ 동역학 지연 $L_V$}(N7$\cdot$N8) 구조를 유지하며, 입자 크기 분포 위로 합성곱하는
다입자 기계장치를 도입하지 않는다. broadening 은 위 (b) 의 \emph{설명}으로 복원되었을 뿐, 모델 차원은 늘지 않는다.
\end{enumerate}

\begin{keybox}
\textbf{broadening 한 줄.} 연속 전이(dilute$\cdot$4L--3L)는 평형 단일 입자가 \emph{이미} 폭 $n_jRT/F$ 의 종이고,
2상 전이($\mathrm{LiC_{12}}\cdot\mathrm{LiC_6}$)만 평형(plateau 내부) 델타가 유한 종이 된다 — 그 폭은 \emph{전류가
켜는} 유한율속 비대칭 꼬리($L_V$, $|I|\!\to\!0$ 소멸)$+$plateau 양끝의 \emph{평형 잔여} 내재 폭($\sim RT/F$, 전류
무관)이 정하는 \emph{현상학적 자유 피팅 폭}이고(apparent-U $=U_j+\eta$, 분포하는 건 $\eta$),
\emph{평형 $U_j\cdot$반경 분포를 역산하거나 다입자/PSD 모델로 확장하지 않는다}(역문제 ill-posed$\cdot$forward-only).
\end{keybox}
...
선(파선, 단 양끝 단상 꼬리 탓에 폭이 정확히 0 은 아님)이나, 실측은 \emph{전류가 켜는} 유한율속 비대칭
꼬리($\eta$, \S\ref{sec:tail} 의 $L_V$, $|I|\!\to\!0$ 소멸)와 \emph{전류 무관}한 가장자리 내재 폭($\sim RT/F$)이
펼친 치우친 종(실선)이다. 후자의 폭 $w_j$ 는 평형 예측이 아니라 broadening 이 정하는 현상학적 자유 피팅 폭이며,
이 종에서 평형 $U_j\cdot$반경 분포를 역산하지 않는다(\S\ref{sec:broadening}(c)).}
...
% --- broadening 절 참고문헌 (Ch1 v10 추가; radius 조사 verdict DOI grounded) ---
...
\bibitem{fly2020} A. Fly, R. Schaltz, and D.-I. Stroe, ``Temperature and current-rate dependence of incremental capacity peaks,'' \emph{J. Energy Storage} (2020). DOI: 10.1016/j.est.2019.101329. [C-rate$\uparrow$ 일수록 peak 고전위 이동$\cdot$둔화 — 동역학 비대칭 꼬리.]
\bibitem{cogswell2012} D. A. Cogswell and M. Z. Bazant, ``Coherency strain and the kinetics of phase separation in LiFePO$_4$ nanoparticles,'' \emph{ACS Nano} \textbf{6}, 2215 (2012). DOI: 10.1021/nn204177u. [크기-의존 상도, 임계 반경 $\approx22$ nm — 마이크론 흑연 적용 영역 밖.]
