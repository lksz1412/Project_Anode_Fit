...
%     forward-only(ρ(U_j) 역산 안 함·ill-posed). w_eff 완전제거 유지. 인용 정정: +dahn1995·dreyer2010, -cogswell2012(size), fly2020=E.Schaltz vol.29.
...
\emph{전극이 단일 입자가 아니라 입자 앙상블이라는 사실}이다. 성격이 다른 세 출처를 구분해 합성한다.
...
남는다(다만 plateau 끝 유효 다중도 $n_j$ 가 $1$ 미만이면 그 절댓값은 $RT/F$ 보다 작을 수 있어, 표~\ref{tab:staging}
의 폭 폴백 0.012--0.020 V 와 모순되지 않는다 — $RT/F\!\approx\!26$ mV 는 $n_j\!=\!1$ 의 척도일 뿐 하한이 아니다).

\emph{③ 집합 다입자 통계역학(앙상블 몫).} 셋째는 앞 두 출처가 한 입자 안에서 일어나는 일이었던 것과 달리, \emph{전극이
$N$ 개 입자의 앙상블}이라는 데서 온다. 실측 dQ/dV 는 한 입자가 아니라 수많은 흑연 입자가 동시에 내는 응답의 \emph{합}이며,
입자마다 결정성$\cdot$흑연화도$\cdot$turbostratic 무질서$\cdot$국소 환경이 달라 그 전이 전위$\cdot$유효 배리어가
\emph{분포} $\rho(U_j)$ 를 이룬다 — 이 분포는 입자 \emph{크기}가 아니라 입자 \emph{구조의 무질서}에서 오는 비-크기
heterogeneity 다(층간 배열의 무질서가 평형 전위를 흩뜨린다는 흑연 격자기체 통계역학; Dahn 등의 $x_{\max}=1-P$ 무질서
모델\cite{dahn1995}). 단일 입자의 2상 평형은 한 전위의 델타이지만, 전극이 보는 양은 그 델타를 앙상블 분포로 가중 평균한
\emph{통계 평균}이다:
\begin{equation}
\Big\langle\frac{\dd Q}{\dd V}\Big\rangle_\mathrm{ens}
=\int \rho(U_j)\;\Big(\frac{\dd Q}{\dd V}\Big)^\mathrm{single}_{U_j}\,\dd U_j,
\label{eq:ensavg}
\end{equation}
곧 단일 입자 응답 $(\dd Q/\dd V)^\mathrm{single}_{U_j}$(평형 델타에 가까운 ①② 폭의 봉우리)을 중심 $U_j$ 가 분포한
앙상블 위로 \emph{forward} 합성한 것이다. 이 합성이 좁은 봉우리를 \emph{유한 폭의 종}으로 번지게 하며, 분포가 좁아져
$\rho\!\to\!\delta(U_j-\bar U_j)$ 면 식~\eqref{eq:ensavg} 는 단일 델타로 환원되어 ③ 이 사라진다(앙상블 폭은 입자
집합의 구조 무질서가 클수록 크다). 이 다입자 평균은 평형 전압이 입자별로 미세하게 갈려 셀 단위에서 단계적으로 전환된다는
Dreyer 등의 다입자 관점과 같은 언어다\cite{dreyer2010}. \emph{단, 본 문건은 식~\eqref{eq:ensavg} 의 forward 통계
평균만 쓴다} — 측정된 종에서 $\rho(U_j)$ 를 \emph{역산}하지 않는다(그 까닭은 아래 (c)).

이 세 출처가 평형의 (plateau 내부) 델타를 유한 폭의 치우친 종으로 펼친다 — 그리고 관측되는 peak 전위는 평형 중심이
아니라 $V_\mathrm{peak}=U_j+\eta$(평형 중심에 과전압이 얹힌 값)이므로, peak 이 \emph{분포}하는 것처럼 보인다면 그
분포의 정체는 \emph{과전압 $\eta$ 의 분포(①)와 앙상블 전이 전위 $\rho(U_j)$ 의 분포(③)}이지, 단일 입자 평형 중심
$\bar U_j$ 자체가 흔들리는 것이 아니다(평형 중심은 GITT 준평형에서 입자 무관 상수로 측정된다 — 아래 (c)). 그러므로
못박는다 — \emph{2상 전이의 폭 $w_j$ 는 식~\eqref{eq:wbase} 의 평형 예측($n_jRT/F$)을 그대로 믿을 양이 아니라, 위
세 broadening(전류 몫 $L_V$ $+$ 평형 잔여 가장자리 폭 $+$ 앙상블 $\rho(U_j)$)을 한꺼번에 흡수하는 현상학적 자유 피팅
폭이다.} 이것이 앞 \S\ref{sec:width} 에서 선언한 폭 이중지위의 \emph{두-상} 측을 떠받치는 물리이며, 거기서 ``두-상
$w$ 는 피팅이 정한다''고 적은 까닭이 바로 이 broadening 이다.
...
\emph{입자 사이즈(반경) 효과는 전면 배제}하고, (b)-③ 의 앙상블 통계역학은 \emph{비-크기} 전이 전위 분포로만 다룬다는
점을 가른다.
...
peak 을 해석하는 결론은 우리 조건에서 의미를 갖지 못한다. 따라서 (b)-③ 의 앙상블 분포 $\rho(U_j)$ 는 입자 \emph{크기}가
아니라 \emph{전이 전위$\cdot$구조 무질서}(결정성$\cdot$흑연화도$\cdot$turbostratic$\cdot$국소 환경)의 분포로만 해석하며,
크기 분포로 읽지 않는다.
\item \textbf{dQ/dV$\to\rho(U_j)$ 역산은 ill-posed 이므로 하지 않는다(forward-only).} (b)-③ 의 ``peak $=$ 단일 입자
커널 $\circledast$ 앙상블 분포 $\rho(U_j)$''(식~\eqref{eq:ensavg})는 1종 Fredholm 적분방정식(distribution of
relaxation times 와 동형)이라, 정방향(분포$\to$peak)만 잘 정의되고 역방향은 정규화$\cdot$형태가정 없이는
비유일$\cdot$불안정하다 — 서로 다른 분포가 거의 같은 곡선을 준다. 따라서 입자 heterogeneity(구조 무질서$\cdot$국소 환경)는
\emph{현상학적 폭 $w_j$ 에 흡수되는 것으로만} 두고 식~\eqref{eq:ensavg} 를 forward 로만 쓰며, 측정된 종에서 $\rho(U_j)$
를 \emph{역산하지 않는다}. 평형 중심 $\bar U_j$ 자체는 GITT 준평형 OCP 가 \emph{입자 무관 상수}로 측정하므로\cite{park2021},
분포하는 것은 그 중심이 아니라 앙상블의 폭 $\rho$ 와 과전압 $\eta$ 다(분포는 폭을 \emph{넓힐} 뿐 중심을 옮기지 않는다).
\item \textbf{입자 \emph{크기 분포}(PSD) 위로 합성곱하는 다입자 \emph{기계장치}는 모델에 넣지 않는다.} (b)-③ 의 앙상블
통계 평균은 \emph{설명}(forward $\int\rho\,\dd U_j$)으로 복원되었을 뿐, 코드의 forward 모델은 \emph{단일 유효 입자 $+$
동역학 지연 $L_V$}(N7$\cdot$N8) 구조를 유지한다. 곧 앙상블 무질서는 현상학적 $w_j$ 한 파라미터로 흡수되며, 입자 크기
분포를 명시적으로 적분하는 별도 다입자 convolution 층(크기 기계장치)은 도입하지 않는다 — 그 크기$\to$폭 맵은 본 장의
소관이 아니다(후속). 요컨대 모델 차원은 늘지 않고, 늘어난 것은 (b)-③ 의 \emph{비-크기} 통계 평균 \emph{설명}뿐이다.
\end{enumerate}
...
내재 폭($\sim RT/F$, 전류 무관) ③ \emph{입자 앙상블}의 전이 전위 분포 $\rho(U_j)$ 를 forward 평균한 집합 통계역학
($\int\rho\,(\dd Q/\dd V)^\mathrm{single}\,\dd U_j$, 식~\eqref{eq:ensavg}; $\rho\!\to\!\delta$ 면 단일 델타로 환원;
\emph{비-크기} 구조 무질서). 셋을 한꺼번에 흡수하는 것이 \emph{현상학적 자유 피팅 폭}이며(apparent-U $=U_j+\eta$, 분포하는
건 $\eta$ 와 앙상블 $\rho(U_j)$ — 단일 입자 평형 중심 $\bar U_j$ 는 입자 무관 상수). \emph{입자 사이즈(반경) 효과는
전면 배제}하고, $\rho(U_j)$ 역산$\cdot$PSD 크기 convolution 모델로 확장하지 않는다(역문제 ill-posed$\cdot$forward-only).
\end{keybox}
...
\node[font=\scriptsize] at (1.3,1.15) {$\eta$ tail};
...
켜는} 유한율속 비대칭 꼬리($\eta$, \S\ref{sec:tail} 의 $L_V$, $|I|\!\to\!0$ 소멸), ② \emph{전류 무관}한 가장자리
내재 폭($\sim RT/F$), ③ \emph{입자 앙상블}의 전이 전위 분포 $\rho(U_j)$ 를 forward 평균한 집합 통계역학
(식~\eqref{eq:ensavg}; 비-크기 구조 무질서). 그 폭 $w_j$ 는 평형 예측이 아니라 이 broadening 이 정하는 현상학적 자유
피팅 폭이며, 이 종에서 $\rho(U_j)$ 를 역산하거나 입자 사이즈(반경)로 해석하지 않는다(\S\ref{sec:broadening}(c)).}
...
\bibitem{rsc2021} 흑연 staging 전이별 1차/연속 구분(dilute$\cdot$4L--3L $=$ solid-solution, 나머지 $=$ two-phase plateau), \emph{J. Mater. Chem. A} (RSC, 2021). DOI: 10.1039/D0TA12607A. [tier A — ``plateau \dots except for 4L--3L''.]
\bibitem{fly2020} A. Fly and E. Schaltz, ``Temperature and current-rate dependence of incremental capacity peaks,'' \emph{J. Energy Storage} \textbf{29}, 101329 (2020). DOI: 10.1016/j.est.2019.101329. [C-rate$\uparrow$ 일수록 peak 고전위 이동$\cdot$둔화 — 동역학 비대칭 꼬리. 저자명은 DOI 로 식별, 권 \textbf{29}.]
\bibitem{dahn1995} J. R. Dahn 그룹(Dahn \emph{et al.}), 탄소 음극의 구조 무질서$\cdot$흑연화도와 리튬 삽입 통계역학($x_{\max}=1-P$ 무질서 모델), \emph{Science} \textbf{270}(5236), 590 (1995). DOI: 10.1126/science.270.5236.590. [비-크기 구조 무질서(turbostratic$\cdot$흑연화도$\cdot$국소 환경)가 전이 전위$\cdot$용량을 입자별로 분포시킴; 정확한 저자 표기는 DOI 로 식별.]
