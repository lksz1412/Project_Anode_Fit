...
상전이를 \emph{억제}한다 — 정규용액 틀에서 이는 $\Omega_j$ 를 $2RT$ 쪽으로 낮춰 peak 를 smear 하고
$U_j$ 를 미세 shift 시키는 것으로 들어온다(도핑으로 현상학적 $w_j$ 가 넓어지는 것으로 피팅에 반영된다).
흑연 \code{GRAPHITE\_STAGING\_LIT} 가 초기값이고 폭이 피팅 대상이었듯, pure-LCO 의 $\Omega_j$ 가 초기값이고
도핑에 따른 폭$\cdot$shift 는 우리 데이터로 피팅한다.
...
\textbf{★$w_j$ 이중지위 — 단상과 두-상에서 해석이 다르다.}
\begin{itemize}[leftmargin=1.4em,itemsep=2pt]
\item \textbf{단상($\Omega_j\le2RT$, dilute$\to$stage 4$\cdot$4L$\leftrightarrow$3L):} $w_j=n_jRT/F$ 는
  \emph{검증 가능한 평형 예측}이다 — 정규용액($\Omega=0$ 이상 극한)의 이상 등온선 폭이 $RT/F$ 로 닫히며,
  $n_j>1$ 이면 유효 위치 배타성·협동성의 보정이다. 이 폭은 온도로 검증할 수 있다($w\propto T$).
\item \textbf{두-상($\Omega_j>2RT$, $2\mathrm{L}{\to}2$ LiC$_{12}$· $2{\to}1$ LiC$_6$):} 평형 단일입자는
  이상적으로 델타에 가까우나(\S\ref{sec:broadening}), 실측 peak 는 유한 폭의 종이다 — 그 폭을
  $w_j$ 로 피팅한다. 이때 $w_j$ 는 \emph{현상학적 자유 피팅 폭}이며 평형 예측이 아니다:
  broadening 기작(유한율속 꼬리 $L_V$·내재 $RT/F$ 폭·입자 kinetic 분산)이 폭을 정하므로,
  apparent 중심 $=U_j+\eta$, 분포하는 것은 과전압 $\eta$ 다(\S\ref{sec:broadening} 상세).
  흑연의 두-상 전이 2개($2\mathrm{L}{\to}2$, $2{\to}1$; $\Omega_j>2RT$) 가 이 지위에 해당한다.
\end{itemize}

유효 폭 옵션은, 비단조 등온선~\eqref{eq:Veq} 의 중심 기울기를 이상 logistic 의 중심 기울기와
...
기본 \code{w\_eff\_floor\_frac}$=0.05$). \textbf{단, 이 식은 단일입자 평형 등온선 기울기에서 유도된 이론 폭이며,
두-상 실측 종폭을 설명하지 않는다} — 두-상 실측 종이 넓은 것은 $w^\eff$ 의 ``등온선 좁힘''이 아니라
$w_j$ 가 현상학적으로 흡수하는 broadening 기작(\S\ref{sec:broadening})이다. 이 옵션은 명시적으로
\code{use\_w\_eff} 를 켤 때만 적용되며, 기본(\code{use\_w\_eff=False})은 $w_j=n_jRT/F$ 다(\code{\_width}).
...
폭은 흑연과 같은 $w_j=n_jRT/F$ 를 초기값으로 하되, LCO order--disorder 전이는 $\Omega_j>2RT$ 라
$w_j$ 를 현상학적으로 좁게 피팅하면 T2$\cdot$T3 가 흑연보다 날카롭게 나온다(두-상 전이의 $w_j$
는 현상학적 피팅 폭, \S\ref{sec:width}$\cdot$\S\ref{sec:broadening} 참조). T1 의 위치는 \S\ref{sec:lco-electronic} 의 전자항이
...
% ====================================================================
\section{실측 종폭의 기원 — broadening 과 $w_j$ 의 현상학}\label{sec:broadening}
% ====================================================================
평형 종~\eqref{eq:eqpeak} 의 폭 $w_j$ 가 흑연 각 전이에서 어떤 물리적 의미를 갖는지는 전이 유형에 따라 갈린다.
이 절은 그 분기를 전이별로 닫고, 두-상 전이에서 $w_j$ 가 \emph{현상학적 피팅 폭}임을 명시한다 — 이것이 v8/v9 에서
단순화로 빠졌다가 v10 에서 복원되는 핵심 내용이다.

\subsection{전이별 출발점 — 애초에 델타가 아닌 전이가 있다}\label{sec:broadening-type}
``단일입자 평형 dQ/dV $=$ 델타''는 \textbf{강한 1차 전이($\Omega_j>2RT$)에만} 근사적으로 참이다. 흑연 전이는
성격이 갈린다:

\begin{itemize}[leftmargin=1.4em,itemsep=3pt]
\item \textbf{dilute$\to$stage~4$\cdot$4L$\leftrightarrow$3L = solid-solution(연속 전이):} 평탄역이 없으며
  ($\Omega_j\le2RT$), 단일입자 평형 dQ/dV 는 \emph{이미 폭 $\sim nRT/F$ 의 종}이다
  (Frumkin/정규용액 logistic; Levi \& Aurbach 1999, 10.1016/S0013-4686(99)00202-9;
  RSC 2021, 10.1039/D0TA12607A: ``plateau $\ldots$ except for 4L--3L'').
  이 두 전이에서는 별도 broadening 없이 $w_j=n_jRT/F$ 로 이미 종이 생긴다.
\item \textbf{2L$\to$2(LiC$_{12}$)$\cdot$2$\to$1(LiC$_6$) = two-phase plateau($\Omega_j>2RT$):} 평탄역 있음.
  평형 단일입자 dQ/dV 는 \emph{델타에 가깝다} — ``왜 실측은 종이냐?''는 이 두 전이에만 해당한다.
\end{itemize}

\subsection{두-상 전이의 델타가 실측 종이 되는 기작}\label{sec:broadening-mech}
two-phase 전이(LiC$_{12}$·LiC$_6$)에서 평형 델타가 실측 유한 종이 되는 것은
\emph{측정 조건이 폭을 만드는} 것이지, 평형 $U_j$ 분포가 아니다. 두 기작이 합성된다:

\begin{enumerate}[label=(\roman*),leftmargin=2.2em,itemsep=3pt]
\item \textbf{유한율속 비대칭 꼬리 [확정].} 단일입자라도 유한 전류에서 표면 SOC 가 평형을 앞서/뒤처져
  과전압 $\eta(\mathrm{SOC})$ 가 변하며, peak 를 한쪽으로 밀고 꼬리를 늘인다 — 모델의 $(\xi_\eq-\xi_{\mathrm{lag}})/L_V$
  (N7/N8, \S\ref{sec:lag}$\cdot$\S\ref{sec:tail}). C-rate$\uparrow$ 심화, 평형($C\to0$) 소멸.
  (Fly, Schaltz \& Stroe 2020, 10.1016/j.est.2019.101329)
\item \textbf{내재 $RT/F$ 폭 [확정].} 평탄역 양끝의 단상 꼬리·staging 정렬 폭이
  $\sim nRT/F\!\approx\!26\ \mathrm{mV}$ floor 를 깐다.
\end{enumerate}

\noindent
이 두 기작의 합이 two-phase 전이의 실측 종폭을 만든다.
apparent 관측 peak 위치는 $U_j+\eta$ — 즉 \emph{분포하는 것은 과전압 $\eta$}이고,
평형 $U_j$ 자체는 GITT 로 입자 무관 상수로 측정된다(Park 2021, 10.3390/ma14164683).

\subsection{★무엇을 하지 않는가 — 범위 경고}\label{sec:broadening-scope}
\begin{itemize}[leftmargin=1.4em,itemsep=3pt]
\item \textbf{평형 $U_j$·반경 분포 추출 금지.} 반경$\to$평형 $U_j$ 채널(표면에너지 Gibbs-Thomson·밴드)은
  마이크론 흑연에서 $\sim0.01$ mV 로 무시된다(Cogswell \& Bazant 2012 임계 22 nm). 평형 $U_j$ 는
  GITT상 입자 무관 상수이므로, dQ/dV peak 에서 평형 $U_j$ 분포나 반경 분포를 \emph{추출하지 않는다}.
\item \textbf{forward-only, ill-posed 역산 불가.} dQ/dV $\to$ 분포 역산은 1종 Fredholm 적분방정식의
  ill-posed 문제라, 단독 C-rate 데이터에서 분포를 역산하지 않는다. 모델은
  \emph{forward(가정 분포 $\to$ 예측 곡선)} 방향만 구현한다.
\item \textbf{★다입자/PSD convolution 모델 불포함.} 입자 크기 분포(PSD)의 kinetic 분산이 종폭의
  주요 기작으로 확인되었으나(Yang et al.\ 2023, 10.1038/s41467-023-40574-6; Kirk et al.\ 2022,
  10.1137/20M1344305), 본 모델은 다입자/PSD convolution 을 구현하지 않는다 — 입자 이질성은
  현상학적 $w_j$ 에 흡수되는 것으로만 언급한다(별도 분포 추출·모델 확장 없음).
\end{itemize}

\subsection{$w_j$(두-상)의 현상학적 해석 — apparent-$U$ 와 $\eta$ 분포}\label{sec:broadening-wj}
\textbf{★결론.} two-phase 전이(LiC$_{12}$·LiC$_6$)의 폭 $w_j$ 는 평형 예측($nRT/F$)이 \emph{아니라},
위 broadening 기작이 정하는 \textbf{현상학적 자유 피팅 폭}이다. 이것이 단상 $w_j=nRT/F$ (온도로
검증 가능한 평형 예측)와의 지위 차이이며(\S\ref{sec:width} 이중지위 참조), 피팅할 때는 두-상 $w_j$ 를
평형 등온선 폭으로 해석해서는 안 된다.
