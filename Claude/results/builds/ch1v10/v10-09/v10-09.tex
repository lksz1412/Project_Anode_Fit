...
상전이를 \emph{억제}한다 — 정규용액 틀에서 이는 $\Omega_j$ 를 $2RT$ 쪽으로 낮추거나, 현상학적 폭 $w_j$ 와 $U_j$ 의
피팅 shift 로 peak smear 를 흡수하는 것으로 들어온다. 흑연 \code{GRAPHITE\_STAGING\_LIT} 가 초기값이고 폭이
...
\subsection{폭 $w_j$ — 평형 예측과 현상학적 피팅 폭}
...
\textbf{$w_j$ 의 이중 지위.} $\Omega_j<2RT$ 인 단상(solid-solution) 전이에서는 $w_j=n_jRT/F$ 가 평형 등온선에서 나오는
검증 가능한 폭 예측이다. 반대로 $\Omega_j>2RT$ 인 staging 두-상 전이에서는 평형 단일입자 등온선이 plateau, 곧
델타에 가까운 peak 를 주므로 $w_j$ 를 이 식으로 예측하지 않는다. 이때 $w_j$ 는 유한율속 지연과 내재 폭을 한 폭으로
흡수하는 \emph{현상학적 자유 피팅 폭}이다. 따라서 본 장의 four graphite staging peaks 가운데 두-상으로 취급되는
전이의 $w_j$ 는 평형 좁힘식이 아니라 데이터가 정하는 apparent width 이며, 단상 폭 예측과 지위가 다르다. 특히
표~\ref{tab:staging} 의 현재 네 staging 피팅 항은 모두 $\Omega_j>2RT$ 초기값을 갖기 때문에, 이 네 항의 $w_j$ 는
모두 현상학적 피팅 폭으로 읽는다.
...
폭은 흑연과 같은 입력 자리 $w_j$ 를 쓰되, T1--T3 처럼 $\Omega_j>2RT$ 의 두-상 peak 에서는 평형 예측값이 아니라
현상학적 피팅 폭이다. T1 의 위치는 \S\ref{sec:lco-electronic} 의 전자항이
...
\subsection{평형 델타에서 실측 종으로 — broadening 의 범위}\label{sec:broadening}
평형 peak 식~\eqref{eq:eqpeak} 를 실측 peak 해석에 바로 등치하면 안 된다. 먼저 전이별 출발점이 다르다. dilute$\to$stage~4 와
$4\mathrm L\leftrightarrow3\mathrm L$ 같은 solid-solution 전이는 plateau 가 아니라 연속 등온선이므로, 단일 입자 평형
$\dd Q/\dd V$ 가 이미 $nRT/F$ 규모의 종이다\cite{levi1999}. 이 전이에는 ``델타가 왜 종이 되는가''라는 추가 broadening
문제가 없다. 반대로 $2\mathrm L\to2(\mathrm{LiC_{12}})$ 와 $2\to1(\mathrm{LiC_6})$ 는 두-상 plateau 전이이므로,
평형 단일입자 한계에서는 peak 가 델타에 가까워진다. 실측에서 이 두-상 전이가 유한 폭의 종으로 보이는 까닭이 이 절의
broadening 이다.

두-상 델타를 실측 종으로 바꾸는 주된 몫은 평형 $U_j$ 분포가 아니라 측정 조건과 내재 폭이다. 첫째, 유한 전류에서는 표면
SOC 가 평형 목표를 앞서거나 뒤따르면서 apparent 전위가
\begin{equation}
U_j^\app=U_j+\eta
\label{eq:Uapp}
\end{equation}
처럼 보인다. 이 과전압 $\eta$ 가 SOC 와 진행 방향에 따라 달라져 peak 를 한쪽으로 밀고 비대칭 꼬리를 만든다. 본 모델에서
그 몫은 $L_V$ 와 식~\eqref{eq:peakshape} 의 $(\xi_\eq-\xi_\mathrm{lag})/L_V$ 꼬리로 들어온다; C-rate 가 커질수록 커지고,
$|I|\to0$ 에서 사라지는 동역학 성분이다\cite{fly2020}. 둘째, plateau 양끝의 단상 꼬리와 점유 정렬 폭이 $RT/F$ 급의
하한을 둔다. 따라서 두-상 전이의 $w_j$ 는 평형식이 예측하는 폭이 아니라, 이 유한율속 꼬리와 내재 폭을 한 숫자로 흡수한
\emph{현상학적 자유 피팅 폭}이다. 분포가 있다면 그것은 참 평형 $U_j$ 의 분포가 아니라 $U_j^\app=U_j+\eta$ 에서
\emph{$\eta$ 의 분포}로 읽는다.

\begin{keybox}
\textbf{범위 경고.} 이 장은 평형 $U_j$ 분포나 입자 반경 분포를 dQ/dV peak 에서 추출하지 않는다. 마이크론 흑연에서
반경이 평형 $U_j$ 를 바꾸는 Gibbs--Thomson 류 채널은 무시할 만큼 작고\cite{cogswell2012}, GITT 가 읽는 평형 $U_j$ 는
입자 크기별 자유분포가 아니라 전이 상수로 취급된다\cite{park2021}. 또한 dQ/dV peak 로부터 분포를 역산하는 문제는
ill-posed 이므로 forward-only 로 둔다. 다입자/PSD convolution 모델은 만들지 않으며, 입자 heterogeneity 는 별도 분포
추출 없이 현상학적 $w_j$ 에 흡수된 것으로만 언급한다.
\end{keybox}
...
N4 & $w_j=n_jRT/F$ 또는 현상학적 피팅 폭 & \eqref{eq:wbase} & \code{func\_w},\code{\_width} \\
...
\bibitem{levi1999} M. D. Levi and D. Aurbach, graphite solid-solution/staging peak analysis, \emph{Electrochim. Acta} (1999). DOI: 10.1016/S0013-4686(99)00202-9.
\bibitem{fly2020} A. Fly, E. Schaltz, and D.-I. Stroe, C-rate dependence and resistance-corrected ICA peak analysis, \emph{J. Energy Storage} (2020). DOI: 10.1016/j.est.2019.101329.
\bibitem{cogswell2012} D. A. Cogswell and M. Z. Bazant, ``Coherency strain and the kinetics of phase separation in LiFePO$_4$ nanoparticles,'' \emph{Nano Lett.} \textbf{12}, 2215 (2012). DOI: 10.1021/nl300563u.
\bibitem{park2021} J. Park \emph{et al.}, graphite GITT/OCP staging constants and diffusion analysis, \emph{Materials} \textbf{14}, 4683 (2021). DOI: 10.3390/ma14164683.
