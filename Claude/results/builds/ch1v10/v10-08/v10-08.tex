...
%   불가침: 부호 8/8·11식 G-derive(SymPy/NumPy 검증)·D-PEAK/D-PEAK2·결과 박스·코드 정합·그림 9.
...
%   그림 경쟁 승자 = v7-11 유래 5 + 신규 4(검토2 청정). 이식금지 회귀: v8-02/03 D-PEAK 재정정·v8-07 s_F orphan.
%   ── 이하 베이스 v8-06b 원형 ──
...
$w_j$ (\code{func\_w}) & V & 전이 폭 척도. 단상은 $n_jRT/F$ 평형 예측, 두-상 전이는 현상학적 피팅 폭 \\
...
상전이를 \emph{억제}한다 — 정규용액 틀에서 이는 $\Omega_j$ 를 $2RT$ 쪽으로 낮추거나 실측 peak 폭을 키워
...
\subsection{폭 $w_j$ — 단상 평형 예측과 두-상 현상학}
평형 점유의 폭 척도는 이상 단상 극한에서 $w_j=n_jRT/F$ 다($n_j$ 는 폭 다중도): 아래 \S\ref{sec:width} logistic 유도의
...
여기서 $w_j$ 의 지위는 전이 성격에 따라 갈린다. $\Omega_j<2RT$ 의 단상(solid-solution) 영역에서는 식~\eqref{eq:wbase} 가
검증 가능한 평형 등온선 폭 예측이다. 반대로 $\Omega_j>2RT$ 로 두-상 plateau 와 staging 히스테리시스가 생기는 전이에서는
단일입자 평형 기여가 델타에 가까우므로, 실측 peak 폭 $w_j$ 는 평형에서 계산되는 폭이 아니라 유한율속 꼬리와 실험적
broadening 을 흡수하는 \emph{현상학적 자유 피팅 폭}이다. 본 흑연 staging 초기값 네 건은 모두 이 두 번째 지위로 읽는다.
...
위의 평형 peak 식~\eqref{eq:eqpeak} 은 코드가 쓰는 forward 모양의 기준선이지만, ``평형이면 델타인데 왜 실측은 종인가''라는
질문은 전이별로 나누어야 한다. dilute$\to$stage 4 와 $4\mathrm L\leftrightarrow3\mathrm L$ 계열은 plateau 가 없는
solid-solution 전이라 단일입자 평형 $\dd Q/\dd V$ 가 이미 폭 $\sim nRT/F$ 의 종이다\cite{levi1999}. 이 경우 broadening 은
델타를 종으로 바꾸는 별도 기작이 아니라 단상 등온선 자체다. 반대로 $2\mathrm L\to2(\mathrm{LiC}_{12})$ 와
$2\to1(\mathrm{LiC}_6)$ 는 두-상 plateau 전이라, 이상 평형 단일입자 기여가 델타에 가까워진다. 따라서 ``델타가 왜 유한한
종으로 보이는가''는 이 두 plateau 전이에 집중되는 질문이다.

두-상 델타가 실측 유한 종으로 보이는 첫 이유는 평형 $U_j$ 분포가 아니라 유한율속 지연이다. 단일 유효입자만 보아도 유한
전류에서는 표면 SOC 가 평형을 앞서거나 뒤처져 과전압 $\eta(V)$ 가 생기고, 관측 중심은
\[
U_{\app,j}=U_j+\eta
\]
처럼 보인다. 코드의 $L_{V,j}$ 꼬리, 곧 식~\eqref{eq:peakshape} 의
$(\xi_\eq-\xi_\mathrm{lag})/L_{V,j}$ 가 이 비대칭 broadening 을 담당한다. C-rate 를 낮추면 이 성분은 줄고, 평형 한계
($|I|\to0$)에서 사라진다\cite{fly2020}. 둘째 이유는 plateau 양끝의 단상 꼬리와 열적 점유 폭이 남기는 내재
$RT/F$ 규모의 바닥 폭이다. 따라서 두-상 전이에서 실측 폭 $w_j$ 는 식~\eqref{eq:wbase} 의 평형 예측값이 아니라,
$L_V$ 꼬리와 내재 열폭, 실험적 smear 를 한데 흡수하는 \emph{현상학적 자유 피팅 폭}이다. 분포한다면 참 평형 $U_j$ 가 아니라
apparent-$U=U_j+\eta$ 의 $\eta$ 몫이다.

범위도 여기서 닫는다. 본 장은 peak 모양에서 평형 $U_j$ 분포나 입자 반경 분포를 역산하지 않는다. 마이크론 흑연에서
반경이 평형 전위 $U_j$ 를 움직이는 Gibbs--Thomson 류 채널은 무시할 만큼 작고, 유의한 크기 효과는 나노 임계 크기에서나
문제가 된다\cite{cogswell2012}; GITT 로 읽는 평형 $U_j$ 는 입자 크기와 무관한 상수로 취급한다\cite{park2021}. 또한
$\dd Q/\dd V\to$분포 역산은 악조건 역문제라, 본 문건은 forward-only 로 남긴다. 입자 heterogeneity 는 별도 다입자/PSD
convolution 모델로 만들지 않고, 필요한 경우 현상학적 $w_j$ 와 $L_V$ 에 흡수된 broadening 으로만 언급한다.
...
폭 $w_j$ 는 단상 영역에서는 $n_jRT/F$ 의 평형 등온선 폭, T1/T2/T3 같은 두-상 또는 order--disorder 전이에서는
현상학적 피팅 폭으로 읽는다(\S\ref{sec:broadening}). T1 의 위치는 \S\ref{sec:lco-electronic} 의 전자항이
...
\code{use\_dH\_eff} & 전이/공통 & True & 유효 활성화 장벽 토글(\eqref{eq:dHeff}) \\
...
N4 & $w_j$: 단상 $n_jRT/F$, 두-상 현상학적 폭 & \eqref{eq:wbase}, \S\ref{sec:broadening} & \code{func\_w},\code{\_width} \\
...
\bibitem{levi1999} M. D. Levi and D. Aurbach, ``Frumkin intercalation isotherm---a tool for the description of lithium insertion into host materials: a review,'' \emph{Electrochim. Acta} \textbf{45}, 167 (1999). DOI: 10.1016/S0013-4686(99)00202-9.
\bibitem{fly2020} A. Fly, E. Schaltz, and D.-I. Stroe, ``Graphite anode voltage hysteresis and rate dependent differential capacity features,'' \emph{J. Energy Storage} \textbf{29}, 101329 (2020). DOI: 10.1016/j.est.2019.101329.
...
\bibitem{park2021} M. Park \emph{et al.}, ``Graphite anode staging behavior and equilibrium voltage reference from GITT,'' \emph{Materials} \textbf{14}, 4683 (2021). DOI: 10.3390/ma14164683.
