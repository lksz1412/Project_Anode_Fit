% ====================================================================
% graphite_ica_ch2_v4-11.tex — Chapter 2 (v4-11: 코인 하프셀 엔트로피·가역
%   발열의 통계열역학, Opus build — adversarial 재검수 정정 최종본)
...
\begin{tikzpicture}[x=1.0cm,y=1.0cm]
  ...
  \draw[dashed,red] (5.0,0) -- (5.0,3.2); \node[red,below right] at (5.0,-0.05) {\scriptsize $1$ (critical)};
  ...
  % regimes
  \node[align=center] at (1.3,-0.7) {\scriptsize homogeneous solid solution\\\scriptsize $\Omega<2RT$};
  \node[align=center,red] at (5.5,1.2) {\scriptsize phase separation\\\scriptsize ($\Omega>2RT$)};
\end{tikzpicture}
...
정의)이고, $w_j$ 는 전이별로 피팅한 \emph{실현 폭}이다 — 이상 격자기체($\Omega=0$)에서만 $w_j=RT/F$ 이고, 상호작용이
있으면 $w_j=w_\eff(\Omega)$(eq~\eqref{eq:weff})를 쓴다. 주어진 SOC $x$ 에서 $\xi_j$ 를 평가하려면 먼저 전하 보존
