% ====================================================================
% graphite_ica_ch1_v8-09.tex — Chapter 1 (v8-09: v7-11 layout + v5 derivation restoration)
%   Base layout = Claude/results/v7-11/v7-11.tex.
%   Derivation source = Claude/docs/graphite_ica_ch1_Opus_v5.tex.
%   Code ground truth = Claude/results/v7-00_spine/Anode_Fit_v11_final.py.
%   v8-09 adds step-by-step derivations while preserving v7-11 section order,
%   result identifiers, tables, code names, and sign convention.
...
%   그림 = v7-11 figure set plus one v8-09 TikZ figure, internal text ASCII-English only.
...
\hypersetup{colorlinks=true, linkcolor=blue, urlcolor=blue, citecolor=blue,
  pdftitle={흑연 음극 dQ/dV 이론: 코드 진행을 따라가는 수식-구동 — Chapter 1 (v8-09)}}
\pagestyle{fancy}\fancyhf{}
\lhead{흑연 음극 $dQ/dV$ 이론 — Ch.1 (v8-09, 유도 확장)}\rhead{\thepage/\pageref{LastPage}}
...
\title{\textbf{\LARGE Chapter 1}\\[0.35em]\Large 흑연 음극 $dQ/dV$ 이론: 한 곡선을 그리는 코드 진행을 따라가는 수식-구동 유도 확장판}
\author{}
\date{}
...
세 인자는 모두 하나의 속도식 $k\simeq k_0\exp[-\Delta G_a/RT]$ 의 서로 다른 자리로 들어온다 \cite{bazant2013} — 온도는 지수의 분모,
...
\draw[{Latex[length=1.8mm]}-,thick] (0,-1.05)--(6.45,-1.05) node[pos=0.5,below,font=\scriptsize] {delithiation (discharge): main direction, $\xi_j:0\to1$};
\end{tikzpicture}
...
출발점은 Gibbs 정의와 화학퍼텐셜 정의다. 등온$\cdot$등압에서
\begin{equation}
G=H-TS,\qquad
\mu=\frac{\partial G}{\partial n}\Big|_{T,P},
\label{eq:gmu_bridge}
\end{equation}
이고, 삽입 반응의 비배치 자유에너지 변화는 전기화학 평형식~\eqref{eq:eqcond} 에서 전위로 환산된다. 전이 중심에서는
\begin{equation}
\mu_\mathrm{Li}=\mu^0-sF(V-U_j),
\qquad
V=U_j
...
\label{eq:Uj_depart}
\end{equation}
한편 반응 열역학은 같은 $\Delta G_j$ 를
\begin{equation}
\Delta G_j=\Delta H_{\rxn,j}-T\Delta S_{\rxn,j}
\label{eq:Uj_gibbs}
\end{equation}
로 쓴다. 두 식을 같은 양으로 놓고 방전 기준 $s=+1$ 을 대입하면
\begin{align}
-F\,U_j
...
같은 전이를 서로 다른 준안정 가지로 과주행해 중심이 갈라진다 \cite{dreyer2010}. 이 절은 그 gap 의 닫힌 식과 부호를 유도한다.
...
\label{eq:gxi}
\end{equation}
이다. 한 번 미분하면 평형 전위 곡선의 비상수 몫이 나오고, 두 번 미분하면 spinodal 조건이 된다:
\begin{align}
g_j'(\xi)
&=RT\big[\ln\xi+1-\ln(1-\xi)-1\big]+\Omega_j(1-2\xi)\notag\\
&=RT\ln\frac{\xi}{1-\xi}+\Omega_j(1-2\xi),
\label{eq:gprime_v809}\\
g_j''(\xi)
&=RT\Big(\frac{1}{\xi}+\frac{1}{1-\xi}\Big)-2\Omega_j
...
\label{eq:spinodal_quad}
\end{equation}
이고, 근의 공식에서
\begin{equation}
\xi=\frac{1\pm\sqrt{1-2RT/\Omega_j}}{2}.
\label{eq:spinodal_formula}
\end{equation}
따라서 \textbf{spinodal} 은
\begin{equation}
...
spinodal 값~\eqref{eq:spinodal} 을 $V_{\eq,j}$ 의 두 비상수 항에 먼저 대입한다. 아래첨자 $-$ 는 낮은 진행률 쪽
$\xi_{s,j}^-=\tfrac12(1-u_j)$, $+$ 는 높은 진행률 쪽 $\xi_{s,j}^+=\tfrac12(1+u_j)$ 이다:
\begin{equation}
\frac{\xi_{s,j}^-}{1-\xi_{s,j}^-}=\frac{1-u_j}{1+u_j},\qquad
\frac{\xi_{s,j}^+}{1-\xi_{s,j}^+}=\frac{1+u_j}{1-u_j},
\label{eq:hyssub_log}
\end{equation}
\begin{equation}
1-2\xi_{s,j}^-=+u_j,\qquad
1-2\xi_{s,j}^+=-u_j.
\label{eq:hyssub_int}
\end{equation}
극대에서 극소를 빼면 $U_j$ 가 상쇄되고,
\begin{align}
\Delta U_j^\hys
&=V_{\eq,j}(\xi_{s,j}^-)-V_{\eq,j}(\xi_{s,j}^+)\notag\\
&=\frac{RT}{sF}\left[
\ln\frac{1-u_j}{1+u_j}
-\ln\frac{1+u_j}{1-u_j}\right]
+\frac{\Omega_j}{sF}\big[u_j-(-u_j)\big]\notag\\
&=-\frac{4RT}{sF}\,\mathrm{artanh}\,u_j
+\frac{2\Omega_j}{sF}\,u_j,
\label{eq:hysdiff_v809}
\end{align}
여기서 $\mathrm{artanh}\,u=\tfrac12\ln[(1+u)/(1-u)]$ 이다. 방전 기준 $s=+1$ 로 쓰면
\begin{equation}
\boxed{\;\Delta U_j^\hys
...
두 spinodal 끝점의 \emph{평균}은 로그 몫 합과 $(1-2\xi)$ 몫 합이 둘 다 $0$ 이라 정확히 $U_j$ 다:
\begin{align}
\frac{V_{\eq,j}(\xi_{s,j}^-)+V_{\eq,j}(\xi_{s,j}^+)}{2}
&=U_j+\frac{RT}{2sF}
\left[\ln\frac{1-u_j}{1+u_j}+\ln\frac{1+u_j}{1-u_j}\right]\notag\\
&\quad+\frac{\Omega_j}{2sF}\,[u_j+(-u_j)]
=U_j.
\label{eq:hyssym_v809}
\end{align}
따라서 분기는 $U_j$ 대칭이다. 이 대칭 위에서 실측 분기 중심을 방향별 한 자유도로 적으면
\begin{equation}
...
평형 점유의 폭 척도는 이상 격자기체의 로그 몫 기울기에서 나온다. 상호작용을 끈 이상 극한에서 점유비는
\begin{equation}
\mu-\mu^0=RT\ln\frac{\theta}{1-\theta},
\qquad \xi=1-\theta
\quad\Longrightarrow\quad
\ln\frac{\xi}{1-\xi}=\frac{\mathcal A}{RT}
\label{eq:w_depart}
\end{equation}
로 쓸 수 있다(부호는 진행률 $\xi$ 정의에 흡수). 전이 $j$ 가 한 Li 사건보다 넓은 유효 다중도 $n_j$ 를 갖는 경우, 같은 전기 구동력은 $n_jRT$ 에 나뉘어 들어간다:
\begin{equation}
\ln\frac{\xi}{1-\xi}
=\frac{\sigma_dF(V-U_j^d)}{n_jRT}.
\label{eq:w_mult}
\end{equation}
이를 logistic 표준형 $\ln[\xi/(1-\xi)]=\sigma_d(V-U_j^d)/w_j$ 와 항별 대조하면
\begin{equation}
\frac{1}{w_j}=\frac{F}{n_jRT}
\label{eq:w_compare}
\end{equation}
이고, 따라서 기본 폭은
\begin{equation}
...
상호작용이 종을 좁히는 옵션 유효 폭은 중심 기울기의 국소 선형화에서 읽는다. 식~\eqref{eq:gpp_v809} 와
$sF\,\dd V/\dd\xi=g''(\xi)$ 를 결합하면 전이 중앙 $\xi=\tfrac12$ 에서
\begin{equation}
sF\left.\frac{\dd V}{\dd\xi}\right|_{\xi=1/2}
=4RT-2\Omega_j
=4F\,\frac{RT}{F}\Big(1-\frac{\Omega_j}{2RT}\Big)
\equiv 4F\,w_j^\eff .
\label{eq:weff_midpoint}
\end{equation}
따라서
\begin{equation}
...
정$\cdot$역 두 방향 속도는 같은 전이상태를 지나며, 구동력 $\mathcal A_j$ 가 정방향 장벽을 $\chi\mathcal A_j$ 낮추고
역방향 장벽을 $(1-\chi)\mathcal A_j$ 높인다:
\begin{equation}
r_j^+=k_0e^{-(\Delta G_{a,j}-\chi\mathcal A_j)/RT},\qquad
r_j^-=k_0e^{-(\Delta G_{a,j}+(1-\chi)\mathcal A_j)/RT}.
\label{eq:xieq_rates}
\end{equation}
비를 취하면 $k_0$ 와 $\Delta G_a$ 가 약분되고, detailed balance 가 요구하는 Boltzmann 비가 남는다:
\begin{equation}
\frac{r_j^+}{r_j^-}
...
\label{eq:xieq_db}
\end{equation}
점유 인자를 포함한 정지 조건은 $r_j^+(1-\xi_j)=r_j^-\xi_j$ 이므로
\begin{equation}
...
\label{eq:xieq_logit}
\end{equation}
코드의 다중도 폭에서는 $\mathcal A_j/(RT)=\sigma_d(V-U_j^d)/w_j$ 이므로, 식~\eqref{eq:xieq_logit} 을 $\xi_{\eq,j}$ 에 대해 풀어
\begin{equation}
\xi_{\eq,j}\big(1+e^{\sigma_d(V-U_j^d)/w_j}\big)
=e^{\sigma_d(V-U_j^d)/w_j}
\quad\Longrightarrow\quad
\xi_{\eq,j}=\frac{1}{1+e^{-\sigma_d(V-U_j^d)/w_j}}.
\label{eq:xieq_solve}
\end{equation}
따라서 평형 등온선은 logistic 으로 닫힌다:
\begin{equation}
...
관측 $\dd Q/\dd V$ 는 전하 보존식에서 출발한다:
\begin{equation}
Q_\cell q=Q_\bg(V_n)+\sum_jQ_j\xi_j.
\label{eq:charge_v809}
\end{equation}
궤적을 따라 $q$ 로 미분하면
\begin{equation}
Q_\cell=C_\bg\frac{\dd V_n}{\dd q}+\sum_jQ_j\frac{\dd\xi_j}{\dd q},
\qquad C_\bg\equiv\frac{\partial Q_\bg}{\partial V_n},
\label{eq:charge_deriv_v809}
\end{equation}
이고, 양변을 $\dd V_n/\dd q$ 로 나누면
\begin{equation}
\frac{\dd Q}{\dd V_n}
=C_\bg+\sum_jQ_j\frac{\dd\xi_j}{\dd V_n}.
\label{eq:dqdv_chain_v809}
\end{equation}
평형 추종이면 $\xi_j=\xi_{\eq,j}$ 이다. $z\equiv\sigma_d(V-U_j^d)/w_j$ 로 두고 logistic 을 미분하면
\begin{align}
\frac{\dd\xi_\eq}{\dd z}
&=\frac{\dd}{\dd z}(1+e^{-z})^{-1}
=\frac{e^{-z}}{(1+e^{-z})^2}\notag\\
&=\frac{1}{1+e^{-z}}\frac{e^{-z}}{1+e^{-z}}
=\xi_\eq(1-\xi_\eq),
\label{eq:bell_identity_v809}
\end{align}
연쇄율 $\dd z/\dd V=\sigma_d/w_j$ 를 곱하면
\begin{equation}
...
들어온다. 같은 식은 적분인자로 일반해까지 닫힌다. 식~\eqref{eq:Lq} 를
\begin{equation}
\frac{\dd r_j}{\dd q}+\frac{r_j}{L_{q,j}}=\frac{\dd\xi_{\eq,j}}{\dd q}
\label{eq:Lq_linear}
\end{equation}
로 이항하고 $e^{q/L_{q,j}}$ 를 곱하면
\begin{equation}
\frac{\dd}{\dd q}\!\left(r_j e^{q/L_{q,j}}\right)
=e^{q/L_{q,j}}\frac{\dd\xi_{\eq,j}}{\dd q}.
\label{eq:Lq_ifactor}
\end{equation}
$q_0$ 부터 $q$ 까지 적분한 뒤 다시 $e^{-q/L_{q,j}}$ 를 곱하면
\begin{equation}
...
\label{eq:Lq_memory}
\end{equation}
따라서 $L_q$ 는 지수 기억 커널의 길이다. 완화 속도 $k_j$ 는 평형 근방 선형 완화에서 정$\cdot$역 속도의 합이다:
\begin{equation}
k_j=k^++k^-,
\qquad
\frac{k^-}{k^+}=e^{-\mathcal A_j/RT}
\quad\Longrightarrow\quad
k_j=k^+\big(1+e^{-\mathcal A_j/RT}\big).
\label{eq:k_sum_v809}
\end{equation}
Eyring forward 속도 \cite{eyring1935}
\begin{equation}
k^+=\frac{k_BT}{h}
\exp\!\left[-\frac{\Delta G_{a,j}^\eff-\chi_d\mathcal A_j}{RT}\right]
\label{eq:kplus_v809}
\end{equation}
를 넣으면, 코드가 평가하는 형태가 된다.

\subsection{컷 affinity $\mathcal A$ — 꼬리 시작점의 구동력}
$L_q$ 는 전이당 한 점(꼬리 컷점 $q_a$)에서 \emph{상수로 동결}된다. 컷점은 원천 $\dd\xi_\eq/\dd q$ 가 정점의 일정 분율
(약 $5\%$)로 떨어지는 좌표이다. logistic 원천의 정규화 미분은
\begin{equation}
\frac{(\dd\xi/\dd z)}{(\dd\xi/\dd z)_{\max}}
=4\xi(1-\xi)=\mathrm{sech}^2(z/2).
\label{eq:zcut_source}
\end{equation}
$5\%$ 컷은 $\mathrm{sech}^2(z_\mathrm{cut}/2)=0.05$ 이므로
\begin{equation}
z_\mathrm{cut}=2\,\mathrm{arcosh}\sqrt{20}=4.357\ldots .
\label{eq:zcut_value}
\end{equation}
따라서 그곳의 구동력 크기는 $\mathcal A=z_\mathrm{cut}\cdot n_jRT$ 다(점유 $\xi_\eq$ 자체가 아니라 미분 기준). 상한 $A_\mathrm{cap}RT$ 와 함께
\begin{equation}
...
$\xi\to1$, 충전은 $\xi\to0$ 거울이라 역방향 장벽의 상수 몫이 같은 $+\Omega$ 로 흡수되는 것을 반영한다. 그 흡수는
상호작용을 포함한 구동력에서 직접 보인다:
\begin{equation}
\mathcal A_j=\sigma_dF(V-U_j^d)-\Omega_j(1-2\xi_j).
\label{eq:A_with_Omega_v809}
\end{equation}
장벽 조합은
\begin{align}
\Delta G_{a,j}-\chi_d\mathcal A_j
&=\Delta G_{a,j}-\chi_d\sigma_dF(V-U_j^d)
+\chi_d\Omega_j(1-2\xi_j).
\label{eq:barrier_split_v809}
\end{align}
방전 깊은 꼬리에서 $\xi_j\to1$ 이므로 $(1-2\xi_j)\to-1$ 이고, 충전은 거울상 $\xi_j\to0$ 를 방향별 $\chi_d=1-\chi$ 가 받는다.
따라서 상호작용 상수 몫은 활성화 엔탈피에
\begin{equation}
\Delta H_{a,j}\longmapsto \Delta H_{a,j}-\chi_d\Omega_j
\label{eq:dHeff_map_v809}
\end{equation}
로 흡수된다. 그 결과가 유효 장벽이다:
\begin{equation}
...
정$\cdot$역 합~\eqref{eq:k_sum_v809} 과 $k_0=k_BT/h$ 를 식~\eqref{eq:Lq} 에 넣으면
\begin{align}
L_{q,j}
&=\frac{|I|}{Q_\cell}
\frac{1}{(k_BT/h)
e^{-(\Delta G_{a,j}^\eff-\chi_d\mathcal A)/RT}
(1+e^{-\mathcal A/RT})}\notag\\
&=\frac{|I|h}{Q_\cell k_BT}
\frac{\exp[(\Delta G_{a,j}^\eff-\chi_d\mathcal A)/RT]}
{1+e^{-\mathcal A/RT}}.
\label{eq:Lq_sub_v809}
\end{align}
$T_*\equiv |I|h/(Q_\cell k_B)$ 와 $\Delta G_a^\eff=\Delta H_a^\eff-T\Delta S_a$ 를 쓰면 코드가 평가하는 $L_q$ 가 된다:
\begin{equation}
...
\begin{equation}
\ln L_{q,j}
=\underbrace{\ln(T_*/T)}_{\text{attempt/rate}}
\underbrace{\frac{\Delta H_{a,j}^\eff-T\Delta S_{a,j}}{RT}}_{\text{activation}}
\underbrace{-\ln(1+e^{-\mathcal A/RT})}_{\text{reverse-rate sum}}
\underbrace{-\frac{\chi_d\mathcal A}{RT}}_{\text{field lowering}} .
\label{eq:Lqlog_terms_v809}
\end{equation}
...
점화식은 전압축 완화식에서 직접 나온다. 진행 방향 좌표를 $x$ 라 두면
\begin{equation}
\frac{\dd\xi_\mathrm{lag}}{\dd x}
=\frac{\xi_\eq-\xi_\mathrm{lag}}{L_V}.
\label{eq:lag_ode_v809}
\end{equation}
한 격자 구간 $[x_{i-1},x_i]$ 에서 $\xi_\eq$ 를 $\xi_{\eq,i}$ 로 상수 근사하면
\begin{equation}
\xi_\mathrm{lag}(x_i)-\xi_{\eq,i}
=\big[\xi_\mathrm{lag}(x_{i-1})-\xi_{\eq,i}\big]e^{-\Delta_\mathrm{grid}/L_V},
\label{eq:lag_step_solve_v809}
\end{equation}
따라서 $\rho=e^{-\Delta_\mathrm{grid}/L_V}$ 를 넣어 식~\eqref{eq:lowpass} 가 된다. 그림~\ref{fig:memorykernel} 은 같은 해가
연속 지수 커널과 이산 저역통과로 보이는 두 얼굴임을 보인다.

\begin{figure}[t]
\centering
\begin{tikzpicture}[x=0.9cm,y=1.05cm]
\begin{scope}
\draw[-{Latex[length=1.6mm]}] (0,0) -- (5.4,0) node[below,font=\scriptsize] {$x$};
\draw[-{Latex[length=1.6mm]}] (0,0) -- (0,2.8) node[above,font=\scriptsize] {kernel};
\draw[thick] plot[smooth] coordinates {(0,2.4) (0.5,1.46) (1,0.88) (1.5,0.54) (2,0.32) (2.5,0.20) (3,0.12) (3.5,0.07) (4,0.04) (4.8,0.02)};
\draw[densely dashed] (1,0)--(1,0.88);
\draw[densely dashed] (2,0)--(2,0.32);
\draw[densely dashed] (3,0)--(3,0.12);
\node[font=\scriptsize] at (2.55,2.1) {$K(\Delta x)=e^{-\Delta x/L_V}$};
\node[font=\scriptsize,below] at (2.7,-0.42) {(a) continuous memory};
\end{scope}
\begin{scope}[xshift=7.0cm]
\draw[-{Latex[length=1.6mm]}] (0,0) -- (5.4,0) node[below,font=\scriptsize] {grid index};
\draw[-{Latex[length=1.6mm]}] (0,0) -- (0,2.8) node[above,font=\scriptsize] {$\xi$};
\draw[densely dotted,thick] plot[smooth] coordinates {(0,0.20) (0.8,0.35) (1.4,0.80) (2,1.50) (2.6,2.10) (3.2,2.38) (4,2.48) (5,2.50)};
\draw[thick] plot[smooth] coordinates {(0,0.20) (0.8,0.25) (1.4,0.45) (2,0.82) (2.6,1.28) (3.2,1.70) (4,2.10) (5,2.34)};
\node[font=\scriptsize] at (3.9,2.63) {$\xi_\eq$};
\node[font=\scriptsize] at (4.2,1.85) {$\xi_\mathrm{lag}$};
\node[font=\scriptsize] at (2.55,0.42) {$\rho=e^{-\Delta_\mathrm{grid}/L_V}$};
\node[font=\scriptsize,below] at (2.7,-0.42) {(b) discrete low-pass};
\end{scope}
\end{tikzpicture}
\caption{지연 ODE 의 기억 커널과 이산 저역통과(신규). (a) 식~\eqref{eq:Lq_memory} 의 지수 커널은 최근 길이 $L_V$ 만큼의 과거를 가중한다. (b) 한 격자에서 $\xi_\eq$ 를 상수로 두고 적분하면 식~\eqref{eq:lowpass} 의 $\rho$ 점화식이 된다. 그림 내부 텍스트는 ASCII-English 로 제한했다.}
...
=\frac{\xi_\eq-\xi_\mathrm{lag}}{L_V}
\quad\Longrightarrow\quad
\left|\frac{\dd\xi}{\dd V}\right|_{\text{tail}}
=\frac{\xi_\eq-\xi_\mathrm{lag}}{L_V}
\label{eq:peak_from_ode_v809}
\end{equation}
(방전에서는 $x=V$, 충전에서는 뒤집힌 격자의 진행 좌표). 따라서 코드가 쓰는 모양은
\begin{equation}
...
의 출력). 여기서 $L_V$ 극한은 식~\eqref{eq:lowpass} 의 $\rho=e^{-\Delta_\mathrm{grid}/L_V}$ 로 읽어야 한다. $L_V$ 가
크면 $\rho\to1$ 이라 긴 기억이 남아 매끈한 bell/tail 이 생기지만, $L_V$ 가 작으면 $\rho\to0$ 이라
$\xi_\mathrm{lag}\to\xi_\eq$ 가 되어 식~\eqref{eq:peakshape} 만으로는 numerator 가 사라져 peak 가 $0$ 으로 눌린다.
따라서 작은 $L_V$ 의 평형 회복은 꼬리식의 극한이 아니라 아래 분기 조건이 평형 peak 식~\eqref{eq:eqpeak} 로 직접
갈아타는 방식으로 처리한다:
\begin{equation}
\text{peak\_shape}=
\begin{cases}
...
$\sum_jQ_j\xi_j$ 꼴이라 관측 ICA 는 배경과 전이별 기여의 선형 합이다. 식으로는
\begin{equation}
Q_\cell q=Q_\bg(V_n)+\sum_{j=1}^{N_p}Q_j\xi_j
\label{eq:sum_depart_v809}
\end{equation}
에서 출발해
\begin{equation}
\frac{\dd Q}{\dd V_n}
=C_\bg+\sum_{j=1}^{N_p}Q_j\frac{\dd\xi_j}{\dd V_n}.
\label{eq:sum_mid_v809}
\end{equation}
각 전이는 앞 절의 분기 규칙으로
\begin{equation}
\frac{\dd\xi_j}{\dd V_n}
=
\begin{cases}
\xi_{\eq,j}(1-\xi_{\eq,j})/w_j & L_{V,j}<\nu\Delta_\mathrm{grid}\\[2pt]
(\xi_{\eq,j}-\xi_{\mathrm{lag},j})/L_{V,j} & \text{그 외}
\end{cases}
\equiv \text{peak\_shape}_j
\label{eq:sum_peakshape_v809}
\end{equation}
이므로
\begin{equation}
...
\begin{verifybox}
\textbf{Regression self-test (5 checks, 재현 가능 수치 고정).} 위 정성 검산을 falsifiable 수치에 못박아 회귀를 막는다 — 아래
다섯 항은 코드 \code{\_\_main\_\_} 의 self-test 와 같은 양을 손으로 재산출한 것이며($T=298.15$ K), 한 줄이라도 어긋나면
부호 사슬이 깨진 것이다.
...
\item \textbf{$L_V$ 분기 극한.} 직접 지정 $L_V<2\Delta_\mathrm{grid}$ 에서는 식~\eqref{eq:branch} 가 평형 종
$\xi_\eq(1-\xi_\eq)/w$ 를 선택한다. 꼬리식을 강제로 쓰면 작은 $L_V$ 에서 $\rho=e^{-\Delta_\mathrm{grid}/L_V}\to0$,
$\xi_\mathrm{lag}\to\xi_\eq$, $(\xi_\eq-\xi_\mathrm{lag})/L_V\to0$ 이므로 peak 가 사라진다. 큰 $L_V$ 에서는
$\rho\to1$ 이라 인과 저역통과가 매끈한 bell/tail 을 만든다 — 작은 $L_V$ 평형 회복은 분기식이 보장한다.
\end{enumerate}
