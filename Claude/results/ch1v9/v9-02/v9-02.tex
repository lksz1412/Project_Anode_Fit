...
\lhead{흑연 음극 + LCO 양극 $dQ/dV$ 이론 — Ch.1 (v9, 통합)}\rhead{\thepage/\pageref{LastPage}}
...
\title{\textbf{\LARGE Chapter 1}\\[0.35em]\Large 흑연 음극 + LCO 양극 $dQ/dV$ 이론: 한 곡선을 그리는 코드 진행을 따라가는 수식-구동판 — 유도 확장 (v9, 통합)}
\author{}
\date{}
...
% ====================================================================
리튬이온전지의 incremental capacity analysis(ICA)는 충방전 곡선 $Q(V)$ 의 미분
$\dd Q/\dd V$ 를 본다 \cite{bloom2005,dubarry2012}. 흑연 음극은 staging 상전이(stage
$4\!\to\!3\!\to\!2\mathrm L\!\to\!2\!\to\!1$)를 거치며 \cite{dahn1991,ohzuku1993}, LCO 양극은
order--disorder 전이(~4.05/4.17 V)와 metal--insulator 천이(MIT, ~3.90 V)를 거치며\cite{xia2007}, 각 전위에서
두 상이 공존하거나 상태밀도가 급변하면 $\dd Q/\dd V$ 가 치솟는다. 본 문건은 이 두 전극을
\emph{같은 forward 모델 프레임}으로 통합 서술한다 — 흑연의 결과·식·부호는 v8-11 을 그대로 보존하고,
LCO 고유 항(전자 엔트로피·전이표)을 \emph{추가}한다.
...
\textbf{LCO 양극 전이 인벤토리 (\code{LCO\_STAGING\_LIT}).}
코인 하프셀(LCO vs Li, $\le4.2$--$4.5$ V) 에서 LCO 는 세 전이를 낸다. 흑연 4 전이에 대칭되는
세 전이로, 전자 엔트로피 기여 여부가 각 전이의 고유성을 결정한다(표~\ref{tab:lco_transitions}).

\begin{table}[t]
\centering
\small
\renewcommand{\arraystretch}{1.25}
\caption{LCO 양극 전이 초기값 \code{LCO\_STAGING\_LIT}(3건, 코인 하프셀 vs Li, 전이 위치 tier A\,=\,Xia et al.~\cite{xia2007}; $\Delta S$ tier A/B).
흑연과 동일 클래스(logistic 중심$\cdot$폭$\cdot$히스$\cdot$꼬리)이며, T1 에만 전자 엔트로피 $\Delta S_e$
게이트가 ON 된다(\S\ref{sec:elec_entropy}).}
\label{tab:lco_transitions}
\begin{tabular}{@{}l c c c c l@{}}
\toprule
전이 & $U_j^\cat$ [V] & $x$ 범위 & $\Delta S_\rxn^\cat$ 초기값 & $\Delta S_e$ & 성격 \\
\midrule
T1 (MIT)            & ${\approx}3.90$ & $0.75$--$0.94$ & config$+\Delta S_e$ & ON & insulator$\to$metal \\
T2 (ord--dis.\ a) & ${\approx}4.05$ & ${\approx}0.55$ & config 주도 & off & hex$\to$monoclinic \\
T3 (ord--dis.\ b) & $4.17$--$4.20$ & ${\approx}0.48$ & config 주도 & off & monoclinic$\to$hex \\
\bottomrule
\end{tabular}
\end{table}

\textbf{양극 부호 규약.} LCO 하프셀 전위는 Li/Li$^+$ 기준 양수($U_j^\cat\approx3.9$--$4.2$ V) — 흑연
($U_j\approx0.085$--$0.21$ V)보다 훨씬 높고, 부호 사슬($\partial U_j/\partial T=\Delta S_{\rxn,j}/F$,
방전 $\sigma_d=+1$ 에서 $V\!\uparrow\Rightarrow$ 탈리튬화$\uparrow$)은 흑연과 \emph{동일}하다. 단전극 측정
기준 $\partial\phi_\cat/\partial T\approx+0.83$ mV/K\cite{wang2009} (전체 부호 sanity, 양수 확인). 이하 절은 흑연 사례를
기본으로 전개하되, 각 절 말미에 LCO 대응을 서술한다.
...
\subsection{LCO 양극 $U_j^\cat(T)$ — 같은 식, 다른 파라미터}
흑연의 식~\eqref{eq:Uj} 는 전극 일반식이다 — LCO 에도 동일하게 적용하되, 파라미터가 달라진다.
\begin{equation}
...
\label{eq:Uj_cat}
\end{equation}
세 전이 $T1$--$T3$ 의 초기값은 표~\ref{tab:lco_transitions} 에 있다. \textbf{부호 확인.} 양극 탈리튬화는
$x\!\downarrow$ 방향(Li 빠져나감)이고, 단전극 dφ/dT $\approx+0.83$ mV/K (tier B)는 $\Delta S_{\rxn}^\cat/F>0$
으로 이어진다 — 곧 LCO 의 $\Delta S_{\rxn,j}^\cat$ 는 \emph{흑연 T1($+29$ J/(mol\,K))처럼 양수} 쪽에
있으며(overall config 지배), 흑연 T4($-16$ J/(mol\,K)) 류의 큰 음수는 LCO 범위에 없다.

\textbf{온도 의존 크기.} $|\Delta S_{\rxn}^\cat|\sim$ 수--수십 J/(mol\,K) 이면 30 K 창에서 중심이
수 mV 이동하는 것은 흑연과 같다 — 다온도 피팅에서 LCO peak 위치도 $U_j^\cat(T)$ 를 온도별로 평가해야 한다.

\textbf{LCO 고유: 전자 엔트로피 기여.} $\Delta S_{\rxn,j}^\cat$ 에는 config$+$vib 외에 전자 엔트로피 항
$\Delta S_e$ 가 더해진다 — 특히 T1(MIT 구간)에서 Fermi 준위 상태밀도 $g(E_F)$ 의 급변이 $\partial U^\cat/\partial T$
에 $T$-선형 기여를 낸다. 이 항의 유도는 \S\ref{sec:elec_entropy} 에서 상세히 전개하며, 그 전에
config/vib 몫의 틀(흑연과 동형)을 먼저 확인한다.
...
\textbf{LCO 의 $\Omega_j$.} LCO order--disorder 전이(T2, T3)와 MIT 2상역(T1)도 동일한 정규용액 틀로
기술된다. T2/T3 의 hex$\leftrightarrow$monoclinic 정렬 전이 및 T1 의 insulator/metal 2상 공존은 모두
$\Omega_j>2RT$ 로 2상역이 열리는 구조이며, 흑연의 staging gap 과 같은 spinodal 메커니즘에서
$\Delta U_j^\hys$ 와 분기 중심 $U_j^{\,d}$ 를 산출한다. 순수 LCO 피팅값은 초기값(\S\ref{sec:sum}
표~\ref{tab:lco_transitions})이며, 도핑(Al/Mg 비-redox)은 logistic 폭 $w$ 을 넓히고 $U_j$ 를 미세
shift 해 peak 를 smear 한다(전자항은 보존).
...
\subsection{(e) $\xi_\eq$ 의 분포 관점 — 단일 자리 점유 확률}
앞의 두 유도(kinetic 정지점·thermo 자유에너지 최소)는 같은 결과 식~\eqref{eq:xieq} 에 도달했다. 이
수렴을 하나의 \emph{분포} 언어로도 볼 수 있으며, 그 시각이 Ch.2(분포 $-R\sum p\ln p$ 에서 엔트로피 전개)와
LCO 전자 엔트로피(\S\ref{sec:elec_entropy}, Fermi--Dirac 분포)를 같은 언어로 잇는 다리가 된다.

격자기체 단일 자리의 grand-canonical 분배함수는 빈 자리($\varepsilon=0$)와 찬 자리($\varepsilon=\Delta\mu$,
$\Delta\mu\equiv\mu-\mu^0=-sF(V-U_j)$ 가 화학퍼텐셜 차)로 이루어진다:
\begin{equation}
Z\;=\;1+e^{-\beta\Delta\mu},\qquad \beta\equiv\frac{1}{k_BT}=\frac{F}{RT}.
\label{eq:partition}
\end{equation}
자리가 차 있을 평균 확률(평균 점유)은 $\langle n\rangle=e^{-\beta\Delta\mu}/Z$, 곧
\begin{equation}
\langle n\rangle
...
\label{eq:xieq_gc}
\end{equation}
상호작용이 없는 이상($\Omega_j=0$) 극한에서 이 grand-canonical 평균 점유가 식~\eqref{eq:xieq} 의
logistic 과 \emph{동일}함을 확인한다. 달리 말하면, $\xi_\eq$ 는 단일 자리에서 리튬이 ``점유됨''
상태에 있을 평형 확률이며, 그 분포의 폭 $w_j=n_jRT/F$ 가 열적 흩어짐을 결정한다.

이 분포 관점의 교육 가치는 두 가지다. 첫째, kinetic(detailed balance)과 thermo(자유에너지 최소) 두
경로가 \emph{같은 점유 확률 분포}라는 한 그림으로 통합된다. 둘째, LCO 양극의 전자 엔트로피 유도에
쓰이는 Fermi--Dirac 분포 $f(\varepsilon)=1/(1+e^{(\varepsilon-\mu)/k_BT})$ 가 식~\eqref{eq:xieq_gc}
와 \emph{같은 구조}이다 — ``점유 확률'' 언어가 이온 자리(Li$^+$)와 전자 상태(도전 전자) 두 곳에 동시에
성립하며, 이 대칭이 \S\ref{sec:elec_entropy} 의 전자 엔트로피 유도로 자연스럽게 이어진다.
...
\textbf{LCO 평형 peak 위치.} LCO 에서 같은 식~\eqref{eq:eqpeak} 는 T1$\cdot$T2$\cdot$T3 에 적용되며,
각 중심 $U_j^\cat$ 에서 peak 를 낸다. 그림~\ref{fig:lco_dqdv} 가 흑연과 LCO 의 $\dd Q/\dd V$ 개략도를
비교한다.

\begin{figure}[t]
\centering
\begin{tikzpicture}[x=1.0cm,y=0.55cm]
% LCO dQ/dV schematic: three peaks at 3.90, 4.05, 4.17 V
\draw[-{Latex[length=1.5mm]}] (0,0) -- (9.5,0) node[below,font=\scriptsize] {$V$ [V vs Li]};
\draw[-{Latex[length=1.5mm]}] (0,0) -- (0,6.5) node[above,font=\scriptsize] {$\dd Q/\dd V$ [arb.]};
% x-axis ticks
\foreach \xv/\xl in {1/0.1, 2/0.2, 6/3.9, 7/4.05, 8/4.17}{
  \draw (\xv,0.08)--(\xv,-0.08) node[below,font=\tiny] {\xl};
}
% graphite region label
\node[font=\scriptsize,gray] at (1.5,5.8) {graphite (anode)};
\draw[gray,densely dotted] (0.5,5.5)--(3.0,5.5);
% graphite peaks (schematic, 4 peaks compressed near 0.085-0.21 V)
\draw[gray,thick] plot[smooth] coordinates {(0.5,0.2)(0.75,1.5)(1.0,3.5)(1.1,5.0)(1.2,3.5)(1.45,1.5)(1.6,0.5)(1.75,1.2)(1.9,2.0)(2.0,1.2)(2.15,0.8)(2.3,1.5)(2.5,2.5)(2.65,1.5)(2.8,0.3)};
% LCO region label
\node[font=\scriptsize] at (7.0,6.2) {LCO (cathode)};
% T1 peak at 3.90V: broader (MIT, 2-phase)
\draw[thick] plot[smooth] coordinates {(5.2,0.1)(5.6,0.4)(5.9,0.9)(6.0,1.8)(6.1,3.0)(6.2,3.8)(6.3,2.8)(6.4,1.5)(6.6,0.5)(6.8,0.2)};
\node[font=\tiny,above] at (6.2,3.8) {T1 (MIT)};
% T2 peak at 4.05V
\draw[thick] plot[smooth] coordinates {(6.7,0.1)(6.85,0.4)(6.95,1.0)(7.0,1.6)(7.05,2.2)(7.1,1.6)(7.2,0.8)(7.35,0.2)};
\node[font=\tiny,above] at (7.05,2.2) {T2};
% T3 peak at 4.17V
\draw[thick] plot[smooth] coordinates {(7.5,0.1)(7.65,0.5)(7.75,1.2)(7.8,1.9)(7.85,1.2)(7.95,0.4)(8.1,0.1)};
\node[font=\tiny,above] at (7.8,1.9) {T3};
% voltage separator
\draw[densely dashed,gray] (3.5,-0.3)--(3.5,6.0) node[pos=0.95,right,font=\tiny,gray] {electrode boundary};
\end{tikzpicture}
\caption{흑연 음극(회색, $\approx0.1$--$0.2$ V)과 LCO 양극(검정, $3.9$--$4.2$ V)의 평형
$\dd Q/\dd V$ 개략 비교(신규 그림; 정성 모식도). 흑연 4 peak (stage 전이)와 LCO 3 peak (T1 MIT·T2·T3
order--disorder)가 코인 하프셀에서 각각 독립적으로 측정된다. 두 전극 모두 식~\eqref{eq:eqpeak} 의
$Q_j\xi_\eq(1-\xi_\eq)/w$ 로 기술되며, T1 은 전자 엔트로피(\S\ref{sec:elec_entropy})가 $\partial U^\cat/\partial T$
에 추가 기여를 낸다.}
\label{fig:lco_dqdv}
\end{figure}
...
% ====================================================================
\section{LCO 전자 엔트로피 항 (N5.5 — LCO 고유)}\label{sec:elec_entropy}
% ====================================================================
흑연은 반금속으로 탈리튬화에 따른 Fermi 준위 상태밀도 $g(E_F)$ 변화가 미미해 전자 엔트로피를 무시할 수 있다.
LCO 는 그렇지 않다 — $x=1$ (완전 리튬화, LiCoO$_2$)에서는 절연체이나 $x\!\downarrow$ 탈리튬화로
금속--절연체 천이(MIT)가 일어나 $g(E_F)$ 가 급증한다. 이 변화가 단전극 $\partial U^\cat/\partial T$ 에
추가 $T$-선형 기여를 낳는다. 본 절은 이 항을 Fermi--Dirac 분포에서 유도하고, LCO forward 모델에
$\Delta S_e(x,T)$ 로 plug-in 한다.

\subsection{전자 엔트로피 — Fermi--Dirac 에서 Sommerfeld 전개}
\textbf{(a) 출발 — 도전 전자의 Fermi--Dirac 분포.}
\S\ref{sec:width}(e) 에서 이온 자리의 점유 확률 $\xi_\eq=1/(1+e^{\beta\Delta\mu})$ 를 grand-canonical
분배함수에서 얻었다. 전도 전자 상태 $\varepsilon$ 의 점유 확률도 같은 구조의 분포를 따른다:
\begin{equation}
f(\varepsilon)\;=\;\frac{1}{1+\exp\!\big[(\varepsilon-\mu_e)/k_BT\big]},
\label{eq:fermi_dirac}
\end{equation}
($\mu_e$ = 전자 화학퍼텐셜). 이것이 Fermi--Dirac 분포이며, $k_BT\ll\mu_e$ (축퇴 전자기체) 조건에서
열역학 함수들을 Sommerfeld 전개로 닫을 수 있다.

\textbf{(b) 연산 — Sommerfeld 전개: $\pi^2/3$ prefactor 명시 유도.}
단위 자리당(per Co site) 전자 내부 에너지는 상태밀도 $g(\varepsilon)$[단위: eV$^{-1}$/site] 로 가중한 적분이다:
\begin{equation}
u_e\;=\;\int_0^\infty \varepsilon\,g(\varepsilon)\,f(\varepsilon)\,\dd\varepsilon.
\label{eq:ue_int}
\end{equation}
$k_BT\ll E_F$ 조건(실온 $k_BT\approx25$ meV $\ll E_F\sim3$--$5$ eV) 에서 $\mu_e\approx E_F$ 로 두고
$g(\varepsilon)$ 를 $E_F$ 주변으로 Taylor 전개한다:
$g(\varepsilon)=g(E_F)+g'(E_F)(\varepsilon-E_F)+\tfrac12 g''(E_F)(\varepsilon-E_F)^2+\cdots$.
$\varepsilon-E_F\equiv\eta$ 로 치환해 적분하면, Fermi 함수의 \emph{홀함수 부분}
$[\eta\, g'(E_F)+\cdots]$ 의 $(-\infty,\infty)$ 적분은 소거되고, \emph{짝함수 부분}만 남는다.
핵심 보조 적분: $\int_{-\infty}^{\infty}\eta^2\,\!\left(-\frac{\partial f}{\partial\varepsilon}\right)\dd\eta
=\int_{-\infty}^{\infty}\frac{\eta^2\,e^\eta}{(1+e^\eta)^2}\frac{\dd\eta}{k_BT}$
$=\frac{\pi^2}{3}(k_BT)^2$ (표준 Fermi 적분 $I_2$, 극좌표 잔류정리로 닫힘).
이를 $u_e$ 에 대입하면:
\begin{equation}
u_e\;\approx\;u_e^{(0)}\;+\;\frac{\pi^2}{6}(k_BT)^2\,g(E_F)\;+\;\mathcal O(T^4).
\label{eq:ue_somm}
\end{equation}
(상수 $u_e^{(0)}=\int_0^{E_F}\varepsilon\,g(\varepsilon)\,\dd\varepsilon$ 는 $T$ 에 무관.)
온도 미분으로 전자 비열을 얻고(factor $\times2$ 는 $T$ 미분에서 $6\to3$으로 약분):
\begin{equation}
c_e\;=\;\frac{\dd u_e}{\dd T}\;=\;\frac{\pi^2}{3}\,k_B^2\,T\,g(E_F).
\label{eq:ce_somm}
\end{equation}

\textbf{(c) 중간식 — 전자 엔트로피와 단위 다리.}
Gibbs--Duhem 관계 $\dd S_e=c_e\,\dd T/T$ 를 $0$ 부터 $T$ 까지 적분하면:
\begin{equation}
S_e(T,x)\;=\;\int_0^T\frac{c_e}{T'}\,\dd T'\;=\;\frac{\pi^2}{3}\,k_B^2\,T\,g(E_F,x)
\qquad[\text{단위: }k_B\text{/site}].
\label{eq:Se_sommerfeld}
\end{equation}
\textbf{단위 다리 — 자리당 $k_B$ 에서 몰당 $R$ 으로.}
$g(E_F,x)$ 의 단위를 eV$^{-1}$/site 로 쓰고, 아보가드로 수 $N_A$ 를 곱하면
(Co 자리 1몰 = $N_A$ 개 자리):
\begin{align}
S_e^{\mathrm{mol}}(T,x)
&=N_A\cdot S_e(T,x)
=\frac{\pi^2}{3}\,k_B^2\,N_A\,T\,g(E_F,x)\notag\\
&=\frac{\pi^2}{3}\,k_B R\,T\,g_{\mathrm{mol}}(E_F,x),
\label{eq:Se_mol}
\end{align}
여기서 $R=N_Ak_B$ (기체상수)이고 $g_{\mathrm{mol}}=N_A\,g$ [eV$^{-1}$/mol] 이다.
수치 예 ($g=13$ eV$^{-1}$/site, $T=298.15$ K):
$S_e/k_B=(\pi^2/3)\cdot(k_BT\text{ in eV})\cdot g
=3.290\times0.02569\times13=1.10\;k_B$/site $\approx9.14$ J/(mol$\cdot$K) (×$R/k_B=N_A$).
$S_e\propto T$ — 다른 기여($\Delta S^\config,\,\Delta S^\vib$)와 달리 온도 선형이다.

\textbf{(d) 박스 — 부분몰 전자 엔트로피 (삽입 기준, $\Delta S_\rxn$ 규약 정합).}
삽입반응 기준 Li 1몰 삽입($x\!\uparrow$)당 $S_e$ 변화 — Ch.1 $\Delta S_\rxn$ 의 삽입 방향 슬롯과 일관하는
부분몰 전자 엔트로피(\emph{삽입 기준: $x\!\uparrow$ 로 LCO 가 절연체 쪽으로 이동 $\Rightarrow$
$\partial g/\partial x<0\Rightarrow\Delta S_e<0$}. R2 확정.):
\begin{equation}
\boxed{\;\Delta S_e(x,T)\;=\;\frac{\partial S_e^{\mathrm{mol}}}{\partial x}\bigg|_T\;
=\;\frac{\pi^2}{3}\,k_B\,R\,T\;\frac{\partial g_{\mathrm{mol}}(E_F,x)}{\partial x}\;<\;0\;.}
\label{eq:dSe}
\end{equation}
$\Delta S_e\propto T$ 이므로 $\partial U_j^\cat/\partial T|_e=\Delta S_e/F\propto T$ — \emph{전자 기여만 온도
선형}이며, 흑연의 $\Delta S_{\rxn,j}$ 는 상수(1차 근사)인 것과 대비된다. Ch.2 가역열 해석에서 이 $T$-선형
기여가 분리 관찰 가능한 근거가 된다. 부호(★삽입 기준, $\Delta S_{\rxn,j}^\cat$ 슬롯과 일관):
삽입반응($x\!\uparrow$)에서 LCO 는 절연체 쪽으로 이동해 $g(E_F)\!\downarrow$이므로
$\partial g/\partial x<0$, 따라서 $\Delta S_e<0$. 전자 기여는 $\partial U^\cat/\partial T$ 에
\emph{음의 소량 기여}를 더한다 — 크기는 작아($\approx0.18\,k_B$/atom 부분몰 차, Reynier\cite{reynier2004} tier B)
전체 부호는 config$+$vib 의 양수 $\Delta S_\rxn^\cat$ 가 결정하지만,
단전극 $+0.83$ mV/K (Wang et al.\cite{wang2009} tier B) sanity 가 전자항 부호를 제약하지 않음을 확인한다
— 이는 config$+$vib 지배이며 전자항은 T1 구간 국소 보정이다.
MIT 게이트 없이는 T1 구간의 $T$-선형 미세구조를 재현 불가하다
(config 단독 2상역 재현 실패는 Garikipati 그룹\cite{ml2024}이 1차 계산으로 명시).

\subsection{$g(E_F,x)$ 의 MIT-logistic 게이트 모델}
식~\eqref{eq:dSe} 를 평가하려면 $g(E_F,x)$ 의 $x$-의존성이 필요하다. 1차 문헌에 연속 $g(E_F,x)$ 곡선
정량 데이터가 없는 점(갭 G2)에서, Ch.1 의 ``문헌 초기값 + 우리 데이터 피팅'' 철학을 적용해 MIT 를
logistic 게이트로 근사한다.

\textbf{(a) 물리 근거.} $x=1$ 절연체에서 $x\!\downarrow$ 로 금속이 될 때, MIT 는 $x\approx0.94$--$0.75$
구간에서 발현된다(Reimers·M\'{e}n\'{e}trier 등). $g(E_F)$ 는 절연체 쪽 $\approx0$ 에서 금속 쪽
$\approx g_{\max}$ 로 증가한다.

\textbf{(b) 모델 가정 — logistic 근사.}
\begin{equation}
g(E_F,x)\;\approx\;g_{\max}\,\Big[1-\sigma\!\Big(\frac{x-x_\MIT}{\Delta x_\MIT}\Big)\Big],
\label{eq:g_model}
\end{equation}
$\sigma(z)=1/(1+e^{-z})$ logistic; $x\!\downarrow$ 로 $0\to g_{\max}$ 켜짐. 초기값(피팅 대상):
\begin{itemize}[leftmargin=2.2em,itemsep=1pt]
\item $g_{\max}=13$ e/eV (CoO$_2$ 계산값, Motohashi tier B)
\item $x_\MIT\approx0.85$ (MIT 중심, Ménétrier/Reimers 발현 구간 중간)
\item $\Delta x_\MIT\approx0.05$ (MIT 폭; step 대신 logistic 채택 사유 = 결함 농도 의존 분산 흡수)
\end{itemize}

\textbf{(c) 부분몰 전자 엔트로피의 구체화.} 식~\eqref{eq:g_model} 을 $x$ 로 미분하면
$\partial g/\partial x=g_{\max}\cdot[-\sigma'(\cdots)/\Delta x_\MIT]$($\sigma'=\sigma(1-\sigma)$ 종 모양),
이를 식~\eqref{eq:dSe} 에 넣으면 $\Delta S_e(x,T)$ 는 $x\approx x_\MIT$ 부근에서 \emph{국소 봉우리}를
낸다(크기는 작지만 MIT 게이트로 필수; config 단독으로는 MIT 2상역 $\partial U/\partial T$ 재현 불가\cite{ml2024}).

\textbf{크기 검산.} 식~\eqref{eq:Se_sommerfeld} 에 $g_{\max}=13$ eV$^{-1}$/site, $T=298.15$ K 를 넣으면:
$S_e/k_B=(\pi^2/3)\cdot(k_BT/\mathrm{eV})\cdot g(E_F)
=3.290\times0.02569\times13\approx1.10\;k_B$/site (완전 metal 한계).
부분몰 차(O3 상, $x=0.833$ 비교): $\approx0.18\;k_B$/atom (\cite{reynier2004} tier B) — 같은 자릿수 일치.
작은 값이지만 비-0이며, MIT 게이트 없이는 T1 구간 국소화 불가(분리 재현은 \cite{ml2024} 음의 증거 참조).

\subsection{흑연과의 대비 — 전자항이 있는 이유}
\begin{itemize}[leftmargin=2.2em,itemsep=1pt]
\item \textbf{흑연}: 반금속, $g(E_F)$ 작고 탈리튬화에 거의 불변 → $\Delta S_e\approx0$. $\Delta S_\rxn$ 은
  config($+29$--$-16$ J/(mol\,K)) + vib 로 닫힌다.
\item \textbf{LCO}: $x=1$ 절연체 → $x\!\downarrow$ MIT로 $g(E_F)$ 급증 → $\Delta S_e\ne0$ (MIT 구간 국소 봉우리). 전자 기여 없이는 T1 구간 $\partial U^\cat/\partial T$ 를 재현 불가.
\end{itemize}
이 대비가 v9 통합 챕터의 핵심 교육 가치다 — ``같은 forward 프레임에서 왜 LCO 만 전자항을 추가해야 하는가''가 MIT 유무 하나로 설명된다.
...
\subsection{LCO 양극 $\Delta S_{\rxn,j}^\cat$ 분해 — 이중계산 없는 세 항}
\label{subsec:lco_dS_decomp}
LCO 전이의 반응 엔트로피는 세 기여로 분해된다:
\begin{equation}
\boxed{\;\Delta S_{\rxn,j}^\cat(x,T)\;=\;
\underbrace{\Delta S_j^\config}_{\text{중심 표준값 }\Delta S_j^0}
+\underbrace{\Delta S_j^\vib}_{\text{음의 baseline}}
+\underbrace{\Delta S_e(x,T)}_{\text{MIT 게이트, }\propto T}\;.}
\label{eq:dS_cat_decomp}
\end{equation}
\textbf{이중계산 금지(B).} $\Delta S_j^\config$ 는 \emph{중심 표준값} $\Delta S_j^0$ 이다 — logistic
이 자동으로 $\xi$ 에 따른 배치 엔트로피 변화($R\ln[\xi/(1-\xi)]$)를 포함하지만, 그 평균값을
$\Delta S_j^0$ 에 이미 흡수한 것이 Ch.1 의 처리다. 따라서 $\Delta S_j^\config$ 를 별도로 더하면
이중계산이 된다 — $\Delta S_j^0$ 는 중심 표준값이고, 그 범위 내 분포는 logistic 이 책임진다.

항별 설명:
\begin{itemize}[leftmargin=2.2em,itemsep=1pt]
\item $\Delta S_j^\config$: O3 ΔS 를 지배(총 $\Delta S$ 의 절반 이상, Reynier tier A). 표~\ref{tab:lco_transitions} 의 중심값(피팅 대상).
\item $\Delta S_j^\vib$: 음의 baseline (phonon 기여, 고-$x$). 흑연 $\Delta S_\rxn$ 의 음수 부분과 동형.
\item $\Delta S_e(x,T)$: 식~\eqref{eq:dSe}·\eqref{eq:g_model}. T1 에서만 유의미(MIT 게이트 ON), T2/T3 에서는 $g(E_F)\approx0$ 라 무시 가능(게이트 OFF). $\propto T$ 선형.
\end{itemize}
코드에서는 $\Delta S_{\rxn,j}^\cat$ 를 단일 파라미터 \code{dS\_rxn} 로 입력한다. 완전 분해를 원하면
세 항을 외부에서 합산해 넣는다(구조 변경 0).

\subsection{forward 코드 LCO 일반화 (H)}
Ch.1 의 forward 모델 클래스(\code{func\_ksi\_eq}·\code{func\_U\_j}·\code{func\_dU\_hys} 등)는
전이 파라미터만 교체하면 LCO 에 그대로 적용된다. 이것은 MSMR(Multi-Site Model for Reversible Insertion)
logistic 과 Ch.1 transition-logistic 이 \emph{동형}이기 때문이다:
\begin{align}
\text{MSMR: }&x_j=\frac{X_j}{1+\exp[f(U-U_j^0)/\omega_j]}\label{eq:msmr}\\
\text{Ch.1:  }&\xi_{\eq,j}=\frac{1}{1+\exp[-\sigma_d(V-U_j^d)/w_j]}\label{eq:ch1_logistic}
\end{align}
두 식은 중심 $U_j^0\leftrightarrow U_j^d$, 폭 $\omega_j\leftrightarrow w_j$, 용량 가중 $X_j\leftrightarrow Q_j$
의 일대일 대응으로 같은 구조이다(부호는 측정 전위 방향 규약 차이).

\textbf{LCO 일반화 절차 (구조 변경 0).}
\begin{enumerate}[leftmargin=2.4em,itemsep=2pt]
\item 전이 dict 를 \code{GRAPHITE\_STAGING\_LIT} 대신 \code{LCO\_STAGING\_LIT} (표~\ref{tab:lco_transitions}) 로 교체 — $U_j^\cat$, $\Delta H_{\rxn,j}^\cat$, $\Delta S_{\rxn,j}^\cat$, $\Omega_j^\cat$, $Q_j$, $\Delta H_{a,j}^\cat$.
\item 전자 엔트로피 plug-in: $\Delta S_{\rxn,j}^\cat\leftarrow\Delta S_j^\config+\Delta S_j^\vib+\Delta S_e(x,T)$ 를 외부에서 계산해 \code{dS\_rxn} 에 주입(\S\ref{subsec:lco_dS_decomp}).
\item 나머지 코드(\code{func\_U\_j}·\code{func\_ksi\_eq}·\code{func\_dU\_hys}·꼬리 사슬) 는 \emph{무수정}.
\end{enumerate}
도핑 보정(Al/Mg 비-redox): Co redox 와 전자 엔트로피 항 보존, logistic 폭 $w_j^\cat$ 를 데이터 피팅으로 넓힘.
...
\textbf{LCO 부호 추가 확인.}
\begin{enumerate}[label=\textbf{L\arabic*.},leftmargin=2.6em,itemsep=2pt]
\item \textbf{$\partial U_j^\cat/\partial T=\Delta S_{\rxn,j}^\cat/F>0$ (전체).} 단전극 측정 $+0.83$ mV/K
  가 $\Delta S_{\rxn}^\cat>0$ 의 sanity 를 준다. 식~\eqref{eq:Uj_cat} 의 부호 사슬과 일치. $\checkmark$
\item \textbf{전자 엔트로피 부호(★삽입 기준, R2 확정).}
  삽입반응($x\!\uparrow$)에서 LCO 는 절연체 쪽으로 이동하므로 $g(E_F)\!\downarrow$, 즉
  $\partial g_{\mathrm{mol}}/\partial x<0$. 식~\eqref{eq:dSe}:
  $\Delta S_e=(\pi^2/3)k_B R T\,\partial g_{\mathrm{mol}}/\partial x<0$ (삽입 기준 음수 — $\Delta S_{\rxn,j}^\cat$ 슬롯과 일관).
  $\partial U^\cat/\partial T|_e=\Delta S_e/F<0$ — MIT 구간에서 온도 상승 시 전자항이 $U^\cat$ 를 약간
  낮추는 방향. 크기는 작고($\approx0.18\,k_B$/atom 차, \cite{reynier2004} tier B),
  전체 $\partial U^\cat/\partial T\approx+0.83$ mV/K(\cite{wang2009}) 는 config$+$vib 의 양수 $\Delta S_\rxn^\cat$
  가 지배 — 전자항은 T1 구간 국소 보정(비-0, MIT 게이트 필수). 부호 일관. $\checkmark$
\item \textbf{$\Delta S_e\propto T$ (이중계산 없음).} config 중심값 $\Delta S_j^0$ 와 $\Delta S_e$ 는 별도 항이며, logistic 배치 분산과 $\Delta S_e$ T-선형이 혼동되지 않는다(\S\ref{subsec:lco_dS_decomp} 이중계산 금지 B). $\checkmark$
\end{enumerate}
...
\bibitem{xia2007} H. Xia, L. Lu, Y. S. Meng, G. Ceder, ``Phase Transitions and High-Voltage Electrochemical Behavior of LiCoO$_2$ Thin Films Grown by Pulsed Laser Deposition,'' \emph{J. Electrochem. Soc.} \textbf{154}(4), A337--A342 (2007). DOI: 10.1149/1.2509021. [LCO dQ/dV transition voltages, MIT assignment, tier A.]
\bibitem{reynier2004} Y. Reynier, J. Graetz, T. Swan-Wood, P. Rez, R. Yazami, B. Fultz, ``Entropy of Li intercalation in $\mathrm{Li}_x\mathrm{CoO_2}$,'' \emph{J. Electrochem. Soc.} \textbf{151}, A422 (2004). DOI: 10.1149/1.1646152. [$\Delta S(x)$ LCO three-way decomposition; electronic $\approx0.18\,k_B$/atom tier B.]
\bibitem{wang2009} C. Wang, Y. Reynier \emph{et al.}, ``Effect of entropy change of Li intercalation in cathodes and anodes on Li-ion battery thermal management,'' \emph{J. Power Sources} (2009). [PII: S0378775309021119; single-electrode $\partial\phi_\cat/\partial T\approx+0.83$ mV/K for LCO, tier B.]
\bibitem{ml2024} (Garikipati group) ``Bridging scales with Machine Learning: from first-principles statistical mechanics to continuum phase field in LixCoO2,'' \emph{J. Mech. Phys. Solids} \textbf{190}, 105727 (2024). arXiv:2302.08991. [Config-only stat-mech fails to reproduce MIT two-phase plateau at 3.9 V, tier A.]
\bibitem{motohashi2009} T. Motohashi, T. Ono, Y. Sugimoto, Y. Masubuchi, S. Kikkawa, R. Kanno, M. Karppinen, H. Yamauchi, ``Electronic phase diagram of the layered cobalt oxide system $\mathrm{Li}_x\mathrm{CoO_2}$,'' \emph{Phys. Rev. B} \textbf{80}, 165114 (2009). DOI: 10.1103/PhysRevB.80.165114. [$g(E_F)$ and order-disorder, tier A/B.]
\bibitem{menetrier1999} M. M\'{e}n\'{e}trier, I. Saadoune, S. Levasseur, C. Delmas, ``The insulator-metal transition upon lithium depletion from $\mathrm{LiCoO_2}$: Electronic properties and $^7$Li NMR study,'' \emph{J. Mater. Chem.} \textbf{9}, 1135 (1999). DOI: 10.1039/a900016j. [MIT occurrence x range.]
\bibitem{reimers1992} J. N. Reimers, J. R. Dahn, ``Electrochemical and in situ x-ray diffraction studies of lithium intercalation in $\mathrm{Li}_x\mathrm{CoO_2}$,'' \emph{J. Electrochem. Soc.} \textbf{139}, 2091 (1992). DOI: 10.1149/1.2221184. [LCO phase diagram, MIT 2-phase region.]
\bibitem{msmr2017} D. A. Verbrugge, D. R. Baker, B. Koch, X. Xiao, W. Gu, ``Thermodynamic model for substitution within mixed insertion electrode materials,'' \emph{J. Electrochem. Soc.} \textbf{164}, E3243 (2017). [MSMR logistic formulation.]
\end{thebibliography}
