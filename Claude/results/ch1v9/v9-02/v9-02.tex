...
order--disorder 전이(~4.05/4.17 V)와 metal--insulator 천이(MIT, ~3.90 V)를 거치며, 각 전위에서
...
\caption{LCO 양극 전이 초기값 \code{LCO\_STAGING\_LIT}(3건, 코인 하프셀 vs Li, tier A = Xia; $\Delta S$ tier A/B).
...
\toprule
전이 & $U_j^\cat$ [V vs Li] & $x$ 범위 & $\Delta S_\rxn^\cat$ 초기값 & 전자항 & 성격 \\
\midrule
T1 (MIT)            & $\approx3.90$ & $x\!\approx\!0.94$--$0.75$ & config$+\Delta S_e$ & \textbf{ON} & insulator$\to$metal \\
T2 (order--dis.\ a) & $\approx4.05$ & $x\!\approx\!0.55$         & config 주도 & off & hex$\to$monoclinic \\
T3 (order--dis.\ b) & $\approx4.17$--$4.20$ & $x\!\approx\!0.48$ & config 주도 & off & monoclinic$\to$hex \\
\bottomrule
\end{tabular}
\end{table}
...
기준 $\partial\phi_\cat/\partial T\approx+0.83$ mV/K (전체 부호 sanity, 양수 확인). 이하 절은 흑연 사례를
...
\textbf{(b) 연산 — 전자 비열과 엔트로피.}
단위 자리당 전자 내부 에너지는 상태밀도 $g(\varepsilon)$ 로 가중한 적분이다:
\begin{equation}
u_e\;=\;\int_0^\infty \varepsilon\,g(\varepsilon)\,f(\varepsilon)\,\dd\varepsilon.
\label{eq:ue_int}
\end{equation}
$k_BT\ll E_F$ 조건에서 이를 $\mu_e\approx E_F$ 주변 Sommerfeld 전개하면 — $g(\varepsilon)$ 를 $E_F$ 에서
$g(E_F)+g'(E_F)(\varepsilon-E_F)+\cdots$ 로 전개하고, Fermi 함수의 홀함수 적분은 소거, 짝함수만 남아
$u_e\approx u_e^{(0)}+\frac{\pi^2}{6}(k_BT)^2\,g(E_F)+\mathcal O(T^4)$. 온도 미분(비열)은
$c_e=\dd u_e/\dd T=(\pi^2/3)k_B^2T\,g(E_F)$, Gibbs--Duhem 관계($\dd S_e=c_e\,\dd T/T$)로 적분하면

\textbf{(c) 중간식 — 전자 엔트로피.}
\begin{equation}
S_e(T,x)\;=\;\int_0^T\frac{c_e}{T'}\,\dd T'\;=\;\frac{\pi^2}{3}k_B^2T\,g(E_F,x).
\label{eq:Se_sommerfeld}
\end{equation}
이것이 Sommerfeld 전개의 닫힌 결과다($g(E_F,x)$ = 탈리튬화율 $x$ 에서의 Fermi 준위 상태밀도, 자리당 단위).
$S_e\propto T$ — 다른 기여($\Delta S^\config,\,\Delta S^\vib$)와 달리 온도 선형이다.

\textbf{(d) 박스 — 부분몰 전자 엔트로피.}
삽입반응 기준 Li 1몰 삽입($x\!\uparrow$)당 $S_e$ 변화 — Ch.1 $\Delta S_\rxn$ 규약과 일치하는 부분몰 전자 엔트로피:
\begin{equation}
\boxed{\;\Delta S_e(x,T)\;=\;\frac{\partial S_e}{\partial x}\bigg|_T\;=\;\frac{\pi^2}{3}k_B^2T\,\frac{\partial g(E_F,x)}{\partial x}\;.}
\label{eq:dSe}
\end{equation}
...
기여가 분리 관찰 가능한 근거가 된다. 부호: 삽입반응 기준에서 $\partial g/\partial x<0$ 이라 $\Delta S_e<0$,
곧 전자 기여는 $\partial U^\cat/\partial T$ 에 \emph{음의 소량 기여}를 더한다 — 크기는 작아 전체 부호는
config$+$vib 의 양수 $\Delta S_\rxn^\cat$ 가 결정하지만, MIT 게이트 없이는 T1 구간의 $T$-선형 미세구조를
재현 불가하다(config 단독 2상역 실패).
...
낸다(크기는 작지만 MIT 게이트로 필수; config 단독으로는 MIT 2상역 $\partial U/\partial T$ 재현 불가).

\textbf{크기 검산.} $S_e/k_B=(\pi^2/3)(k_BT/\text{eV})\,g(E_F)=3.29\times0.0259\,\text{eV}\times13\,\text{eV}^{-1}\approx1.1\,k_B$/atom (완전 metal 한계). O3 상 부분 기여 부분몰 차 $\approx0.18\,k_B$/atom (Reynier tier B) — 같은 자릿수 일치. 작은 값이지만 비-0이며 MIT 게이트로 국소화.
...
\item \textbf{전자 엔트로피 부호.} 삽입반응(Li 들어감, $x\!\uparrow$)에서 LCO 는 절연체 쪽으로 이동해
  $g(E_F)\!\downarrow$, 따라서 $\partial g/\partial x>0$... 아니다 — $x\!\uparrow$ 로 금속→절연체이므로
  $\partial g/\partial x<0$($x\!\uparrow$ 하면 $g$ 감소). 식~\eqref{eq:dSe}: $\Delta S_e=\frac{\pi^2}{3}k_B^2T\,\partial g/\partial x<0$.
  $\partial U^\cat/\partial T|_e=\Delta S_e/F<0$ — MIT 구간에서 온도가 올라갈수록 전자항이 $U^\cat$ 를 약간
  낮추는 방향. 그러나 크기가 작고(0.18 k\_B/atom), 전체 $\partial U^\cat/\partial T\approx+0.83$ mV/K 의
  지배 성분은 config$+$vib 의 양수 $\Delta S_\rxn^\cat$이다 — 전자항은 T1 구간 국소 보정(비-0 이며 MIT 게이트 필수). 부호 일관(전자항 부호 정직 기술). $\checkmark$
...
\bibitem{xia2006} Q. Xia, X. H. Zhao, M. Xu, Z. D. Ding, J. Z. Liu, L. W. Chen, D. G. Ivey, W. F. Wei, ``A lithium-rich layered oxide cathode with enhanced structural stability,'' \emph{J. Power Sources} (2006). [LCO voltage profile tier A.]
\bibitem{reynier2004} Y. Reynier, R. Yazami, B. Fultz, ``Thermodynamics of lithium intercalation in $\mathrm{Li}_x\mathrm{CoO_2}$: Entropy analysis,'' \emph{J. Electrochem. Soc.} \textbf{151}, A422 (2004). DOI: 10.1149/1.1646152. [ΔS(x) LCO tier A/B.]
