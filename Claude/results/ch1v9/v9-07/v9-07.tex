...
\hypersetup{colorlinks=true, linkcolor=blue, urlcolor=blue, citecolor=blue,
  pdftitle={흑연 음극 dQ/dV 이론: LCO 하프셀 보완 통합 — Chapter 1 (v9-07)}}
\pagestyle{fancy}\fancyhf{}
\lhead{Ch.1 $dQ/dV$ 이론 — v9-07 LCO 보완}\rhead{\thepage/\pageref{LastPage}}
...
\title{\textbf{\LARGE Chapter 1}\\[0.35em]\Large 흑연 음극 $dQ/dV$ 이론: 한 곡선을 그리는 코드 진행을 따라가는 수식-구동판 — LCO 하프셀 보완 통합(v9-07)}
\author{}
\date{}
...
\cite{reimers1992lco,menetrier1999lco,motohashi2009lco,xia2007lco,reynier2004lco}.}
\label{tab:lco_transitions}
\begin{tabular}{>{\raggedright\arraybackslash}p{0.105\textwidth}
...
단전극 $d\phi/dT\approx+0.83$ mV/K 는 $\Delta S_{\rxn}^\cat\approx F(0.83\,\mathrm{mV/K})\approx80$
J/(mol\,K) 규모의 하프셀 sanity check 로만 쓰고, 전셀 부호전환과 섞지 않는다\cite{wang2009thermal}.
...
$g(E_F,x)$ 가 켜지는 metal--insulator transition(MIT)을 지난다
\cite{reimers1992lco,menetrier1999lco,motohashi2009lco,xia2007lco}. 그러면 이온 자리 점유분포
$\xi_\eq$ 만이 아니라 전도 전자의 Fermi--Dirac 점유분포가 엔트로피에 들어간다. Configurational 통계역학만으로
MIT 2상역을 재현하지 못한다는 계산 결과는 이 전자항을 독립 plug-in 으로 두는 근거다\cite{garikipati2024ml}.
...
\begin{equation}
\boxed{\;\Delta S_e^{\mathrm{ins}}(x,T)=\frac{\partial S_e}{\partial x}\Big|_T
=\frac{\pi^2}{3}k_B^2T\,\frac{\partial g(E_F,x)}{\partial x}\;.}
\label{eq:dSe}
\end{equation}
여기서 위 첨자 ``ins'' 는 삽입 기준이다. 이 기준은 표~\ref{tab:staging} 의 $\Delta S_{\rxn,j}$ 슬롯처럼
Li 삽입 반응 엔트로피를 저장한다는 뜻이고, LCO 에서는 $x\uparrow$ 삽입 시 metal 쪽에서 insulator 쪽으로 가며
$g(E_F)$ 와 $S_e$ 가 줄어든다. 따라서
\[
\boxed{\;\Delta S_e^{\mathrm{ins}}=\partial S_e/\partial x<0\;}
\]
가 저장 부호다. 반대로 탈리튬화($x\downarrow$)는 전자 상태밀도가 켜지는 방향이라 Reynier 계열의
$\sim+0.18\,k_B$/atom 규모 전자 엔트로피 방출로 읽히며\cite{reynier2004lco}, 삽입 기준 저장값과 부호가 반대다.
최종 사전의 \code{dS\_rxn} 은 반드시 $\partial U_j^\cat/\partial T=\Delta S_{\rxn,j}^\cat/F$ 로 검산한다. 단전극
$d\phi/dT\approx+0.83$ mV/K 는 config+vib 지배의 총합 sanity check 이므로, 전자항 단독 부호를 뒤집는 제약으로 쓰지 않는다.
...
\bibitem{reimers1992lco} J. N. Reimers and J. R. Dahn, ``Electrochemical and in situ X-ray diffraction studies of lithium intercalation in Li$_x$CoO$_2$,'' \emph{J. Electrochem. Soc.} \textbf{139}, 2091--2097 (1992). DOI: 10.1149/1.2221184.
\bibitem{menetrier1999lco} M. Ménétrier, I. Saadoune, S. Levasseur, and C. Delmas, ``The insulator--metal transition upon lithium deintercalation from LiCoO$_2$: electronic properties and $^7$Li NMR study,'' \emph{J. Mater. Chem.} \textbf{9}, 1135--1140 (1999). DOI: 10.1039/a900016j.
\bibitem{motohashi2009lco} T. Motohashi, T. Ono, Y. Sugimoto, Y. Masubuchi, S. Kikkawa, R. Kanno, M. Karppinen, and H. Yamauchi, ``Electronic phase diagram of the layered cobalt oxide system, Li$_x$CoO$_2$,'' \emph{Phys. Rev. B} \textbf{80}, 165114 (2009). DOI: 10.1103/PhysRevB.80.165114; arXiv:0909.3556.
\bibitem{xia2007lco} H. Xia, L. Lu, Y. S. Meng, and G. Ceder, ``Phase transitions and high-voltage electrochemical behavior of LiCoO$_2$ thin films grown by PLD,'' \emph{J. Electrochem. Soc.} \textbf{154}(4), A337--A342 (2007). DOI: 10.1149/1.2509021.
\bibitem{reynier2004lco} Y. Reynier, J. Graetz, T. Swan-Wood, P. Rez, R. Yazami, and B. Fultz, ``Entropy of Li intercalation in Li$_x$CoO$_2$,'' \emph{J. Electrochem. Soc.} \textbf{151}, A422 (2004); companion article in \emph{Phys. Rev. B} \textbf{70}, 174304 (2004). DOI: 10.1103/PhysRevB.70.174304.
\bibitem{wang2009thermal} Q. Wang, Y. Reynier \emph{et al.}, ``Effect of entropy change of Li intercalation in cathodes and anodes on Li-ion battery thermal management,'' \emph{J. Power Sources} (2009), PII: S0378775309021119.
\bibitem{garikipati2024ml} K. Garikipati group, ``Bridging scales with machine learning: from first-principles statistical mechanics to continuum phase field for order--disorder transitions in Li$_x$CoO$_2$,'' \emph{J. Mech. Phys. Solids} \textbf{190}, 105727 (2024). DOI: 10.1016/j.jmps.2024.105727; arXiv:2302.08991.
\end{thebibliography}
