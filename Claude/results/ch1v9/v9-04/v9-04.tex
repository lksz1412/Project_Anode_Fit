...
철학이며($U_j$ tier A=Xia; $\Delta S$ tier=Reynier\cite{reynier2004}/Motohashi\cite{motohashi2009}), 값의 신뢰 정도(tier)를
함께 표기해 허위 정밀을 피한다.
...
\textbf{T2} (order--dis.\ a) & $\sim4.05$        & $\approx0.55$  & hex$\to$monoclinic 정렬   & $\config$ 주도($0.47$ J/K$\cdot$mol@$x\tfrac12$) \\
\textbf{T3} (order--dis.\ b) & $\sim4.17$--$4.20$ & $\approx0.48$  & monoclinic$\to$hex       & $\config$($1.49$ J/K$\cdot$mol@$x\tfrac23$); T2 와 한 쌍 \\
\midrule
...
$\Delta S_{\rxn,j}$ 초기값은 양극에서 \emph{양수} 경향이라($\config$ 와 전자 항이 양으로 더해짐, \S\ref{sec:sum} 의 분해;
단전극 $\dd\phi/\dd T\approx+0.83$ mV/K[tier B]와 정합), 식~\eqref{eq:dUdTcat} 로 $\partial U_j/\partial T>0$ — 곧 LCO
전이 중심은 온도가 오르면 \emph{올라간다}. 흑연은 stage $4\!\to\!3$($\Delta S_\rxn=+29$)이 양, stage $2\!\to\!1$
($\Delta S_\rxn=-16$)이 음으로 섞여 있던 데 비해, LCO 핵심 전이는 전자 항이 더해져 양 쪽으로 쏠린다. 이 한 부호 차이가
...
\label{eq:dSegate}
\end{equation}
— MIT 부근 $x\approx x_\MIT$ 에 국소화된 종 모양 기여다(T1 전이에서만 켜지고, T2$\cdot$T3 에서는 $0$). \textbf{부호 부기(명시).}
정의상 $\Delta S_e\equiv\partial S_e/\partial x<0$ 이다(식 박스의 음부호; $x$ 를 \emph{올리면}, 곧 리튬화하면 $g$ 가
줄어 $S_e$ 감소). 그런데 forward 모델의 전이는 \emph{탈리튬화}($x\downarrow$, $\xi:0\to1$) 방향이므로, 이 전이에
들어가는 ``Li 1몰 \emph{제거}당'' 엔트로피 변화는 부호가 뒤집힌 $\Delta S_{\rxn}^{(e)}=-\partial S_e/\partial x>0$ 이다 —
곧 탈리튬화로 금속 전자가 켜지며 $S_e$ 가 \emph{늘어} 양극 $\partial U_j/\partial T=\Delta S_{\rxn,j}/F$ 에 $+$ 로 더해진다.
(\S\ref{sec:sum} 식~\eqref{eq:dScat} 의 $\Delta S_e(x,T)$ 항은 이 전이 방향 부호로 $\Delta S_{\rxn,j}^\cat$ 에 들어간다 —
정의 $\partial/\partial x$ 와 전이 방향 $-\partial/\partial x$ 의 한 번 부호 차이를 \S\ref{sec:notation} 의 $\theta\!=\!1-\xi$
부호 약속과 같은 자리에서 흡수한다.)
...
$\Delta S_{\rxn}$ 에 \emph{국소} 양봉우리를 더해야 MIT 구간의 $\partial U/\partial T$ 부호$\cdot$크기와 dQ/dV 미세 비대칭이
맞는다. 작은 항이 정성을 바꾸는 전형이다.
...
게이트가 켜져 $\Delta S_{\rxn,\text{T1}}$ 에 국소 양봉우리를 더한다. 유일하게 $T$ 함수($\propto T$)라 배열 $T_\work$
에서 격자별로 평가한다.
\end{itemize}
...
$\partial g/\partial x<0$(탈리튬화로 $g$ 증가)라 Li 제거당 $\Delta S_e>0$ 으로 양극 $\partial U/\partial T$ 에 $+$ 기여,
MIT($x\approx x_\MIT$)에만 국소(T2$\cdot$T3 에서 $0$), 유일하게 $\propto T$(상수 $\Delta S_\rxn$ 항과 대비). 흑연은
$\partial g/\partial x\!\approx\!0$ 이라 이 항 $\to0$(전극 대비 일관). $\checkmark$
...
\bibitem{reynier2004} Y. Reynier, R. Yazami, B. Fultz, ``The entropy and enthalpy of lithium intercalation into graphite and Li$_x$CoO$_2$,'' \emph{J. Power Sources} \textbf{119--121}, 850 (2003). DOI: 10.1016/S0378-7753(03)00285-4.
