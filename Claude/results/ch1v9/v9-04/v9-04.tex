...
\hypersetup{colorlinks=true, linkcolor=blue, urlcolor=blue, citecolor=blue,
  pdftitle={하프셀 dQ/dV 이론 (흑연 음극 + LCO 양극): 코드 진행을 따라가는 수식-구동 (유도 확장) — Chapter 1 (v9)}}
\pagestyle{fancy}\fancyhf{}
\lhead{하프셀 $dQ/dV$ 이론 (흑연 + LCO) — Ch.1 (v9, 코드 진행 + 유도)}\rhead{\thepage/\pageref{LastPage}}
...
\newcommand{\MIT}{\mathrm{MIT}}
\newcommand{\code}[1]{\texttt{#1}}

\renewcommand{\theequation}{1.\arabic{equation}}
\renewcommand{\thesection}{1.\arabic{section}}
\title{\textbf{\LARGE Chapter 1}\\[0.35em]\Large 하프셀 $dQ/dV$ 이론: 한 곡선을 그리는 코드 진행을 따라가는 수식-구동판 — 흑연 음극에서 LCO 양극까지 (유도 확장)}
\author{}
\date{}
...
\textbf{두 전극을 한 프레임으로 — 이 장의 통합 범위.} 위 forward 모델은 ``동등한 자리에 리튬이 차고 비는'' 격자기체
상전이를 보는 한 \emph{전극에 무관}하다 — 곧 식의 골격(평형 중심 $U_j(T)$$\cdot$logistic 점유$\cdot$$\partial U_j/\partial T$
$\cdot$히스테리시스$\cdot$꼬리)은 흑연 음극에만 묶이지 않는다. 본 장은 같은 골격을 \emph{양극} 대표 물질
$\mathrm{Li}_x\mathrm{CoO_2}$(LCO)로 확장한다 — 코인 하프셀(LCO vs Li/Li$^+$) 범위에서. 전개 순서는 줄곧
\emph{일반(전극 불문) $\to$ 흑연 사례 $\to$ LCO 사례}이며, 흑연 서술$\cdot$식$\cdot$부호$\cdot$그림은 그대로 두고
(이 forward 모델은 이미 전극 일반으로 세워져 있다), 전극이 바꾸는 것은 (i) 전이 파라미터(전이 전위$\cdot$$\Delta S$
$\cdot$$\Omega$)와 (ii) 한 개의 \emph{고유 항}뿐이다. 그 고유 항이 LCO 의 \textbf{전자(electronic) 엔트로피}다 —
흑연엔 없고(반금속$\cdot$전도성 변화 미미) LCO 엔 있는(탈리튬화 시 절연체$\to$금속 천이) 항으로, \S\ref{sec:eentropy}
가 이를 Fermi--Dirac 분포에서 새로 유도해 같은 합산식에 plug-in 한다. 이 ``한 항의 차이''가 두 전극을 한 장에서
설명하는 교육적 핵심이다. 전셀(음극$+$양극) 합성은 본 장 범위 밖이다(후속).
...
\subsection{양극 사례 — LCO 전이와 부호 규약}\label{sub:lcomap}
같은 N0 매핑이 양극에도 그대로 선다 — 바뀌는 것은 전이 리스트와 전위 \emph{대}뿐이다. $\mathrm{Li}_x\mathrm{CoO_2}$(LCO)
하프셀의 코어 식별자$\cdot$방향 부호$\cdot$폭$\cdot$용량 가중은 위 표와 동일하며(전극 일반 forward 모델), 전극별로 갈리는
것은 (i) 전이 인덱스가 가리키는 물리(흑연 staging $\to$ LCO 의 절연체$\to$금속 천이$\cdot$order--disorder 정렬)와
(ii) \S\ref{sec:eentropy} 의 전자 엔트로피 항이다.

\textbf{양극 부호 규약(흑연과 1:1).} LCO 도 하프셀 음극 전위가 아니라 \emph{양극} 반쪽전지 전위(vs Li/Li$^+$)를 쓰며,
값의 \emph{대}가 흑연($\sim0.1$ V)과 달리 높다($\sim3.9$--$4.2$ V) — Co$^{3+}/$Co$^{4+}$ redox 가 흑연의 Li 삽입보다
훨씬 깊은 자리이기 때문이다. 방향 규약은 동일하다: 방전($\sigma_d=+1$)은 \emph{리튬화}(전셀에서 양극이 받는 방향)이나,
본 forward 모델의 ``진행률 $\xi$''는 전극과 무관하게 \emph{전이의 한 방향 진행}으로 정의되므로(흑연은 탈리튬화를 $\xi:0\to1$
로 잡았다), LCO 도 같은 부호 기계 $\xi_\eq=\mathrm{logistic}[\sigma_d(V-U)/w]$ 를 쓴다. 곧 ``$V$ 를 올리면 더 탈리튬화''
($V\!\uparrow\Rightarrow\xi_\eq\!\uparrow$, 방전 규약)가 양극에서도 성립한다 — 전위를 올릴수록 Li 가 더 빠지는 충전 쪽
진행이다. 온도 민감도의 부호 사슬 $\partial U_j/\partial T=\Delta S_{\rxn,j}/F$ 도 \emph{식 그대로} 양극에 적용되며
(\S\ref{sec:center} 의 \S\ref{sub:lcocenter}), 양극에서 $\Delta S_{\rxn,j}$ 가 양수(아래 표)면 $U_j$ 가 온도에 따라
올라간다. 이 부호 사슬이 흑연과 한 글자도 다르지 않은 것이 통합 프레임의 출발점이다.

흑연 staging 4건과 대칭으로, LCO 하프셀(코인, $\lesssim4.2$--$4.5$ V)의 핵심 전이 3건의 초기값을 \code{LCO\_STAGING\_LIT}
로 둔다(표~\ref{tab:lcostaging}). 흑연 \code{GRAPHITE\_STAGING\_LIT} 와 같은 ``문헌값$=$초기값, 데이터로 피팅''
철학이며($U_j$ tier A=Xia\cite{xia2007}; $\Delta S$ tier=Reynier\cite{reynier2004}/Motohashi\cite{motohashi2009}), 값의 신뢰 정도(tier)를
함께 표기해 허위 정밀을 피한다.

\begin{table}[t]
\centering
\renewcommand{\arraystretch}{1.3}
\caption{양극 전이 초기값 \code{LCO\_STAGING\_LIT}(3건) — $\mathrm{Li}_x\mathrm{CoO_2}$ 하프셀(vs Li), 출발점$\cdot$피팅
override. 흑연 표~\ref{tab:staging} 와 대칭. $U_j$ tier A(Xia); $\Delta S_{\rxn,j}$ 는 \S\ref{sec:sum} 의 분해
($\config+\vib+$전자)로 닫히는 초기값(tier=Reynier/Motohashi). T1 은 전자 엔트로피 게이트가 켜지는 전이다(\S\ref{sec:eentropy}).}
\label{tab:lcostaging}
\begin{tabular}{@{}l c c p{0.20\textwidth} p{0.30\textwidth}@{}}
\toprule
전이 & $U_j$ [V vs Li] & $x$ 범위 & 성격 & 비고(엔트로피 성분) \\
\midrule
\textbf{T1} (MIT)            & $\sim3.90$        & $0.94$--$0.75$ & insulator$\to$metal      & $\config+$\textbf{전자 게이트 ON}(\S\ref{sec:eentropy}) \\
\textbf{T2} (order--dis.\ a) & $\sim4.05$        & $\approx0.55$  & hex$\to$mono.\ 정렬       & $\config$ 주도($0.47$ J/K$\cdot$mol@$x\tfrac12$) \\
\textbf{T3} (order--dis.\ b) & $\sim4.17$--$4.20$ & $\approx0.48$  & mono.$\to$hex            & $\config$($1.49$ J/K$\cdot$mol@$x\tfrac23$); T2 와 한 쌍 \\
\midrule
\textit{(T4 O3$\to$H1-3)}    & $\sim4.55$        & $\approx0.2$   & 고전압 상전이             & 하프셀 $\lesssim4.2$--$4.5$ V 면 \emph{범위 밖}(옵션) \\
\bottomrule
\end{tabular}
\end{table}

표~\ref{tab:lcostaging} 의 T1--T3 이 흑연 4 전이와 같은 자리에서 forward 루프를 돈다(전이당 $U_j$$\cdot$$w_j$$\cdot$$Q_j$
$\cdot$$\Omega_j$). 단 한 가지가 더 붙는다 — T1 의 절연체$\to$금속 천이에서 켜지는 전자 엔트로피 항으로, 이는
\S\ref{sec:eentropy} 가 새 항으로 도입한다. 단전극 $\dd\phi/\dd T\approx+0.83$ mV/K(${\approx}+80$ J/(mol$\cdot$K))[tier B]가
양극 전체 부호의 sanity(양수)다 — 양극 $\Delta S$ 가 평균적으로 양수임과 정합한다.
...
\subsection{양극 사례 — LCO 중심 $U_j(T)$ 와 온도 부호}\label{sub:lcocenter}
식~\eqref{eq:Uj} 의 $U_j(T)=(-\Delta H_{\rxn,j}+T\Delta S_{\rxn,j})/F$ 는 \emph{전극에 무관}하게 유도됐다 — 식~\eqref{eq:eqcond}
의 평형 조건 $\Delta G_j=-sFU_j$ 와 $\Delta G_j=\Delta H_{\rxn,j}-T\Delta S_{\rxn,j}$ 만 썼고, 어디에도 ``흑연''이 들어가지
않았다. 따라서 LCO 양극도 \emph{같은 식}으로 중심을 얻는다 — 바뀌는 것은 입력 $(\Delta H_{\rxn,j},\Delta S_{\rxn,j})$ 의
값과, 그것이 환산하는 전위 \emph{대}뿐이다. 양극에서 $U_j\sim3.9$--$4.2$ V 이려면 식~\eqref{eq:Uj} 의 분자
$-\Delta H_{\rxn,j}+T\Delta S_{\rxn,j}$ 가 $\sim4\,F\approx3.9\times10^5$ J/mol 규모로 양(陽)이어야 하므로, LCO 전이의
삽입 반응 엔탈피는 흑연($\Delta H_\rxn\sim-13$ kJ/mol)보다 훨씬 깊은 발열($\Delta H_\rxn\sim-4\times10^5$ J/mol 규모)이다
— Co$^{3+}/$Co$^{4+}$ redox 가 그만큼 깊은 자리라는 물리와 정합한다(절대값 $\Delta H_f$ 의 anchor 는 OCV plateau 로
보정 — \S\ref{sec:sum} 의 갭 G5).

\textbf{온도 부호 사슬 — 흑연과 1:1.} 온도 의존도 식 그대로다: 식~\eqref{eq:Uj} 를 $T$ 로 미분하면
\begin{equation}
\frac{\partial U_j}{\partial T}=\frac{\Delta S_{\rxn,j}}{F}
\qquad(\text{전극 무관 — Gibbs 항등식 }\partial\Delta G/\partial T=-\Delta S\text{ 와 }\Delta G=-FU\text{ 를 잇는다}),
\label{eq:dUdTcat}
\end{equation}
이며 흑연에서 쓴 것과 한 글자도 다르지 않다(흑연은 \S\ref{sec:center} 본문이 이 미분을 이미 적었다 — 여기서는 그 \emph{같은}
관계가 양극에도 성립함을 명시할 뿐, 재유도가 아니다). 부호의 결과만 전극별로 갈린다 — LCO T1(MIT)$\cdot$T2$\cdot$T3 의
$\Delta S_{\rxn,j}$ 초기값은 양극에서 \emph{양수} 경향인데, 그 양수 우세는 $\config$ 항이 끌고 간다(O3 상 $\Delta S$ 의
절반 이상을 config 가 지배 — Reynier tier A; 단전극 $\dd\phi/\dd T\approx+0.83$ mV/K[tier B]와 정합). 전자 항 $\Delta S_e$
자체는 삽입 기준 \emph{음수}($\partial S_e/\partial x<0$, \S\ref{sec:eentropy})로 T1 에 작은 국소 보정을 줄 뿐 양수 우세를
만드는 주역이 아니다. 어느 쪽이든 식~\eqref{eq:dUdTcat} 로 $\partial U_j/\partial T>0$ — 곧 LCO 전이 중심은 온도가 오르면
\emph{올라간다}. 흑연은 stage $4\!\to\!3$($\Delta S_\rxn=+29$)이 양, stage $2\!\to\!1$($\Delta S_\rxn=-16$)이 음으로 섞여
있던 데 비해, LCO 핵심 전이는 config 가 커서 양 쪽으로 쏠린다(전자 항은 T1 에 음의 국소 보정). 이 부호 경향 차이가
다온도 dQ/dV 피팅에서 양극 peak 가 흑연과 \emph{반대로} 이동할 수 있는 이유다.

\textbf{전자 항의 자리 — 미리보기.} 식~\eqref{eq:dUdTcat} 의 $\Delta S_{\rxn,j}$ 는 \S\ref{sec:sum} 에서
$\Delta S_{\rxn,j}^\cat=\Delta S^\config_j+\Delta S^\vib_j+\Delta S_e(x,T)$ 로 분해되는데, 마지막 항 $\Delta S_e$ 가
LCO 고유의 \emph{전자} 엔트로피다 — 흑연엔 없는 항으로, \S\ref{sec:eentropy} 가 Fermi--Dirac 분포에서 새로 유도한다.
그 항이 \emph{온도에 선형}($\Delta S_e\propto T$)이라, T1 부근에서는 $\partial U_j/\partial T$ 자체가 온도에 의존하는
(곧 $U_j(T)$ 가 $T$ 의 1차가 아닌) 미세 비선형이 생긴다 — 이 점은 \S\ref{sec:eentropy} 에서 정량화한다.

\begin{codebox}
\textbf{N2 (양극).} 같은 \code{func\_U\_j(T\_work, dH\_rxn, dS\_rxn)} 호출, 입력만 \code{LCO\_STAGING\_LIT}
(표~\ref{tab:lcostaging})로 교체. 코드 변경 0 — 전이 dict 의 $(\Delta H_\rxn,\Delta S_\rxn)$ 값(또는 \code{'U'} 폴백)이
양극 대로 바뀔 뿐이다. 전자 항은 N2 가 아니라 \S\ref{sec:eentropy} 에서 $\Delta S_{\rxn,j}$ 에 더해 plug-in 한다.
\end{codebox}
...
\subsection{양극 사례 — LCO order--disorder 와 2상역 $\Omega$}\label{sub:lcohys}
\S\ref{sec:hys} 의 정규용액 틀($\mu(\theta)$ 의 상호작용 몫 $\Omega(1-2\theta)$, spinodal $\Omega>2RT$, gap
식~\eqref{eq:dUhys})은 ``동등한 자리의 점유 이웃 상호작용''만 가정했을 뿐 전극 종을 가정하지 않았다 — 따라서 LCO 의
상전이도 \emph{같은} $\Omega$ 언어로 들어온다. 바뀌는 것은 $\Omega_j$ 가 가리키는 물리적 정렬 기제다.
\begin{itemize}[leftmargin=1.4em,itemsep=1pt,topsep=1pt]
\item \textbf{T2$\cdot$T3 — Li/공공(vacancy) order--disorder.} $x\approx0.5$ 부근에서 LCO 는 hexagonal$\leftrightarrow$
monoclinic 초격자 정렬(Li 와 공공이 줄무늬로 분리)을 거친다. 이 정렬은 ``동종 이웃 인력''($\Omega_j>0$)이 한 조성
근방에서 2상역을 여는 식~\eqref{eq:gpp}$\cdot$\eqref{eq:spinodal} 와 정확히 같은 구조다 — $\Omega_j>2RT$ 면 spinodal 이
열려 좁은 2상 plateau 와 히스테리시스를 낳고, 그 gap 이 식~\eqref{eq:dUhys} 로 닫힌다. T2$\cdot$T3 은 한 쌍의
order--disorder 천이라 dQ/dV 에서 인접 봉우리 쌍으로 나타난다.
\item \textbf{T1 — 절연체$\to$금속(MIT) 2상역.} $x\approx0.75$--$0.94$ 의 천이는 \emph{전자적} 2상 공존(절연 상 $+$
금속 상)으로, 조성-자유에너지에도 비단조성(공통 접선)을 준다 — 곧 \emph{형식상으로는} 같은 $\Omega>2RT$ 격자기체
2상역으로 모형화된다(코드 구조 불변). 단 그 2상역의 \emph{엔트로피}에는 흑연에 없는 전자 항이 더해지는데(아래
\S\ref{sec:eentropy}), 정렬(히스/분기)의 \emph{형태}는 여전히 식~\eqref{eq:dUhys}$\cdot$\eqref{eq:Ubranch} 가 담는다.
\end{itemize}
요컨대 N3 코드는 한 줄도 바뀌지 않는다 — LCO 전이 dict 에 $\Omega_j$(2상역 폭)와 $\gamma_j$(분기 축소)를 양극 값으로
주면, 같은 \code{func\_dU\_hys}$\cdot$\code{func\_U\_branch} 가 양극 히스테리시스를 낸다. 도핑(Al$^{3+}/$Mg$^{2+}$,
비-redox)은 이 상전이들을 \emph{억제}하므로(정렬 교란), 우리 시료에서는 $\Omega_j$ 를 낮춰 폭을 넓히고 peak 를 smear 하는
방향으로 피팅된다(pure-LCO $\Omega$ = 초기값, shift$\cdot$폭은 우리 데이터 — 갭 G4). $\Delta S$ 의 \emph{양}(전자
2상역의 엔트로피 크기)은 N3 가 아니라 \S\ref{sec:eentropy}$\cdot$\S\ref{sec:sum} 가 담당한다.
...
\subsection{같은 결과의 분포 관점 — $\xi_\eq$ 는 평형 점유 확률 분포}\label{sub:dist}
앞서 logistic 평형은 두 길로 닿았다 — (i) 운동학적으로 식~\eqref{eq:db} 의 detailed balance 정지점(\S\ref{sec:width}),
(ii) 열역학적으로 자유에너지 최소 $g_j'(\xi)=sF(V_\eq-U)$(식~\eqref{eq:Veq}, \S\ref{sec:hys}). 이 둘이 \emph{같은}
$\xi_\eq$ 를 주는 것은 우연이 아니다 — 둘 다 하나의 \emph{평형 점유 확률 분포}의 그림자다. 이 소절은 그 분포를 명시해
두 경로를 한 언어로 묶고, LCO 전자 엔트로피(\S\ref{sec:eentropy} 의 Fermi--Dirac)와 Ch2 의 분포 전개로 가는 다리를 놓는다.
(결과식$\cdot$부호$\cdot$코드는 불변 — 유도 관점만 추가한다.)

\textbf{(a) 출발 — 단일 자리의 대정준 점유.} 한 전이의 한 자리는 ``비어 있음''(0)과 ``리튬 점유''(1) 두 상태만 갖는
2준위계다. 리튬 저장조와 입자를 교환하는 대정준 앙상블에서 이 한 자리의 분배함수는, 빈 상태 에너지를 기준($0$)으로 잡고
점유 상태의 \emph{초과} 자유에너지를 $\Delta\mu_j\equiv\mu_\mathrm{Li}^\mathrm{site}-\mu_\mathrm{Li}^\mathrm{res}$ 로 쓰면
\begin{equation}
Z_j=1+e^{-\beta\,\Delta\mu_j},\qquad \beta\equiv\frac{1}{k_BT}
\label{eq:zsite}
\end{equation}
이다(두 항 $=$ 빈$\cdot$참 두 상태의 Boltzmann 가중). \textbf{(b) 연산 — 평균 점유.} 점유 확률(평균 점유)은 참 상태
가중을 $Z_j$ 로 나눈 것이고
\begin{equation}
\langle n_j\rangle=\frac{e^{-\beta\Delta\mu_j}}{1+e^{-\beta\Delta\mu_j}}
=\frac{1}{1+e^{+\beta\Delta\mu_j}}
\label{eq:fermisite}
\end{equation}
— 곧 \emph{Fermi 함수} 꼴이다(분모$\cdot$분자를 $e^{-\beta\Delta\mu}$ 로 나눈 둘째 꼴). \textbf{(c) 중간식 — 전기화학
구동력과의 연결.} 자리 초과 퍼텐셜 $\Delta\mu_j$ 는 ``리튬을 자리에 잡아두는 비용''(저장조 대비)이라, 평형 조건
식~\eqref{eq:eqcond}($\mu_\mathrm{Li}=\mu^0-sF(V-U)$)에서 전위를 올릴수록 자리의 Li 가 \emph{덜} 안정해지므로
$\Delta\mu_j=+sF(V-U_j)$($\Omega=0$ 이상 극한 — 전위가 오르면 비용 증가, 점유 감소)이다. 이를 식~\eqref{eq:fermisite} 에
넣으면 점유율 $\theta_\eq=\langle n_j\rangle=[1+e^{+sF(V-U_j)/RT}]^{-1}$ 이고, 진행률 $\xi=1-\theta$ 의 부호 약속으로 옮기면
$\xi_\eq=1-\theta_\eq=[1+e^{-sF(V-U_j)/RT}]^{-1}$ — 정확히 식~\eqref{eq:logisticsolve}$\cdot$\eqref{eq:xieq} 의
logistic 이다($RT=N_A k_B T$, 자리당$\to$몰당 환산; 高$V\Rightarrow\theta\!\downarrow$, 곧 $\xi\!\uparrow$ 탈리튬화로
\S\ref{sec:width} signbox 와 부호 일치).
\textbf{(d) 핵심 — 세 그림이 한 분포.} 따라서
\begin{equation}
\boxed{\;\xi_{\eq,j}=\text{평형 점유 확률 분포}\;=\;
\underbrace{\text{detailed balance 정지점}}_{\text{(i) 운동학}}
\;=\;\underbrace{\text{자유에너지 최소}}_{\text{(ii) 열역학}}
\;=\;\underbrace{Z_j=1+e^{-\beta\Delta\mu_j}\text{ 의 Fermi 점유}}_{\text{(iii) 분포}}\;.}
\label{eq:threeviews}
\end{equation}
세 유도가 같은 분포의 세 얼굴이다 — 운동학은 그 분포로 \emph{이완}하는 동역학, 열역학은 그 분포를 \emph{최소화}가
고르는 변분, 분포 관점은 그 분포를 \emph{직접} 센 것이다.

\textbf{(e) 상호작용이 있을 때 — 평균장 Fermi.} $\Omega\ne0$ 이면 평형 조건 $\mu_\mathrm{Li}(\theta)=\mu^0-sF(V-U)$
(식~\eqref{eq:eqcond})에 격자기체 $\mu$ 식~\eqref{eq:mu}($\mu^0+RT\ln\frac{\theta}{1-\theta}+\Omega(1-2\theta)$)를 넣고
$\theta$ 로 풀면, 자기무모순(self-consistent) Fermi 점유
$\theta=\big[1+e^{+[sF(V-U_j)+\Omega_j(1-2\theta)]/RT}\big]^{-1}$ 가 된다 — 지수 안에 점유 $\theta$ 자신이 들어가는 것이
평균장의 핵심이다. 이 자기무모순성이 바로 \S\ref{sec:hys} 의 비단조 등온선~\eqref{eq:Veq}$\cdot$이중웰을 낳는다(한 $V$ 에
해 여럿 $=$ 상분리). 곧 히스테리시스는 ``Fermi 점유 분포가 평균장에서 다중해를 갖는 것''으로 다시 읽힌다.

\textbf{왜 이 분포 프레이밍이 LCO 로 가는 다리인가.} 식~\eqref{eq:zsite}--\eqref{eq:fermisite} 의 ``2준위 자리의 Fermi
점유''는 \S\ref{sec:eentropy} 의 \emph{전도 전자} Fermi--Dirac 점유 $f(E)=[1+e^{\beta(E-E_F)}]^{-1}$ 와 \emph{같은
수학 대상}이다 — 전자는 ``에너지 $E$ 의 한 상태''를, 격자기체는 ``조성 자리 하나''를 채우는 차이뿐, 둘 다
$1/(1+e^{\beta\cdot\text{초과퍼텐셜}})$ 점유다. 흑연 forward 모델이 \emph{이온 점유} 분포로 dQ/dV 를 닫았다면, LCO 의
전자 엔트로피는 \emph{전자 점유} 분포로 한 항을 더한다 — 같은 ``점유 분포'' 언어로. 이 평행(parallel)이 \S\ref{sec:eentropy}
의 Sommerfeld 유도를 자연스럽게 만든다(분포 $\to$ 엔트로피 $-k_B\sum p\ln p$ 전개는 Ch2 의 본격 전개이며, 여기서는 그
입구만 연다).
...
% ====================================================================
\section{LCO 전자(electronic) 엔트로피 — 흑연에 없는 한 항 (양극 고유)}\label{sec:eentropy}
% ====================================================================
지금까지 forward 모델은 \emph{이온 점유} 분포 하나로 닫혔다 — 흑연은 그것으로 충분하다. 그러나 LCO 에는 흑연에 없는
성분이 하나 더 있다. 탈리튬화가 진행되면($x\!\downarrow$) LCO 는 절연체에서 금속으로 천이하고(metal--insulator transition,
MIT), 그 순간 \emph{전도 전자}가 생긴다 — 그 전자 기체가 엔트로피에 기여한다. 흑연은 (반)금속적 전도성이 조성에 따라 거의
변하지 않아 이 항이 사실상 $0$ 이지만(전자 자유도가 켜졌다 꺼졌다 하지 않는다), LCO 는 MIT 가 이 항을 \emph{켜고 끈다}.
이 한 항의 유무가 두 전극을 한 장에서 설명하는 교육적 핵심이며, 이 절은 그 항을 \S\ref{sub:dist} 의 분포 언어에서
이어받아 Fermi--Dirac 분포 $\to$ Sommerfeld 전개 $\to$ 부분몰 항으로 한 단계씩 유도한다.

\subsection{전자 엔트로피 닫힌식 — Fermi--Dirac 에서 Sommerfeld 까지}\label{sub:sommerfeld}
\textbf{(a) 출발 — 전도 전자의 Fermi--Dirac 점유.} 금속 상의 전도 전자는 에너지 $E$ 의 한 상태를 점유 확률
\begin{equation}
f(E)=\frac{1}{1+e^{\beta(E-E_F)}},\qquad \beta=\frac{1}{k_BT}
\label{eq:fd}
\end{equation}
로 채운다(\S\ref{sub:dist} 의 2준위 Fermi 점유 식~\eqref{eq:fermisite} 와 같은 꼴 — ``자리''가 ``에너지 상태''로,
``초과 퍼텐셜''이 $E-E_F$ 로 바뀌었을 뿐이다). 자리당 상태밀도를 $g(E)$ 라 하면 한 자리당 전자 엔트로피는, 각 상태가
독립 2준위(점유 확률 $f$)임을 써서 정보 엔트로피 합으로
\begin{equation}
S_e=-k_B\!\int\! g(E)\,\big[f\ln f+(1-f)\ln(1-f)\big]\,\dd E
\label{eq:sefull}
\end{equation}
이다(이것이 \S\ref{sec:hys} 의 섞임 엔트로피 $-k_B[\theta\ln\theta+\cdots]$ 와 \emph{같은} 형태 — 그곳은 조성 자리,
여기는 에너지 상태 위의 합이다). \textbf{(b) 연산 — Sommerfeld 전개.} $k_BT\ll E_F$(상온에서 금속은
$E_F\sim$ eV $\gg k_BT\sim25$ meV)인 축퇴 극한에서, 피적분 함수 $-[f\ln f+(1-f)\ln(1-f)]$ 는 $E=E_F$ 근방 폭
$\sim k_BT$ 의 좁은 창에만 값을 갖는다(깊은 점유$f\!\to\!1$$\cdot$빈 상태$f\!\to\!0$ 양쪽에서 $0$). 이 창 안에서 $g(E)$ 를
$g(E_F)$ 로 두고($g$ 가 $k_BT$ 폭에서 거의 상수) 무차원 변수 $u=\beta(E-E_F)$ 로 바꾸면 $\dd E=k_BT\,\dd u$ 이고
$f=1/(1+e^u)$ 라
\begin{equation}
S_e\simeq -k_B\,g(E_F)\,k_BT\!\int_{-\infty}^{\infty}\!\big[f\ln f+(1-f)\ln(1-f)\big]\,\dd u .
\label{eq:semid}
\end{equation}
\textbf{(c) 중간식 — 표준 정적분.} 괄호 안 $-[f\ln f+(1-f)\ln(1-f)]$ 는 $u$ 의 우함수이고, 그 정적분은 표준 결과
\begin{equation}
-\!\int_{-\infty}^{\infty}\!\big[f\ln f+(1-f)\ln(1-f)\big]\,\dd u=\frac{\pi^2}{3}
\label{eq:pisq}
\end{equation}
이다(부분적분으로 $\int u\,(-\dd f/\dd u)\,\dd u=\int \frac{u\,e^u}{(1+e^u)^2}\dd u$ 로 바꾼 뒤, 이 적분이
$2\!\int_0^\infty\! \frac{u}{e^u+1}\dd u=2\cdot\frac{\pi^2}{12}=\frac{\pi^2}{6}$ 의 두 배 기여로 $\pi^2/3$ 가 되는
Fermi 적분 $\int_0^\infty u/(e^u+1)\,\dd u=\pi^2/12$ — Sommerfeld 보조정리의 1차 계수). \textbf{(d) 박스 — 전자 엔트로피.}
식~\eqref{eq:pisq} 를 식~\eqref{eq:semid} 에 넣으면 자리당 전자 엔트로피가 닫힌다:
\begin{equation}
\boxed{\;S_e(T,x)=\frac{\pi^2}{3}\,k_B^2\,T\,g(E_F,x)\;.}
\label{eq:se}
\end{equation}
이것이 축퇴 전자기체의 표준 Sommerfeld 결과다(tier A; Reynier\cite{reynier2004}$\cdot$Motohashi\cite{motohashi2009} 가 LCO 에
적용). \textbf{핵심 — $\propto T$.}
다른 모든 엔트로피 항($\config$$\cdot$$\vib$ 의 상수 $\Delta S_{\rxn}$)과 달리, 전자 항은 \emph{온도에 선형}이다 — Fermi
창의 폭 자체가 $\sim k_BT$ 라 점유 흐림이 온도에 비례하기 때문이다. 곧 식~\eqref{eq:dUdTcat} 의 $\partial U_j/\partial T$
에 들어가는 전자 기여는 그 자체가 $T$ 함수라($\propto T$), $U_j(T)$ 가 $T$ 의 순수 1차에서 살짝 휘는 비선형을 T1 부근에
남긴다. 이 점은 Ch2 의 가역 발열($T\,\dd S/\dd T$)로 확장할 때 결정적이다.

\subsection{부분몰 전자 엔트로피와 MIT-logistic 게이트}\label{sub:mitgate}
forward 모델이 쓰는 양은 ``Li 1몰 제거(또는 삽입)당'' 엔트로피 변화, 곧 \emph{부분몰} 양이다. \textbf{(a) 출발$\cdot$연산
— $x$ 미분.} 식~\eqref{eq:se} 를 조성 $x$ 로 미분하면(자리당 $S_e$ 의 조성 변화율)
\begin{equation}
\Delta S_e(x,T)=\frac{\partial S_e}{\partial x}\bigg|_T
=\frac{\pi^2}{3}\,k_B^2\,T\,\frac{\partial g(E_F,x)}{\partial x},
\label{eq:dSe}
\end{equation}
이고 — 전자 항이 forward 모델에 들어오는 한 줄이다. \textbf{(b) 모델 가정 — $g(E_F,x)$ 의 MIT 게이트.} 문제는
1차 문헌에 $g(E_F,x)$ 의 \emph{연속} 정량 곡선이 없다는 것이다(Motohashi 는 완전 탈리튬 CoO$_2$ 의
$g(E_F)\approx13$ e/eV 단일 anchor 와 정성 추세만 준다; 이 갭이 G2). Ch1 의 ``문헌값$=$초기값, 데이터로 피팅'' 철학을
그대로 적용해, MIT 를 \emph{logistic 게이트}로 근사한다 — 곧 흑연 전이에서 쓴 것과 같은 $\sigma$ 함수다:
\begin{equation}
g(E_F,x)\;\approx\;g_{\max}\Big[1-\sigma\!\Big(\frac{x-x_\MIT}{\Delta x_\MIT}\Big)\Big],
\qquad \sigma(z)=\frac{1}{1+e^{-z}},
\label{eq:ggate}
\end{equation}
\textbf{초기값}(피팅 대상): $g_{\max}=13$ e/eV, $x_\MIT\approx0.85$, $\Delta x_\MIT\approx0.05$
(MIT 발현 2상역 $x\approx0.75$--$0.94$[Reimers\cite{reimers1992}$\cdot$Ménétrier\cite{menetrier1999}]를 게이트
중심$\pm$폭으로 흡수). $x$ 가 $x_\MIT$
아래로 내려가면($\sigma\to0$) $g\to g_{\max}$ 로 \emph{켜지고}, 위($\sigma\to1$)면 $g\to0$ 로 꺼진다 — 절연체
($g\!=\!0$)$\to$금속($g\!=\!g_{\max}$) 천이를 매끄럽게 모형화한다(그림~\ref{fig:egate} 실선). \emph{step 대신 logistic 을 쓰는 이유}는 흑연 전이와
같다 — 매끄러운 $\partial U_j/\partial T$(겹침 가중과 일관)와 결함농도에 따른 발현 $x$ 분산을 폭 $\Delta x_\MIT$ 로 흡수하기
위함이다. \textbf{(c) 중간식 — 게이트의 미분.} 식~\eqref{eq:ggate} 를 $x$ 로 미분하면(logistic 미분 항등식
$\sigma'=\sigma(1-\sigma)$, \S\ref{sec:eqpeak} 의 종 항등식과 같은 것)
\begin{equation}
\frac{\partial g}{\partial x}=-\frac{g_{\max}}{\Delta x_\MIT}\,
\sigma\!\Big(\tfrac{x-x_\MIT}{\Delta x_\MIT}\Big)\Big[1-\sigma\!\Big(\tfrac{x-x_\MIT}{\Delta x_\MIT}\Big)\Big]<0,
\label{eq:dgdx}
\end{equation}
$x_\MIT$ 에서 최대 크기인 \emph{음의} 종이다($\partial g/\partial x<0$ — $x$ 를 줄이면 $g$ 가 는다). \textbf{(d) 박스 —
부분몰 전자 항.} 식~\eqref{eq:dgdx} 를 식~\eqref{eq:dSe} 에 넣으면
\begin{equation}
\boxed{\;\Delta S_e(x,T)=-\frac{\pi^2}{3}k_B^2T\,\frac{g_{\max}}{\Delta x_\MIT}\,
\sigma\!\Big(\tfrac{x-x_\MIT}{\Delta x_\MIT}\Big)\Big[1-\sigma\!\Big(\tfrac{x-x_\MIT}{\Delta x_\MIT}\Big)\Big]\;}
\label{eq:dSegate}
\end{equation}
— MIT 부근 $x\approx x_\MIT$ 에 국소화된 종 모양 기여다(T1 전이에서만 켜지고, T2$\cdot$T3 에서는 $0$). \textbf{부호 규약(★명시).}
이 항이 들어가는 슬롯 $\Delta S_{\rxn,j}$ 는 \emph{삽입 반응 엔트로피}다 — 흑연과 한 글자도 다르지 않은 규약으로,
\S\ref{sec:center} 의 삽입 반응 $\mathrm{Li}^++e^-+[\,]\rightleftharpoons\mathrm{Li}$ 에 대해 정의되고($\Delta G_j=\Delta H_\rxn-T\Delta S_\rxn$,
$\partial U_j/\partial T=\Delta S_{\rxn,j}/F$), 흑연 \code{GRAPHITE\_STAGING\_LIT} 의 stage $2\!\to\!1$ 이 $\Delta S_\rxn=-16$
J/(mol\,K)인 것이 바로 ``Li 1몰 \emph{삽입}당'' 엔트로피(stage 1 정렬로 감소)다. 따라서 전자 항도 \emph{같은 삽입 방향}으로
슬롯에 들어가야 한다 — 곧 식 박스의 부호 그대로 $\Delta S_e\equiv\partial S_e/\partial x<0$($x$ 를 \emph{올리면}, 곧 리튬화하면
$g$ 가 줄어 $S_e$ 감소; 삽입 방향 $\Delta S_{\rxn,j}$ 와 일관). \S\ref{sec:sum} 식~\eqref{eq:dScat} 의 $\Delta S_e(x,T)$ 항은 이
\emph{음수}값 그대로 $\Delta S_{\rxn,j}^\cat$ 에 더해진다(부호 뒤집기 없음 — 슬롯 자체가 이미 삽입 기준이므로 흑연과 같은
자리에 그대로 plug-in). \textbf{물리 부기.} 반대로 읽으면, 전이의 주방향인 \emph{탈리튬화}(Li 제거, $x\downarrow$)에서는 금속
전자가 켜지며 $S_e$ 가 \emph{늘어나}, ``Li 1몰 제거당'' 변화 $-\partial S_e/\partial x>0$ 로 $+0.18\,k_B$/atom 규모를 방출한다
— 이것은 삽입 기준 음수항의 부호를 뒤집은 \emph{같은 물리}의 다른 서술일 뿐, 슬롯에 넣는 값은 어디까지나 삽입 기준
$\partial S_e/\partial x<0$ 이다. 단전극 $\dd\phi/\dd T\approx+0.83$ mV/K 은 config$+$vib 가 지배하는 양이라(전자 항은
$\sim0.18\,k_B$/atom 으로 작다) 이 작은 전자 항의 \emph{부호를 제약하지 않는다}(sanity 로 오용 금지).

\textbf{크기 검산 — 작지만 $0$ 이 아니다.} 완전 금속 한 자리의 절대 전자 엔트로피는 식~\eqref{eq:se} 로
$S_e/k_B=\frac{\pi^2}{3}(k_BT)\,g(E_F)=3.29\times0.0257\,\text{eV}\times13\,\text{eV}^{-1}\approx1.1\,k_B$/atom
($25^\circ$C, $k_BT=0.0257$ eV, 완전 metal). O3 상 내 \emph{부분몰}차(T1 게이트 폭에 걸친 $\Delta S_e$ 적분 규모)는 $\approx0.18\,k_B$/atom
[Reynier, tier B] — 같은 자릿수다($\checkmark$). 곧 전자 항은 $\config$($x\approx0.5$ 에서 $R\ln2\approx0.69\,k_B$
규모) 대비 \emph{작다}. 그럼에도 \textbf{필수}인 이유 — config 엔트로피만으로는 MIT 의 2상역(절연$\cdot$금속 공존)을 재현할
수 없기 때문이다[ML 2024, tier A]: config 는 Li/공공 배열만 세고 전자 자유도의 켜짐을 모른다. 전자 게이트가 T1 의
삽입 기준 $\Delta S_{\rxn}$ 에 \emph{국소} 음봉우리($\Delta S_e<0$, 식~\eqref{eq:dSegate})를 더해야 MIT 구간의
$\partial U/\partial T$ 크기와 dQ/dV 미세 비대칭이 맞는다(탈리튬화 방향으로 읽으면 같은 항이 $S_e$ 증가를 낸다). 작은 항이 정성을 바꾸는 전형이다.

\textbf{흑연 대비 — 왜 흑연엔 이 항이 없나.} 흑연(LiC$_6$)은 모든 staging 단계에서 금속$\cdot$반금속적 전도를 유지하며
$g(E_F)$ 가 조성에 따라 큰 \emph{천이}(절연$\leftrightarrow$금속)를 겪지 않는다 — 곧 식~\eqref{eq:dSe} 의
$\partial g/\partial x$ 가 작아 전자 항을 \emph{무시}할 수 있다(흑연 forward 모델이 $\config+\vib$ 로 닫힌 이유). LCO 는
T1 에서 $g(E_F)$ 가 $0\!\to\!g_{\max}$ 로 켜지므로 $\partial g/\partial x$ 가 그 구간에서만 크게 살아난다. \emph{이 대비가
통합 챕터의 교육 가치다} — 같은 분포 언어(Fermi 점유)에서, 전극의 전자구조가 전자 엔트로피 항의 유무를 결정한다.

\begin{figure}[t]
\centering
\begin{tikzpicture}[x=8.0cm,y=1.7cm]
% x-axis: composition x from 1.0 (left) down to 0.6 (right) — delithiation to the right
\draw[-{Latex[length=1.6mm]}] (0.62,0) -- (1.02,0);
\node[font=\scriptsize] at (1.0,-0.32) {$x=1$ (insulator)};
\node[font=\scriptsize] at (0.66,-0.32) {$x\!\downarrow$ (delithiation $\to$ metal)};
\draw[-{Latex[length=1.6mm]}] (1.0,-0.05) -- (1.0,2.35) node[above,right,font=\scriptsize] {$g(E_F,x)$ / $|\Delta S_e|$ (arb.)};
% ticks for x
\foreach \xx/\lab in {0.95/0.95,0.85/0.85,0.75/0.75,0.65/0.65} {\draw (\xx,0.03)--(\xx,-0.03) node[below,font=\tiny] {\lab};}
% g(E_F,x): logistic gate, =0 at x>~0.9, rises to g_max as x decreases past x_MIT=0.85
% plotted left(x=1) to right(x=0.65); value low at x=1, high at x=0.65
\draw[thick] plot[smooth] coordinates {
 (1.00,0.04) (0.97,0.07) (0.94,0.14) (0.91,0.30) (0.88,0.60) (0.85,1.00) (0.82,1.40) (0.79,1.70) (0.76,1.86) (0.73,1.94) (0.70,1.97) (0.67,1.99) (0.65,2.0)};
\node[right,font=\scriptsize] at (0.70,1.70) {$g(E_F,x)$ gate (eq.~\eqref{eq:ggate})};
% |dSe/dx| ~ bell centered at x_MIT=0.85 (dashed)
\draw[densely dashed,thick] plot[smooth] coordinates {
 (1.00,0.03) (0.97,0.07) (0.94,0.18) (0.91,0.42) (0.88,0.78) (0.86,0.95) (0.85,1.0) (0.84,0.95) (0.82,0.78) (0.79,0.42) (0.76,0.18) (0.73,0.07) (0.70,0.03) (0.67,0.015) (0.65,0.01)};
\node[font=\scriptsize,align=center] at (0.885,1.25) {$|\Delta S_e|$ bell\\(eq.~\eqref{eq:dSegate})};
% x_MIT marker
\draw[densely dotted] (0.85,0)--(0.85,1.0);
\node[below,font=\tiny] at (0.85,-0.10) {$x_\mathrm{MIT}$};
% T1 region shading
\fill[black!8] (0.90,-0.05) rectangle (0.76,2.1);
\node[font=\tiny] at (0.83,2.18) {T1 (MIT 2-phase)};
\end{tikzpicture}
\caption{LCO 전자 엔트로피 게이트(신규). 가로축 $=$ 조성 $x$($x\!=\!1$ 왼쪽, 탈리튬화로 오른쪽). 실선 $=$ MIT-logistic
게이트 $g(E_F,x)$(식~\eqref{eq:ggate}, $x_\MIT\!=\!0.85$, $\Delta x_\MIT\!=\!0.05$, $g_{\max}\!=\!13$ e/eV) —
$x\!>\!0.9$ 절연체($g\!\approx\!0$)에서 $x\!<\!0.76$ 금속($g\!\to\!g_{\max}$)으로 켜진다. 파선 $=$ 부분몰 전자 항
$|\Delta S_e|$(식~\eqref{eq:dSegate}) — $x_\MIT$ 에 국소화된 종으로, T1 전이(음영)에서만 $\Delta S_{\rxn}$ 에 더해진다.
크기는 작지만($\sim0.18\,k_B$/atom) MIT 2상역을 config 단독이 재현 못 하므로 필수. 흑연엔 이 게이트가 없다($g(E_F)$
조성 천이 미미 $\Rightarrow$ 파선이 $0$).}
\label{fig:egate}
\end{figure}

\begin{codebox}
\textbf{전자 항 plug-in.} 코드 구조 변경 $0$ — \code{func\_U\_j} 에 들어가는 전이 $j$ 의 $\Delta S_{\rxn,j}$ 가
$\Delta S^\config_j+\Delta S^\vib_j+\Delta S_e(x,T)$ 로 합산될 뿐이다(\S\ref{sec:sum}). $\Delta S_e$ 만 $T$ 함수라
배열 $T_\work$ 에서 격자별로 평가하고($\propto T$), T2$\cdot$T3 에서는 게이트가 꺼져 $\Delta S_e=0$ 이다. 게이트
파라미터 $(g_{\max},x_\MIT,\Delta x_\MIT)$ 는 전이 dict 의 새 키(초기값 표~\ref{tab:lcostaging} T1 행)이며 피팅 대상이다.
\end{codebox}

다음 절은 이 항들이 들어간 평형 종(평형 peak)으로 돌아간다 — 전자 항은 $\Delta S_{\rxn}$ 을 통해 중심 $U_j(T)$ 를 미세
이동시킬 뿐, peak 의 \emph{모양} 식~\eqref{eq:eqpeak} 자체는 흑연과 동일하다.
...
\subsection{양극 사례 — LCO dQ/dV peak 위치}\label{sub:lcopeak}
평형 peak 식~\eqref{eq:eqpeak} 은 전극 무관이므로 LCO 도 같은 종 $Q_j\,\xi_\eq(1-\xi_\eq)/w_j$ 를 전이마다 그린다 —
바뀌는 것은 종이 \emph{앉는 위치}(중심 $U_j^{\,d}$, 표~\ref{tab:lcostaging})뿐이다. 흑연이 $\sim0.085$--$0.21$ V 에
4 봉우리를 남기는 데 비해, LCO 하프셀은 $\sim3.9$--$4.2$ V 에 3 봉우리를 남긴다 — T1(MIT, $\sim3.90$ V),
T2$\cdot$T3(order--disorder 쌍, $\sim4.05$ 와 $\sim4.17$--$4.20$ V). 전자 엔트로피 항은 peak 의 모양이 아니라 중심
$U_j(T)$ 를 통해 T1 봉우리의 \emph{온도 이동}에만 미세하게 들어간다(식~\eqref{eq:dUdTcat} 의 $\Delta S_e$ 기여,
$\propto T$). 그림~\ref{fig:lcopeak} 가 세 봉우리를 보인다.

\begin{figure}[t]
\centering
\begin{tikzpicture}[x=2.7cm,y=1.05cm]
\draw[-{Latex[length=1.6mm]}] (3.80,0) -- (4.30,0) node[below,font=\scriptsize] {$V$ [V vs Li]};
\draw[-{Latex[length=1.6mm]}] (3.80,0) -- (3.80,3.0) node[above,font=\scriptsize] {$\dd Q/\dd V$ [arb.]};
\foreach \vv in {3.9,4.0,4.1,4.2} {\draw (\vv,0.04)--(\vv,-0.04) node[below,font=\tiny] {\vv};}
% baseline Cbg
\draw[densely dotted] (3.80,0.30)--(4.30,0.30);
\node[right,font=\tiny] at (4.22,0.42) {$C_\mathrm{bg}$};
% T1 bell at 3.90 (broad, MIT — electronic gate region)
\draw[thick] plot[smooth] coordinates {(3.80,0.34) (3.84,0.55) (3.88,1.35) (3.90,1.75) (3.92,1.35) (3.96,0.62) (4.00,0.42)};
\node[font=\scriptsize] at (3.90,1.95) {T1};
\node[font=\tiny,align=center] at (3.875,1.05) {MIT\\(elec.\ gate)};
% T2 bell at 4.05
\draw[thick] plot[smooth] coordinates {(4.00,0.42) (4.02,0.85) (4.04,1.95) (4.05,2.45) (4.06,1.95) (4.08,0.95) (4.11,0.55)};
\node[font=\scriptsize] at (4.05,2.62) {T2};
% T3 bell at 4.18
\draw[thick] plot[smooth] coordinates {(4.11,0.55) (4.13,0.95) (4.16,1.85) (4.18,2.25) (4.20,1.75) (4.23,0.80) (4.27,0.45)};
\node[font=\scriptsize] at (4.185,2.42) {T3};
% T2-T3 pair bracket
\draw[{Latex[length=1.2mm]}-{Latex[length=1.2mm]},gray] (4.05,2.7) -- (4.18,2.7);
\node[font=\tiny,gray] at (4.115,2.85) {order--disorder pair};
\end{tikzpicture}
\caption{LCO 하프셀 dQ/dV 평형 peak 도식(신규, 정성). 세 종 $=$ T1(MIT, $\sim3.90$ V; 전자 게이트 구간이라 폭이
넓고 온도 민감), T2$\cdot$T3(order--disorder 쌍, $\sim4.05$/$\sim4.18$ V). 각 봉우리는 같은 평형 종
$Q_j\xi_\eq(1-\xi_\eq)/w_j$(식~\eqref{eq:eqpeak})이며 중심만 표~\ref{tab:lcostaging} 로 바뀐다. 흑연(그림은
$0.085$--$0.21$ V)과 같은 forward 모델, 다른 전위 대. 전자 엔트로피는 T1 중심의 온도 이동에만 미세 기여한다.}
\label{fig:lcopeak}
\end{figure}
...
\subsection{양극 합산 — $\Delta S_{\rxn}^\cat$ 분해와 전자 항 plug-in}\label{sub:lcosum}
LCO 도 같은 합산식~\eqref{eq:sum} $\dd Q/\dd V=C_\bg+\sum_jQ_j[\text{peak\_shape}_j]$ 로 닫힌다 — 전이 리스트가
\code{LCO\_STAGING\_LIT}(표~\ref{tab:lcostaging})로 바뀔 뿐이다. 단 한 곳, 전이 $j$ 의 중심 $U_j(T)$ 에 들어가는
반응 엔트로피 $\Delta S_{\rxn,j}$ 가 양극에서는 세 성분의 합으로 분해된다:
\begin{equation}
\boxed{\;\Delta S_{\rxn,j}^\cat(x,T)=
\underbrace{\Delta S^\config_j}_{\text{logistic }w=RT/F\text{ 가 담음}}
+\underbrace{\Delta S^\vib_j}_{\text{음의 baseline}}
+\underbrace{\Delta S_e(x,T)}_{\text{MIT 게이트, }\propto T\ (\S\ref{sec:eentropy})}\;.}
\label{eq:dScat}
\end{equation}
세 성분의 자리와 \emph{이중계산 금지}가 핵심이다.
\begin{itemize}[leftmargin=1.4em,itemsep=1pt,topsep=1pt]
\item $\Delta S^\config_j$ — Li/공공 배열의 섞임 엔트로피로, O3 상 LCO 의 $\Delta S$ 를 지배한다($>\tfrac12$,
Reynier tier A). \textbf{★이중계산 금지(B)} — 봉우리 \emph{내부}의 조성 분포 섞임 엔트로피는 이미
\S\ref{sec:hys}$\cdot$\S\ref{sec:width} 의 logistic($\xi_\eq$ 와 폭 $w=RT/F$)이 자동으로 담는다 — 격자기체 $\mu$ 식~\eqref{eq:mu}
의 로그 몫 $RT\ln[\theta/(1-\theta)]$ 가 곧 섞임 엔트로피 $S_\mathrm{mix}=-R[\theta\ln\theta+(1-\theta)\ln(1-\theta)]$
(식~\eqref{eq:gxi} 의 출처)의 조성 미분이고, 이것이 봉우리 폭 $w=RT/F$ 로 들어가 있다. 따라서 식~\eqref{eq:dScat}
의 $\Delta S^\config_j$ 에 \emph{입력}하는 값은 봉우리 내부 분포가 아니라 \emph{전이 중심의 표준값} $\Delta S^0_j$ 뿐이다 —
봉우리 안의 조성 분포 엔트로피를 다시 더하면 같은 양을 두 번 세게 된다(흑연 $\Delta S_\rxn$ 입력이 중심 표준값인 것과 동형).
\item $\Delta S^\vib_j$ — phonon 진동 엔트로피의 음의 baseline(고-$x$). 흑연과 동형으로 $\Delta S^0_j$ 안에 흡수되어
중심 표준값의 일부로 들어간다.
\item $\Delta S_e(x,T)$ — \S\ref{sec:eentropy} 의 전자 항(식~\eqref{eq:dSegate}). 흑연엔 없고, T1(MIT)에서만
게이트가 켜져 \emph{삽입 기준} $\Delta S_{\rxn,\text{T1}}$ 에 국소 음봉우리($\Delta S_e<0$, 흑연 $\Delta S_\rxn$ 슬롯과 한 규약)를
더한다. 유일하게 $T$ 함수($\propto T$)라 배열 $T_\work$ 에서 격자별로 평가한다.
\end{itemize}
곧 코드는 $\Delta S_{\rxn,j}$ 입력을 $\Delta S^0_j$(=config 중심 표준 $+$ vib, 흑연과 같은 자리) $+\Delta S_e(x,T)$
(전자, 새 plug-in)로 받아 같은 \code{func\_U\_j} 에 넣는다. config 봉우리 내부 분포는 logistic 이 담으므로 중복 입력하지
않는다 — 이 한 규칙이 양극 합산의 유일한 새 주의점이다.

\subsection{코드 일반화 — MSMR $\leftrightarrow$ Ch1 동형 (H)}\label{sub:msmr}
양극 dQ/dV 피팅의 표준 모델인 MSMR(Multiple Site Multiple Reaction)\cite{verbrugge2017}은 전이 $j$ 의 점유를
$x_j=X_j/\{1+\exp[f(U-U_j^0)/\omega_j]\}$ 로 적는다 — 이것은 본 장 식~\eqref{eq:xieq} 의 transition-logistic
$\xi_{\eq,j}=1/\{1+\exp[-\sigma_d(V-U_j^{\,d})/w_j]\}$ 와 \emph{대수적으로 동형}이다($X_j\!\leftrightarrow\!Q_j$ 용량 가중,
$\omega_j\!\leftrightarrow\!w_j$ 폭, $U_j^0\!\leftrightarrow\!U_j^{\,d}$ 중심, 부호 인자 $f\!\leftrightarrow\!\sigma_d$).
곧 MSMR 은 본 forward 모델의 평형 항($|I|\to0$ 극한, \S\ref{sec:eqpeak})과 같은 식이며, 본 모델은 거기에 동역학
꼬리(N7$\cdot$N8)와 히스테리시스(N3)를 \emph{더} 얹은 상위 집합이다. 따라서 LCO 적용은 다음 둘만으로 닫힌다 —
구조 변경 $0$:
\begin{enumerate}[label=(\roman*),leftmargin=2.0em,itemsep=1pt,topsep=1pt]
\item \textbf{전이 파라미터 교체} — \code{GRAPHITE\_STAGING\_LIT} $\to$ \code{LCO\_STAGING\_LIT}(표~\ref{tab:lcostaging}):
$U_j$$\cdot$$\Delta S_{\rxn,j}$$\cdot$$\Omega_j$$\cdot$$Q_j$ 를 양극 값으로. \code{func\_ksi\_eq}$\cdot$\code{func\_U\_j}
$\cdot$\code{func\_dU\_hys} 등 함수 본체는 한 줄도 바뀌지 않는다(전극 일반으로 유도됨).
\item \textbf{전자 항 plug-in} — T1 전이 dict 에 게이트 키 $(g_{\max},x_\MIT,\Delta x_\MIT)$ 를 더하고,
$\Delta S_{\rxn,\text{T1}}\leftarrow\Delta S^0+\Delta S_e(x,T)$(식~\eqref{eq:dSegate})로 합산. $\partial U_j/\partial T
\leftarrow\Delta S_{\rxn,j}/F$ 경로(식~\eqref{eq:dUdTcat})는 그대로다.
\end{enumerate}
요컨대 ``파라미터$+1$항''만으로 흑연 코드가 LCO 코드가 된다 — 이것이 본 장이 두 전극을 \emph{한 클래스}로 보는 실증이다.
도핑(Al$^{3+}/$Mg$^{2+}$, 비-redox)은 전자 항을 \emph{보존}한 채(Co redox 불변) 상전이를 억제하므로, 우리 시료에서는
$\Omega_j$ 를 낮춰(폭 smear) $U_j$ 를 미세 shift 하는 방향으로 피팅된다 — pure-LCO 값$=$초기값, shift$\cdot$폭은 우리
데이터(갭 G4).

\textbf{문헌 초기값의 갭(문서 명시).} 표~\ref{tab:lcostaging}$\cdot$\S\ref{sec:eentropy} 의 값은 \emph{초기값}이며,
다음은 우리 코인 하프셀 데이터의 round-trip 피팅으로 식별한다(허위 정밀 금지, tier 표기) — G1 연속 $\Delta S(x)$,
G2 $g(E_F,x)$ 곡선($=$게이트 $g_{\max}$$\cdot$$x_\MIT$$\cdot$$\Delta x_\MIT$), G3 MSMR LCO 파라미터,
G4 도핑 shift$\cdot$폭, G5 $\Delta H_f$ 절대값($=$OCV plateau, 예: $3.9$ V 로 anchor). 전부 흑연
\code{GRAPHITE\_STAGING\_LIT}=초기값 철학 안이다.
...
\item \textbf{양극(LCO) 부호 사슬 — 흑연과 1:1.} 식~\eqref{eq:dUdTcat}$\cdot$\eqref{eq:dScat}. $U_j(T)=
(-\Delta H_\rxn+T\Delta S_\rxn)/F$ 와 $\partial U_j/\partial T=\Delta S_{\rxn,j}/F$ 가 \emph{전극 무관}으로 유도돼
(\S\ref{sub:lcocenter}), LCO 에 식 그대로 적용된다. 양극 $\Delta S_{\rxn,j}^\cat$ 가 양수 경향이라
$\partial U_j/\partial T>0$(단전극 $\dd\phi/\dd T\approx+0.83$ mV/K[B]와 부호 정합), $V\!\uparrow\Rightarrow\xi_\eq\!\uparrow$
방전 규약(식~\eqref{eq:xieq})도 양극에서 동일. $\checkmark$
\item \textbf{전자 엔트로피 항 — 부호$\cdot$국소성$\cdot$$\propto T$.} 식~\eqref{eq:dSe}$\cdot$\eqref{eq:dSegate}.
슬롯 $\Delta S_{\rxn,j}$ 가 \emph{삽입 기준}이므로 전자 항도 삽입 기준 $\Delta S_e=\partial S_e/\partial x<0$(식 박스 음부호)으로
그대로 더해진다 — 흑연 $\Delta S_\rxn$ 슬롯과 한 규약(부호 뒤집기 없음). 물리적으로 탈리튬화(Li 제거) 시 $g\!\uparrow$$\cdot$$S_e\!\uparrow$
($+0.18\,k_B$/atom 방출)는 같은 항의 $-\partial S_e/\partial x$ 서술일 뿐. MIT($x\approx x_\MIT$)에만 국소(T2$\cdot$T3 에서 $0$),
유일하게 $\propto T$(상수 $\Delta S_\rxn$ 항과 대비). $+0.83$ mV/K 단전극 sanity 는 config$+$vib 지배라 이 부호를 제약하지 않음.
흑연은 $\partial g/\partial x\!\approx\!0$ 이라 이 항 $\to0$(전극 대비 일관). $\checkmark$
\item \textbf{이중계산 금지(B).} 식~\eqref{eq:dScat}. config 봉우리 내부 분포는 logistic($\xi_\eq$$\cdot$$w$)이 담으므로,
$\Delta S^\config_j$ 입력은 \emph{중심 표준값} $\Delta S^0_j$ 만(봉우리 내부 분포 재가산 금지 — 흑연 $\Delta S_\rxn$ 이
중심 표준값인 것과 동형). $\checkmark$
\end{enumerate}
전건이 기준 명제와 정합한다 — 부호 결함 $0$(흑연 S1--S8 $+$ 양극 S9--S11 통합).
...
\bibitem{reynier2004} Y. Reynier, J. Graetz, T. Swan-Wood, P. Rez, R. Yazami, B. Fultz, ``Entropy of Li intercalation in Li$_x$CoO$_2$,'' \emph{Phys. Rev. B} \textbf{70}, 174304 (2004). DOI: 10.1103/PhysRevB.70.174304. (companion \emph{J. Electrochem. Soc.} \textbf{151}, A422 (2004).)
\bibitem{xia2007} H. Xia, L. Lu, Y. S. Meng, G. Ceder, ``Phase Transitions and High-Voltage Electrochemical Behavior of LiCoO$_2$ Thin Films Grown by Pulsed Laser Deposition,'' \emph{J. Electrochem. Soc.} \textbf{154}, A337 (2007). DOI: 10.1149/1.2509021.
\bibitem{menetrier1999} M. Ménétrier, I. Saadoune, S. Levasseur, C. Delmas, ``The insulator-metal transition upon lithium deintercalation from LiCoO$_2$,'' \emph{J. Mater. Chem.} \textbf{9}, 1135 (1999). DOI: 10.1039/a900016j.
\bibitem{motohashi2009} T. Motohashi \emph{et al.}, ``Electronic phase diagram of the layered cobalt oxide Li$_x$CoO$_2$,'' \emph{Phys. Rev. B} \textbf{80}, 165114 (2009). DOI: 10.1103/PhysRevB.80.165114.
\bibitem{reimers1992} J. N. Reimers, J. R. Dahn, ``Electrochemical and in situ X-ray diffraction studies of lithium intercalation in Li$_x$CoO$_2$,'' \emph{J. Electrochem. Soc.} \textbf{139}, 2091 (1992). DOI: 10.1149/1.2221184.
\bibitem{verbrugge2017} M. W. Verbrugge \emph{et al.}, ``Thermodynamic model for substitutional materials (MSMR) for lithium-ion batteries,'' \emph{J. Electrochem. Soc.} \textbf{164}, E3243 (2017). DOI: 10.1149/2.0341708jes.
\end{thebibliography}
