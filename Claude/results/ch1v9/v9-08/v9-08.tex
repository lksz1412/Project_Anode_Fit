...
LCO 의 T2/T3 peak 는 graphite staging 이 아니라 Li/vacancy order--disorder 전이에서 나온다. 그래도 Ch1 이 쓰는
...
($x\approx0.94$--$0.75$)은 전자 상태밀도 변화가 함께 켜지는 2상역이고, T2/T3 는 monoclinic 정렬을 동반하는
...
이 켜진다. 그러면 Li/vacancy 점유 분포 위에 전도 전자의 Fermi--Dirac 점유 분포가 추가 엔트로피를 만든다.
...
부분몰 전자 엔트로피는 좌표 정의를 분리해 써야 부호가 흐려지지 않는다. LCO 조성 $x$ 를 Li 함량으로 잡으면
\begin{equation}
\Delta S_e^{(x)}(x,T)=\frac{\partial S_e}{\partial x}\Big|_T
=\frac{\pi^2}{3}k_B^2T\,\frac{\partial g(E_F,x)}{\partial x}.
\label{eq:dSedx}
\end{equation}
반면 탈리튬화 진행 좌표 $\zeta=1-x$ 로 쓰는 코드 plug-in 은
\begin{equation}
\Delta S_e^{(\zeta)}(x,T)=\frac{\partial S_e}{\partial \zeta}\Big|_T
=-\frac{\pi^2}{3}k_B^2T\,\frac{\partial g(E_F,x)}{\partial x}.
\label{eq:dSedzeta}
\end{equation}
MIT 에서 $x$ 가 내려갈수록 $g(E_F)$ 가 증가하므로 $\partial g/\partial x<0$ 이고, Li 제거 좌표의 기여
$\Delta S_e^{(\zeta)}$ 는 양의 봉우리로 나타난다. 양극 전위 부호는 여전히 식~\eqref{eq:Ujcat} 의
...
\label{eq:gmitprime}
\end{equation}
따라서 전자 엔트로피 plug-in 은 T1 MIT 부근에 국소화된 $T$-선형 봉우리다. 완전 metallic anchor 의 크기 검산은
$S_e/k_B=(\pi^2/3)(k_BT)g(E_F)\approx3.29\times0.0259\times13\approx1.1\,k_B$/atom 이고,
O3 내부의 부분몰 차는 약 $0.18\,k_B$/atom 수준으로 작다. 작지만 T1 2상역은 configurational 항만으로 재현하기 어렵기
때문에 gate 자체는 필수 성분으로 둔다. 흑연에는 대응되는 sharp MIT 가 없으므로 Ch1 graphite 기본 모델에서는
$\Delta S_e=0$ 으로 둔다.
...
LCO 전이 dict 도 같은 키를 쓴다. 다른 점은 $\Delta S_{\rxn,j}^\mathrm{cat}$ 를 한 숫자로 끝내지 않고, 중심 표준값과
LCO 고유 electronic plug-in 을 분리해 생각해야 한다는 것이다:
\begin{equation}
\boxed{\;
...
+\underbrace{\Delta S_e(x,T)}_{\text{MIT gate, } \propto T}\; .}
...
표준값이고, peak 내부의 조성별 mixing 로그를 다시 더하지 않는다. $\Delta S_e(x,T)$ 만 LCO T1 MIT 구간에서
