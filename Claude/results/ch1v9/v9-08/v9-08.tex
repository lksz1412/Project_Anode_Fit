...
\hypersetup{colorlinks=true, linkcolor=blue, urlcolor=blue, citecolor=blue,
  pdftitle={Chapter 1 v9-08: 흑연 음극 dQ/dV 이론 + LCO 보완}}
\pagestyle{fancy}\fancyhf{}
\lhead{Ch.1 v9-08 — graphite $dQ/dV$ + LCO supplement}\rhead{\thepage/\pageref{LastPage}}
...
\title{\textbf{\LARGE Chapter 1 v9-08}\\[0.35em]\Large 흑연 음극 $dQ/dV$ 이론: 한 곡선을 그리는 코드 진행을 따라가는 수식-구동판 — LCO 보완 통합}
\author{}
\date{}
...
LCO 양극 반쪽전지(Li$_x$CoO$_2$ vs Li/Li$^+$)도 같은 전이 리스트 언어로 들어온다. 차이는 전위 스케일이다
\cite{reimers1992,xia2007,motohashi2009}:
흑연 음극 전이는 $0.08$--$0.21$ V 근방에 놓이지만, LCO 양극 전이는 높은 전위의 $3.9$--$4.2$ V 근방에 놓인다.
표~\ref{tab:lco-staging-lit} 의 값은 \code{LCO\_STAGING\_LIT} 초기값으로 쓰는 문헌 tier 출발점이며, graphite 표와
같이 피팅으로 override 하는 값이다. T4 고전압 O3$\to$H1-3 전이는 $4.55$ V 부근이라 본 Ch1 의 코인 하프셀
$\lesssim4.2$--$4.5$ V 범위에서는 기본 전이 목록 밖에 둔다.

\begin{table}[t]
\centering\footnotesize
\renewcommand{\arraystretch}{1.18}
\setlength{\tabcolsep}{3pt}
\caption{LCO 전이 초기값 \code{LCO\_STAGING\_LIT}(tier A/B 출발점, 피팅으로 override). $U_j^\mathrm{cat}$ 는
양극 반쪽전지 전위(vs Li/Li$^+$)다.}
\label{tab:lco-staging-lit}
...
\toprule
전이 & $U_j^\mathrm{cat}$ [V] & $x$ 범위/중심 & 성격 & 사용 \\ \midrule
T1 MIT & $\sim3.90$ & $x\approx0.94$--$0.75$ & insulator$\to$metal; 전자항 gate ON & 기본 \\
T2 order--disorder a & $\sim4.05$ & $x\approx0.55$ & hexagonal$\to$monoclinic 정렬 & 기본 \\
T3 order--disorder b & $\sim4.17$--$4.20$ & $x\approx0.48$ & monoclinic$\to$hexagonal 재배열 & 기본 \\
T4 O3$\to$H1-3 & $\sim4.55$ & $x\approx0.2$ & 고전압 구조 전이 & 범위 밖/옵션 \\
\bottomrule
\end{tabular}
\end{table}
...
\subsection{LCO 양극 중심 $U_j^\mathrm{cat}(T)$ — 같은 식, 높은 전위}
LCO 전이는 \code{GRAPHITE\_STAGING\_LIT} 대신 표~\ref{tab:lco-staging-lit} 의 \code{LCO\_STAGING\_LIT} 를 넣는
같은 루프다. 전극이 양극이어도 반쪽전지 평형 전위는 같은 Gibbs 사슬에서 나오므로
\begin{equation}
...
\label{eq:Ujcat}
\end{equation}
따라서 양극 부호도 흑연과 1:1 이다: 양의 $\Delta S_{\rxn,j}^\mathrm{cat}$ 는 $T$ 상승 시 양극 전이 중심을 올리고,
음의 값은 낮춘다. 차이는 $\Delta H^\mathrm{cat}$ anchor 가 LCO plateau 를 $3.90$, $4.05$, $4.17$--$4.20$ V
근방으로 맞춘다는 점뿐이다. tier 정직성은 표~\ref{tab:lco-staging-lit} 의 전위를 문헌 초기값으로 두고, 우리 데이터에서
$U_j^\mathrm{cat}$, 폭, peak shift 를 함께 피팅한다는 뜻이다.

단전극 검산으로는 LCO 하프셀의 온도계수 $d\phi/dT\approx+0.83$ mV/K 가 보고되어
\cite{wang2009thermal}, 식~\eqref{eq:Ujcat} 로 환산하면 전체 반응 엔트로피 스케일
$F(0.00083)\approx80$ J/(mol\,K) 이다. 이 값은 configurational+vibrational+electronic 합산 검산이다.
따라서 전자항 단독 부호를 이 값으로 고정하면 안 된다; 전자항 부호는 \S\ref{sec:electronicentropy} 의
삽입 기준 $\partial S_e/\partial x$ 로 따로 정한다.
...
\subsection{LCO order--disorder 와 MIT 2상역 — 같은 $\Omega$ 언어}
LCO 의 T2/T3 peak 는 graphite staging 이 아니라 Li/vacancy order--disorder 전이에서 나온다
\cite{reimers1992,xia2007,ml2024lco}. 그래도 Ch1 이 쓰는
정규용액 골격은 그대로다. LCO 자리 점유율을 $x$ 또는 진행률 $\xi$ 로 잡으면, 평균장 자유에너지의 상호작용 몫은
\begin{equation}
g_j^\mathrm{cat}(\xi)=g_j^{0,\mathrm{cat}}+
RT\big[\xi\ln\xi+(1-\xi)\ln(1-\xi)\big]
\Omega_j^\mathrm{cat}\xi(1-\xi),
\label{eq:gxicathode}
\end{equation}
이고 $\Omega_j^\mathrm{cat}>2RT$ 일 때 같은 spinodal gap 식~\eqref{eq:dUhys} 가 적용된다. T1 MIT 구간
($x\approx0.94$--$0.75$)은 전자 상태밀도 변화가 함께 켜지는 2상역이고
\cite{menetrier1999,motohashi2009}, T2/T3 는 monoclinic 정렬을 동반하는
order--disorder 쌍이다. 그러므로 LCO 에서 $\Omega^\mathrm{cat}$ 는 ``흑연 staging 의 복사''가 아니라, LCO 의
Li/vacancy 정렬과 metal--insulator two-phase width 를 같은 수학적 자리로 담는 피팅 파라미터다.
...
위 유도는 kinetic 경로다. 같은 결과는 단일 자리 grand-canonical 분포로도 닫힌다. 진행 상태 $n=1$ 과 미진행 상태
$n=0$ 의 몰 자유에너지 차를 $\Delta\mu_j$ 라 두고, $\beta_m\equiv1/(RT)$ 로 쓰면
\begin{equation}
Z_j=1+\exp(-\beta_m\Delta\mu_j),\qquad
P(n=1)=\frac{\exp(-\beta_m\Delta\mu_j)}{Z_j}.
\label{eq:xidistZ}
\end{equation}
평형 구동력 정의상 $\Delta\mu_j=-\mathcal A_j=-\sigma_dF(V-U_j^{\,d})$ 이므로
\begin{equation}
P(n=1)=\frac{1}{1+\exp[-\sigma_dF(V-U_j^{\,d})/(RT)]}=\xi_{\eq,j}
\quad(\Omega_j=0,\ n_j=1).
\label{eq:xidist}
\end{equation}
폭 다중도 $n_j$ 는 $RT/F$ 를 $w_j=n_jRT/F$ 로 넓히는 coarse-graining 이고, $\Omega_j\ne0$ 은
\S\ref{sec:hys} 의 평균장 상호작용으로 같은 분포의 자유에너지 지형을 비튼다. 따라서 $\xi_\eq$ 는 단순한 curve-fit
S-curve 가 아니라 평형 점유 확률 분포다. 이 문장은 두 경로를 하나로 묶는다: kinetic 에서는 detailed balance 의
정지점, thermo 에서는 $g'(\xi)=0$ 의 최소점, LCO 전자항에서는 다음 절의 Fermi--Dirac 전자 점유 분포가 같은
``점유 분포'' 언어로 이어진다.
...
% ====================================================================
\section{LCO 전자 엔트로피 항 — Fermi--Dirac 분포에서 (N5 뒤)}\label{sec:electronicentropy}
% ====================================================================
흑연은 Ch1 범위에서 configurational 항과 vibrational baseline 으로 닫을 수 있다. LCO 는 다르다. LiCoO$_2$ 는
$x\simeq1$ 에서 insulating 에 가깝고, 탈리튬화가 진행되어 $x$ 가 내려가면 T1 구간에서 metal--insulator transition(MIT)
이 켜진다 \cite{menetrier1999,motohashi2009,xia2007}. 그러면 Li/vacancy 점유 분포 위에 전도 전자의 Fermi--Dirac
점유 분포가 추가 엔트로피를 만든다 \cite{reynier2004entropy,ml2024lco}.
...
\label{eq:fermi}
\end{equation}
이고, 상태밀도 $g(E,x)$ 를 갖는 축퇴 전자계의 엔트로피는 점유/비점유 이진 분포의 Shannon entropy 적분이다:
\begin{equation}
S_e(T,x)=-k_B\int g(E,x)\{f\ln f+(1-f)\ln(1-f)\}\,\dd E.
\label{eq:Seint}
\end{equation}
$k_BT$ 가 전자 band scale 보다 작고 $g(E,x)$ 가 $E_F$ 근방에서 완만하면 Sommerfeld 전개가 $E_F$ 근방
$\sim k_BT$ 폭의 상태만 남긴다. leading term 은
\begin{equation}
\boxed{\;S_e(T,x)=\frac{\pi^2}{3}k_B^2T\,g(E_F,x)\;}
\label{eq:SeSommerfeld}
\end{equation}
이다. 핵심은 $S_e\propto T$ 라는 점이다. 중심 표준 엔트로피처럼 상수로만 두면 MIT 구간의 온도 의존을 잃는다.

부분몰 전자 엔트로피는 좌표 정의를 분리해 써야 부호가 흐려지지 않는다. 본 문건의
$\Delta S_{\rxn,j}$ 슬롯은 삽입 반응 엔트로피이므로, LCO 전자항도 \emph{삽입 기준} $x\uparrow$ 로 둔다.
LCO 조성 $x$ 를 Li 함량으로 잡으면
\begin{equation}
\Delta S_e^{\mathrm{ins}}(x,T)=\frac{\partial S_e}{\partial x}\Big|_T
=\frac{\pi^2}{3}k_B^2T\,\frac{\partial g(E_F,x)}{\partial x}.
\label{eq:dSedx}
\end{equation}
MIT 에서 $x$ 가 내려갈수록 $g(E_F)$ 가 증가하므로 $\partial g/\partial x<0$ 이고,
\begin{equation}
\boxed{\;\Delta S_e^{\mathrm{ins}}(x,T)<0\quad\text{(삽입 기준, $\Delta S_{\rxn,j}$ 와 일관)}\;.}
\label{eq:dSeinsign}
\end{equation}
물리적으로 같은 변화를 탈리튬화 진행 좌표 $\zeta=1-x$ 로 쓰면 부호가 반대다:
\begin{equation}
\Delta S_e^{\mathrm{delith}}(x,T)=\frac{\partial S_e}{\partial \zeta}\Big|_T
=-\frac{\pi^2}{3}k_B^2T\,\frac{\partial g(E_F,x)}{\partial x}.
\label{eq:dSedzeta}
\end{equation}
즉 Li 제거 좌표의 기여 $\Delta S_e^{\mathrm{delith}}$ 는 양의 봉우리로 나타난다. Reynier--Fultz 계열의
전자 엔트로피 크기 anchor $\sim0.18\,k_B$/atom 은 이 탈리튬화 양의 변화로 읽고, 삽입 기준 plug-in 에서는
그 부호를 뒤집어 넣는다 \cite{reynier2004entropy}. 양극 전위 부호는 여전히 식~\eqref{eq:Ujcat} 의
$\partial U^\mathrm{cat}/\partial T=\Delta S^\mathrm{cat}/F$ 를 따른다.

\subsection{MIT-logistic gate — 모델 가정과 tier}
연속 함수 $g(E_F,x)$ 는 Ch1 입력 문헌만으로 정밀하게 고정되지 않는다. 따라서 tier 정직한 모델은 MIT 를 smooth gate 로
근사하고, 그 위치와 폭을 데이터로 피팅하는 것이다:
\begin{equation}
...
\]
이 초기값은 문헌 anchor 이며, $g_{\max}$, $x_\mathrm{MIT}$, $\Delta x_\mathrm{MIT}$ 는 피팅 대상이다. 미분하면
\begin{equation}
...
\label{eq:gmitprime}
\end{equation}
따라서 전자 엔트로피 plug-in 은 T1 MIT 부근에 국소화된 $T$-선형 항이다. 완전 metallic anchor 의 크기 검산은
$S_e/k_B=(\pi^2/3)(k_BT)g(E_F)\approx3.29\times0.0259\times13\approx1.1\,k_B$/atom 이고,
O3 내부의 부분몰 차는 물리적으로 탈리튬화 시 약 $+0.18\,k_B$/atom 방출, 삽입 기준으로는 약
$-0.18\,k_B$/atom 수준으로 작다
\cite{reynier2004entropy}. 작지만 T1 2상역은 configurational 항만으로 재현하기 어렵기
때문에 gate 자체는 필수 성분으로 둔다 \cite{ml2024lco}. 흑연에는 대응되는 sharp MIT 가 없으므로 Ch1 graphite 기본 모델에서는
$\Delta S_e=0$ 으로 둔다.

\begin{figure}[t]
\centering
\begin{tikzpicture}[x=9.0cm,y=2.5cm]
\draw[-{Latex[length=1.5mm]}] (0,0) -- (1.04,0) node[below,font=\scriptsize] {$x$ in Li$_x$CoO$_2$};
\draw[-{Latex[length=1.5mm]}] (0,0) -- (0,1.2) node[above,font=\scriptsize] {scaled value};
\foreach \xx/\lab in {0.75/{0.75},0.85/{0.85},0.95/{0.95}} {
  \draw (\xx,0.025)--(\xx,-0.025) node[below,font=\scriptsize] {\lab};
}
\draw[densely dotted] (0.85,0) -- (0.85,1.05) node[above,font=\scriptsize] {$x_\mathrm{MIT}$};
\draw[thick] plot[smooth] coordinates {(0.72,1.05) (0.76,0.98) (0.80,0.88) (0.84,0.60) (0.85,0.50) (0.86,0.40) (0.90,0.14) (0.94,0.04) (0.98,0.01)};
\node[font=\scriptsize] at (0.77,1.12) {$g(E_F,x)/g_{\max}$};
\draw[thick,densely dashed] plot[smooth] coordinates {(0.72,0.05) (0.76,0.12) (0.80,0.33) (0.83,0.72) (0.85,1.00) (0.87,0.72) (0.90,0.33) (0.94,0.12) (0.98,0.05)};
\node[font=\scriptsize] at (0.93,0.62) {$-\partial g/\partial x$ peak};
\draw[-{Latex[length=1.2mm]},gray] (0.96,0.95) -- (0.75,0.95) node[pos=0.5,above,font=\scriptsize] {delithiation};
\end{tikzpicture}
\caption{LCO MIT electronic-entropy gate(new). Solid line is the model assumption $g(E_F,x)/g_{\max}$ in
Eq.~\eqref{eq:gmit}; dashed line is the Li-removal entropy kernel $-\partial g/\partial x$ from Eq.~\eqref{eq:gmitprime}.
Labels are initial tier values, not fixed truths.}
\label{fig:lco-electronic-gate}
\end{figure}
...
\subsection{LCO 평형 peak — $3.9$ V 와 $4.05$--$4.20$ V}
LCO 에서도 peak 모양은 식~\eqref{eq:eqpeak} 그대로다. 표~\ref{tab:lco-staging-lit} 의 중심을 넣으면 T1 MIT peak 는
$\sim3.90$ V 부근에, T2/T3 order--disorder peak 쌍은 $\sim4.05$ V 와 $\sim4.17$--$4.20$ V 부근에 선다.
폭은 $w_j^\mathrm{cat}$ 또는 $\Omega_j^\mathrm{cat}$ 로, 면적은 $Q_j^\mathrm{cat}$ 로, 도핑에 의한 smear/shift 는
$w_j^\mathrm{cat}$ 증가와 $U_j^\mathrm{cat}$ 이동으로 흡수한다. T1 에서는 \S\ref{sec:electronicentropy} 의
$\Delta S_e(x,T)$ gate 가 $U_1^\mathrm{cat}(T)$ 의 온도 기울기에 추가로 들어가므로, 같은 $dQ/dV$ peak 라도
$\partial U/\partial T$ 해석에는 electronic 성분이 포함된다.

\begin{figure}[t]
\centering
\begin{tikzpicture}[x=3.7cm,y=1.15cm]
\draw[-{Latex[length=1.5mm]}] (3.78,0) -- (4.28,0) node[below,font=\scriptsize] {$U$ vs Li [V]};
\draw[-{Latex[length=1.5mm]}] (3.78,0) -- (3.78,3.1) node[above,font=\scriptsize] {$dQ/dV$ [arb.]};
\foreach \xx/\lab in {3.90/{3.90},4.05/{4.05},4.17/{4.17},4.20/{4.20}} {
  \draw (\xx,0.04)--(\xx,-0.04) node[below,font=\scriptsize] {\lab};
}
\draw[thick] plot[smooth] coordinates {(3.82,0.05) (3.86,0.28) (3.88,1.10) (3.90,2.25) (3.92,1.10) (3.95,0.25) (3.99,0.08)};
\draw[thick] plot[smooth] coordinates {(4.00,0.06) (4.03,0.70) (4.05,1.75) (4.07,0.75) (4.10,0.12)};
\draw[thick] plot[smooth] coordinates {(4.12,0.08) (4.15,0.45) (4.175,1.45) (4.20,0.70) (4.23,0.12)};
\node[font=\scriptsize] at (3.90,2.55) {T1 MIT};
\node[font=\scriptsize] at (4.05,2.05) {T2};
\node[font=\scriptsize] at (4.18,1.78) {T3};
\draw[densely dashed] (4.24,0.05) -- (4.27,0.20);
\node[font=\scriptsize,align=center] at (4.22,2.55) {T4 high-V\\outside default};
\end{tikzpicture}
\caption{LCO half-cell equilibrium peak locations(new, schematic). The reusable peak formula is
Eq.~\eqref{eq:eqpeak}; only transition centers, widths, and the T1 electronic-entropy plug-in change.}
\label{fig:lco-dqdv}
\end{figure}
...
\subsection{LCO 엔트로피 분해와 코드 일반화}
LCO 전이 dict 도 같은 키를 쓴다 \cite{xia2007}. 다른 점은 $\Delta S_{\rxn,j}^\mathrm{cat}$ 를 한 숫자로 끝내지 않고, 중심 표준값과
LCO 고유 electronic plug-in 을 분리해 생각해야 한다는 것이다 \cite{reynier2004entropy,wang2009thermal}:
\begin{equation}
\boxed{\;
\Delta S_{\rxn,j}^\mathrm{cat}(x,T)=
\underbrace{\Delta S_{j}^{0,\mathrm{config}}}_{\text{center standard only}}
+\underbrace{\Delta S_{j}^{\mathrm{vib}}}_{\text{baseline}}
+\underbrace{\Delta S_e(x,T)}_{\text{MIT gate, insertion basis, } \propto T}\; .}
\label{eq:lco-dS-decomp}
\end{equation}
이중계산 금지의 기준은 다음이다. 식~\eqref{eq:xieq} 와 폭 $w_j$ 는 이미 $R\ln[(1-\xi)/\xi]$ 형태의
configurational 점유 분포를 만든다. 따라서 \code{dS\_rxn} 에 넣는 $\Delta S_j^{0,\mathrm{config}}$ 는 전이 중심의
표준값이고, peak 내부의 조성별 mixing 로그를 다시 더하지 않는다. $\Delta S_e(x,T)$ 는 식~\eqref{eq:dSeinsign} 의
삽입 기준 음의 전자항이며, 탈리튬화 기준 양의 봉우리를 그대로 넣는 항이 아니다. 이 항만 LCO T1 MIT 구간에서
\S\ref{sec:electronicentropy} 의 gate 로 추가한다. 흑연은 이 항을 $0$ 으로 둔다.

MSMR 표기와 Ch1 표기는 동형이다:
\begin{equation}
...
\label{eq:msmr-ch1}
\end{equation}
그러므로 forward 코드는 구조를 바꾸지 않는다. \code{GRAPHITE\_STAGING\_LIT} 를 \code{LCO\_STAGING\_LIT} 로 교체하고,
T1 에 $\Delta S_e(x,T)$ 를 plug-in 하며, $U_j^\mathrm{cat}$, $w_j^\mathrm{cat}$, $\Omega_j^\mathrm{cat}$,
$Q_j^\mathrm{cat}$ 를 우리 half-cell 데이터에 맞춰 피팅하면 된다. Al/Mg 같은 비-redox 도핑은 Co redox 와
전자항 자체를 새 항으로 바꾸는 것이 아니라, 전이 폭 증가, peak smear, $U_j$ shift, 약한 configurational 보정으로
피팅한다. G1 연속 $\Delta S(x)$, G2 $g(E_F,x)$, G3 MSMR LCO 파라미터, G4 도핑 shift 는 모두 이 round-trip 피팅에서
식별할 gap 으로 남긴다.
...
\bibitem{reimers1992} J. N. Reimers and J. R. Dahn, \emph{J. Electrochem. Soc.} \textbf{139}, 2091--2097 (1992). DOI: 10.1149/1.2221184.
\bibitem{menetrier1999} M. Ménétrier, I. Saadoune, S. Levasseur, and C. Delmas, \emph{J. Mater. Chem.} \textbf{9}, 1135--1140 (1999). DOI: 10.1039/a900016j.
\bibitem{motohashi2009} T. Motohashi, T. Ono, Y. Sugimoto \emph{et al.}, ``Electronic phase diagram of the layered cobalt oxide system, Li$_x$CoO$_2$ ($0\le x\le1$),'' \emph{Phys. Rev. B} \textbf{80}, 165114 (2009). DOI: 10.1103/PhysRevB.80.165114; arXiv:0909.3556.
\bibitem{xia2007} H. Xia, L. Lu, Y. S. Meng, and G. Ceder, ``Phase Transitions and High-Voltage Electrochemical Behavior of LiCoO$_2$ Thin Films Grown by PLD,'' \emph{J. Electrochem. Soc.} \textbf{154}, A337--A342 (2007). DOI: 10.1149/1.2509021.
\bibitem{reynier2004entropy} Y. Reynier, J. Graetz, T. Swan-Wood, P. Rez, R. Yazami, and B. Fultz, ``Entropy of Li intercalation in Li$_x$CoO$_2$,'' \emph{J. Electrochem. Soc.} \textbf{151}, A422 (2004); \emph{Phys. Rev. B} \textbf{70}, 174304 (2004). DOI: 10.1103/PhysRevB.70.174304.
\bibitem{wang2009thermal} J. Wang, Y. Reynier \emph{et al.}, ``Effect of entropy change of Li intercalation in cathodes and anodes on Li-ion battery thermal management,'' \emph{J. Power Sources} (2009), PII S0378775309021119.
\bibitem{ml2024lco} K. Garikipati group, ``Bridging scales with Machine Learning: from first-principles statistical mechanics to continuum phase field for order--disorder transitions in Li$_x$CoO$_2$,'' \emph{J. Mech. Phys. Solids} \textbf{190}, 105727 (2024). DOI: 10.1016/j.jmps.2024.105727; arXiv:2302.08991.
\end{thebibliography}
