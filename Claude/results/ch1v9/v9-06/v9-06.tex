...
\begin{verifybox}
\textbf{LCO 중심 부호 검산($T=298.15$ K).} 대표 단전극 엔트로피 계수 $\dd\phi/\dd T\approx+0.83$ mV/K\cite{reynier2009}
...
\label{eq:Se}
\end{equation}
로 닫힌다($g(E_F,x)$ = 자리당 Fermi 준위 상태밀도). \textbf{(d) 박스 — 전이당(삽입당) 전자 엔트로피.} forward 모델이
...
\label{eq:dSe}
\end{equation}
로 정의한다. \textbf{★부호 규약(★최우선 결함 클래스).} \emph{삽입 기준이며, $\Delta S_{\rxn,j}$ 슬롯(흑연 $2\!\to\!1$
...
(식~\eqref{eq:dSe}; 삽입 기준 $\Delta S_e=\partial S_e/\partial x<0$ 의 절댓값): 천이 중심의 \emph{국소
...
+\underbrace{\Delta S_{e,j}(x,T)}_{\text{MIT 게이트, 삽입 기준 }<0,\ \propto T\ (\text{식~\eqref{eq:dSe}})}\;.}
\label{eq:lco-decomp}
\end{equation}
...
\item \textbf{electronic.} \S\ref{sec:lco-electronic} 의 신규 항 $\Delta S_{e,j}(x,T)<0$(식~\eqref{eq:dSe}, \emph{삽입
기준} — $\Delta S_{\rxn,j}$ 슬롯과 일관; 탈리튬화 시 $|\Delta S_{e,j}|\approx0.18\,k_B$/atom 방출). 흑연엔 0, LCO 는 T1 의
MIT 게이트에서만 켜진다. 셋 중 유일하게 $\propto T$ 라, 다온도 피팅에서 다른 둘과 분리 식별된다.
\end{itemize}
...
N5$+$ & $\Delta S_{e,j}=\tfrac{\pi^2}{3}k_B^2T\,\partial g/\partial x<0$ (삽입) & \eqref{eq:dSe},\eqref{eq:ggate} & $\Delta S_e$ plug-in(전자항) \\
...
\bibitem{menetrier1999} M. M\'en\'etrier, I. Saadoune, S. Levasseur, C. Delmas, ``The insulator--metal transition upon lithium deintercalation from LiCoO$_2$,'' \emph{J. Mater. Chem.} \textbf{9}, 1135 (1999). DOI: 10.1039/a900016j.
...
\bibitem{reynier2004} Y. Reynier, J. Graetz, T. Swan-Wood, P. Rez, R. Yazami, B. Fultz, ``Entropy of Li intercalation in Li$_x$CoO$_2$,'' \emph{Phys. Rev. B} \textbf{70}, 174304 (2004). DOI: 10.1103/PhysRevB.70.174304. (동 그룹 관련: \emph{J. Electrochem. Soc.} \textbf{151}, A422 (2004).)
\bibitem{reynier2009} Wang, Y. Reynier \emph{et al.}, ``Effect of entropy change of lithium intercalation in cathodes and anodes on Li-ion battery thermal management,'' \emph{J. Power Sources} (2009). [Elsevier ID S0378775309021119; LCO 단일전극 $\dd\phi/\dd T\approx+0.83$ mV/K, 가역열. 정확 권/페이지는 full-text gated — tier B.]
\bibitem{ml2024} K. Garikipati group, ``Bridging scales with machine learning: from first-principles statistical mechanics to continuum phase-field computations of order--disorder transitions in Li$_x$CoO$_2$,'' \emph{J. Mech. Phys. Solids} \textbf{190}, 105727 (2024). DOI: 10.1016/j.jmps.2024.105727. arXiv: 2302.08991.
\bibitem{msmr2024} M. W. Verbrugge, D. R. Baker \emph{et al.}, ``Multiphase species and reaction (MSMR) model''; ``Quantifying the Temperature Dependence of the MSMR Model. Part I: Parameterization for MCMB Graphite,'' \emph{J. Electrochem. Soc.} (2024), DOI: 10.1149/2754-2734/ad7d1c; ``Part II,'' DOI: 10.1149/1945-7111/ad70d9.
\end{thebibliography}
