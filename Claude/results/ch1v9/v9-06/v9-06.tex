...
하프셀(코인, $\le$4.2--4.5 V)은 세 전이를 남긴다: $\sim$3.90 V 의 절연체$\to$금속 천이(MIT), $\sim$4.05 V 와
...
Xia$\cdot$Reynier$\cdot$Motohashi 에서 읽은 출발점), 우리 시료 데이터로 피팅해 override 하는 전제다.
...
\toprule
전이 & $U_j$ [V vs Li] & $x$ 범위 & 성격 & $\Delta S_{\rxn,j}^\mathrm{cat}$ 초기값 & 전자항(\S\ref{sec:lco-electronic}) \\
\midrule
...
\begin{verifybox}
\textbf{LCO 중심 부호 검산($T=298.15$ K).} 대표 단전극 엔트로피 계수 $\dd\phi/\dd T\approx+0.83$ mV/K[tier B,
Reynier thermal 2009 — LCO \emph{단일전극} potentiometric, 크기$\cdot$부호 스케일 검증용 초기값]를 식~\eqref{eq:lco-dUdT}
...
[tier A, Motohashi])가 표~\ref{tab:lco-staging} 의 config 주도 $\Delta S$ 의 출처다.
...
열려 \emph{금속}이 된다($g(E_F)\!\to\!$유한)[Ménétrier, Marianetti]. 이 절연체$\to$금속 천이(MIT)가 $x\approx0.75$--$0.94$
...
열폭($\sim k_BT$)에만 기여가 살아 Sommerfeld 전개로 닫힌다. \textbf{(c) 중간식 — Sommerfeld 전개로 $C_e$, 그리고 적분으로 $S_e$.} 상태밀도 $g(E)$ 가
$E_F$ 근방에서 완만하면, 표준 Sommerfeld 적분 $\int(-\partial f/\partial E)(E-E_F)^2\dd E=\tfrac{\pi^2}{3}(k_BT)^2$ 가
전자 \emph{비열}을 먼저 닫는다 — 내부에너지 $U_e=\int E\,g(E)f\,\dd E$ 의 온도 미분에 이 적분을 대입하면
...
\qquad[\text{표준 Sommerfeld 결과; }g(E_F)\text{ anchor 는 tier B--A}]
\label{eq:Se}
\end{equation}
로 닫힌다($g(E_F,x)$ = 자리당 Fermi 준위 상태밀도). \textbf{(d) 박스 — 전이당(탈리튬화당) 전자 엔트로피.} forward 모델이
$\Delta S_{\rxn,j}$ 자리에 쓰는 양은 ``전이 진행(탈리튬화, $x\!\downarrow$)당'' 엔트로피 변화다. $g(E_F)$ 의 조성 미분에
\emph{탈리튬화 방향의 부호}를 명시적으로 실어, 전이당 전자 엔트로피를
\begin{equation}
\boxed{\;\Delta S_{e,j}(x,T)\;\equiv\;-\frac{\partial S_e}{\partial x}\Big|_T
=-\frac{\pi^2}{3}\,k_B^2\,T\,\frac{\partial g(E_F,x)}{\partial x}\;\;(\ge0\ \text{at MIT})\;}
\label{eq:dSe}
\end{equation}
로 정의한다(앞의 $-$ 부호가 ``$x$ 감소 = 탈리튬화 진행''을 싣는다 — 흑연에서 진행률 $\xi=1-\theta$ 로 점유 미분과
진행당 부호가 한 번 갈리는 것과 같은 자리다). \textbf{부호.} 탈리튬화($x\!\downarrow$)로 절연체$\to$금속이라 $g(E_F)$ 가
$0\!\to\!g_{\max}$ 로 \emph{증가}하므로 $\partial g/\partial x<0$, 따라서 식~\eqref{eq:dSe} 의 $\Delta S_{e,j}=-\partial S_e/\partial x>0$
— MIT 구간에서 전이당 전자 엔트로피가 \emph{양의 봉우리}다(그림~\ref{fig:lco-electronic} 의 파선이 곧 이 양수
$-\partial g/\partial x$). 이 정의로 박스$\cdot$프레임 그림$\cdot$분해식(\eqref{eq:lco-decomp})이 모두 같은 양수
$\Delta S_{e,j}$ 를 가리킨다(부호 일관). \textbf{★온도 의존.} 식~\eqref{eq:dSe} 는 다른 항과 결정적으로 다르다 —
...
식~\eqref{eq:dSe} 를 쓰려면 $g(E_F,x)$ 의 \emph{연속 곡선}이 필요하나, 1차 문헌엔 그것이 없다 — Motohashi 등은
...
\Delta x_\mathrm{MIT})$ 이 피팅 대상이며, 식~\eqref{eq:dSe} 에 넣으면 $\Delta S_e(x)\propto-\partial\sigma/\partial x$ 가
MIT 부근의 \emph{국소 양봉우리}(작지만 0 아님)가 된다.
...
\item \textbf{$g_{\max}=13$ e/eV — anchor.} 완전 탈리튬 $\mathrm{CoO_2}$ 의 metallic $g(E_F)$ 가 이 값이다[Motohashi,
tier A 단일점]. 곧 게이트의 ``켜진'' 높이는 측정된 끝점이지 자유 파라미터가 아니다(초기값으로 고정, 도핑 시 소폭 조정).
\item \textbf{$x_\mathrm{MIT}\approx0.85$ — 천이 중심.} MIT 2상역이 $x\approx0.75$--$0.94$ 에서 관측되므로[Ménétrier$\cdot$
Reimers], 그 중앙 $\approx0.85$ 가 게이트 중심이다 — 천이가 일어나는 조성을 직접 읽은 값이다.
...
($T{=}300$ K)이고, O3 영역의 부분몰 차는 $\approx0.18\,k_B$/atom[Reynier, tier B] — 같은 자릿수다($\checkmark$). 이는
config 의 O3 ΔS(>$\tfrac12$ 지배[Reynier A])보다 \emph{작다}. 그럼에도 필수인 이유는 크기가 아니라 \emph{게이트}에 있다 —
config 단독으로는 MIT 2상역의 엔트로피 거동을 재현하지 못하고[ML 2024, tier A], 절연체$\to$금속의 전자 자유도가 켜지는
...
파선 $=$ 부분몰 전자 엔트로피 $\Delta S_e\propto-\partial g/\partial x$(식~\eqref{eq:dSe}): 천이 중심의 \emph{국소
...
+\underbrace{\Delta S_{e,j}(x,T)}_{\text{MIT 게이트, }\ge0,\ \propto T\ (\text{식~\eqref{eq:dSe}})}\;.}
\label{eq:lco-decomp}
\end{equation}
세 몫의 역할과 \emph{이중계산 방지}는 다음과 같다.
\begin{itemize}[leftmargin=1.4em,itemsep=2pt]
\item \textbf{config.} O3 영역에서 LCO $\Delta S$ 를 지배($>\tfrac12$[Reynier A])하며, 기존 Ch1 logistic 이 이를
...
\item \textbf{electronic.} \S\ref{sec:lco-electronic} 의 신규 항 $\Delta S_{e,j}(x,T)\ge0$(식~\eqref{eq:dSe}, 제거당
양수). 흑연엔 0, LCO 는 T1 의 MIT 게이트에서만 켜진다. 셋 중 유일하게 $\propto T$ 라, 다온도 피팅에서 다른 둘과 분리
식별된다.
\end{itemize}
...
reaction(MSMR) 모델은 양극 전위를 전이별 logistic 의 합으로 적는다:
\begin{equation}
...
가 1:1 대응한다. 따라서 Ch1 의 곡선 클래스(\code{func\_ksi\_eq}$\cdot$\code{func\_U\_j}$\cdot$합산~\eqref{eq:sum})는
...
\item \textbf{전자 엔트로피 항 plug-in} — T1 전이의 $\Delta S_{\rxn}$ 평가에 식~\eqref{eq:dSe} 의 $\Delta S_e(x,T)$ 를
더하는 한 줄(식~\eqref{eq:lco-decomp}). $g(E_F,x)$ 는 식~\eqref{eq:ggate} 의 게이트로, 초기값 3개를 피팅 인자로 노출.
\end{enumerate}
...
N5$+$ & $\Delta S_{e,j}=-\tfrac{\pi^2}{3}k_B^2T\,\partial g/\partial x$ & \eqref{eq:dSe},\eqref{eq:ggate} & $\Delta S_e$ plug-in(전자항) \\
