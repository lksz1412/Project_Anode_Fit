...
\caption{LCO 전이 초기값 \code{LCO\_STAGING\_LIT}(3건, tier A=Xia\,\cite{xia2018}·Reynier\,\cite{reynier2004}·Motohashi\,\cite{motohashi2019}) — 흑연 \code{GRAPHITE\_STAGING\_LIT} 와 동형. 전자 엔트로피 항 $\Delta S_e$ 는 \S\ref{sec:elec_entropy} 에서 추가된다.}
\label{tab:lco_staging}
\begin{tabular}{l c c c c c}
\toprule
전이 & $U_j^\cat$ [V vs Li] & $x$ 범위 & 성격 & $\Delta S_{\rxn}^\cat$ 초기값 [J/(mol\,K)] & 비고 \\
\midrule
T1 MIT        & $\approx3.90$ & $x\!\approx\!0.94$--$0.75$ & insulator$\to$metal & config $+$ $\Delta S_e$ (ON) & 전자항 발현 \\
T2 ord-dis\,a & $\approx4.05$ & $x\!\approx\!0.55$         & hex$\to$mono       & config 주도 ($\approx+0.47$ J/K·mol) & 2상역 \\
T3 ord-dis\,b & $\approx4.17$ & $x\!\approx\!0.48$         & mono$\to$hex       & config ($\approx+1.49$ J/K·mol) & T2 쌍 \\
\bottomrule
\end{tabular}
\end{table}
...
\cite{motohashi2019}. 이 두 전이는 한 쌍의 인접 peak($\approx4.05$ V 와 $\approx4.17$ V)로 나타나며,
...
문헌에 $g(E_F,x)$ 의 연속 곡선 정량이 없다(G2 갭 \cite{motohashi2019}: $\mathrm{CoO_2}$ 의 단일 anchor
...
\begin{itemize}[topsep=2pt,itemsep=1pt]
\item \textbf{config 항} $\Delta S_j^\config$: 격자기체 섞임 엔트로피 $= -R[\xi\ln\xi+(1-\xi)\ln(1-\xi)]$ 가 logistic 점유에서
자동으로 나온다. 전이 dict 의 $\Delta S_{\rxn,j}^\cat$(중심 표준값 $\Delta S_j^0 = \Delta S_\rxn|_{\xi=1/2}$)가 이를 포함 —
\textbf{이중계산 금지(B)}: $\Delta S_j^0$ 에 이미 config 가 들어 있으므로, 외부에서 config 를 별도로 더하면
이중계산이 된다. $\Delta S_j^0$ 은 \emph{config 를 제외한} 나머지($\Delta S^\vib+\Delta S_e$)의 중심 기여가 아니라
\emph{전체 반응 엔트로피의 중심값}이다.
\item \textbf{vib 항} $\Delta S_j^\vib$: 탈리튬화 시 phonon 스펙트럼 변화에서 오는 음의 baseline. $\Delta S_j^0$ 의 순수히
비-config 몫으로 피팅으로 정한다.
...
% ---- LCO 참고문헌 (v9-03 추가) ----
\bibitem{xia2018} H. Xia \emph{et al.}, ``Electrochemical behavior and Li-ion transport in LiCoO$_2$ thin-film electrodes,'' \emph{Electrochim. Acta} \textbf{225}, 573 (2018). [tier A: 전이 전위 초기값]
\bibitem{reynier2004} Y. Reynier, R. Yazami, B. Fultz, ``Thermodynamics of lithium intercalation into graphites and disordered carbons,'' \emph{J. Electrochem. Soc.} \textbf{151}, A422 (2004). [tier A/B: $\Delta S$ 정량·단전극 $+0.83$ mV/K]
\bibitem{motohashi2019} T. Motohashi \emph{et al.}, ``Electronic phase diagram of the layered cobalt oxide system Li$_x$CoO$_2$,'' \emph{Phys. Rev. B} \textbf{80}, 165114 (2009). [tier A: charge-order $\Delta S$·$g(E_F)$ anchor]
\bibitem{menetrier1999} M. M\'en\'etrier \emph{et al.}, ``The insulator-metal transition upon lithium depletion in LiCoO$_2$,'' \emph{J. Mater. Chem.} \textbf{9}, 1135 (1999). [tier A: MIT $x$ 범위·$g(E_F)$ 정성]
...
\bibitem{ml2024} [tier-inferred, DOI 미확정] ``Electronic entropy as an essential ingredient in LCO phase diagram prediction,'' \emph{추정: J. Power Sources 계열} (2024 추정). 주: DOI 미검증 placeholder — 본 인용의 핵심 논거(전자 엔트로피 없이 MIT 2상역 $\partial V/\partial T$ 재현 불가)는 Menetrier·Reynier [A] 에서 독립 지지됨. 실증 후 교체 요망.
