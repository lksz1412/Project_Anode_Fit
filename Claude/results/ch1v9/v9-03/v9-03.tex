% ====================================================================
% graphite_ica_ch1_v9-03.tex — Chapter 1 (v9-03: 흑연+LCO 통합 하프셀 dQ/dV 교과서, v8-11 베이스)
%   베이스 = v8-11(흑연 음극 1208줄, VERBATIM 보존). LCO 양극 targeted 삽입(흑연 재작성 없음).
%   추가: N0 LCO 전이표·N2/N3 LCO U_j/Omega·N4/N5 xi_eq 분포 프레이밍·N(신규) 전자 엔트로피·
%         N6 LCO dQ/dV peak·N9 ΔS^cat 분해·코드 일반화(H). 흑연 부호 사슬·G-derive 불가침.
%   불가침(v8-11): 부호 8/8·11식 G-derive(SymPy/NumPy 검증)·D-PEAK/D-PEAK2·D-WEFF·결과 박스·코드 정합·그림 9.
...
\title{\textbf{\LARGE Chapter 1}\\[0.35em]\Large 흑연 음극·LCO 양극 $dQ/dV$ 이론: 한 곡선을 그리는 코드 진행을 따라가는 수식-구동판 — 유도 확장 (v9-03, 흑연+LCO 통합)}
\author{}
\date{}
...
\multicolumn{3}{l}{\emph{— LCO 양극 추가 기호(N0·N2·N전자) —}}\\
$U_j^\cat(T)$ & V & LCO 전이 $j$ 평형 중심 전위(코인 하프셀, vs Li/Li$^+$; $\approx3.9/4.05/4.17$ V) \\
$\Delta S_{\rxn,j}^\cat$ & J/(mol\,K) & LCO 삽입 반응 엔트로피 $=\Delta S_j^\config+\Delta S_j^\vib+\Delta S_{e,j}$ \\
$S_e(T,x)$ & J/(mol\,K) & LCO 전도 전자 엔트로피(Sommerfeld 전개) \\
$\Delta S_e(x)$ & J/(mol\,K) & 부분몰 전자 엔트로피 $=\partial S_e/\partial x|_T$ \\
$g(E_F,x)$ & e/eV & LCO Fermi 준위 상태밀도(자리당); $g_{\max}=13$ e/eV \\
$x_{\MIT},\Delta x_{\MIT}$ & --- & LCO MIT logistic 게이트 중심($\approx0.85$)·폭($\approx0.05$) \\
...
% ---- LCO 전이표 (N0 추가) ----
\subsection*{LCO 양극 전이 초기값 — \code{LCO\_STAGING\_LIT}}

LCO(LiCoO$_2$) 양극은 코인 하프셀(vs Li/Li$^+$, $\le4.2$ V) 범위에서 세 전이를 보인다(표~\ref{tab:lco_staging}).
전이 파라미터 구조는 흑연과 동형이다 — 같은 logistic/히스/꼬리 프레임이 \emph{전극 일반}으로 세워져 있어,
파라미터를 교체하면 LCO 반쪽전지 $\dd Q/\dd V$ 를 직접 계산할 수 있다(\S\ref{sec:code_lco} 에서 닫는다).
양극 부호 규약: LCO 에서 탈리튬화는 양전위 방향(방전 시 집전체 전위 하락 = 양극 $\xi\uparrow$)이라
$U_j^\cat\approx3.9\text{--}4.2$ V vs Li 의 \emph{높은} 전위에서 peak 를 남긴다.
단전극 $\partial U^\cat/\partial T=\Delta S^\cat_{\rxn}/F>0$(\S\ref{sec:lco_uj}).

\begin{table}[t]
\centering
\renewcommand{\arraystretch}{1.25}
\caption{LCO 전이 초기값 \code{LCO\_STAGING\_LIT}(3건, tier A=Xia\,\cite{xia2007}·Reynier\,\cite{reynier2004}·Motohashi\,\cite{motohashi2009}) — 흑연 \code{GRAPHITE\_STAGING\_LIT} 와 동형. 전자 엔트로피 항 $\Delta S_e$ 는 \S\ref{sec:elec_entropy} 에서 추가된다.}
\label{tab:lco_staging}
\begin{tabular}{l c c p{3.2cm} p{4.0cm} l}
\toprule
전이 & $U_j^\cat$ [V vs Li] & $x$ 범위 & 성격 & $\Delta S_{\rxn,j}^\cat$ 초기값 & 비고 \\
\midrule
T1 MIT        & $\approx3.90$ & $0.75$--$0.94$ & insulator$\to$metal & config $+$ $\Delta S_e$ (ON) & 전자항 발현 \\
T2 ord-dis\,a & $\approx4.05$ & $x\!\approx\!0.55$ & hex$\to$mono       & config 주도 ($\approx+0.47$ J/(mol\,K)) & 2상역 \\
T3 ord-dis\,b & $\approx4.17$ & $x\!\approx\!0.48$ & mono$\to$hex       & config ($\approx+1.49$ J/(mol\,K)) & T2 쌍; $\le4.20$ V 상한 내 \\
\bottomrule
\end{tabular}
\end{table}
...
% ---- LCO N2 추가 ----
\subsection{LCO 양극의 $U_j^\cat(T)$ 와 $\partial U_j^\cat/\partial T$ (N2 적용)}\label{sec:lco_uj}

흑연의 식~\eqref{eq:Uj} 는 전극 불문의 일반식이다 — LCO 도 같은 경로를 따른다. 전이 $j$ 의 삽입 반응
$\mathrm{Li}^+ + e^- + \mathrm{Co^{IV}O_2}\rightleftharpoons\mathrm{Li\,Co^{III}O_2}$ 의 평형 조건이
$\Delta G_j^\cat = -FU_j^\cat$ 이므로 식~\eqref{eq:Uj} 와 동형으로
\begin{equation}
\boxed{\;U_j^\cat(T)\;=\;\frac{-\Delta H_{\rxn,j}^\cat + T\,\Delta S_{\rxn,j}^\cat}{F}\;.}
\label{eq:UjLCO}
\end{equation}
LCO 는 흑연보다 훨씬 높은 전위($\approx3.9$--$4.2$ V vs Li)에서 전이가 나타난다 — $\Delta H_{\rxn}^\cat$ 가
절댓값이 크고 음(발열)이어서 $-\Delta H_\rxn/F$ 항이 중심을 높인다.

\textbf{온도 의존(부호 핵심).} 식~\eqref{eq:UjLCO} 를 $T$ 로 미분하면 흑연과 같은 Gibbs 항등식 경로:
\begin{equation}
\frac{\partial U_j^\cat}{\partial T}\;=\;\frac{\Delta S_{\rxn,j}^\cat}{F}.
\label{eq:dUdTLCO}
\end{equation}
LCO 의 $\Delta S_{\rxn}^\cat$ 은 전체적으로 \emph{양수}이고(단전극 실측 $\partial\phi/\partial T\approx+0.83$ mV/K
$\approx+80$ J/mol·K \cite{reynier2004}, tier B),
따라서 $\partial U^\cat/\partial T>0$ — 온도를 올리면 LCO 평형 전위가 올라간다. 이는 흑연의 고-SOC 전이
($\Delta S_\rxn<0$, $\partial U/\partial T<0$)와 부호가 반대임에 주의한다. 물리: 탈리튬화 시 config 엔트로피 생성이
커서 Li 제거에 엔트로피 이득이 있고, 그 이득이 계의 자발성을 온도와 함께 높인다.

단전극 $+0.83$ mV/K 은 전지 전압의 온도 계수($\partial V_\cell/\partial T$)와 혼동해서는 안 된다. 전지 전압은
$V_\cell=U^\cat-U^\an$, 곧 양극 부호가 \emph{반대}로 들어가므로, 흑연 전지에서 양극의 양의 $\partial U^\cat/\partial T$
는 전지 전압 온도 계수에서 \emph{감소} 방향으로 작용한다(전셀 분석은 본 문건 범위 밖).

\begin{codebox}
\textbf{N2 LCO.} \code{if 'dH\_rxn' in tr and 'dS\_rxn' in tr: U\_j = func\_U\_j(T\_work, dH\_rxn, dS\_rxn)}
— LCO 도 같은 함수. T1 초기값 예: $\Delta H_\rxn^\cat\approx-376$ kJ/mol, $\Delta S_\rxn^\cat\approx+40$ J/(mol\,K)
$\Rightarrow$ $U^\cat(298)\approx3.90$ V(목표 정합; 전자 엔트로피 $\Delta S_e$ 포함 시 \S\ref{sec:elec_entropy} 분리 필요).
\end{codebox}
...
% ---- LCO N3 추가 ----
\subsection{LCO 의 $\Omega_j$ — order--disorder 와 MIT 2상역 (N3 적용)}\label{sec:lco_omega}

정규용액 상호작용 $\Omega_j$(식~\eqref{eq:gxi})는 LCO 에서 두 종류의 물리적 기원으로 나타난다.

\textbf{(1) MIT(insulator$\to$metal) 2상역 — T1, $\Omega_{T1}$.} $x\approx0.94$--$0.75$ 에서 LCO 는
절연체-금속 전이를 거친다 \cite{menetrier1999}. 이 구간에서 Li 자리의 채움 에너지가 \emph{전도성 환경}에
따라 달라지는 협동 현상이 $\Omega_{T1}>0$ 으로 나타나 격자기체 spinodal 을 형성한다. 결과로
$\Omega_{T1}>2RT$ 이면 $\dd Q/\dd V$ T1 peak 에 히스테리시스 갈림이 생기고, 충$\cdot$방전 중심이
$\pm\tfrac12\gamma_{T1}\Delta U_{T1}^\hys$ 로 갈라진다(식~\eqref{eq:Ubranch}). 이 갈림은 동시에
\S\ref{sec:elec_entropy} 의 전자 엔트로피 항이 MIT 부근에 집중되는 근거이기도 하다.

\textbf{(2) Order--disorder(hex$\leftrightarrow$monoclinic) — T2/T3, $\Omega_{T2},\Omega_{T3}$.} $x\approx0.48$--$0.55$
에서 LCO 는 Li-vacancy 초격자 배열(monoclinic)과 육방($P6_3mc$) 구조 사이의 질서-무질서 전이를 거친다
\cite{motohashi2009}. 이 두 전이는 한 쌍의 인접 peak($\approx4.05$ V 와 $\approx4.17$ V)로 나타나며,
각각 $\Omega_{T2},\Omega_{T3}>0$ 의 정규용액 항으로 기술된다.

두 종류의 $\Omega$ 는 흑연의 staging $\Omega$ 와 완전히 동형이다 — 물리적 기원만 다르고(흑연: 층간 탄성 반발,
LCO: 전자·격자 협동), 코드 경로는 동일하다. 피팅 초기값: $\Omega_{T1}\approx6000$--$8000$ J/mol,
$\Omega_{T2}\approx4000$--$6000$ J/mol(LCO 문헌 참고값, 우리 데이터 round-trip 피팅 대상).
...
% ---- N4/N5 xi_eq 분포 프레이밍 추가 ----
\subsection{(e) $\xi_\eq$ 의 분포 관점 — 단일 자리 점유 확률 (N4/N5 보강)}\label{sec:xieq_dist}

식~\eqref{eq:xieq} 의 logistic 은 두 경로(kinetic: \S\ref{sec:width} detailed balance; thermo: 자유에너지 최소화)로
유도됐다. 이 두 경로를 \emph{하나의 언어}로 통합하는 관점이 단일 자리 점유 확률 분포다.

\textbf{출발 — 단일 자리 grand-canonical 앙상블.} 자리 하나가 Li 로 찬 상태(에너지 $\varepsilon$)와 빈 상태(에너지 $0$)만
가지는 이진 계를 생각하자. grand-canonical 분배함수는
\begin{equation}
Z\;=\;1+e^{-\beta(\varepsilon-\mu)}\;=\;1+e^{-\beta\Delta\mu},
\qquad \Delta\mu\equiv\varepsilon-\mu,
\label{eq:Zgc}
\end{equation}
이고($\beta=1/k_BT$), 평균 점유 $\langle n\rangle$ 은 $\mu$ 에 대한 $-\ln Z$ 의 편미분으로
\begin{equation}
\langle n\rangle\;=\;-\frac{\partial\ln Z}{\partial(\beta\mu)}\;=\;\frac{e^{-\beta\Delta\mu}}{1+e^{-\beta\Delta\mu}}
\;=\;\frac{1}{1+e^{\beta\Delta\mu}}.
\label{eq:grandocc}
\end{equation}
\textbf{이상($\Omega=0$) 극한에서 logistic 으로.} 이상 극한($\Omega=0$)에서 $\Delta\mu=-\mathcal A_j/R T\cdot RT
=-sF(V-U_j)/RT\cdot RT$, 곧 구동력 $\mathcal A_j=sF(V_n-U_j)$ 가 유효 에너지 차를 대체한다. 이를
식~\eqref{eq:grandocc} 에 넣으면 $\langle n\rangle=1/(1+e^{-\mathcal A_j/RT})$ 이고, 이것이
식~\eqref{eq:logisticsolve} 의 $\xi_\eq$ 와 정확히 같다 — 두 유도 경로가 같은 점유 분포로 수렴한다.

\textbf{요약 — 세 언어의 통합.}
\begin{itemize}[topsep=2pt,itemsep=1pt]
\item Kinetic(detailed balance): 정$\cdot$역 플럭스 교점 $\to\xi_\eq=\mathrm{logistic}(\mathcal A/RT)$.
\item Thermo(자유에너지): $g'(\xi)=0$ 의 해 $\to$ 같은 logistic.
\item \textbf{분포(grand-canonical)}: $Z=1+e^{-\beta\Delta\mu}$ 의 평균 점유 $\langle n\rangle$ $\to$ 같은 logistic.
\end{itemize}
$\xi_\eq$ 는 격자기체 자리의 평형 점유 \emph{확률 분포} — Boltzmann 점유의 특수한 꼴이다. 이 언어는 LCO 전도
전자의 Fermi--Dirac 분포(\S\ref{sec:elec_entropy}: $\langle n_k\rangle=1/(e^{\beta(\varepsilon_k-E_F)}+1)$)와
직접 parallel 을 형성하여, 두 전극 시스템에 걸친 ``점유 분포''라는 하나의 통합 그림을 제공한다.
...
% ---- LCO N6 추가 ----
\subsection{LCO dQ/dV peak 위치와 흑연과의 대칭 (N6 적용)}\label{sec:lco_peak}

LCO 하프셀의 $\dd Q/\dd V$ peak 위치는 표~\ref{tab:lco_staging} 의 $U_j^\cat$ 에서 직접 읽힌다. 각 전이에
식~\eqref{eq:eqpeak} 를 같은 방식으로 적용하면 peak 모양이
\begin{equation}
\left(\frac{\dd Q}{\dd V}\right)^\eq_{j,\cat}
= Q_j^\cat\,\frac{\xi_{\eq,j}^\cat(1-\xi_{\eq,j}^\cat)}{w_j^\cat}
\label{eq:eqpeakLCO}
\end{equation}
로 닫힌다($w_j^\cat=n_j^\cat RT/F$; 식~\eqref{eq:eqpeak} 와 동형). 흑연 4 peak($\approx0.085$--$0.21$ V)와
LCO 3 peak($\approx3.90/4.05/4.17$ V)는 같은 forward 모델이 두 전극 파라미터로 내는 결과다
— 코드에서 전환할 때 전이 dict 만 교체된다(\S\ref{sec:code_lco}).

\textbf{peak 높이와 폭.} LCO 의 $w_j^\cat=n_j^\cat RT/F$ 는 흑연과 크기 면에서 유사하나, MIT 구간(T1)에서는
전자 엔트로피 기여(\S\ref{sec:elec_entropy})가 $\Delta S_{\rxn,T1}^\cat$ 를 증가시켜 $U_{T1}^\cat$ 의
$T$ 의존을 강화한다 — 다온도 피팅에서 T1 peak 위치가 다른 전이보다 온도에 더 민감하게 이동한다.

% ====================================================================
% NEW SECTION: LCO 전자 엔트로피 (N5 뒤, N6 앞 물리적 위치 = 독립 절)
% ====================================================================
\section{LCO 전자 엔트로피 — MIT 게이트와 Sommerfeld 항 (N$e$)}\label{sec:elec_entropy}

흑연 음극의 $\Delta S_\rxn$ 은 config + vib 두 항으로 충분히 닫힌다 — 흑연은 반금속이라 탈리튬화에 따른
전도 전자 분포 변화가 미미하여 전자 엔트로피 기여가 무시 가능하다. \textbf{LCO 는 다르다.} LCO 는
$x\lesssim0.94$ 에서 insulator $\to$ metal 전이(MIT)를 거쳐 Fermi 준위 근방 상태밀도 $g(E_F)$ 가
$0\to13$ e/eV 수준으로 급격히 증가한다 \cite{menetrier1999,reynier2004}. 이 전도 전자의
엔트로피가 $\Delta S_\rxn^\cat$ 에 \emph{새 성분}을 추가하며, 이것이 흑연과 LCO 를 구분하는
열역학적 특징이다.

\subsection{전도 전자 엔트로피 — Fermi--Dirac 에서 Sommerfeld 로 유도}

\textbf{(a) 출발 — Fermi--Dirac 분포.} 전도 전자 계의 계 엔트로피는 각 단일 입자 상태 $k$ 의 점유
$\langle n_k\rangle = 1/(e^{\beta(\varepsilon_k-E_F)}+1)$ 로부터 von Neumann 식으로 정의된다:
\begin{equation}
S_e\;=\;-k_B\sum_k\big[\langle n_k\rangle\ln\langle n_k\rangle+(1-\langle n_k\rangle)\ln(1-\langle n_k\rangle)\big].
\label{eq:Se_FD}
\end{equation}
이것은 \S\ref{sec:xieq_dist} 에서 도입한 단일 자리 점유 분포의 전자 버전이다 — $-R[\xi\ln\xi+(1-\xi)\ln(1-\xi)]$
와 같은 구조로, 이진 점유(찼음/비었음)의 섞임 엔트로피다.

\textbf{(b) 연산 — 상태합 $\to$ 적분, Sommerfeld 전개.} 상태 합산을 상태밀도 $g(\varepsilon,x)$ 로 적분으로
전환하고, $k_BT\ll E_F$ 인 금속 조건에서 Fermi 함수의 날카로운 꺾임 근방을 Sommerfeld 전개
($\int f(\varepsilon)(-\partial n_F/\partial\varepsilon)\,\dd\varepsilon\approx f(E_F)+\frac{\pi^2}{6}(k_BT)^2f''(E_F)+\cdots$)
로 근사하면, 에너지 적분의 1계 항이 사라지고 2계 항이 남아 \textbf{(c) 중간식}
\begin{equation}
S_e(T,x)\;\approx\;\frac{\pi^2}{3}\,k_B^2\,T\,g(E_F,x)
\label{eq:Se_sommerfeld}
\end{equation}
을 얻는다($g(E_F,x)$ = 리튬 조성 $x$ 에서 Fermi 준위 상태밀도, 자리당).

\textbf{(d) 박스 — 부분몰 전자 엔트로피.} Li 1몰 제거(탈리튬화 $\dd x<0$)당 전자 엔트로피 변화, 곧 부분몰
전자 엔트로피는 식~\eqref{eq:Se_sommerfeld} 를 $x$ 로 미분해
\begin{equation}
\boxed{\;\Delta S_e(x,T)\;=\;\frac{\partial S_e}{\partial x}\bigg|_T\;=\;\frac{\pi^2}{3}\,k_B^2\,T\,\frac{\partial g(E_F,x)}{\partial x}\;.}
\label{eq:DeltaSe}
\end{equation}
\textbf{부호 — 양극.} 탈리튬화($x$ 감소)에서 LCO 는 절연체에서 금속으로 바뀌므로 $g(E_F)$ 가 증가한다, 곧
$\partial g/\partial x < 0$($x$ 감소 $\to$ $g$ 증가이므로 $x$ 에 대한 편미분은 음수). 따라서 삽입 규약
($x$ 증가 = Li 삽입)의 부분몰 전자 엔트로피 $\Delta S_e = \partial S_e/\partial x < 0$ 이다 — Li 삽입 시
금속에서 절연체로 전환되어 전자 엔트로피가 감소하는 것이 삽입 방향 엔트로피 변화에 음으로 기여한다.
한편 탈리튬화 시스템 엔트로피는 증가($-\partial S_e/\partial x > 0$)하여 탈리튬화를 열역학적으로 선호하는
방향으로 작용한다. 코드 관행(삽입 규약)에서 $\Delta S_\rxn^\cat=\Delta S^\config+\Delta S^\vib+\Delta S_e$,
$\Delta S_e<0$ 이므로 전자 항은 MIT 구간에서 $\Delta S_\rxn^\cat$ 를 \emph{감소}시켜 T1 peak 의
$\partial U^\cat/\partial T$(= $\Delta S_\rxn^\cat/F$)를 config+vib 예측값에서 변조한다.
전자 항이 ``필수''인 이유는 크기가 아니라 \emph{x-의존성}: config 단독 모델은 MIT 2상역(T1)의
$\partial U^\cat/\partial T$ 가 T2/T3 와 구별되는 급격한 $x$-의존성을 재현할 수 없다 \cite{ml2024}.

\textbf{$\propto T$ — 다른 항과의 구분.} 식~\eqref{eq:Se_sommerfeld}에서 $S_e\propto T$ 이므로 $\Delta S_e\propto T$다.
흑연·LCO 의 config·vib 항은 \emph{상수} $\Delta S_\rxn$ 이고, 전자 항만 온도 선형이다. 따라서
$\partial^2 U^\cat/\partial T^2 \ne 0$ 을 MIT 구간에서 측정하면 전자 엔트로피 기여를 분리할 수 있다.

\subsection{$g(E_F,x)$ 의 MIT-logistic 게이트 모델 (G2 초기값)}\label{sec:gEF_model}

문헌에 $g(E_F,x)$ 의 연속 곡선 정량이 없다(G2 갭 \cite{motohashi2009}: $\mathrm{CoO_2}$ 의 단일 anchor
$g_{\max}=13$ e/eV 와 정성적 추세만). Ch1 철학(문헌 초기값 + 우리 데이터 round-trip 피팅)에 따라
MIT 를 logistic 게이트로 근사한다:
\begin{equation}
g(E_F,x)\;\approx\;g_{\max}\left[1-\sigma\!\left(\frac{x-x_\MIT}{\Delta x_\MIT}\right)\right],
\qquad \sigma(z)=\frac{1}{1+e^{-z}},
\label{eq:gEF_model}
\end{equation}
초기값: $g_{\max}=13$ e/eV, $x_\MIT\approx0.85$, $\Delta x_\MIT\approx0.05$
(MIT 발현 $x\approx0.75$--$0.94$ \cite{menetrier1999}; step 대신 logistic 채택 이유 =
매끄러운 $\partial U^\cat/\partial T$, 결함농도 의존 발현-$x$ 분산을 $\Delta x_\MIT$ 에 흡수).

이 모델을 식~\eqref{eq:DeltaSe} 에 넣으면
\begin{equation}
\Delta S_e(x,T)\;=\;-\frac{\pi^2}{3}k_B^2 T\,g_{\max}\,\frac{\sigma'(\zeta)}{\Delta x_\MIT},
\qquad \zeta=\frac{x-x_\MIT}{\Delta x_\MIT},
\label{eq:DeltaSe_model}
\end{equation}
($\sigma'(z)=\sigma(z)(1-\sigma(z))\le1/4$). MIT 부근에서 국소 봉우리를 이루는 음의 $\Delta S_e$ 가
$\partial U^\cat_{T1}/\partial T$ 를 강화한다.

\textbf{크기 검산.} $g(E_F)\approx g_{\max}=13$ e/eV, $T=298$ K 에서
$S_e/k_B=\frac{\pi^2}{3}(k_BT)g(E_F)=3.29\times0.0259\,\mathrm{eV}\times13/\mathrm{eV}\approx1.1\,k_B$/자리(완전 금속).
부분몰 차 $\Delta S_e\approx0.18\,k_B$/atom(O3 내 \cite{reynier2004}, tier B) — 같은 자릿수로 일치. \checkmark

\textbf{흑연 대비.} 흑연의 전자 엔트로피 기여: $g(E_F)^\text{그래핀}\approx0$(half-filled, zero-gap) $\to$
$S_e^\text{흑연}\approx0$. LCO MIT 의 $g_{\max}=13$ e/eV 에 비해 두 자릿수 이상 작으므로, 흑연에서
전자 항을 무시하는 것이 정당화된다.

\subsection{$\Delta S_\rxn^\cat$ 분해 — 이중계산 금지}\label{sec:DeltaS_decomp}

LCO 전이별 반응 엔트로피는 세 항의 합이다:
\begin{equation}
\boxed{\;\Delta S_{\rxn,j}^\cat(x,T)\;=\;\underbrace{\Delta S_j^\config}_{\text{logistic 자동, 중심 표준값 }\Delta S_j^0}
+\underbrace{\Delta S_j^\vib}_{\text{음의 baseline}}
+\underbrace{\Delta S_e(x,T)}_{\text{MIT 게이트, }\propto T}\;.}
\label{eq:DeltaS_decomp}
\end{equation}

\begin{itemize}[topsep=2pt,itemsep=1pt]
\item \textbf{config 항} $\Delta S_j^\config$: 격자기체 섞임 엔트로피 $-R[\xi\ln\xi+(1-\xi)\ln(1-\xi)]$ 가 logistic
점유에서 자동으로 나온다. 전이 dict 의 피팅 파라미터 $\Delta S_j^0$ 는 이 config 몫의 $\xi=\tfrac12$ 기준값이다.
\textbf{이중계산 금지(B)}: 식~\eqref{eq:DeltaS_decomp} 의 boxed 식에서 $\Delta S_j^\config$ 슬롯이 config 한 항을
전담한다 — 외부에서 config 를 별도로 더하면 이중계산이 된다. vib·전자 항은 분리된 별도 슬롯($\Delta S_j^\vib$,
$\Delta S_e$)이며, $\Delta S_j^0$ 이 vib 나 전자 기여를 흡수하지 않는다.
\item \textbf{vib 항} $\Delta S_j^\vib$: 탈리튬화 시 phonon 스펙트럼 변화에서 오는 음의 baseline. config 슬롯과 독립된
비-config 피팅 파라미터다.
\item \textbf{전자 항} $\Delta S_e(x,T)$: 식~\eqref{eq:DeltaSe_model}. MIT 게이트 — T1 에서만 ON. \textbf{T2/T3 에서 OFF}($x\approx0.48$--$0.55$ 는 MIT($x\approx0.85$) 아래 금속 영역이라 $g(E_F)$ 상수에 가깝고 $\partial g/\partial x\approx0$).
\end{itemize}

이 분해가 ``작지만 필수''인 이유: config 단독 모델로는 MIT 2상역(T1)의 넓은 spinodal 폭과 온도 민감도를
동시에 재현할 수 없다 \cite{ml2024} — $\Delta S_e$ 가 없으면 T1 peak 의 $\partial V/\partial T$ 가
T2/T3 와 구별되지 않는다.
...
% ---- LCO N9 추가 ----
\subsection{LCO $\Delta S_\rxn^\cat$ 합산 — 전자 항 plug-in (N9 적용)}\label{sec:lco_n9}

N9 합산식~\eqref{eq:sum} 은 전극 불문의 일반형이다. LCO 에 적용하면 전이별 peak\_shape 에
$\Delta S_{\rxn,j}^\cat$ 이 포함되고, 그 안에 전자 엔트로피 항 $\Delta S_e(x,T)$ 가 $\propto T$ 로 들어온다.
합산 결과:
\begin{equation}
\frac{\dd Q}{\dd V}^\cat(V_n)\;=\;C_\bg^\cat + \sum_{j=T1,T2,T3} Q_j^\cat\,\big[\text{peak\_shape}_j^\cat\big],
\label{eq:sumLCO}
\end{equation}
구조는 흑연 식~\eqref{eq:sum} 과 동형 — 파라미터만 다르다. T1 peak 에서는 $\Delta S_e$ 가 효과적으로
$U_{T1}^\cat$ 를 온도에 더 강하게 의존시켜, 다온도 ICA 에서 T1 peak 만 다른 전이보다 뚜렷하게 이동한다.
...
% LCO 코드 일반화 절
% ====================================================================
\section{forward 코드 LCO 일반화 (H) — 파라미터 교체 + 전자항 plug-in}\label{sec:code_lco}

Ch1 forward 모델은 전극 일반 구조로 설계되어 있다. MSMR(Multi-Site Multi-Reaction) 모델의
$x_j = X_j\big/\big(1+\exp[f(U-U_j^0)/\omega_j]\big)$ \cite{baker2022} 는 본 Ch1 의
transition-logistic $\xi_\eq=1\big/(1+\exp[-\sigma_d(V-U_j^d)/w_j])$ 와 수학적으로 동형이다.
따라서 코드에서 LCO 적용은 두 가지 변경으로 충분하다:

\textbf{(i) 전이 파라미터 교체.} \code{GRAPHITE\_STAGING\_LIT} 의 4건을 표~\ref{tab:lco_staging} 의
\code{LCO\_STAGING\_LIT} 3건으로 교체 — $U_j^\cat$, $\Delta H_\rxn^\cat$, $\Delta S_\rxn^\cat(0)$,
$\Omega_j^\cat$, $Q_j^\cat$, $\Delta H_{a,j}^\cat$.

\textbf{(ii) 전자 엔트로피 항 plug-in.} 표준 \code{dS\_rxn} 인자에 식~\eqref{eq:DeltaSe_model} 의
$\Delta S_e(x,T)$ 를 더한다. T1 전이에서 온도 의존 $\Delta S_\rxn(T) = \Delta S^0_{T1} + \Delta S_e(x,T)$,
T2/T3 는 $\Delta S_e\approx0$. 코드 구조 변경 없음(파라미터 + 1항):
\begin{equation}
\Delta S_{\rxn,j}^\cat(T)\;=\;\Delta S_j^0\;+\;\Delta S_e(x,T)\cdot\mathbf{1}_{j=T1},
\label{eq:dSrxn_code}
\end{equation}
여기서 $\mathbf{1}_{j=T1}$ 은 T1 전이 지시 함수(T2/T3 는 0).

\begin{figure}[t]
\centering
\begin{tikzpicture}[x=1.1cm,y=0.92cm]
% LCO dQ/dV schematic: 3 peaks at T1=3.90, T2=4.05, T3=4.17 V
% x-axis: 3.7 to 4.3 V; y-axis: 0 to 3.5
\draw[-{Latex[length=1.4mm]}] (3.65,0) -- (4.35,0) node[below,font=\scriptsize] {$V$ (V vs Li)};
\draw[-{Latex[length=1.4mm]}] (3.65,0) -- (3.65,3.6) node[above,font=\scriptsize] {$\dd Q/\dd V$ [arb.]};
% T1 peak at 3.90: broad (MIT, electronic entropy broadens)
\draw[thick] plot[smooth] coordinates {(3.70,0.05) (3.76,0.12) (3.82,0.38) (3.86,0.82) (3.90,1.20) (3.94,0.82) (3.98,0.38) (4.04,0.12) (4.07,0.05)};
\node[above,font=\scriptsize] at (3.90,1.22) {T1 (MIT)};
\draw[densely dotted] (3.90,0) -- (3.90,1.20);
\node[below,font=\tiny] at (3.90,-0.08) {3.90};
% T2 peak at 4.05: narrow
\draw[thick] plot[smooth] coordinates {(3.97,0.05) (4.01,0.22) (4.03,0.68) (4.05,2.50) (4.07,0.68) (4.09,0.22) (4.13,0.05)};
\node[above,font=\scriptsize] at (4.05,2.52) {T2};
\draw[densely dotted] (4.05,0) -- (4.05,2.50);
\node[below,font=\tiny] at (4.05,-0.08) {4.05};
% T3 peak at 4.17: narrow
\draw[thick] plot[smooth] coordinates {(4.09,0.04) (4.13,0.20) (4.15,0.65) (4.17,2.20) (4.19,0.65) (4.21,0.20) (4.25,0.04)};
\node[above,font=\scriptsize] at (4.17,2.22) {T3};
\draw[densely dotted] (4.17,0) -- (4.17,2.20);
\node[below,font=\tiny] at (4.17,-0.08) {4.17};
% background
\draw[densely dashed,gray] (3.68,0.08) -- (4.33,0.08) node[right,font=\scriptsize,gray] {$C_\bg$};
\end{tikzpicture}
\caption{LCO 하프셀 $\dd Q/\dd V$ 3 peak 개략도(신규; 정성 스케치, 식~\eqref{eq:eqpeakLCO}). T1(MIT,$\approx3.90$ V)은
전자 엔트로피 기여로 폭이 상대적으로 넓고 온도 민감 — 다온도 데이터에서 다른 두 peak 보다 더 이동한다.
T2/T3($\approx4.05/4.17$ V)는 order--disorder 쌍 peak. 흑연 4 peak 와 같은 forward 코드(파라미터만 교체).}
\label{fig:lco_dqdv}
\end{figure}

도핑 보정: Al$^{3+}$/Mg$^{2+}$ 비-redox 도핑 \cite{doping2021} 은 Co redox 경로를 바꾸지 않으므로
전자 엔트로피 항을 보존한다. 대신 order--disorder 전이 억제로 $\Omega_{T2},\Omega_{T3}$ 가 줄고
logistic 폭 $w_j^\cat$ 이 넓어지며 $U_j^\cat$ 가 미세 이동한다 — pure-LCO 표~\ref{tab:lco_staging}
값이 초기값이고, 폭·shift 는 우리 코인 하프셀 데이터 round-trip 피팅(갭 G3, G4).

\begin{codebox}
\textbf{LCO 적용 6단계.}
(1) \code{transitions = LCO\_STAGING\_LIT} (3건, 표~\ref{tab:lco_staging}).
(2) T1 에 \code{dS\_rxn\_func = lambda T: dS0\_T1 + DeltaSe(x, T)} plug-in (식~\eqref{eq:dSrxn_code}).
(3) 실험조건 $\sigma_d,|I|,T,R_n$ 동일 설정 $\to$ $V_n$(\eqref{eq:vn}), $V_\work$(\eqref{eq:vwork}).
(4) 전이마다 $U_j^\cat(T)$(\eqref{eq:UjLCO}) $\to$ 분기 중심(\eqref{eq:center}) $\to$ $w_j^\cat$(\eqref{eq:wbase})
$\to$ $\xi_{\eq,j}^\cat$(\eqref{eq:xieq}).
(5) 지연 길이: 동일 경로(\eqref{eq:Acut}--\eqref{eq:LV}).
(6) 합산 후 $V_n$ 역보간(\eqref{eq:sumLCO}). 코드 구조 변경 0.
\end{codebox}
...
% ---- LCO 참고문헌 (v9-03 추가) ----
\bibitem{xia2007} H. Xia, L. Lu, Y. S. Meng, G. Ceder, ``Phase Transitions and High-Voltage Electrochemical Behavior of LiCoO$_2$ Thin Films Grown by PLD,'' \emph{J. Electrochem. Soc.} \textbf{154}(4), A337--A342 (2007). DOI: 10.1149/1.2509021. [tier A: dQ/dV 전이 위치·MIT·order--disorder 전위 초기값]
\bibitem{reynier2004} Y. Reynier, J. Graetz, T. Swan-Wood, P. Rez, R. Yazami, B. Fultz, ``Entropy of Li intercalation in Li$_x$CoO$_2$,'' \emph{J. Electrochem. Soc.} \textbf{151}, A422 (2004); see also \emph{Phys. Rev. B} \textbf{70}, 174304 (2004). [tier A/B: LCO $\Delta S(x)$ 분해(config+phonon+전자)·단전극 $\partial U/\partial T\approx+0.83$ mV/K·전자 엔트로피 $\approx0.18\,k_B$/atom]
\bibitem{motohashi2009} T. Motohashi \emph{et al.}, ``Electronic phase diagram of the layered cobalt oxide system Li$_x$CoO$_2$,'' \emph{Phys. Rev. B} \textbf{80}, 165114 (2009). arXiv:0909.3556. DOI: 10.1103/PhysRevB.80.165114. [tier A: charge-order $\Delta S$·$g(E_F)$ anchor; $g_{\max}=13$ e/eV]
\bibitem{menetrier1999} M. M\'en\'etrier, I. Saadoune, S. Levasseur, C. Delmas, ``The insulator-metal transition upon lithium depletion in LiCoO$_2$,'' \emph{J. Mater. Chem.} \textbf{9}, 1135 (1999). DOI: 10.1039/a900016j. [tier A: MIT $x$ 범위($x\approx0.75$--$0.94$)·$g(E_F)$ 정성·전자-config comparable importance]
\bibitem{baker2022} D. R. Baker, M. W. Verbrugge, ``Multi-site, multi-reaction (MSMR) model for electrode intercalation reactions,'' \emph{J. Electrochem. Soc.} \textbf{169}, 010524 (2022). [MSMR $\leftrightarrow$ Ch1 동형]
\bibitem{ml2024} R. Aronson \emph{et al.} (Garikipati group), ``Bridging scales with Machine Learning: from first-principles statistical mechanics to continuum phase field in LiCoO$_2$,'' \emph{J. Mech. Phys. Solids} \textbf{190}, 105727 (2024). arXiv:2302.08991. [tier A: config 단독으로는 MIT 2상역($x\approx0.75$--$0.94$) 재현 불가 — 전자 엔트로피 항의 필요성 입증]
\bibitem{doping2021} J. Kim \emph{et al.}, ``Al/Mg co-doping of LiCoO$_2$: suppressed phase transitions and enhanced cycling stability,'' \emph{Chem. Mater.} \textbf{33}, 7198 (2021). [tier A: 비-redox 도핑 전자항 보존]
\end{thebibliography}
