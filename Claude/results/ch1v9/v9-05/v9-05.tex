...
값만 양극. $U_j$ 는 tier A(Xia); $\Delta S_\rxn$ 초기값은 tier B 거친 출발점(단전극 $\dd\phi/\dd T\!\approx\!+0.83$ mV/K
$\approx\!+80$ J/(mol\,K) 전역 평균을 전이별로 배분한 추정 — 연속 $\Delta S(x)$ 표 부재 = G1, 데이터 피팅으로 식별);
절대 $\Delta H_\rxn$ 은 G5 갭(OCV plateau anchor)이라 $U_j$ 직접값을 \code{'U'} 폴백으로 쓴다(흑연과 동일 메커니즘).}
\label{tab:lcostaging}
\begin{tabular}{l c c c c l}
\toprule
전이 & $U_j$ [V vs Li] & $x$ 범위(Li$_x$CoO$_2$) & 성격 & $\Delta S_\rxn$ 초기 [J/(mol\,K)] & 고유항 \\
\midrule
T1 MIT            & 3.90        & $0.94$--$0.75$ & insulator$\to$metal & $\sim\!+12$ (config $+\Delta S_e$ 게이트) & ★전자항 ON \\
T2 order--disorder $a$ & 4.05   & $\approx0.55$  & hex$\to$monoclinic & $\sim\!+5$ (config 주도) & 2상역 \\
T3 order--disorder $b$ & 4.17--4.20 & $\approx0.48$ & monoclinic$\to$hex & $\sim\!+15$ (config, T2 와 한 쌍) & peak 쌍 \\
\bottomrule
\end{tabular}
\end{table}
...
짚으면 — LCO 단전극의 측정 엔트로피 계수는 $\dd\phi/\dd T\approx+0.83$ mV/K(Reynier, tier B), 곧
$\Delta S_{\rxn}\approx F\cdot0.83\,\mathrm{mV/K}\approx+80$ J/(mol\,K) 규모로 \emph{전이별로 부호$\cdot$크기가 갈린다}
(MIT 발현 구간은 전자 엔트로피 게이트가 추가로 $+$ 기여 — \S\ref{sec:electronic}). 흑연의 $\Delta S_\rxn$
...
M{\'e}n{\'e}trier, Reimers). 곧 $g(E_F)$ 가 $0$ 에서 금속값($\sim13$ e/eV$\cdot$자리, CoO$_2$ anchor)으로 켜진다 —
이 켜짐이 전자 엔트로피 항을 \emph{국소적으로} 발현시킨다. ``흑연은 config$+$vib 로 닫히나 LCO 는 MIT 로 전자 분포가
엔트로피에 추가''라는 이 대비가, 두 전극을 한 챕터에 묶는 교육적 핵심이다.
...
\label{eq:Se}
\end{equation}
\textbf{(d) 부분몰 항 — Li 1몰 제거당.} 곡선에 들어가는 것은 전이당 \emph{변화}이므로, 조성 $x$ 미분이 전자 엔트로피의
부분몰 기여다:
\begin{equation}
...
\label{eq:dSe}
\end{equation}
탈리튬화(Li 제거, $x\!\downarrow$)로 절연체$\to$금속이라 $g(E_F)$ 가 $0\!\to\!13$ e/eV 로 \emph{증가}하므로
$\partial g/\partial x<0$, 곧 Li \emph{제거}당 $S_e$ 가 증가한다(부호: 양극 $\partial U_j/\partial T$ 에 $+$ 기여,
MIT 구간 국소). \textbf{★온도 의존 — 전자항만 $T$-선형.} 식~\eqref{eq:Se}$\cdot$\eqref{eq:dSe} 가 명시적으로
...
일차 문헌에 $g(E_F,x)$ 의 \emph{연속} 정량 곡선은 없다(G2; Motohashi 는 CoO$_2$ 의 $13$ e/eV 단일 anchor 와 정성
...
그대로 넣으면 $N_A$ 배만큼 어긋난다. 크기 감각: 완전 금속 $\Delta(S_e/k_B)\approx0.8\,k_B$/자리 $\Rightarrow$
$\approx0.8R\approx6.7$ J/(mol\,K) 규모(흑연 $\Delta S_\rxn$ 의 $+29\sim-16$ 과 같은 자릿수, 부호는 MIT 게이트가 정함).
...
O3 영역 부분몰차 $\approx0.18\ k_B$/자리(Reynier, tier B)와 같은 자릿수 — \emph{작지만 0 아님}이 확인된다.
\textbf{필수성:} config 단독으로는 MIT 2상역의 엔트로피 구조를 재현하지 못하므로(전자 상태밀도 켜짐이 빠짐), 이
...
$g\!\to\!13$ e/eV. 파선 $=$ 그 기울기 $|\partial g/\partial x|$, 곧 부분몰 전자 엔트로피 $\Delta S_e\propto T\,
\partial g/\partial x$(식~\eqref{eq:dSe})의 \emph{국소 양봉우리} — MIT 2상역($x\approx0.75$--$0.94$, 음영)에만 산다.
...
쌍(그림~\ref{fig:lcopeak}). 흑연이 $\sim0.085$--$0.21$ V 에 네 peak 을 남기는 것과 \emph{같은 종 모양}이며, 전위
...
\label{eq:dScat}
\end{equation}
\textbf{성분의 출처와 ★이중계산 금지.} (i) $\Delta S^{\mathrm{config},0}_j$ 는 O3 영역 ΔS 를 지배하며(Reynier, $>\tfrac12$),
...
\textbf{★코드 일반화 — MSMR 가 Ch1 logistic 과 동형(H).} 양극 분야 표준인 MSMR(Multi-Species Multi-Reaction) 모델의
