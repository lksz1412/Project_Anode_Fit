...
\hypersetup{colorlinks=true, linkcolor=blue, urlcolor=blue, citecolor=blue,
  pdftitle={Chapter 1 v9-09: 흑연 음극 dQ/dV 이론 + LCO 양극 보완}}
\pagestyle{fancy}\fancyhf{}
\lhead{Ch.1 v9-09 — 흑연 $dQ/dV$ + LCO 보완}\rhead{\thepage/\pageref{LastPage}}
...
\title{\textbf{\LARGE Chapter 1 v9-09}\\[0.35em]\Large 흑연 음극 $dQ/dV$ 이론: 한 곡선을 그리는 코드 진행을 따라가는 수식-구동판 — LCO 양극 보완 포함}
\author{}
\date{}
...
v9 의 추가 목적은 같은 진행을 LCO 양극 하프셀에도 적용하는 것이다. 흑연 전이, 부호, 그림, 초기값은 그대로 두고,
각 절의 끝에 LCO 대응을 덧붙인다. LCO 는 전위가 높다($\sim3.9$--$4.2$ V vs Li)와 metal--insulator transition(MIT) 때문에
전자 엔트로피 항이 필요하다는 점만 다르며, logistic 전이, $U_j(T)$, 폭, peak 합산, 꼬리 계산은 같은 forward 틀을 쓴다
\cite{reimers1992,menetrier1999,motohashi2009,xia2007,reynier2004,ml2024}.
...
LCO 양극도 같은 전이 리스트로 들어간다. 흑연의 \code{GRAPHITE\_STAGING\_LIT} 이 ``문헌값 = 초기값, 데이터 피팅으로
override'' 라는 철학을 갖듯이, LCO 는 \code{LCO\_STAGING\_LIT} 초기값을 둔다. 범위는 코인 하프셀 LCO vs Li 단독이며,
전셀 합성($\partial U_\cell/\partial T=\partial U_\mathrm{cat}/\partial T-\partial U_\mathrm{an}/\partial T$)은 여기서
하지 않는다.

\begin{table}[t]
\centering\footnotesize
\renewcommand{\arraystretch}{1.18}
\caption{LCO 전이 초기값 \code{LCO\_STAGING\_LIT}. $U_j^\mathrm{cat}$ 는 vs Li 전위이며, 수치는 문헌 초기값과 피팅 시작점이다.}
\label{tab:lco_staging}
\begin{tabularx}{\textwidth}{@{}L{0.14\textwidth}L{0.12\textwidth}L{0.16\textwidth}YY@{}}
\toprule
transition & $U_j^\mathrm{cat}$ [V] & $x$ range / center & character & entropy / fitting role \\
\midrule
T1 MIT & $\sim3.90$ & $x\approx0.94$--$0.75$ & insulator to metal & config $+$ electronic gate on; $g(E_F,x)$ parameters fitted \\
T2 order--disorder a & $\sim4.05$ & $x\approx0.55$ & hexagonal to monoclinic ordering & config dominated; $\Omega$ sets two-phase width \\
T3 order--disorder b & $\sim4.17$--$4.20$ & $x\approx0.48$ & monoclinic to hexagonal branch & paired high-voltage peak; width/shift fitted \\
T4 O3 to H1-3 & $\sim4.55$ & $x\approx0.2$ & high-voltage transition & outside the normal $\le4.2$--$4.5$ V Ch1 half-cell window \\
\bottomrule
\end{tabularx}
\end{table}

Al/Mg 같은 비-redox 도핑은 이 표를 새 물리로 바꾸지 않는다. pure-LCO 값은 초기값으로 두고, 상전이 억제는 logistic 폭
$w_j$ 증가, peak smear, $U_j$ shift 로 피팅한다.
...
\subsection{LCO 양극의 $U_j^\mathrm{cat}(T)$}
LCO 에서도 같은 식을 쓴다. 전이 중심만 흑연의 $0.08$--$0.21$ V 가 아니라 표~\ref{tab:lco_staging} 의
$3.9$--$4.2$ V vs Li 영역에 놓인다. 양극 단전극 전위라 해도 Gibbs 부호 사슬은 바뀌지 않는다:
\begin{equation}
U_j^\mathrm{cat}(T)=\frac{-\Delta H_{\rxn,j}^\mathrm{cat}
...
\label{eq:Ujcat}
\end{equation}
따라서 LCO 단전극에서 양의 $\Delta S_{\rxn,j}^\mathrm{cat}$ 는 양의
$\partial U_j^\mathrm{cat}/\partial T$ 로 읽힌다. 문헌 단전극 $d\phi/dT\approx+0.83$ mV/K 는 전체 부호 sanity 로만 쓰고,
개별 T1--T3 값은 문헌 초기값과 우리 데이터 round-trip 피팅으로 정한다 \cite{wang2009}. 이 sanity 는 config$+$vib$+$electronic
합산 기울기 검산이지 전자항 단독 부호 제약이 아니다. 전자 엔트로피가 들어가는 경우에도 경로는 같다:
그 항은 $\Delta S_{\rxn,j}^\mathrm{cat}$ 의 한 성분으로 더해져 식~\eqref{eq:Ujcat} 의 기울기를 바꾼다.
...
\subsection{LCO order--disorder 와 MIT 의 $\Omega$ 자리}
LCO T2/T3 의 hexagonal--monoclinic order--disorder 전이는 식~\eqref{eq:gxi} 의 정규용액 상호작용 $\Omega_j$ 로
같이 표현한다. $\Omega_j>2RT$ 이면 두 조성 가지가 갈라지고, $\gamma_j$ 는 실제 실험에서 spinodal 상한보다 작게 보이는
분기 축소를 흡수한다. T1 MIT 구간도 같은 두 상역 틀로 peak 를 만들 수 있지만, 여기서는 $\Omega_j$ 만으로 닫지 않는다:
절연체에서 금속으로 바뀌며 $g(E_F)$ 가 켜지므로, 새 전자 엔트로피 항이 $\Delta S_{\rxn,j}^\mathrm{cat}$ 에 더해진다
\cite{menetrier1999,motohashi2009,reynier2004,ml2024}(\S\ref{sec:electronic_entropy}). 곧 LCO 의 차이는 분기 식이 아니라
파라미터와 엔트로피 성분이다.
...
위 유도는 kinetic route(detailed balance)에서 logistic 을 얻었다. 같은 식은 단일 자리 grand-canonical 분포로도 닫힌다.
한 자리가 비어 있음($n=0$) 또는 차 있음($n=1$) 두 상태만 갖고, 점유의 상대 자유에너지 차를
$\Delta\mu\equiv\mu_\mathrm{empty}-\mu_\mathrm{filled}$ 로 쓰면 분배함수는
\begin{equation}
Z=1+\exp(-\beta\Delta\mu),\qquad \beta=(k_BT)^{-1},
\label{eq:xieqZ}
\end{equation}
이고 평균 점유 확률은
\begin{equation}
\langle n\rangle=\frac{\exp(-\beta\Delta\mu)}{1+\exp(-\beta\Delta\mu)}
=\frac{1}{1+\exp(\beta\Delta\mu)}.
\label{eq:xieqdist}
\end{equation}
$\Omega=0$ 이고 $\Delta\mu=-sF(V-U)$ 로 두면 식~\eqref{eq:xieqdist} 는 식~\eqref{eq:xieq} 와 같은 Fermi-함수형
점유가 된다. 따라서 $\xi_\eq$ 는 단순 curve-fit sigmoid 가 아니라 평형 점유 확률 분포다. 이 한 언어가
kinetic route(정$\cdot$역 플럭스 교점), thermo route($g'(\xi)=sF(V-U)$), LCO 전자 Fermi--Dirac 분포를 연결한다.
결과식, 부호, 코드는 바뀌지 않고 유도 해석만 보강된다.
...
% ====================================================================
\section{LCO 전자 엔트로피 항 — Fermi 분포에서 MIT 게이트로}\label{sec:electronic_entropy}
% ====================================================================
흑연은 이 장의 forward 모델에서 configurational entropy 와 vibrational baseline 만으로 충분하다. 반면 LCO 는
$x\approx1$ 의 절연 상태에서 탈리튬화로 금속성이 켜지는 MIT 를 갖기 때문에, 전도 전자 분포의 엔트로피가
$\Delta S_{\rxn}^\mathrm{cat}$ 의 추가 성분으로 들어간다. 이 절은 LCO 고유 plug-in 이며, 흑연 식은 바꾸지 않는다.

\subsection{Fermi--Dirac 분포에서 Sommerfeld 식까지}
\textbf{(a) 출발 — 전자 점유 분포.} 전도 전자는 Fermi--Dirac 분포
\begin{equation}
f(E)=\frac{1}{1+\exp[(E-E_F)/(k_BT)]}
\label{eq:fdist}
\end{equation}
로 점유된다. \textbf{(b) 엔트로피 적분.} 상태밀도 $g(E,x)$ 를 쓰면 전자 엔트로피는
\begin{equation}
S_e=-k_B\int g(E,x)\{f\ln f+(1-f)\ln(1-f)\}\,\dd E.
\label{eq:Seint}
\end{equation}
\textbf{(c) 저온 전개.} $k_BT$ 가 band scale 보다 작으면 $E_F$ 근방 폭 $\sim k_BT$ 만 열적으로 섞이고,
$g(E,x)\approx g(E_F,x)$ 로 둘 수 있다. Sommerfeld 전개는 그 좁은 창의 적분을
\begin{equation}
\int \{ -f\ln f-(1-f)\ln(1-f)\}\,\dd E
=\frac{\pi^2}{3}k_BT
\label{eq:sommerfeldwindow}
\end{equation}
로 준다. \textbf{(d) 박스.} 따라서 자리당 전자 엔트로피는
\begin{equation}
\boxed{\;S_e(T,x)=\frac{\pi^2}{3}k_B^2T\,g(E_F,x)\;}.
\label{eq:Se}
\end{equation}
핵심은 $S_e\propto T$ 이다. LCO 전자 기여는 상수 $\Delta S^0_j$ 와 달리 온도에 선형으로 커진다.

\subsection{부분몰 전자 엔트로피와 부호}
LCO 조성 $x$ 를 Li 함량으로 두면, Li 제거는 $x\downarrow$ 이다. 전자 엔트로피의 조성 기울기는 삽입 반응
$\Delta S_{\rxn,j}$ 슬롯과 같은 \emph{삽입 기준}으로 둔다:
\begin{equation}
\Delta S_e(x,T)=\frac{\partial S_e}{\partial x}\Big|_T
=\frac{\pi^2}{3}k_B^2T\,\frac{\partial g(E_F,x)}{\partial x}.
\label{eq:dSe}
\end{equation}
MIT 에서 탈리튬화가 진행되면 $g(E_F)$ 가 거의 $0$ 에서 금속값으로 증가한다. 따라서 삽입 방향($x\uparrow$)의
$\partial g/\partial x$ 는 음수이고,
\[
\boxed{\text{삽입 기준 }\Delta S_e=\partial S_e/\partial x<0}
\]
이다. 같은 사실을 탈리튬화 기준으로 읽으면 전자 entropy gain 이 양수이며, 문헌의 $\sim0.18\,k_B$/atom 전자항도
물리적으로는 탈리튬화 시 방출되는 양의 크기다 \cite{reynier2004}. 이 항은 양극 단전극 기울기
식~\eqref{eq:Ujcat} 의 같은 자리, 곧
$\partial U_j^\mathrm{cat}/\partial T=\Delta S_{\rxn,j}^\mathrm{cat}/F$ 로 들어가되, Ch1 의 삽입 반응
엔트로피 슬롯과 일관되게 음의 기여로 기록한다.

\subsection{MIT-logistic gate — 모델 가정과 초기값}
연속 $g(E_F,x)$ 곡선은 Ch1 입력으로 확정되어 있지 않으므로, 문헌 anchor 를 초기값으로 삼고 데이터로 피팅한다. gate 는
\begin{equation}
...
\end{equation}
\begin{equation}
g_{\max}=13\ \mathrm{e/eV},\qquad
x_\mathrm{MIT}\approx0.85,\qquad
\Delta x_\mathrm{MIT}\approx0.05.
\label{eq:gmitpars}
\end{equation}
$x\downarrow$ 으로 갈수록 bracket 이 $0\to1$ 로 켜진다. 식~\eqref{eq:dSe} 에 식~\eqref{eq:gmit} 를 넣으면
\begin{equation}
...
\label{eq:gmitderiv}
\end{equation}
즉 삽입 기준으로는 MIT 근방의 국소 음의 봉우리다. 완전 metal anchor 에서 $S_e/k_B\approx(\pi^2/3)(k_BT)g(E_F)
\approx3.29\times0.0259\times13\approx1.1$ 이고, O3 내부 부분몰 차는 $\sim0.18\,k_B$/atom 크기다. 작지만
configurational entropy 만으로 MIT 두 상역을 재현할 수 없으므로 gate 는 LCO 에 필수 plug-in 이다
\cite{motohashi2009,reynier2004,ml2024}.

\begin{figure}[t]
\centering
\begin{tikzpicture}[x=8.2cm,y=2.6cm]
\draw[-{Latex[length=1.6mm]}] (0,0) -- (1.02,0) node[below,font=\scriptsize] {$x$ in Li$_x$CoO$_2$};
\draw[-{Latex[length=1.6mm]}] (0,0) -- (0,1.18) node[above,font=\scriptsize] {scaled value};
\foreach \xx/\lab in {0.48/0.48,0.55/0.55,0.85/0.85,0.94/0.94} {
  \draw (\xx,0.02)--(\xx,-0.02) node[below,font=\scriptsize] {\lab};
}
\draw[thick] plot[smooth] coordinates {(0.45,0.98) (0.55,0.98) (0.65,0.97) (0.75,0.88) (0.80,0.73) (0.85,0.50) (0.90,0.27) (0.94,0.14) (0.98,0.06)};
\draw[densely dashed,thick] plot[smooth] coordinates {(0.45,0.03) (0.55,0.04) (0.65,0.07) (0.75,0.24) (0.80,0.60) (0.85,1.00) (0.90,0.60) (0.94,0.29) (0.98,0.12)};
\node[font=\scriptsize,align=left] at (0.59,0.83) {$g(E_F,x)$ gate};
\node[font=\scriptsize,align=left] at (0.70,1.08) {$|\partial g/\partial x|$};
\draw[densely dotted] (0.85,0)--(0.85,1.05);
\node[font=\scriptsize] at (0.85,-0.16) {$x_\mathrm{MIT}$};
\node[font=\scriptsize] at (0.83,0.18) {MIT};
\node[font=\scriptsize] at (0.54,0.20) {OD peaks};
\end{tikzpicture}
\caption{LCO electronic gate (new). Solid line: $g(E_F,x)$ model in Eq.~\eqref{eq:gmit}. Dashed line: local electronic entropy contribution proportional to $|\partial g/\partial x|$ in Eq.~\eqref{eq:dSe}. Labels are model anchors, not fitted results.}
\label{fig:lco_gate}
\end{figure}

\subsection{$\Delta S_{\rxn,j}^\mathrm{cat}$ 분해와 이중계산 금지}
LCO 전이의 온도계수는 한 숫자에 여러 물리가 섞인 것이므로 다음처럼 분해한다:
\begin{equation}
\boxed{\;\Delta S_{\rxn,j}^\mathrm{cat}(x,T)
=\Delta S_j^\mathrm{config}
+\Delta S_j^\mathrm{vib}
+\Delta S_e(x,T)\;.}
\label{eq:dScatdecomp}
\end{equation}
$\Delta S_j^\mathrm{config}$ 는 logistic 폭과 중심 표준값이 이미 담는 배치 엔트로피이고,
$\Delta S_j^\mathrm{vib}$ 는 phonon baseline 이며, $\Delta S_e$ 만 LCO MIT 고유 항이다. 피팅할 때
$\Delta S_j^0$ 를 중심 표준값으로 두고 logistic 내부의 $R\ln[(1-\xi)/\xi]$ 또는 식~\eqref{eq:dSe} 를 다시 같은
상수에 더하지 않는다. 이것이 이중계산 금지 규칙이다. 흑연은 metallic transition 이 없으므로 이 절의 plug-in 을
$0$ 으로 둔다.
...
LCO 의 평형 peak 도 같은 식~\eqref{eq:eqpeak} 이다. 다른 점은 $U_j$ 의 위치와 entropy plug-in 이다. T1 은
$\sim3.90$ V 의 MIT peak 로 나타나고, T2/T3 는 $4.05$--$4.20$ V 부근 order--disorder peak 쌍으로 나타난다.
그림~\ref{fig:lco_peaks} 는 코드가 보게 되는 초기 peak 위치 후보를 보인다. 실제 높이와 폭은 $Q_j,w_j,\Omega_j$ 와
도핑 smear/shift 피팅으로 정한다.

\begin{figure}[t]
\centering
\begin{tikzpicture}[x=2.65cm,y=1.35cm]
\draw[-{Latex[length=1.6mm]}] (3.74,0) -- (4.31,0) node[below,font=\scriptsize] {$U$ vs Li [V]};
\draw[-{Latex[length=1.6mm]}] (3.74,0) -- (3.74,2.55) node[above,font=\scriptsize] {$dQ/dV$};
\foreach \xx/\lab in {3.90/3.90,4.05/4.05,4.17/4.17,4.20/4.20} {
  \draw (\xx,0.04)--(\xx,-0.04) node[below,font=\scriptsize] {\lab};
}
\draw[thick] plot[smooth] coordinates {(3.77,0.04) (3.82,0.12) (3.86,0.55) (3.90,1.82) (3.94,0.58) (3.98,0.18) (4.02,0.22) (4.05,1.18) (4.08,0.36) (4.12,0.20) (4.17,1.35) (4.20,1.02) (4.25,0.18) (4.29,0.05)};
\draw[densely dashed] (3.90,0)--(3.90,1.82);
\draw[densely dashed] (4.05,0)--(4.05,1.18);
\draw[densely dashed] (4.17,0)--(4.17,1.35);
\node[font=\scriptsize] at (3.90,2.08) {T1 MIT};
\node[font=\scriptsize] at (4.05,1.45) {T2 OD};
\node[font=\scriptsize] at (4.19,1.62) {T3 OD};
\node[font=\scriptsize,align=center] at (4.13,2.25) {initial positions;\\fit width/shift};
\end{tikzpicture}
\caption{LCO half-cell peak candidates (new). T1 near 3.90 V is the MIT peak with electronic entropy gate; T2/T3 near 4.05 and 4.17--4.20 V are order--disorder peaks. The drawing is a forward-model initial map, not a fitted spectrum \cite{xia2007}.}
\label{fig:lco_peaks}
\end{figure}
...
\subsection{LCO 파라미터 교체와 전자항 plug-in}
LCO 는 별도 solver 가 아니라 전이 리스트와 entropy 함수의 교체다. MSMR 표기의 한 전이
\begin{equation}
x_j=\frac{X_j}{1+\exp[f(U-U_j^0)/\omega_j]}
\label{eq:msmrhomology}
\end{equation}
는 식~\eqref{eq:xieq} 의 transition-logistic 과 동형이다. 따라서 코드 구조는 유지하고,
\code{GRAPHITE\_STAGING\_LIT} 대신 표~\ref{tab:lco_staging} 기반 \code{LCO\_STAGING\_LIT} 를 넣는다. 차이는
전이 dict 의 $\Delta S_{\rxn,j}^\mathrm{cat}$ 평가가
\begin{equation}
\Delta S_{\rxn,j}^\mathrm{cat}(x,T)
=\Delta S_j^0+\Delta S_j^\mathrm{vib}+\Delta S_e(x,T)
\label{eq:dScatcode}
\end{equation}
형태로 확장된다는 점이다. 여기서 $\Delta S_j^0$ 는 중심 표준값이며, logistic 내부의 configurational slope 와 중복해서
더하지 않는다. $\Delta S_e$ 는 식~\eqref{eq:dSe}--\eqref{eq:gmitderiv} 의 MIT plug-in 이고, T1 에서 주로 켜지며 T2/T3 는
order--disorder $\Omega,w,U_j$ 피팅이 주 역할이다.

\begin{codebox}
\textbf{LCO code path.} 기존 전이 루프를 그대로 둔다. (1) \code{transitions=LCO\_STAGING\_LIT} 로 초기 중심
$3.90/4.05/4.17$ V, 폭, $Q_j$, $\Omega_j$ 를 공급한다. (2) \code{func\_U\_j} 에 들어가는 \code{dS\_rxn} 을
\code{dS0 + dS\_vib + dS\_electronic(x,T)} 로 평가한다. (3) \code{func\_ksi\_eq}, \code{func\_w},
\code{\_resolve\_lag\_length}, \code{\_causal\_lowpass}, \code{np.interp} 는 구조 변경 없이 재사용한다. Pure-LCO 값은
초기값이고, Al/Mg 도핑은 폭 증가와 $U_j$ shift 로 피팅한다.
\end{codebox}
...
N5e & LCO $S_e=(\pi^2/3)k_B^2Tg(E_F)$ & \eqref{eq:Se}--\eqref{eq:gmit} & \code{dS\_electronic} plug-in \\
...
\bibitem{menetrier1999} M. Ménétrier, I. Saadoune, S. Levasseur, and C. Delmas, ``The insulator--metal transition upon lithium deintercalation from LiCoO$_2$: electronic properties and $^7$Li NMR study,'' \emph{J. Mater. Chem.} \textbf{9}, 1135--1140 (1999). DOI: 10.1039/a900016j.
\bibitem{motohashi2009} T. Motohashi \emph{et al.}, ``Electronic phase diagram of the layered cobalt oxide system, Li$_x$CoO$_2$ ($0\le x\le1$),'' \emph{Phys. Rev. B} \textbf{80}, 165114 (2009). DOI: 10.1103/PhysRevB.80.165114; arXiv:0909.3556.
\bibitem{xia2007} H. Xia, L. Lu, Y. S. Meng, and G. Ceder, ``Phase transitions and high-voltage electrochemical behavior of LiCoO$_2$ thin films grown by PLD,'' \emph{J. Electrochem. Soc.} \textbf{154}(4), A337--A342 (2007). DOI: 10.1149/1.2509021.
\bibitem{reynier2004} Y. Reynier, J. Graetz, T. Swan-Wood, P. Rez, R. Yazami, and B. Fultz, ``Entropy of Li intercalation in Li$_x$CoO$_2$,'' \emph{J. Electrochem. Soc.} \textbf{151}, A422 (2004); related \emph{Phys. Rev. B} \textbf{70}, 174304 (2004). DOI: 10.1103/PhysRevB.70.174304.
\bibitem{wang2009} Y. Wang, Y. Reynier \emph{et al.}, ``Effect of entropy change of Li intercalation in cathodes and anodes on Li-ion battery thermal management,'' \emph{J. Power Sources} (2009). PII: S0378775309021119.
\bibitem{ml2024} Garikipati group, ``Bridging scales with Machine Learning: from first-principles statistical mechanics to continuum phase field computations to study order--disorder transitions in Li$_x$CoO$_2$,'' \emph{J. Mech. Phys. Solids} \textbf{190}, 105727 (2024). DOI: 10.1016/j.jmps.2024.105727; arXiv:2302.08991.
\end{thebibliography}
