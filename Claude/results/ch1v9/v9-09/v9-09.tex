...
\begin{tabular}{l c c p{0.23\textwidth} p{0.31\textwidth}}
\toprule
...
개별 T1--T3 값은 문헌 초기값과 우리 데이터 round-trip 피팅으로 정한다. 전자 엔트로피가 들어가는 경우에도 경로는 같다:
...
(\S\ref{sec:electronic_entropy}). 곧 LCO 의 차이는 분기 식이 아니라 파라미터와 엔트로피 성분이다.
...
LCO 조성 $x$ 를 Li 함량으로 두면, Li 제거는 $x\downarrow$ 이다. 전자 엔트로피의 조성 기울기는
\begin{equation}
\Delta S_e(x,T)=\frac{\partial S_e}{\partial x}\Big|_T
=\frac{\pi^2}{3}k_B^2T\,\frac{\partial g(E_F,x)}{\partial x}.
\label{eq:dSe}
\end{equation}
MIT 에서 탈리튬화가 진행되면 $g(E_F)$ 가 거의 $0$ 에서 금속값으로 증가한다. 따라서 $x$ 를 증가시키는 방향의
$\partial g/\partial x$ 는 음수이고, Li 제거 방향으로는 전자 entropy gain 이 양의 전압 온도계수에 기여한다. 이 항은
양극 단전극 기울기 식~\eqref{eq:Ujcat} 의 같은 자리, 곧 $\partial U_j^\mathrm{cat}/\partial T=\Delta S_{\rxn,j}^\mathrm{cat}/F$
로 들어간다.
...
\label{eq:gmitderiv}
\end{equation}
즉 MIT 근방의 국소 봉우리다. 완전 metal anchor 에서 $S_e/k_B\approx(\pi^2/3)(k_BT)g(E_F)
...
\end{tikzpicture}
\caption{LCO half-cell peak candidates (new). T1 near 3.90 V is the MIT peak with electronic entropy gate; T2/T3 near 4.05 and 4.17--4.20 V are order--disorder peaks. The drawing is a forward-model initial map, not a fitted spectrum.}
\label{fig:lco_peaks}
\end{figure}
