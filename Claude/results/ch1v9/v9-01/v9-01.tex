...
% ====================================================================
% v9-01 additions: LCO cathode integration (targeted insertions over v8-11 graphite base)
%   Graphite content VERBATIM preserved. LCO: N0 table, N2/N3 cathode subsections,
%   N4/N5 xi_eq distribution framing, NEW electronic entropy section, N6 LCO peak,
%   N9 DeltaS decomposition, code generalization note.
%   Build: xelatex 2-pass (kotex/D2Coding).
% ====================================================================
...
\hypersetup{colorlinks=true, linkcolor=blue, urlcolor=blue, citecolor=blue,
  pdftitle={흑연 음극 + LCO 양극 dQ/dV 이론 (통합 하프셀) — Chapter 1 (v9)}}
\pagestyle{fancy}\fancyhf{}
\lhead{흑연 + LCO 하프셀 $dQ/dV$ 이론 — Ch.1 (v9)}\rhead{\thepage/\pageref{LastPage}}
...
\title{\textbf{\LARGE Chapter 1}\\[0.35em]\Large 흑연 음극 + LCO 양극 $dQ/dV$ 이론: 통합 하프셀 코드 진행 수식-구동판 — 유도 확장 (v9)}
\author{}
\date{}
...
% ---- LCO N0 insertion: cathode transition table ----
\subsection*{양극(LCO) 전이 목록과 부호 규약}\label{sec:lco_staging}
음극(흑연) 4 전이와 대칭으로, LCO 양극 코인 하프셀(LCO vs Li, $\le4.2$--$4.5$ V)에도 뚜렷한 3 전이가 있다.
표~\ref{tab:lco_staging} 가 그 초기값(이하 \code{LCO\_STAGING\_LIT}) 이며 — 문헌 초기값, 우리 코인 데이터 round-trip 피팅 대상(G1--G3).

\textbf{부호 규약(양극 vs 음극).} 양극 하프셀에서 전위 $U_j^\cat$ 는 흑연보다 \emph{훨씬 높다}($\sim3.9$--$4.2$ V vs Li/Li$^+$).
탈리튬화(방전, $\xi\!\uparrow$) 방향은 양극에서 Li 빠짐이므로 $\sigma_d=+1$ 부호 사슬은 음극과 동형이다 — 같은 logistic·히스·꼬리 코드가 파라미터 교체만으로 적용된다.
온도 의존: $\partial U_j^\cat/\partial T=\Delta S_{\rxn,j}^\cat/F$ (흑연과 1:1 정합, 아래 §\ref{sec:center_lco}).
LCO 고유 항 = \emph{전자 엔트로피}(§\ref{sec:elec_entropy}); 흑연에는 없음.

\begin{table}[ht]
\centering
\renewcommand{\arraystretch}{1.25}
\caption{LCO 양극 전이 초기값 \code{LCO\_STAGING\_LIT}(3건, tier A=Xia \cite{xia2007}; 하프셀, ≤4.2--4.5 V). $\Delta S_{\rxn,j}^\cat$ \textbf{부호 규약}: \emph{삽입} 기준($\Delta S_\rxn$ 슬롯은 흑연 stage~2$\to$1의 $-16$ J/(mol\,K) 과 동일 방향), Li 삽입 시 엔트로피 감소(음) 또는 MIT 구간의 전자 항 기여가 더해진다. T4 O3→H1-3($\sim$4.55 V)는 ≤4.5 V 범위에서 선택적.}
\label{tab:lco_staging}
\begin{tabular}{@{}l c c l l l@{}}
\toprule
전이 & $U_j^\cat$ [V] & $x$ 범위 & \multicolumn{1}{c}{성격} & \multicolumn{1}{c}{$\Delta S_{\rxn,j}^\cat$ 특징} & \multicolumn{1}{c}{비고} \\
\midrule
T1 MIT          & $3.90$ & $0.75$--$0.94$ & 절연체$\to$금속 & config$+\Delta S_\elec$ 게이트   & 전자항 발현 \\
T2 ord--dis\,a  & $4.05$ & $x\approx0.55$ & hex$\to$mono    & config주도($0.47$ J/K$\!\cdot\!$mol) & 2상역 \\
T3 ord--dis\,b  & $4.17$--$4.20$ & $x\approx0.48$ & mono$\to$hex & config($1.49$ J/K$\!\cdot\!$mol) & T2 한 쌍 \\
\bottomrule
\end{tabular}
\end{table}
% ---- end LCO N0 insertion ----
...
% ---- LCO N2 insertion ----
\subsection{양극(LCO) 평형 중심 $U_j^\cat(T)$ — 부호 연장}\label{sec:center_lco}
흑연 식~\eqref{eq:Uj} 의 프레임은 전극 불문 일반형이다 — 같은 식이 양극에도 성립하며, 파라미터만 바뀐다.
LCO 전이 $j$ 에 대해
\begin{equation}
U_j^\cat(T)\;=\;\frac{-\Delta H_{\rxn,j}^\cat + T\,\Delta S_{\rxn,j}^\cat}{F},
\label{eq:Uj_cat}
\end{equation}
온도 의존
\begin{equation}
...
\label{eq:dUdT_cat}
\end{equation}
은 흑연 $\partial U_j/\partial T=\Delta S_{\rxn,j}/F$ 와 1:1 동형이다. \textbf{★ 부호 규약 명시}: $\Delta S_{\rxn,j}^\cat$ 슬롯은 \emph{삽입} 반응 엔트로피 기준 — 흑연 stage 2$\to$1 의 $\Delta S_\rxn=-16$ J/(mol\,K)(삽입 시 엔트로피 감소)와 동일 규약. 전자 항 $\Delta S_\elec$ 도 삽입 기준으로 $\partial S_e/\partial x<0$(삽입 $x\!\uparrow$ 시 MIT 해제 → $g(E_F)\!\downarrow$ → $S_e\!\downarrow$), 즉 삽입 엔트로피 변화로 음수다. 양극 중심 전위는 흑연보다 매우 높다($U^\cat\sim3.9$--$4.2$ V vs Li).

\textbf{$\Delta S_{\rxn,j}^\cat$ 분해.} 양극 삽입 반응 엔트로피는 세 성분의 합이다:
\begin{equation}
\Delta S_{\rxn,j}^\cat(x,T)\;=\;\Delta S_j^\config+\Delta S_j^\vib+\Delta S_\elec(x,T).
\label{eq:dS_cat}
\end{equation}
$\Delta S^\config$ 는 logistic 이 자동 생성하는 배치 몫(이중계산 금지: 중심 표준값 $\Delta S^{0}_j$ 는 중심 표준값에만), $\Delta S^\vib$ 는 음의 phonon baseline(흑연과 동형), $\Delta S_\elec$ 는 §\ref{sec:elec_entropy} 의 신규 전자 항(MIT 게이트, LCO 고유 — 흑연에는 없음, $\propto T$).
% ---- end LCO N2 insertion ----
...
% ---- LCO N3 insertion ----
\subsubsection*{LCO 히스테리시스 — order--disorder 와 MIT 2상역}
LCO 의 T2/T3 전이(order--disorder, $x\approx0.5$)는 monoclinic 대칭 정렬이 발생·소멸하는 구조 상전이다. 이 정렬은 LiLi$\cdot$VacVac 이웃 쌍에너지($\Omega^\cat>0$)로 격자기체 정규용액 $\Omega\theta(1-\theta)$ 에 들어온다 — 흑연 N3 프레임과 동일한 수학적 구조($g_j(\xi)$, spinodal 조건 $\Omega_j>2RT$, 분기 gap $\Delta U_j^\hys$)로 기술된다. 물리 차이는 두 가지다: (i) LCO 는 $\xi_{1/2}$ 부근 2상역이 \emph{질서 매개변수}(monoclinic 정렬)를 수반하며, (ii) T1 MIT 2상역에는 추가로 §\ref{sec:elec_entropy} 의 전자 엔트로피 게이트가 올라온다. 따라서 LCO 파라미터 딕셔너리에 $\Omega_j^\cat$·$\gamma_j$ 를 넣으면 식~\eqref{eq:dUhys}·\eqref{eq:Ubranch} 가 양극 히스 분기도 그대로 닫는다(구조 변경 0).
% ---- end LCO N3 insertion ----
...
% ---- ξ_eq distribution framing insertion (N4/N5) ----
\subsection{$\xi_\eq$ 의 분포 관점 — kinetic·thermo 통합과 LCO 전자 Fermi--Dirac 다리}\label{sec:xieq_dist}
위에서 $\xi_\eq$ 는 두 경로 — (i) kinetic(detailed balance, §\ref{sec:width} 유도) 과 (ii) thermo(자유에너지 최소화, 식~\eqref{eq:Veq}) — 로 유도되었다. 이 소절은 두 경로가 \emph{같은 분포 언어}로 통합됨을 보인다. 이 프레이밍은 결과·부호를 바꾸지 않으며, Ch.2 분포 전개와 §\ref{sec:elec_entropy} 의 LCO 전자 Fermi--Dirac 를 같은 언어로 묶는 다리다.

\textbf{(a) 분배함수 표현.} 자리 하나($\Omega=0$ 이상 한계)가 Li 를 가지면 에너지 $\varepsilon=\mu_\mathrm{Li}-\mu^0$, 비어 있으면 $0$ 인 grand-canonical 계를 생각하면, 단일 자리 분배함수는
\begin{equation}
Z\;=\;1+e^{-\beta(\varepsilon-\tilde\mu)}\;=\;1+e^{-\beta\Delta\mu},
\qquad \Delta\mu\equiv\varepsilon-\tilde\mu\;=\;-\mathcal{A}/RT\cdot RT=-\mathcal{A},
\label{eq:Z1site}
\end{equation}
($\beta=1/k_BT$, $\tilde\mu$ = 전기화학 퍼텐셜 기준). \textbf{(b) 평균 점유 = Fermi 함수형.} 이 계의 평균 점유(Li 있을 확률)는
\begin{equation}
\langle n\rangle\;=\;\frac{e^{-\beta\Delta\mu}}{Z}\;=\;\frac{1}{1+e^{\beta\Delta\mu}}\;=\;\frac{1}{1+e^{-\mathcal{A}/RT}},
\label{eq:fermi1site}
\end{equation}
이고 $\mathcal{A}=sF(V_n-U_j)$, $\Omega=0$ 에서 $\langle n\rangle=\xi_\eq$ (Li 점유 = 탈리튬화 진행률 — 방전 방향 $\sigma_d=+1$ 에서 $\xi_\eq$ 는 Li 가 \emph{빠진} 자리 비율)이므로
\begin{equation}
\xi_\eq\;=\;\langle n\rangle\;=\;\frac{1}{1+e^{-\mathcal{A}/RT}}\;=\;\frac{1}{1+e^{-\sigma_d(V-U_j)/w_j}},
\label{eq:xieq_fermi}
\end{equation}
식~\eqref{eq:xieq} 와 동일하다($w_j=RT/F$, $\Omega=0$ 한계). ($\langle n\rangle = 1-\xi_\eq$ 와 $\langle n\rangle = \xi_\eq$ 의 차이는 Li 점유 vs 탈리튬화 진행률의 관례 차이 — 본 문건은 방전 진행률 $\xi_\eq\!\uparrow$ 를 기본으로 한다.) \textbf{(c) 통합 해석.} kinetic 경로(정지점)와 thermo 경로(자유에너지 최소) 둘 다 이 단일 자리 Boltzmann 분포로 수렴한다 — $\xi_\eq$ 는 전이 자리가 Li 로 \emph{채워질 확률 분포}이며, Fermi 함수형 $1/(1+e^{-z})$ 는 $\Omega=0$ 이상 한계의 점유 분포다. $\Omega\ne0$ 이 있으면 식~\eqref{eq:mu} 의 상호작용 몫이 유효 에너지를 조성 의존하게 바꾸어 logistic 을 비선형 등온선(식~\eqref{eq:Veq})으로 일반화한다. \textbf{(d) LCO 전자 다리.} §\ref{sec:elec_entropy} 에서 LCO 전도 전자의 점유 분포(Fermi--Dirac $f(\varepsilon)=1/(1+e^{(\varepsilon-\mu_F)/k_BT})$)가 등장한다 — 이는 같은 단일 자리 점유 언어의 \emph{전자판}이다. 두 Fermi 함수($\xi_\eq$ = Li 점유, $f$ = 전자 점유)의 공통 언어가 Ch.1 프레임을 LCO 전자 엔트로피로 자연 확장한다.
% ---- end ξ_eq distribution framing insertion ----
...
% ====================================================================
\section{LCO 전자 엔트로피 — Sommerfeld 유도와 MIT 게이트 (LCO 고유)}\label{sec:elec_entropy}
% ====================================================================
흑연은 반금속/semimetal 이라 탈리튬화에 따른 전자 상태밀도 변화가 미미하고, $\Delta S_\elec\approx0$ 으로 무시할 수 있다.
LCO 는 다르다. $x\!\to\!1$ 완전 리튬화 상태(LiCoO$_2$)에서는 절연체(band gap $\sim2$ eV)이나, 탈리튬화($x\!\downarrow$)로 Co$^{3+}$→Co$^{4+}$ 이 진행되면 $x\!\approx\!0.75$--$0.94$ 에서 절연체→금속 전이(MIT, metal--insulator transition)가 일어나며\cite{menetrier1999}, Fermi 준위 상태밀도 $g(E_F)$ 가 $0$→$\sim13$ e/eV 로 급증한다\cite{motohashi2009}. 이 MIT 로 인해 전도 전자의 열 점유 분포가 열역학적 엔트로피에 비자명한 기여를 남긴다 — 이것이 \emph{전자 엔트로피 항} $\Delta S_\elec$ 이다.

\subsection{Fermi--Dirac 분포와 Sommerfeld 전개 — $S_e$ 유도}
\textbf{(a) 출발 — 전자 열 점유.} 온도 $T$ 에서 에너지 $\varepsilon$ 의 전자 상태가 채워질 확률은 Fermi--Dirac 분포
\begin{equation}
f(\varepsilon)\;=\;\frac{1}{1+e^{(\varepsilon-\mu_F)/k_BT}},
\label{eq:FD}
\end{equation}
($\mu_F$ = 화학퍼텐셜 / Fermi 준위). §\ref{sec:xieq_dist} 의 Li 점유 분포 $\xi_\eq=1/(1+e^{-\mathcal{A}/RT})$ 와 같은 Fermi 함수형이며, 에너지 연속 분포로의 확장이다. \textbf{(b) 연산 — 총 전자 엔트로피.} 상태밀도 $g(\varepsilon)$ 위의 Fermi--Dirac 분포에서 전자 엔트로피를 $-k_B\int g(\varepsilon)[f\ln f+(1-f)\ln(1-f)]\,\dd\varepsilon$ 로 쓰고, $T\!\ll\!T_F$(축퇴 전자기체 조건 — 금속 LCO 의 $T_F\sim$ 수만 K) 에서 Sommerfeld 급수로 전개하면 $T^1$ 항이 지배한다. \textbf{(c) 중간식 — Sommerfeld 적분.} Fermi 함수의 ``창'' 폭 $\sim k_BT\ll E_F$ 라 $g(\varepsilon)$ 를 Fermi 준위 $E_F$ 에서 Taylor 전개하고($g(\varepsilon)\approx g(E_F)+g'(E_F)(\varepsilon-E_F)+\cdots$), $-k_B\int(\cdots)\,\dd\varepsilon$ 를 표준 Sommerfeld 적분 $\int_{-\infty}^\infty H(\varepsilon)(-\partial f/\partial\varepsilon)\,\dd\varepsilon\approx H(E_F)+\frac{\pi^2}{6}(k_BT)^2H''(E_F)+\cdots$ 로 근사하면 엔트로피 밀도는 선형 $T$ 항으로 닫힌다:
\begin{equation}
S_e(T,x)\;=\;\frac{\pi^2}{3}\,k_B^2\,T\,g(E_F,x).
\label{eq:Se_sommerfeld}
\end{equation}
이것이 \textbf{Sommerfeld 전자 엔트로피}이며, $T=0$ 에서 $S_e=0$, $T$ 에 비례해 증가한다. \textbf{단위 다리(자리당 → 몰당)}: 식~\eqref{eq:Se_sommerfeld} 의 $g(E_F)$ 단위가 eV$^{-1}$\,per f.u.(unit cell)일 때 $S_e$ 단위는 $k_B$ per f.u. 이고, 1몰량으로 올리려면 $N_A k_B=R$ 을 쓴다: $S_e^\mathrm{mol}=\frac{\pi^2}{3}R\,(k_BT)\,g(E_F)$($g$ 의 단위를 eV$^{-1}$\,per f.u.로 유지, $R=8.314$ J/(mol\,K), $k_BT\approx0.0259$ eV at 25$^\circ$C). 이하 $S_e$·$\Delta S_\elec$ 는 자리당 $k_B$ 단위이며, J/(mol\,K) 로 환산 시 $R/k_B=N_A$ 를 곱한다. \textbf{★ $\propto T$ 명시} — config·vib 의 $\Delta S_\rxn$ 이 $T$ 독립 상수인 것과 달리, 전자 항은 $T$-선형이다. 이 차이가 Ch.2 가역열 전개에서 중요해진다.

\textbf{크기 검산(tier B).} $g(E_F)\approx13$ e$\cdot$eV$^{-1}$\,f.u.$^{-1}$(CoO$_2$)\cite{motohashi2009}, $k_BT\approx0.0259$ eV(25$^\circ$C):
$S_e/k_B=(\pi^2/3)\times0.0259\times13\approx1.1\,k_B$/f.u.(완전 금속). 몰 단위: $S_e^\mathrm{mol}\approx1.1\,R\approx9.1$ J/(mol\,K). 실측 O3(부분 금속) 삽입 부분몰 $|\Delta S_\elec|\approx0.18\,k_B$/atom\cite{reynier2004} — 자릿수 일치(완전 금속 한계보다 작음, MIT 게이트 반영).

\subsection{부분몰 전자 엔트로피 $\Delta S_\elec(x,T)$}
\textbf{(a)(b) 출발·연산.} $\Delta S_\rxn$ 슬롯은 \emph{삽입}(Li 추가, $x\!\uparrow$) 반응 엔트로피 기준이다 — 흑연 stage~2$\to$1 의 $\Delta S_\rxn=-16$ J/(mol\,K) 가 삽입 시 엔트로피 감소를 뜻하는 것과 같은 규약. 전자 엔트로피 $S_e$ 의 부분몰 변화는 삽입 ($x\!\uparrow$) 당 $S_e$ 의 조성 기울기로 정의한다:
\begin{equation}
\Delta S_\elec(x,T)\;\equiv\;\frac{\partial S_e}{\partial x}\bigg|_T
\;=\;\frac{\pi^2}{3}\,k_B^2\,T\,\frac{\partial g(E_F,x)}{\partial x}.
\label{eq:dSe}
\end{equation}
리튬화($x\!\uparrow$)는 MIT 구간을 역행하여 $g(E_F)$ 가 감소하므로 $\partial g/\partial x<0$, 따라서
$\Delta S_\elec<0$ — 삽입 기준으로 전자 엔트로피 변화는 \emph{음수}이며, 탈리튬화($x\!\downarrow$) 시 전자 엔트로피가 방출된다(탈리튬화 시 $-\Delta S_\elec>0$).
\textbf{(c) 부호 사슬.} $\Delta S_\elec<0$ 이 $\Delta S_{\rxn,j}^\cat=\cdots+\Delta S_\elec$ (식~\eqref{eq:dS_cat})에 음의 기여를 주어 config+vib 합계에 더해진다. 단전극 $\partial\varphi/\partial T\approx+0.83$ mV/K(tier B\cite{reynier2009})는 config+vib 지배 합산값이며, 전자 항 $\Delta S_\elec$ 의 부호를 개별 제약하지 않는다.
\textbf{(d) ★$\propto T$.} $|\Delta S_\elec|\propto T$ — 다른 항(상수)과 달리 전자 항만 온도 선형이다.

\subsection{$g(E_F,x)$ — MIT-logistic 게이트 모델 (G2 해소)}\label{sec:gEF_model}
1차 문헌에 $g(E_F,x)$ 의 연속 곡선 데이터가 없으므로(G2 갭), Ch.1 ``문헌 초기값+데이터 피팅'' 철학을 적용해 MIT 를 logistic 게이트로 근사한다:
\begin{equation}
g(E_F,x)\;\approx\;g_{\max}\Big[1-\sigma\!\Big(\frac{x-x_\MIT}{\Delta x_\MIT}\Big)\Big],
\label{eq:gEF_model}
\end{equation}
($\sigma(z)=1/(1+e^{-z})$ = logistic). $x\!\downarrow$ 로 게이트가 $0\to g_{\max}$ 켜지며, 발현 구간 $x\approx0.75$--$0.94$\cite{reimers1992,menetrier1999} 를 폭 $\Delta x_\MIT$ 가 흡수한다. \textbf{모델 가정(초기값, 피팅 대상):} $g_{\max}=13$ e/eV, $x_\MIT\approx0.85$, $\Delta x_\MIT\approx0.05$.

step 대신 logistic 을 채택한 이유는 두 가지다: (i) 매끈한 $\partial g/\partial x$ (→ 매끈한 $\partial U^\cat/\partial T$, Ch.2 가역열 겹침과 일관), (ii) 발현 $x$ 의 결함 농도 분산을 폭 $\Delta x_\MIT$ 가 흡수(Ch.1 흑연 폭 $w_j$ 와 같은 철학).

식~\eqref{eq:gEF_model} 을 식~\eqref{eq:dSe} 에 대입하면, $g(E_F,x)$ 는 $x\!\uparrow$(삽입)에 따라 감소하는 게이트($1-\sigma$ 형태)이므로 $\partial g/\partial x<0$:
\begin{equation}
\Delta S_\elec(x,T)\;=\;-\frac{\pi^2}{3}k_B^2T\,\frac{g_{\max}}{\Delta x_\MIT}\,\sigma\!\Big(\frac{x-x_\MIT}{\Delta x_\MIT}\Big)\!\Big[1-\sigma\!\Big(\frac{x-x_\MIT}{\Delta x_\MIT}\Big)\Big],
\label{eq:dSe_model}
\end{equation}
MIT 부근 국소 음봉우리(삽입 기준 $\Delta S_\elec<0$; 탈리튬화 방향에서 이 항이 엔트로피를 방출). 크기는 작지만($|{\Delta S_\elec}|\sim0.18\,k_B$/atom 급\cite{reynier2004}) T1 봉우리의 온도 감도를 결정한다. \textbf{왜 config 단독으로 MIT 2상역을 재현할 수 없는가}: config 만으로는 T1 MIT 구간의 $dQ/dV$ 봉우리 위치와 온도 의존을 동시에 맞출 수 없다\cite{garikipati2024} — 전자 항의 $\propto T$ 성질이 T1 봉우리의 $T$-감도를 담당한다.

\begin{figure}[ht]
\centering
\begin{tikzpicture}[x=4.2cm,y=1.8cm]
% x-axis: horizontal = 1-x (so x=1 at left, x=0 at right; delithiation goes right)
% g(E_F,x) ~ 2 * sigma((0.85-x)/0.05) — MIT logistic gate, manually evaluated
% u=1-x: x=1.00->u=0.00, x=0.94->u=0.06, x=0.90->u=0.10, x=0.85->u=0.15,
%         x=0.80->u=0.20, x=0.75->u=0.25, x=0.70->u=0.30, x=0.60->u=0.40,
%         x=0.50->u=0.50, x=0.00->u=1.00
% g/gmax values (sigma((x-0.85)/0.05) reflected, x decreasing => g increasing):
%   x=1.00: sigma(-3)=0.047 => g=0.094
%   x=0.94: sigma(-1.8)=0.142 => g=0.284
%   x=0.90: sigma(-1.0)=0.269 => g=0.538
%   x=0.85: sigma(0)=0.500 => g=1.000
%   x=0.80: sigma(1.0)=0.731 => g=1.462
%   x=0.75: sigma(1.8)=0.858 => g=1.716
%   x=0.70: sigma(3.0)=0.953 => g=1.906
%   x=0.60: sigma(5.0)=0.993 => g=1.986
%   x=0.50 and below: ~2.000
\draw[thick] plot[smooth] coordinates {
  (0.00,0.094) (0.06,0.284) (0.10,0.538) (0.15,1.000)
  (0.20,1.462) (0.25,1.716) (0.30,1.906) (0.40,1.986)
  (0.50,1.998) (1.00,2.000)};
\node[right,font=\scriptsize] at (0.55,1.7) {$g(E_F,x)$: MIT gate (model)};
% dSe proportional to sigma*(1-sigma): peak at x=0.85 (u=0.15)
% dSe_peak ~ 7*0.25=1.75 at u=0.15; wings fall as logistic bell
%   u=0.06: 7*0.142*0.858=0.852
%   u=0.10: 7*0.269*0.731=1.378
%   u=0.15: 7*0.500*0.500=1.750 (peak)
%   u=0.20: 7*0.731*0.269=1.378
%   u=0.25: 7*0.858*0.142=0.852
%   u=0.30: 7*0.953*0.047=0.314
%   u=0.40 and beyond: ~0
\draw[densely dashed,thick] plot[smooth] coordinates {
  (0.00,0.050) (0.06,0.852) (0.10,1.378) (0.15,1.750)
  (0.20,1.378) (0.25,0.852) (0.30,0.314) (0.40,0.050)
  (0.50,0.010) (1.00,0.005)};
\node[right,font=\scriptsize] at (0.10,1.3) {$\Delta S_e(x,T)\propto T$ (MIT peak)};
% axes
\draw[-{Latex[length=1.4mm]}] (-0.05,0) -- (1.15,0) node[below,font=\scriptsize] {$x$ in Li$_x$CoO$_2$ ($\leftarrow$ delithiation)};
\draw[-{Latex[length=1.4mm]}] (0,-0.12) -- (0,2.35) node[above,font=\scriptsize] {$g(E_F,x)$ [arb.] / $\Delta S_e$ [arb.]};
% annotation: x_MIT at u=0.15
\draw[densely dotted] (0.15,0) -- (0.15,1.00) node[above,font=\scriptsize] {$x_\MIT$};
\node[below,font=\scriptsize] at (0.15,-0.12) {0.85};
% x labels (x = 1-u)
\foreach \u/\xval in {0.00/1.0, 0.25/0.75, 0.50/0.5, 0.75/0.25, 1.00/0.0} {
  \draw (\u,0.03) -- (\u,-0.03) node[below,font=\scriptsize] {\xval};
}
% Voltage annotations
\node[font=\scriptsize,align=center] at (0.15,-0.45) {T1\\3.90V};
\node[font=\scriptsize,align=center] at (0.48,-0.45) {T2\\4.05V};
\node[font=\scriptsize,align=center] at (0.52,-0.45) {T3\\4.17V};
\end{tikzpicture}
\caption{LCO 전자 상태밀도 게이트 $g(E_F,x)$(실선, 식~\eqref{eq:gEF_model})와 부분몰 전자 엔트로피 $\Delta S_\elec(x,T)$(파선, 식~\eqref{eq:dSe_model}). 탈리튬화($x\!\downarrow$, 오른쪽 방향)로 MIT 구간($x_\MIT\approx0.85$)에서 $g$ 가 증가하고, $\Delta S_\elec$ 가 MIT 부근 국소 양봉우리를 만든다. 파선은 $\propto T$ (온도 선형). 흑연에는 이 곡선이 없다($\Delta S_\elec^\mathrm{graphite}\approx0$). 전이 위치(아래 annotation)는 표~\ref{tab:lco_staging}.}
\label{fig:elec_entropy}
\end{figure}

\subsection{흑연 대비(전자항 = 0)와 필수성}
\textbf{흑연.} 흑연은 탈리튬화 전 과정에서 반금속 특성이 유지되고 $g(E_F)$ 변화가 미미하다($\partial g/\partial x\approx0$). 따라서 $\Delta S_\elec^\mathrm{graphite}\approx0$ — 흑연 모델의 $\Delta S_\rxn$ 은 config+vib 만으로 닫힌다. \textbf{LCO 의 필수성.} config 항 단독으로는 T1 MIT 2상역의 $dQ/dV$ 봉우리 온도 의존을 재현하지 못한다\cite{garikipati2024} — $|\Delta S_\elec|\propto T$ 가 T1 봉우리의 $T$-감도를 제공하는 유일한 성분이다. \emph{이 대비가 통합 챕터의 교육 가치} — 같은 logistic 프레임 위에서 LCO 는 전자 자유도 하나를 추가로 연다.

\begin{codebox}
\textbf{LCO 전자 엔트로피 plug-in(코드 일반화 H).} MSMR $x_j=X_j/\{1+\exp[f(U-U_j^0)/\omega_j]\}$ = Ch.1 transition-logistic 동형\cite{msmr2023}. 따라서 코드 클래스(\code{func\_ksi\_eq}·\code{func\_U\_j} 등)에 (i) \code{LCO\_STAGING\_LIT} 전이 파라미터 교체 + (ii) $\Delta S_\elec(x,T)$ 를 \code{dS\_rxn} 에 plug-in 하면 LCO 적용이 완성된다(구조 변경 0). $\Delta S_\elec$ 는 식~\eqref{eq:dSe_model}(\emph{삽입} 기준, 값 $<0$; \code{dS\_rxn} 슬롯에 더할 때 삽입 규약 일관): 온도 $T$·조성 $x$·초기값 $(g_{\max},x_\MIT,\Delta x_\MIT)$ 를 받는 함수 \code{func\_dSe(T, x, gmax, xMIT, dxMIT)}.
\end{codebox}
...
% ---- LCO N6 insertion: dQ/dV peak candidate figure ----
\subsection*{LCO 양극 평형 $\dd Q/\dd V$ 그림 후보}
식~\eqref{eq:eqpeak} 의 평형 peak 프레임은 LCO 에도 동일하게 적용된다.
그림~\ref{fig:lco_dqdv} 가 표~\ref{tab:lco_staging} 의 3 전이(T1/T2/T3, 초기값)를 식~\eqref{eq:eqpeak} 로 합산한 평형 $\dd Q/\dd V$ 후보를 보인다.
세 봉우리 위치($\sim3.90$/$\sim4.05$/$\sim4.17$ V)는 문헌 초기값이고, 우리 코인 하프셀 피팅으로 override 된다.
T1 봉우리의 온도 감도는 $\Delta S_\elec\propto T$ 항에 의해 다른 두 봉우리와 다른 $T$-의존성을 보인다(§\ref{sec:elec_entropy}).

\begin{figure}[ht]
\centering
\begin{tikzpicture}[x=10.0cm,y=1.4cm]
% Axes: V from 3.75 to 4.35, dQ/dV arb
% We use (V-3.75) as x-coordinate with x-scale 10cm/0.60V => 1V=10cm
% Adjusted: let t = V - 3.75, x coord = t, tmax=0.60
\draw[-{Latex[length=1.4mm]}] (-0.02,0) -- (0.65,0) node[below,font=\scriptsize] {$V$ vs Li/Li$^+$ [V]};
\draw[-{Latex[length=1.4mm]}] (-0.02,0) -- (-0.02,3.2) node[above,font=\scriptsize] {$dQ/dV$ [eq., arb.]};
% T1: center at V=3.90, t=0.15; Gaussian width sigma=0.025V
% Logistic bell qualitative: peak=1.5, t-values and heights manually:
% t=0.05(V=3.80):~0.04, t=0.10(3.85):~0.35, t=0.12(3.87):~0.80, t=0.15(3.90):1.50,
% t=0.18(3.93):~0.80, t=0.20(3.95):~0.35, t=0.25(4.00):~0.04
\draw[thick] plot[smooth] coordinates {
  (0.02,0.02)(0.10,0.35)(0.12,0.80)(0.15,1.50)(0.18,0.80)(0.20,0.35)(0.25,0.04)};
\node[above,font=\scriptsize] at (0.15,1.52) {T1 (MIT)};
% T2: center V=4.05, t=0.30; peak=2.0
% t=0.25(4.00):0.04, t=0.27(4.02):0.50, t=0.29(4.04):1.50, t=0.30(4.05):2.00,
% t=0.31(4.06):1.50, t=0.33(4.08):0.50, t=0.35(4.10):0.04
\draw[thick] plot[smooth] coordinates {
  (0.25,0.04)(0.27,0.50)(0.29,1.50)(0.30,2.00)(0.31,1.50)(0.33,0.50)(0.35,0.04)};
\node[above,font=\scriptsize] at (0.30,2.02) {T2};
% T3: center V=4.18, t=0.43; peak=2.5
% t=0.38(4.13):0.04, t=0.40(4.15):0.60, t=0.42(4.17):1.90, t=0.43(4.18):2.50,
% t=0.44(4.19):1.90, t=0.46(4.21):0.60, t=0.48(4.23):0.04
\draw[thick] plot[smooth] coordinates {
  (0.38,0.04)(0.40,0.60)(0.42,1.90)(0.43,2.50)(0.44,1.90)(0.46,0.60)(0.48,0.04)};
\node[above,font=\scriptsize] at (0.43,2.52) {T3};
% voltage tick labels
\foreach \t/\vlabel in {0.05/3.80, 0.15/3.90, 0.25/4.00, 0.30/4.05, 0.43/4.18, 0.55/4.30} {
  \draw (\t,0.04) -- (\t,-0.04) node[below,font=\scriptsize] {\vlabel};
}
\node[font=\scriptsize,align=left] at (0.58,2.8) {LCO hf.\\(init.)};
\end{tikzpicture}
\caption{LCO 양극 평형 $\dd Q/\dd V$ 후보(정성적, 식~\eqref{eq:eqpeak} 적용). T1(MIT, $\sim3.90$ V)·T2(order--dis a, $\sim4.05$ V)·T3(order--dis b, $\sim4.17$--$4.20$ V)의 세 봉우리 — 초기값 위치(표~\ref{tab:lco_staging}), 우리 코인 피팅으로 override. 봉우리 높이·폭은 정성 시연이며 $Q_j$·$w_j$ 피팅 대상이다. T1 온도 감도($\Delta S_\elec\propto T$)는 T2/T3 와 다름에 주의.}
\label{fig:lco_dqdv}
\end{figure}
% ---- end LCO N6 insertion ----
...
% ---- LCO N9 insertion: ΔS decomposition and code generalization ----
\subsection{LCO 양극 $\Delta S_\rxn^\cat$ 분해와 코드 일반화}\label{sec:lco_dS_sum}
식~\eqref{eq:dS_cat} 의 분해를 N9 합산 관점에서 정리한다.
각 전이 $j$ 의 \code{dS\_rxn} 은 세 항의 합이다:
\begin{equation}
\Delta S_{\rxn,j}^\cat(x,T)
=\underbrace{\Delta S_j^\config}_{\text{logistic 자동·표준값 }{\Delta S^0_j}}
+\underbrace{\Delta S_j^\vib}_{\text{음의 baseline}}
+\underbrace{\Delta S_\elec(x,T)}_{\text{MIT 게이트, }\propto T\text{ (LCO 고유)}},
\label{eq:dS_sum}
\end{equation}
이중계산 금지(B): $\Delta S^0_j$ 는 중심 표준값에만(config 가 logistic 내부에서 이미 생성하는 몫과 분리).
$\Delta S_j^\vib$ 는 음수(phonon, 고-$x$) — 흑연 $\Delta S_\rxn$ 과 같은 부호 패턴.
$\Delta S_\elec$ 는 §\ref{sec:elec_entropy} 식~\eqref{eq:dSe_model}(삽입 기준 음수, MIT 게이트 국소 봉우리, $\propto T$).

\textbf{코드 일반화(H).} \code{LCO\_STAGING\_LIT} 는 흑연 \code{GRAPHITE\_STAGING\_LIT} 와 같은 구조 — \code{dH\_rxn}, \code{dS\_rxn}, \code{U}, \code{Q}, \code{Omega}, \code{dH\_a}, \code{dVdq\_qa} 필드. 전자 엔트로피는 \code{dS\_rxn} 필드에 플러그인된다(\code{dS\_rxn\_cat = dS\_config + dS\_vib + func\_dSe(T, x, ...)}). MSMR $x_j$ = Ch.1 transition-logistic 동형\cite{msmr2023} 이므로 \code{func\_ksi\_eq}·\code{func\_U\_j} 등 코어 메서드는 파라미터 교체(+전자항 1항)만으로 LCO 에 적용된다(구조 변경 0, §\ref{sec:elec_entropy} 코드 박스).

\textbf{갭 처리.} G1(연속 $\Delta S(x)$)·G2($g(E_F)(x)$)·G3(MSMR LCO 파라미터)·G4(도핑 shift)·G5($\Delta H_f$ 절대값) = 우리 코인 하프셀 데이터 round-trip 피팅으로 식별. 허위 정밀 금지(tier 표기: A=Xia/Motohashi·B=단전극 실측·F=파생·G=갭).
% ---- end LCO N9 insertion ----
...
표의 모든 행이 코드 입력이라 ``고정/피팅 스코프''는 자유롭게 잡을 수 있다 — 예컨대 $(\Delta H_\rxn,\Delta S_\rxn,Q,w)$ 를 풀고
$(\Omega,\Delta H_a,\code{dVdq\_qa})$ 는 좁은 상하한으로 두는 식이다. 리액션$\cdot$DFT 값은 신뢰 상한이 아니라
...
\bibitem{menetrier1999} M. M\'en\'etrier, I. Saadoune, S. Levasseur, C. Delmas, ``The insulator-metal transition upon lithium deintercalation from LiCoO$_2$: electronic properties and $^7$Li NMR study,'' \emph{J. Mater. Chem.} \textbf{9}, 1135 (1999). DOI: 10.1039/a900016j.
\bibitem{motohashi2009} T. Motohashi, T. Ono, Y. Sugimoto, Y. Masubuchi, S. Kikkawa, R. Kanno, M. Karppinen, H. Yamauchi, ``Electronic phase diagram of the layered cobalt oxide system Li$_x$CoO$_2$ ($0\le x\le1$),'' \emph{Phys. Rev. B} \textbf{80}, 165114 (2009). arXiv:0909.3556. DOI: 10.1103/PhysRevB.80.165114.
...
\bibitem{reynier2004} Y. Reynier, J. Graetz, T. Swan-Wood, P. Rez, R. Yazami, B. Fultz, ``Entropy of Li intercalation in Li$_x$CoO$_2$,'' \emph{J. Electrochem. Soc.} \textbf{151}, A422 (2004). DOI: 10.1149/1.1646139. [tier B: partial molar $\Delta S_\elec\approx0.18\,k_B$/atom in metallic O3 phase]
\bibitem{reynier2009} Y. Reynier, J. Graetz, R. Yazami \emph{et al.}, ``Effect of entropy change of lithium intercalation in cathodes and anodes on Li-ion battery thermal management,'' \emph{J. Power Sources} (2009). DOI: S0378775309021119. [single-electrode LCO $\partial\varphi/\partial T\approx+0.83$ mV/K, tier B]
\bibitem{garikipati2024} K. Garikipati \emph{et al.} (Garikipati group), ``Bridging scales with machine learning: from first-principles statistical mechanics to continuum phase-field modelling of order--disorder transitions in LiCoO$_2$,'' \emph{J. Mech. Phys. Solids} \textbf{190}, 105727 (2024). arXiv:2302.08991. [config-only DFT+MC cannot reproduce MIT two-phase plateau at 3.9 V; electronic entropy required]
\bibitem{msmr2023} D. A. Verbrugge \emph{et al.}, ``Thermodynamic model for substitution reactions and phase transitions in electrode materials for lithium-ion batteries,'' \emph{J. Electrochem. Soc.} \textbf{168}, 010526 (2021). DOI: 10.1149/1945-7111/abd518. [MSMR framework]
\end{thebibliography}
