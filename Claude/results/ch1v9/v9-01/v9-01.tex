...
\caption{LCO 양극 전이 초기값 \code{LCO\_STAGING\_LIT}(3건, tier A=Xia; 하프셀, ≤4.2--4.5 V). $\Delta S_\rxn^\cat$ 부호: 양극 양수 = Li 방출 시 엔트로피 증가. T4 O3→H1-3($\sim$4.55 V)는 ≤4.5 V 범위에서 선택적.}
\label{tab:lco_staging}
\begin{tabular}{l c c c c c}
\toprule
전이 & $U_j^\cat$ [V vs Li] & $x$ 범위 & 성격 & $\Delta S_{\rxn,j}^\cat$ 특징 & 비고 \\
\midrule
T1 MIT         & $\sim3.90$ & $x\approx0.94$--$0.75$ & 절연체$\to$금속(MIT) & config + $\Delta S_\elec$ 게이트 ON & 전자항 발현 핵심 \\
T2 order--dis a & $\sim4.05$ & $x\approx0.55$          & hex$\to$monoclinic  & config 주도 ($\sim0.47$ J/K$\cdot$mol) & 2상역 \\
T3 order--dis b & $\sim4.17$--$4.20$ & $x\approx0.48$   & monoclinic$\to$hex  & config ($\sim1.49$ J/K$\cdot$mol)      & T2 와 한 쌍 \\
\bottomrule
\end{tabular}
\end{table}
...
\label{eq:dUdT_cat}
\end{equation}
은 흑연 $\partial U_j/\partial T=\Delta S_{\rxn,j}/F$ 와 1:1 동형이다. 부호 주의: 단전극 LCO 하프셀 실측 $\partial\varphi/\partial T\approx+0.83$ mV/K (전체 부호 sanity; tier B)이며, 양극 중심 전위는 흑연보다 매우 높다($U^\cat\sim3.9$--$4.2$ V vs Li).
...
\end{equation}
이것이 \textbf{Sommerfeld 전자 엔트로피}이며, 단위 자리당($g$ 의 단위에 의존), $T=0$ 에서 $S_e=0$, $T$ 에 비례해 증가한다. \textbf{★ $\propto T$ 명시} — config·vib 의 $\Delta S_\rxn$ 이 $T$ 독립 상수인 것과 달리, 전자 항은 $T$-선형이다. 이 차이가 Ch.2 가역열 전개에서 중요해진다.

\textbf{크기 검산(tier B).} $g(E_F)\approx13$ e/eV, $k_BT\approx0.0259$ eV(25$^\circ$C):
$S_e/k_B=(\pi^2/3)\times0.0259\times13\approx1.1\,k_B$/atom (완전 금속). 실측 O3(부분) 부분몰차 $\approx0.18\,k_B$/atom\cite{reynier2004} — 자릿수 일치.

\subsection{부분몰 전자 엔트로피 $\Delta S_\elec(x,T)$}
\textbf{(a)(b) 출발·연산.} 삽입 반응의 맥락에서 Li 1몰 \emph{제거}당(탈리튬화 $x\!\downarrow$) $S_e$ 가 어떻게 변하는지를 묻는다. $S_e$ 는 $x$ 에 의존하므로($x\!\downarrow$ 시 MIT 로 $g(E_F)\!\uparrow$), Li 제거 1몰당 전자 엔트로피 변화는 — Ch.1 규약에서 $\Delta S_\rxn$ 은 Li 제거 방향을 양으로 정의하므로 — $x$ 에 대한 기울기의 \emph{부호 반전}으로 정의된다:
\begin{equation}
\Delta S_\elec(x,T)\;\equiv\;-\frac{\partial S_e}{\partial x}\bigg|_T
\;=\;-\frac{\pi^2}{3}\,k_B^2\,T\,\frac{\partial g(E_F,x)}{\partial x}.
\label{eq:dSe}
\end{equation}
탈리튬화($x\!\downarrow$)로 MIT 가 발현하면 $g(E_F)$ 가 증가하므로 $\partial g/\partial x<0$, 따라서 $-\partial g/\partial x>0$ — Li 제거 1몰당 $\Delta S_\elec>0$ (MIT 구간). \textbf{(c) 부호 사슬.} $\Delta S_\elec>0$ 이 $\Delta S_{\rxn,j}^\cat=\cdots+\Delta S_\elec$ (식~\eqref{eq:dS_cat})에 양의 기여를 주어 $\partial U_j^\cat/\partial T=\Delta S_{\rxn,j}^\cat/F>0$ 에 기여한다 — 단전극 dφ/dT$\approx+0.83$ mV/K sanity(tier B)와 정합. \textbf{(d) ★$\propto T$.} 식~\eqref{eq:dSe} 에서 $\Delta S_\elec\propto T$ 이고, 따라서 $\partial U_j^\cat/\partial T|_\elec=\Delta S_\elec/F\propto T$ — 다른 항(상수)과 달리 전자 항만 온도 선형이다.
...
식~\eqref{eq:gEF_model} 을 식~\eqref{eq:dSe} 에 대입하면($\partial g/\partial x<0$ 이므로 $-\partial g/\partial x>0$):
\begin{equation}
...
\label{eq:dSe_model}
\end{equation}
MIT 부근 국소 양봉우리(작지만 0 이 아님). \textbf{왜 config 단독으로 MIT 2상역을 재현할 수 없는가}: config 만으로는 T1 MIT 구간의 $dQ/dV$ 봉우리 위치와 온도 의존을 동시에 맞출 수 없다\cite{ml2024lco} — 전자 항의 $\propto T$ 성질이 T1 봉우리의 $T$-감도를 담당한다.
...
\textbf{흑연.} 흑연은 탈리튬화 전 과정에서 반금속 특성이 유지되고 $g(E_F)$ 변화가 미미하다($\partial g/\partial x\approx0$). 따라서 $\Delta S_\elec^\mathrm{graphite}\approx0$ — 흑연 모델의 $\Delta S_\rxn$ 은 config+vib 만으로 닫힌다. \textbf{LCO 의 필수성.} config 항 단독으로는 T1 MIT 2상역의 $dQ/dV$ 봉우리 온도 의존을 재현하지 못한다\cite{ml2024lco} — $\Delta S_\elec\propto T$ 가 T1 봉우리의 $T$-감도를 제공하는 유일한 성분이다. \emph{이 대비가 통합 챕터의 교육 가치} — 같은 logistic 프레임 위에서 LCO 는 전자 자유도 하나를 추가로 연다.

\begin{codebox}
\textbf{LCO 전자 엔트로피 plug-in(코드 일반화 H).} MSMR $x_j=X_j/\{1+\exp[f(U-U_j^0)/\omega_j]\}$ = Ch.1 transition-logistic 동형\cite{msmr2023}. 따라서 코드 클래스(\code{func\_ksi\_eq}·\code{func\_U\_j} 등)에 (i) \code{LCO\_STAGING\_LIT} 전이 파라미터 교체 + (ii) $\Delta S_\elec(x,T)$ 를 \code{dS\_rxn} 에 plug-in 하면 LCO 적용이 완성된다(구조 변경 0). $\Delta S_\elec$ 는 식~\eqref{eq:dSe_model}: 온도 $T$·조성 $x$·초기값 $(g_{\max},x_\MIT,\Delta x_\MIT)$ 를 받는 함수 \code{func\_dSe(T, x, gmax, xMIT, dxMIT)}.
\end{codebox}
...
$\Delta S_\elec$ 는 §\ref{sec:elec_entropy} 식~\eqref{eq:dSe_model}, T1 MIT 게이트 국소 양봉우리, $\propto T$.
...
\bibitem{motohashi2010} K. Motohashi, T. Ina, S. Yonemura, T. Kamiyama, Y. Funahashi, A. Yoshida, ``Electronic phase diagram of the layered cobalt oxide system Li$_x$CoO$_2$ ($0.0\le x\le1.0$),'' \emph{Phys. Rev. B} \textbf{83}, 195128 (2010). DOI: 10.1103/PhysRevB.83.195128.
\bibitem{reynier2004} Y. Reynier, R. Yazami, B. Fultz, ``Thermodynamics of lithium intercalation into graphites and disordered carbons,'' \emph{J. Electrochem. Soc.} \textbf{151}, A422 (2004). DOI: 10.1149/1.1646139. [LCO tier B value for partial molar $\Delta S_e$]
...
\bibitem{ml2024lco} (Tier G — gap) Simulated evidence that config-only models cannot reproduce MIT two-phase region T-sensitivity in LCO; round-trip fitting required. Internal note, 2024.
\end{thebibliography}
