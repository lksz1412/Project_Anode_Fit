% ====================================================================
% graphite_ica_ch1_v8-05b.tex — Chapter 1 (v8-05b: v8-05 증분 보완)
%   증분(원본 v8-05.tex 보존, 이 파일만 수정):
%     (1) D-PEAK 정정(\S sec:tail): "L_V 작으면 차분식이 종으로 환원"은 틀림 —
%         L_V->0 => rho->0 => 분자 지수적 소멸 => peak->0. 매끈한 환원은 L_V>>Δgrid(rho->1).
%         작은 L_V 평형회복은 eq:branch 스위치 담당(모순 해소).
%     (2) D-VEQ: eq:eqcond 의 μ_Li 에서 g'(ξ)=sF(V_eq-U) 다리(eq:gveqbridge) 명시.
%     (3) orphan bib 인용: dreyer2010(히스 기원)·bazant2013(BV)·eyring1935(k0=kBT/h).
%     (4) 새 회귀 self-test R5: 차분식 환원 방향 수치 못박음(L_V->0 정점=0, not 9.73).
% ----- 이하 원본 v8-05 헤더 -----
...
같은 전이를 서로 다른 준안정 가지로 과주행해 중심이 갈라진다(삽입 전극 히스테리시스의 이 열역학적 기원은
\cite{dreyer2010} 가 다입자 평형의 비볼록성에서 닫았다). 이 절은 그 gap 의 닫힌 식과 부호를, 자유에너지의 이중웰
...
자유에너지~\eqref{eq:gxi} 의 첫 미분이 평형 전위 곡선을 준다. 이 다리를 평형 조건~\eqref{eq:eqcond} 에서 명시적으로
세운다 --- 그 식은 삽입 Li 의 화학퍼텐셜을 $\mu_\mathrm{Li}(\theta_\eq)=\mu^0-sF(V_\eq-U_j)$ 로 적었고, 화학퍼텐셜은
점유 1몰당 자유에너지 변화 $\mu_\mathrm{Li}=\partial g/\partial\theta$(식~\eqref{eq:mudef} 의 자리당 형태)다. 진행률은
점유의 여수 $\xi=1-\theta$ 라 $\partial/\partial\theta=-\partial/\partial\xi$ 이므로 $\mu_\mathrm{Li}-\mu^0=-g_j'(\xi)$ 이고,
이를 평형 조건에 넣어 $-g_j'(\xi)=-sF(V_\eq-U_j)$, 곧
\begin{equation}
g_j'(\xi)\;=\;sF\,(V_{\eq,j}-U_j)
\label{eq:gveqbridge}
\end{equation}
가 선다 --- 자유에너지의 $\xi$ 기울기(진행 1단위당 열역학 비용)가 전기 일 $sF(V_\eq-U_j)$ 와 균형을 이룬다는 평형
다리다($\mu^0$ 는 식~\eqref{eq:eqcond} 의 중심 정의로 $U_j$ 에 이미 흡수됨). 이 다리의 좌변에 식~\eqref{eq:gxi} 를
한 번 미분한
\begin{equation}
...
정방향 장벽이 $\chi\mathcal A_j$ 만큼 낮아지고 역방향이 $(1-\chi)\mathcal A_j$ 만큼 높아진다(비평형 열역학에 정초한
Butler--Volmer 형 \cite{bazant2013}):
\begin{equation}
...
\label{eq:kuniv}
\end{equation}
이다(전이상태이론의 보편 시도 빈도 $k_0=k_BT/h$ 는 Eyring 의 활성 착물 이론에서 온다 \cite{eyring1935}; 괄호 인자
$(1+e^{-\mathcal A/RT})$ 가 식~\eqref{eq:Lqfull} 분모의 출처; 정방향 장벽의 전달 계수 $\chi$ 와 활성화
...
의 출력). 이 차분식이 어느 극한에서 평형 종으로 환원되는지를 정확히 짚어야 한다 --- 점화식~\eqref{eq:lowpass} 의 한 줄
대수로 분자가 $\xi_{\eq,i}-\xi_{\mathrm{lag},i}=\rho\,(\xi_{\eq,i}-\xi_{\mathrm{lag},i-1})$ 이므로, peak\_shape 의 거동은
$\rho=e^{-\Delta_\mathrm{grid}/L_V}$ 가 어디로 가느냐로 갈린다. \emph{매끈한 환원은 $L_V\gg\Delta_\mathrm{grid}$ 쪽이다}:
이때 $\rho\to1$, $1-\rho\simeq\Delta_\mathrm{grid}/L_V$ 라 점화식이 연속 1계 지연의 충실한 이산화가 되고, $\xi_\mathrm{lag}$ 가
실제 점유 $\xi$ 의 역할을 해 식~\eqref{eq:peakshape} 가 $r_j/L_V=\dd\xi/\dd V$ 의 종으로 수렴한다(빠른 동역학 $L_q\to0$
극한의 평형 추종). \emph{반대로 $L_V\to0$ 이면 종으로 가지 \textbf{않는다}}: $\rho\to0$ 이라 한 줄 만에
$\xi_\mathrm{lag,i}\to\xi_{\eq,i}$(``한 칸 뒤처진'' 신호가 아니라 원신호 자체로 붕괴)가 되어 분자가 $\rho$ 처럼 지수적으로
$0$ 으로 죽고, 분모 $L_V$ 는 그보다 느리게(로그 규모로만) 줄어 차분식 전체가 $\to0$ 으로 사라진다 --- 곧 작은 $L_V$ 에서
차분식 \emph{자체}는 평형 종이 아니라 $0$ 으로 무너진다. 따라서 작은 $L_V$ 의 평형 회복은 차분식의 연속 극한이 아니라
\emph{코드의 명시 분기 스위치}~\eqref{eq:branch} 가 담당한다 --- $L_V$ 가 격자 몇 칸보다 짧아져 차분 추정이 신뢰를
잃기 전에 코드가 해석적 종~\eqref{eq:eqpeak} 으로 갈아탄다(이 두 진술은 모순이 아니라 역할 분담이다 — 매끈한 극한은
큰 $L_V$, 작은 $L_V$ 의 환원은 스위치):
\begin{equation}
\text{peak\_shape}=
\begin{cases}
...
\item \textbf{차분식 환원 방향(\S\ref{sec:tail} 정정의 못박음).} 단일 logistic($w=RT/F$, 해석 종 정점
$1/(4w)=9.730$)에 점화식~\eqref{eq:lowpass}$+$차분식~\eqref{eq:peakshape} 를 격자 $\Delta_\mathrm{grid}$ 위에서
적용하면, 차분식 정점은 \emph{큰 $L_V$ 에서} 종으로 수렴하고 \emph{작은 $L_V$ 에서 $0$ 으로 죽는다}:
$L_V=50\Delta_\mathrm{grid}$($\rho=0.980$)에서 정점 $9.61$($=$ 해석 종의 $98.8\%$), $L_V\!\to\!0$($\rho\!\to\!0$)에서
정점 $\to0$(종의 $0\%$). 스위치 문턱 $L_V=2\Delta_\mathrm{grid}$($\rho=0.607$)에서는 차분 정점이 $7.50$(종의 $77\%$)에
그쳐, 코드가 이 아래로 해석 종~\eqref{eq:eqpeak} 으로 갈아타는~\eqref{eq:branch} 까닭을 수치로 확인한다 --- ``작은
$L_V$ 가 차분식 자체로 종이 된다''는 거짓 진술이 이 한 줄로 falsify 된다($L_V\!\to\!0$ 정점은 $9.73$ 이 아니라 $0$).
\end{enumerate}
