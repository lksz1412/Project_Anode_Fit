...
주고받는 열}이다. SOC 에 따라 $\Delta S(x)$ 의 부호가 바뀌므로 흡$\cdot$발열이 교대한다: 저-$x$ 에서 config
양의 발산이 지배해 방전 시 $\dot Q_\rev>0$(발열), 고-$x$ 에서 vibrational 음 baseline 이 드러나 흡열로 돈다.
stage $2\to1$($x\approx0.5$)의 큰 음의 $\Delta S$ 는 가역 흡열의 두드러진 신호를 남기며, 이는 calorimetry 의
가역 흡열 직접 관측과 정합한다 \cite{standardised2024}. 충/방전 대칭(가역)이고, 비선형 $\partial U_\oc/\partial T(x)$
는 분포의 $x$-의존을 직접 반영한다 — 모양이 곧 분포다.
...
\begin{srcbox}
방법론 출구. 전이별 중심 표준값 $\Delta S^0_j$ 는 다온도 $dQ/dV$ 동시 피팅으로 추정한다 — MSMR
(Multi-Species, Multi-Reaction)이 반응별 $U_j^0(T)$ 를 온도 연속함수로 두어 $\dd U_j^0/\dd T$(=엔트로피
계수)를 뽑는 방식과 직접 대응하며 \cite{msmr_partI,msmr_partII}, Allart 값으로 검증, Bernardi 를 출구로 삼는다.
저율 준평형 데이터$\cdot$과평활 회피$\cdot$분극 분리(Chapter 1 의 $V_n=V_\mathrm{app}-\sigma_d|I|R_n$)가 피팅
전제다. 봉우리 내부 $x$-의존은 식~\eqref{eq:single_config} 의 분포 config 가 자동으로 채운다.
\end{srcbox}
