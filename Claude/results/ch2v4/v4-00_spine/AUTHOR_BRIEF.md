...
5. ★**섞임/겹침 수식**: A 겹침 가중 $\partial U_\oc/\partial T=\frac1F\sum Q_jg_j\Delta S_{\rxn,j}/\sum Q_jg_j$(+수치검증)·봉우리 내부 config(★이중계산 B: ΔS^0_j=중심 표준값)·C $w_\eff(\Omega)=w/(1-\Omega/2RT)$·D 히스 분기별 ∂U/∂T(가역 평균/비가역 분리).
