# v2.0.0 Plan — 수식 연구 진보 마스터 플랜 (열역학·동역학 관점의 일반화 재구축) · 통합 초안 v3

> **초안 v3 · 워크플로 수정 에이전트 [fixer](Fable 5.1) · 2026-09-03 — v2(통합 저작 에이전트 [integrator], 2026-09-03)에 검수 sub 감사 발견 55건(SPEC-01~13 · FORMAT-01~22 · LOGIC-01~20)을 반영한 판. 반영 내역 = 같은 폴더 `fix_change_log.md`.** v2 는 직전 초안 v1(`iter_1/plan_draft.md`, 작업 sub — 본 문서에서 "v1 초안"·"v1 sub 실측"·"v1 work_log DQ-n" 은 전부 이 파일과 `iter_1/work_log.md` 를 가리킨다)을 시드로 하고, 판독 산출 8본(`wf/R1`~`R7`)을 반영해 보강·정정한 **초안**이다. 최종 통합·commit·`Claude/plans/2026-09-02-v2-master-plan.md` 저장은 master 가 한다. 본 파일은 handoff 산출물이다.
> **성격** = 마스터 플랜(top-level). 실행은 사용자 GO 뒤의 일이며 본 문서는 계획만 담는다. 페이즈별 세부 계획서(1e)의 3단 구분 = **작업 챕터 1** 은 본 문서의 Phase 1.1~1.5 절이 곧 세부 계획서(Step 단위까지 세부화 — 진행 예정 Phase) · **작업 챕터 2·3** 은 본 문서에 Phase·Step 범위·게이트·중단·다음 조건을 적고, 각 Phase 착수 시 그 절을 시드로 **별도 세부 계획서** `Claude/plans/2026-MM-DD-v2-phase-<id>-plan.md` 를 작성한다 · **챕터 4~6** 은 Phase 목록·게이트와 "3.7 확정 후 세부화" 표시까지만 적고 3.7 확정 뒤 별도 세부 계획서를 쓴다. "매 Step 착수 시 세부 계획서 재독" 의 대상은 챕터 1 = 본 문서의 해당 Phase 절, 챕터 2 이후 = 그 별도 세부 계획서다. 매 Phase(1.1~6.3 전건) 종료 시 `PHASE_<id>_V2_<topic>_RESULT.md`+`.json` 을 남기고(스킬 2b·헌법 §0), 챕터 말 Phase(1.5·2.7·3.7·4.9·5.3·6.3)는 챕터 통합 Result 를 겸한다.
...
| 군 | 실측(v1 sub) | 판독 8본이 전문 정독한 것(중복 제거 전 합산 — 참조용; 본 arc 정독 배정을 줄이지 않는다, DR-7 (i)) | 미검독(정독 배정 Phase) |
|---|---|---|---|
...
| `HANDOVER*.md` | 25(old/ 제외) / old/ 포함 28 · 줄수 1612 — ★brief §3-C 는 "28본(old/ 제외)·1612줄" 이라 건수가 다른데 같은 줄수 1612 가 두 건수에 붙어 있어 **한쪽은 틀렸다**(1.1 게이트: 파일명·줄수로 차이 열거 후 정본 확정); `HANDOVER_v24.md` 사본 4곳 | R1 14본·R2 7본·R3 1본(중복 포함) → 고유본 약 20 | v1.0.10(HANDOVER_v1.0.10)·v1.0.12·구트랙 3 등 잔여 + hash 판정 |
...
|---|---|---:|---|---|---|---|
| **1.1** | 인벤토리·정독 배정 | 1–2 | 인벤토리 전건 path+줄수(Test-Path True 100%)·빌드 포함/orphan 열·brief §3-C 건수 차이 열거·untracked Claude 8 지위/Codex 13 무접근 표기·정독 배정표 = **전건 arc 내 신규 전문 정독**(판독 기정독은 참조 열·배정 축소 없음 — DR-7 (i)) + 정독 주체 열 · Phase Result | DR-7 | 본 §2.8·§2.9 | 대기 |
...
| **1.5** | Result — 챕터 1 | 13 | 챕터 통합 Result `PHASE_1_V2_HISTORY_RESULT.md`+`.json`(1.1~1.4 Phase Result 4쌍 위) · Ledger 행 5 · Read Coverage 행 범위(정독 주체 열) · 문서 렌즈셋 6종 3-Pass | — | — | 대기 |
| **2.1** | 자산 지도 | 14–15 | 카운트 정의 고정(모집단 = 파일 집합 60 = 빌드 58 + 미포함 2) 후 재계수 = brief §4.2 전건 일치 · 마스터 3본 + 지원 4본 전문 정독(행 범위) · boxed 64 ID 표(라벨 부재 1 표시) · 빌드 포함/미포함 열 · 자산 태그 계열 확정 · G-금지 게이트 실물 수 · diff 3+3+hash · touch-up 4건 판정 · Phase Result | — | 본 §2.2 표·R4a §1·R4b §1 | 대기 |
...
| **2.7** | Result — GAP REGISTER | 32 | 챕터 통합 Result `PHASE_2_V2_GAP_RESULT.md`+`.json`(2.1~2.6 Phase Result 위) · GAP 전건 4-tier · Read Coverage = 본 arc Read 로 v1.0.25.1 9214줄 전 영역 + v1.0.25 diff hunk 4건 | — | — | 대기 |
...
| **3.3** | 문건 구조 결정안 | 42–43 | (a)/(b) 각 **10항목**(장단·자산 이동 맵·xr 영향·페이지 추정·P1 원구상 대응·D22 supersede·기준 1~5 정합·승계표·sec16b 분리안·다중 수록 해소) 빈 셀 0 + 빌드 방식 옵션 + 자산 이동 맵 **57/57**(`_sections` 56[orphan 포함] + 독립 부록 1; 마스터 3본은 4.0 골격 소관) → DG-A 안건서 · Phase Result | (DG-A 예고)·DR-12 | R6 §2·R4b §8·R3 A-6 | 대기 |
...
| **3.7** | Result — THEORY BLUEPRINT + ★정지 | 50 | 챕터 통합 Result md+json(3.1~3.6 Phase Result 위) · Phase audit 3-Pass · R5·R6 DQ 24건 해소/이관표 · **DG-A 구조 / DG-B 채택 이론 목록(A/B 후보 명단 + DR-10·15~18 연동) / DG-C 버전 라벨** 확정 전 저작 착수 금지 | ★정지 | — | 대기 |
| **4.0~4.9** | 저작 v2.0.0 | 51– | 절 단위 루프 · 빌드 게이트(xelatex 3-pass err 0·undefined 0·STRUCTURE PASS) · 본문 코드 토큰 0 · Phase 별 Result · A/B 채택 항목별 (a)~(d) 사슬 · T-19 사다리 대조표·T-20 교재 형식·T-21 가독성 절별 게이트 · boxed 회수 합 64/64(27+10+4+3+16+4) | 3.7 확정 후 세부화 | R5·R6 배치 열 | 대기 |
| **5.1~5.3** | 서지 완결 | (4 에 이어 연속) | 원장 DOI 전수 검증·한 문헌 한 키·저자 전원 · 절별 밀도 하한 · Result | 3.7 확정 후 세부화 | R7 §5 | 대기 |
| **6.1~6.3** | 검수·수렴·마감 | (5 에 이어 연속) | 10R + 커버리지×렌즈 6종 완주 **둘 다**(follow 변형 = 수식·박스·라벨만 노출한 청크 ≥1회) · P3 8항(현행 기호 재해석)+헌법 3종+F 게이트+T-19/T-20/T-21 · 빌드 GREEN·PDF·HANDOVER·INDEX·commit·push | 3.7 확정 후 세부화 | — | 대기 |
...
- **코드 동기(doc-leads)** — 문건 확정 후 **별도 플랜**(DR-4). 본 계획은 문건. 기존 코드(`Anode_Fit_v1.0.24.py`·조사 코드 `regsol_kernel.py` 등)는 읽기·실행(수치 대조 목적)만 허용하고 수정하지 않는다. `CODE_GUIDE_v24`·`FITTING_GUIDE` 미갱신(R2:P-14)도 별도 플랜 소관.
...
- 효율을 이유로 정독·검수 하한을 낮추는 것 X(사용자 기준 6). 판독 시드가 있어도 정독 배정을 줄이지 않는다 — 기정독 파일도 본 arc 작업 sub 가 신규 전문 정독한다(DR-7 (i) 기본값; 축약 여부는 DR-7 에서 사용자가 결정).
...
| OUT-DETAIL | 페이즈별 세부 계획서 | 챕터 1 = 본 문서 Phase 1.1~1.5 절 / 챕터 2·3 = Phase 착수 시 `Claude/plans/2026-MM-DD-v2-phase-<id>-plan.md`(본 문서 Phase 절이 시드) / 챕터 4~6 = 3.7 확정 후 별도 파일 | 생성 | 각 Phase 착수 시 | R5·R6 배치 열 | Phase 착수 전 존재 · 매 Step 착수 시 재독 |
| OUT-INV | 이력 인벤토리 | `Claude/results/V2_HISTORY_INVENTORY.md` | 생성 | 1.1 | 본 §2.8·§2.9(v1 초안 §2.8 실측 승계) | Test-Path 100% · 줄수 실측 · orphan 검사 · brief §3-C 차이 열거 · untracked 8/13 · 정독 배정표(정독 주체 열) |
| OUT-REG1 | 등록부 ① 버전별 변경점 | `Claude/results/V2_REG1_VERSION_CHANGES.md` | 생성 | 1.2 | **R1 + R2**(초안·통합 필요) | 31+5 = 36행 · 11열 빈 셀 0 · 불일치 4건 확정 |
| OUT-REG2 | 등록부 ② 유효 결정·제약 | `Claude/results/V2_REG2_BINDING_DECISIONS.md` | 생성 | 1.3 | **R3 등록부 A** | 전건 path+line+등급+상태 · 미검증 0 |
| OUT-REG3 | 등록부 ③ 방향성 유실·park·미결 | `Claude/results/V2_REG3_LOST_DIRECTIONS.md` | 생성 | 1.4 | **R1 §5 + R2 §4 + R3 등록부 B** | 통합 ID · 6열 · 유실 자산 원문 정독 목록 · 명시 결정 대기 표 |
| OUT-GAP | GAP REGISTER | `Claude/results/V2_GAP_REGISTER.md`(+`.json`) | 생성 | 2.1~2.7 | **R4a·R4b·R7 + 본 §2.2 표** | 4-tier · boxed 64 판정 · 간소화 65 · 서지 95 · GAP-ID 통합 |
| OUT-BP | THEORY BLUEPRINT | `Claude/results/V2_THEORY_BLUEPRINT.md`(+`.json`) | 생성 | 3.1~3.7 | **R5·R6 카탈로그 + R7 §4·§5** | 카탈로그·사다리·구조안·skeleton·원장 설계·적대검토 · DG-A/B/C 안건서 |
| OUT-DOC | 새 문건 v2.0.0 | `Claude/docs/v2.0.0/`(DR-2) — 마스터 tex·`_sections/`·프리앰블·bib(원장 생성)·PDF | 생성 | 4.0~4.9 | 현행 60 tex 자산 + 블루프린트 | 빌드 GREEN·STRUCTURE PASS·본문 코드 토큰 0·라벨 매핑 429/429 |
| OUT-REFLEDGER | 서지 마스터 원장 + 검증 로그 | `Claude/docs/v2.0.0/results/V2_REFERENCE_LEDGER.md`(+`.json`) · `refcheck/` | 생성 | 3.5 설계 → 5.1~5.3 확장 | **R7 §2 표·json 부본·§5 규약** | DOI 검증 100% · 기억 서지 0 · 한 문헌 한 키 · bib 생성 |
| OUT-STEP | 스텝 이력 | `Claude/results/Step <N> — <제목>.md` | 생성(step 하나=파일 하나) | 전 Step | — | 5항목 |
| OUT-RESULT | Phase Result | `Claude/results/PHASE_<id>_V2_<topic>_RESULT.md` + `.json` — id = 1.1~6.3 각 Phase(topic 은 세부 계획서에서 확정, 예: 1.1 INV·1.2 REG1·1.3 REG2·1.4 REG3·2.1 ASSET·2.2 DERIV·…) · 챕터 말 Phase 는 챕터 통합 Result 를 겸함(id = 1·2·3·4·5·6, topic = HISTORY / GAP / BLUEPRINT / AUTHOR / BIB / CLOSING) | 생성 | **매 Phase 종료(1.1~6.3 전건)** — 스킬 2b "매 Phase 종료 시 Result 12항목(md+json)·Result 없이 다음 Phase 진입 금지"·헌법 §0 Result 생략 금지 | — | 12항목 · Read Coverage 행 범위(정독 주체 열) |
| OUT-LEDGER | Ledger | `Claude/results/PHASE_1-6_V2_EXECUTION_LEDGER.md` | 생성·갱신 | 각 Phase 종료 | — | 12-col |
| OUT-HANDOFF | 핸드오프 | `Claude/results/handoffs/<task>/` | 생성 | 서브 스폰 시 | 본 폴더 | brief + iter_N/wf |
| OUT-HANDOVER | 인계 | `Claude/docs/HANDOVER_v2.0.0.md` | 생성 | 6.3 | — | 5항목 + chain |
| OUT-INDEX1 | `Claude/docs/INDEX.md` | 갱신(행 추가 — 기존 행 무수정) | 갱신 | 6.3(및 v2.0.0 폴더 생성 시) | — | 본문이 진실·INDEX 갱신 |
| OUT-INDEX2 | `Claude/plans/INDEX.md` | 갱신(활성 행 추가; 스테일 정정은 별도 작업 — `iter_1/work_log.md` DQ-11) | 갱신 | OUT-PLAN 저장 시 | — | |
| OUT-CLAUDEMD | CLAUDE.md 개정안(문서만) | `Claude/results/V2_CLAUDE_MD_REVISION_PROPOSAL.md` | 생성 | 6.3 | R3 A-3 스테일 조항 | 사용자 결정 전 CLAUDE.md 무수정(DR-13) |
...
**목적.** 작업 챕터 1(이력 통합)의 정독 대상 전건을 실물로 고정하고, "효율을 이유로 축약하지 않는" 전문 정독 배정표를 만든다. 판독 8본이 이미 읽은 파일은 참조 열에 **"판독 R# 1회 정독"** 으로 표시하되 배정에서 빼지 않고, **본 arc 의 작업 sub 가 head→tail 로 다시 전문 정독한다**(DR-7 기본값 (i) — brief:138 "전문 정독 = 전부 — 효율 이유로 축약하지 않음"·사용자 기준 6). 검수 sub 는 등록부 확정 근거 행을 원천에서 전건 대조한다 — 작업 sub 정독의 대체가 아니라 cross-check 다. Read Coverage 표에는 **정독 주체 열**(판독 R# / arc 작업 sub / 검수 sub 대조)을 두고, 등록부 3종의 Read Coverage 는 arc 작업 sub 의 Read 만 계수한다. DR-7 (ii)(기정독분 검수 대조 대체)가 채택되면 이 문단과 Step 2 를 그 규칙으로 다시 쓴다.
...
**게이트 1.2(정량).** 필수 행 31/31 + 병기 행 5/5 = 36/36 · 11열 빈 셀 0(근거 없는 셀은 "근거 미발견" 명기 = 빈 셀 아님) · 각 행 근거 path ≥1 · verbatim 등급 열 값 ∈ {verbatim / 요지 / 근거 미발견} · 수치 불일치 4건 확정 또는 "미확정·병기" · 사용자 평 행에 A12 line · R1·R2 DQ 22건(10+12) 각각 "해소/이관/미해소" 표기. **중단 조건.** 없음(부재 근거는 4-tier). **다음 조건.** Step 3~7 이력 파일 + OUT-REG1 저장 + `PHASE_1.2_V2_<topic>_RESULT.md`+`.json` + Ledger 행.
...
**게이트 1.4(정량).** 시드 합집합 전건 등재(합집합 수 명기·누락 0) · 6열(원 지시 시점·유실/park 시점·현행 처리 상태·재개방 후보 여부·근거 path·시드 출처) 빈 셀 0 · (a)~(i) 각 판정에 grep 수치 또는 path 첨부 · 유실 자산 원문 4건 정독/미정독 표시 · 명시 결정 대기 표 ≥9 항목 각각 원 지시 근거 · 4축 대응표 5행. **중단 조건.** 없음. **다음 조건.** OUT-REG3 저장 + `PHASE_1.4_V2_<topic>_RESULT.md`+`.json` + Ledger 행.

## Phase 1.5 — Result: 챕터 1 (Step 13)

- **Step 13**: 챕터 통합 Result `Claude/results/PHASE_1_V2_HISTORY_RESULT.md` + `.json`(1.1~1.4 의 Phase Result 4쌍 위에 챕터 종합; 12항목: Summary / Step Range 1–13 / Inputs / Files Created(OUT-INV·OUT-REG1~3) / Files Updated / **Read Coverage(파일·행 범위 전건 — 본 arc Read 만 계수, 판독 R# 커버리지는 참조 열)** / Execution Evidence / Validation·Gate·Confirmed / Non-Changes / Open Issues / Decision Queue / Next=Step 14) · Ledger `PHASE_1-6_V2_EXECUTION_LEDGER.md` 행 1.1~1.5 · Phase audit(문서 렌즈셋 6종 = 사실 정합·출처/번호/카운트 일치·orphan 0·라벨·용어 잔존·follow-ability·usability — 헌법 "10차원×3-Pass" 의 문서 프로파일(스킬 3c), 3-Pass) · 검수 sub 청크 검수 ≥1R + 연속 2R 확정결함 0 수렴(등록부 3종 = A1·A2 통상 산출물 등급 — DR-23 에서 상향 가능).
...
**게이트 2.1.** 카운트 표 = brief §4.2 전건 일치(정의 문서 첨부) · boxed 표 64행(지위 열 빈 셀 0) · 빌드 포함/미포함 60/60 · 자산 태그 계열 표(상충 해소 명시) · G-금지 실물 수 확정 · diff 파일 집합 = {3+3} + hash · 승계표 시드. **중단 조건.** 카운트 불일치 → 원인(정의·파일 집합) 규명 후 정본 확정(정지 아님). **다음 조건.** Step 14·15 이력 + 마스터 3본·지원 4본 정독 행 범위 + `PHASE_2.1_V2_<topic>_RESULT.md`+`.json` + Ledger 행.
...
**게이트 2.3.** 등록부 65행 × 6열(위치/가정/유효범위/레퍼런스/tier/GAP) 빈 셀 0 · 계보도의 잎 = boxed 64 전건 포함(누락 0) · 부재 단 목록(R5·R6 대조 — 단 ID 집합 23 을 그대로 첨부) · 최우선 지점(S10·S16·S23·G04·G23) 판정 확정. **중단 조건.** 없음(발견은 GAP·정지 아님). **다음 조건.** Step 21~23 이력 + `PHASE_2.3_V2_<topic>_RESULT.md`+`.json` + Ledger 행.
...
**게이트 2.4.** 95/95 행 · DOI 열 값 ∈ {검증 DOI / DOI 없음(서지 확인) / 미검증} · 결함 6 + 헤더 3 GAP 등재 · 원장 대조표(미등재 6·only 4·사본 1) · 절별 밀도 49행(정독 기반 표시) · 공백 목록(주제별·후보 DOI 확인 여부 열). **중단 조건.** 없음(DR-6 불허 시 "미검증" 표기·정지 아님). **다음 조건.** Step 24~26 이력 + `PHASE_2.4_V2_<topic>_RESULT.md`+`.json` + Ledger 행.
...
**게이트 2.5.** 체크표 49(+115)행(= T-20 진단판) · grep 수치 3종 + 헌법 ① 4종 첨부(토큰 정의 문서 동반) · **F-02/F-03 노테이션 grep(확률 P 대문자 0 — 압력 P 수동 제외 · 총량 F/S 대문자 vs 자리당 f_int/s_int 소문자)** · 가독성 7항목 표 빈 셀 0(= T-21 진단판) · 용어 결정 필요 항목 목록(DR-20). **중단 조건.** 없음. **다음 조건.** Step 27~29 이력 + `PHASE_2.5_V2_<topic>_RESULT.md`+`.json` + Ledger 행.
...
**게이트 2.6.** 상충표 4행 × 조건 열 빈 셀 0 · 정식화 문서 (i)~(v) 전항 · 수치는 원천 path+line 인용(새 수치 생성 0) · 판정 기준 ≥5 항목 · 커널 단위 기본값 명기. **중단 조건.** 없음(판정 상충은 표로 분리·정지 아님). **다음 조건.** Step 30·31 이력 + `PHASE_2.6_V2_<topic>_RESULT.md`+`.json` + Ledger 행.

## Phase 2.7 — Result: GAP REGISTER (Step 32)

- **Step 32**: OUT-GAP `V2_GAP_REGISTER.md`(+`.json`) = 2.1~2.6 산출 통합 · 각 GAP = GAP-ID·기준(①~⑤)·위치·성격·4-tier·별칭(R4a/R4b/R7 원 ID)·챕터 3 인계 여부·v2 처리 후보(저작 4.x 배치) · 챕터 통합 Result `PHASE_2_V2_GAP_RESULT.md`+`.json`(2.1~2.6 Phase Result 위) · Ledger 행 2.1~2.7 · Phase audit(문서 렌즈셋 6종) 3-Pass · 검수 sub 청크 검수 ≥1R + 연속 2R 수렴(GAP REGISTER = 통상 등급 — DR-23).
...
**게이트 3.1.** 카탈로그 = 승계 표 항목 전건 포함(누락 0) + R5·R6 후보 전건(A 13·B 20·C 18) 재판정 열 + 신규 항목 서지 DOI/링크 · 평가 열 8 빈 셀 0 · 기각군 재조사 0(기각군에 재평가 행 없음) · 경계 정리 표(중복 계상 0) · 데이터 의존 패키지 목록(H-1 포함). **중단 조건.** 없음. **다음 조건.** Step 33~37 이력 + `PHASE_3.1_V2_<topic>_RESULT.md`+`.json` + Ledger 행.
...
**게이트 3.2.** 사다리 3축 각 단 = 가정·레퍼런스·회수 조건 3열 빈 셀 0 · 부재 단 전건 신규 절 대응 · 정직 조건 4 명기 · 매핑표 64/64 + 15 · 회수 불가 항목 0(또는 설계 GAP 명시) · 통합안 3계열. **중단 조건.** 없음. **다음 조건.** Step 38~41 이력 + `PHASE_3.2_V2_<topic>_RESULT.md`+`.json` + Ledger 행.
...
**게이트 3.3.** 두 안 각 10항목(장단·자산 이동 맵·xr 영향·페이지 추정·P1 원구상 대응·D22 supersede·기준 1~5 정합·승계표·sec16b 분리안·다중 수록 해소) 빈 셀 0 · 자산 이동 맵 **57/57**(`_sections` 56 — orphan 은 56 안에 포함(§2.2) — + 독립 부록 1; v2 의 58 은 orphan 이중 계상; 파일 집합은 T-15 출력에서 기계 생성) · 승계표 · D22 supersede 문안 · 빌드 방식 옵션. **중단 조건.** 없음(결정은 3.7). **다음 조건.** Step 42·43 이력 + `PHASE_3.3_V2_<topic>_RESULT.md`+`.json` + Ledger 행.
...
**게이트 3.4.** 목표 boxed 목록(현행 64 + 신규 ≥ 부재 단 수) · 사슬 계획 4열 + **목표 boxed 별 사다리 좌표(축·단 ID)·가정 ID·레퍼런스 V1 키 열 빈 셀 0(T-19 계획판 — 기준 5 의 저작 게이트를 저작 전에 고정)** · 신규/승계/회수 표시 100% · alias 정책 문서 · dependency graph 1본 + 4분류 표(행 = 순환, 열 = 관여 식·변수·종·분류·해법·유일성 근거). **중단 조건.** 없음. **다음 조건.** Step 44~46 이력 + `PHASE_3.4_V2_<topic>_RESULT.md`+`.json` + Ledger 행.
...
- **Step 48 — 주제별 필수 문헌 맵(리뷰급)**: R7 §4 12주제 ~150행 × 3.2 사다리 단 → 각 셀 = 현행 bib O/△/X · DOI 상태(Crossref 확인/서적/미검증) · v2.0.0 Phase 4.1~4.9 배치 · 성격(원전/교재/리뷰) · 필수/선택 판정 권위(R7 DQ-13: 다공전극군은 Non-goal 경계 표기용 한정 여부 master 삼각검증) · 2.4 공백 + 3.1 신규 → 원장 초판 골격(OUT-REFLEDGER). 정본 후보 표기 = 원장 상태 V0 후보/V1/V2/V3.

**게이트 3.5.** 규약 10조항 확정 문서 · 문헌 맵 축×단 셀 빈 칸 0(공백은 "공백" 명기) · 소급 전사 설계(키 93 + 부록 5 + 후보) · 검증 절차 문서. **중단 조건.** 없음(DR-6 불허 시 미검증 표기). **다음 조건.** Step 47·48 이력 + `PHASE_3.5_V2_<topic>_RESULT.md`+`.json` + Ledger 행.
...
**게이트 3.6.** 검수 보고서(최약점 ≥1·발견 건 4-tier) · 라운드 표(DR-23 기본값 10행) + 커버리지×렌즈 6종 매트릭스 missing 0 · 연속 2R 확정결함 0 · SymPy 재검산 결과 ≥3 항목 · master 삼각검증·수정 이력 · 수정 후 재게이트. **중단 조건.** 없음(FAIL 은 수정·재검). **다음 조건.** Step 49 이력 + `PHASE_3.6_V2_<topic>_RESULT.md`+`.json` + Ledger 행.

## Phase 3.7 — Result: THEORY BLUEPRINT + ★사용자 결정 정지 (Step 50)

- **Step 50**: OUT-BP `V2_THEORY_BLUEPRINT.md`(+`.json`) · 챕터 통합 Result `PHASE_3_V2_BLUEPRINT_RESULT.md`+`.json`(3.1~3.6 Phase Result 위) · Ledger 행 3.1~3.7 · **Phase audit(문서 렌즈셋 6종) 3-Pass · 3.6 검수 라운드 표(커버리지×렌즈 매트릭스·연속 2R 확정결함 0) · R5 DQ 12·R6 DQ 12 각각 "해소/이관/미해소" 표**(2.7 의 R4a/R4b/R7 표기와 동형) · **DG-A 문건 구조(D22 supersede·빌드 방식 포함) / DG-B 채택 이론 목록(A 13 + B 20 후보 명단 각각 채택·조건부·보류 + 데이터 의존 패키지 + DR-10 regsol·DR-15 KWW·DR-16 Eyring·DR-17 §1.18·DR-18 [C-92]·DR-11 부록 편입 연동) / DG-C 버전 라벨(DR-2 재확인·마스터 파일명·xr 키)** 안건을 평문으로 제시하고 **정지**(사용자만 결정 가능한 blocking). 확정 전 챕터 4 착수 금지. 확정 뒤 챕터 4~6 세부 계획서를 작성한다(Correction History 에 재baseline 기록).
...
|---|---|---|---|
| 4.0 | 골격·프리앰블·기호표·빌드 baseline | `Claude/docs/v2.0.0/` 생성 · 마스터 tex(파일명·빌드 방식 = DG-A/DG-C) · `_sections/` 골격(새 접두 규약·승계표) · 공통 프리앰블(F-06 조판 승계·`\newtheorem` 정의·정리·증명 환경 신설·keybox 구현 통일 G41) · **기호 통합표 TH-9.2**(g 4종·q 3종·u_j 2종·γ 2종·F 2종·z 2종·ξ↔θ·Ω 정의식 통일·h_η 포함·U_oc 표기 통일) · 두문자어 표(11종+) · 용어 결정표(regular solution 역어·Onsager·affinity·flux–force·entropy production 등, DR-20) · 라벨 네임스페이스 계획(429 보존/alias 매핑표) · 자산 태그 3분류 · 원장에서 bib 생성 파이프라인(OUT-REFLEDGER) · CRLF/LF 규약(N12) · 도구 동작 확인(`iter_1/work_log.md` DQ-12) · 빈 절 빌드 | xelatex 3-pass err 0 · STRUCTURE PASS · 라벨 매핑표 429/429 · 기호표 충돌 0 · **F-02/F-03 노테이션 grep(확률 P 대문자 0 — 압력 P 수동 제외 · 총량 F/S 대문자 vs 자리당 f_int/s_int 소문자 규약, T-6)** · T-19 대조표 골격(빈 표) · 도구 동작 확인 기록 |
| 4.1 | 열역학·통계역학 기초(일반식) | L0 공준·앙상블 · **TH-1.1 일반 격자기체 Hamiltonian(cluster expansion·Ising 동치)** · L1′ 내부 자유도 · TH-1.2 일반 요동 항등(SM2-A 회수·평균장 감수율 통일 시드) · TH-1.3(C)·TH-2.2(C)·TH-4.3(C) 선택 · **T-1 entropy production 일반 상태식**(L0 동역학) · T-2 선형 flux–force 회수창(warnbox) · TH-9.1 일반→특수 대조표 골격 | (a)~(d) 사슬 100% · T-19 사다리 대조표 절별 빈 셀 0 · T-20 교재 형식 요소 절별 O/X · T-21 가독성 7항목 · 코드 토큰 0 · 신규 서지 V1 등재 · 빌드 |
| 4.2 | 평형 열역학 | 중심(eq:Uj + **TH-7.5 Kirchhoff 일반 U(T)**) · 폭 · 정칙용액(Bragg–Williams 원전) · **TH-5.1 상분리 부록 본문 승격**(ξ↔θ 치환·[A1]~[A5] 키화·spinodal 정본 1곳) · **TH-2.1 정칙용액+Maxwell 두-상 dQ/dV 사슬**(델타+단상 꼬리·⊗ 폭 합성·Dreyer 연결·Ω→2RT⁺ 연속성 명문화) · **TH-3.1 Redlich–Kister → α_j 회수**(opt-in 분기·식별 가드) · TH-2.4 Frumkin 계보 srcbox · TH-5.5·TH-8.5 bgbox(C) · staging 은 4.6 | 회수 조건 검산(Ω→0 eq:eqpeak·Ω≤2RT eq:lcoomega-kernel·L_1=0 eq:dUhys) SymPy · **독립 부록 boxed 3/3 회수**(eq:app-fxi·eq:app-gain·eq:app-binodal — ξ↔θ 치환 후 SymPy 동치) · T-19/T-20/T-21 · 빌드 |
| 4.3 | 동역학 | **K-1 Eyring/TST 척추 재배열**(eq:tst-rate → eq:bv → eq:db → eq:kuniv → eq:tst-box → 세 인자 → L_q·L_V·동결) · K-2 가정 등록 · E-1 일반 BV ↔ κ(ξ) 통일(B) · E-2 Marcus 상단 일반형+회수(C) · (E-3 농도 분극·R-2 KWW·R-4 확산 = DG-B 데이터 패키지) · R-1 master equation → 완화 ODE·Kramers · R-3 Watson 큐뮬런트 · lag·tail(N8 승계) · **S-1 부록 E 3층 승계**(위치 = DG-A) · 전달함수·S-3 Kubo 정적 극한 명명·S-4 각주 | 동결극한·평형극한 회수 검산 · **부록 E boxed 4/4 회수**(eq:sc-true·eq:sc-ratio·eq:sc-valid·eq:sc-transfer — 위치 = DG-A; 4.9 승계 시 4.9 에서 계수) · L4 옵션 채택 시 rank-1 상실 warnbox · T-19/T-20/T-21 · 빌드 |
| 4.4 | 열특성 | TH-7.1 세 분포 분리 가정 · config(승계) · **TH-7.2 포논 DOS 일반 → Einstein/Debye**(파라미터화 Einstein opt-in) · 전자 Sommerfeld(정본 1곳·TH-7.3) · **TH-7.4 일반 엔트로피 수지 → Bernardi → 단일활물질** · **Q-1 Q̇_irr 분해(옴·지연·분기)** · Q-2 히스 소산(완전 cycle, DR-18) · Q-3 소거 항 조건 · TH-6.3 Maxwell 관계 2계 · CLOSING Part 4 (4) n(T)↔config 규칙 명문화 · TH-2.3/1.4 Ω(T)(C·opt-in) | Part T 자산 회수(boxed 10/10) · σ≥0 검산 · T-19/T-20/T-21 · 빌드 |
| 4.5 | 히스테리시스 | spinodal·gap(승계) · **H-2 정칙용액→로지스틱 rung**(극한 회수; 커널 채택은 DG-B/2.6) · **TH-5.2/H-1 CNT+CH → γ_j 함수형**(tier C 상수 남김) · H-3 Preisach 명명 노트·H-4 cusp 각주(C) · Dreyer 다입자(승계) · TH-3.1 비대칭 하의 binodal·spinodal·Maxwell 일반화(TH-8.2) | γ_j→1 spinodal·γ_j→0 Maxwell 극한 회수 · T-19/T-20/T-21 · 빌드 |
| 4.6 | 흑연 적용 | Part II 첫 장 — 현행 Ch1 곡선 사슬 회수(boxed 27/27)·gr2L·gallery≠상·해상도 사다리 · **TH-4.1 다중 부분격자 staging 일반식(설명층)·TH-4.2 Daumas–Hérold/Safran bgbox** · U_j 간격의 열역학 해석 · TH-3.2 dilute 공백 warnbox · tab:staging ΔS 초기값(R2:P-11) · N4 두-상 4 vs 2 확정 표기 | boxed 회수 27/27 · T-19/T-20/T-21 · 빌드 |
| 4.7 | LCO 적용 | 현행 Ch2 회수(16/16; 차이 선도 F-08) · G01 사다리 뒤집기(일반 적분형 박스·상수형 회수) · sec16b 분리(Ω 커널 → 4.2 일반 / 전자항 토글 → 부록 옵션) · G04 Ω↔ECI 축약 수학 보강 · G11 순환 논증 제거 · 검수 ID·버전 라벨 0 · Sommerfeld 정본 참조 | boxed 회수 16/16 · T-19/T-20/T-21 · 빌드 |
| 4.8 | Si·블렌드 적용 | 현행 Ch3 회수(4/4) · eq:blend-dqdv 박스 승격·eq:sifr-blend 지위(G21) · regsol 지위(2.6·DG-B 결과) · G23 Larché–Cahn 유도 + Λ_σ 닫힘 · H-5 분리 branch(B) · GS-1/GS-2 4분류 유지 · SiO_x placeholder 정직 공백 · orphan `ch1_appD_si` 이동 기록 | boxed 회수 4/4 · T-19/T-20/T-21 · 빌드 |
| 4.9 | 부록 | 기호(통합표 정본) · 부호검산 · 코드맵(코드 = 부록 전용 — §3.5 명명 정합·eq:si-code-bitexact 식별자 이동·`ch2_appA_traps` `\code{}` 용법 정정) · 자기일관(부록 E 승계·**S-2 refs 6/7 ② JCP147 PDF 재대조 확정·⑤ dossier 대조**) · **S-5 dependency graph·4분류 표**(P3 #3·#4) · 상분리(승격 후 잔여) · 수학 방법 노트(SURV Tier3 각주군) · numverif2026 이관 | 부록 전건 · 부록 E 4/4(4.3 미계수 시) · **boxed 회수 합 64/64 확인(27+10+4+3+16+4 — T-4·T-12)** · P3 ⑤ 5항(② 미완 0) · P3 #3 graph 1본 · T-19 대조표 전건 완성(TH-9.1 실물) · 빌드 |
...
| Phase | 내용 | 게이트(정량) |
|---|---|---|
| 6.1 | 가변 청크 검수 | **10라운드 + 커버리지×렌즈 6종(구조·적대검산·follow·usable·완결성·regression) 완주 둘 다 충족**(고가치 reference 등급) · 매 라운드 청크 스킴·렌즈 전환 · coverage missing 0 · 수렴 = 연속 2R 확정결함 0 · **follow 렌즈 변형 = 산문을 가리고 수식·박스·라벨만 노출한 청크로 ≥1회 따라오기**(기준 1 "수식만으로도 80~90%" 의 조작적 근사 — proxy 임을 Result 에 명기; 실독자 reading-test 는 미수행) · usable 렌즈 입력 = T-21 표 · **실행 기반 검증 렌즈**(SymPy 재유도(T-12 회수 boxed 64 전건)·수치 극한 Ω→0·L_V→0·α=1·L_1=0·T 극한·기존 코드로 회수 가능한 식의 수치 대조 — 코드 수정 없이) · 수렴 후 편집 시 게이트 재실행(V5.R8 미게이트 회귀 사고 방지, R1 §2-보충) |
| 6.2 | 규범 게이트 | CLAUDE.md P3 8항(현행 기호·조립 단위로 재해석한 확인 가능 조건, T-13) + 헌법 3종(D1~D6 grep/수동 분해) + F-04/F-10/F-11 grep 0 + 버전 번호·검수 ID·고백조 0(T-7 확장) + **T-19 사다리 대조표 전건(간소화 지점 × 가정·유효범위·레퍼런스·회수 조건 빈 셀 0) + T-20 교재 형식 요소 절별 O/X + T-21 가독성 7항목** + 자산 무유실(라벨 429 매핑·boxed 64 회수 = 27+10+4+3+16+4·태그 승계) |
| 6.3 | 마감 | 빌드 GREEN·PDF·렌더 육안 점검(좌측 넘침 F-07) · `Claude/docs/HANDOVER_v2.0.0.md` · `docs/INDEX.md`·`plans/INDEX.md` 갱신 · **OUT-CLAUDEMD CLAUDE.md 개정안 문서 제출(DR-13)** · commit·push(master) · `PHASE_6_V2_CLOSING_RESULT.md`+`.json` · Ledger |
...
- **유닛** = master + 작업 sub + 검수 sub, **직렬**. 동시 산 서브 ≤1. 병렬(fan-out)은 DR-5 sign-off 시에만 · 미승인 병렬 필요는 안 묻고 직렬 진행 + Decision Queue 기록. 판독 단계(2026-09-03)의 8 에이전트 병렬은 본 arc 착수 전 master 워크플로에서 있었던 사실이며, 그 승인 출처(사용자 sign-off·Workflow 지목)는 본 문서 원천에 기록이 없다(**근거 미발견** — 감사 FORMAT-16); master 가 통합 시 출처를 적거나 부록 A DQ-I16 에 남긴다. 본 계획의 유닛 계수 예외로 자기 선언하지 않는다.
...
- **기록** = 스텝 이력 `Claude/results/Step <N> — <제목>.md`(step 하나 = 파일 하나 · 5항목 · 즉시 기록) · Result `Claude/results/PHASE_<id>_V2_<topic>_RESULT.md`+`.json`(12항목) · Ledger `Claude/results/PHASE_1-6_V2_EXECUTION_LEDGER.md`(12-col) · 세부 계획서 = 챕터 1 은 마스터 내 Phase 절 / 챕터 2 이후 = `Claude/plans/2026-MM-DD-v2-phase-<id>-plan.md`(헤더의 3단 구분) · 핸드오프 `Claude/results/handoffs/<task>/` · 인계 `Claude/docs/HANDOVER_v2.0.0.md`.
- **재독 강제** = 매 Phase 착수 시 마스터 플랜 본문 재독(첫 Step 이력에 `[Phase N 착수 — 마스터 플랜 재독]` 기록) · 매 Step 착수 시 세부 계획서 재독 · 매 Phase(1.1~6.3 전건) 종료 시 Result 12항목(md+json) + Ledger(Result 없이 다음 Phase 금지 — 챕터 말 Phase 1.5·2.7·3.7·4.9·5.3·6.3 은 챕터 통합 Result 를 겸함) · 컴팩션·재개 직후 5-check.
- **정지 조건** = 파괴·비가역 · 사용자만 결정 가능한 blocking(**3.7 DG-A/B/C** — 챕터 1~3 에서 유일한 사용자 결정 정지점) · 권한 부족 · 보호영역 침범 · 새 의존성(SymPy 부재 등) · confirmed FAIL gate · 사용자 stop · 통제문서 모순(1.3 — 더 제한적 지시 채택·사용자만 결정 가능하면 정지). **병렬 승인 필요는 정지 사유 아님**(직렬·DQ). DQ 항목은 blocker 아님. 3.7 외의 중간 "재확인" 지점은 두지 않는다(DR-6 (iii) 도 지금 결정).
...
- **검수 강도** = A1·A2 고가치 reference(v2.0.0 문건): 10R + 6렌즈 완주 둘 다(6.1) · **THEORY BLUEPRINT(3.6~3.7): 고가치 reference 등급 — 10R + 커버리지×렌즈 6종 완주 둘 다 + 연속 2R 수렴**(DG-A/B/C 와 챕터 4 전부의 유일 근거 → 헌법 §3 "애매하면 고가치"; 등급 확정 = DR-23) · 챕터 1~2 산출물(등록부 3종·GAP REGISTER): 검수 sub 1R 이상 + 연속 2R 확정결함 0 수렴(통상 — DR-23 에서 상향 가능) · 정형·기계 작업(카운트·grep·Crossref 조회): 자체검수 1회.
...
| T-4 | 자산 카운트(정의 고정) | PowerShell/Python — **카운트 집합 정의: 파일 집합 60 tex(마스터 3 + `_sections` 56 + 독립 부록 1) = 빌드 포함 58 + 빌드 미포함 2(orphan `ch1_appD_si`·독립 부록), 열별 부분합 병기(빌드 포함 58 만 세면 boxed 61·label 392); 주석 제외 여부 명시** — **정의 문서**: display = `equation/align/gather/multline(*)` 230 과 `\[…\]` 38 분리 · boxed 64 전 문건(27+10+4+3+16+4) · cite 명령 265(262+3)/키 단위 315 · bibitem 95/distinct 93 | 카운트 표 + 정의 문서 | v1.0.25.1 = brief §4.2 전건 일치(파일 집합 60 기준) · v2.0.0 = 자산 무유실(라벨 429 전건 존재 또는 alias 매핑표 · boxed 64 회수표 100% = 27+10+4+3+16+4 · 태그 승계표) | 2.1·4.0·6.2 |
...
| T-6 | F-10 용어 | grep: `요동|양성|유일근` → 0 · `음함수|섭동|준위` → 국문 + 첫 출현 영문 병기 · `정준|대정준` 카운트만 · `정규용액|정칙용액` → 통일 역어 + "regular solution" 첫 출현 병기 · 두문자어 표 대조 · **F-02/F-03 노테이션: 확률 대문자 `P(`·`P_i` 류 0(압력 P·전력 등은 수동 판정 제외) · 총량 대문자 F/S vs 자리당 소문자 `f_\mathrm{int}`·`s_\mathrm{int}` 규약 grep(대문자 `F_\mathrm{int}`·`S_\mathrm{int}` 0)** | 히트 수 + 병기 표 + 노테이션 표 | 본문 0(독립 부록 포함) · 병기 100% · 역어 1종 · P/p·F/f 규약 위반 0 | 2.5·4.0·6.2 |
...
| T-8 | 헌법 ③ D3 | boxed 각각 (a)(b)(c)(d) 존재 표(4단 척도·항목 분해) + "대입하면|풀면|수치 확인|표준" 류 점프 grep 후보 | 표 | 현행 64/64 진단 · v2.0.0 목표 boxed 100% 완결(비유도 박스는 해제 또는 규약 표시) · 비박스 결과식 15 판정(라벨 단위) · 6.1 에서 산문 가림 follow 변형 ≥1회(proxy 명기) | 2.2·4.x·6.1 |
...
| T-14 | 기록 완결 | Step 파일 수 = 수행 Step 수 · **Result md+json 쌍 = 종료 Phase 수(1.1~6.3 전건; 챕터 말 Phase 는 통합 Result 로 계수)** · Ledger 행 | 파일 목록 | 누락 0 | 매 Phase |
...
| T-20 | 교재 형식 요소(기준 2 저작 게이트)(신규 v3) | 절별(section·subsection) O/X 표: 정의·정리(명제) 환경(4.0 `\newtheorem` 신설 후) ≥1 또는 "N/A(적용 절)" 사유 · (a)~(d) 표지(유도 절 100%) · 예제 ≥1(유도 절 — R4a DQ-10 (iv) "절당 1건 수준" 후보) · 절 요약 keybox · 기호 첫 출현 정의(기호표 대조) · 도입/마무리 다리 | 표 | 빈 셀 0 · 유도 절 (a)~(d) 100% · 예제·요약·정의 열의 X 는 사유 명기(빈 통과 금지); 하한값 조정은 4.x 세부 계획서 DQ | 2.5(Step 27 진단판)·4.1~4.9·6.2 |
...
11. 이력 문서군 규모 = plans 91(9503줄) + docs/**/PLAN_* 16 + HANDOVER 25(1612줄) + Fable 8(885줄) + CLOSING 106 + INDEX 401 + ledger 30(+docs/vN/results) + 조사 문서군 + v1.0.26 실물 3본 + 서지 원장 4본 — 측정분 ≈ 12,400줄 + 미측정분 + 유실 자산 원문 4건(1.4 Step 11 (i))(DR-7 비용 근거). 판독 8본이 그중 인계·감사·계획서·조사 문서 약 60본을 1회 정독했다(§2.9) — 이는 정독 배정을 줄이는 근거가 아니다(DR-7 (i) 기본값).
12. brief §3-C "plans 90·9567줄·HANDOVER 28(old/ 제외)·1612줄" 은 실측 91·9503 / 25(old/ 제외)·1612 와 다르다 — HANDOVER 는 건수가 다른데 같은 줄수 1612 가 붙어 있어 한쪽이 틀렸다. 1.1 게이트에서 파일명·줄수로 차이를 열거하고 정본화(집계 기준 차이 추정).
...
22. **신규 예정 파일 전건 부재(GO 전 Glob — 1g "없다/신규 작성" 전제)**: `Claude/plans/2026-09-02-v2-master-plan.md` · `Claude/results/PHASE_1-6_V2_EXECUTION_LEDGER.md` · `Claude/results/V2_*.md`(등록부 3종·GAP·BLUEPRINT·인벤토리) · `Claude/results/Step <N> — *.md`(N ≥ 1, 본 arc 명명) · `Claude/results/PHASE_*_V2_*_RESULT.*` · `Claude/docs/v2.0.0/`(항목 4) — 하나라도 존재하면 STOP → Correction History 재baseline. 근거 = 스킬 1g·§2.8 구조 맵 "신규 예정" 목록.
...
- **DR-4 — 코드 동기 = 별도 후속 플랜.**
  - 내용: 본 계획은 문건만. 코드 `Anode_Fit_v1.0.24.py`(release 1.0.25)·조사 코드·`CODE_GUIDE_v24`·`FITTING_GUIDE`(미갱신 R2:P-14)는 수정하지 않고 v2.0.0 문건 확정 후 별도 doc-leads 동기 플랜에서 다룬다. 기존 코드의 **읽기·게이트 실행(수치 대조 목적)** 은 허용(T-12).
...
- **DR-7 — 이력 정독 범위와 판독 기정독분의 취급.**
  ...
  - 기본값: **(i) 전부(기정독분 포함 신규 전문 정독)** — 비용 = 측정분 ≈ 12,400줄 + 미측정분 + 유실 자산 원문 4건. (ii) 채택 시에도 등록부 확정 근거 파일(§2.0 A1~A14·B1~B7 + v1.0.26 실물 3본 + 서지 원장 4본)은 전문 정독 필수 집합으로 고정한다.
...
> 위 DR 확정 + GO 사인이 오면 master 가 최종 마스터 플랜을 `Claude/plans/2026-09-02-v2-master-plan.md` 로 저장하고, 실행 직전 load-bearing 전제(§Assumptions)를 실물 대조한 뒤 Step 1 부터 5-stage 루프로 진행한다. 판독 에이전트의 추가 후보·이견은 부록 A 「Decision Queue」에 있다(master 승격 판단). 아래 부록 A~C 는 11-section 밖의 handoff 전용 노트다.
