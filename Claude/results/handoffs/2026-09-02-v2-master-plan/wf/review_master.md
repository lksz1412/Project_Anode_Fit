...
## 5. v5 재baseline (사용자 결정 DR-1~23 + 신규 요구 · 2026-09-03)

사용자가 DR-1~23 전건에 답했고(6건은 master 위임) "통계역학 전체 → 챕터 1 후반부 부록, 교과서급 확장" 을 추가 요구했다. master 가 v4 를 v5 로 재baseline(스킬 1j 중대 편입 — 기존 계획서에 편입, Phase·Step·게이트 삭제 0). 핵심 변경 = 라벨 v1.0.27 · 진단 대상 v1.0.25/.1/v1.0.26A/B · 구조 = 재료별 3장 유지·Ch1 이론 집중·부록 D 통계역학·Ch2/3 분기·분리 빌드·코드 문건 밖 · 3.7 정지 → 보고점 · 작업 챕터 7 코드 동기 신설 · 정독 = 핵심 전문 + 토픽 한정 · 조사 서브 Haiku/Sonnet · 용어 영어 원어·tier 폐지 · 리뷰 논문 서지 포맷. 상세 = 계획서 Correction History v5 행 + Decisions Required 확정표. 실측 추가: refs 6·7 원문 파일 리포·`_local_only/` 부재(Glob 0건) — JCP147 PDF·jcp_extract(724줄)·dossier 는 존재.

## 6. 미해결(사용자)

GO 사인(DR-9 범위 = 챕터 1→7 연속, 3.7 보고점). DG-B(채택 이론)는 3.7 에서 master 확정·보고.
