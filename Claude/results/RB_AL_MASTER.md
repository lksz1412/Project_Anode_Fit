...
| AL-52 | signed current/coordinate $s_I$·state 유지(branch 진행 방향) | BOUNDED | doyle1993, newman | 10.1149/1.2221597 | Ch5 signed |
| AL-53 | ★ 충전 부호 $s_\phi^\chg=-1$ logistic 유도(기울기 $\partial\xi_\eq/\partial V$ 부호=$s_\phi^b$); branch 역함수 $V_\eq^b=U^b+s_\phi^b w^b\ln[\xi/(1-\xi)]$ (충전 ξ-감소형) | GROUNDED | bardfaulkner, newman, bazant2013 | ISBN; 10.1021/ar300145c | Ch5 chargesign |
| AL-54 | ★ 충전서 $n^\eff I_j\eta_j^b\ge0$ 부호 상쇄(2법칙): $V_\eq^\chg$ 감소형 → $\eta^\chg<0$, $I_j<0$ → 곱>0. robust 근거 = flux $I_j\propto(\xi_\eq^b-\xi_j)$ 와 공액력 $\eta_j^b$ 의 conjugacy(부호 우연 아님) | GROUNDED | degrootmazur1962(book), onsager1931, schnakenberg1976 | 10.1103/PhysRev.37.405; 10.1103/RevModPhys.48.571 | Ch5 qirr_charge |
| AL-55 | ★ branch target $=$ Ch3 $\xi_\ss$; branch 비대칭($\xi_\ss\ne\xi_\eq$)서 Ch3 Level A=Level B detailed-balance 극한 부분 붕괴($C_j$ 이탈/r±의 ξ 의존) | GROUNDED/BOUNDED | dreyer2010, sethna1993, bazant2013 | 10.1038/nmat2730; 10.1103/PhysRevLett.70.3347; 10.1021/ar300145c | Ch5 branch_db |
| AL-56 | hysteresis state $h_j$ 완화(metastable memory) | BOUNDED | sethna1993, dreyer2010 | 10.1103/PhysRevLett.70.3347; 10.1038/nmat2730 | Ch5 hys_state |
| AL-57 | ★ kinetic dissipation($n^\eff I_j\eta_\prog^b$) ≠ metastable free-energy loss($V_\tar^b-V_\eq$); 이중계상 금지 | BOUNDED | dreyer2010, degrootmazur1962(book), bazant2013 | 10.1038/nmat2730; 10.1021/ar300145c | Ch5 loop_heat |
| AL-58 | ★ $\Delta V_\obs\ne\Delta V_\hys$ (관측 loop 폭=분극 $R_n$ 포함 ≠ 진짜 열역학 히스테리시스); $|I_s|\to0$ 잔존/소멸 분리 진단 | BOUNDED | dreyer2010, bardfaulkner, newman | 10.1038/nmat2730; ISBN | Ch5 obs_not_hys |
| AL-59 | loop-area heat $q_\hys$ = 히스테리시스 소산열(entropy production 정합, Ch4); kinetic/metastable 분리비 검증 전제 | GROUNDED/BOUNDED | dreyer2010, degrootmazur1962(book), onsager1931 | 10.1038/nmat2730; 10.1103/PhysRev.37.405 | Ch5 loop_area |
| **수치·closure (Ch6)** | | | | | |
...
| AL-64 | solver/root-find tolerance = 물리 scale($\delta_q Q_\cell$, $\xi\in[0,1]$)·측정 잡음 floor·기계 epsilon 근거(임의값 아님) | BOUNDED | brenan1996, hindmarsh2005 | 10.1137/1.9781611971224; 10.1145/1089014.1089020 | Ch6 solver_tol |
| AL-65 | 분포 축약(Prony/g-grid) 검증; 독립 완화 closure = Ch1 AL-13 평균장(협동적 staging 검증 본 작업 미이행 → FLAGGED 승계) | BOUNDED/FLAGGED(closure) | johnston2006, lindsey1980 | 10.1103/PhysRevB.74.184430; 10.1063/1.440530 | Ch6 grid |
| AL-66 | ★ $\varepsilon_\tol=\max(\varepsilon_\disc,\varepsilon_\noise)$ (측정 기반 정의: 기준해 이산화오차∨ICA 잡음); \emph{특정 수치값}은 데이터 의존 FLAGGED (Phase A ε_tol 허위 정정) | FLAGGED(값) / BOUNDED(정의) | dubarry2022, fly2020 | 10.3389/fenrg.2022.1023555; 10.1016/j.est.2020.101329 | Ch6 epsilon |
| AL-67 | calorimetry 부재 시 발열(Ch2/Ch4)은 forward-prediction 만(역산 금지); 검증 modality 분리 | BOUNDED | bernardi1985, dubarry2022 | 10.1149/1.2113792; 10.3389/fenrg.2022.1023555 | Ch6 falsify |
| AL-68 | 식별성: $R_n$ vs $\chi_j$ 공선형·$R_n\ne R_\ct\ne R_{n,\eff}$ 동시 자유피팅 금지 → 순차 제약(OCV→GITT→Arrhenius→C-rate) | BOUNDED | bardfaulkner, newman | ISBN | Ch6 identifiability |
| AL-69 | Arrhenius 분해: 고정 $\mathcal A_j$ 기울기 $=-(\Delta H_a-\chi_j\mathcal A_j)/R$ = \emph{유효} 엔탈피($\chi\mathcal A/RT$ 가 $1/T$-선형으로 기여); 순수 $\Delta H_a$ 는 $\mathcal A_j\to0$ 외삽/보정. 절편 $k_0\cdot\Delta S_a$ 공선형($\kappa$ 미지 잔여) | BOUNDED | eyring1935, bardfaulkner, fly2020 | 10.1063/1.1749604; 10.1016/j.est.2020.101329 | Ch6 arrhenius |
