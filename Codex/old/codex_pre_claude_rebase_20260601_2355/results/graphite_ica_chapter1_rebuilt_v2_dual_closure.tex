...
Self-consistent integral의 analytic closure는 별도 layer로 둔다. Fredholm / ratio-substitution
route가 검증되면 Plan A로 사용하고, 검증되지 않으면 local ODE와 spectrum-kernel integral을
Plan B conservative theoretical formulation으로 유지한다.
\end{abstract}
...
\section{Dual Closure Hierarchy for Self-Consistent Integral Layer}\label{sec:self_consistent}
...
\subsection{Why two closure routes are kept}\label{subsec:two_routes}

식 \eqref{eq:volterra_q}와 spectrum integral 식 \eqref{eq:tail_integral}을 함께 쓰면 두 종류의
문제가 분리된다.

\begin{enumerate}[label=\textbf{I\arabic*.}]
\item \textbf{Physical tail formation.} Activation barrier distribution이 rate constant
distribution으로, 다시 relaxation-length spectrum으로 바뀌고, 그 spectrum이 local exponential
kernel을 중첩해 observed tail을 만든다.
\item \textbf{Mathematical closure.} \(V_n\), \(\xi_{\eq,j}\), \(k_j\), \(A_L\), \(\xi_j\)가
서로 의존하므로, 실제 fitting expression을 만들 때 self-consistent integral equation을 어떻게
닫을지 결정해야 한다.
\end{enumerate}

Chapter 1의 core theory는 I1이다. I2는 analytic convenience와 fitting tractability를 위한
closure layer다. 따라서 I2의 특정 method 정식화가 성립하지 않아도 I1이 무너지지 않아야 한다.
여기서 Plan B는 구현상 예비 경로가 아니라, Chapter 1이 항상 보존해야 하는 \emph{논리적 기준식}
이다. 본 장은 이 원칙을 반영해 두 route를 병기한다.

\begin{center}
\fbox{\parbox{0.92\textwidth}{
\textbf{Plan A.} Fredholm / ratio-substitution closure가 현재 graphite ICA integral structure에
정합함을 보이면, refs. 6/7 method를 analytic closure로 사용한다.

\textbf{Plan B.} 그 정합성이 증명되지 않거나 오차가 크면, refs. 6/7 closure를 Chapter 1의
핵심식으로 채택하지 않고 local ODE + relaxation-length spectrum kernel integral을 conservative
theoretical core로 유지한다.
}}
\end{center}

\subsection{Plan A: Refs 6/7 as candidate Fredholm / ratio-substitution closure}\label{subsec:refs67}
...
Plan A는 다음 조건을 모두 통과할 때만 Chapter 1의 analytic closure로 승격한다.

\begin{enumerate}[label=\textbf{M\arabic*.}]
\item Charge-balance feedback과 barrier spectrum을 fixed-domain Fredholm-type second-kind
equation으로 재정식화할 수 있어야 한다.
\item Unknown function이 integral 안팎에 같은 function class로 나타나야 한다.
\item Ratio-substitution ansatz가 현재 변수, 예를 들어 \(\xi_j(s)/\xi_j(Q)\),
\(r_j(s)/r_j(Q)\), 또는 \(\Theta(s)/\Theta(Q)\), 중 어느 것에 적용되는지 명시되어야 한다.
\item Single-mode limit에서 식 \eqref{eq:local_exponential}의 exponential kernel로 돌아와야 한다.
\item 식 \eqref{eq:volterra_q}의 exact limiting case, direct mathematical solution, 또는 future
numerical validation과 비교해 closure error가 허용 범위에 들어야 한다.
\end{enumerate}

이 조건이 성립하면 Plan A는 다음과 같은 candidate form을 갖는다. 어떤 effective unknown
\(Y(Q)\)가
\begin{equation}
Y(Q)=Y_0+\int_{\mathcal D}K(Q,S)\,\Phi[Y](S)\,\dd S
\label{eq:closure_general}
\end{equation}
처럼 self-referential하게 나타날 때, refs. 6/7의 핵심은 \(Y(S)/Y(Q)\) 같은 unknown ratio를
노출시킨 뒤, 그 ratio를 simple reference solution 또는 controlled expansion으로 닫는 것이다. 이때
analytic closure는 final truth가 아니라
\begin{equation}
Y_{\mathrm A}(Q)
=
\mathcal C_{\mathrm{ratio}}\!\left[
K,\Phi,Y^{\mathrm{ref}}
\right](Q)
\label{eq:plan_a_closure}
\end{equation}
로 표시되는 candidate approximation이다. 여기서 \(\mathcal C_{\mathrm{ratio}}\)는 specific
ratio-closure operator이고, 그 form은 matching integral equation이 확정된 뒤에만 쓴다.

\subsection{Plan B: conservative theoretical formulation}\label{subsec:planb}

Plan A의 Fredholm 정식화가 성립하지 않아도 Chapter 1의 physical core는 유지된다. Conservative
theoretical formulation은 다음 세 식만으로 구성된다.
...
이 route에서는 refs. 6/7 closure를 Chapter 1의 핵심 논리로 쓰지 않는다. 대신 local ODE에서 local
kernel을 얻고, relaxation-length spectrum 위에서 그 kernel을 중첩한다. 이 경우 얻는 것은
closed-form analytic shortcut이 아니라 안정적인 reduced theory다. 후속 검증 단계에서 이
reduced theory를 기준해로 대조할 수는 있지만, Chapter 1에서 필요한 것은 구현이 아니라
논리 구조다. Low \(T\)에서 large-\(L\) weight가 커지고, positive potential assistance가 spectrum을
short-\(L\)로 shift한다는 Chapter 1의 핵심 physical conclusion은 그대로 유지된다.

\subsection{Decision rule between Plan A and Plan B}\label{subsec:closure_decision}

본 장의 canonical rule은 다음과 같다.

\begin{enumerate}[label=\textbf{R\arabic*.}]
\item Plan A는 \emph{validated analytic closure}가 될 수 있지만, 검증 전에는 core assumption이 아니다.
\item Plan B는 항상 남는 conservative theoretical base다.
\item 논문 본문에서는 Plan A를 사용하더라도, appendix 또는 methods section에 Plan B derivation과
limiting-case check를 함께 제시해 논리 일관성을 확인해야 한다.
\item Plan A와 Plan B가 같은 limiting cases를 주지 않으면, Chapter 1의 core 논리로는 Plan B
formulation만 채택한다.
\item 따라서 refs. 6/7은 battery-specific physical assumption이 아니라 self-consistent integral을
닫는 mathematical method candidate다.
\end{enumerate}
...
\item Self-consistent integral closure는 dual route로 남긴다. Plan A는 refs. 6/7 기반 Fredholm /
ratio-substitution closure이고, Plan B는 local ODE + relaxation-length spectrum kernel integral로
구성되는 conservative theoretical formulation이다.
\end{enumerate}
