...
제시 + barrier distribution 의 non-unique 역산 회피, (iv) fitting solver 구현은 Chapter 1 범위 외 (P1).
...
\item Plan A와 Plan B가 같은 limiting cases를 주지 않으면, Chapter 1의 core 논리로는 Plan B
  formulation만 채택한다.
\item 이 dual hierarchy는 coding route이 아니라 논리적 책임 분리다.
\end{enumerate}
...
제약 필수. fitting solver 구현은 Chapter 1 범위 외 (P1).
...
\item ★ 관측 tail = spectrum kernel integral (S9); closure = \textbf{Plan B (conservative theoretical formulation, 항상 보존) + Plan A (Refs 6/7 instantiated analytic closure, M1-M5+$\varepsilon$ gated) +
