% ====================================================================
% graphite_ica_ch5_rebuilt.tex — Chapter 5 (RB 재구성본)
%   흑연 음극 ICA(dQ/dV) 이론의 충방전·히스테리시스 계층: Chapter 3 의
%   forward/backward kinetics(r_j^±·detailed balance·ξ_ss·keystone) 위에서
%   충전/방전 branch, metastable target, loop area 를 다룬다.
%     · signed current I_s, branch index b, signed state coordinate q_state
%     · ★ 충전 branch 부호 유도(CHARTER step1, Phase A HIGH, Ch4 이월 수령):
%       방전 s_{φ,j}=+1, 충전 s_{φ,j}^b=-1 을 logistic 지수 부호로부터 유도.
%       충전서 𝒜_j·η_j·q_irr 의 부호가 어떻게 뒤집히고, n^eff I_j η^b ≥0(2법칙)
%       이 부호 상쇄로 여전히 성립함을 유도로 보임(주장 금지).
%     · metastable branch target = Ch3 ξ_ss (detailed balance 이탈; dreyer2010/sethna1993)
%     · ★ ΔV_obs ≠ ΔV_hys (관측 loop 폭 ≠ 진짜 열역학 히스테리시스)
%     · q_hys = loop area = 히스테리시스 소산열 (entropy production 정합, Ch4)
%     · Ch3→Ch5 전달: r^±·keystone(Level A=Level B detailed-balance 극한)이
%       branch 비대칭(ξ_ss≠ξ_eq)에서 어떻게 깨지는지 명시.
%
%   재구성 원칙(RB plan, RB_CHARTER, RB_AL_MASTER 상속):
%     · 방향-1 학부 무비약: 모든 식 = 물리 의미 prose 선행 → 중간단계 전부 명시.
%       "자명/clearly/쉽게/obviously" 0건.
%     · 방향-2 흐름 보존: consolidated_ch5 식 순서·역할 유지, 각 식 재유도(차원·부호·극한·2법칙).
%     · 방향-3: 무근거 가정 발견 시 재작성·FLAGGED 표기(★ 충전 부호 "주장만" → 유도로).
%     · 방향-4: cap/clip/step/softplus/max/min/Heaviside 정의식 금지(물리 병목·domain guard 명시).
%     · 방향-5 grounding: 모든 가정 AL-# + tier + cite + 검증 DOI.
%       ★ DOI 정확 귀속: dreyer2010=10.1038/nmat2730, sethna1993=10.1103/PhysRevLett.70.3347,
%         degrootmazur1962=book(DOI없음), onsager1931=10.1103/PhysRev.37.405,
%         schnakenberg1976=10.1103/RevModPhys.48.571.
%     · CHARTER 준수: 충전 branch 부호 s_{φ,j}^b 유도(step1) / keystone Level A↔B 상속(step4).
%   AL 번호 = 통합 master(RB_AL_MASTER.md). Ch5 = AL-50~59.
...
\hypersetup{colorlinks=true, linkcolor=blue, urlcolor=blue, citecolor=blue,
  pdftitle={흑연 음극 ICA tail — Chapter 5 (RB 재구성본)}}
...
\subsection{내부 state $\xi_j$ 유지와 보조 변수 $\zeta_j$}\label{sec:ch5_xikeep}
...
% ====================================================================
\section{★ 충전 branch 부호 인자 $s_{\phi,j}^b$ 의 유도 (CHARTER step1, Ch4 이월 수령)}\label{sec:ch5_chargesign}
% ====================================================================
...
\subsection{logistic 지수의 부호 정합에서 $s_{\phi,j}^b$ 가 나온다}\label{sec:ch5_sign_derive}
...
\subsection{충전서 $\mathcal A_j$, $\eta_j$, $q_\irr$ 의 부호 뒤집힘 (무비약)}\label{sec:ch5_sign_flip}
...
\subsection{Ch3$\to$Ch5 전달: detailed balance$\cdot$keystone 이 깨지는 자리}\label{sec:ch5_dbbreak}
...
% ====================================================================
\section{Hysteresis energy 와 heat (loop area = 소산열)}\label{sec:ch5_hysheat}
% ====================================================================
...
% ====================================================================
\section{Chapter 5 의 가정 원장 (AL-50$\sim$59)}\label{sec:ch5_al}
% ====================================================================
...
AL-54 & ★ 충전서도 $n_j^\eff I_j\eta_j^b\ge0$(2법칙) — 부호 상쇄(충전 $I_j{<}0$ \emph{이고} $\eta_j^\chg{<}0$, 음$\times$음$=$양)로 유도; 가역 열 $q_\rev^b{=}I_j\Pi_j$ 는 $I_j$ 부호 반전으로 branch 부호가변 & GROUNDED & degrootmazur1962(book), onsager1931, schnakenberg1976 (DOI 10.1103/\brk PhysRev.37.405; 10.1103/\brk RevModPhys.48.571) & \eqref{eq:ch5_A_branch},\allowbreak\eqref{eq:ch5_eta_branch},\allowbreak\eqref{eq:ch5_qirr_charge} \\
AL-55 & ★ branch target $\xi_{\tar,j}^b{=}$ Ch3 $\xi_{\sstat,j}^b$(branch-local DB; true-eq 에서 $C_j$ 이탈); Level A ``방향'' 부분 붕괴(target 이력 의존, mobility 불변); metastable free-energy 근거 시만 본문 모델 & GROUNDED/\brk BOUNDED & dreyer2010, sethna1993, bazant2013 (DOI 10.1038/\brk nmat2730; 10.1103/\brk PhysRevLett.70.3347; 10.1021/\brk ar300145c) & \eqref{eq:ch5_branch_target},\allowbreak\eqref{eq:ch5_branch_db},\allowbreak\eqref{eq:ch5_ratio_branch} \\
...
$q_\stt$ 와 reference form 명시; LLI/LAM 임의 흡수 금지 & 통과(식~\eqref{eq:ch5_qstate},\allowbreak\eqref{eq:ch5_relbalance}) \\
방전 기준 $\xi_j$ continuity 유지; $\zeta_j$ 는 종속 보조 변수(독립 state 증식 X) & 통과(\S\ref{sec:ch5_xikeep}) \\
★ 충전 branch 부호 $s_{\phi,j}^\chg{=}-1$ 을 logistic 지수 부호 정합으로 \emph{유도}(주장 X; CHARTER step1) & 통과(식~\eqref{eq:ch5_dxidV_branch},\allowbreak\eqref{eq:ch5_schg}) \\
...
branch forward/backward kinetics 를 Ch3 구조와 연결; branch-local DB; 부호 검산 & 통과(식~\eqref{eq:ch5_branch_db},\allowbreak\eqref{eq:ch5_branch_affinity}) \\
...
★ loop area $=$ 소산열이되 전체 $\ne$ true hys heat; flux--force 후보; entropy production 정합(Ch4) & 통과(\S\ref{sec:ch5_hysheat}) \\
branch-dependent heat 를 물리 근거 항으로 제한; 충전 $\dd\xi/\dd t<0$ sign 명시 & 통과(\S\ref{sec:ch5_branchheat}) \\
rest relaxation 의 eq vs meta 두 후보 구분; branch switching $\xi_j,h_j,V_n$ continuity & 통과(\S\ref{sec:ch5_rest}) \\
...
모든 가정 AL-50$\sim$59 인용; BOUNDED 유효범위 병기; DOI 정확 귀속(오귀속 0) & 통과(\S\ref{sec:ch5_al}) \\
empirical branch fit 을 실무 팁으로 격하; solver 0 & 통과(\S\ref{sec:ch5_ident}) \\
