...
% ====================================================================
% ===== Chapter 1 body (원본 graphite_ica_ch1_rebuilt.tex, 수정 0; heading 1단계 demote) =====
% ====================================================================
\clearpage
\section{Chapter 1 — 열역학 무대 + 극판 전위에 의한 배리어 낮춤 (relaxation-length spectrum)}\label{chap:ch1}
...
$\chi_j$ & --- & 배리어 낮춤 민감도 ($0\le\chi_j\le1$;\brk Ch.3 의 transfer coefficient $\beta_j$ 와 동일물) \\
...
FLAGGED — Ch.3(반응속도론 일반화)$\cdot$Ch.6(수치)에서 검증한다.
\end{flagbox}
...
Ch.6). $L$ 은 $k_j$ 의 \emph{역수}에 비례하므로 $1/k_j=(1/k_0)\exp[+(G-\chi_j\mathcal A_j)/RT]$ 로
...
\begin{equation}
\boxed{\;A_L^{\mathrm{prob}}(L)=\rho_G\big(G(L)\big)\,\frac{RT}{L}\,H(L-L_{\min}),\qquad
A_L^{\mathrm{amp}}(L)=A_0(L)\,A_L^{\mathrm{prob}}(L),\qquad L_{\min}=L_0\,e^{-\chi_j\mathcal A_j/RT}\;,}
\label{eq:spectrum}
\end{equation}
여기서 $H$ 는 Heaviside 계단함수, $L_0=|I|/(Q_\cell k_0)$ 다. \textbf{지지(support) 제한의 근거}: $\rho_G$
는 $G\ge0$ 에만 정의되고(활성화 장벽은 음수가 될 수 없음, \S\ref{sec:notation}), 식~\eqref{eq:L_of_G} 에서
$G=0$ 일 때 $L=L_{\min}=L_0 e^{-\chi_j\mathcal A_j/RT}$ 이며 $\dd G/\dd L=RT/L>0$(단조)이므로
$G\ge0\Leftrightarrow L\ge L_{\min}$ — 즉 $L<L_{\min}$ 에서는 $A_L=0$ 이다(이 절단을 빠뜨리면 음의 배리어
영역을 잘못 적분하게 된다). 변수변환(밀도 보존)에서 나오는 것은 자코비안 $RT/L$(과 위 support) \emph{뿐}이며,
이것이 규격을 보존하는 \emph{확률밀도} $A_L^{\mathrm{prob}}$ 다($\int_{L_{\min}}^\infty A_L^{\mathrm{prob}}\,\dd L
=\int_0^\infty\rho_G\,\dd G=1$). 한편 $A_0(L)$ 는 그와 별개로 mode 별 진폭$\cdot$population$\cdot$접근성을 싣는
\emph{물리적} 가중이다 — 임의의 피팅 knob 이 아니라 입자 크기 분포$\cdot$활성 site 밀도 같은 독립 물리량에서
와야 한다. $A_0\not\equiv1$ 이면 $A_L^{\mathrm{amp}}=A_0\,A_L^{\mathrm{prob}}$ 는 더 이상 규격화된 확률밀도가
아니므로(곱하는 순간 $\int=1$ 이 깨짐) \emph{진폭 스펙트럼}으로 명칭을 구분한다. $A_0\equiv1$(균일 가중)이면
$A_L^{\mathrm{amp}}=A_L^{\mathrm{prob}}$ 로 규격이 보존된다. \textbf{이하 kernel 식에서 $A_L\equiv A_L^{\mathrm{amp}}$
로 표기}한다(관측 꼬리는 물리 진폭 가중된 중첩이므로).
\begin{boundbox}
...
% ====================================================================
% ===== Chapter 2 body (원본 graphite_ica_ch2_rebuilt.tex, 수정 0; heading 1단계 demote) =====
% ====================================================================
\clearpage
...
방향성 barrier lowering($\beta_j$)은 Ch3 Level B.\\
...
$h_\bg$ & V/K & background entropy coefficient $(\partial V_{\bg,\eq}/\partial T)$ \\
...
첫 항 $-U'_j/w_j$ 는 reaction-entropy 기원($U'_j=\partial U_j/\partial T$, \S\ref{sec:ch2_entropy})이고, 둘째
항 $-(V_n-U_j)w'_j/w_j^2$ 는 $(V_n-U_j)$ 의 부호에 따라 transition 의 양쪽에서 부호가 바뀐다(중심 $V_n=U_j$
에서 $0$). 실제 graphite 의 $U_j(T)$(entropy profile)는 비단조라 $(\partial V_{n,\OCV}/\partial T)_q$ 가
...
convention}; 방전 $s_{\phi,j}=+1$). \textbf{주의(혼동 금지):} $\Delta G_{\rxn,j}$(평형 reaction Gibbs energy)
...
\end{equation}
즉 \emph{activation} entropy $\Delta S_{a,j}$ 는 rate 의 \emph{prefactor}(Arrhenius intercept, $k_BT/h$ 보정
후)에서 나온다 — 전이상태와 reactant 의 entropy 차이다. 반면 $\Delta S_{\rxn,j}$(식~\eqref{eq:ch2_entropy_coeff})
...
이것이 Bernardi--Pawlikowski--Newman 에너지 balance 의 control-volume 형태다 \cite{bernardi1985,rao1997}.
...
$\dot{\mathcal Q}_{\rev,n}$ & 평형 entropy coefficient($\partial V_{n,\OCV}/\partial T$) 또는 transition entropy($\Delta S_j$)에 따른 가역 열(열역학 기원, 부호가변) \\
...
flux 와 force 가 식~\eqref{eq:ch2_fluxforce} 에서 같은 부호임이 보장되어 합 $\ge0$ 이 2법칙으로 성립). 과전압
$\eta$ 는 charge-transfer($\eta_\kin$, Ch1 의 effective barrier 를 넘는 활성화), ohmic($\eta_\ohm$, Ch1 의
$R_n$ 분극), transport($\eta_\conc$) 기여의 합으로 분해되며 각각 $\ge0$ 이다.
\begin{boundbox}
...
오목성과 동형, \cite{bazant2013}, \AL{29}). flux 는 $J_j=\dd\xi_j/\dd t=k_j r_j$. linear irreversible
...
3--4 의 forward/backward rate 와 free-energy/entropy-production 구조가 있어야 확정된다. 본 장에서는 후보로
둔다(식~\eqref{eq:ch2_sigma} 의 $\sigma_j\ge0$ 는 비가역 \emph{성분}의 존재는 보장하나, 가역 entropy exchange
와의 분리 비율은 미정).
\end{flagbox}
...
\item 물리적으로 정의된 과전압$\cdot$affinity$\cdot$entropy coefficient 로 열원 항(식~\eqref{eq:ch2_srcdecomp}) 계산:
  ...
  비가역 $q_\irr=|I|\eta$(식~\eqref{eq:ch2_qirr}).
...
% ====================================================================
\subsection{비가역열 tail = ICA tail 의 열적 거울 (consistency 가설)}\label{sec:ch2_thermal_tail}
% ====================================================================
\textbf{물리.} Chapter 1 은 ICA tail 을 relaxation-length spectrum 의 kernel integral 로 설명했다. 같은
relaxation 이 비가역 열을 만들므로(식~\eqref{eq:ch2_sigma}), \emph{같은 kinetic 정보}가 비가역열 tail 에도
나타나야 한다 — 이것이 ICA tail 의 ``열적 거울''이며, calorimetry 로 Chapter 1 의 kinetic 귀속을 \emph{독립
modality 로 반증}할 수 있게 한다.

\textbf{무비약 유도(per-mode).} Chapter 1 post-peak 에서 단일 mode 의 잔차는 $r_j(q)=r_j(q_a)\exp[-(q-q_a)/L]$
...
hypothesis 다. consistency 로만 제시하며, \emph{확정}은 calorimetry cross-check($q_\rev$$\cdot$mixing 항을
먼저 차감한 뒤 비가역열 tail 이 Chapter 1 spectrum $\{L\}$ 로부터의 forward prediction 과 정합하는지) 통과를
...
임의 clip/cap/softplus/step 정의식 0(GS-4). solver/numerical-validation 구현 0(Chapter 6 로 분리).
...
\item 온도 되먹임 시간 적분기$\cdot$root-find solver$\cdot$수렴 판정 (Ch6).
\end{enumerate}
...
% ====================================================================
% ===== Chapter 3 body (원본 graphite_ica_ch3_rebuilt.tex, 수정 0; heading 1단계 demote) =====
% ====================================================================
\clearpage
\section{Chapter 3 — 반응속도론: forward/backward kinetics $\cdot$ detailed balance $\cdot$ Butler--Volmer}\label{chap:ch3}
...
\item \textbf{Level B 는 방향성을 넣는다.} forward 장벽만 $-\beta_j\mathcal A_j$, backward 는 $+(1-\beta_j)\mathcal A_j$
  ...
  Chapter 1 의 $\chi_j$ 가 transfer coefficient $\beta_j$ 와 \emph{동일물}임을 명시한다($\chi_j\equiv\beta_j$).
...
이다($\chi_j\equiv\beta_j$). 방전 기준 $s_{\phi,j}=+1$, $|I|>0$.
\end{groundingbox}
...
폭 $w_j=RT/F$; 일반 effective width 일 때 $n_j^{\eff}\equiv RT/(Fw_j)$.\\
...
heat-force $\eta_j$(국소 평형 이탈) 구분; $R_n\ne R_{n,\eff}$. 본 장은 $n^\eff=1$ 특수형 한계를 일반화한다.\\
...
$k_j^{\mathrm{phys}}$ & 1/s & Level A mobility $=r_j^++r_j^-$ (평형 접근 속도; (L2) 의 $k_j$ 와 동일물) \\
$\xi_{\sstat,j}$ & --- & 비평형 정상(stationary) 진행률 target $=r_j^+/(r_j^++r_j^-)$ \\
$\beta_j$ & --- & transfer coefficient ($0\le\beta_j\le1$); \textbf{$\equiv\chi_j$}(Ch1 배리어 낮춤 민감도) \\
...
$\alpha_n$ & --- & Butler--Volmer transfer coefficient (계면; $\beta_j$ 와 같은 계열) \\
...
\begin{equation}
\boxed{\;\mathcal A_j=s_{\phi,j}\,n_j^{\eff}\,F\,[V_{n,\drive}-U_j(T)]\;,\qquad \eta_j\equiv\frac{\mathcal A_j}{n_j^{\eff}F}=s_{\phi,j}[V_{n,\drive}-U_j(T)]\;.}
\label{eq:ch3_Aj}
\end{equation}
$s_{\phi,j}\in\{+1,-1\}$ 는 $\mathcal A_j>0$ 이면 방전 방향 진행이 촉진되도록 선택한다(방전 기본 $s_{\phi,j}=+1$).
...
로 환원된다(본 장이 일반화하는 대상). 식~\eqref{eq:ch3_Aj} 둘째 식은 $\eta_j$ 가 $\mathcal A_j$ 를 $n_j^\eff F$
로 정규화한 \emph{전위 단위}[V] 구동력임을 보인다(Ch2 의 dissipation force $\eta_j$ 와 같은 단위; 단 Ch2 의
$\eta_j$ 는 \emph{현재} $\xi_j$ 의 국소 평형 $V_{\eq,j}(\xi_j)$ 기준, 여기 $\eta_j$ 는 \emph{중심} $U_j$ 기준
— \S\ref{sec:ch3_db} 검산에서 둘의 관계를 재확인한다).
...
\label{eq:ch3_db}
\end{equation}
(둘째 등호: 양변을 $r_j^-(1-\xi_{\eq,j})$ 로 나눔; $0\le\xi_{\eq,j}<1$ 에서 분모 $\ne0$.) 즉 \emph{rate 의 비가
평형 점유율의 odds 와 같다}는 것이 detailed balance 의 내용이다.
...
\end{equation}
이것이 \AL{30}(forward/backward + detailed balance)의 boxed 결과다. \textbf{차원 검산.} 좌변은 rate 비의
...
우변 $(V_n-U_j)/w_j$ 는 \emph{중심} $U_j$ 기준이라 식~\eqref{eq:ch3_Aj} 의 $\eta_j=s_{\phi,j}(V_{n,\drive}-U_j)$
와 같은 ``중심 기준''이고($V_{n,\drive}=V_n$ 기본형서 $\eta_j=V_n-U_j$, $s_{\phi,j}=+1$), Ch2 의 dissipation
...
keystone 의 ``$k_j=r_j^++r_j^-$''(거기서 인수분해로 유도한 것)와 동일하다. 그리고 Chapter 1 의 $\chi_j\mathcal A_j$
...
\emph{동일물}이 된다($\chi_j\equiv\beta_j$).
\end{keybox}
...
\emph{nonequilibrium stationary progress} 다. detailed balance(식~\eqref{eq:ch3_db})가 성립하면 식~\eqref{eq:ch3_split}
의 split 에서 $\xi_{\sstat,j}=k_j^{\mathrm{phys}}\xi_{\eq,j}/k_j^{\mathrm{phys}}=\xi_{\eq,j}$ 로 환원된다. Chapter 5
...
항은 $n_j^{\eff}F(V_n-U_j)/(RT)$ 다. 우변과 \emph{$V_n$ 전 구간}에서 같으려면 (i) $V_n$ 에 \emph{선형}인 항의
...
식~\eqref{eq:ch3_ratio_B} 를 대입한다. $\xi_{\sstat,j}=r_j^+/(r_j^++r_j^-)=1/(1+r_j^-/r_j^+)=[1+e^{-\ln(r_j^+/r_j^-)}]^{-1}
=[1+e^{-(C_j+\mathcal A_j/RT)}]^{-1}$. 정합 제약(식~\eqref{eq:ch3_neff}; $C_j$ 무의존 가정, $V_{n,\drive}=V_n$)
...
\emph{그리고} (ii) ratio $r_j^+/r_j^-$(즉 $C_j$)가 $\xi_j$\emph{-무관}이어야 한다. (ii)가 있어야 $\xi_{\sstat,j}=
...
단계 합성이며, Chapter 1 의 $1/k_j^{\eff}=1/k_j+1/k_{j,\lim}$ 를 forward rate 수준으로 일반화한 것이다(\AL{35}):
\begin{equation}
...
\S\ref{sec:ch3_current}) 병목. $r_j^+\to\infty$(활성화 지배)면 $r_{j,\obs}^+\to\min$ 의 병목 rate 로 매끄럽게
포화하고, 병목이 모두 $\to\infty$ 면 $r_{j,\obs}^+\to r_j^+$ 로 환원된다. \textbf{병목의 비대칭 주의}: forward
...
\emph{대칭}으로 적용해 $r_j^+/r_j^-$ 비를 보존한다 — 식~\eqref{eq:ch3_forward_limiter} 의 forward-only 형은 강구동
branch(Chapter 5)에서 detailed balance 이탈을 \emph{의도적으로} 도입하는 일반 template 이며, 기본 전개의 기본값이
아니다.
\begin{practicebox}
...
diffusion/finite-current 병목으로 대체한다(GS-4; solver 구현은 Ch.6).
\end{practicebox}
...
$r_j^\pm$ 를 \emph{계면 charge-transfer} 수준에서 다시 쓴 것이며, BV 의 $\alpha_n$ 이 Level B 의 $\beta_j$ 와
같은 transfer-coefficient 계열이다(\cite{bardfaulkner,newman}, \AL{31}):
\begin{equation}
...
환산하면 charge-transfer resistance 는 $\eta_n/I_n$ 의 small-signal 값이다:
\begin{equation}
...
=\mathrm{J}/(\mathrm{C}\cdot\mathrm{A})=\mathrm{V/A}=\Omega$($\mathrm{J/C}=\mathrm V$, $\mathrm{C/s}=\mathrm A$이라
$\mathrm{C}\cdot\mathrm{A}=\mathrm{C}\cdot\mathrm{C/s}$? — 명시: $\mathrm{A}=\mathrm{C/s}$ 이므로
$\mathrm{C\cdot A}=\mathrm{C^2/s}$; $\mathrm J/(\mathrm{C^2/s})=\mathrm{J\,s/C^2}=(\mathrm{J/C})/(\mathrm{C/s})
=\mathrm{V/A}=\Omega$) — charge-transfer 저항의 올바른 단위.
\begin{keybox}
...
\end{keybox}
\begin{boundbox}
\textbf{BV $\leftrightarrow$ Level B 대응(층위 구분; \AL{32}).} 식~\eqref{eq:ch3_bv} 의 $\alpha_n$ 은
식~\eqref{eq:ch3_rplus} 의 $\beta_j$ 와 같은 transfer-coefficient 계열이지만, BV 는 \emph{계면 charge-transfer}
...
\emph{일부가 이미 $U_n$ 안에} 있다(그것이 $\eta_n=\phi_s-\phi_e-U_n$ 의 $U_n$). 따라서 $\mathcal A_{\resid}$ 는
$U_n$ 에 \emph{포함되지 않은} residual nonideality, phase-boundary driving force, surface contribution 으로만
해석한다.
\begin{boundbox}
...
\toprule 항 & 물리적 의미 \\ \midrule \endhead
$\eta_{\ct,n}$ & charge-transfer overpotential (식~\eqref{eq:ch3_eta_standard},\allowbreak\eqref{eq:ch3_rct}; $R_{\ct,n}$ 의 전압강하) \\
$\eta_{\ohm,n}$ & electrolyte/solid ohmic drop \\
$\eta_{\conc,n}$ & concentration polarization / transport limitation \\
$\eta_{\film,n}$ & SEI/surface film contribution \\
$\eta_{\resid,n}$ & reduced model 에서 아직 분리 못 한 residual \\
\bottomrule
\end{longtable}
\textbf{Ch1 falsification 연결.} $\chi_j(=\beta_j)$ barrier-lowering 기울기(Chapter 1 spectrum shift
...
체류 시간 & $Q_\cell/|I|$ & 고율일수록 같은 $\dd q$ 에서 반응 시간 짧음 \\
...
\bottomrule
\end{longtable}
따라서 고율 tail/broadening 은 이 요소들의 함수다:
\begin{equation}
...
(Chapter 1 spectrum shift 와 정합; $r_{j,\curr}^+$ 병목, \S\ref{sec:ch3_current}).
...
검증이 필요하다(Chapter 1$\cdot$2 의 forward-only$\cdot$식별성 원칙 계승).
\begin{boundbox}
\textbf{병목 도입 층위 명시(\AL{35}; 비대칭$\Rightarrow$detailed balance 충돌).} Coarse-grained(Level A)에서
$k_{j,\lim}$ 은 $\xi_j\to\xi_{\eq,j}$ relaxation mobility 의 병목이다(대칭, ratio 불변). explicit(Level B)에서는
...
식~\eqref{eq:ch3_forward_limiter} 물리 병목, 수치 cap 은 practicebox 로 격하). solver/numerical 구현 0(Ch.6 분리).
...
★ $\chi_j\equiv\beta_j$ 동일물 명시(CHARTER step4) & 통과(\S 서,\brk keybox,\brk groundingbox) \\
...
% ====================================================================
% ===== Chapter 4 body (원본 graphite_ica_ch4_rebuilt.tex, 수정 0; heading 1단계 demote) =====
% ====================================================================
\clearpage
\section{Chapter 4 — entropy-production 기반 발열 일반화 ($\sum_j n_j^{\eff}I_j\eta_j$)}\label{chap:ch4}
...
heat 의 branch 의존 부호(Ch5)와 수치 해법(Ch6)은 닫지 않는다.
...
=s_{\phi,j}n_j^{\eff}F(V_{n,\drive}-U_j)$, $s_{\phi,j}=+1$ 방전 기본); $\chi_j\equiv\beta_j$ (transfer coeff
동일물).\\
\textbf{(L5) 네 전위 구분.} 내부 $V_n$(보존식 해), apparent $V_{n,\app}=V_n+s_I|I|R_n$(분극 포함, 평형 전위
...
\end{equation}
이며 \textbf{기준 전위는 $V_{n,\drive}$ 단일}이다($V_n$ 과 $V_{n,\drive}$ 혼용 0). 기본 전개에서는 RB\_CHARTER
...
\textbf{부호 양정치 형(CHARTER step1).} 단일 우세 transition 극한에서 이를 전류$\cdot$과전압 크기 곱으로 축약하면
...
EIS/small-signal & $R_\ct$,\brk transport time constant,\brk film \\
...
\toprule AL\# & 가정 진술 & tier & 근거 cite (DOI/ISBN) & 사용 식 \\ \midrule \endhead
AL-40 & entropy production $T\dot S_\irr=\sum_\alpha J_\alpha X_\alpha\ge0$ (flux--force,\brk 2법칙) & GROUNDED & degrootmazur\brk 1962(book),\brk onsager1931,\brk prigogine1967(book) (onsager DOI 10.1103/\brk PhysRev.37.405) & \eqref{eq:ch4_fluxforce} \\
...
AL-47 & transport/병목 발열 $=$ flux--force $\sum_\ell J_\ell X_\ell$ (선형 near-eq 서 $I^2R_\transp$); $R_n\ne R_{n,\eff}\ne R_\ct$ 유지($R_{n,\eff}$ 는 독립 calorimetric 제약 시만) & BOUNDED & degrootmazur\brk 1962(book),\brk newman,\brk bard\brk faulkner (ISBN 978-0-\brk 471-\brk 47756-3;\brk 978-0-\brk 471-\brk 04372-0) & \eqref{eq:ch4_transport_ff},\allowbreak\eqref{eq:ch4_Rsep} \\
...
% ====================================================================
\subsection{Chapter 5 $\cdot$ Chapter 6 으로 전달되는 항목}\label{sec:ch4_transmit}
% ====================================================================
\begin{enumerate}[label=\arabic*),leftmargin=2.0em]
...
% ====================================================================
% ===== Chapter 5 body (원본 graphite_ica_ch5_rebuilt.tex, 수정 0; heading 1단계 demote) =====
% ====================================================================
\clearpage
\section{Chapter 5 — 충방전 branch + 히스테리시스 (충전 부호 유도 $\cdot$ $\Delta V_\obs\ne\Delta V_\hys$)}\label{chap:ch5}
...
수치 해법$\cdot$validation pipeline 은 Chapter 6 로 분리한다.
...
(미시$=$거시, Ch.4); $n_j^{\eff}=RT/(Fw_j)$, $\chi_j\equiv\beta_j$.\\
...
$\Delta U_j^\hys$ & V & 방전--충전 branch center 차(signed;\brk magnitude 강제엔 $s_{h,j}$) \\
...
\item \textbf{방전 branch.} 방전은 전위가 낮아질수록(탈리튬화 진행, $V_n$ 이 평형서 \emph{위}로 구동) 진행이
  촉진되며 $\xi_j$ 가 증가한다. Chapter 1 의 규약대로 $V_n>U_j$ 에서 $\xi_{\eq,j}>1/2$(점유 우세)가 되려면
...
값에서 branch 만큼 \emph{이탈}}했음을 뜻한다(Level A 분업에서 ``방향''이 metastable 자유에너지로 인해 이력을
...
제외한다(GS-4; solver/numerical 구현과 validation pipeline 은 Chapter 6).
\end{practicebox}
...
정의 부여 포함: Y1$\to$AL-50/51/55). tier $=$ GROUNDED/BOUNDED/FLAGGED.
{\footnotesize
\setlength{\tabcolsep}{3pt}
\begin{longtable}{>{\raggedright\arraybackslash}p{0.045\textwidth} >{\raggedright\arraybackslash}p{0.335\textwidth} >{\raggedright\arraybackslash}p{0.115\textwidth} >{\raggedright\arraybackslash}p{0.225\textwidth} >{\raggedright\arraybackslash}p{0.155\textwidth}}
\toprule AL\# & 가정 진술 & tier & 근거 cite (DOI/ISBN) & 사용 식 \\ \midrule \endhead
...
은 practicebox 로 격하). solver/numerical 구현 0(Ch.6 분리). \textbf{DOI 정확 귀속(★ Ch3$\cdot$Ch4 오귀속 교훈
...
% ====================================================================
\subsection{Chapter 6 으로 전달되는 항목}\label{sec:ch5_transmit}
% ====================================================================
\begin{enumerate}[label=\arabic*),leftmargin=2.0em]
...
사슬 안에서 정합하며, Chapter 6 에서 이 구조를 실제 수치 해법$\cdot$validation pipeline$\cdot$GITT/저율/고율
데이터 비교로 연결한다.

\subsection*{Chapter 6 으로의 전달}
...
위에서: Chapter 6 은 stiff kinetics $+$ branch switching 의 시간 적분, 매 시간점 root-find 로 $V_n$ 결정,
...
% ====================================================================
% ===== Chapter 6 body (원본 graphite_ica_ch6_rebuilt.tex, 수정 0; heading 1단계 demote) =====
% ====================================================================
\clearpage
\section{Chapter 6 — 피팅 실무: 식별성 $\cdot$ 순차 제약 $\cdot$ 반증 (수치는 보조 종속)}\label{chap:ch6}

% ====================================================================
\subsection*{서 — 이 장이 답하는 질문: ``유도식을 어떻게 피팅하는가''}
\addcontentsline{toc}{subsection}{서 — 유도식을 어떻게 피팅하는가}
% ====================================================================
...
% ====================================================================
\subsection{Chapter 1--5 에서 전달받는 고정 기준식과 피팅 파라미터}\label{sec:ch6_inherit}
% ====================================================================
...
% ====================================================================
\subsection{Chapter 6 자체 검수표}\label{sec:ch6_selfcheck}
% ====================================================================
{\small
\begin{longtable}{p{0.74\textwidth} p{0.16\textwidth}}
\toprule 검수 항목 & 결과 \\ \midrule \endhead
Ch1--5 본문 수정 없이 Ch6만 적층(P3-6,\brk P5) & 통과(\S\ref{sec:ch6_inherit}) \\
...
% ====================================================================
\subsection{Chapter 6 의 가정 원장 (Assumption Ledger, AL-60$\sim$69)}\label{sec:ch6_al}
% ====================================================================
...
% ====================================================================
\subsection{Chapter 6 의 결론}\label{sec:ch6_concl}
% ====================================================================
...
% ====================================================================
% ===== 통합 참고문헌 (33 키 union+dedupe; refs 문건과 동일) =====
% ====================================================================
\clearpage
\section{통합 참고문헌 (Chapter 1$\sim$6 union, 33 키)}\label{sec:full_bib}
아래 33 키는 Chapter 1$\sim$6 의 \texttt{\textbackslash bibitem} 합집합(중복 제거)이며, 별도 통합 references 문건(\texttt{graphite\_ica\_refs\_rebuilt.tex})의 bibliography 와 동일하다. 통합 Assumption Ledger(AL-1$\sim$69)는 그 문건에 수록.
...
\bibitem{dahn1991} J. R. Dahn, ``Phase Diagram of Li$_x$C$_6$,'' \emph{Phys. Rev. B} \textbf{44}, 9170
  (1991). DOI: 10.1103/\brk PhysRevB.44.9170.
...
\bibitem{funabiki1999jes} A. Funabiki, M. Inaba, T. Abe, Z. Ogumi, ``Stage Transformation of
  ...
  \emph{J. Electrochem. Soc.} \textbf{146}, 2443 (1999). DOI: 10.1149/\brk 1.1391953.
% --- stretched 완화·disorder (Ch1) ---
\bibitem{baessler1993} H. B\"assler, ``Charge Transport in Disordered Organic Photoconductors,''
  \emph{Phys. Status Solidi B} \textbf{175}, 15 (1993). DOI: 10.1002/\brk pssb.2221750102.
\bibitem{svare2000} I. Svare, S. W. Martin, F. Borsa, ``Stretched Exponentials with $T$-Dependent
  ...
  \emph{Phys. Rev. B} \textbf{61}, 228 (2000). DOI: 10.1103/\brk PhysRevB.61.228.
