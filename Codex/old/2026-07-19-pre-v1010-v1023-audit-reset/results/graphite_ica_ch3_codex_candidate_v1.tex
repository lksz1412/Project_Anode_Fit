% Chapter 3: electrochemical kinetic extension of the graphite ICA tail theory.
...
\hypersetup{colorlinks=true, linkcolor=blue, urlcolor=blue, citecolor=blue,
  pdftitle={흑연 음극 ICA tail — Chapter 3}}
...
Level A(mobility) $\to$ Level B(directional barrier, $\beta_j\ne\chi_j$) $\cdot$ Butler--Volmer 계층\\[0.2em]
...
\item \textbf{Level B 는 방향성을 넣는다.} forward 장벽만 $-\beta_j\mathcal A_j$, backward 는 $+(1-\beta_j)\mathcal A_j$
  ...
  $\beta_j$ 는 Chapter 3 에서 새로 도입되는 directional transfer coefficient 이며, Chapter 1 의 $\chi_j$ 와
  동일시하지 않는다. 둘을 비교하려면 Level B 의 $r_j^++r_j^-$ 를 Level A mobility 로 투영한 apparent sensitivity 를
  별도로 정의해야 한다.
\item \textbf{detailed-balance 정합이 $n_j^{\eff}$ 를 고정한다.} Level B 의 ratio 가 Chapter 1 평형(단계 (1))과
  ...
  $\xi_{\sstat,j}$ 가 평형 target $\xi_{\eq,j}$ 로 환원된다 — \textbf{Chapter 3 detailed-balance stationary limit 이
  Chapter 1 Level-A target 을 회복한다}는 keystone 의 \emph{한정} 명제다.
...
coarse-grained 축약형이며}(detailed-balance stationary limit 에서 Chapter 1 target 이 회복됨), 강구동에서 forward rate 증가는
...
$\mathcal A_j$ 에 의존(방향성 O). 본 장 transfer coefficient $\beta_j$ 는 새 Level-B kinetic parameter 이며
Chapter 1 의 $\chi_j$ 와 \textbf{동일물이 아니다}. $\chi_j$ 는 Level-A scalar mobility sensitivity, $\beta_j$ 는
Level-B directional split parameter 이다. 방전 기준 $s_{\phi,j}=+1$, $|I|>0$.
\end{groundingbox}
...
% ====================================================================
본 장은 Chapter 1 V5와 Chapter 2 V1의 다음을 \textbf{수정 없이} 입력으로 받는다. 각 식
번호는 앞 장의 boxed 결과를 가리키며, 본 장은 그 위에 미시 kinetics 계층을 \emph{적층}한다.
\begin{linkbox}
\textbf{(L1) 전하 보존식(무대).} $Q_\cell\,q=Q_\bg^{\eq}(V_n,T)+\sum_{j=1}^{N_p}Q_{j,\tot}\xi_j$. $V_n$ 은 그 해(외부
입력 아님); 평형 특수해가 $V_{n,\OCV}(q,T)$. 단조성 floor $\partial Q_\bg^{\eq}/\partial V_n\ge\epsilon_Q>0$ 로 해 유일.\\
...
$\beta_j$ & --- & Level-B directional transfer coefficient ($0\le\beta_j\le1$); Ch1 $\chi_j$ 와 동일시하지 않음 \\
...
\textbf{구동력($s_{\phi,j}$ 명시형 기준).} Chapter 1 transition 구동력(reduced affinity)을
\emph{본 장에서 기준형으로 동결}한다. 부호 인자 $s_{\phi,j}$ 와 effective electron number $n_j^{\eff}$ 를 명시한
일반형은(\AL{31}; $\mathcal A_j=s_{\phi,j}n_j^\eff F(V_\drive-U_j)$ 명시형 기준)
\begin{equation}
...
\textbf{구동 전위($V_{n,\drive}$ 정의).} 속도식이 쓰는 구동 전위와 내부 전위의 가장 보수적
연결을 \emph{본 장에서 정의로 동결}한다:
\begin{equation}
...
$V_{n,\drive}=V_n$, $\eta_{n,\drive}=0$)으로 둔다. 이는 ``$\eta_j$ 의 기준 전위를 한
...
$(1-\xi_j)$ 에, backward 는 이미 진행한 fraction $\xi_j$ 에 비례한다(2-상태 mass-action; \cite{eyring1935,degrootmazur1962,onsager1931},
\AL{30}):
\begin{equation}
...
\textbf{Chapter 1 logistic 대입(keystone 1차 결과; $w_j$ scale).} Chapter 1 평형 진행률
...
% ====================================================================
\section{Chapter 1 relaxation ODE 와의 정합}\label{sec:ch3_consistency}
% ====================================================================
...
\textbf{Level B}(\S\ref{sec:ch3_levelB})에서 도입한다. 그러나 이때 도입되는 $\beta_j$ 는 방향성 split 을 정하는
새 parameter 이며, Chapter 1 의 $\chi_j$ 와 동일물이 아니다. Level B 를 다시 Level A 꼴로 축약할 때의
mobility sensitivity 는 $r_j^++r_j^-$ 의 국소 projection 으로 정의해야 하며, 그 값은 $\beta_j$ 하나만으로
정해지지 않는다.
\end{keybox}
...
rate 로 세운다. Chapter 1 의 Level-A ``공통 mobility 가속''과 달리, 본 절은 구동력이 forward/backward 방향성을
어떻게 바꾸는지를 분해한다. transfer coefficient
...
($\beta_j$ 가 식~\eqref{eq:ch3_ratio_raw}$\to$\eqref{eq:ch3_ratio_B} 에서 상쇄). 즉 $\beta_j$ 는 detailed-balance
ratio 의 기울기 자체가 아니라, \emph{sum} $r_j^++r_j^-$(=mobility)의 $\mathcal A_j$-응답 \emph{비대칭}을 정한다.
mobility 의 명시형은(\AL{31})
\begin{equation}
...
$\beta_j$ 무관($=\nu_j^+a_j^+e^{-\Delta G^{+\ddagger}/RT}+\nu_j^-a_j^-e^{-\Delta G^{-\ddagger}/RT}$)이다. $\mathcal A_j>0$
이면 forward 항이 $e^{\beta_j\mathcal A_j/RT}$ 로 커지고 backward 항이 $e^{-(1-\beta_j)\mathcal A_j/RT}$ 로 작아진다.
따라서 $r_j^++r_j^-$ 의 apparent mobility sensitivity 는 현재 state, prefactor, forward/backward weight 에 의존한다.
이를 국소적으로 정의하면
\begin{equation}
\chi_{j,\mathrm{proj}}\equiv RT\,\frac{\partial \ln (r_j^++r_j^-)}{\partial \mathcal A_j}
=\frac{\beta_j K_j^+e^{\beta_j\mathcal A_j/RT}-(1-\beta_j)K_j^-e^{-(1-\beta_j)\mathcal A_j/RT}}
{K_j^+e^{\beta_j\mathcal A_j/RT}+K_j^-e^{-(1-\beta_j)\mathcal A_j/RT}},
\label{eq:ch3_chi_projection}
\end{equation}
여기서 $K_j^\pm\equiv\nu_j^\pm a_j^\pm e^{-\Delta G_j^{\pm\ddagger}/RT}$. 식~\eqref{eq:ch3_chi_projection} 이
Chapter 1 의 scalar $\chi_j$ 와 비교될 수 있는 Level-A projection 이다. 그러나 이것은 일반적으로 $\beta_j$ 와
같지 않다. symmetric case($\beta_j=1/2$, $K_j^+=K_j^-$, $\mathcal A_j=0$)에서는 $\chi_{j,\mathrm{proj}}=0$ 이므로,
방향성 split 만으로는 Chapter 1 의 공통 barrier-lowering 항을 재현하지 못한다. 따라서 Chapter 1 의 $\chi_j$ 는
Level-A 공통 mobility sensitivity 로 남고, Chapter 3 의 $\beta_j$ 는 별도 Level-B directional parameter 로 둔다.
\begin{boundbox}
...
따라서 $n_j^{\eff}=RT/(Fw_j)$ 는 선택적 피팅 자유도가 아니라 logistic width 와 전기화학 affinity scale 의 정합 조건이다.
\begin{boundbox}
...
\begin{verifybox}
\textbf{Detailed-balance stationary limit 의 Chapter 1 target 회복.} 식~\eqref{eq:ch3_xiss} 에
...
\textbf{Chapter 1 의 평형 target 은 Chapter 3 detailed-balance \emph{stationary 극한}}에서 회복되며, 두 계층은 충돌하지
않는다. \textbf{한정 — 무조건 ``$A=B$'' 금지.} 일치하는 것은 \emph{정상 target}
...
아님)다. 전하 보존식 (L1) 을 시간 미분하면(Chapter 2 식과 동형, \cite{doyle1993}, \AL{37})
\begin{equation}
Q_\cell\frac{\dd q}{\dd t}=\frac{\partial Q_\bg^{\eq}}{\partial V_n}\frac{\dd V_n}{\dd t}+\frac{\partial Q_\bg^{\eq}}{\partial T}\frac{\dd T}{\dd t}+\sum_j Q_{j,\tot}\frac{\dd\xi_j}{\dd t}.
...
\begin{equation}
|I|=I_\bg^{\mathrm{prog}}+\sum_j I_j,\qquad I_\bg^{\mathrm{prog}}=\frac{\partial Q_\bg^{\eq}}{\partial V_n}\frac{\dd V_n}{\dd t},\qquad I_j=Q_{j,\tot}\frac{\dd\xi_j}{\dd t}.
\label{eq:ch3_current_decomp}
\end{equation}
\textbf{차원 검산.} $I_j=Q_{j,\tot}[\mathrm C]\cdot\dd\xi_j/\dd t[\mathrm{s^{-1}}]=\mathrm{C/s}=\mathrm A$;
$I_\bg^{\mathrm{prog}}=(\partial Q_\bg^{\eq}/\partial V_n)[\mathrm{C/V}]\cdot\dd V_n/\dd t[\mathrm{V/s}]=\mathrm{C/s}=\mathrm A$
— 모두 전류. 비등온서는 $(\partial Q_\bg^{\eq}/\partial T)(\dd T/\dd t)$ 가 추가되며 Chapter 2 처럼 coordinate shift
...
\bottomrule
\end{longtable}
\textbf{Ch1 falsification 연결.} $\chi_j$ barrier-lowering 기울기(Chapter 1 spectrum shift
...
대표적으로 Arrhenius 형(\cite{fly2020}, \AL{39}):
\begin{equation}
...
\begin{equation}
j_{0,n}(T,\theta)\leftrightarrow\nu_j(T)\leftrightarrow\Delta G_{a,j}(T)\leftrightarrow\chi_j\leftrightarrow\beta_j\leftrightarrow k_{j,\lim}\leftrightarrow R_n\leftrightarrow\rho_j(g;T).
...
$n_j^{\eff}=RT/(Fw_j)$ & Ch4 비가역열 일반형 가중; Ch2 특수형($n^\eff=1$)의 모장 \\
...
% ====================================================================
본 장에서 사용한 가정을 Ch3 그룹(AL-30$\sim$39)으로 정리한다. tier = GROUNDED/BOUNDED/FLAGGED 이며,
본문 식은 해당 AL 항목으로 다시 추적할 수 있어야 한다.
{\footnotesize
...
식~\eqref{eq:ch3_forward_limiter} 물리 병목). 구현 절차는 본 장의 이론 범위 밖으로 둔다.
...
둘째, directional barrier lowering(Level B)에서 transfer coefficient $\beta_j$ 는 rate ratio 의
$\mathcal A_j$-기울기에서 \emph{상쇄}되고($\beta+(1-\beta)=1$) mobility(sum)의 $\mathcal A_j$-응답 \emph{비대칭}을
조절한다(식~\eqref{eq:ch3_ratio_B},\allowbreak\eqref{eq:ch3_ksum}). Chapter 1 의 $\chi_j$ 는 Level-A scalar
mobility sensitivity 이며, Level B 와 비교할 때는 식~\eqref{eq:ch3_chi_projection} 의 local projection 으로
연결해야 한다. 즉 $\beta_j$ 와 $\chi_j$ 는 동일물이 아니다. \textbf{Chapter 1 Level-A 는 방향성 없는 공통 mobility
가속(ratio 불변), Chapter 3 Level-B 는 방향성 있는 비대칭 split} 이라는 구분이 명확히 닫힌다.
...
$\xi_{\sstat,j}$ 가 평형 target $\xi_{\eq,j}$ 로 환원된다 — \textbf{Chapter 3 detailed-balance stationary limit 이
Chapter 1 Level-A target 을 회복}하며 일치는 \emph{정상 target} $\xi_{\sstat,j}\to\xi_{\eq,j}$ \emph{한정}이다.
모든
