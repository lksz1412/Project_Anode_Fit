...
$T,\ R,\ F$ & K,\ J mol$^{-1}$ K$^{-1}$,\ C mol$^{-1}$ & absolute temperature, gas constant, Faraday constant. $RT/F$ 는 ideal Nernst voltage scale이다 \\
$k_B,\ h,\ \kappa(T)$ & J K$^{-1}$,\ J s,\ --- & Boltzmann constant, Planck constant, Eyring transmission coefficient. $k_0=(k_BT/h)\kappa(T)$ \\
$q$ & --- & 방전 진행 좌표, $q=Q_\ext/Q_\cell$ \\
$q_a$ & --- & local post-peak tail fitting window의 시작 좌표(anchor); peak 위치 또는 $\partial\xi_\eq/\partial q$가 충분히 작아지는 첫 좌표 \\
...
$Q_\bg^{\eq}(V_n,T)$ & C & staging 으로 분리 안 되는 \emph{equilibrium} 배경 누적 용량; $C_\bg\equiv\partial Q_\bg^{\eq}/\partial V_n$=잔류 chemical capacitance \\
$Q_\bg^{\mathrm{lag}}$ & C & 별도 dynamic lag 로 분리할 수 있는 rate-dependent background. 본 장의 $C_\bg$ denominator에는 넣지 않는다 \\
$C_\bg$ & C/V & equilibrium background capacitance, $C_\bg=\partial Q_\bg^{\eq}/\partial V_n>0$ \\
...
$D,\ \eta_{\mathrm{ct}}$ & m$^2$/s,\ V & diffusion coefficient, charge-transfer overpotential. 본 장에서는 경쟁/퇴화 항으로만 등장하고 Ch.3 이후에서 kinetics model로 확장한다 \\
...
\AL{4} & bounded calibration & low-rate OCV/GITT & $U_j,w_j,Q_{j,\tot},Q_{\bg}^{\eq}$는 equilibrium-stage fit에서 먼저 제한한다. \\
...
\AL{8} & definition/math gate & dimensional constants and coordinates & $R,F,T,k_B,h,q_a,C_\bg$ 등 universal constants와 fitting coordinates는 처음 등장 전에 정의한다. \\
\AL{9} & bounded fitting gate & narrow-spectrum local tail approximation & single-mode $L$ extraction은 narrow-spectrum/local-window 근사일 때만 활성화 파라미터 추출에 쓴다. \\
\AL{10} & established & transition-state/Eyring theory \cite{eyring1935} & rate scale은 $\exp[-\Delta G^\ddagger/(RT)]$를 따른다. \\
\AL{11} & bounded empirical analogy & BEP/BV small-drive slope \cite{evanspolanyi1938,bardfaulkner} & Ch.1의 일반 $\chi_j$는 scalar relaxation mobility의 empirical first-order sensitivity로만 사용한다. \\
...
\AL{14} & established mechanism, bounded application & relaxation-time distributions \cite{svare2000,johnston2006,lindsey1980} & barrier distribution can create broad/stretched tails; graphite ICA application is model contribution. \\
...
\begin{equation}
\boxed{\;Q_\cell\,q \;=\; Q_\bg^{\eq}(V_n,T) \;+\; \sum_{j=1}^{N_p} Q_{j,\tot}\,\xi_j\;.}
...
\textbf{배경 용량의 의미.} $Q_\bg^{\eq}(V_n,T)$ 는 단순 그래프 기저선이 아니라 뚜렷한 staging 으로 분리하지 않은
\emph{equilibrium} 방전 방향 잔류 누적 용량이다. 따라서 그 전위 기울기
$C_\bg\equiv\partial Q_\bg^{\eq}/\partial V_n$ 은 equilibrium chemical capacitance 로만 쓴다. rate-dependent
background lag 가 따로 보이면 $Q_\bg^{\mathrm{lag}}$ 또는 추가 mode 로 분리하고, 아래 $C_\bg$ denominator 에 섞지 않는다.
섞으면 staging tail 이 만든 $\dd\Theta/\dd Q$ 와 background lag 를 같은 capacitance 안에서 다시 세어 double counting 이 된다.
$V_n$ 을 안정적으로 풀려면 이 equilibrium 기울기가 양이어야 한다(\AL{5}; bazant2013 의 비평형 열역학적 미분용량):
\begin{equation}
C_\bg(V_n,T)\equiv\frac{\partial Q_\bg^{\eq}}{\partial V_n}\ \ge\ \epsilon_Q\ >\ 0 .
\label{eq:monotone}
\end{equation}
...
식~\eqref{eq:charge_balance} 가 $Q_\cell q=Q_\bg^{\eq}+\sum_j Q_{j,\tot}\xi_{\eq,j}$ 가 되어 그 해가 평형 음극
...
용량의 치역 안에 있어야 한다(\AL{7}): $Q_\cell q-\sum_j Q_{j,\tot}\xi_j\in\mathrm{Range}[Q_\bg^{\eq}(\cdot,T)]$.
...
대칭 포물면의 활성화 장벽은
$\Delta G^\ddagger(\mathcal A_j)=\dfrac{(\lambda-\mathcal A_j)^2}{4\lambda}
...
$\mathcal A_j^2/(4\lambda)$ 은 $|\mathcal A_j|\ll\lambda$ 에서 무시된다. 따라서 \emph{대칭 Marcus 포물면 하나만}
근거로 삼으면 선형 계수는 정확히 $\chi_j=1/2$ 이다(\AL{15}). 본 장의 일반 $0\le\chi_j\le1$은 이
대칭 Marcus 유도에서 나온 값이 아니라, BEP/Butler--Volmer 류 선형 자유에너지 관계의 \emph{empirical slope}를
Level-A common-mode mobility sensitivity로 빌린 coarse-grained parameter다(\AL{11}, \cite{evanspolanyi1938,bardfaulkner}).
따라서 $\chi_j\ne1/2$ 를 쓰는 순간 본 장은 ``Marcus 대칭 포물면의 직접 유도''가 아니라 ``small-drive empirical
mobility slope'' 모델이다. 이 1차 항이 장벽을 낮추므로($\mathcal A_j>0$ 일 때 $-\chi_j\mathcal A_j<0$)
\begin{equation}
...
(forward/backward 동일 가속)은 microscopic Marcus/BV 방향 장벽을 그대로 복사한 것이 아니라, 세부 방향성을 Ch.3 로 넘긴 뒤
net relaxation mobility $k_j=r_j^++r_j^-$ 에만 전위 보조를 주는 \emph{reduced scalar model}이다. 이 reduced model 에서
세부 균형(detailed balance) 비는 열역학 target $\xi_{\eq,j}$ 가 보존한다. Level B(Ch.3)와 일치하는 것은
\emph{정상 target 극한} $\xi_{\mathrm{ss},j}\to\xi_{\eq,j}$ 한정이다(이것이 Ch.3 연결 조건이다). 본 장
결론식에서는 ``forward barrier lowering'' 같은 방향성 함의 용어를 피하고, 필요할 때 ``potential-assisted mobility''로 쓴다.
\end{keybox}
\begin{boundbox}
...
분포 $A_L$ 로 적분. 여기서 $L$-적분이 kernel 로만 묶이려면 두 조건이 필요하다:
\emph{(a)} 목표 $\Theta_\eq$ 는 mode barrier $g$ 에 직접 의존하지 않고 공통 internal potential $V_n$ 으로만 정해진다
(식~\eqref{eq:xi_dist} 의 $\xi_{\eq,j}$ 가 $g$-무관), \emph{(b)} mode 차이는 relaxation length $L$ 과 amplitude
$A_L(L)$ 로만 들어간다. 두 조건이 깨지면 아래 식은 reduced approximation 이고 Plan B의 직접 $g$-grid를 써야 한다.
조건 (a),(b) 아래에서는 목표 $\Theta_\eq$ 가 $L$-적분 밖으로 빠지고 적분은 kernel 에만 작용하므로,
$(1/L)e^{-(q-q')/L}\to\mathcal K(q,q')=\int
A_L(1/L)e^{-(q-q')/L}\dd L$, 초기항은 mode별 동차해의 합으로
$\Theta(0)e^{-q/L}\to\Theta_\mathrm{init}(q)=\int A_L(L)\Theta_{\mathrm{init},0}(L)e^{-q/L}\dd L$
...
Q_{j,\tot}\xi_j$($Q_p=\sum_j Q_{j,\tot}$=총 phase 용량), $C_\bg\equiv\partial Q_\bg^{\eq}/\partial V_n$(equilibrium background chemical
capacitance, 상세 \S\ref{sec:fiteq})로 묶은 $Q_\cell q=Q_\bg^{\eq}(V_n)+Q_p\Theta$ 를 $\Theta$ 로 미분해 얻고($C_\bg>0$), $\partial\Theta_\eq/\partial V_n>0$(평형 진행률은 전위에 증가, 식~\eqref{eq:dxidV} 의 $\dd\xi_\eq/\dd V_n>0$
...
\end{equation*}
이를 $\Theta(q)=c(q)+(\text{위 적분})$ 에 넣으면
\begin{equation*}
\Theta(q)\approx c(q)+\frac{\Theta(q)}{\Theta^{(0)}(q)}
J(q),\qquad
J(q)\equiv\int_0^q\mathcal K(q,q')\,b(q')\,\Theta^{(0)}(q')\,\dd q' .
\end{equation*}
이제 $\Theta(q)$ 항을 좌변으로 모으면
\begin{equation*}
\Theta(q)\left[1-\frac{J(q)}{\Theta^{(0)}(q)}\right]\approx c(q),
\end{equation*}
따라서 분모가 생긴다:
\begin{equation}
...
$Q_\cell=C_\bg\dfrac{\dd V_n}{\dd q}+\sum_j Q_{j,\tot}\dfrac{\dd\xi_j}{\dd q}$,
따라서
\begin{equation}
\frac{\dd V_n}{\dd q}=\frac{Q_\cell-\sum_j Q_{j,\tot}\,\dd\xi_j/\dd q}{C_\bg(V_n,T)}.
\label{eq:dVdq}
\end{equation}
...
Q_\cell\,\dd q$ 는 equilibrium 배경 저장 $\dd Q_\bg^{\eq}=C_\bg\,\dd V_n$($C_\bg\equiv\partial Q_\bg^{\eq}/\partial V_n$)과
phase progress $Q_p\,\dd\Theta$ 의 합이다:
\begin{equation}
\dd Q=C_\bg\,\dd V_n+Q_p\,\dd\Theta.
\label{eq:dQsplit}
\end{equation}
이는 식~\eqref{eq:dVdq} 양변에 $C_\bg=\partial Q_\bg^{\eq}/\partial V_n$ 을 곱하고 $\dd q=\dd Q/Q_\cell$ 로 다시 쓴
...
미분한 값이다. 분모
$1-Q_p\,(\dd\Theta/\dd Q)>0$ 은 단조성(식~\eqref{eq:monotone}, $C_\bg=\partial Q_\bg^{\eq}/\partial V_n>0$)과
...
\item 꼬리 중첩 $\dd\Theta_\mathrm{tail}/\dd q$(식~\eqref{eq:kernel_integral}); 자기참조는 \emph{본 장에서} 닫는다 —
  Plan B(분포 적분 기준식, 항상) 또는 Plan A 닫힌형(식~\eqref{eq:closed}, $\varepsilon$ 통과 시).
\item $\dd Q/\dd V_n$(식~\eqref{eq:fiteq}) $\to$ 측정축 $\dd Q/\dd V_{n,\app}$.
\end{enumerate}
\textbf{피팅 파라미터}: $\{U_j,w_j,Q_{j,\tot},Q_\bg^{\eq}\}$(평형, 저속 OCV/GITT), $\chi_j$(여러 $|I|$/전위, current prefactor 제거),
$\{\Delta H_a,\Delta S_a,k_0,\kappa(T)\}$(Eyring-corrected 여러 $T$), $\rho_G$(독립 입력 — 역산 금지). \textbf{유효범위}:
$G-\chi_j\mathcal A_j\ge0$(Marcus, \AL{15}), $V_{n,\drive}\approx U_j+O(w_j)$. 전체 spectrum$\cdot$자기참조를
쓰는 정밀 평가는 부록 \S\ref{sec:ch1_numeric} 의 적분 기준과 근사 승격 조건을 따른다.

\subsection{심플 실데이터 피팅 근사식 (narrow-spectrum single-mode; 본 장만으로 사용 가능)}\label{sec:ch1_simplefit}
전체 spectrum 적분(식~\eqref{eq:kernel_integral}) 없이, \emph{하나의 우세 mode} 가 꼬리를 지배하는 \textbf{narrow-spectrum}
극한($A_L(L)=\Theta_0\delta(L-L^\ast)$; amplitude-bearing single-mode)에서는 위 모든 단계가 \textbf{종이와 연필로 닫히는}
가장 단순한 근사식으로 줄어든다. 이것이 실데이터 1차 피팅의 출발점이다. 단, 이 절의 $L$-based Arrhenius extraction은
$\sigma_{\ln L}\simeq\sigma_G/(RT)\ll1$ 인 local window 에서만 물리 파라미터 추출식으로 쓴다(\AL{9}). 저온에서 꼬리가
stretched 로 휘는 바로 그 경우는 $\sigma_G/(RT)$ 가 커진 경우이므로, 아래 단일 $L$ 은 window-dependent effective descriptor일
뿐이고 $\Delta H_a$ 를 직접 뽑는 단일 활성화 길이가 아니다. post-peak($q>q_a$) 에서 목표가 거의 정지하면
(식~\eqref{eq:single_kernel}) 진행은
\begin{equation}
...
$\{U_j,w_j,Q_{j,\tot},Q_\bg^{\eq}\}$ 를, 펄스/전류 차단으로 $R_n$ 을 고정한다($R_n$ 과 $\chi_j$ 가 공선형이므로 $R_n$ 을
...
$V_{n,\drive}=V_n$(0단계 환산)를 가로축으로 삼는다. 그러나 $L=|I|/(Q_\cell k_j)$ 이므로 서로 다른 전류를 섞으면
$\ln L$ 안의 \emph{current prefactor} $\ln |I|$ 가 $\chi_j$ slope 와 공선형이 된다. 따라서 회귀량은
\begin{equation*}
Z_L\equiv \ln L-\ln |I|
=-\ln(Q_\cell k_0)+\frac{G-\chi_j\mathcal A_j}{RT}
\end{equation*}
로 둔다. 같은 $T$ 에서 $G,k_0$ 를 같은 local transition의 상수로 묶으면
$\partial Z_L/\partial V_{n,\drive}=-\chi_j s_{\phi,j}F/RT$ 이다. 또는 \emph{동일 $|I|$} 조건에서 전위만 sweep 하면
$\ln|I|$ 가 상수 intercept 로 흡수되어 같은 식을 쓸 수 있다. \textbf{비순환 보장}: $\mathcal A_j$ 는
각 데이터점에서 (0)단계 환산 $V_n$ 과 (0)단계 고정 $U_j(T)$ 로 계산되는 기지값이며, 이 단계에서는 $\Delta H_a$ 를 아직 쓰지 않는다.
따라서 $\chi_j$ 추출은 Arrhenius fit 과 독립인 shape/slope extraction 이지만, 반드시 current prefactor 를 제거하거나 고정해야 한다.
\textbf{(3) 유효 enthalpy(prefactor$\cdot U_j(T)\cdot\kappa(T)$ 보정).} 이제 (2)에서 얻은 $\chi_j$ 를 고정하고 온도별 $L(T)$ 를 해석한다.
$1/L=Q_\cell k_j/|I|$ 이고 $k_j=k_0(T)e^{-(G-\chi_j\mathcal A_j)/RT}$, $k_0=(k_BT/h)\kappa(T)$ 이므로
$\ln(1/L)$ 를 그대로 $1/T$ 로 회귀하면 current prefactor, $T$ prefactor, $\kappa(T)$, 그리고 $U_j(T)$ 를 통한
$\mathcal A_j(T)$ 가 모두 기울기에 섞인다. 따라서 Eyring 선형화는 전류와 $T$ prefactor를 같이 제거한
\begin{equation*}
y_\kappa(T)\equiv\ln\!\frac{|I|}{L\,T}-\frac{\chi_j\,\mathcal A_j(T)}{RT}-\ln\kappa(T)
\quad\text{를}\quad \frac1T\ \text{로 회귀} \;\Rightarrow\; \text{기울기}=-\frac{\Delta H_a}{R}
\end{equation*}
로 둔다. $\kappa(T)$ 를 독립적으로 모르면 $\ln\kappa(T)$ 는 잔차로 남는다. 이를 무시할 수 있는 조건은
$|\,\dd\ln\kappa/\dd(1/T)\,|\ll\Delta H_a/R$ 이며, 이 조건을 확인하지 않으면 slope 는 apparent activation enthalpy다.
부호가 \emph{minus} 인 이유는 $\ln(|I|/(LT))$ 안에 이미 $+\chi_j\mathcal A_j/(RT)$ 가 들어 있기 때문이다. 이 항을 더하면 전위 보조 효과를 두 번 더하는 오류가 된다.
\textbf{즉 narrow-spectrum 조건에서는 $\{L,\chi_j,\Delta H_a\}$ 가 단일 지수 꼬리 피팅 + 식별성 순서로 나온다}
(근사식~\eqref{eq:simplefit}, 식별성 \S\ref{sec:falsify}). 반대로 꼬리가 단일 지수에서 벗어나(stretched) 휘면 그 휨 자체가
$\rho_G$ spectrum 또는 자기참조 결합의 신호다. 그때 단일 $L(T)$ 로 $\Delta H_a$ 를 뽑아 이론을 판정하지 않고,
독립 입력 또는 bounded parameterization of $\rho_G$ 로부터 식~\eqref{eq:spectrum}--\eqref{eq:volterra} 를 forward prediction 하여
곡률과 tail shape 를 검정한다(반증, \S\ref{sec:falsify}).
...
평형 열역학 & $U_j(T),w_j(T),Q_\bg^{\eq}(V_n,T)$ & 전이 중심 전위$\cdot$평형 폭$\cdot$equilibrium 배경 용량의 온도 의존 \\
...
\item \textbf{Narrow-spectrum Arrhenius}: single-mode gate($\sigma_{\ln L}\ll1$) 안에서
  $y_\kappa(T)=\ln[|I|/(LT)]-\chi_j\mathcal A_j/(RT)-\ln\kappa(T)$ 가 $1/T$에 대해 직선이 아니면
  \emph{단일 활성화 길이} 그림을 기각한다. stretched tail 영역에서의 곡률은 곧바로 전체 이론 기각이 아니라
  spectrum 필요성의 신호다.
\item \textbf{전위 의존}: current prefactor를 제거한 $Z_L=\ln L-\ln|I|$ 또는 동일 $|I|$ sweep에서
  $\partial Z_L/\partial V_{n,\drive}$ 가 위 예측과 다르면 $\chi_j$ 해석 기각.
\item \textbf{비유일성 차단}: $\rho_G$ 를 꼬리에서 역산하지 않고 독립 입력으로만 — 역산하면 비유일이라
  반증력이 없다.
\item \textbf{비퇴화 — transport$\cdot$charge-transfer 와의 분리.} 꼬리 길이 scaling
  ...
  귀속하는 것은 금지한다.
\item \textbf{Spectrum forward prediction}: 저온 stretched tail은 단일 $L$ Arrhenius가 아니라 독립 입력 또는 bounded
  parameterization of $\rho_G$ 로부터 식~\eqref{eq:spectrum}--\eqref{eq:volterra} 가 예측하는 curvature/tail shape 로 검정한다.
  예측 곡률이 관측과 체계적으로 어긋나면 barrier-spectrum ansatz 또는 Plan A closure 를 기각한다.
\end{enumerate}
...
$\{U_j,w_j,Q_{j,\tot},Q_\bg^{\eq}\}$ 를 먼저 고정 — 동역학 꼬리가 사라진 평형 무대를 잡는다. \textbf{(2)} 펄스/전류
차단으로 $R_n$(전압축 분극)을 독립 제한 — $R_n$ 과 $\chi_j$ 는 둘 다 꼬리를 늘려 공선형이므로 $R_n$ 을 먼저
묶는다. \textbf{(3)} $Z_L=\ln L-\ln|I|$ 또는 동일 $|I|$ sweep의 $\partial Z_L/\partial V_{n,\drive}$ 로 $\chi_j$ 를 먼저 추출한다.
\textbf{(4)} 그 $\chi_j$ 로 $y_\kappa(T)=\ln[|I|/(LT)]-\chi_j\mathcal A_j/(RT)-\ln\kappa(T)$ 를 만든 뒤 narrow-spectrum 데이터에서
Eyring-corrected Arrhenius($y_\kappa(T)$ 대 $1/T$)로
$\{\Delta H_a,\Delta S_a,k_0\}$ 를 제한한다. \textbf{(5)} stretched tail은 $\rho_G$ 독립 입력 또는 bounded parameterization 기반 forward prediction으로만 검정한다(역산 금지, N4).
이 순서를 어기면 식별성이 무너져 \emph{어떤 적합도라도} 물리 해석이 불가능하다(과적합).

% ====================================================================
\section{부록: 자기참조식의 적분 평가 경계 (Plan B 기준식)}\label{sec:ch1_numeric}
% ====================================================================
전체 spectrum$\cdot$자기참조를 쓰는 정밀 평가는 Plan B(분포 적분 기준식)를 기준으로 삼는다. 단순 꼬리는 본문
\S\ref{sec:ch1_simplefit} 의 단일 지수 근사로 충분하나, stretched 꼬리(저온)나 강한 자기참조에서는 단일 $L$ 이
window-dependent descriptor 일 뿐이므로 분포 적분식이 기준식이다.
\textbf{기준식.} (i) 배리어 변수 $g$ 에 대해 관심 $T\cdot$전위에서 $L(g)$(식~\eqref{eq:L_of_G})이
관측 윈도를 덮는 영역을 잡는다. (ii) 각 $g$-mode 는
$\dd\xi_j(g)/\dd t=k_j(g)[\xi_{\eq,j}-\xi_j(g)]$ 를 따르며, 같은 시점의 $V_n$ 은 전하 보존
(식~\eqref{eq:charge_balance})의 대수 제약으로 정해진다. 이때
$\partial Q_\bg^{\eq}/\partial V_n=C_\bg\ge\epsilon_Q>0$(식~\eqref{eq:monotone})이 전하 보존식의 해를 국소적으로
유일하게 만든다. (iii) 관측 진행률은 $\xi_j(t)=\int \rho_G(g)\xi_j(g,t)\,\dd g$ 의 분포 평균으로 정의한다.
\textbf{근거 기반 tolerance(임의 상수 아님).} 전하 보존 잔차와 mode-분포 이산화 오차는 좌표 분해능, 측정 ICA
잡음 floor, 그리고 $\xi\in[0,1]$ 상대오차 scale 에서 정한다 — cap/clip 이 아닌 측정$\cdot$물리 scale 기준.
...
\textbf{분포 해상도 검증.} $\lVert\Theta_{N_g}-\Theta_{2N_g}\rVert/\lVert\Theta_{2N_g}\rVert\le\varepsilon_{\mathrm{disc}}$
(Plan A 승격에 쓰는 이산화 오차 기준 $\varepsilon_{\mathrm{disc}}$ 와 동일값)가 될 때까지 mode 해상도를 높인다 —
정성적 ``수렴할 때까지''가 아니라 위 $\varepsilon_{\mathrm{tol}}$ 체인과 닫힌 정량 종료조건이다. 코드 구현 자체는
본 이론 범위 밖이며, 여기서는 이론식$\cdot$검증 원리만 제시한다.
...
발열을, Ch.5 는 충방전 히스테리시스를 다룬다. 본 장은 심플 근사식(\S\ref{sec:ch1_simplefit})과
Plan B/A closure(\S\ref{sec:planB}, \S\ref{sec:planA}), 그리고 분포 적분 평가 경계(\S\ref{sec:ch1_numeric})까지
제시하여 Chapter 1의 이론 배경을 자기완결적으로 닫는다.
