...
\emph{단계 3 — 물리적 진폭 가중.} 단계 1$\cdot$2 는 ``각 길이에 mode 가 몇이나''(확률밀도)만 준다. 실제 꼬리 기여엔
mode 별 \emph{진폭}(입자 크기$\cdot$활성 site 밀도$\cdot$접근성)이 더 실리며 이를 $A_0(L)$ 로 둔다. 관측 spectrum 은
그 곱이다:
\begin{equation}
