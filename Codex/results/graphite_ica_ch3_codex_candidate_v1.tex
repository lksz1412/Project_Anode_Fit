...
\begin{verifybox}
\textbf{Level A $=$ Level B 의 detailed-balance 극한.} 식~\eqref{eq:ch3_xiss} 에
...
% ====================================================================
본 장에서 사용한 가정을 Ch3 그룹(AL-30$\sim$39)으로 정리한다. AL-30,31 은 기등재분
...
식~\eqref{eq:ch3_forward_limiter} 물리 병목). numerical implementation 은 본 장의 이론 범위 밖으로 둔다.
...
연결해야 한다. 즉 $\beta_j$ 와 $\chi_j$ 는 동일물이 아니다. \textbf{Level A=방향성 없는 공통 mobility 가속
(ratio 불변), Level B=방향성 있는 비대칭 split} 이라는 구분이 명확히 닫힌다.
...
일치는 \emph{정상 target} $\xi_{\sstat,j}\to\xi_{\eq,j}$ \emph{한정}이다(무조건 ``$A=B$'' 아님). 모든
