% Chapter 2: thermal extension of the graphite ICA tail theory.
...
\hypersetup{colorlinks=true, linkcolor=blue, urlcolor=blue, citecolor=blue,
  pdftitle={흑연 음극 ICA tail — Chapter 2}}
...
$n_j^{\eff}\ne1$ 의 비가역열 합은 Chapter 4 로 forward-ref 한다(\S\ref{sec:ch2_irr}; V5 convention).
\end{groundingbox}
...
% ====================================================================
본 장은 Chapter 1 V5의 다음을 \textbf{수정 없이} 입력으로 받는다. 각 식 번호는
Chapter 1 의 boxed 결과를 가리킨다.
\begin{linkbox}
\textbf{(L1) 전하 보존식(무대).} $Q_\cell\,q=Q_\bg^{\eq}(V_n,T)+\sum_{j=1}^{N_p}Q_{j,\tot}\,\xi_j$. 주어진
$(q,T,\{\xi_j\})$ 에서 이 식을 $V_n$ 에 대해 푼 것이 내부 전위이며, 평형 특수해($\xi_j=\xi_{\eq,j}$)가
$V_n=V_{n,\OCV}(q,T)$ 다. 단조성 floor $\partial Q_\bg^{\eq}/\partial V_n\ge\epsilon_Q>0$ 로 해가 유일.\\
...
방향성 transfer coefficient $\beta_j$ 는 Ch3 Level B에서 별도로 정의하며, 본 장은 $\beta_j=\chi_j$ 를 가정하지 않는다.\\
\textbf{(L6) 집계 진행률.} $Q_p\Theta\equiv\sum_jQ_{j,\tot}\xi_j$ ($\Theta\in[0,1]$ 무차원,
$Q_p=\sum_jQ_{j,\tot}$). 따라서 $Q_\cell q=Q_\bg^{\eq}+Q_p\Theta$.
\end{linkbox}
...
$P_{\rev,j}$ & V & reversible heat coefficient $=T\,\partial U_j/\partial T$ \\
...
$D_q$ & C/V & 평형 chemical capacitance $\partial Q_\bg^{\eq}/\partial V_n+\sum_jQ_{j,\tot}\partial\xi_{\eq,j}/\partial V_n$ \\
...
가역 열의 기초가 되는 전위 온도계수도 \emph{외부 lookup 이 아니라 전하 보존식에서 유도}해야 한다. 그래야 Chapter 1 의 ``OCV 는 보존식의 평형 특수해''라는 중심식 흐름이 열 계층에서도 유지된다. 평형
($\xi_j=\xi_{\eq,j}$)에서 식 (L1) 은
\begin{equation}
Q_\cell\,q=Q_\bg^{\eq}(V_n,T)+\sum_{j=1}^{N_p}Q_{j,\tot}\,\xi_{\eq,j}(V_n,T),\qquad V_n=V_{n,\OCV}(q,T).
\label{eq:ch2_eqbal}
\end{equation}
...
간접 의존(연쇄법칙)과 (나) $U_j(T),w_j(T)$ 를 통한 $V_n$ 고정 명시적 의존. $Q_\bg^{\eq}(V_n,T)$ 도 같은 두 경로다.
전미분을 한 줄씩 펼치면(이변수 함수의 전미분 $\dd f=\partial_{V_n}f\,\dd V_n+\partial_T f\,\dd T$, 양변을 $\dd T$
로 나누고 $q$ 고정)
\begin{equation}
0=\frac{\partial Q_\bg^{\eq}}{\partial V_n}\left(\frac{\partial V_{n,\OCV}}{\partial T}\right)_q+\frac{\partial Q_\bg^{\eq}}{\partial T}
...
\begin{equation}
0=\underbrace{\left[\frac{\partial Q_\bg^{\eq}}{\partial V_n}+\sum_{j}Q_{j,\tot}\frac{\partial\xi_{\eq,j}}{\partial V_n}\right]}_{\equiv\,D_q\ (\text{평형 chemical capacitance})}
\left(\frac{\partial V_{n,\OCV}}{\partial T}\right)_q
+\frac{\partial Q_\bg^{\eq}}{\partial T}+\sum_{j}Q_{j,\tot}\left(\frac{\partial\xi_{\eq,j}}{\partial T}\right)_{V_n}.
\label{eq:ch2_implicitT}
\end{equation}
$D_q$ 가 \emph{평형 chemical capacitance} 인 것은 단위가 $\mathrm{C/V}$ 이고($\partial Q_\bg^{\eq}/\partial V_n$ 의
단위 + $Q_{j,\tot}\cdot\partial\xi_{\eq,j}/\partial V_n$ 의 단위 $=\mathrm C/\mathrm V$), $V_n$ 변화에 대한
저장 전하의 응답을 재기 때문이다. $D_q>0$ 은 Chapter 1 의 단조성 floor 에서 보장된다: $\partial Q_\bg^{\eq}/\partial V_n\ge\epsilon_Q>0$
...
=-\,\frac{\dfrac{\partial Q_\bg^{\eq}}{\partial T}+\displaystyle\sum_{j}Q_{j,\tot}\left(\dfrac{\partial\xi_{\eq,j}}{\partial T}\right)_{V_n}}
{\dfrac{\partial Q_\bg^{\eq}}{\partial V_n}+\displaystyle\sum_{j}Q_{j,\tot}\dfrac{\partial\xi_{\eq,j}}{\partial V_n}}\;.}
\label{eq:ch2_dVocvdT}
\end{equation}
\textbf{차원 검산.} 분자 단위 $=\partial Q_\bg^{\eq}/\partial T$ 의 $\mathrm{C/K}$, 분모 $=D_q$ 의 $\mathrm{C/V}$,
따라서 전체 $=(\mathrm{C/K})/(\mathrm{C/V})=\mathrm{V/K}$ — 전위 온도계수의 올바른 단위다. \textbf{이 식은 평형
상태함수의 온도계수이며 apparent 전위 경로 미분이 아니다}: 즉 $(\partial V_{n,\OCV}/\partial T)_q\ne
...
$(\partial V_{n,\OCV}/\partial T)_q$ 가 $Q_\bg^{\eq}$, $U_j$, $w_j$, $\xi_{\eq,j}$ 의 온도 의존이 결합된 결과로
...
\begin{equation}
\left(\frac{\partial V_{n,\OCV}}{\partial T}\right)_q
=-\frac{\dfrac{\partial Q_\bg^{\eq}}{\partial T}+\displaystyle\sum_j\!\left[Q_{j,\tot}\!\left(\dfrac{\partial\xi_{\eq,j}}{\partial T}\right)_{V_n}\!\!+\dfrac{\partial Q_{j,\tot}}{\partial T}\,\xi_{\eq,j}\right]}{D_q}
...
$\xi_{\mathrm{ss},j}\to\xi_{\eq,j}$''에서 Level B(Ch3)와 일치한다는 한정은 V5 convention 와 같다
...
=I_j\,P_{\rev,j},\qquad P_{\rev,j}\equiv T\,\frac{\partial U_j}{\partial T}.\;}
\label{eq:ch2_qrev}
\end{equation}
\textbf{차원 검산.} $I_j\,T\,(\partial U_j/\partial T)=\mathrm A\cdot\mathrm K\cdot(\mathrm{V/K})=\mathrm{A\,V}=\mathrm W$
— 열률의 올바른 단위. $P_{\rev,j}=T\,\partial U_j/\partial T$ 가 reversible heat coefficient[V]이며 $q_{\rev,j}=I_jP_{\rev,j}$ 는
전류$\times$entropy-coefficient 전위 형이다. \textbf{부호.} $I_j$ 가 부호 있는 전류이므로 $q_{\rev,j}$ 는 부호가변이다:
...
뒤 고정하는 bookkeeping factor 다(피팅 X; V5 convention 의 $q_\rev$ 부호 convention). 여기서
...
double counting 이다(double-counting check). 물리 우선 원칙은 둘 중 하나를 선택하거나, 동시에 쓸 때 \textbf{정합 제약}을
부과하는 것이다:
\begin{enumerate}[label=\Alph*),leftmargin=2.0em]
\item 전체 평형 저장 반응을 $(\partial V_{n,\OCV}/\partial T)_q$ 하나로 대표(식~\eqref{eq:ch2_revOCV}).
\item $Q_\bg^{\eq}$ 와 각 $\xi_j$ transition 에 별도 entropy coefficient($h_\bg,\Delta S_j$)를 부여하되, 이들이
...
=s_\bg^{\conv}\,T\,h_\bg\,\dot Q_\bg^{\eq,\prog}
...
% ====================================================================
$Q_\bg^{\eq}$ 는 잔류 background capacity 저장소이지 entropy 자체가 아니다. background 영역의 reversible heat 를
...
\begin{equation}
\dot{\mathcal Q}_{\rev,\bg}^{\cand}=s_\bg^{\conv}\,T\,h_\bg(V_n,T)\,\dot Q_\bg^{\eq,\prog},
\label{eq:ch2_revbg}
\end{equation}
$\dot Q_\bg^{\eq,\prog}$ 는 실제 background 저장/방출 \emph{진행} 율(단위 C/s)이다.

\textbf{total derivative 와 progress 의 구분(무비약, 중요).} 전하 보존식 (L1) 을 시간 미분한다. $Q_\bg^{\eq}$ 는
$(V_n,T)$ 의 함수이므로 전미분 연쇄법칙으로
\begin{equation}
Q_\cell\frac{\dd q}{\dd t}=\frac{\partial Q_\bg^{\eq}}{\partial V_n}\frac{\dd V_n}{\dd t}+\frac{\partial Q_\bg^{\eq}}{\partial T}\frac{\dd T}{\dd t}
+\sum_jQ_{j,\tot}\frac{\dd\xi_j}{\dd t}.
\label{eq:ch2_dQdt}
\end{equation}
비등온 조건에서 $\dd Q_\bg^{\eq}/\dd t$ 전체를 곧바로 reaction progress 로 해석하면 안 된다 — 온도 변화에 의한 항이
섞이기 때문이다. 개념적으로 분리하면
\begin{equation}
\dot Q_\bg^{\eq,\mathrm{tot}}=\dot Q_\bg^{\eq,\prog}+\dot Q_\bg^{\eq,\coord},\qquad
\dot Q_\bg^{\eq,\prog}=\frac{\partial Q_\bg^{\eq}}{\partial V_n}\frac{\dd V_n}{\dd t},\qquad
\dot Q_\bg^{\eq,\coord}=\frac{\partial Q_\bg^{\eq}}{\partial T}\frac{\dd T}{\dd t},
\label{eq:ch2_bgsplit}
\end{equation}
여기서 $\dot Q_\bg^{\eq,\prog}$(전위 구동 progress)만 heat-generating 반응 진행으로 식~\eqref{eq:ch2_revbg} 에
들어가고, $\dot Q_\bg^{\eq,\coord}$(온도 변화에 따른 capacity-coordinate shift)는 그대로 reaction rate 로 넣지
...
를 통한 $\xi_{\eq,j}(V_{n,\drive})$(\emph{구동} 전위 기준)이다. 두 평형이 같은 집합이 되는 것은 V5 convention
...
\subsection{전류$\times$과전압 축약형 $q_\irr=|I|\eta$ (V5 convention 부호 양정치)}\label{sec:ch2_Ieta}
...
(V5 convention: $q_\irr=|I|\eta$ 형, ``$I\eta$ 단독'' 금지 — 단일 transition 항별 부호는 BOUNDED 이나
...
\begin{boundbox}
\textbf{본 장은 $n^{\eff}=1$ 특수형(\AL{20}, V5 convention).} 식~\eqref{eq:ch2_qirr} 의 $q_\irr$ 합은
...
\begin{equation}
Q_\cell\,q_{\mathrm{rest}}=Q_\bg^{\eq}(V_n(t),T)+\sum_jQ_{j,\tot}\xi_j(t).
...
\begin{equation}
Q_\bg^{\eq}(V_n,T),\ \xi_{\eq,j}(V_n,T),\ U_j(T),\ w_j(T),\ \rho_G(g;T),
\label{eq:ch2_feedT1}
\end{equation}
\begin{equation}
...
\label{eq:ch2_feedT2}
\end{equation}
따라서 비등온 계산에서 $V_n,\xi_j,T$ 를 독립 후처리하지 않고 같은 시간 적분 루프에서 갱신한다(순환 의존성). 계산 순서:
...
\end{enumerate}
(수치 적분기$\cdot$root-find 구현과 수렴 판정은 본 이론 장 범위 밖인 후속 구현 문제다; GS-4 의
이론식/피팅 실무 분리.)
\begin{boundbox}
...
% ====================================================================
\section{비가역열 tail = ICA tail 의 열적 거울 (consistency 가설)}\label{sec:ch2_thermal_tail}
% ====================================================================
...
$Q_\bg^{\eq}$ 와 $h_\bg$ & 배경 용량과 background entropy coefficient 를 독립 자료 없이 분리 곤란 \\
...
% ====================================================================
본 장에서 사용한 가정을 통합 ledger(Chapter 2 assumption ledger) Ch2 그룹(AL-20$\sim$29)으로 정리한다. AL-20,21 은 기등재분
...
AL-29 & near-eq entropy production $\sigma_j=(g''_j/T)k_j r_j^2\ge0$ ($g''_j>0$ 안정성); 비가역열 tail = ICA tail 열적 거울 (per-mode arc-length doubling; spectrum 재척도) & BOUNDED($\sigma$) / FLAGGED(열적 tail novel) & degrootmazur1962, onsager1931, bazant2013 (10.1103/PhysRev.37.405; 10.1021/ar300145c) & \eqref{eq:ch2_affinity},\eqref{eq:ch2_sigma},\eqref{eq:ch2_thermal_kernel},\eqref{eq:ch2_relax} \\
\bottomrule
\end{longtable}
}
...
임의 clip/cap/softplus/step 정의식 0(GS-4). validation 구현 절차는 본 장의 이론 범위 밖으로 둔다.
...
\item 온도 되먹임을 실제 계산 절차로 구현하는 일은 별도 실무 문서의 대상이며, 본 장은 그 물리적 연결식만 제공한다.
\end{enumerate}
...
=I_jP_{\rev,j}$ (식~\eqref{eq:ch2_qrev},\eqref{eq:ch2_revOCV})로, 부호가변(entropic)이다. transition entropy basis
...
(식~\eqref{eq:ch2_qirr}; V5 convention 부호 양정치), 또는 물리적으로 제한된 $I^2R_{n,\eff}$ 다. 공액 force 는 국소
...
본 장이 확정한 열 계층($q_\rev=I_jP_{\rev,j}$, $q_\irr=|I|\eta$, $\sigma_j\ge0$)과 상태변수($V_n,\xi_j,k_j,U_j,
\eta_j$) 위에서: Chapter 3 은 Chapter 1 의 mobility 가속 $k_j$ 를 forward/backward 반응속도론(Level B)으로 확대한다.
이때 Ch3 의 transfer coefficient $\beta_j$ 는 새 kinetic parameter 이며 Ch1 의 $\chi_j$ 와 동일하다고 두지 않는다.
그 구조 위에서 본 장이 후보로 남긴 휴지 relaxation heat 의 가역/비가역 \emph{정량} 분해와
