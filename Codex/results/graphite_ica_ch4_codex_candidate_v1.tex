...
\newcommand{\brk}{\allowbreak}% 표/참고문헌 긴 DOI/ISBN 줄바꿈 허용
...
$V_{n,\OCV}$(평형 보존식 해).\\
\textbf{(L6) Ch2 가역열 경계.} $q_{\rev,j}=I_j\,T\,\partial U_j/\partial T=I_jP_{\rev,j}$ ($P_{\rev,j}=T\partial U_j/\partial T$,
...
log 발산은 물리적 병목과 definition-domain guard 로 둔다.
...
% ====================================================================
\section{Dissipation force 기준 전위 단일화}\label{sec:ch4_eta_single}
% ====================================================================
...
entropy-production / 평형 entropy / dissipative-loss 기원 분해; \cite{bernardi1985,rao1997,thomasnewman2003}, \AL{46}):
\begin{equation}
...
그 합의 비음성을 보장한다(\cite{degrootmazur1962,onsager1931,prigogine1967,bazant2013}, \AL{40}):
\begin{equation}
...
\subsection{Network form of transition entropy production}\label{sec:ch4_trep}
...
\AL{41},\AL{43}). 이 ``$(\text{net flux})\times\ln(\text{flux ratio})$'' 구조가 master equation 의 국소 detailed
...
RT[\mathrm{J/mol}]\times(\mathcal J_j^+-\mathcal J_j^-)[\mathrm{s^{-1}}]\times\ln(\cdot)[\text{dimensionless}]
...
\eta_j=s_{\phi,j}\big[V_{n,\drive}-V_{\eq,j}(\xi_j)\big]\ \ (s_{\phi,j}=+1\ \text{discharge}).}
...
$\cdot$Ch4(entropy production)가 한 표현 $n_j^{\eff}I_j\eta_j$ 로 닫힌다}.
...
\textbf{유도가 쓴 전제의 명시.} 위 verifybox 는 (i) detailed balance(Ch3, \emph{내부}
...
박아야 한다. 또 (iv) Chapter 3 의 keystone 한정, 즉 Level B detailed-balance 극한에서 Level A 의 stationary
target $\xi_{\eq,j}$ 가 회복되는 조건($\xi_{\sstat,j}\to\xi_{\eq,j}$ 가 $C_j$ 의 $V_n$$\cdot$$\xi_j$-무의존
partition 에서만 성립)을 상속한다. 강구동 branch 비대칭(non-detailed-balance, Chapter 5)에서는
$\mathcal A_j^{\chem}$ 가 일반화된다.
...
아니라 \emph{definition-domain guard} 다(GS-4 의 cap 금지와 구분: 물리 모델의 임계값이 아니라 계산 정의역 보호).
...
% ====================================================================
\section{비가역열 일반형과 일반/특수 위계}\label{sec:ch4_general}
% ====================================================================
...
일반/특수 위계}다:
\begin{equation}
\underbrace{\dot{\mathcal Q}_{\irr,n}=\sum_j n_j^{\eff}I_j\eta_j}_{\text{Ch4 general}}
\ \xrightarrow{\ n_j^{\eff}=RT/(Fw_j)\to1\ (w_j=RT/F,\ \text{ideal})\ }\
\underbrace{\sum_j I_j\eta_j}_{\text{Ch2 special}}.
...
고정, \AL{20})\textemdash 외부 lookup 이 아니라 \emph{전하 보존식의 평형 특수해 미분}이다:
\begin{equation}
\left(\frac{\partial V_{n,\OCV}}{\partial T}\right)_q
=-\,\frac{\dfrac{\partial Q_\bg^{\eq}}{\partial T}+\displaystyle\sum_j Q_{j,\tot}\left(\dfrac{\partial\xi_{\eq,j}}{\partial T}\right)_{V_n}}
{\dfrac{\partial Q_\bg^{\eq}}{\partial V_n}+\displaystyle\sum_j Q_{j,\tot}\dfrac{\partial\xi_{\eq,j}}{\partial V_n}}.
...
\textbf{부호가변성.} $q_{\rev,j}=I_jP_{\rev,j}$ ($P_{\rev,j}=T\partial U_j/\partial T$, 식 (L6))는 $\partial U_j/\partial T$
...
=I_j\,T\,\frac{\partial U_j}{\partial T}=I_j\,P_{\rev,j},
\label{eq:ch4_qrev_micro}
\end{equation}
($s_{\phi,j}F$ 와 $1/(s_{\phi,j}F)$ 상쇄). 즉 \textbf{미시 reaction entropy $\Delta S_{\rxn,j}$ 의 합}
$\dot{\mathcal Q}_{\rev}=\sum_j I_jP_{\rev,j}=\sum_j I_j T\partial U_j/\partial T$ 가 거시 Bernardi 형
...
% ====================================================================
$Q_\bg^{\eq}$ 는 배경 누적 용량이지 entropy 자체가 아니다. background reversible heat 를 쓰려면 별도 background
entropy coefficient 가 필요하다(\AL{26}):
\begin{equation}
h_\bg(V_n,T)\equiv\left(\frac{\partial V_{\bg,\eq}}{\partial T}\right)_{\mathrm{bg}},\qquad
\dot{\mathcal Q}_{\rev,\bg}=s_\bg^{\conv}\,T\,h_\bg(V_n,T)\,\dot Q_\bg^{\eq,\prog}.
\label{eq:ch4_qrev_bg}
\end{equation}
\textbf{total derivative 와 progress 의 구분(무비약, 중요).} 전하 보존식 (L1) 을 시간 미분하면 $Q_\bg^{\eq}$ 가
$(V_n,T)$ 의 함수이므로 전미분 연쇄법칙으로 $\dot Q_\bg^{\eq,\mathrm{tot}}=(\partial Q_\bg^{\eq}/\partial V_n)\dd V_n/\dd t
+(\partial Q_\bg^{\eq}/\partial T)\dd T/\dd t$ 다. 이를 progress 와 coordinate shift 로 나눈다:
\begin{equation}
...
\end{equation}
여기서 \emph{$\dot Q_\bg^{\eq,\prog}$(전위 구동 progress)만} heat-generating 반응 진행으로 식~\eqref{eq:ch4_qrev_bg}
에 들어가고, $\dot Q_\bg^{\eq,\coord}$(온도 변화에 따른 capacity-coordinate shift)는 그대로 reaction rate 로 넣지
않는다 -- 이 구분을 빠뜨리면 단순 온도 drift 가 가역 발열로 오계상된다(Chapter 2 의 bgsplit 계승). 등온 정전류
구간서는 실용적으로 $\dot Q_\bg^{\eq,\prog}\approx|I|-\sum_j Q_{j,\tot}(\dd\xi_j/\dd t)$ 로 쓸 수 있으나(전류 보존,
Chapter 3 식), \emph{비등온서는 $\dot Q_\bg^{\eq,\coord}$ 항 때문에 이 근사를 그대로 쓰지 않는다}.
\begin{equation}
\dot Q_\bg^{\eq,\prog}\ \approx\ |I|-\sum_j Q_{j,\tot}\frac{\dd\xi_j}{\dd t}\qquad(\text{isothermal galvanostatic limit}).
...
flux$\times$force 가 더 기본이다(\cite{newman,bardfaulkner}, \AL{47}). 선형 근사:
\begin{equation}
\dot{\mathcal Q}_{\transp,n}\approx I^2 R_{\transp,n}\qquad(\text{linear near-equilibrium}),
...
\textbf{차원 검산.} $I^2R=\mathrm{A^2}\cdot\Omega=\mathrm{A^2}\cdot\mathrm{V/A}=\mathrm{A\,V}=\mathrm W$; flux$\times$force
...
\label{eq:ch4_Rsep}
\end{equation}
첫째, $R_n$ 은 apparent voltage-axis shift 를 나타내는 계수다. 둘째,
$R_{\ct,n}=RT/(FA_\eff j_{0,n})$ 는 Chapter 3 의 charge-transfer small-signal resistance 다.
셋째, $R_{n,\eff}$ 는 $\dot{\mathcal Q}_\irr=I^2R_{n,\eff}$ 꼴로 열 생성을 표현할 때의
heat-generation effective resistance 이며 선형 응답에서만 쓴다. $R_{n,\eff}$ 는 독립적인 물리 제약이나
calorimetry 제약이 있을 때만 허용하고, $R_n$, $R_{\ct,n}$, $R_{n,\eff}$ 를 동시에 자유 피팅하지 않는다
(BOUNDED; Chapter 3 의 식별성 chain).
\end{boundbox}
...
\begin{equation}
\dot{\mathcal Q}_{\irr,\xi}^{\rlx}=\sum_j n_j^{\eff}\,I_j\,\eta_j\ \ge\ 0\qquad(\text{external }|I|=0,\ I_j=Q_{j,\tot}\dot\xi_j\ne0),
...
% ====================================================================
온도는 Chapter 1--3 의 다음 항들에 되먹임된다: $Q_\bg^{\eq}(V_n,T)$, $\xi_{\eq,j}(V_n,T)$, $U_j(T)$, $w_j(T)$,
$r_j^\pm(\cdot,T)$, $k_j^{\mathrm{phys}}(\cdot,T)$, $R_n(q,T,|I|)$, $\rho_j(g;T)$, $V_{n,\OCV}(q,T)$. 따라서
비등온 발열 계산에서 $V_n,\xi_j,T$ 를 독립 후처리하지 않고 같은 시간 적분 루프에서 갱신한다.
계산 순서:
\begin{enumerate}[label=\arabic*),leftmargin=2.0em]
\item 외부 전류로 $q(t)$ 갱신: $\dd q/\dd t=|I|/Q_\cell$.
\item 현재 $q,T,\{\xi_j\}$ 에서 전하 보존식 (L1) 이 정하는 implicit state $V_n$ 를 결정.
...
\item 비가역 열: transition entropy production 의 거시형(식~\eqref{eq:ch4_qirr_general})과
  transport flux$\times$force(식~\eqref{eq:ch4_transport_ff})를 계산.
...
\item background heat 사용 시 $\dot Q_\bg^{\eq,\prog}$ 와 $\dot Q_\bg^{\eq,\coord}$ 분리(식~\eqref{eq:ch4_qbg_split}).
...
\end{enumerate}
이 순서는 이론식의 의존 관계를 보이기 위한 것이며, 실제 수치 구현 절차나 수렴 판정은 본 장의 범위에 넣지 않는다.
\begin{boundbox}
...
% ====================================================================
\section{Chapter 4 의 가정 원장 (AL-40--49)}\label{sec:ch4_al}
% ====================================================================
본 장에서 사용한 가정을 통합 Assumption Ledger 의 Ch4 그룹(AL-40$\sim$49)으로 정리한다. tier 는
GROUNDED/BOUNDED/FLAGGED 로 둔다.
{\footnotesize
...
AL-43 & transition chemical affinity $\mathcal A_j^{\chem}=RT\ln(\mathcal J_j^+/\mathcal J_j^-)$; 부호 보조정리 $(\mathcal J^+{-}\mathcal J^-)\ln(\mathcal J^+/\mathcal J^-)\ge0$ (두 양수 차와 로그비의 곱) & GROUNDED & schnakenberg1976, prigogine1967(book) (DOI 10.1103/\brk RevModPhys.48.571) & \eqref{eq:ch4_tr_entropy},\allowbreak\eqref{eq:ch4_affinity_raw} \\
AL-44 & 미시$=$거시: DB(AL-30)$+n_j^\eff{=}RT/(Fw_j)$(AL-34) $\Rightarrow\mathcal A_j^{\chem}{=}n_j^\eff F\eta_j$, $\dot{\mathcal Q}_{\irr,j}^{\tr}{=}n_j^\eff I_j\eta_j$ ($\eta_j{=}V_{n,\drive}{-}V_{\eq,j}$, 기본형 $V_{n,\drive}{=}V_n$ 전제; $s_{\phi,j}$ 부호 규약 명시) & GROUNDED & schnakenberg1976, bazant2013, degrootmazur1962(book) (DOI 10.1103/\brk RevModPhys.48.571; 10.1021/\brk ar300145c) & \eqref{eq:ch4_affinity_eta},\allowbreak\eqref{eq:ch4_micro_macro} \\
AL-45 & 비가역열 일반형 $\dot{\mathcal Q}_\irr{=}\sum_j n_j^\eff I_j\eta_j\ge0$ (2법칙); Ch2 $\sum_j I_j\eta_j$ 는 $n^\eff{=}1$ ideal-width 특수해 & GROUNDED & degrootmazur1962(book), onsager1931, schnakenberg1976 (DOI 10.1103/\brk PhysRev.37.405; 10.1103/\brk RevModPhys.48.571) & \eqref{eq:ch4_qirr_general},\allowbreak\eqref{eq:ch4_hierarchy} \\
AL-46 & Ch2 3-분해에서 Ch4 4-분해로 정련: $\dot{\mathcal Q}_{\irr}^{\mathrm{Ch2}}\supset\dot{\mathcal Q}_{\irr}^{\mathrm{Ch4}}+\dot{\mathcal Q}_{\transp}$ (계면반응 EP 와 transport dissipation 분리; 이중계상 금지) & BOUNDED & bernardi1985, rao1997 (DOI 10.1149/\brk 1.2113792; 10.1149/\brk 1.1837884) & \eqref{eq:ch4_decomp},\allowbreak\eqref{eq:ch4_ch2refine} \\
...
AL-49 & log singularity($\mathcal J^\pm{\to}0$)는 coarse-graining resolution positive floor (definition-domain guard, heat fitting 아님); $n_j^\eff I_j\eta_j$ 형은 평형서 $I_j{\to}0$ 로 매끄럽게 0 (발산 없음) & BOUNDED & schnakenberg1976 (DOI 10.1103/\brk RevModPhys.48.571) & \eqref{eq:ch4_tr_entropy} boundbox \\
\bottomrule
\end{longtable}
}
...
수치 구현식 0. \textbf{DOI 정확 귀속:} degrootmazur1962$\cdot$
...
을 전제로 닫히며, $s_{\phi,j}$ 부호 인자가 명시된다.
...
이며, \emph{Chapter 2 의 $\sum_j I_j\eta_j$ 는 그 ideal-width($n_j^{\eff}=1$) 특수해}다(일반/
...
hysteresis heat 와 polarization heat 를 분리한다.
